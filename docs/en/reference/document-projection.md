...
A Harness projection is a readable view of work that already exists in canonical state or artifact storage. The future projector reads `state.sqlite` records, `state.sqlite.task_events`, and registered artifact references, then renders a compact status card or, in later profiles, Markdown such as `TASK`, `APR`, `RUN-SUMMARY`, `EVIDENCE-MANIFEST`, `EVAL`, `DIRECT-RESULT`, and other report projections.
...
| Audience | Shape | Rule |
|---|---|---|
| User-facing compact card | Short status card with ordinary language and refs | MVP-1 User Work Loop projection. Display-only; derived from Core state and refs. |
...
User judgment displays must name the judgment type instead of flattening everything into "Judgment" or "Approval." Render Product/UX judgment, technical architecture judgment, security/privacy judgment, scope/autonomy judgment, sensitive-action approval, QA/verification waiver, work acceptance, and residual-risk acceptance as distinct labels when those routes are pending. If multiple judgments are pending, render separate lines or cards. Residual-risk acceptance displays must name the risk being accepted.
...
| Task continuity | `state.sqlite` Task, Change Unit, Run, Evidence Manifest, Eval, Manual QA, Decision Packet, Approval, Residual Risk, `task_gates.acceptance_gate`, acceptance Decision Packet state, close events, artifact refs, `task_spine_entries` / public `journey_spine_entry` records when needed, and `state.sqlite.task_events` | `TASK` Journey Spine | Core transition or reconcile, Journey reconstruction, then projector |
| Decision Packet | `state.sqlite.decision_packets` including `judgment_category`, `judgment_route`, `display_depth`, related `decision_gate` state, judgment events, related approval or reconcile records, artifact refs, and linked `state.sqlite.residual_risks` when applicable | Compact status card judgment line, status/next responses, judgment-context resources, decision-packet resources, or a dedicated prompt; optional `TASK`, Journey Card, or `DEC` display when a later/standalone projection is enabled | `request_user_judgment` / `record_user_judgment`, then projector |
| Journey Spine | `state.sqlite` Task, Change Unit, Run, Decision Packet, Approval, Evidence Manifest, Eval, Manual QA, Residual Risk, `task_gates.acceptance_gate`, acceptance Decision Packet state, close events, artifact refs, `task_spine_entries` / public `journey_spine_entry` records when needed, and `state.sqlite.task_events` | `TASK` Journey Spine section, resume views, Journey Spine-oriented cards | Core transition or reconcile, Journey reconstruction, then projector |
...
| Sensitive-action approval | Minimum MVP-1: `state.sqlite.decision_packets` with `judgment_route=approve-sensitive-action`, `judgment_payload.approval_scope`, and judgment events. Later Approval profile: `approvals`, linked approval-shaped Decision Packet, optional decision request routing/replay record if implementation keeps one, and events. Never `approval_request_candidate` alone | Minimum MVP-1: compact status card or Decision Packet prompt/display. Later Approval profile: `APR` projection and approval card | Minimum MVP-1: `request_user_judgment(judgment_route=approve-sensitive-action)` creates the Decision Packet and `record_user_judgment` resolves it, then projector/card if enabled. Later Approval profile may also create/update the Approval record, then projector |
...
Decision Packet projections and cards must show the concrete judgment type. Render canonical schema fields separately from friendly labels: `judgment_category` is the user-facing grouping, `judgment_route` is the owner path and recorded-answer route, and `display_depth` is the prompt depth. Templates may translate enum values into friendly labels, but those labels remain display-only. The route and depth determine whether the display is a concise choice, product/UX trade-off, architecture trade-off, approval-shaped prompt, waiver, work acceptance, residual-risk acceptance, reconcile, or mixed judgment. Sensitive-action approval must not look like work acceptance, and work acceptance must not look like residual-risk acceptance. Affected gates or blocked actions come from the packet's `affected_gates` and related owner records, not from the category or display-depth label. `display_depth` is not a `ProjectionKind`, gate, close aggregation rule, approval substitute, waiver substitute, work-acceptance substitute, residual-risk acceptance rule, or authority path. `judgment_category` is schema-validated as an enum, but it is not a payload branch selector, gate recompute input, close aggregation rule, approval substitute, waiver substitute, residual-risk acceptance rule, authority path, or replacement for `judgment_route`. Friendly rendered labels derived from these fields are not schema fields, validator inputs, gates, owner records, or authority paths.
...
- Decision Packet: `state.sqlite.decision_packets` including `judgment_category`, `judgment_route`, `display_depth`, and related refs -> compact status card judgment line, status/next responses, judgment-context resources, decision-packet resources, or a dedicated prompt; optional `TASK`, Journey Card, or `DEC` projection when a later/standalone projection is enabled
...
- Sensitive-action approval: minimum MVP-1 uses the approval-shaped Decision Packet -> compact status card or Decision Packet prompt/display; later Approval profiles use `approvals` plus the approval-shaped Decision Packet -> `APR` projection only after the Approval record exists or changes. An `approval_request_candidate` from `prepare_write` may appear as candidate display, but it is not an `APR` source
...
| MVP-1 User Work Loop projection | [Compact Status Card](templates/compact-status-card.md) | The MVP-1 projection shape is one compact card showing what is happening, scope/non-goals, pending user judgments, known evidence summaries, evidence gaps, close blockers, visible residual risk, next safe action, and source/freshness refs. Work-acceptance and residual-risk facts remain distinct when relevant, but they do not add required projection kinds. |
...
No template class makes a projection canonical state or creates evidence, QA, verification, work acceptance, residual-risk acceptance, close authority, or Write Authorization. If a source record does not exist, the projector must not invent placeholder state to satisfy template completeness.
...
Persisted `JOURNEY-CARD` Markdown, Journey Spine output, `TASK` continuity reports, Direct Result reports, Run Summary, detailed Evidence Manifest, detailed Eval, full Manual QA card, TDD Trace, Domain Language, Module Map, Interface Contract, Design, Export bundles, full Approval Card, and other polished reports are not mandatory early scope. Current-position context for status, next, and resume is satisfied by core status output, the MVP-1 compact status card, or agent compact context/reference payloads.

Decision Packet visibility required by the active stage/profile is provided through status/next responses, judgment-context resources, decision-packet resources, the compact status card, or a dedicated user judgment prompt. That requirement is for the Decision Packet user judgment request display shape, not the standalone `DEC` `ProjectionKind`. Standalone `DEC` Markdown is optional unless the standalone Decision Packet projection feature is enabled.
...
| `APR` | later Approval profile creates or updates a committed Approval record through `request_user_judgment(judgment_route=approve-sensitive-action)` or `record_user_judgment` | linked Approval record status, scope, baseline, expiry, related approval-shaped Decision Packet, or judgment note changes |
