...
This is intentionally small. It shows the v0.2 shape without implying that a full template renderer, generated reports, or `TASK` Markdown exists.
...
| Evidence coverage | Minimum v0.2: evidence summary records/refs, Run refs, ArtifactRefs, and visible gap summaries. Full Evidence Manifest profile: `evidence_manifests` plus artifact refs | Compact status card evidence summary/gap display in v0.2; `EVIDENCE-MANIFEST` projection only when the full evidence profile is active | `record_run` evidence updates in v0.2; evidence module update when the full Evidence Manifest profile is active, then projector |
...
- Sensitive-action approval: minimum v0.2 uses the approval-shaped Decision Packet -> compact status card or Decision Packet prompt/display; later Approval profiles use `approvals` plus the approval-shaped Decision Packet -> `APR` projection only after the Approval record exists or changes. An `approval_request_candidate` from `prepare_write` may appear as candidate display, but it is not an `APR` source
...
Persisted `JOURNEY-CARD` Markdown, Journey Spine output, `TASK` continuity reports, Direct Result reports, Run Summary, detailed Evidence Manifest, detailed Eval, full Manual QA card, TDD Trace, Domain Language, Module Map, Interface Contract, Design, Export bundles, full Approval Card, and other polished reports are not mandatory early scope. Current-position context for status, next, and resume is satisfied by core status output, the v0.2 compact status card, or agent compact context/reference payloads.
