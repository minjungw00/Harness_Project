...
| Approval | `approvals`, approval-shaped Decision Packet, optional decision request routing/replay record if implementation keeps one, and events; never `approval_request_candidate` alone | `APR` projection and approval card | `request_user_judgment(judgment_route=approve-sensitive-action)` creates the pending Approval record, `record_user_judgment` updates the approval judgment, then projector |
...
| `APR` | committed approval request is created by `request_user_judgment(judgment_route=approve-sensitive-action)`, or approval judgment changes through `record_user_judgment` | approval-shaped Decision Packet, linked Approval record status, scope, baseline, expiry, or judgment note changes |
