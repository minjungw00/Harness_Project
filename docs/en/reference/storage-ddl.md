...
- `projects.state_db_path` is retained as a stored column. The SQL column definition does not enforce its equality with `project_home/state.sqlite`; Store application-level current-registration validation enforces that relationship before operational `ProjectRecord` lookup or listing, project-state migration or writable open, surface management, Agent Integration Profile project access, Core execution, setup reuse, or MCP project availability. A mismatching registry row remains inspectable as raw registry content for diagnosis, but operational paths must reject it rather than omit it or return it as a normal project.
