...
The current active MVP is closed in [MVP Plan](../build/mvp-plan.md). It includes ordinary-language intake and Task creation, `harness.update_scope`, user judgment recording, sensitive approval recording, path-level `harness.prepare_write` and Write Authorization, `harness.record_run`, artifact staging through `harness.stage_artifact` with registration only through the compatible `record_run` path, compact `EvidenceSummary`, `harness.close_task` blocker calculation, read-time read-only status/projection output, verified local surface access through a registered surface, cooperative guarantee display, and detective guarantee display only after the relevant capability check has actually passed.
