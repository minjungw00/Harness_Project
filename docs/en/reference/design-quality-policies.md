...
| Stale context affecting write or close | `context_hygiene_check` only when stale state, baseline, readable status/projection view, or source refs make scope, evidence, acceptance criteria, or close basis unsafe to rely on. |
...
|---|---|
| `shared_design` | Has requirements clarification / Discovery or another shaping path checked answerable facts from available repo/docs/current state, classified the work shape, separated missing information into blocking questions and useful non-blocking questions, identified user-owned judgments, and persisted the active shaping result through Task, Change Unit, and user-judgment candidate/record boundaries instead of requiring a committed Shared Design record? |
| `decision_quality` | Is this a product, design, technical, architecture, waiver, or risk choice that needs a user judgment or an existing waiver, residual-risk, QA, or acceptance owner path when that path is active? |
...
- projection template bodies; see [Projection And Templates Reference](projection-and-templates.md)
...
| `required_record` | Active owner record, candidate boundary, or later/profile record family that stores the result. A policy must not require a later/profile record in active MVP unless the owning profile is active. |
...
When a policy contract names a later/profile record, projection, or full artifact, that name is conditional. Active MVP records are limited to the active schema and storage owners. In particular, `shared_design` is a policy name in active MVP, not an accepted `StateRecordRef.record_kind` value, committed `shared_designs` row, required projection, or full design artifact.
...
`shared_design` is the design-quality policy name for requirements clarification. In active MVP-1, it is not a committed Shared Design record, accepted `StateRecordRef.record_kind` value, active storage table, projection, or full design artifact. `Discovery` is the stable internal name for the clarification posture, not a user command. It is used when clarification is needed, not as mandatory ceremony for every task.

The active MVP canonical boundary is:

- Task stores the original user request, current goal summary, confirmed facts, remaining uncertainties, blocking question or blocker summary when the question is not yet a user judgment, and next safe action.
- Change Unit stores the proposed or active work boundary: scope summary, non-goals, success criteria, affected area or affected path candidates, allowed and denied paths, sensitive categories, completion conditions, and Autonomy Boundary when needed.
- `UserJudgmentCandidate` and `user_judgment` store choices only the user can make, including product, technical, scope, sensitive-action, QA waiver, verification-risk, final-acceptance, residual-risk, and cancellation judgments.

Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate, Shared Design projection, full Decision Packet, and full design artifact are support or display names in active MVP. They may be rendered from active owner records or used as later/profile material, but they are not separate active MVP committed records unless an owner profile explicitly enables them. A First Safe Change Unit Candidate is represented by a proposed Change Unit update when product writes are near; it is not a standalone record.

The flow is: preserve the user request, check what available repository, docs, and current Harness state can answer, classify the work shape, identify missing information, separate blocking questions from useful non-blocking questions, identify user-owned judgments, present options, consequences, uncertainty, and a recommendation when a user judgment is required, then propose the next safe action or a candidate Change Unit. Shaping can pause or proceed when confirmed facts and user-owned judgments are separated; goals, non-goals, success criteria, and major judgment candidates are clear enough; safe next work, a smaller scope, or a work split can be proposed without hiding unresolved user-owned judgment; and remaining uncertainty is explicit. It is not approval, sensitive-action Approval, Write Authorization, evidence, verification, QA, final acceptance, residual-risk acceptance, close, scope authority, or a new authority path.
...
Example: A user says, "We need better onboarding for new workspace owners. Inspect what exists, separate product choices from facts, and ask only what the repo cannot answer." Before writing, persist the goal, user value, non-goals, success criteria, repo/docs/state facts the agent can inspect, user-owned product choices such as inline checklist versus modal setup prompt, QA expectations for the flow, rejected options, remaining uncertainty, and a safe next action through the Task, proposed Change Unit, and user-judgment candidate boundaries.

Example: A user says, "I want to replace our login approach, but I do not know whether sessions, magic links, or OAuth/OIDC fit best. Inspect the current auth shape first." Before writing, inspect existing user/session/auth docs and code, then separate user-owned Technical decision such as local email/password sessions, magic links, OAuth/OIDC, account enumeration, redaction, rate limits, and session lifetime; verification and Manual QA expectations; remaining uncertainty; and the proposed Change Unit or work split.

Phrases such as "safe next-work candidate," "work split," and "First Safe Change Unit Candidate" are proposal/support phrases, not standalone schemas, canonical record types, gate values, projection kinds, storage rows, or authority paths.

Requirements shaping records shared understanding through active owner records; it is not broad approval, sensitive-action Approval, final acceptance, residual-risk acceptance, QA judgment, or Write Authorization. It can expose decisions that require a user judgment, and it can support `design_gate` readiness, but it does not settle user-owned judgment by itself.

| Field | Contract |
|---|---|
| `name` | `shared_design` |
| `applies_when` | Work request is ambiguous, scope/non-scope is unclear, user value needs alignment, affected product areas, user-facing screens or flows, modules, interfaces, sensitive categories, verification or Manual QA expectations, user-owned product or material technical trade-offs, or known product, implementation, verification, QA, operational, or follow-up risks need shaping, public interface/schema/auth/security/privacy/UX/workflow is affected, or a `work` task needs shaping. Tiny direct must leave the tiny path as soon as any of those signals appear. It may escalate to ordinary Direct when the work remains narrow but needs evidence beyond the tiny changed-path/self-check note. It must route to Work and use requirements clarification when needed; later/profile owners may record that support as Shared Design when enabled. |
| `default_requirement` | Use active shaping owner paths. Preserve the original user request and current goal summary on the Task; record confirmed facts, remaining uncertainties, the blocking question when one exists, and the next safe action. Record proposed or active Change Unit shaping for scope summary, non-goals, success criteria, affected product areas, user-facing screens or flows, modules, interfaces, affected path candidates, allowed and denied paths, sensitive categories, completion conditions, and Autonomy Boundary when relevant. Emit `UserJudgmentCandidate` values or request/record `user_judgment` records for judgments only the user can make, including separated Product decision candidates, Technical decision candidates with security/privacy considerations when relevant, QA and verification expectations, scope decisions, waivers, final acceptance, residual-risk acceptance, or cancellation. Inspect before asking: use repo, codebase, docs, and Harness state that are available and current for answerable facts the agent can discover safely; if a source is unavailable or stale, record that uncertainty rather than treating it as authority. Classify open items as answerable facts to inspect, blocking questions, or useful non-blocking questions. Ask the user only for judgments the current codebase, docs, Harness state, or safe inspection cannot answer. Clarify through targeted questions grouped by judgment area rather than a long questionnaire, usually starting with the most blocking area; each user-owned judgment prompt must include `presentation`-appropriate options or chosen outcome, and trade-off prompts include recommendation, uncertainty, and deferral consequence, including what can continue if deferred or why nothing should continue until the judgment is made. Separate agent assumptions from choices, sensitive-action permission, QA judgment, final acceptance, residual-risk acceptance, or public commitments that need user judgment. Do not harden implementation planning until confirmed facts and user-owned judgments are separated; goals, non-goals, success criteria, and major judgment candidates are clear enough; remaining uncertainty is explicit; and enough scoped information exists to propose safe next work, a candidate Change Unit, a smaller scope, or a work split without hiding unresolved user judgment. |
...
| `required_record` | Active MVP: Task shaping fields, proposed or active Change Unit fields, `UserJudgmentCandidate` drafts before commit, and `user_judgment` records after request/record. `blocker`, `evidence_summary`, or artifact refs may carry the same concern only when those active owner paths apply. Later/profile only: committed Shared Design record, `DESIGN` projection, full design artifact, or full-format Decision Packet when an owner profile explicitly enables them. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate, Shared Design projection, full Decision Packet, and full design artifact are not separate active MVP required records. |
| `validator` | `shared_design_alignment` |
| `evidence` | Task refs, Change Unit refs, user judgment refs, success criteria, rejected option refs, product-area, user-facing-flow, module, interface, affected-path, and sensitive-category notes, verification or Manual QA expectation refs, risk notes, risk candidates, residual-risk refs where applicable, and domain/module/interface impact refs. Later/profile Shared Design refs may appear only when that profile is active. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate, Shared Design projection, full Decision Packet, and full design artifact remain shaping support, display, or later/profile material; by themselves they do not satisfy the active `evidence_summary`, any later Evidence Manifest, or close evidence. |
| `close_impact` | Active MVP default: routed candidate. If required shaping support is absent, show one advisory next action, ask one focused user judgment, or request evidence. It may block write or close only when the gap is also missing active scope, unresolved user judgment, missing required evidence, stale context affecting write/close, or surface capability insufficient for the claimed guarantee. A valid waiver may allow `design_gate=waived` for policy-owned impact only. |
...
- `judgment_kind=product_decision`, `presentation=full`: failed-login feedback uses inline message, toast, or modal/layer. Record the user-flow, interruption, accessibility, copy, and product-risk trade-offs. A similar judgment can cover whether a blocking form problem should use inline error text, a toast, or a modal/layer.
- `judgment_kind=product_decision`, `presentation=full`: a product taste choice changes how opinionated the experience feels, such as an empty state that invites setup now versus a quieter page that waits for data. Record the product intent, user segment, clarity, interruption, Manual QA need, recommendation, uncertainty, and deferral effect, including what can continue if deferred or why nothing should continue until the judgment is made.
- `judgment_kind=technical_decision`, `presentation=full`: session auth, token auth, or social login. Record revocation, CSRF/XSS exposure, client compatibility, operational complexity, migration path, and why the recommendation fits the Task.
- `judgment_kind=technical_decision`, `presentation=full`: dependency addition, schema migration, public API/interface change, or module boundary change. Record alternatives, blast radius, compatibility, rollback or migration cost, test boundary, future maintenance cost, recommendation, and deferral effect.
...
- `judgment_kind=scope_decision`, `presentation=full`: scope expansion, non-goal removal, Change Unit boundary change, or Autonomy Boundary change. Record what becomes in scope, what remains out of scope, why the agent cannot infer this from prior broad approval, and what writes or close path are affected.
- `judgment_kind=sensitive_approval`, `presentation=full`: auth, permission, secret, or data-export action. The Approval boundary may authorize the sensitive step, but product, technical, scope, security, QA, verification, final-acceptance, or residual-risk judgment still needs a separate compatible user judgment when roles, exported fields, redaction, audit logging, retention, rollback, or user notice remain undecided.
- `judgment_kind=qa_waiver`, `presentation=full`: skipped Manual QA when policy allows a waiver. Record what was not inspected, why the waiver is allowed, the visible risk path, and why the waiver does not create QA evidence or a passed QA result.
- `judgment_kind=verification_risk_acceptance`, `presentation=full`: required verification is waived or unavailable. Record what verification is missing, the risk accepted, why the result must not be displayed as `detached_verified`, and the smallest credible follow-up.
- `judgment_kind=residual_risk_acceptance`, `presentation=full`: close with known remaining risk. Record the visible limitation, evidence already present, why close can still proceed, visible residual-risk refs, and follow-up.
...
| `evidence` | User judgment refs, option refs, active evidence summary refs, later Evidence Manifest refs only when that profile is active, risk/waiver refs, residual-risk state refs when residual-risk acceptance is involved, and final-acceptance refs when user judgment is required. |
...
| `required_record` | Canonical Autonomy Boundary record on the active Change Unit; Task shaping fields or proposed Change Unit boundary before a Change Unit is active; user judgment records for user-judgment items; and stop-condition refs when triggered. Later/profile Shared Design refs may carry this support only when that profile is enabled. |
...
| `evidence` | Change Unit record, run summary, active evidence summary refs, tests, and later Manual QA refs only when that owner path is active. |
...
Public mutation path when the feedback-loop profile is active: selected-loop definitions and waivers are recorded with `FeedbackLoopUpdate` during `record_run(kind=shaping_update)`. Execution refs and status are updated with `EvidenceUpdates.feedback_loop_updates` during implementation/direct runs, or with `record_manual_qa.feedback_loop_ref` when Manual QA is the selected loop.
...
| `required_record` | `tdd_traces` records and rendered TDD trace projection when active. |
| `validator` | `tdd_trace_required` |
| `evidence` | Actual failing test artifact/log/result or another explicit policy-recognized failing-check evidence, passing test log, refactor check log when relevant, diff refs, active evidence summary refs, later Evidence Manifest coverage refs only when that profile is active, non-TDD justification, and alternate feedback loop when waived. RED targets or RED plans are planning records, not evidence. |
...
2. Record RED evidence before non-test implementation writes. RED evidence means an actual failing test artifact/log/result or another explicit policy-recognized failing-check evidence. A RED target or plan does not satisfy this precondition, the active `evidence_summary`, or later Evidence Manifest coverage.
...
6. Link the TDD trace, Feedback Loop, run logs, and artifacts to active evidence coverage, or to Evidence Manifest acceptance-criteria and changed-file coverage only when the later evidence profile is active.

This is policy and write-check behavior, not a Kernel Authority Invariant. Kernel authority still comes from active Task, active Change Unit scope, `prepare_write`, Write Authorization, sensitive-action permission, user judgments, evidence, active/profile-gated verification or QA owner paths when enabled, final acceptance, and close semantics in the owner documents.
...
| `required_record` | When the domain/stewardship profile is active, `domain_terms` records referenced with `record_kind=domain_term`; the domain-language projection is only a projection/proposal surface. Active MVP may route terminology concerns as advisory guidance, evidence requests, residual-risk visibility, or user judgment without accepting `record_kind=domain_term`. |
...
| `required_record` | When the module/interface profile is active, `module_map_items` and `interface_contracts` records referenced with `record_kind=module_map_item` and `record_kind=interface_contract`, decision records, and optional module-map or interface-contract projections. Active MVP may route boundary concerns as advisory guidance, evidence requests, residual-risk visibility, or user judgment without accepting those later record kinds. |
...
| `required_record` | Task or Change Unit stewardship refs plus later/profile owner records such as `domain_terms`, `module_map_items` records with module-local watchpoints when relevant, `interface_contracts` records, `feedback_loops` records, `tdd_traces` refs when TDD is used, decision records, Task/Change Unit watchpoints, Journey Spine Entry refs, and reconcile items for drift when those profiles are active. Dedicated architecture watchpoint refs may be used only if a later DDL batch defines them. Canonical design-support refs use `record_kind=domain_term`, `record_kind=module_map_item`, `record_kind=interface_contract`, and `record_kind=feedback_loop` only when the matching profile extends `StateRecordRef`; Markdown projection refs are optional display/proposal refs. |
...
| `required_record` | Source records used to render the envelope or context profile come from existing owners such as current Task state, acceptance criteria, Change Unit, Autonomy Boundary, user judgments, gate states, Write Authority summary, approval status, surface capability/guarantee summary, projection freshness and `source_state_version` when known, active evidence summary, Run, ArtifactRef, validator results, and profile-gated Evidence Manifest, Eval, Manual QA, report, residual-risk, or reconcile refs when those owners are active. The compact envelope and context profile are rendered/derived context display, not a canonical record, schema field, DDL value, authority input, gate, evidence, verification, QA, final acceptance, residual-risk acceptance, close, or storage object. |
