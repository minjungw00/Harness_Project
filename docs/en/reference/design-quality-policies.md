...
|---|---|
| `shared_design` | Has Discovery or another shaping path separated inspectable facts from user-owned decisions and recorded enough goal, user value, scope, non-goals, assumptions, product, technical, security/privacy, QA, operational, follow-up risk, and first-work-candidate context to propose safe next work? |
...
Example: A user asks for "better onboarding." Before writing, record the goal, user value, non-goals, acceptance criteria, repo/docs/state facts the agent can inspect, user-owned product/UX choices such as inline checklist versus modal setup prompt, QA expectations for the flow, rejected options, and a first implementation candidate or split. If product writes are near, that candidate can later become a First Safe Change Unit Candidate.

Example: A user asks for "email login." Before writing, inspect existing user/session/auth docs and code, then separate user-owned authentication architecture choices such as local email/password sessions, magic links, or OAuth/OIDC; security/privacy choices such as account enumeration, redaction, rate limits, and session lifetime; verification and Manual QA expectations; and the first implementation candidate or work split.

Phrases such as "first implementation candidate" and "work split" are proposal/support phrases, not standalone schemas, canonical record types, gate values, projection kinds, or authority paths. When product writes are near, one proposal may be expressed as a First Safe Change Unit Candidate for Change Unit shaping.
...
| `evidence` | Decision Packet refs, option refs, evidence manifest refs, risk/waiver refs, residual-risk state refs when residual-risk acceptance is involved, and user acceptance refs when user judgment is required. |
...
| `default_requirement` | Use a compact always-on rule set of ten items or fewer, read current status or the Journey Card first, and choose the active Intake, Discovery, Write, Evidence, Verification, or Close phase bundle. Use refs-first summaries for current Task, Journey, Change Unit, Decision Packet, Write Authority, Evidence, Residual Risk, Gate, and Projection freshness where phase-relevant. Keep larger Reference docs, schemas, historical records, older PRDs/designs, old projections, logs, screenshots, diffs, raw artifacts, module maps, interface contracts, domain records, coding standards, and TDD guidance pull-on-demand. Retrieved, indexed, remembered, or summarized context remains pull-only and non-authoritative. Mark stale docs, warn on projection freshness drift, and avoid treating chat, retrieved context, indexed context, summarized context, or remembered recommendations as state or authority. |
...
| `required_record` | Source records used to render the envelope or phase bundle come from existing owners such as current Task state, acceptance criteria, Change Unit, Autonomy Boundary, Decision Packets, gate states, Write Authority summary, approval status, surface capability/guarantee summary, projection freshness and `source_state_version` when known, Evidence Manifest, Run, Eval, Manual QA, ArtifactRef, report, residual-risk, reconcile items, and validator results. The compact envelope and phase bundle are rendered/derived context display, not a canonical record, schema field, DDL value, authority input, gate, evidence, verification, QA, acceptance, residual-risk acceptance, close, or storage object. |
| `validator` | `context_hygiene_check` |
| `evidence` | Current projection refs, freshness state, `source_state_version`, stale refs, retrieved/indexed context freshness notes, missing phase-relevant bundle material, reconcile item refs, and evaluator bundle contents. Stale or over-broad critical context is reported as a context-hygiene finding; the finding itself is not evidence unless an existing owner path records supporting refs. |
| `close_impact` | Stale or over-broad critical context, stale projection freshness, stale retrieved/indexed context, or missing phase-relevant bundle material may warn, mark stale, or block write/close only through existing owner paths and gates. It can block write or close when the agent cannot safely determine scope, evidence, current acceptance criteria, or whether the readable projection matches canonical state. |
