...
| `required_record` | When the domain/stewardship profile is active, `domain_terms` records referenced with `record_kind=domain_term`; `DOMAIN-LANGUAGE` is only a projection/proposal surface. Active MVP may route terminology concerns as advisory guidance, evidence requests, residual-risk visibility, or user judgment without accepting `record_kind=domain_term`. |
...
| `required_record` | When the module/interface profile is active, `module_map_items` and `interface_contracts` records referenced with `record_kind=module_map_item` and `record_kind=interface_contract`, decision records, and optionally `MODULE-MAP` / `INTERFACE-CONTRACT` projections. Active MVP may route boundary concerns as advisory guidance, evidence requests, residual-risk visibility, or user judgment without accepting those later record kinds. |
