...
|---|---|
| `shared_design` | Is Discovery or another shaping path enough to record goal, user value, scope, non-goals, assumptions, product, technical, security, QA, operational, follow-up risks, and user-owned judgments sufficient to propose the first safe Change Unit? |
...
Discovery is the conditional Shared Design intake posture for clarifying requirements before implementation planning and write authority. It is used when clarification is needed, not as mandatory ceremony for every task. It may ask multiple targeted questions and keep a Question Queue, but it must stop when a First Safe Change Unit Candidate can be proposed without hiding unresolved user-owned judgment. It routes outputs into Shared Design, Decision Packet candidates, and Change Unit shaping; it is not a standalone schema, canonical record field list, approval, sensitive-action Approval, Write Authorization, evidence, verification, QA, acceptance, residual-risk acceptance, close, scope authority, or a new authority path.
