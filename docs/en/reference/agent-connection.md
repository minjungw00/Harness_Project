...
| Verification state | `last_verification_status` in the Agent Connection registry record, plus command output owned by [Administrative CLI](admin-cli.md#agent-connection-result-states). | `volicord init`, `volicord connect`, and `volicord connection verify` run observable host-configuration, MCP startup, MCP initialization, and `tools/list` checks where available. | Verification can inspect both Volicord-side state and host/MCP readiness. MCP startup validation alone is not a `complete` Agent Connection. |
