...
For source builds, the debug executable is expected under `target/debug` and the release executable under `target/release`. For separately installed executables, select an installation layout where setup can find `volicord` through an explicit setup option or `PATH`.
