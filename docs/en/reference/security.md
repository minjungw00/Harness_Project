...
The MVP also does not claim that local files are trustworthy because they are local, that runtime boundaries are OS isolation boundaries, that MCP reachability or `surface_id` is authorization, that Product Repository files, projections, chat, generated Markdown, or agent memory can create or refresh authority or surface registration, or that conformance fixture language proves runtime security behavior before implementation exists.
...
- `harness.stage_artifact` creates only a temporary same-project same-Task staging handle for caller-provided safe bytes or a safe notice, with `created_by_surface_id` and `created_by_surface_instance_id` recorded by the server from `VerifiedSurfaceContext`; it is not proof that arbitrary local files are safe or authorized
...
- `record_run` uses `run_recording` to consume a valid same-surface-instance staging handle and promote it to a persistent `ArtifactRef`; `StagedArtifactHandle` is not a bearer token, cross-surface handoff is not active, and `artifact_registration` remains reserved for `harness.stage_artifact`
