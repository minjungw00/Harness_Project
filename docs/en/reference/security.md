...
| Stale approval replay | Old approval text or a stale Approval record is reused after scope, baseline, sensitive category, expiry, or actor context changes. | Check scope, baseline/state version, expiry, sensitive category, and actor compatibility through Kernel and MCP owner paths before the pre-write scope check can produce a compatible record. |
| Out-of-scope write | An agent writes a path, runs a command, reaches a network target, or accesses a secret outside the active Change Unit or Approval. | Use active scope, `prepare_write`, internal Write Authorization records, changed-path validation, and command/path/network/secret allowlists for high-risk work. These checks do not imply OS permission, sandboxing, or physical pre-tool blocking unless a preventive profile proves that covered operation. |
