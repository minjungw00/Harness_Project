...
- `harness.stage_artifact` creates only a temporary same-project same-Task staging handle for caller-provided safe bytes or a safe notice; it is not proof that arbitrary local files are safe or authorized
...
- `record_run` uses `run_recording` to consume a valid staging handle and promote it to a persistent `ArtifactRef`; `artifact_registration` remains reserved for `harness.stage_artifact`
