...
Write Authorization is a single-use cooperative Harness record created only by the compatible non-dry-run `prepare_write` path and consumed by compatible `record_run` under `run_recording`. It is a Harness record/check, not OS permission, sandboxing, tamper-proof enforcement, physical pre-tool blocking, or isolation.

Sensitive-action approval is a separate cooperative user-permission record stored as a user judgment with `SensitiveActionScope`. It can permit a named command, dependency change, network access, secret access, deployment, destructive action, system access, product-file write, or other scoped sensitive action, but that permission does not create Write Authorization, grant artifact body read access, or prove Harness observed, blocked, enforced, sandboxed, or isolated the action. Any honest observation, blocking, enforcement, or artifact body access claim must be limited to the active surface capability and access class for the exact operation.

For the baseline `reference-local-mcp` profile, Write Authorization and product-write Run compatibility are path-level. The profile is cooperative by default and has limited detective support only when `changed_path_detection_supported=true` and `changed_path_detection_verification=passed`, and only for observed changed paths within that verified scope. `changed_path_detection_verification=not_run`, legacy `planned_not_run` wording, `failed`, or `stale` cannot justify a `detective` label. A failed or stale capability check must produce `CAPABILITY_INSUFFICIENT` where the method requires that capability, or a downgraded `cooperative` display where the method can proceed without the stronger claim. The profile has no command observation, network observation, secret-access observation, native artifact capture, pre-tool blocking, or isolation. `manual_artifact_attachment_supported=true` means the active `harness.stage_artifact` path stages caller-provided safe bytes or safe notices into the temporary staging area with `access_class=artifact_registration`; it is input staging, not connector artifact capture, and it does not prove that arbitrary local files are safe, authorized, or produced by Harness-observed work. On successful staging, the server records `created_by_surface_id` and `created_by_surface_instance_id` from `VerifiedSurfaceContext`; those fields are not user-provided authority claims. `StagedArtifactHandle` is a server-validated reference, not a bearer token or arbitrary artifact-registration authority. `harness.record_run` uses `access_class=run_recording` only, including eligible staged-handle promotion, and may promote only when the staged handle passes project, task, current verified surface instance, expiration, consumed status, checksum, and size validation. It does not use `artifact_registration` as a second access class, and the active MVP has no cross-surface staged artifact handoff. Artifact body reads remain separate from staged-handle promotion and require `access_class=artifact_read`; Write Authorization and sensitive-action approval do not grant artifact body read access. `native_artifact_capture_supported=false` must stay consistent with this active manual staging model.
...
The MVP also does not claim that local files are trustworthy because they are local, that runtime boundaries are OS isolation boundaries, that MCP reachability or `surface_id` is authorization, that Product Repository files, projections, chat, generated Markdown, or agent memory can create staged handles, refresh authority or surface registration, grant promotion authority, or that conformance fixture language proves runtime security behavior before implementation exists.
...
| `state.sqlite` and Runtime Home metadata | Persist project registration, current state, event history, surfaces, Write Authorizations, staged-artifact rows, and artifact metadata. | [Storage](storage.md) owns layout and defensive checks; storage is not tamper-proof. Runtime Home staging area and persistent artifact storage are distinct. |
| Write Authorization and `AuthorizedAttemptScope` | Records one compatible intended product-file write attempt for one compatible consumption. | [Core Model Reference](core-model.md#write-authorization), [MVP API](api/mvp-api.md), and [Storage](storage.md) own exact behavior. |
| `user_judgment` records and `SensitiveActionScope` | Preserve active user-owned product, technical, scope, sensitive-action, final-acceptance, residual-risk, or cancellation judgments. `SensitiveActionScope` names the sensitive action that was permitted without becoming path-level Write Authorization or an enforcement claim. | Core/API owners decide exact routes; chat text is input until recorded through the owner path. |
...
| Product Repository files and generated projections | Can influence agents and users, but are input or derived display from a Harness perspective. Product Repository paths and staged artifact storage are different trust domains. | [Runtime Boundaries Reference](runtime-boundaries.md) and [Projection And Templates Reference](projection-and-templates.md) own display boundaries. |
...
| Product Repository | Product files, repository rules, generated Markdown, and projections are product work, inputs, or derived display. They are not Harness operational authority by presence or proximity, and Product Repository paths are not staged artifact storage. |
...
| Harness Runtime Home | Runtime Home stores Core-owned records and artifacts for future operation. The temporary staging area and persistent artifact storage are separate storage roles. Treat broad local read/write access as tampering and confidentiality risk; do not claim tamper-proof storage. |
| MCP / local API surface | Reachability and `surface_id` are not authorization. The server must derive `VerifiedSurfaceContext` from the local transport/session/binding and stored registration before mutating APIs or artifact body reads can rely on the surface. Core/API validation, project/task/surface compatibility, idempotency, expected state version, local access posture, surface status, and active capability still apply. |
...
| Artifact store | Artifact bytes are untrusted until registered, related to the owner record, and validated for required integrity/redaction metadata. Staged bytes remain in temporary staging until an eligible `record_run` promotion creates a persistent `ArtifactRef`. |
...
| Artifact or evidence tampering | Bytes, paths, hashes, staging handles, or metadata are swapped, stale, expired, consumed, missing, redacted, blocked, cross-task, cross-surface, or unrelated to the owner record. | Treat evidence as insufficient or blocked until staging-handle project, task, surface-instance, expiration, consumed-status, checksum, size, registration, integrity, redaction, and owner relation checks pass where applicable. Raw paths, raw logs, arbitrary local path strings, and native capture claims are not artifact authority. |
...
- `harness.stage_artifact` stages new artifact bytes or a safe notice into a temporary staging area with `access_class=artifact_registration`, creating only a same-project same-Task staging handle with `created_by_surface_id` and `created_by_surface_instance_id` recorded by the server from `VerifiedSurfaceContext`; it is not proof that arbitrary local files are safe or authorized
...
- `record_run` uses `access_class=run_recording` to consume a staged handle only after project, task, same verified surface instance, expiration, consumed status, checksum, and size validation pass, then promote it to a persistent `ArtifactRef`; `StagedArtifactHandle` is not a bearer token, cross-surface handoff is not active, and `artifact_registration` remains reserved for `harness.stage_artifact`
- artifact body read is a separate operation with `access_class=artifact_read`; sensitive-action approval and Write Authorization do not grant it
