...
| `ArtifactRef` | Public pointer to a registered artifact. A registered artifact supports evidence only when the relevant evidence coverage links it to a claim. | [API Schema Core](api/schema-core.md), [Storage](storage.md) |
...
| native artifact capture | Later capability candidate. Active MVP artifact intake is manual staging through `harness.stage_artifact` plus owner registration, not surface-native capture. | [Later](../later/index.md), [Agent Integration](agent-integration.md), [API Schema Core](api/schema-core.md) |
