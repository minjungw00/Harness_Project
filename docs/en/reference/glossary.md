...
| `ToolError` | Shared public error shape carrying error identity, retry guidance, and structured details. In `ToolRejectedResponse.errors`, `ToolError` values carry pre-commit failure errors. Exact public codes and precedence are owned by API Errors. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md) |
| `WriteDecisionReason` | `prepare_write` decision reason returned in `PrepareWriteResult.write_decision_reasons` when `decision=blocked`, `decision=approval_required`, or `decision=decision_required`. It is not a `CloseBlocker`, not a dry-run preview blocker, and not a container for pre-commit failures. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [Core Model](core-model.md) |
| `PlannedBlocker` | Dry-run preview data used only in `DryRunSummary.would_blockers` for expected blockers. It is not stored, not a `WriteDecisionReason`, not a `CloseBlocker`, and not a committed blocker result. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [API Errors](api/errors.md) |
| `STATE_VERSION_CONFLICT` | Current MVP public `ErrorCode` for project-wide freshness and idempotency conflicts, including stale `project_state.state_version` expectations, stale `WriteAuthorization.basis_state_version` before consumption, and idempotency request-hash conflicts. It is returned only in `ToolRejectedResponse` with `effect_kind=no_effect`; it is not `WriteDecisionReason.code`, `CloseBlocker.code`, or `PlannedBlocker.code`. | [API Errors](api/errors.md), [Storage](storage.md) |
...
| `CloseBlocker` | `close_task` close-matrix blocker returned in `CloseTaskResult.blockers` for a committed blocked close result after close preflight succeeds. It is not a generic blocker type, not used by `prepare_write`, not used by `DryRunSummary.would_blockers`, and not a container for pre-commit failures. `STATE_VERSION_CONFLICT` must not be `CloseBlocker.code`. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md), [Core Model](core-model.md) |
