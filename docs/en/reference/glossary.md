...
| `STATE_VERSION_CONFLICT` | Current MVP public `ErrorCode` for project-wide freshness and idempotency conflicts, including stale `project_state.state_version` expectations and idempotency request-hash conflicts. It is returned in `ToolRejectedResponse` with `effect_kind=no_effect`; it is not a committed close blocker and must not be `CloseBlocker.code`. | [API Errors](api/errors.md), [Storage](storage.md) |
...
| `CloseBlocker` | Structured semantic close blocker produced after a close matrix has run successfully enough to evaluate close eligibility. It represents committed semantic close blockers, not preflight rejection. `STATE_VERSION_CONFLICT` must not be `CloseBlocker.code`. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md), [Core Model](core-model.md) |
