...
Baseline registry storage includes Agent Connection records and `connection_projects`. Baseline project-state storage includes Core state records, replay rows, staged artifacts, persistent artifacts, evidence, user judgments, runs, blockers, and `write_checks`.
