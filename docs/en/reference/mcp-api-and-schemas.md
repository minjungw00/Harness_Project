...
- `one_of:` defines a branch object. Exactly one listed child field is present, and its value must match that child field's stated type. Branch prose may further require the selected branch to match a discriminator field such as `decision_profile`. All non-selected branches must be absent, not present with `null`.
...
This request answers the existing canonical Decision Packet. It does not accept a caller-supplied replacement `decision_profile`; Core validates the answer against the stored packet's `decision_kind`, `decision_profile`, status, refs, and compatible payload branch.
