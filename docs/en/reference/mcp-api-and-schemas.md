...
| `reconcile` | User decision over managed drift, proposal/state mismatch, or generated/projection edit. | `decision_kind=reconcile`, `reconcile_item_id`, reconcile target, candidate outcome options, and state/projection refs being reconciled. |
| `mixed` | One user-owned decision with truly cross-cutting considerations. | Common fields plus the profile-specific fields for the primary route and any included consideration. Use separate Decision Packets instead when approval, final acceptance, residual-risk acceptance, waiver, and product/technical judgment are separate decisions. |
