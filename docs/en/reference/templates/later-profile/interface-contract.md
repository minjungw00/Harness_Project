...
When a public interface change, compatibility risk, breaking change, or caller-impact choice requires user-owned product decision or material technical decision, route it through the existing design-quality and user judgment paths. Rendering the contract here does not resolve the `design_gate`, `decision_gate`, or close impact by itself.
