...
- profile-required detail: common fields plus the selected `profile_payload` branch chosen by `decision_profile`; minimal decisions may omit non-material detailed fields or render them as null where the schema allows null
- final recorded answer: `decision_kind` chooses the user-decision route and `RecordUserDecisionPayload` resolution branch
