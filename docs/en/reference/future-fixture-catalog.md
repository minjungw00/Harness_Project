...
The core conformance model, exact fixture body, execution rules, assertion semantics, and narrow v0.1 Kernel Smoke authoring order stay in [Conformance Fixtures Reference](conformance-fixtures.md). This catalog is deliberately downstream of that model. Catalog rows are not fixture bodies, not public API schemas, not DDL, not stage exits, and not proof that fixtures already run.
