...
The default supported MCP access assumption is local-process or localhost access for a registered project surface. This is a local operating assumption, not trust in every process on the machine. Concrete local risks include another local process issuing tool calls, a forwarded or tunneled port, stale connector configuration, spoofed `project_id`, `task_id`, `surface_id`, or `actor_kind` claims, and IPC or file permissions that let unrelated users or processes read or change Runtime Home data.

Exposing MCP beyond local-process or localhost scope requires a documented connector capability profile, access-control contract, and guarantee display. Acceptable access-control contracts can include localhost-only binding, a Unix-domain socket constrained by owner-only filesystem permissions, a per-project token, process-scoped configuration material, or an equivalent local control. This reference does not require one mechanism, but the profile must name what prevents unrelated callers from using the endpoint and what Core still validates.
...
Runtime Home files should be treated as user-private local control data. File permissions or storage locations that allow unrelated users or processes to read secrets/PII, edit `state.sqlite`, `registry.sqlite`, `project.yaml`, connector manifests, artifact files, staging files, or generated operational files are local tampering or confidentiality risks. Harness does not claim to enforce operating-system permissions by itself; it treats those files as authoritative only through Core, `doctor`, `recover`, and artifact-integrity validation paths.
