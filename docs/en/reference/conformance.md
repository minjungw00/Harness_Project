...
| `MVP-ACTIVE-stage-artifact-temporary-handle-only` | `harness.stage_artifact` uses `artifact_registration` to create only a temporary same-project same-Task `StagedArtifactHandle` for caller-provided safe bytes or a safe notice. | Staging creates no Core record, persistent `ArtifactRef`, evidence summary, blocker, event, `tool_invocations` replay row, close effect, or `project_state.state_version` increment. The handle must remain scoped, expiring, and single-consumption; it is not evidence authority and not arbitrary local-file authority. |
| `MVP-ACTIVE-record-run-promotes-staged-artifact-to-artifact-ref` | Only compatible `harness.record_run` under `run_recording` can consume an unexpired same-project same-Task `StagedArtifactHandle` and promote it to a persistent `ArtifactRef`. | `record_run` rejects missing, expired, mismatched, already-consumed, cross-project, cross-Task, or integrity-incompatible handles with `ARTIFACT_MISSING` and `ToolError.details.staging_handle_reason`, without creating a Run, artifact, evidence update, authorization consumption, replay row, or state-version increment. A valid consumed handle produces registered artifact metadata and owner links that match staged `sha256`, `size_bytes`, `content_type`, `redaction_state`, project, and Task. This promotion does not require `artifact_registration`; that access class was used only by the earlier `harness.stage_artifact` request. |
