...
| `MVP-ACTIVE-prepare-write-requires-compatible-scope-and-approval` | `harness.prepare_write` may return `decision=allowed` only when the intended product-file write is compatible with the active Task, Change Unit, scope, baseline, Autonomy Boundary, required user judgment, required separate sensitive-action approval, surface capability, and current project-wide state version. | Missing or incompatible preconditions return `blocked`, `approval_required`, `decision_required`, `state_version_conflict`, or the applicable public error such as `SCOPE_REQUIRED`, `SCOPE_VIOLATION`, `APPROVAL_REQUIRED`, `CAPABILITY_INSUFFICIENT`, or `STATE_VERSION_CONFLICT`. These outcomes and dry runs create no consumable Write Authorization, Run, artifact, evidence, close, final-acceptance, or residual-risk state. |
...
| `MVP-ACTIVE-stage-artifact-temporary-handle-only` | `harness.stage_artifact` creates only a temporary same-project same-Task `StagedArtifactHandle` for caller-provided safe bytes or a safe notice. | Staging creates no Core record, persistent `ArtifactRef`, evidence summary, blocker, event, `tool_invocations` replay row, close effect, or `project_state.state_version` increment. The handle must remain scoped, expiring, and single-consumption; it is not evidence authority and not arbitrary local-file authority. |
| `MVP-ACTIVE-record-run-promotes-staged-artifact-to-artifact-ref` | Only compatible `harness.record_run` can consume an unexpired same-project same-Task `StagedArtifactHandle` and promote it to a persistent `ArtifactRef`. | `record_run` rejects missing, expired, mismatched, already-consumed, cross-project, cross-Task, or integrity-incompatible handles with `ARTIFACT_MISSING` and `ToolError.details.staging_handle_reason`, without creating a Run, artifact, evidence update, authorization consumption, replay row, or state-version increment. A valid consumed handle produces registered artifact metadata and owner links that match staged `sha256`, `size_bytes`, `content_type`, `redaction_state`, project, and Task. |
