...
Projection support is source-backed. MVP-1 can satisfy user-readable output with the compact view set owned by [Projection And Templates Reference](projection-and-templates.md#mvp-1-view-set) and [Template Reference](templates/README.md#mvp-1-template-set) without persisted projection support. Later/full-profile, assurance, operations/export, and diagnostic report kinds remain with their projection/template owners unless an owner profile promotes them. Projection refresh must report missing source records as unavailable or not applicable rather than creating state to satisfy a template.
