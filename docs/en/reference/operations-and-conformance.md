...
| 10 | `CORE-close-evidence-missing-blocked` | `harness.close_task` | Active Task with close-compatible scope and decisions isolated from the test; Evidence Manifest absent, partial, stale, blocked, or insufficient | Task remains non-terminal; close blocker points to evidence sufficiency; no close state, acceptance, or assurance upgrade is recorded | `close_requested`, `close_blocked` | None unless existing evidence artifacts are seeded as insufficient context | `TASK` enqueued or stale if the blocked close commits blocker/status state | `EVIDENCE_INSUFFICIENT` |
| 11 | `CORE-close-decision-unresolved-blocked` | `harness.close_task` | Active Task with evidence sufficient enough not to be primary; unresolved, deferred-without-coverage, blocked, stale, or incompatible Decision Packet relevant to close | Task remains non-terminal; `decision_gate` and close blocker reference the unresolved Decision Packet; evidence state is not used to mask the decision blocker | `close_requested`, `close_blocked` | None unless the Decision Packet context uses committed artifacts | `TASK` enqueued or stale if the blocked close commits blocker/status state | `DECISION_UNRESOLVED` or `DECISION_REQUIRED` according to seeded Decision Packet state and API precedence |
