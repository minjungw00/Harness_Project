...
`harness.stage_artifact` is a local artifact utility, not a Core mutation authority. It uses `artifact_registration` to create a temporary `StagedArtifactHandle`, but it does not create canonical evidence, satisfy gates, promote a persistent `ArtifactRef`, or affect close readiness. It stages caller-provided safe bytes or safe notices; it is not proof that arbitrary local files are safe, authorized, or observed by Harness. Persistent artifact promotion happens only when a later compatible `harness.record_run` request uses `run_recording` and validates the staged handle or links a compatible existing `ArtifactRef`.
