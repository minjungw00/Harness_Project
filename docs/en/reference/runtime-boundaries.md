...
`harness.stage_artifact` is a local artifact utility, not a Core mutation authority. It uses `artifact_registration` to create a temporary `StagedArtifactHandle`, but it does not create canonical evidence, satisfy gates, promote a persistent `ArtifactRef`, or affect close readiness. It stages caller-provided safe bytes or safe notices and records `created_by_surface_id` and `created_by_surface_instance_id` from the server-derived `VerifiedSurfaceContext`; those provenance fields are not user-provided authority claims. Staging is not proof that arbitrary local files are safe, authorized, or observed by Harness. Persistent artifact promotion happens only when a later compatible `harness.record_run` request uses `run_recording` and validates the staged handle, including the same verified `surface_instance_id`, or links a compatible existing `ArtifactRef`.
