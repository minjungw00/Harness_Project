...
Product Repository files are not canonical Harness state. A product file can be an input, a changed work target, or product-owned truth about the product, but it does not become Harness operational authority merely because it is near Harness. Product Repository paths and staged artifact storage are different trust domains. Product Repository files, generated Markdown, projections, chat text, and agent memory cannot create staged handles, refresh Harness authority or local surface registration, or grant staged artifact promotion authority.
...
Runtime Home files should be treated as private local control data, but Harness does not claim to enforce operating-system permissions, make those files tamper-proof, or isolate them from arbitrary local tools by itself. Within Runtime Home, the temporary staging area used by `harness.stage_artifact` and persistent artifact storage are distinct storage roles; a staged byte is not a registered artifact body until an eligible owner path promotes it.
...
`harness.stage_artifact` is a local artifact utility, not a Core mutation authority. It uses `access_class=artifact_registration` to stage new artifact bytes or a safe notice into a temporary staging area and create a temporary `StagedArtifactHandle`, but it does not create canonical evidence, satisfy gates, promote a persistent `ArtifactRef`, or affect close readiness. It records `created_by_surface_id` and `created_by_surface_instance_id` from the server-derived `VerifiedSurfaceContext`; those provenance fields are not user-provided authority claims. Staging is not proof that arbitrary local files are safe, authorized, or observed by Harness. `StagedArtifactHandle` is a server-validated reference, not a standalone permission token or arbitrary artifact registration authority. Persistent artifact promotion happens only when a later compatible `harness.record_run` request uses `access_class=run_recording` and validates the staged handle, including project, task, same verified `surface_instance_id`, expiration, consumed status, checksum, and size, or links a compatible existing `ArtifactRef`.
...
Artifact reads use the same authority boundary. A raw artifact-store path is not granted to local callers by default. Reading artifact metadata or bytes requires a registered `ArtifactRef`, compatible project/task context, a matching owner relation, and redaction/availability checks owned by [Storage](storage.md) and [API Schema Core](api/schema-core.md#artifactref). Artifact body reads additionally require a server-verified local surface context for `access_class=artifact_read`; this is separate from staged artifact promotion during `record_run`. Sensitive-action approval and Write Authorization do not grant artifact body reads. A copied `surface_id`, projection file, generated Markdown claim, chat text, Product Repository file, or agent memory cannot authorize access to artifact bytes, create staged handles, refresh surface authority, create staged-handle provenance, or grant promotion authority.

New artifact bytes enter the active MVP only through `harness.stage_artifact` input staging, which uses `access_class=artifact_registration` to return an owner-documented `StagedArtifactHandle` with server-recorded creation surface provenance. `record_run` does not stage new bytes and does not require `artifact_registration`; under `access_class=run_recording`, it may validate and consume an eligible staged handle into a persistent `ArtifactRef` only when project, task, current verified `surface_instance_id`, expiration, consumed status, checksum, and size validation pass, or link a compatible `existing_artifact` reference. The active MVP does not support cross-surface staged artifact handoff, and `StagedArtifactHandle` is not a bearer token that any local caller may use. A raw local path, raw log, arbitrary local path string, copied file, `captured_artifact` handle, raw capture-adapter output, native capture claim, or claim that a local file is safe is not artifact authority in the active MVP.
