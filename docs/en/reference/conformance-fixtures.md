...
- Active `expected_storage_rows` may use only active Storage-owned record areas: `project_state`, `surfaces` or an equivalent reference-surface registration record, `tasks`, `task_events`, `change_units`, `user_judgments`, `write_authorizations`, `runs`, `artifacts`, `artifact_links`, `evidence_summaries` or an equivalent minimal evidence coverage record, `blockers`, and `tool_invocations`. It must not list `shared_designs`, Discovery Brief rows, Question Queue rows, Assumption Register rows, First Safe Change Unit Candidate rows, committed `approvals`, `manual_qa_records`, `evidence_manifests`, `evals`, durable `projection_jobs` or projection caches, export/recover tables, validator-run archives, rich residual-risk tables, close-readiness/status-card caches, or other later/profile table families.
- Requirements-shaping fixture assertions store shaping output only on active owner rows: `tasks`, `change_units`, `user_judgments` when a committed judgment request exists, `blockers` when Core commits a blocker, `evidence_summaries` when a minimal evidence coverage effect exists, and `runs.kind=shaping_update` for the committed shaping run. A shaping fixture must not require Shared Design storage, a Discovery Brief, a Question Queue, an Assumption Register, or a separate First Safe Change Unit Candidate record.
...
- `forbidden_side_effects` may be readable in documentation drafts, but materialized executable fixtures should expand each forbidden effect into owner-record absence, row-effect, artifact, event, projection, or generated-output assertions where practical. For failed operations, `expected_storage_rows` and `forbidden_side_effects` must agree: a fixture must not forbid a Run, artifact, replay row, evidence mutation, authorization consumption, projection job, or later/profile record while also expecting that row or effect. When absence is part of the behavior being proved, use `expected_storage_rows` table-effect assertions and compatible exact-mode metadata or explicit negative side-effect assertions.
