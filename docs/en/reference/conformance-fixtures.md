...
- You need Kernel Smoke, Agency-Hardened MVP, staged suite coverage, examples, or catalog-only future candidate guidance.
...
| `CONTEXT-HYGIENE-compact-context-loads-by-phase` | `status`, `next`, `prepare_write`, `record_run`, `launch_verify`, or `close_task` | Agent context uses the compact always-on rules and the current Intake, Discovery, Write, Evidence, Verification, or Close bundle instead of a full documentation or task-history dump. Fixtures assert pushed context is refs-first, current, and phase-relevant; larger Reference docs, schemas, historical records, and raw artifacts remain pull-on-demand and do not create new gates or authority. |
...
- context-hygiene: current-state bundle, compact phase-based context loading, stale projection and stale PRD handling, stale `TASK` projection write guard, stale chat memory and retrieved/indexed context pull-only behavior, evaluator bundle freshness, resume from current state rather than chat memory
