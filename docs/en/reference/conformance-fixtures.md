...
Use this queue as future authoring guidance for v0.1 Core Authority Slice smoke-check candidates. Kernel Smoke is the narrow authoring label for that internal runnable target, not for the product MVP and not a full conformance suite. These rows do not imply executable fixture files already exist, and they are not a v0.1 exit checklist that must be exhausted before the minimal authority loop is proven. The order is for fixture authors, not a dependency between executable fixtures; each future executable fixture should remain isolated, seed its own minimum owner records in `initial_state`, use one public Core action or a minimal registration action, and keep the exact fixture body shape unchanged.
...
Template implementation classes and freshness behavior are owned by [Document Projection](document-projection.md#template-implementation-classes) and [Freshness and failure rules](document-projection.md#freshness-and-failure-rules). Kernel Smoke should default to no projection requirement; it may assert projection freshness or enqueue/failure facts only when those facts are produced by the minimal owner path and prove the target behavior. It must not require projection-template polish, detailed report templates, or multiple projection kinds.
...
|---|---|---|---|---|---|---|---|---|
| 1 | `CORE-status-no-active-task` | `harness.status` | Registered local project; no active Task | `active_task=null`, idle/no-active status, no Write Authorization, and no state mutation | None | None | No projection enqueue; read freshness may be `unknown`, `current`, `stale`, or `failed` according to seeded projection state | None |
| 2 | `CORE-project-registration` | `connect`, project registration, or owner registration action | Empty or unregistered isolated runtime home and temporary Product Repository | One local project is registered idempotently; any minimal surface declaration is only the local caller/support record needed to reach Core; no active Task is created by registration alone | Registration event only when the owner stable event catalog defines one | None | No projection requirement | None |
...
| 9 | `CORE-record-run-consumes-authorization-registers-artifact-evidence` | `harness.record_run` with `kind=direct` or `kind=implementation` | Active Task, active scope, compatible unconsumed Write Authorization, baseline, staged artifact or evidence input | Run committed; Write Authorization consumed once; artifact/evidence ref linked to the Run or minimal owner relation; status/blocker output can cite the ref or report missing support | `run_recorded`, `write_authorization_consumed`, and evidence/artifact event only when promoted by owner catalog | Registered `ArtifactRef` or owner evidence ref with integrity/redaction metadata and a same-Task owner link | No projection requirement; `TASK` stale/enqueued is allowed if state changes | None |
...
| 11 | `CORE-status-blocker-read-only-current-state` | `harness.status`, or `harness.next` if implemented for the smoke path | Active Task with current scope, write authority summary, artifact/evidence support, and optional blockers | Read returns current Task, scope, write authority, artifact/evidence support, and blockers; no state mutation, no gate satisfaction, no Write Authorization | None | None | No projection enqueue from read-only status/blocker output | None |
| 12 | `CORE-status-or-close-smoke-structured-blocker` | `harness.status`; `harness.close_task` only when the implementation chooses a narrow close-task smoke | Active Task with compatible scope for the smoke path; artifact/evidence support absent or insufficient, or write authority missing | Task remains non-terminal; structured blocker points to missing scope, missing write authority, or missing artifact/evidence support; no close state, final acceptance, residual-risk acceptance, or assurance upgrade is recorded | `close_requested`, `close_blocked` only when `close_task` commits blocker/status state | None unless existing evidence context uses committed artifacts | No projection requirement; `TASK` stale/enqueued is allowed if blocker/status state commits | `EVIDENCE_INSUFFICIENT`, `WRITE_AUTHORIZATION_REQUIRED`, `SCOPE_REQUIRED`, or owner-equivalent structured blocker error |

The rows above are candidate decompositions of the Core Authority Slice smoke path. They are not a required full v0.1 conformance suite; an implementation plan may use the smallest subset that proves the one authority loop named by Build. v0.2 User-Facing Harness MVP and later packs keep the same fixture body shape and extend coverage through the staged catalog families below, especially ordinary-language routing, procedural budget, Decision Packet quality, approval separation, residual-risk visibility, final acceptance, surface honesty, artifact redaction/export, stewardship/design-quality, context hygiene, reconcile, detached verification, and Manual QA.
...
The examples below are future exact-shape examples for Core behavior broadly. They may exceed the minimal v0.1 Kernel Smoke subset; use the [Kernel Smoke Authoring Queue](#kernel-smoke-authoring-queue) and Build scope when deciding what the first Core Authority Slice must prove.
...
Future suite families group under the fixture profiles above. The `core` family below is broader than the v0.1 Core Authority Slice smoke subset; v0.1 uses only the minimal authority-loop checks called out in Build and the Kernel Smoke queue.
