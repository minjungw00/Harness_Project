...
| `MVP1-focused-judgment-request` | When user-owned judgment is required, Core records or returns one focused user judgment request with `judgment_kind`, `presentation`, `display_label`, options/consequences, and related refs; it does not dump a broad questionnaire or choose for the user. |
