...
For the user-facing agent procedure, read [Agent Guide](../use/agent-guide.md). For surface-specific setup notes, read [Surface Cookbook](surface-cookbook.md).
...
Read [Agent Guide](../use/agent-guide.md) for the user-facing procedure. For exact authority, consult only the relevant owner section when the current phase needs it: [Core Model Reference](core-model.md) for write and close authority, [MVP API](api/mvp-api.md) for active MVP-1 method contracts, [API Schema Core](api/schema-core.md) for shared API shapes, and [Security Reference](security.md) for MCP exposure, generated-file, stale-context, artifact, secret, and capability-overclaiming threats. This reference explains connector behavior and capability display, not kernel state transitions, and it is not an instruction to load all Reference docs by default.
...
| Public MCP request/response schemas | [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md), [API Errors](api/errors.md), and [API Schema Later](api/schema-later.md) |
| Kernel state transitions and write/close rules | [Core Model Reference](core-model.md) |
| Guarantee level meanings and security control expectations | [Security Reference](security.md#honest-guarantee-display) |
| Runtime placement for guarantee display | [Runtime Architecture Reference](runtime-architecture.md#guarantee-levels) |
| Security assets, trust boundaries, threat categories, and control categories | [Security Reference](security.md) |
...
The security reason for these fields is owned by [Security Reference](security.md); this reference owns how connector profiles report them.
...
Integration uses the guarantee levels defined in [Security Reference](security.md#honest-guarantee-display) and applies them to connected surface profiles, current enforcement paths, and fallback choices.
...
Stage display defaults mirror the [Security Threat Model stage map](security.md#guarantee-levels-by-stage):
...
Connector context uses a three-part model:

- User status card: a short readable current-state view for the user. It shows where the work stands, what blocks progress, what the next safe action is, and whether the view is fresh.
- Agent context packet: the minimal current state, refs, freshness, and owner-section pointers needed for the agent's next safe action. It is the always-on connector payload target.
- Core state: the local authority record and the only operational source of truth. It owns state changes, gates, user judgments, evidence refs, Write Authorization records, work acceptance, residual-risk acceptance, and close.

The always-on agent context packet should fit on one screen or less. It includes only:
...
A field may be `none`, `unknown`, or a one-line ref summary when the item is absent or not relevant. The connector should not add broader role text, full phase history, schema detail, or procedure text to the always-on packet by default; those remain connector setup or phase-specific pull context.

Always-on injection is prohibited from including full reference docs, full public schemas, full Storage DDL, complete history, the full historical event log, full projection bodies, full artifact contents, raw logs/screenshots/diffs/traces, full templates, unrelated templates, Future Fixtures or other future-catalog content, old task history, or unrelated Roadmap material. Those stay pull-on-demand for the exact phase and next action that needs them.

Stale agent memory, stale chat history, remembered recommendations, pulled context, stale status cards, rendered templates, and stale projections may point the agent toward refs to inspect, but they cannot authorize writes, satisfy gates, close tasks, create evidence, create approval, accept results, waive QA or verification, accept residual risk, create close readiness, replace current owner records, or repair stale projections. Status cards and projections can summarize state and refs, but they are not authority. When state matters, the connector should retrieve current Core state or an agent context packet derived from current Core state; any older context that matters to authority must first be refreshed or reconciled through the owning Core path.

Keep the static always-on rule compass to these ten items or fewer:

1. Read current status or an agent context packet before significant work or resume; use Journey Card only when that projection/profile is enabled and fresh.
2. Product/runtime/code writes require compatible `prepare_write` and Write Authorization.
3. User-owned product or material technical judgment routes through user judgment requests.
4. Approval is not product judgment, work acceptance, or residual-risk acceptance.
5. User status cards and projections are readable output, not canonical state.
...
10. Pull Reference docs, schemas, historical records, templates, later-profile resource bodies, and large artifacts only when the next action needs them.

Token savings must not starve the agent of user-owned judgments, blockers, scope limits, safety boundaries, evidence gaps, close blockers, or close-relevant residual-risk information needed for correct behavior. The user-visible part of a judgment request should stay focused: the decision, concise options or chosen outcome, consequences, uncertainty, and what the agent is not deciding for the user. Add only the smallest affected-scope refs needed to identify the decision; put broader schema detail, evidence bodies, logs, templates, and route metadata behind pull/drill-down or full-format presentation when an active profile requires it.

These are the compact current-state packet fields. They are not a public schema and do not prove that a context API is implemented; they define the compactness target for future connector payloads and prompt-sized context.

Always-on packet contract:
...
| Pending user judgments | User judgment refs and one-line questions, or `none`. |
...
- Write Authorization, sensitive-action permission refs, sensitive-action approval user judgment refs in minimum MVP-1, Approval refs only when the later Approval profile is active, `evidence_ref` refs and derived evidence summaries, Evidence Manifest refs only when that profile is active, Eval, Manual QA, work-acceptance, Run, report, artifact, and residual-risk refs
...
Refs-first means the connector should push stable ids, paths, hashes, summaries, outcomes, and freshness, not paste large bodies into the default prompt. Embed excerpts only when the next safe action requires inspecting the content, and keep the excerpt tied to its source ref. Retrieved, indexed, remembered, or summarized context follows the same rule: it can tell the agent what to inspect next, but it remains pull-only context until an owner path records an actual state change. It must not authorize writes or create Write Authorization, resolve user judgments, grant Approval, satisfy gates, create evidence, perform or record verification, record QA, waive QA or verification, accept results, accept residual risk, update projection freshness, or close tasks.

Use context profiles so agents do not load the whole documentation set. Each profile narrows both the current state packet and the owner sections the connector may pull. MCP resource pulls follow the staged [Read-only resources](api/schema-core.md#read-only-resources) map: Engineering Checkpoint uses only the current project/current task/status subset, MVP-1 adds user-judgment reads only when user judgment context is needed, and Assurance Profile/Operations Profile/future resources stay profile-gated or pull-on-demand. A connector may push refs, one-line summaries, and freshness markers from enabled resources, but it should not inject full resource outputs by default. The default exclusions apply to every profile unless that exact phase and next action require a specific section: full [Storage](storage.md) DDL, the full [Conformance Fixtures Reference](conformance-fixtures.md) or [Future Fixtures](../later/future-fixtures.md), any future catalog, the full [Template Reference](templates/README.md) set, unrelated [Roadmap](../roadmap.md) items, old task history, historical event logs, complete history, full projection bodies, old projections, full artifact contents, raw logs/screenshots/diffs/traces, full MCP schemas, and full Reference documents.
...
|---|---|---|---|---|---|
| Session start | Current Task summary or explicit `none`/`unknown`, likely work shape, scope/non-goals when known, active blockers, pending user judgments, next safe action, evidence gaps, close blockers, residual-risk summary, guarantee level, and source/freshness refs. | [Agent Guide: Session start](../use/agent-guide.md#session-start), [Resume](../use/agent-guide.md#resume), current `harness.status` / `harness.status.next_actions`, later/compatibility `harness.next` only when that profile includes it, and [Document Projection: Freshness and failure rules](projection-and-templates.md#freshness-and-failure-rules) only when projection freshness affects the next action. | Full task history, full reference docs, full schemas, old projections, unrelated templates, unrelated Roadmap, future catalog. | Show the plain current position, primary blocker, pending user judgment, next safe action, and what the surface can block or only detect. | Read current Core status or a state-derived agent context packet when state matters. Use Journey Card only when that projection/profile is enabled and fresh. If Core/MCP is unavailable, say so instead of using memory as state. |
| Planning/clarification (Discovery) | Goal, user value when known, scope and non-goals, acceptance cues, answerable facts inspected from repo/docs/current state, missing information, blocking questions, useful non-blocking questions, assumptions, tracked uncertainty, user-owned judgment candidates, active blockers, and next allowed clarification action. | [User Guide: What the agent should answer first](../use/user-guide.md#what-the-agent-should-answer-first), [Agent Guide: Intake](../use/agent-guide.md#intake), [Scope and Write Boundary](../use/agent-guide.md#scope-and-write-boundary), and relevant current Task/Change Unit/Shared Design refs. Pull [Kernel: Judgment route boundaries](core-model.md#judgment-route-boundaries) only when a user-owned route must be separated. | Whole module maps, old PRDs/designs, design-policy catalogs, unrelated templates, full Storage DDL, full Conformance catalog, future catalog. | Show answerable facts, the next blocking question when any, useful non-blocking questions parked for later, recommendation, uncertainty, deferral effect, and what can safely continue. | Inspect current repo/docs/state refs before asking. Mark unavailable or stale sources and do not turn stale design prose, chat memory, or projections into authority. |
| Write preparation | Active Task and Change Unit, intended paths/tools/commands/network/secrets summary, scope and out-of-bounds areas, Autonomy Boundary, pending judgments or sensitive-action permission needs, later Approval needs only when that profile is active, baseline/state freshness, Write Authority Summary, guarantee/MCP availability. | [Agent Guide: Product writes](../use/agent-guide.md#product-writes), [Kernel: prepare_write](core-model.md#prepare_write), [Kernel: Write Authorization](core-model.md#write-authorization), and [`harness.prepare_write`](api/mvp-api.md#harnessprepare_write) for the relevant method only. Pull security owner sections only for the sensitive category at issue. | Full Kernel/reference docs, unrelated schemas, historical event logs, large diffs/logs, full Storage DDL, future catalog. | Show what would change, what is blocked, write-authority status, and the smallest unblocker. | Use current Core state and exact `prepare_write` inputs. Refresh or reconcile on stale baseline, stale projection, state conflict, changed intended paths, or changed sensitive category. |
| Execution/run recording | Consumed Write Authorization or no-write basis, changed-path summary, command/tool summary, run outcome, artifact refs with integrity/freshness, redaction/omission/block notes, and immediate next action. | [Agent Guide: Evidence and checks](../use/agent-guide.md#evidence-and-checks), [Kernel: record_run](core-model.md#record_run), [`harness.record_run`](api/mvp-api.md#harnessrecord_run), and [Document Projection: Artifact reference rendering](projection-and-templates.md#artifact-reference-rendering) only for display or artifact-ref questions. | Full logs, raw diffs, screenshots, traces, bundles, artifact inventories, full projection bodies, full Template set, future catalog. | Show what ran, what changed, what was recorded, and what was redacted, omitted, blocked, stale, or unsupported. Keep raw artifact bodies pull-only. | Record from current run/artifact refs and current state. Do not present an audit or violation Run as satisfying evidence, QA, verification, work acceptance, or close. |
| Evidence review | Known evidence summary, `evidence_ref` when present, Run refs, ArtifactRefs, visible evidence gaps, stale or insufficient support, redaction/integrity notes, affected acceptance criteria or claims, and next evidence action. Include an Evidence Manifest ref only when the full Evidence Manifest profile is active. | [Agent Guide: Evidence and checks](../use/agent-guide.md#evidence-and-checks), [`harness.record_run`](api/mvp-api.md#harnessrecord_run), artifact-ref display rules, and [Kernel: Evidence Manifest](core-model.md#evidence-manifest) only when that profile is active. | Full evidence bodies, full logs, raw diffs, screenshots, traces, bundles, artifact inventories, full projection bodies, full Template set, future catalog. | Show known coverage and gaps tied to criteria or claims, with refs and redaction/integrity notes. | Mark evidence stale when baseline, paths, sensitive-action permission / Approval, artifact integrity, or relevant owner records changed. A visible artifact ref is not evidence sufficiency by itself. |
| User judgment request | Exact user-owned judgment, `judgment_type`, `presentation`, display label, concise options or chosen outcome, consequences, uncertainty, and what the agent is not deciding for the user. Include only the smallest affected-scope refs needed to identify the decision. | [Agent Guide: Blocking User-Owned Judgments](../use/agent-guide.md#blocking-user-owned-judgments), [Kernel: User Judgment](core-model.md#user-judgment), [Decision Gate](core-model.md#decision-gate), and [`harness.request_user_judgment`](api/mvp-api.md#harnessrequest_user_judgment) only when exact API fields are needed. | Broad approval language, unrelated judgments, full evidence bodies, full logs, full schema references, full Template set, future catalog. | Show the focused decision: choice, options, consequences, uncertainty, and what the agent is not deciding. Put broader trade-offs, recommendation, affected gates/criteria, route metadata, and evidence bodies behind pull/drill-down or full-format presentation when active. | Source from a current user judgment or current state-derived judgment candidate. If a ref is stale or missing, label it before asking. Replay ids, routing metadata, and enum detail stay secondary unless they clarify the boundary. Token saving must not remove the minimal context the user needs to decide. |
| Close readiness | Scope match, acceptance criteria, evidence coverage, verification status, Manual QA status, work-acceptance need/status, residual-risk visibility and accepted refs when relevant, close blockers, projection freshness, and smallest unblocker. | [Agent Guide: Close](../use/agent-guide.md#close), [Verification, Manual QA, Residual Risk, Work Acceptance](../use/agent-guide.md#verification-manual-qa-residual-risk-work-acceptance), [Kernel: close_task](core-model.md#close_task), [Gates](core-model.md#gates), [Waiver semantics](core-model.md#waiver-semantics), [`harness.close_task`](api/mvp-api.md#harnessclose_task), and [Document Projection: Freshness and failure rules](projection-and-templates.md#freshness-and-failure-rules) when readable close context is used. | Generic all-done rollups, full report bodies, full historical logs, unrelated templates, full Conformance catalog, full projection bodies. | Show the close basis before acceptance or close: scope, user judgments, evidence, verification, Manual QA, residual risk, acceptance status, primary blocker, and smallest unblocker. | Read current Core gates, owner records, evidence/artifact refs, and projection freshness. A stale projection can summarize a blocker but cannot become authority. Exact `close_task` payloads stay internal unless a blocker needs exact detail. |
| Recovery/error | Primary error or blocker, owner, last safe/current state known, stale or unavailable source, affected authority claims, next recovery action, and whether writes/close must hold. | [Agent Guide: Resume](../use/agent-guide.md#resume), [Reading status and blockers](../use/agent-guide.md#reading-status-and-blockers), [Fallback Semantics](#fallback-semantics), [API Errors: Error taxonomy](api/errors.md#error-taxonomy), [State conflict behavior](api/errors.md#state-conflict-behavior), and [Document Projection: Freshness and failure rules](projection-and-templates.md#freshness-and-failure-rules) when stale readable context is involved. Pull specific operations/recovery owner sections only when operator repair is the next action. | Historical event logs, stack traces, full artifacts, unrelated status, full Storage DDL, full Conformance catalog, unrelated Roadmap. | Show the plain blocker, owner, affected authority claim, smallest recovery step, and whether writes or close must hold. | Re-read Core when possible. If Core is unreachable, do not invent state from agent memory, chat history, cached projection, old status text, or operator prose. |
...
Context profiles are context discipline, not new schemas or gates. Moving from one phase to another changes what the connector pushes by default; it does not authorize writes, resolve decisions, create evidence, perform verification, create approval, accept work, accept risk, or close a Task.

MVP-1 uses five compact views: `status-card` for "where are we," `agent-context-packet` for the next safe action context, `judgment-request` when user judgment is needed, `run-evidence-summary` for minimal run/evidence support, and `close-result` for close readiness and blockers. Keep each view compact and profile-filtered. Route metadata, broad refs, recommendations, and deferral effects are pull/drill-down or full-format material when the active profile requires them; full evidence or artifact bodies never become always-on context.

Status, next-action, recommendation, rendered-template, and recommended-playbook outputs are read-only guidance. They may recommend `prepare_write`, a user judgment request, optional full-format Decision Packet presentation, a Change Unit update, evidence collection, verification, QA, reconcile, or close attempt, but they do not mutate state, satisfy gates, authorize writes, create evidence, create approval, perform verification, record Manual QA, waive QA or verification, record work acceptance, record residual-risk acceptance, create close readiness, close Tasks, or upgrade assurance by themselves. State effects happen only when the recommended action later runs through the existing MCP/Core mutation path.

Evaluators should receive a tighter verification bundle: acceptance criteria, changed files, approval scope, relevant user judgments, residual risk summary, Autonomy Boundary, deferred judgments, codebase stewardship refs, `evidence_ref` refs and derived evidence summaries, Evidence Manifest refs only when that profile is active, required TDD trace refs, Manual QA requirement, artifact refs, freshness state, and forbidden patterns.
...
If MCP works but pre-tool guard is weak, low-risk direct work may proceed with cooperative `prepare_write` and detective changed-path validation. Medium/high-risk work must not rely on cooperative-only claims when the assessed threat/control path requires preventive or isolated controls. The [Security Threat Model](security.md) names the security reason; connector profiles, operations, API, kernel, and conformance owners define the exact behavior.
...
- a user judgment candidate or a route to an existing user judgment
...
AFK, unattended, or "continue while I am away" instructions are connector display and posture concerns; they do not create new authority. A connector should keep AFK work inside the active Change Unit, active Autonomy Boundary, granted sensitive-action permission, and compatible `prepare_write` / Write Authorization before actual product writes. Minimum MVP-1 uses a sensitive-action approval user judgment for that permission; later Approval profiles may use Approval records.
...
- `T2 MCP` available for the Engineering Checkpoint public tool/resource subset needed by the Engineering Checkpoint, including only the current project/current task/status resources needed for the first authority loop, not the full later-profile MCP surface documented in [API Schema Later](api/schema-later.md)
...
|---|---|
| MVP-1 User Work Loop | User-readable status/next cards, compact user judgment display, pending user judgment routing, evidence and close readiness summaries, work-acceptance separation, and residual-risk visibility when relevant. |
...
- Autonomy Boundary breach stops or reports a structured blocker; full-format Decision Packet presentation is later-profile unless that profile is enabled
...
- status recommendations remain read-only guidance unless the recommended action follows the existing Core mutation path; separate `harness.next` recommendations are later/compatibility behavior only when that method is included in an active profile
...
- user judgment request shown instead of broad approval for blocking user-owned judgment
- public commitments route to user judgment or another existing owner path when they require user-owned product or material technical judgment
...
- sensitive-action permission request, granted, denied, and expired paths; minimum MVP-1 uses sensitive-action approval user judgments, while later Approval profiles may use Approval records
...
- QA waiver with product/user risk routes through user judgment
...
- risk-accepted close requires a residual-risk acceptance user judgment and related blocker/evidence refs in MVP-1; rich Residual Risk refs apply only when that profile is active
