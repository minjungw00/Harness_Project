...
|---|---|---|---|---|---|
| Session start | Role or connected surface posture, current phase/context profile, active or likely Task summary, active blockers, pending user-owned judgments, next allowed action, guarantee/MCP availability, and projection freshness only when a projection is used. | [Agent Session Flow: Session start](../use/agent-session-flow.md#session-start), [Resume](../use/agent-session-flow.md#resume), current `harness.status` / `harness.next` output, and [Document Projection: Freshness and failure rules](document-projection.md#freshness-and-failure-rules) only when projection freshness affects the next action. | Full task history, full reference docs, full schemas, old projections, unrelated templates, unrelated Roadmap. | Show the plain current position, primary blocker, pending user decision, next safe action, and what the surface can block or only detect. | Read current Core status or state-derived current-position context when state matters. Use Journey Card only when that projection/profile is enabled and fresh. If Core/MCP is unavailable, say so instead of using memory as state. |
| Requirements clarification (Discovery) | Goal, user value when known, scope and non-goals, acceptance cues, inspectable facts, assumptions, tracked uncertainty, user-owned judgment candidates, active blockers, and next allowed clarification action. | [User Guide: What the agent should answer first](../use/user-guide.md#what-the-agent-should-answer-first), [Agent Session Flow: Intake](../use/agent-session-flow.md#intake), [Scope and Change Unit](../use/agent-session-flow.md#scope-and-change-unit), and relevant current Task/Change Unit/Shared Design refs. Pull [Kernel: Judgment route boundaries](kernel.md#judgment-route-boundaries) only when a user-owned route must be separated. | Whole module maps, old PRDs/designs, design-policy catalogs, unrelated templates, full Storage DDL, full Conformance catalog. | Show focused questions with recommendation, uncertainty, deferral effect, inspectable facts, and what can safely continue. | Inspect current repo/docs/state refs before asking. Mark unavailable or stale sources and do not turn stale design prose, chat memory, or projections into authority. |
| Decision request | Exact user-owned decision, decision profile, judgment domain, profile-appropriate options or chosen outcome, affected scope, relevant refs or explicit absence, what the answer does not settle, and next action after the answer. Full profiles also show trade-offs, recommendation, uncertainty, affected gates/acceptance criteria, and consequence of deferral. | [Agent Session Flow: Blocking User-Owned Decisions](../use/agent-session-flow.md#blocking-user-owned-decisions), [Kernel: Decision Packet](kernel.md#decision-packet), [Decision Gate](kernel.md#decision-gate), and the [`harness.request_user_decision`](mcp-api-and-schemas.md#harnessrequest_user_decision) section only when exact API fields are needed. | Broad approval language, unrelated decisions, full evidence bodies, full logs, full schema references, full Template set. | Show enough context for informed judgment: exact choice, options, recommendation when required, scope, refs, uncertainty, and what the answer does not settle. | Source from a current Decision Packet or current state-derived decision candidate. If a ref is stale or missing, label it before asking. Replay ids, routing metadata, and enum detail stay secondary unless they clarify the boundary. |
| Prepare-write | Active Task and Change Unit, intended paths/tools/commands/network/secrets summary, scope and out-of-bounds areas, Autonomy Boundary, pending decisions or Approval needs, baseline/state freshness, Write Authority Summary, guarantee/MCP availability. | [Agent Session Flow: Product writes](../use/agent-session-flow.md#product-writes), [Kernel: prepare_write](kernel.md#prepare_write), [Kernel: Write Authorization](kernel.md#write-authorization), and [`harness.prepare_write`](mcp-api-and-schemas.md#harnessprepare_write) for the relevant method only. Pull security owner sections only for the sensitive category at issue. | Full Kernel/reference docs, unrelated schemas, historical event logs, large diffs/logs, full Storage DDL. | Show what would change, what is blocked, write-authority status, and the smallest unblocker. | Use current Core state and exact `prepare_write` inputs. Refresh or reconcile on stale baseline, stale projection, state conflict, changed intended paths, or changed sensitive category. |
| Run/evidence | Consumed Write Authorization or no-write basis, changed-path summary, command/tool summary, run outcome, Evidence Manifest ref, artifact refs with integrity/freshness, evidence gaps, redaction/omission/block notes, and next evidence action. | [Agent Session Flow: Evidence and checks](../use/agent-session-flow.md#evidence-and-checks), [Kernel: record_run](kernel.md#record_run), [Kernel: Evidence Manifest](kernel.md#evidence-manifest), [`harness.record_run`](mcp-api-and-schemas.md#harnessrecord_run), and [Document Projection: Artifact reference rendering](document-projection.md#artifact-reference-rendering) only for display or artifact-ref questions. | Full logs, raw diffs, screenshots, traces, bundles, artifact inventories, full projection bodies, full Template set unless a specific display repair needs one. | Show coverage and gaps tied to criteria or claims, with refs and redaction/integrity notes. Keep raw artifact bodies pull-only. | Record from current run/artifact refs and current state. Mark evidence stale when baseline, paths, approval, artifact integrity, or relevant owner records changed. |
| Close readiness | Scope match, acceptance criteria, evidence coverage, verification status, Manual QA status, final acceptance need/status, residual-risk visibility and accepted refs when relevant, close blockers, projection freshness, and smallest unblocker. | [Agent Session Flow: Close](../use/agent-session-flow.md#close), [Verification, Manual QA, residual risk, acceptance](../use/agent-session-flow.md#verification-manual-qa-residual-risk-acceptance), [Kernel: close_task](kernel.md#close_task), [Gates](kernel.md#gates), [Waiver semantics](kernel.md#waiver-semantics), [`harness.close_task`](mcp-api-and-schemas.md#harnessclose_task), and [Document Projection: Freshness and failure rules](document-projection.md#freshness-and-failure-rules) when readable close context is used. | Generic all-done rollups, full report bodies, full historical logs, unrelated templates, full Conformance catalog, full projection bodies. | Show the close basis before acceptance or close: scope, user decisions, evidence, verification, Manual QA, residual risk, acceptance status, primary blocker, and smallest unblocker. | Read current Core gates, owner records, evidence/artifact refs, and projection freshness. A stale projection can summarize a blocker but cannot become authority. Exact `close_task` payloads stay internal unless a blocker needs exact detail. |
...
|---|---|
| v0.2 User-Facing Harness MVP | User-readable status/next cards, Decision Packet display, pending user judgment routing, evidence and close readiness summaries, final-acceptance separation, and residual-risk visibility when relevant. |
