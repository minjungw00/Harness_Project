...
| `harness.close_task intent=check` | Read-only | Yes; returns read-only result | No | No; may be `null` | No; close blockers are computed response fields only | No | No | No |
...
| `intent` | API behavior |
|---|---|
| `check` | Read-only. Returns `CloseTaskResult` with `base.effect_kind=read_only`, computes close readiness and blockers without storing blockers, events, replay rows, close state, artifact creation, update, or linking, staged-handle consumption, Write Authorization creation or consumption, or a state-version increment. If `dry_run=true`, it still returns `CloseTaskResult` with `base.dry_run=true` and `base.effect_kind=read_only`. `close_reason` must be `null`. |
...
- **Dry run and branch selection:** `harness.close_task` is a mixed-intent method, so the response branch is chosen by the selected intent's state effect, not by the method name alone. `intent=check` with `dry_run=true` returns `CloseTaskResult` with `base.dry_run=true` and `base.effect_kind=read_only`. `intent=complete`, `intent=cancel`, or `intent=supersede` with `dry_run=true` returns `ToolDryRunResponse` when the request is otherwise valid. Its `DryRunSummary` may preview terminal or blocked close effects, close blockers, and next actions, but it does not change close state, Task lifecycle, blockers, events, replay rows, artifact creation, update, or linking, staged-handle consumption, Write Authorization creation or consumption, or `state_version`.
