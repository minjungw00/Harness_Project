...
- **Write decision reasons:** `PrepareWriteResult.write_decision_reasons` contains prepare_write decision reasons. It is `[]` when `decision=allowed`. It is non-empty when `decision=blocked`, `decision=approval_required`, or `decision=decision_required`. These reasons are `WriteDecisionReason` values, not `CloseBlocker` values and not close matrix results.
- **Close boundary:** `harness.prepare_write` does not create `CloseBlocker`, does not change `close_state`, does not run the close matrix, and does not create `close_task` replay rows.
...
Before any close matrix execution or terminal close commit, Core performs close preflight. Preflight checks request validation, local access, `expected_state_version` freshness when required or supplied, `idempotency_key` replay lookup and request hash compatibility for replay-row-creating intents, Project/Task identity, and any close-relevant `WriteAuthorization.basis_state_version` freshness against current `project_state.state_version`. A preflight failure returns `ToolRejectedResponse` with `effect_kind=no_effect`, not `CloseTaskResult`. It creates no `CloseBlocker`, no `task_event` or `task_events` append, no replay row, no `tool_invocations.response_json`, no `close_state` mutation, no Write Authorization creation or consumption, no staged-handle consumption, no artifact promotion or link, no evidence update, and no `project_state.state_version` increment. `STATE_VERSION_CONFLICT` may appear for `harness.close_task` only as a `ToolRejectedResponse` preflight error; it must never be `CloseBlocker.code`.

`CloseTaskResponse` branch use is intent-specific:

- `ToolRejectedResponse`: failure before close matrix execution.
- `CloseTaskResult(close_state=blocked)`: valid close matrix execution with committed semantic blockers.
...
| `complete` | After close preflight succeeds, runs the ordered complete blocker matrix from [Core Model](../core-model.md#close_task). If no blocker remains, stores `lifecycle_phase=completed`, `result=completed`, and the derived `close_reason`. A blocked complete that commits blocker state returns `CloseTaskResult` with `close_state=blocked`. |
...
For `intent=complete`, after close preflight succeeds, the API response reflects the ordered Core blocker matrix through `CloseTaskResult.blockers`. The first unsatisfied row supplies the primary close-blocker basis; later rows may still appear as secondary blockers, but they cannot satisfy or hide earlier rows.
...
`STATE_VERSION_CONFLICT` is outside the complete close blocker matrix. It is not a public error path for `write_compatibility`, `recovery`, or any `CloseBlocker.code`; stale `expected_state_version`, stale `WriteAuthorization.basis_state_version`, and `idempotency_key` request hash conflict are `ToolRejectedResponse` preflight cases only.
...
- **State effect:** `intent=check` is always read-only, including when the envelope has `dry_run=true`. It creates no `task_events`, replay rows, `project_state.state_version` increment, `close_state` mutation, artifact update, staged-handle consumption, or Write Authorization creation/consumption. A committed non-dry-run terminal close updates `tasks.lifecycle_phase`, `tasks.close_reason`, `tasks.result`, `tasks.closed_at`, affected `change_units`, blockers, project active-task state when needed, events, replay, and `project_state.state_version` exactly once. A committed blocked `CloseTaskResult` may create only the blocker, event, replay-row, and `project_state.state_version` effects allowed by the method contract, and it must leave the Task open. Close preflight rejection returns `ToolRejectedResponse` with `effect_kind=no_effect` and creates no `CloseBlocker`, no `task_event` or `task_events` append, no replay row, no `tool_invocations.response_json`, no `close_state` mutation, no Write Authorization creation or consumption, no staged-handle consumption, no artifact promotion or link, no evidence update, and no `project_state.state_version` increment.
- **Dry run and branch selection:** `harness.close_task` is a mixed-intent method, so the response branch is chosen by the selected intent's state effect, not by the method name alone. `intent=check` with `dry_run=true` returns `CloseTaskResult` with `base.dry_run=true` and `base.effect_kind=read_only`; it must not return `ToolDryRunResponse`. `intent=complete`, `intent=cancel`, or `intent=supersede` with `dry_run=true` returns `ToolDryRunResponse` when the request is otherwise valid and previewable. Its `DryRunSummary` may preview terminal or blocked close effects, expected close blockers, and next actions. Expected close blockers are represented only as `PlannedBlocker` entries in `DryRunSummary.would_blockers` with `source_kind=close_matrix`; the preview does not store or return real `CloseBlocker` objects. It does not change close state, Task lifecycle, blockers, events, replay rows, artifact creation, update, or linking, staged-handle consumption, Write Authorization creation or consumption, or `state_version`.
