...
- **Access class:** Always requires `VerifiedSurfaceContext.access_class=run_recording` and only that request access class. `ArtifactInput[]` does not add `artifact_registration`; staged artifact promotion during `record_run` is authorized by `run_recording` plus staged-handle validity checks.
- **Artifact inputs:** New artifact bytes must already be represented by a valid `StagedArtifactHandle` created by `harness.stage_artifact`; `record_run` does not stage new bytes. That handle records input staging, not native capture or arbitrary local-file authorization. Existing bytes may be reused only through a compatible `existing_artifact_ref`. Invalid `source_kind`/source-field shape is rejected with `VALIDATION_FAILED`. Missing, expired, already-consumed, mismatched, cross-project, cross-Task, or integrity-incompatible staging handles are rejected before mutation with `ARTIFACT_MISSING` and a specific `ToolError.details.staging_handle_reason`.
