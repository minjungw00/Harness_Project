...
| `artifact_registration` | `ArtifactInput[]` accepted by `harness.record_run`. | `run_recording` conditions plus documented `staged_file` or `existing_artifact` handles only. Caller-supplied raw filesystem paths, raw secrets, tokens, full sensitive logs, and native capture-adapter outputs are not accepted as registration authority in the baseline profile. |
