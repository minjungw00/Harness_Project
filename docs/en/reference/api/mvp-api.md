...
Method state effects are fixed by this matrix. "Event", "replay row", and
"state version increment" mean a new committed non-dry-run mutation; idempotent
replay returns the existing committed response and does not create a second
event, replay row, or version increment.
...
| `harness.record_user_judgment` | Mutating | Yes; never commits | Yes for non-dry-run | Yes for non-dry-run | Yes, when the addressed judgment is committed as rejected, deferred, blocked, denied, or otherwise blocker-producing | Yes, on commit | Yes, on first commit | Yes, on commit |
...
`dry_run=true` is never authoritative. It may return diagnostics, candidate blockers, or a would-change result, but it creates no current record, `task_events` row, artifact, consumable Write Authorization, evidence summary, close state, or idempotency replay row.
...
| `artifact_registration` | `ArtifactInput[]` accepted by `harness.record_run`. | `run_recording` conditions plus documented `staged_file`, `captured_artifact`, or `existing_artifact` handles only. `captured_artifact` requires an owner-documented captured-handle path and active surface support for presenting that handle. Caller-supplied raw filesystem paths, raw secrets, tokens, full sensitive logs, raw capture-adapter outputs, and native capture claims are not accepted as registration authority in the baseline profile. |
