...
Every public tool response is exactly one response branch. A caller's `dry_run=true` flag does not by itself select the `ToolDryRunResponse` branch. Method-specific success fields attach only to a method-specific `MethodResult` branch built on `ToolResultBase`; `ToolRejectedResponse` and `ToolDryRunResponse` must not invent success-only fields such as a write decision, run summary, or staged artifact handle.
...
`ToolResultBase` is the base for actual method results. `ToolResultBase.dry_run` may be `true` only when the caller sent `dry_run=true` and the selected operation is still a real `MethodResult` because it is read-only. In that case the response remains the method-specific `MethodResult` with `response_kind=result` and `effect_kind=read_only`; current MVP examples are `harness.status` and `harness.close_task` with `intent=check`. Actual method results that commit Core state or create staging use `dry_run=false`, because valid dry-run previews for those selected operations use `ToolDryRunResponse`. Its `state_version` is the project-wide version: the resulting `project_state.state_version` after a committed Core mutation, or the current project-wide version observed for read-only results and temporary staging results. `effect_kind=staging_created` means a temporary staged artifact handle was created by `harness.stage_artifact`; it is not a Core state transition, event, replay row, or `state_version` increment.

`ToolRejectedResponse` is the response for pre-commit failures, including `STATE_VERSION_CONFLICT`, request validation failure, unavailable Core or local MCP surface, local access failure, capability failure, and invalid staged artifact handle. It takes precedence regardless of the `dry_run` flag: if the request fails before any Core commit or staging side effect, the response is `ToolRejectedResponse`, not a method-specific `MethodResult` and not `ToolDryRunResponse`. It has `effect_kind=no_effect`, contains no method-specific result-only fields, and has no state effect: no current record, event, artifact, evidence summary, Write Authorization creation or consumption, close state, `tool_invocations` replay row, staged-handle consumption, or `state_version` increment. `ToolRejectedResponse.errors` is always non-empty. `ToolRejectedResponse.events` is always `[]`.
