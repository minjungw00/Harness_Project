...
|---|---|
| `ToolError` | Public error details in an `errors` array. `ToolRejectedResponse.errors` is the only place for pre-commit failure codes, including request rejection before any commit, staging side effect, replay row, close matrix evaluation, or state-version increment. |
...
| `CloseBlocker` | The committed `close_task` close-matrix blocker returned in `CloseTaskResult.blockers` when `close_state=blocked` after close preflight succeeds and the close matrix finds semantic blockers. |
| `PlannedBlocker` | Dry-run preview data returned only in `DryRunSummary.would_blockers` for expected blockers. It is not stored and is not a committed decision or close blocker. |

These structures must not be substituted for one another. `prepare_write decision=blocked`, `decision=approval_required`, and `decision=decision_required` use `WriteDecisionReason`, not `CloseBlocker`. `close_task close_state=blocked` uses `CloseBlocker`, not `WriteDecisionReason`. Dry-run preview uses `PlannedBlocker`, not `CloseBlocker` or `WriteDecisionReason`. A pre-commit failure uses `ToolRejectedResponse.errors: ToolError[]`, not any blocker type.

`STATE_VERSION_CONFLICT` appears only in `ToolRejectedResponse.errors`. It must not be `WriteDecisionReason.code`, `CloseBlocker.code`, `PlannedBlocker.code`, or a `MethodResult.decision` value. Pre-commit failure codes such as `VALIDATION_FAILED`, `MCP_UNAVAILABLE`, `LOCAL_ACCESS_MISMATCH`, and `NO_ACTIVE_TASK` are not blocker codes when they indicate request rejection; they stay in `ToolRejectedResponse.errors` and the response has `effect_kind=no_effect`.
...
For `PrepareWriteResult`, committed `decision=blocked`, `decision=approval_required`, or `decision=decision_required` uses `write_decision_reasons: WriteDecisionReason[]`; it does not return or store `CloseBlocker` and does not change `close_state`. For `CloseTaskResult(close_state=blocked)`, committed close blockers use `blockers: CloseBlocker[]`; they do not use `WriteDecisionReason`. Request rejections before either committed result use `ToolRejectedResponse.errors`.
...
- `prepare_write decision=allowed` creates the owner-scoped single-use Write Authorization. `decision=blocked`, `decision=approval_required`, and `decision=decision_required` are committed `PrepareWriteResult` values only when the method state-effect table permits that blocked commit; their reasons are `WriteDecisionReason` entries in `write_decision_reasons`, not `CloseBlocker` entries. They must not create a consumable Write Authorization. `STATE_VERSION_CONFLICT` and request validation failure are `ToolRejectedResponse.errors` branches, never `PrepareWriteResult.decision` values.
