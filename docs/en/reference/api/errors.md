...
`prepare_write decision=blocked`, `decision=approval_required`, and `decision=decision_required` use `WriteDecisionReason`. `prepare_write` must not use `CloseReadinessBlocker`, run close-readiness evaluation, or mutate `close_state`.

`ToolDryRunResponse` uses `PlannedBlocker` in `DryRunSummary.would_blockers`. Dry-run preview must not store or return current or stored `CloseReadinessBlocker` objects. `PlannedBlocker.source_kind` uses `close_readiness` for close-readiness preview findings and does not use any other close-readiness source value.
