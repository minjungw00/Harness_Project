...
| `missing_evidence` | `EVIDENCE_INSUFFICIENT`, `VERIFY_NOT_DETACHED`, `QA_REQUIRED`, or `ARTIFACT_MISSING` | Required evidence, verification independence, Manual QA, or artifact support is absent, stale, blocked, or insufficient. | "Evidence is insufficient for that claim." | Run or record the missing check when the agent can; otherwise show the gap, affected claim, and smallest unblocker. | Evidence-dependent next action / only if evidence is a write precondition / yes when close depends on evidence. | Evidence, test results, QA, artifact integrity, verification independence, sufficiency, or close readiness. |
