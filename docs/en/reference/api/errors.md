...
Every fresh non-dry-run state mutation compares `ToolEnvelope.expected_state_version` with the current project-wide `project_state.state_version`. A stale `expected_state_version` returns `ToolRejectedResponse` with `STATE_VERSION_CONFLICT`; it is not a method-specific result and is never a `PrepareWriteResult.decision` value. The pre-commit failure creates no current records, events, artifacts, evidence summaries, Write Authorizations, close state, replay rows, staged-handle consumption, or `state_version` increment. `tasks.state_version` is not an active conflict or concurrency basis.
