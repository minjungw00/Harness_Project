...
- When the verified connection is in `guarded` or `managed` mode, close readiness also checks guard health, including hook path safety, prompt-capture availability facts, unresolved unrecorded Product Repository changes, guard-detected write-readiness issues, and session-watch availability. In `mcp_only` mode, missing or partial watcher coverage is reported through guard health, and an active session watch remains detective-only; unresolved watcher-created unrecorded-change findings block close until reconciliation resolves them. In `managed` mode, missing watcher coverage or a partial watcher coverage window blocks close when managed policy requires full watcher coverage. In `guarded` mode with full host hooks active, a partial watcher coverage window may remain a warning unless the applicable guard policy requires a blocker.
...
| `session_watch_unavailable` | `connection_capability` | A managed close path requires full Product Repository session-watch coverage, but the selected watcher state is `disabled`, `degraded`, `unavailable`, or active with a partial-coverage warning. |
...
| `intent=complete` | Produces close readiness blockers when the completion path reaches close readiness evaluation and owner-defined close requirements remain unresolved. In `guarded` or `managed` mode this includes guard-health, unresolved unrecorded-change, session-watch, and guard-detected write-readiness blockers. In `mcp_only` mode, unresolved watcher-created unrecorded-change findings also block close while the watcher is active, while partial watcher coverage remains a guard-health warning unless another owner-defined policy blocks it. |
