...
Only a compatible non-dry-run `decision=allowed` path returns `PrepareWriteResult` as a Core committed `MethodResult` with `effect_kind=core_committed`. That result creates one compatible active Write Authorization and stores the current project-wide `project_state.state_version` as `basis_state_version`.

`decision=blocked`, `decision=approval_required`, and `decision=decision_required` return `PrepareWriteResult` only when Core commits the blocked decision under the method state-effect contract. Those committed blocked results may persist method-owned blockers, events, replay, and project-wide state-version effects only as allowed by the API method matrix, but they must not create a consumable authorization row, evidence record, close state, or Harness write-compatibility record.

Stale project-wide state returns `ToolRejectedResponse` with `STATE_VERSION_CONFLICT` before commit. `STATE_VERSION_CONFLICT` is not a `PrepareWriteResult.decision` value and creates no Write Authorization, blocker commit, replay row, evidence record, close state, or state-version increment. A valid `dry_run=true` call returns `ToolDryRunResponse`; it may preview the non-dry-run path, but it creates no Write Authorization and returns no real `write_authorization_ref`.
...
Branch identity is part of close authority. `intent=check` returns `CloseTaskResult` as a read-only `MethodResult` with `effect_kind=read_only`. A committed blocked `intent=complete` returns `CloseTaskResult` with `close_state=blocked` only when Core commits the close-matrix result under the method state-effect contract; the Task remains open. A precondition failure before a close-matrix commit returns `ToolRejectedResponse` with `effect_kind=no_effect`, not a success-shaped `CloseTaskResult`. For mutating close intents, a valid `dry_run=true` call returns `ToolDryRunResponse`; it may preview terminal or blocked close effects, but it does not change Task lifecycle, close fields, blockers, events, replay rows, or `project_state.state_version`.
