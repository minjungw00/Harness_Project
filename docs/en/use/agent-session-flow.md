...
`request_user_decision`, `judgment_domain`, `decision_kind`, and `decision_profile` are compatibility aliases for older request shapes. Do not present either the current fields or the aliases as independent axes the user has to understand. Affected gates or blocked actions are owned by separate fields and owner records. The exact public fields are owned by [`harness.request_user_judgment`](../reference/api/mvp-api.md#harnessrequest_user_judgment), and canonical authority is owned by [User Judgment](../reference/kernel.md#user-judgment) and [Decision Gate](../reference/kernel.md#decision-gate). Render the judgment in ordinary language and keep refs available for drill-down.
