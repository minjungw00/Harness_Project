...
`harness.status`, `harness.next`, compact status cards, and recommendation lines are read-only displays. They can recommend a judgment request, full-format Decision Packet view, `prepare_write`, evidence collection, work-acceptance request, verification, QA, reconcile, or close attempt, but the recommendation itself does not mutate state, authorize writes, satisfy gates, accept results, accept residual risk, or close the Task.
