...
| Administrative agent setup behavior | `binary_admin` and colocated CLI module tests for `agent_command.rs`, host adapters, repository guidance, and registration helpers. | Add Store tests when bootstrap, inspection, registry, migration, Agent Connection, project membership, or managed host configuration state inventory behavior changes. |
