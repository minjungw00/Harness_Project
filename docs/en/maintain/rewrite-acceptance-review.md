...
- Core Model English/Korean judgment schema wording spot check: the prior wording drift is resolved. The checked summary paragraphs and canonical-schema bullets are aligned on `user_judgment`, `harness.request_user_judgment`, `harness.record_user_judgment`, `judgment_kind`, `presentation`, `display_label`, and compatibility/legacy terms.
