...
| Korean later-profile template localization check | Scanned all 19 files in `docs/ko/reference/templates/later-profile/` for English cue-label candidates and manually spot-checked `README.md`, `decision-packet.md`, `task.md`, `direct-result.md`, `run-summary.md`, `eval.md`, `interface-contract.md`, and `export.md`. The folder has a Korean rendering-label rule and many headings/prose labels are now natural Korean while exact template IDs, schema/API names, field names, refs, placeholders, enum values, and stable labels are preserved. The exact older examples `display label:`, `active path:`, `write authorization:`, `approval refs`, `Write Authority Summary`, and `Close Summary` were not found in the targeted exact scan. The cleanup is still not complete: broader scans still show English or mixed rendered cue/table-label candidates such as `boundary tests`, `actor kind`, `secret scope`, `redaction state`, `source input`, `run summary`, `redacted artifacts`, and `User judgment compatibility`. Some may be valid exact/stable labels or field-like identifiers, but the folder still needs a full rendered-label classification and localization pass. Treat this as documentation drift/localization follow-up, not runtime conformance or server-coding status. |
...
- Local relative link and anchor checker over all repository Markdown entrypoints and docs, including explicit `<a id="...">` / `<a name="...">` anchors: checked 131 Markdown files, 2,098 relative links, and 867 anchor links; no unresolved relative links or anchors were reported.
...
- Korean later-profile template localization check: broad scan plus targeted spot check found that cleanup is improved but not complete; remaining English/mixed rendered-label candidates are listed above as follow-up.
