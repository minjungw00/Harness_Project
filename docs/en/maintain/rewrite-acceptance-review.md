...
| Korean later-profile template localization check | Scanned all 19 files in `docs/ko/reference/templates/later-profile/` for English cue labels and manually spot-checked `README.md`, `decision-packet.md`, `task.md`, `direct-result.md`, `run-summary.md`, and `eval.md`. The folder now has a Korean rendering-label rule and many headings/prose labels are natural Korean while exact template IDs, schema/API names, field names, refs, and stable labels are preserved. The cleanup is not complete: rendered template bodies still contain English or mixed cue/table labels such as `display label`, `active path`, `write authorization`, `approval refs`, `Evidence Manifest`, `Eval`, `Approval`, `Write Authority Summary`, and `Close Summary`. Some are valid exact/stable labels, but user-visible rendered-label localization still needs a full pass. Treat this as documentation drift/localization follow-up, not runtime conformance or server-coding status. |
| Core Model judgment routes and schema wording | Manually checked `docs/en/reference/core-model.md` and `docs/ko/reference/core-model.md`. The route boundary is aligned: route verbs are internal owner-path metadata, broad approval is absent from the user-facing model, display depth is presentation metadata, users see the same five display types, and the canonical-schema bullets name the same `user_judgment`, request/record actions, `judgment_type`, `presentation`, `display_label`, and compatibility/legacy terms. One minor bilingual wording drift remains: the Korean `User Judgment` summary paragraph explicitly names `presentation` and `display_label`, while the nearby English summary paragraph omits them even though the canonical-schema bullets below are aligned. Treat this as documentation-drift wording polish, not a server-coding or runtime-conformance blocker. |
...
- Local relative link and anchor checker over all repository Markdown entrypoints and docs, including explicit `<a id="...">` / `<a name="...">` anchors: checked 131 Markdown files, 123 relative links, and 72 anchor links; no unresolved relative links or anchors were reported.
...
- Fenced code block balance check over the same 131 Markdown files: no unclosed fenced code blocks were reported.
...
- Korean later-profile template localization check: broad scan plus targeted spot check found that cleanup is not complete; remaining English/mixed cue labels are listed above as follow-up.
- Core Model English/Korean judgment schema wording spot check: the canonical-schema bullets are aligned on `user_judgment`, `harness.request_user_judgment`, `harness.record_user_judgment`, `judgment_type`, `presentation`, `display_label`, and compatibility/legacy terms, but one nearby summary-paragraph wording asymmetry remains as listed above.
...
- Full Korean later-profile rendered-label localization audit and cleanup. A broad scan plus targeted spot check was performed, but not every rendered label was manually classified or rewritten.
...
- Korean later-profile template localization polish remains for English or mixed user-visible cue labels and table labels in rendered template examples. Route this as documentation drift/localization follow-up, not as a runtime or server-coding blocker.
- Minor Core Model bilingual wording polish remains: align the English and Korean `User Judgment` summary paragraph around `presentation` and `display_label`. The canonical-schema bullets are already aligned.
