...
| Kernel Smoke | Narrow future smoke-check authoring label under Engineering Checkpoint. It is not a stage and not a current runnable fixture set. |
...
| Future fixture catalog scope pressure | Detailed future fixture catalog prose is retired from active documentation and summarized in [Later Candidate Index: Future Fixture Families](../later/index.md#future-fixture-families). The remaining index rows are candidate inventory only, not fixture bodies, runnable suites, active API/DDL, current conformance, or implementation tasks. |
...
| Representative conformance examples | Present in [Conformance Reference](../reference/conformance.md) as compact non-executable behavior examples and future fixture-shape guidance. The examples are not fixture files, full YAML bodies, current pass/fail criteria, generated artifacts, or runtime conformance results. |
...
## Current Conformance Cleanup Note

This note records the current collapsed conformance structure. It is not implementation-readiness approval, final documentation acceptance, runtime conformance, a conformance runner pass, or server-coding acceptance. Readiness, handoff, documentation acceptance, implementation-planning readiness, and coding acceptance remain maintainer decisions in the Build owner docs.

| Conformance area | Current cleanup record |
|---|---|
| Owner path | The active conformance owner is [Conformance Reference](../reference/conformance.md) in both languages. |
| Example boundary | Active examples are compact representative behavior examples only. They are not full YAML bodies, fixture files, runner-loadable snippets, runnable suites, current pass/fail criteria, generated artifacts, or runtime conformance results. |
| Assertion boundary | Examples point to structured assertion categories over response facts, Core state, storage rows, stable events when promoted, artifact refs, blockers, errors, guarantee facts, and forbidden side effects. Prose-only matching remains insufficient. |
| Future family boundary | [Later Candidate Index: Future Fixture Families](../later/index.md#future-fixture-families) lists names only. It does not contain scenario scripts, fixture bodies, active API payload examples, suite requirements, or implementation tasks. |
| Korean prose | Korean conformance prose is expected to preserve exact identifiers while using natural Korean terms such as `적합성`, `fixture 형태`, `주장 권한`, `대표 예시`, `실행 가능한 suite 아님`, and `future fixture family`. |
...
- Future fixture catalog scope-pressure spot check: manually checked the English and Korean Later Candidate Index future fixture family rows and targeted search hits; no wording was found that turns the catalog into an MVP-1 requirement, runnable fixture set, active API/DDL, current conformance result, or implementation checklist.
