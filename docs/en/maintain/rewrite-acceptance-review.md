...
| Korean later-profile template localization check | Scanned all 19 files in `docs/ko/reference/templates/later-profile/` for English cue-label candidates and manually spot-checked `README.md`, `decision-packet.md`, `export.md`, and targeted search hits. The folder has a Korean rendering-label rule, and many headings/prose labels are natural Korean while exact template IDs, schema/API names, field names, refs, placeholders, enum values, and stable labels are preserved. The exact older examples `display label:`, `active path:`, `write authorization:`, `approval refs`, `Write Authority Summary`, and `Close Summary` were not found in the targeted exact scan. The cleanup is still not complete: broader scans still show English or mixed rendered cue/table-label candidates such as `QA waiver user judgment`, `Residual Risk`, `Task gate`, `Change Unit`, `Run`, `EVIDENCE-MANIFEST`, `confidence`, `reversibility`, `run_summary`, `approval_scope`, `source_state_version`, and `blocked_by`. Some are valid exact/stable labels, field names, or template IDs, but the folder still needs a full rendered-label classification and localization pass. Treat this as documentation drift/localization follow-up, not runtime conformance or server-coding status. |
...
- Korean later-profile template localization check: targeted exact scan plus broader rendered-label candidate scan found that cleanup is improved but not complete; remaining English/mixed rendered-label candidates are listed above as follow-up.
