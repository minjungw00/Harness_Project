...
- Do not assign the same state effects to `ToolRejectedResponse` and committed blocked `CloseTaskResult`. A close preflight `ToolRejectedResponse` creates no `CloseBlocker`, `task_event`, `task_events` append, `tool_invocations` replay row, replay row, `close_state` mutation, Write Authorization consumption, staged-handle consumption, artifact effect, evidence update, or `state_version` increment; a committed blocked `CloseTaskResult` may have only the effects allowed by the `close_task` owner contract.
