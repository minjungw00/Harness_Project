...
- Do not bind a domain data shape and state effects to the same type name. State effects are determined by the response branch and the method state-effect table.
- Blocker-like response fields must declare type ownership and document `source`, response branch, persistence behavior, and forbidden `ErrorCode` values. Current MVP ownership is `ToolRejectedResponse.errors: ToolError[]` for pre-commit failures; `PrepareWriteResult.write_decision_reasons: WriteDecisionReason[]` for `prepare_write` decision reasons; `StatusResult.close_blockers: CloseReadinessBlocker[]` and `CloseTaskResult.blockers: CloseReadinessBlocker[]` for close-readiness findings; and `DryRunSummary.would_blockers: PlannedBlocker[]` for dry-run expected blockers. New blocker-like fields must declare the owning method or owning response branch before they appear in examples.
- `CloseBlocker` must not be reintroduced as an active contract type. Do not use it for `prepare_write`, `close_task`, dry-run preview, storage, examples, or generic blocker prose.
- `CloseReadinessBlocker` can appear in read-only and committed result branches, but the branch determines effects. `StatusResult.close_blockers` is read-only observation. `CloseTaskResult` from `intent=check` is read-only evaluation. `CloseTaskResult(close_state=blocked)` from committed `intent=complete` is a committed blocked close result only when the method state-effect table allows it.
- `CloseReadinessBlocker` must not be described as a storage row, persistence signal, or automatic state effect. `StatusResult.close_blockers` must not create `task_events`, replay rows, `close_state` mutations, Write Authorization changes, staged-handle consumption, artifact effects, evidence updates, or `project_state.state_version` increments. `close_task intent=check` blockers are not stored blockers and not committed close results.
- For `close_task`, define preflight rejection before close-readiness evaluation. Preflight rejection returns `ToolRejectedResponse`; close-readiness findings return `CloseTaskResult(close_state=blocked)` only after preflight succeeds and the response branch and method state-effect table allow that result. Do not mix preflight failure `ErrorCode` values with `CloseReadinessBlocker.code`. `STATE_VERSION_CONFLICT`, stale `WriteAuthorization.basis_state_version`, and `idempotency_key` reuse with a different request hash are preflight rejection cases, not committed `write_compatibility` or `recovery` blockers.
- `PrepareWriteResult` must not require `CloseReadinessBlocker[]` and must not use `CloseBlocker`. `DryRunSummary.would_blockers` must remain `PlannedBlocker[]`; it must not require `CloseReadinessBlocker[]` or `CloseBlocker[]`. Close-readiness dry-run preview findings use `source_kind=close_readiness`, not `source_kind=close_matrix`. `STATE_VERSION_CONFLICT` must not be used as `WriteDecisionReason.code`, `CloseReadinessBlocker.code`, or `PlannedBlocker.code`.
...
- [ ] Blocker-like fields preserve current type ownership: pre-commit failures use `ToolRejectedResponse.errors`, `prepare_write` decision reasons use `WriteDecisionReason`, close-readiness findings use `CloseReadinessBlocker`, dry-run expected blockers use `PlannedBlocker`, and `CloseBlocker` is not reintroduced as an active contract type.
