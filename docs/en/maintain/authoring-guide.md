...
- [ ] Identify the document family: Learn, Use, Build, Reference, Maintain, Later, or Roadmap.
...
- User-owned judgment stays with the user unless an owner contract explicitly says otherwise. Product decisions, material technical decisions, QA expectations, waivers, work acceptance, and residual-risk acceptance must not be silently delegated to the agent.
- Evidence, verification, Manual QA, work acceptance, close readiness, and residual risk are separate records and judgments. None substitutes for another.
...
|---|---|---|
| Learn | Explain why Harness exists and what concepts mean. | Do not define strict schemas, gates, DDL, implementation sequences, or fixture mechanics. |
...
| Core transition collisions | [Core Model Reference](../reference/core-model.md), with architecture placement in [Runtime Architecture Reference](../reference/runtime-architecture.md) | Learn/Use task explanations, Build stage summaries, API/storage/projection references, and template summaries | Task/run/gate state transitions, `prepare_write`, Write Authorization, `record_run`, waiver, close, non-substitution rules, or Core-only mutation authority conflict with Core owner language or are treated as display-only/report-only behavior. |
| Active MVP vs later/profile boundary drift | [Implementation Overview](../build/implementation-overview.md), [Engineering Checkpoint](../build/engineering-checkpoint.md), [MVP-1 User Work Loop](../build/mvp-user-work-loop.md), and Later profile docs | Reference introductions, Use examples, README routes, Roadmap entries, and Build summaries outside the stage owner | Engineering Checkpoint, Kernel Smoke, MVP-1, later-profile, or Roadmap material is promoted or required by wording without the stage/profile owner promoting it with scope, fallback behavior, and proof expectations. |
...
| User output vs agent context mixing | [Agent Integration Reference](../reference/agent-integration.md), [Surface Cookbook](../reference/surface-cookbook.md), and [Projection And Templates Reference](../reference/projection-and-templates.md) | User Guide, Agent Guide, templates, README routes, Learn examples, and Operations summaries | User-facing summaries, status cards, projections, retrieved context, or chat memory are treated as operational authority, write authorization, or enough to satisfy gates, user judgments, evidence, QA, work acceptance, residual-risk acceptance, or close. |
...
Major implementation decisions found during review belong in [MVP-1 User Work Loop: Implementation decisions needed before server coding](../build/mvp-user-work-loop.md#implementation-decisions-needed-before-server-coding). Do not leave major decisions as scattered `TODO_DECISION` markers in active docs. The short maintainer handoff status is owned by [Implementation Overview: Maintainer handoff summary](../build/implementation-overview.md#maintainer-handoff-summary).
...
Introduce concepts through examples before strict definitions. A Learn or Use page should not open with a dense definition list unless it is explicitly a glossary or reference table.
...
| User-facing docs open with heavy implementation disclaimers. | Learn and Use openings start with what users can ask, what the agent clarifies, what Harness preserves, and what users see; detailed status warnings route to README, Build handoff, and Maintain docs. | Documentation drift |
...
| Judgment terminology reintroduces legacy aliases as current axes. | Active owner docs prefer `UserJudgment` / `user_judgment`, `harness.request_user_judgment`, `judgment_type`, `presentation`, and `display_label`; legacy aliases stay compatibility-only. | Schema/design decision |
...
| Sensitive-action Approval, work acceptance, and residual-risk acceptance blur together. | Examples and routing keep permission for a named sensitive step, acceptance of the work result, and acceptance of visible remaining risk separate. | Schema/design decision |
...
Learn and Use docs may start with ordinary requests, practical examples, or user workflow. A workflow-first opening should show what users can ask, what the agent should clarify, what Harness preserves, what users should expect to see, and where exact owner details live.
...
Template files must make the non-authority boundary visible: a template is rendered display, not canonical state, gate authority, approval, acceptance, evidence, schema, DDL, or runtime behavior.
...
Do not paste the gate matrix, request schema, DDL block, fixture body, template body, enum table, validator table, projection table, or glossary definition into Learn, Use, Build, or Maintain docs.
