...
| Active value-set ownership | active owner boundary | Current active method-name and schema enum-like values live in `docs/*/reference/api/schema-value-sets.md`; later names listed here do not extend those sets. | none |
...
| Projection refresh and freshness diagnostics | later candidate | Projection owner behavior that keeps projections non-authoritative and read-only unless a future owner defines a separate state-changing path. | none until promoted |
| Persistent projection jobs and projection job storage | later candidate | Projection and Storage owners must define job lifecycle, storage rows, freshness, failure behavior, queue behavior if any, and proof expectations. Active MVP uses read-time compact status/projection only and has no active persistent projection jobs. | none until promoted |
| Projection reconcile, editable projection areas, reconcile queues, and managed block drift repair | later candidate | Projection, Core, API, and Storage owners must define editable input handling, reconcile outcomes, queue behavior, managed block hash/drift semantics, repair candidates, state-change routing, non-substitution, and proof expectations. Human-edited projections, Markdown status cards, generated documents, and managed blocks are not active state. | none until promoted |
...
| `captured_artifact` and captured artifact handles | value names only | Promoted native-capture/API/schema/storage owners define exact handle source, capability profile gate, validation, storage, redaction, fallback behavior, and proof expectations. Active MVP uses `harness.stage_artifact` for new artifact bytes, `ArtifactInput.source_kind=staged_artifact` only for eligible staged-handle promotion, and `existing_artifact` refs only for already persisted artifact refs. | none until promoted |
...
| Read-only playbook and judgment context | metadata names only | Agent Integration/API owner rules that keep metadata read-only and non-satisfying. | none until promoted |
