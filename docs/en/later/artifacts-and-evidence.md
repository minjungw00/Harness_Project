...
This category is for candidates whose main question is "what richer material can Harness record, reference, package, or summarize later?" It includes native capture, full evidence manifests, export bundles, artifact body policy, browser-capture evidence, and evidence-related template names.

It does not own security control claims, connector mechanics, validator catalogs, or collaboration lifecycle except when those topics are needed to describe artifact or evidence material before promotion.
...
This document has no effect on the current MVP. Mentioning a candidate here does not activate artifact capture, evidence acceptance, body storage, export formats, template bodies, UI cards, or conformance expectations.
