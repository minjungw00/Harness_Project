...
| `host_hook_guarded` | Project-local host hooks have verified generated config, native host output, required phases, write matchers, matching policy hash, and runtime observation. | Pre-tool decisions, post-tool correlation, prompt capture, guard state, and close/write blockers can participate in the workflow. |
