...
This document fixes canonical ownership for v2 of the harness documentation set. The reference explanation for a concept is owned by exactly one document. Other documents provide only a one-sentence summary and a link.

## 1. Ownership Rules

```text
1. Each concept has exactly one owner document.
2. Non-owner documents provide only a short summary.
3. Schemas are not repeated outside designated schema documents.
4. State transitions are not repeated outside the kernel spec.
5. Full templates are not repeated outside the appendix.
6. Surface-specific details are not repeated in the core integration document.
7. Later features are not written as implementation requirements in MVP body text.
```
