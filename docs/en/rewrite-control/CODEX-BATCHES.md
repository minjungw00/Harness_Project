...
This document defines batch-by-batch work instructions for Codex. Codex is the repo editor that actually edits repository files. It does not make new architecture decisions.
...
- "human-editable document area | reconcile item" as a source-of-truth conflict
- "detached verification passed or accepted exception"
- "scope and approval" where the distinction is not explained
