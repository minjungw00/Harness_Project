...
This is planning documentation for documentation redesign / review and maintainer handoff. The repository is documentation-only today, and its intended future role is the Harness Server source repository. Server/runtime implementation in this repository may start only after documentation acceptance and a separate implementation-planning readiness decision; no Harness Server/runtime implementation, executable fixture files, generated runtime records, generated projections, or runnable Harness Server conformance tests exist here yet. This revision is in post-redesign review and is a documentation acceptance candidate for maintainer review, not an accepted implementation start. The first runnable target is Engineering Checkpoint, with Kernel Smoke as a narrow future smoke-check authoring label for the smallest local authority loop. The first user-value target is MVP-1 User Work Loop. Assurance Profile and Operations Profile harden agency assurance, operations, and handoff behavior. Roadmap remains future scope unless owner docs promote and prove it.
...
No server/runtime implementation decisions have been formally accepted for coding in this repository phase. The decision ledger in [Staged Delivery Plan: Implementation decisions needed before server coding](mvp-plan.md#implementation-decisions-needed-before-server-coding) records documentation-resolved MVP-1 contract decisions and the implementation-readiness items that still must be accepted or explicitly deferred before coding the affected stage.
...
| Stage boundary decision | A choice about whether a capability belongs in Engineering Checkpoint, MVP-1 User Work Loop, Assurance Profile, Operations Profile, or Roadmap. | Implementation Overview, Staged Delivery Plan, owner docs, or Roadmap promotion when applicable. | Blocks implementing the affected stage until the boundary is accepted. It may be non-blocking for documentation review if explicitly recorded. |
...
| Server-coding decision log | MVP-1 scope decisions from the old ledger are documented as resolved in the Staged Delivery Plan, but no server/runtime implementation decision has been accepted for coding yet. | Maintainers must accept the documentation-resolved decisions and the implementation-readiness gate in [Staged Delivery Plan: Implementation decisions needed before server coding](mvp-plan.md#implementation-decisions-needed-before-server-coding) before coding the affected API, DDL, state transition, local access, status card, evidence, acceptance, or risk behavior. |
...
- Documentation-resolved server-coding decision-ledger items are recorded in [Staged Delivery Plan: Implementation decisions needed before server coding](mvp-plan.md#implementation-decisions-needed-before-server-coding).
...
- The previously tracked judgment-model drift is resolved in this documentation baseline: canonical docs use `user_judgment`, `harness.request_user_judgment`, `judgment_type`, `presentation`, and `display_label`, and treat Decision Packet as optional full-format/legacy presentation. If review exposes a remaining owner-contract decision, route it to the Staged Delivery Plan decision log instead of scattering TODOs.
...
- Any confirmed schema/design decision, stage boundary decision, or other server-coding decision is recorded in the Staged Delivery Plan before server code or DDL changes begin. Documentation-resolved decisions must be accepted for coding, and any implementation decision still open must be resolved or explicitly deferred with stage impact.
...
- Maintainers complete the final docs-maintenance pass in the [Authoring Guide](../maintain/authoring-guide.md#final-pre-acceptance-review) during acceptance review, including English/Korean parity, link/anchor checks, owner-boundary checks, TODO hygiene, and current status wording.
...
- Links, TODOs, terminology, and English/Korean semantic parity are clean. There are no unresolved major-decision TODOs scattered through active docs; implementation decisions needed before server coding are classified in [Staged Delivery Plan](mvp-plan.md#implementation-decisions-needed-before-server-coding), with documentation-resolved decisions separated from implementation decisions still open.
...
- The local-only MCP exposure baseline is accepted for Engineering Checkpoint. Remote, shared, tunneled, or non-loopback exposure remains outside the Engineering Checkpoint baseline unless owner docs promote and prove a connector profile; see [Runtime Architecture](../reference/runtime-architecture.md#local-access-expectations), [Security Threat Model Reference](../reference/security-threat-model.md#mcp-local-access-and-caller-boundaries), and [API Schema Core](../reference/api/schema-core.md#mcp-boundary-and-caller-trust).
...
The artifact store is not a loose file dump. Any artifact that supports Harness state must be registered through the artifact owner path and linked to the Task or owner record that uses it. Exact artifact refs, integrity fields, redaction states, and retention rules belong to [API Schema Core](../reference/api/schema-core.md#artifactref) and [Storage And DDL](../reference/storage-and-ddl.md#artifact-directory-layout).
...
The first runnable target is Engineering Checkpoint: the smallest runnable path that proves Harness can make and enforce one authority decision. Kernel Smoke is the narrow future authoring label for this target's smoke checks, not a full conformance suite.
...
2. [Staged Delivery Plan](mvp-plan.md) for Engineering Checkpoint through Operations Profile staged delivery, stage boundaries, and the server-coding decision log.
3. [First Runnable Slice](first-runnable-slice.md) for the Engineering Checkpoint implementation sequence.
