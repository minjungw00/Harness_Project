...
The first runnable slice may enqueue a minimal `TASK` projection job or render a minimal `TASK` projection. The final MVP must support the MVP-required `ProjectionKind` values when their source records exist or change. `ProjectionKind` values and API-owned MVP tiering belong to [MCP API And Schemas](../reference/mcp-api-and-schemas.md#shared-schemas). [Document Projection Reference](../reference/document-projection.md#template-tiers) owns projection authority boundaries, source-record rules, freshness rules, and template tier presentation; [Template Reference](../reference/templates/README.md) owns rendered template bodies and display cards.
