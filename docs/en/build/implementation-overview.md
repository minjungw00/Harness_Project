...
Build owns the staged boundary and reading path. Exact public method contracts live in [MVP API](../reference/api/mvp-api.md), shared shapes in [API Schema Core](../reference/api/schema-core.md), and compact view/template contracts in [Projection And Templates Reference](../reference/projection-and-templates.md) and [Template Reference](../reference/templates/README.md). The active MVP-1 method and compact output lists are centralized in [MVP-1 User Work Loop](mvp-user-work-loop.md#main-idea).
...
| API | Minimal status/blocker read, owner-valid setup path, pre-write check, run/evidence recording, one artifact/evidence ref path, and narrow close-blocker check. Natural-language intake is not required. | Adds the owner-defined MVP-1 public methods for status/next actions, intake, user judgment, write checks, run/evidence, and close. | [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md). |
