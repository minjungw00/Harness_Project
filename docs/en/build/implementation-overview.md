...
| First user work loop | MVP-1 User Work Loop. Adds ordinary-language intake, status with `harness.status.next_actions`, focused user judgment request/record, Core-owned evidence summary, close result/blockers, final-acceptance and residual-risk visibility, accepted-risk close when explicitly accepted, and compact Core-derived views. | Full Evidence Manifest, detached Eval, full Manual QA matrix, Assurance hardening, operations/export/recover, dashboards, hosted UI, broad connectors, automation, or detailed reports. |
