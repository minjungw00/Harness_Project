...
- stale `harness.prepare_write` `expected_state_version` pre-commit failure: a stale `ToolEnvelope.expected_state_version` returns `ToolRejectedResponse` with `errors[0].code=STATE_VERSION_CONFLICT`, has no `decision`, no `write_decision_reasons`, no `CloseBlocker`, and no state/storage effects. It creates or consumes no Write Authorization, stores no write-decision reason, creates no close-matrix blocker, appends no event, creates no `tool_invocations` replay row, mutates no `close_state`, and does not increment `project_state.state_version`; `tasks.state_version` is not used as the active conflict basis
- valid blocked `harness.prepare_write` behavior: the response is `PrepareWriteResult` with `decision=blocked`, and `write_decision_reasons` is a non-empty `WriteDecisionReason[]`; those prepare-write decision reasons are not `CloseBlocker` records, are not close-matrix blockers, do not mutate `close_state`, and do not create a consumable Write Authorization. If the same first-smoke batch covers `decision=approval_required` or `decision=decision_required`, those outcomes also return `PrepareWriteResult` with non-empty `write_decision_reasons` and create no `CloseBlocker`, no `close_state` mutation, and no consumable Write Authorization
...
- `dry_run=true` for otherwise valid and previewable state-effecting selected operations returns `ToolDryRunResponse`; `DryRunSummary.would_blockers` is `PlannedBlocker[]`, and expected blockers appear only as `PlannedBlocker` preview items. The preview does not create `WriteDecisionReason` or `CloseBlocker` as stored/current records, creates no actual generated refs or method-specific success fields such as `run_summary`, `staged_artifact_handle`, or `write_authorization_ref`, and appends no event, creates no replay row, mutates no `close_state`, consumes no Write Authorization, consumes no staged handle, and makes no `project_state.state_version` increment
...
- first smoke coverage for `harness.close_task` with `intent=check` and `dry_run=true` as a read-only result branch, not dry-run preview coverage: the response is `CloseTaskResult` with `base.dry_run=true` and `effect_kind=read_only`, and is not `ToolDryRunResponse`. It may compute close readiness and blockers for the response, but it must create no `close_state` mutation, no `task_events`, no replay row, no `project_state.state_version` increment, no Write Authorization creation or consumption, no staged handle consumption, and no blocker storage, artifact promotion, or artifact linking.
- first internal smoke coverage for `close_task intent=complete` with stale `expected_state_version`: the response is `ToolRejectedResponse` with `effect_kind=no_effect` and `errors[0].code=STATE_VERSION_CONFLICT`, not `CloseTaskResult`; it creates no `CloseBlocker`, no `task_event` or `task_events` append, no replay row, no `tool_invocations.response_json`, no `close_state` mutation, no Write Authorization creation or consumption, no staged-handle consumption, no artifact promotion or link, no evidence update, and no `project_state.state_version` increment
- first internal smoke coverage for `close_task intent=complete` with stale `WriteAuthorization.basis_state_version`: the response is `ToolRejectedResponse` with `effect_kind=no_effect` and `errors[0].code=STATE_VERSION_CONFLICT`, not `CloseTaskResult`; it creates no `CloseBlocker`, no `task_event` or `task_events` append, no replay row, no `tool_invocations.response_json`, no `close_state` mutation, no Write Authorization creation or consumption, no staged-handle consumption, no artifact promotion or link, no evidence update, and no `project_state.state_version` increment
- first internal smoke coverage for `close_task intent=complete` with a semantic `write_compatibility` blocker after freshness preflight succeeds: the response is `CloseTaskResult` with `close_state=blocked`, and `blockers` is `CloseBlocker[]`; for the `write_compatibility` blocker, `CloseBlocker.code` is not `STATE_VERSION_CONFLICT`. Committed blocked effects are limited to the `CloseBlocker`, `task_events`, replay row, and `project_state.state_version` effects explicitly allowed by the API/storage contract, and the Task remains open
