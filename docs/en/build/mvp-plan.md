...
- stale `harness.prepare_write` response shape and no-effect behavior: a stale `ToolEnvelope.expected_state_version` returns `ToolRejectedResponse` with `errors[0].code=STATE_VERSION_CONFLICT`, has no `decision` field, creates no Write Authorization, commits no blocker, appends no event, creates no `tool_invocations` replay row, and does not increment `project_state.state_version`; `tasks.state_version` is not used as the active conflict basis
- valid blocked `harness.prepare_write` behavior: the response is `PrepareWriteResult` with `decision=blocked`, creates no consumable Write Authorization, and commits only the blocked state effects allowed by the method contract
...
- Write Authorization one-time consumption by a compatible product-write `record_run`, including `STATE_VERSION_CONFLICT` for a stale project-wide authorization basis and blocked handling for missing, expired, revoked, consumed, incompatible, or observed-outside-authorized-scope attempts without creating completion evidence
- `dry_run=true` for otherwise valid and previewable state-effecting selected operations returns `ToolDryRunResponse`, creates no actual generated refs or method-specific success fields such as `run_summary`, `staged_artifact_handle`, or `write_authorization_ref`, and appends no event, creates no replay row, and makes no `project_state.state_version` increment
...
- first internal smoke coverage for `close_task intent=complete` with stale `expected_state_version`: the response is `ToolRejectedResponse` with `errors[0].code=STATE_VERSION_CONFLICT`, not `CloseTaskResult`; it creates no `CloseBlocker`, no `task_event`, no replay row, no `close_state` mutation, and no `project_state.state_version` increment
- first internal smoke coverage for `close_task intent=complete` with stale `WriteAuthorization.basis_state_version`: the response is `ToolRejectedResponse` with `errors[0].code=STATE_VERSION_CONFLICT`, consumes no Write Authorization, creates no `CloseBlocker`, creates no replay row, and makes no `project_state.state_version` increment
- first internal smoke coverage for `close_task intent=complete` with a semantic `write_compatibility` blocker after freshness preflight succeeds: the response is `CloseTaskResult`, `close_state=blocked`, and includes a `CloseBlocker` where `blocker.category=write_compatibility` and `blocker.code` is not `STATE_VERSION_CONFLICT`; committed blocked effects are limited to the `CloseBlocker`, `task_events`, replay row, and `project_state.state_version` effects explicitly allowed by the API/storage contract, and the Task remains open
