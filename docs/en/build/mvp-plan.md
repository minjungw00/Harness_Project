...
- valid `dry_run=true` calls for state-changing methods return `ToolDryRunResponse`, create no actual generated refs or method-specific success fields such as `run_summary`, `staged_artifact_handle`, or `write_authorization_ref`, and append no event, create no replay row, and make no `project_state.state_version` increment
...
- `close_task intent=check` as read-only: it may compute close readiness and blockers for the response but must not store blockers, append events, create replay rows, change close state, consume Write Authorization, promote or link artifacts, or increment `project_state.state_version`
