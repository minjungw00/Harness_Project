...
- MVP-1 needs only five compact Core-derived views: `status-card`, `agent-context-packet`, `judgment-request`, `run-evidence-summary`, and `close-result`. Together they cover current work status, next safe action, user-owned judgment, minimal run/evidence summary, close readiness, acceptance, residual-risk visibility, blockers, and separate display of sensitive approval, work acceptance, and risk acceptance. They are not a full projection/reporting system.
...
- the five compact MVP-1 views: `status-card` and `agent-context-packet` for current state and next safe action, `judgment-request` for user-owned judgment, `run-evidence-summary` for run/evidence refs and gaps, and `close-result` for close readiness, acceptance, residual risk, and blockers; a separate `harness.next` method is later/compatibility material, not an MVP-1 requirement
...
Build staging does not upgrade security guarantees by itself. Security wording follows the [Security Reference stage map](../reference/security.md#guarantee-levels-by-stage):
...
| MVP-1 User Work Loop | User-facing intake/start/resume behavior, work-shape classification, next-safe-action output through `harness.status.next_actions`, minimal `harness.request_user_judgment` / `harness.record_user_judgment`, evidence summaries through `harness.record_run`, close blocker summaries through `harness.close_task`, and the five compact Core-derived MVP views. | Separate `harness.next`, full detached verification independence unless required by active profile/user request/task type/risk profile, full Manual QA matrix, full waiver machinery, Approval hardening, detailed Eval, TDD Trace, Module Map, Interface Contract, export/recover, broad operations. |
...
| Local access error taxonomy | API-visible early failures use the error taxonomy and precedence owned by [API Errors](../reference/api/errors.md), with caller/profile context from [API Schema Core](../reference/api/schema-core.md) and guarantee wording from [Security Reference](../reference/security.md). Operations may add diagnostic distinctions only through the operations owner path. | Build docs do not define new error codes or precedence. Use the API, Operations, and Security owner contracts for exact values and display-safe details. |
| MVP-1 compact view scope | MVP-1 output is limited to five Core-derived views: `status-card`, `agent-context-packet`, `judgment-request`, `run-evidence-summary`, and `close-result`. Together they show current work shape, scope/non-goals/success criteria, pending judgments, evidence summary or gaps, close blockers, residual-risk summary, next safe action, guarantee level, and source/freshness refs. Optional persistence of rendered views is not required. | These views must not create Write Authorization records, make writes compatible, satisfy evidence, record acceptance, accept residual risk, close a Task, or become canonical state. Stale, failed, or unknown freshness must be visible. |
...
- five compact MVP views derived from Core state, not from chat or rendered Markdown
...
- the five compact MVP views show current work status, user judgment request, run/evidence summary, close result, and blockers without template polish becoming the source of truth
...
- The five compact MVP views are derived from Core records and are sufficient for the MVP-1 path without making template polish authoritative.
