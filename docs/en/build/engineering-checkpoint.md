...
5. The owner-defined `harness.prepare_write` path demonstrates compatible, blocked, dry-run, and replay behavior without Build redefining request/response details.
...
8. Artifact/evidence refs use the owner path for integrity, redaction, owner relation, and safe omission/block metadata.
9. Evidence summary can show minimal coverage and gaps from registered refs.
...
| 6. Artifact/evidence ref | Register one durable artifact or evidence ref through the owner path. | A Run or minimal owner relation can cite that registered ref using the owner-defined artifact and evidence contract. | [API Schema Core](../reference/api/schema-core.md#artifactref), [Storage](../reference/storage.md). |
...
- It covers the owner-defined blocker and error cases for scope, idempotency/replay, Write Authorization, observed scope mismatch, raw secret artifact input, and missing artifact/evidence support where the active path requires support.
...
- Its future smoke checks use the structured draft shape owned by [Conformance Fixtures Reference](../reference/conformance-fixtures.md), not a Build-defined fixture schema.
...
Build docs do not create fixture files, runner code, generated conformance artifacts, or pass/fail results. Runtime conformance starts only after implementation and fixture materialization.
