...
Codex, Claude Code, 또는 generic MCP 호스트 통합이 `volicord agent install`,
`volicord agent verify`, `volicord agent status`, 프로젝트 멤버십 변경, uninstall 뒤
...
`volicord agent status`는 registry와 Host Installation 인벤토리를 보고합니다. Codex
또는 Claude Code가 MCP 서버를 로드했다는 증명이 아닙니다. `volicord-mcp --check`는
...
### `volicord-mcp`가 없거나, 실행 불가능하거나, 해석되지 않음

- **관찰 증상:** 설정, 검증, 또는 호스트 시작이 `volicord-mcp`가 없거나, 사용할 수
  없거나, 실행할 수 없거나, `PATH`에서 찾을 수 없다고 보고합니다.
- **가장 가능성 높은 원인:** 선택한 실행 파일 디렉터리에 `volicord`와 `volicord-mcp`
  둘 다 있지 않습니다. 파일을 선택한 사용자가 실행할 수 없습니다. 또는 프로젝트
  범위 호스트 설정은 `volicord-mcp`를 저장했지만 미래의 호스트 프로세스가 이를 찾을
  `PATH`를 받지 못합니다.
- **진단 점검:** 절대 명령이면 `test -x "$VOLICORD_BIN/volicord-mcp"`와
  `"$VOLICORD_BIN/volicord-mcp" --version`을 실행합니다. 프로젝트 범위의 이식 가능한
  명령이면 호스트를 시작할 같은 셸, 실행기, 서비스 환경에서 `command -v
  volicord-mcp`를 실행합니다.
...
- **검증:** 의도한 `VOLICORD_HOME`으로 `volicord-mcp --check --integration
  <integration_id>`를 다시 실행한 뒤, 영향받은 통합 또는 설치에 대해 `volicord agent
  verify`를 다시 실행합니다.
...
- **가장 가능성 높은 원인:** 경로가 절대 경로가 아니거나, 오래된 빌드 산출물을
  가리키거나, `volicord-mcp`가 아니라 `volicord`를 가리키거나, 재빌드 또는 이동 뒤 더
  이상 존재하지 않습니다.
- **진단 점검:** 호스트 설정을 바꾸지 않고 `test -x /absolute/path/to/volicord-mcp`와
  `/absolute/path/to/volicord-mcp --help`를 실행합니다.
- **제한된 복구 동작:** 같은 `integration_id`, 호스트, 범위, 서버 이름으로
  `volicord agent install`을 다시 실행하되 올바른 절대 `--mcp-command`를 제공합니다.
  기존 관리 항목이 사용자가 바꾼 상태라면, 관리 지문이나 소유 marker가 그 내용이
  여전히 Volicord 관리 내용임을 보여 줄 때만 교체를 사용합니다.
- **검증:** `volicord agent verify --integration-id <integration_id> --runtime-home
...
- **관찰 증상:** 프로젝트 범위 Codex 또는 Claude Code 설정에 `command =
  "volicord-mcp"` 또는 `"command": "volicord-mcp"`가 있지만, 이후 호스트 세션이
  Volicord를 시작하지 못합니다. 또는 관리 명령에 `PATH="$VOLICORD_BIN:$PATH"`를 더할
  때만 검증이 성공합니다.
- **가장 가능성 높은 원인:** 프로젝트 범위 설정은 의도적으로 개인 빌드 경로와 개인
  `VOLICORD_HOME`을 생략합니다. 미래의 호스트 프로세스가 `volicord-mcp`를 찾을 수 없는
  환경에서 시작되었습니다.
- **진단 점검:** 호스트 시작 환경에서 `command -v volicord-mcp`를 실행합니다. 의도한
  Runtime Home이 기본값이 아니라면 같은 시작 환경이 `VOLICORD_HOME`도 제공하는지
  확인합니다.
- **제한된 복구 동작:** 호스트 시작 환경, 셸 시작 파일, 서비스 설정, 또는 그에
  준하는 호스트 소유 경로가 `volicord-mcp`를 찾을 수 있게 합니다. 프로젝트 범위
  호스트 파일은 이식 가능한 형태로 유지합니다.
- **검증:** 그 환경에서 호스트를 시작하거나 다시 로드한 뒤, 관리 검증 명령의 `PATH`에
  선택한 디렉터리를 둔 상태로 `volicord agent verify`를 실행합니다.
...
- **건드리지 말아야 할 상태 또는 파일:** 공유 Product Repository 파일 안의 프로젝트
  범위 `volicord-mcp` 명령을 개인 절대 빌드 경로로 바꾸지 않습니다.
- **담당 문서 링크:** [관리 CLI](../reference/admin-cli.md),
...
- **관찰 증상:** 프로젝트 등록, 설정 재사용, MCP 시작, 프로젝트 목록 조회, 공개 도구
  ...
  관계 오류가 여기에 해당합니다.
- **가장 가능성 높은 원인:** 선택한 `Volicord Runtime Home`이 Product Repository
...
- **진단 점검:** 어떤 것도 바꾸기 전에 해석된 Runtime Home 경로와 저장소 루트를
  비교합니다. 등록된 상태를 보려면 `volicord agent status` 또는 `volicord project
...
- **검증:** 수정된 설정을 적용한 뒤 `volicord agent install --dry-run` 또는
  `volicord project list`를 다시 실행하고, 이후 `volicord-mcp --check --integration
...
- **진단 점검:** `volicord agent install --dry-run --output json` 또는 `volicord agent
...
- **제한된 복구 동작:** 호스트 소유 파일 또는 디렉터리 상태를 고친 뒤 같은 관리
  ...
  fingerprint가 맞는 Volicord 관리 내용만 교체하거나 제거합니다.
- **검증:** 인벤토리를 확인하려면 `volicord agent status`를 다시 실행하고, 호스트
  대상이 다시 읽기 가능해지면 `volicord agent verify`를 실행합니다.
...
- **가장 가능성 높은 원인:** Volicord가 마지막으로 지문을 기록한 뒤 사용자나 호스트가
  Volicord 관리 블록 또는 MCP 항목을 바꿨습니다. 또는 같은 `<server_name>`을 관리되지
  않는 호스트 설정이 이미 사용합니다.
- **진단 점검:** `volicord agent status --output json`을 실행하고 보고된 호스트 대상,
  `managed_fingerprint`, `fingerprint_state`, 경고 문구를 확인합니다. 이름 붙은 호스트
  항목이나 관리 블록만 비교합니다.
- **제한된 복구 동작:** 현재 내용이 여전히 Volicord 관리 내용이고 교체하려는 의도가
  분명하면 install 또는 `volicord agent guidance apply`를 `--replace-managed`와 함께 다시 실행합니다.
  관리 내용을 제거하려면 소유권 점검이 허용할 때만 uninstall 또는 `volicord agent guidance remove`를
  `--remove-managed`와 함께 사용합니다. 그렇지 않으면 다른 `--server-name`을 고르거나
  사용자 소유 항목을 보존합니다.
- **검증:** 교체 뒤에는 `volicord agent verify`를 다시 실행합니다. 보존 또는 제거 뒤에는
  `volicord agent status`를 다시 실행합니다.
- **이미 존재할 수 있는 지속 효과:** Host Installation 인벤토리가 이전 지문을 유지할
  수 있고, 검증은 변경된 설치에 대해 `failed`를 기록할 수 있습니다.
- **건드리지 말아야 할 상태 또는 파일:** Volicord 지문을 맞추기 위해 관련 없는 호스트
...
- **진단 점검:** 관리 검증에 쓰는 환경에서 `command -v codex`와 `codex --version`을
  실행합니다. 관리 설정 대상을 확인하려면 `volicord agent status`를 사용합니다.
...
- **건드리지 말아야 할 상태 또는 파일:** `complete`를 강제로 만들기 위해 Volicord
...
- **검증:** `volicord agent verify`를 다시 실행합니다. 승인 대기 중에도 진단 MCP
...
- **제한된 복구 동작:** 멤버십만 바뀐 경우 `volicord.list_projects`가 새 목록을
  ...
  프로세스를 시작하게 합니다.
- **검증:** 재시작 뒤 `volicord agent verify`를 실행하고, 호스트 안에서는 프로젝트
...
- **관찰 증상:** `volicord agent install` 또는 `volicord agent verify`가 성공 종료하고
...
- **제한된 복구 동작:** 이름 붙은 호스트 소유 동작만 수행합니다. 프로젝트를 trust하고,
  ...
  복구합니다.
- **검증:** 통합 또는 특정 `--installation-id`에 대해 `volicord agent verify`를 다시
  실행합니다.
...
- **제한된 복구 동작:** 보고된 원인을 고친 뒤, 이름 붙은 잔여 대상만 처리합니다. 광범위한
  파일 또는 데이터베이스 삭제 대신 uninstall, `volicord agent guidance remove`, project default, project
  membership 명령을 사용합니다.
- **검증:** 실패했던 명령을 다시 실행하거나, 설정 또는 호스트 상태를 고친 뒤 `volicord
...
- **검증:** 원인을 고친 뒤 `volicord agent status`를 다시 실행하고 `volicord agent
  verify`를 실행합니다.
...
- **관찰 증상:** status가 통합에 허용 프로젝트가 없다고 경고하거나, `volicord-mcp
...
- **진단 점검:** `volicord agent status --integration-id <integration_id>
  --runtime-home <runtime_home>`를 실행합니다. MCP 프로세스가 이미 실행 중이면
  `volicord.list_projects`를 호출해 지금 빈 허용 목록을 보는지 확인합니다.
- **제한된 복구 동작:** 명시적 프로젝트 하나를 추가하거나 복구합니다. `volicord agent
  ...
  default를 설정합니다.
- **검증:** `volicord-mcp --check --integration <integration_id>`를 실행한 뒤 `volicord
...
- **제한된 복구 동작:** 공개 도구 호출을 명시적 `envelope.project_id`와 함께 다시
  시도합니다. 생략 선택이 정말 필요하다면 `volicord agent project default set
...
- **진단 점검:** `volicord agent status --output json`을 실행하고
  `default_project_id`, `allowed_projects`, 프로젝트 가용성 경고를 확인합니다.
- **제한된 복구 동작:** 다른 허용 프로젝트가 편의 default로 남아야 하면 `volicord agent
  project default set`을 실행합니다. 마지막 프로젝트를 제거한다면 먼저 `volicord agent
  project default clear`를 실행한 뒤 멤버십을 제거합니다.
- **검증:** `volicord agent status`를 다시 실행합니다. 새 시작 자격을 확인하려면 하나
  이상의 프로젝트를 다시 허용한 뒤 `volicord-mcp --check --integration <integration_id>`를
  실행합니다.
...
- **진단 점검:** 진단 명령이 마이그레이션이나 복구를 시도하지 않도록 `volicord agent
  install --dry-run --output json` 또는 `volicord agent status --output json`을
  우선합니다.
...
- **검증:** 선택한 호환 Runtime Home에 대해 같은 dry-run 또는 status 명령을 다시
  실행합니다. 그 뒤 일반 설정 또는 `volicord-mcp --check`를 실행합니다.
...
- **관찰 증상:** `volicord agent uninstall`이 상태 `1`로 종료하고 `status:
...
- **진단 점검:** 같은 `--integration-id`, 사용했다면 `--installation-id`, 그리고
  `--allow-repository-write`, `--remove-managed` 플래그로 `volicord agent uninstall
  --dry-run --output json`을 실행해 정확한 남은 대상을 미리 봅니다.
- **제한된 복구 동작:** 이름 붙은 잔여 지침 또는 호스트 대상만 해결합니다. 소유
  marker가 여전히 맞을 때 `--remove-managed`와 함께 uninstall 또는 `volicord agent guidance remove`를
  다시 실행합니다.
- **검증:** 남은 Host Installation 인벤토리와 지침 상태를 확인하려면 `volicord
  agent status`를 실행합니다. 호스트 설정이 남아 있으면 그것에 의존하기 전에 `volicord
  agent verify`를 실행합니다.
...
- **관찰 증상:** `volicord agent status`가 Host Installation 인벤토리 또는 호스트 설정을
...
- **진단 점검:** `volicord agent status --integration-id <integration_id>`를 실행합니다.
  이전 MCP 프로세스가 아직 살아 있다면 `volicord.list_projects`를 호출해 빈 목록을
  반환하는지 확인합니다.
- **제한된 복구 동작:** 통합을 다시 사용해야 하면 `volicord agent project add`로
  프로젝트를 추가합니다. 통합을 완전히 제거해야 하면 지침 제거가 필요할 때 필수
  repository-write 플래그와 함께 `volicord agent uninstall --remove-managed`를
  실행합니다.
- **검증:** 다시 사용하려면 프로젝트를 추가한 뒤 `volicord-mcp --check`와 `volicord
  agent verify`를 실행합니다. 제거하려면 `volicord agent status`를 다시 실행하고 남은
  설치와 지침을 확인합니다.
