...
관리되는 내용은 호스트에게 범위, 상태, 쓰기 준비, 실행 증거, 사용자 판단, 닫기 준비 상태 추적에 하네스를 사용하라고 안내합니다. 대상 저장소가 불분명하면 `harness.list_projects`를 호출하고, 산문으로 하네스 상태를 만들어 내지 말라고 안내합니다. 또한 MCP 서버 instructions와 저장소 지침이 모델 동작을 보장할 수 없다는 점도 말합니다.
