...
| `host_hook_guarded` | 프로젝트 로컬 host hook에 검증된 생성 설정, native host output, 필수 phase, 쓰기 matcher, 일치하는 policy hash, 런타임 관찰이 있습니다. | pre-tool 결정, post-tool 상관, prompt capture, guard 상태, 닫기/쓰기 차단 사유가 workflow에 참여할 수 있습니다. |
