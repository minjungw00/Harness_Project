...
Codex, Claude Code, 또는 아직 직접 지원하지 않는 호스트를 위한 Volicord MCP 통합을 설치, 확인, 점검, 안내, 제거해야 할 때 이 가이드를 사용합니다.

먼저 `volicord`와 `volicord-mcp`를 빌드하거나 찾으려면 [설치](../getting-started/installation.md)를 보고, 가장 짧은 첫 설정은 [빠른 시작](../getting-started/quickstart.md)을 봅니다. 이 가이드는 그 뒤의 운영 경로를 다룹니다.

정확한 명령 동작은 [관리 CLI](../reference/admin-cli.md)가 담당합니다. 정확한 Agent Integration Profile, Host Installation, 프로젝트 선택, 지침 경계는 [에이전트 통합](../reference/agent-integration.md)이 담당합니다. 정확한 프로세스 동작은 [MCP 전송](../reference/mcp-transport.md)이 담당합니다. Runtime Home과 Product Repository 쓰기 경계는 [런타임 경계](../reference/runtime-boundaries.md)가 담당합니다.

증상별 복구는 [에이전트 호스트 문제 해결](agent-host-troubleshooting.md)을 사용합니다. 이 설정 가이드는 일반 운영 흐름을 먼저 유지합니다.

## 실행 파일 선택 규칙

아래 명령 예시는 `volicord`와 `volicord-mcp`가 함께 들어 있는 절대 디렉터리 하나를 선택하고 현재 셸에서 내보냈다고 가정합니다.

```sh
export VOLICORD_BIN="/absolute/path/to/selected/bin"
```

Volicord 소스 저장소 루트에서 빌드한다면 디버그 빌드는 아래 값을 사용할 수 있습니다.
...
사용자 범위와 로컬 범위는 다릅니다. 이 범위의 관리 호스트 항목은 선택된 Runtime Home을 `VOLICORD_HOME`으로 저장할 수 있고, 절대 `volicord-mcp` 실행 파일 경로를 저장할 수도 있습니다. 프로젝트 범위 시작 환경 요구사항을 이후 모든 호스트 프로세스에 같은 인라인 셸 값을 항상 다시 설정해야 한다는 보편 규칙으로 읽지 않습니다.

아래 생성 설정 예시에서 `/absolute/path/to/selected/bin/volicord-mcp`는 선택한 경로가 해석된 값을 나타내는 자리표시자입니다. 실제 생성 설정에는 사용자, 로컬, export 범위에서는 확장된 경로가, 프로젝트 범위에서는 이식 가능한 명령이 들어가며 문자 그대로의 `VOLICORD_BIN` 변수는 들어가지 않습니다. 프로젝트 범위 공유 설정은 개인 빌드 경로와 개인 `VOLICORD_HOME`을 의도적으로 생략합니다.

## 책임

| 부분 | 담당 | 참고 |
|---|---|---|
| Volicord 설치 | `volicord`와 `volicord-mcp` 실행 파일. | 소스 빌드는 `target/` 아래에 쓰고, 설치된 실행 파일은 다른 위치에 있을 수 있습니다. |
| `Volicord Runtime Home` | 프로젝트 registry, Agent Integration Profile, integration project membership, Host Installation 인벤토리, Volicord 런타임 데이터. | 모든 `Product Repository`와 분리해 둡니다. |
| `Product Repository` | 제품 파일과 명시적으로 선택한 프로젝트 범위 통합 파일. | Volicord 런타임 데이터베이스와 런타임 기록은 여기에 저장하지 않습니다. |
| Codex 또는 Claude Code | 호스트 설정, 프로젝트 신뢰, 프로젝트 MCP 승인, 재로드/재시작 동작, MCP 서버를 시작할 때 쓰는 환경, 모델의 도구 선택. | Volicord는 호스트가 소유한 결정을 우회할 수 없습니다. |
| `volicord-mcp` 프로세스 | `--integration <integration_id>`로 시작되는 하나의 통합 바인딩 stdio 서버. | 프로젝트 선택은 공개 도구 호출마다 일어납니다. |

## 설정 순서

운영자 관점에서 `volicord agent install`은 아래 지속 순서를 따릅니다. 자세한 구현 지도는 [관리 에이전트 설정 흐름](../development/architecture.md#administrative-agent-setup-flow)에 있습니다.

1. 명령은 호스트, 범위, 저장소 쓰기, 지침, Runtime Home, 저장소, integration, 실행 파일 입력을 파싱하고, 기존 registry와 호스트 상태를 읽어 프로젝트, 통합, 호스트, 선택적 지침 계획을 만듭니다. 충돌은 지속 설정 전에 거부됩니다.
2. `--dry-run`이면 명령은 계획만 반환합니다. Runtime Home 상태를 만들거나, SQLite에 쓰거나, `volicord-mcp --check`를 실행하거나, 호스트 설정을 바꾸거나, 지침을 적용하거나, MCP를 초기화하거나, 도구를 발견하지 않습니다.
3. `--dry-run`이 아니면 Runtime Home과 프로젝트 상태를 초기화하거나 재사용한 뒤, 에이전트 접점, Agent Integration Profile, 프로젝트 멤버십, 기본 프로젝트 라우팅을 만들거나 재사용합니다.
4. 명령은 호스트 설정을 적용하기 전에 해석된 Runtime Home으로 `volicord-mcp --check --integration <integration_id>`를 실행합니다.
...
| 상태 | 의미 |
|---|---|
| `complete` | 지속되는 통합 상태가 있고, 관리되는 호스트 설정이 관리 지문과 일치하며, 호스트별 로드 가능성 게이트가 충족되고, 필요한 신뢰나 승인 동작이 남아 있지 않고, 통합 사전 점검과 MCP 초기화가 성공했으며, 도구 발견이 필요한 도구를 노출했습니다. |
...
Codex project 범위는 Codex 프로젝트 신뢰를 확인할 수 없는 동안 `action_required`로 남습니다. Claude Code project 범위는 프로젝트 MCP 승인이 대기 중인 동안 `action_required`로 남습니다. 거절됨, 없음, 변경됨, 사용할 수 없음, 알 수 없음 호스트 상태는 `complete`가 아닙니다. Generic export는 Volicord가 사용자가 관리하는 호스트가 내보낸 설정을 로드했다는 사실을 증명할 수 없으므로 `action_required`로 남습니다.

`volicord-mcp --check --integration <integration_id>`는 MCP 시작 검증일 뿐입니다. Volicord가 직접 시작한 MCP handshake는 Codex 또는 Claude Code가 서버를 로드, 신뢰, 승인, 노출했다는 증명이 아닙니다. 도구 발견이 성공해도 이후 모델이 매번 Volicord 도구를 선택한다는 보장은 아닙니다. 저장소 지침은 발견 가능성을 높이지만, 강제 장치가 아니라 조언 맥락입니다.

결과가 `complete`가 아니면 쓰기 명령을 반복하기 전에 [에이전트 호스트 문제 해결](agent-host-troubleshooting.md)에서 해당 상태나 관찰된 호스트 상태를 확인합니다.

## 쓰기 전 dry-run

호스트 설정이나 `Product Repository` 지침을 쓸 수 있는 명령에는 dry-run을 사용합니다.
...
Dry-run은 계획된 Runtime Home 동작, 호스트 대상 경로, 지침 대상 경로를 보고합니다. 아무것도 만들거나 수정하지 않습니다. Runtime Home 디렉터리, SQLite 데이터베이스나 행, WAL 또는 SHM 파일, registry 마이그레이션, 호스트 설정, `Product Repository` 지침, generic export 파일, MCP 호스트 상태, `volicord-mcp --check`, MCP 초기화, 도구 발견을 만들거나 실행하지 않습니다.
...
하나의 개인 Codex 설정이 여러 Codex 프로젝트에서 같은 Volicord 통합을 로드해야 할 때 사용자 범위를 사용합니다.
...
- `/Users/alex/.volicord` 아래 Runtime Home 기록
...
실제 생성되는 `command` 값은 `VOLICORD_BIN`으로 선택한 경로가 해석된 절대 경로입니다. 생성된 TOML에는 `VOLICORD_BIN`이 들어가지 않습니다.

Codex 프로젝트 범위도 지원되지만 `/work/acme-api/.codex/config.toml`에 쓰고, 비대화형 실행에서는 `--allow-repository-write`가 필요하며, `PATH`의 `volicord-mcp`를 사용합니다. Codex가 프로젝트를 신뢰할 때까지 `action_required`를 보고할 수 있습니다. 생성되는 프로젝트 항목은 `command = "volicord-mcp"`와 개인 `VOLICORD_HOME` 없는 이식 가능한 형태로 남습니다. 해당 프로젝트의 Codex는 `PATH`에서 `volicord-mcp`를 찾을 수 있는 환경에서 시작하거나 재시작하고, 그 Codex 프로세스가 다른 Runtime Home을 해석하게 된다면 그 환경에서 `VOLICORD_HOME`을 제공해야 합니다. 이 값을 `volicord agent install` 또는 `volicord agent verify`에만 설정하면 그 관리 명령 실행에만 영향을 주며 나중의 Codex 프로세스에는 적용되지 않습니다.
...
저장소 지침은 선택 사항이며 명시적으로 승인해야 합니다.

Codex 지침은 `AGENTS.md`에 Volicord 관리 블록을 씁니다.
...
Claude Code 지침은 `.claude/rules/volicord.md`에 씁니다.
...
지침 적용 전 대상 파일은 없거나 Volicord 관리 블록이 없습니다.

```text
# Existing repository instructions
```

Codex 지침 적용 뒤 `AGENTS.md`에는 관리 블록이 들어갑니다.
...
Claude Code 지침 적용 뒤 `.claude/rules/volicord.md`에는 `## Volicord MCP guidance for Claude Code`를 포함한 같은 관리 marker 모양이 들어갑니다.

관리되는 내용은 호스트에게 범위, 상태, 쓰기 준비, 실행 증거, 사용자 판단, 닫기 준비 상태 추적에 Volicord를 사용하라고 안내합니다. 대상 저장소가 불분명하면 `volicord.list_projects`를 호출하고, 산문으로 Volicord 상태를 만들어 내지 말라고 안내합니다. 또한 MCP 서버 instructions와 저장소 지침이 모델 동작을 보장할 수 없다는 점도 말합니다.

지침 파일은 호스트 설정 또는 조언 맥락입니다. Volicord 런타임 상태, Core 권한, 증거, 수락, 닫기 준비 상태, 잔여 위험 수락, 보안 보장이 아닙니다.

## 상태와 검증

레지스트리와 호스트 인벤토리를 점검합니다.
...
검증을 새로고침합니다. 이것도 별도의 관리 명령 실행입니다. Runtime Home은 `--runtime-home`이나 `VOLICORD_HOME`으로 이 명령에 제공하고, 호스트 설정이 이식 가능한 `volicord-mcp` 명령을 저장한 설치를 검증할 때는 선택한 디렉터리를 `PATH`에 둡니다. 이 값들은 검증 명령이 자체 점검을 시작하게 해 주지만, 관리 호스트 항목에 이미 저장된 값이나 호스트 자신의 시작 환경이 제공하는 값 밖으로 이후 호스트 프로세스가 받는 환경을 바꾸지는 않습니다.
...
- 없는 실행 파일, 경로 실수, 호스트 파일 충돌, 빈 허용 목록, 모호한 프로젝트 선택, 관리 fingerprint 충돌은 문제 해결 가이드에 각각 집중 복구 경로가 있습니다.

상태 인벤토리나 `volicord-mcp --check`를 외부 호스트가 서버를 로드했다는 증거로 취급하지 않습니다. 호스트 소유 복구 동작을 끝낸 뒤 `volicord agent verify`를 사용합니다.
...
허용 프로젝트가 없어도 Agent Integration Profile, Host Installation 인벤토리, 호스트 설정은 남을 수 있지만, 이 저장 상태는 시작 자격이 아닙니다. 이 상태에서 복구하려면 [현재 허용 프로젝트가 없지만 호스트 설정이 남아 있음](agent-host-troubleshooting.md#host-config-remains-zero-projects)을 봅니다.

관리되는 호스트 설정과 관리되는 지침을 완전히 제거합니다.
...
Uninstall은 소유권과 안전 점검이 허용할 때 선택된 Volicord 관리 호스트 항목, 블록, 파일, fingerprint만 제거합니다. `--remove-managed`를 사용하면 선택되어 있고 안전하게 소유된 관리 `Product Repository` 지침도 제거합니다. 성공한 제거는 해당 Host Installation 인벤토리도 제거합니다. Agent Integration Profile에 남은 Host Installation이 없으면 프로필이 비활성화될 수 있으며, 비활성화는 삭제가 아닙니다. `Product Repository` 내용, 프로젝트 등록과 프로젝트 상태, Core의 작업, 증거, 판단, 실행, 아티팩트 관련 기록, 아티팩트 저장소, 관련 없는 호스트 설정은 보존됩니다. 사용자가 수정했거나 관리되지 않는 호스트 항목은 제거하지 않고 보고하거나 보존합니다.

## Generic export fallback

Volicord가 직접 설치하지 않는 호스트에만 generic export를 사용합니다.
...
Export된 JSON에는 `command`, `args = ["--integration", "int-generic-acme"]`, 적용될 때 `VOLICORD_HOME`을 가진 하나의 `mcpServers.volicord-main` 항목이 들어갑니다.
...
Generic export는 호스트가 서버를 로드했다고 주장하지 않습니다. 나중에 호스트별 담당 문서가 관찰 가능한 로드 가능성 게이트를 정의하기 전까지 설치와 검증 결과는 `action_required`로 남습니다.
