...
선택된 Product Repository를 Connection Projects에 추가합니다. 운영자가 내부 연결 식별 정보를 다룰
필요는 없습니다.
...
사용자가 사용 가능한 Product Repository를 물으면 에이전트는 아래를 호출합니다.
...
프로젝트가 연결된 뒤 한 Product Repository를 대상으로 하는 공개 Volicord 메서드 호출은
...
- Agent Connection은 명시적으로 연결된 Product Repository에만 접근합니다.
