...
읽고 나면 어떤 작업이 아주 작게 유지될 수 있는지, 언제 요구사항 구체화가 필요한지, 사용자 판단이 왜 작업을 막을 수 있는지, 근거가 어떤 역할을 하는지, 왜 닫기가 아직 막힐 수 있는지, 읽기용 Markdown 보고서가 왜 상태가 아닌지 구분할 수 있어야 합니다.

## 이런 때 읽기

하네스를 처음 접하고, 정확한 gate, 스키마, DDL, 읽기용 요약(Projection) 규칙, conformance fixture를 배우기 전에 실용적인 예시를 먼저 보고 싶을 때 읽습니다.
...
아래 예시는 온보딩용 예시이지 스키마나 새 권한 경로가 아닙니다. 정확한 동작은 끝부분에 연결한 기준 문서 소유자에 남아 있습니다.
...
Tiny direct는 여전히 `direct` 아래에 있습니다. 별도 mode가 아니며 사용자 판단, 보안 경계, 근거, 범위, 쓰기 허가, 잔여 위험 표시, 닫기 규칙을 우회하지 않습니다. 문서 수정이 의미를 바꾸거나, 링크/렌더링 확인이 필요하거나, 엄격한 Reference contract에 닿거나, 변경 경로와 자체 확인을 넘는 근거가 필요해지면 같은 작업을 일반 작은 변경(`direct`) 또는 추적되는 작업(`work`)으로 옮겨야 합니다.
...
이것은 막연한 "승인할까요?" 질문이 되면 안 됩니다. 에이전트는 실제 선택, 선택지, 추천, 불확실성, 미뤘을 때의 영향을 보여줘야 합니다.
