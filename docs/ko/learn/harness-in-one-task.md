...
읽고 나면 Task, Change Unit, Decision Packet, 승인, Write Authorization, 근거, 검증, Manual QA, 수용, 남은 위험, 닫기가 왜 필요한지 감을 잡을 수 있습니다. 내부 기록 세부사항을 몰라도 흐름을 따라갈 수 있어야 합니다.
...
선택한 동작에 민감한 승인이 필요하면 Harness는 쓰기 전에 멈추고 별도 승인을 요청해야 합니다. 승인은 "이 민감한 행동을 진행해도 되는가?"에 답합니다. Decision Packet, 테스트, QA, 위험 수용, 최종 수용을 대신하지 않습니다.
...
분리된 검증을 실행할 수 없다면 에이전트는 그 사실을 분명히 말하고 남은 검증 위험을 보여야 합니다. 검증 없이 닫는 것은 위험을 수용한 닫기이지 분리된 검증이 아닙니다.
...
수용 또는 위험을 수용한 닫기 전에 에이전트는 알려진 남은 불확실성을 보여 줍니다.
...
최종 수용이 필요한 경로라면 사용자는 근거, 검증 또는 수용된 검증 위험, Manual QA 상태, 남은 위험을 본 뒤 결과를 받아들입니다.

```text
수용합니다. remember-me 동작은 선택한 옵션과 맞고, 표시된 남은 위험도 받아들일 수 있습니다.
```

수용은 승인과 다릅니다. 승인은 민감한 단계를 진행하게 할 수 있지만, 수용은 완료된 결과가 충분히 좋은지 판단합니다.

### 닫기

Harness는 관련 blocker가 처리된 뒤에만 닫습니다. 범위가 맞고, 결정이 해결되었거나 유효하게 미뤄졌고, 쓰기가 허용되었고, 근거가 Task에 충분하고, 검증과 QA가 통과했거나 명시적으로 처리되었고, 남은 위험이 보였고, 필요한 수용이 기록되어야 합니다.

닫기 요약은 짧아야 합니다.

```text
`work` 작업으로 닫음. 근거 기록됨. 검증과 Manual QA 처리됨. 필요한 경우 남은 위험을 표시하고 수용함.
```
...
남은 위험 표시 및 수용: 지원하는 모든 브라우저 정책 조합에서는 확인하지 않았습니다.
수용이 기록되어 `work` 작업으로 닫았습니다.
```

### 참고 문서가 더 엄격하게 정의하는 것

참고 문서는 모드 규칙, Decision Packet compatibility, 승인 처리, Write Authorization 동작, 근거 충분성, 검증 독립성, QA 게이트, 남은 위험 표시, 수용 시점, 닫기 의미를 정확히 정의합니다.
...
| "지금 이 파일을 수정해도 되나?" | Write Authorization | 의도한 쓰기가 현재 Task, Change Unit, 결정, 승인과 맞는지 확인한다. | [커널 참조](../reference/kernel.md); agent surface 세부 담당: [Agent 통합 참조](../reference/agent-integration.md). |
...
| "이제 끝났다고 해도 되나?" | 닫기 | 관련 blocker가 처리된 뒤에만 Task를 완료한다. | [커널 참조](../reference/kernel.md); agent surface 세부 담당: [Agent 통합 참조](../reference/agent-integration.md). |
