...
Self-check는 유용하지만 독립 검증은 아닙니다. `direct` 작업은 보통 사용자가 추가 검증을 요청하지 않는 한 self-checked 수준으로 닫힙니다.
...
제품 파일을 쓰기 전에 에이전트는 의도한 수정이 활성 Task, Change Unit, 승인, 해결된 결정과 맞는지 Harness에 확인합니다.
...
선택한 동작에 민감한 승인이 필요하면 Harness는 쓰기 전에 멈추고 별도 승인을 요청해야 합니다. 승인은 "이 민감한 행동을 진행해도 되는가?"에 답합니다. Decision Packet, 테스트, QA, 남은 위험을 받아들이는 판단, 최종 수락을 대신하지 않습니다.
...
`work`에서는 사용자가 검증 위험을 명시적으로 받아들이지 않는 한 `direct` self-check보다 더 강한 확인이 기대됩니다.
...
수락은 승인과 다릅니다. 승인은 민감한 단계를 진행하게 할 수 있지만, 수락은 완료된 결과가 충분히 좋은지 판단합니다.
...
Remembered session과 non-remembered session에 대한 검증이 통과했습니다.
로그인 화면 흐름에 대한 Manual QA가 통과했습니다.
남은 위험 표시 및 받아들인 판단: 지원하는 모든 브라우저 정책 조합에서는 확인하지 않았습니다.
수락이 기록되어 `work` 작업으로 닫았습니다.
```
...
- Task, Change Unit, Decision Packet, 승인, Write Authorization, 수락, 남은 위험, 닫기의 엄격한 동작이 필요하면 [커널 참조](../reference/kernel.md)를 봅니다.
