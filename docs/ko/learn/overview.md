...
flowchart LR
  ...
  Product -->|요청과 사실| Server
  Server -->|범위 내 쓰기와 요약| Product
  Server -->|상태와 ArtifactRef| Home
  Home -->|Core 기록| Server
```

이 문서 저장소는 세 공간 중 향후 하네스 서버 소스 저장소 역할을 맡도록 준비되고 있습니다. 제품 저장소나 하네스 런타임 홈은 아닙니다. 아직 이곳에는 서버/런타임 구현이 없습니다. 구현을 시작하려면 문서 수락과 별도의 구현 계획 준비 결정이 모두 필요합니다.
