...
| 검증 상태 | Agent Connection 레지스트리 기록의 `last_verification_status`와 [관리 CLI](admin-cli.md#agent-connection-result-states)가 담당하는 명령 출력입니다. | `volicord init`, `volicord connect`, `volicord connection verify`가 관찰 가능한 호스트 설정, MCP 시작, MCP 초기화, `tools/list` 점검을 가능한 곳에서 실행합니다. | 검증은 Volicord 쪽 상태와 호스트/MCP 준비 상태를 모두 살펴볼 수 있습니다. MCP 시작 검증만으로는 `complete` Agent Connection이 아닙니다. |
