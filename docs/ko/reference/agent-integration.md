...
항상 적용되는 고정 규칙과 맥락은 짧고 최신이어야 하며, 그 자체가 권한 출처가 아닙니다. 항상 주입되는 운영 맥락의 예산은 역할, 현재 단계와 맥락 프로필, 현재 Task 요약, 활성 blocker, 대기 중인 사용자 소유 판단, 다음 허용 행동입니다. 항상 적용되는 고정 규칙은 언제 Harness를 쓰는지, current status 또는 현재 위치 맥락을 어디서 읽는지, Journey Card는 해당 projection/profile이 활성화되어 있고 최신일 때만 쓴다는 점, product write에는 `prepare_write`가 필요하다는 점, 사용자 소유 판단은 Decision Packet으로 라우팅한다는 점, status가 실행 전에 실제로 막을 수 있는 것과 실행 뒤에만 감지할 수 있는 것을 보여야 한다는 점, authoritative MCP를 사용할 수 없으면 product write를 보류한다는 점만 알려주면 충분합니다. schema dump, 오래된 작업 이력, evidence body 복사본, 읽기용 요약 전체 본문, reference contract 복제본으로 늘리면 안 됩니다. 세션 절차 자체는 [에이전트 세션 흐름](../use/agent-session-flow.md)이 담당합니다.
...
각 항목에는 source ref와 freshness marker를 붙일 수 있습니다. 하지만 이 예산이 전체 reference 문서, public schema, 오래된 작업 이력, evidence body 복사본, 관련 없는 template, historical event log, 읽기용 요약 전체 본문으로 늘어나면 안 됩니다. 그런 자료는 필요할 때만 가져옵니다.
...
다음은 compact current-state envelope에 들어갈 수 있는 후보 field입니다. 표 전체를 매번 보내라는 뜻이 아닙니다. 현재 맥락 프로필, next safe action, freshness, relevance가 어떤 field를 표시할지 결정합니다.

현재 맥락 프로필로 걸러서 push할 수 있는 envelope 후보:
...
|---|---|---|---|---|---|
| 세션 시작 | 역할 또는 연결된 접점 자세, 현재 단계와 맥락 프로필, active 또는 likely Task 요약, 활성 blocker, 대기 중인 사용자 소유 판단, 다음 허용 행동, guarantee/MCP availability, projection을 사용할 때만 projection freshness. | [에이전트 세션 흐름: 세션 시작](../use/agent-session-flow.md#세션-시작), [이어가기](../use/agent-session-flow.md#이어가기), 현재 `harness.status` / `harness.next` output, projection 최신성이 다음 행동에 영향을 줄 때만 [문서 Projection: Freshness and failure rules](document-projection.md#freshness-and-failure-rules). | 전체 작업 이력, 전체 참조 문서, 전체 schema, 오래된 projection, 관련 없는 template, 관련 없는 Roadmap. | 사용자는 쉬운 현재 위치, primary blocker, 대기 중인 사용자 결정, 다음 안전한 행동, 접점이 실제로 막을 수 있는 것 또는 감지만 할 수 있는 것을 봅니다. | 상태가 중요하면 current Core status 또는 state-derived 현재 위치 맥락을 읽습니다. Journey Card는 해당 projection/profile이 활성화되어 있고 최신일 때만 사용합니다. Core/MCP를 사용할 수 없으면 memory를 상태처럼 쓰지 말고 unavailable이라고 말합니다. |
| 요구사항 구체화 (Discovery) | 목표, 알려진 경우 사용자 가치, 범위와 비목표, 수용 기준 단서, 확인 가능한 사실, assumption, 추적되는 불확실성, 사용자 소유 판단 후보, 활성 blocker, 다음 허용 구체화 action. | [사용자 가이드: 에이전트가 처음 답해야 할 것](../use/user-guide.md#에이전트가-처음-답해야-할-것), [에이전트 세션 흐름: 요청 정리](../use/agent-session-flow.md#요청-정리), [범위와 Change Unit](../use/agent-session-flow.md#범위와-change-unit), 관련 현재 Task/Change Unit/Shared Design 참조. 사용자 소유 route를 나눠야 할 때만 [커널: 판단 경로 경계](kernel.md#판단-경로-경계). | 현재 질문에 필요하지 않은 전체 module map, 오래된 PRD/design, design-policy catalog, 관련 없는 template, 전체 Storage DDL, 전체 Conformance 카탈로그. | 사용자는 추천, 불확실성, 미룰 때 영향, 확인 가능한 사실, 안전하게 계속할 수 있는 일이 포함된 focused question을 봅니다. | 질문하기 전에 current repo/docs/state 참조를 확인합니다. Unavailable 또는 stale source는 표시하고, stale design prose, chat memory, projection을 authority로 바꾸지 않습니다. |
| 사용자 결정 요청 | 정확한 사용자 소유 결정, decision profile, judgment domain, profile에 맞는 options 또는 chosen outcome, 영향을 받는 범위, 관련 참조 또는 명시적 absence, 답변이 확정하지 않는 것, 답변 뒤의 next action. 상세 프로필은 trade-offs, 추천, 불확실성, affected gates/acceptance criteria, deferral consequence도 보여줍니다. | [에이전트 세션 흐름: 사용자 소유 결정으로 막힐 때](../use/agent-session-flow.md#사용자-소유-결정으로-막힐-때), [커널: Decision Packet](kernel.md#decision-packet), [Decision Gate](kernel.md#decision-gate), 정확한 API field가 필요할 때만 [`harness.request_user_decision`](mcp-api-and-schemas.md#harnessrequest_user_decision). | 포괄적 approval 표현, 관련 없는 decision, 전체 근거 본문, 전체 log, 전체 schema reference, 전체 Template 세트. | 사용자가 충분히 결정할 수 있게 exact choice, options, 필요할 때 recommendation, scope, 참조, uncertainty, 답변이 확정하지 않는 것을 보여줍니다. | Current Decision Packet 또는 current state-derived decision candidate에서 가져옵니다. Ref가 stale 또는 missing이면 묻기 전에 표시합니다. Replay id, routing metadata, internal enum detail은 경계를 설명할 때만 보조로 둡니다. |
| 쓰기 준비 | Active Task와 Change Unit, intended paths/tools/commands/network/secrets summary, scope와 out-of-bounds, Autonomy Boundary, pending decisions 또는 Approval needs, baseline/state freshness, Write Authority Summary, guarantee/MCP availability. | [에이전트 세션 흐름: 제품 파일 쓰기](../use/agent-session-flow.md#제품-파일-쓰기), [커널: prepare_write](kernel.md#prepare_write), [커널: Write Authorization](kernel.md#write-authorization), 관련 메서드인 [`harness.prepare_write`](mcp-api-and-schemas.md#harnessprepare_write). Sensitive category가 문제일 때만 해당 보안 소유자 섹션. | 전체 Kernel/reference 문서, 관련 없는 schema, historical event log, 큰 diff/log, 전체 Storage DDL. | 사용자는 무엇이 바뀔 수 있는지, 무엇이 막혔는지, write-authority status, 가장 작은 unblocker를 봅니다. | Current Core state와 정확한 `prepare_write` input을 사용합니다. Stale baseline, stale projection, state conflict, intended path 변경, sensitive category 변경이 있으면 refresh 또는 reconcile합니다. |
| 실행 / 근거 | Consumed Write Authorization 또는 no-write basis, changed-path summary, command/tool summary, run outcome, Evidence Manifest ref, integrity/freshness가 있는 artifact refs, evidence gaps, redaction/omission/block notes, next evidence action. | [에이전트 세션 흐름: 근거와 확인](../use/agent-session-flow.md#근거와-확인), [커널: record_run](kernel.md#record_run), [커널: Evidence Manifest](kernel.md#evidence-manifest), [`harness.record_run`](mcp-api-and-schemas.md#harnessrecord_run), display 또는 artifact-ref 질문이 있을 때만 [문서 Projection: Artifact reference 렌더링](document-projection.md#artifact-reference-렌더링). | 전체 log, raw diff, screenshot, trace, bundle, artifact inventory, 읽기용 요약 전체 본문, 특정 display repair에 필요하지 않은 전체 Template 세트. | 사용자는 criterion 또는 claim에 연결된 coverage와 gap, ref, redaction/integrity note를 봅니다. Raw artifact body는 pull-only로 둡니다. | Current run/artifact refs와 current state에서 기록합니다. Baseline, paths, approval, artifact integrity, relevant owner record가 바뀌면 evidence를 stale로 표시합니다. |
| 닫기 준비 상태 | Scope match, 수용 기준, evidence coverage, verification status, 수동 QA status, 작업 수락 필요/상태, 잔여 위험 표시와 필요할 때 accepted refs, close blockers, projection freshness, smallest unblocker. | [에이전트 세션 흐름: 닫기](../use/agent-session-flow.md#닫기), [검증, 수동 QA, 잔여 위험, 작업 수락](../use/agent-session-flow.md#검증-수동-qa-잔여-위험-작업-수락), [커널: close_task](kernel.md#close_task), [Gates](kernel.md#gates), [Waiver semantics](kernel.md#waiver-semantics), [`harness.close_task`](mcp-api-and-schemas.md#harnessclose_task), readable close context를 쓸 때 [문서 Projection: Freshness and failure rules](document-projection.md#freshness-and-failure-rules). | 일반적인 all-done rollup, 전체 report 본문, 전체 historical log, 관련 없는 template, 전체 Conformance 카탈로그, 읽기용 요약 전체 본문. | Acceptance 또는 close 전에 close basis를 보여줍니다. Scope, user decisions, evidence, verification, 수동 QA, residual risk, acceptance status, primary blocker, smallest unblocker를 분리합니다. | Current Core gates, owner records, evidence/artifact refs, projection freshness를 읽습니다. Stale projection은 blocker를 요약할 수 있어도 authority가 될 수 없습니다. Exact `close_task` payload는 blocker 설명에 필요할 때만 보여줍니다. |
...
|---|---|
| v0.2 User-Facing Harness MVP | User-readable status/next card, Decision Packet display, pending user judgment routing, 근거와 닫기 준비 상태 요약, final-acceptance separation, relevant한 residual-risk visibility. |
