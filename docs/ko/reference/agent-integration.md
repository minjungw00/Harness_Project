...
사용자 세션에서 에이전트가 무엇을 말해야 하는지는 [에이전트 가이드](../use/agent-guide.md)를 봅니다. Core, API, 스키마, 저장소, Projection, 보안, 적합성, 운영 계약은 다음 행동에 필요한 담당 부분만 가져옵니다. later 후보, 접점별 recipe, 적합성 계획을 active 요구사항으로 바꾸면 안 됩니다.
...
- 에이전트가 어떤 맥락을 넣을지 판단하게 돕는 짧은 접점별 recipe
- 커넥터 적합성 경계
...
- Projection/template 권한과 active 렌더링 템플릿 본문. [Projection과 Template 참조](projection-and-templates.md)를 봅니다.
...
이 문서의 접점별 recipe는 통합 지침입니다. Core 상태 권한, Write Authorization, 증거, 검증, QA, 민감 동작 승인, 최종 수락, 잔여 위험 수락, 닫기 준비 상태를 만들지 않고, later 후보를 active 의무로 바꾸지 않으며, 새로운 보안 권한 경계를 만들지 않습니다.
...
정확한 공개 tool/resource 계약은 API 담당 문서가 담당합니다. 커넥터는 사용할 수 있는 부분집합을 요약할 수 있지만, 메서드 스키마 전체를 prompt 맥락에 중복 주입하면 안 됩니다.
...
생성된 rule, skill, MCP snippet, adapter file, 관리형 block에는 connector manifest가 필요합니다. Manifest는 생성 경로, 관리형 block id와 hash, MCP 노출 태세, 표시해도 안전한 handle, profile 최신성, drift, 대체 동작을 기록합니다. 원시 token, secret, 비공개 설정값, 차단된 payload byte, 기준 Task state는 저장하지 않습니다.

## 3. 보장 수준

보장 label 표시는 [보안 참조](security.md#정직한-guarantee-display)를 따릅니다. 정확한 스키마 값 집합은 [API Schema Core](api/schema-core.md#current-mvp-value-sets)가 담당합니다. 이 참조는 커넥터가 `capability_profile`을 사용자에게 보이는 표시로 어떻게 연결하는지 담당합니다.
...
| 이름 | 커넥터 표시 규칙 |
|---|---|
| `preventive` | 승격된 profile에 명시적인 profile 지원이 있을 때만 사용합니다. fixture로 입증된 후크, 래퍼, 권한 계층, 정책 엔진, 사이드카 경로와 실행 전에 막을 수 있는 정확한 동작을 이름 붙입니다. |
| `isolated` | 승격된 profile에 명시적인 profile 지원이 있을 때만 사용합니다. 문서화된 분리 경계를 이름 붙입니다. 해당 메커니즘이 입증되지 않았으면 운영체제 샌드박싱, 권한 격리, 변조 방지 저장소를 암시하지 않습니다. |

에이전트는 사용자가 더 강한 안전성을 요청했거나, guard/freeze/careful mode를 요청했거나, 대화에서 더 강한 표현을 썼다는 이유만으로 `preventive` 또는 `isolated`를 고르면 안 됩니다. 활성 profile이 더 강한 주장을 증명하지 못하면 커넥터는 보장 label을 낮추거나 `CAPABILITY_INSUFFICIENT`를 반환해야 합니다.

Reference local MCP profile은 `cooperative` 동작과, 변경 경로 또는 아티팩트 공백 관찰이 뒷받침하는 제한된 `detective` 동작만 표시할 수 있습니다. `pre_tool_blocking_supported=false`와 `isolation_supported=false`이므로 `preventive`나 `isolated` 동작을 주장하면 안 됩니다.
...
커넥터는 작고 최신인 현재 맥락을 주입하고, 큰 담당 문서는 다음 행동에 필요할 때만 가져옵니다. 맥락 패킷은 다음 에이전트 행동을 돕는 운영 지원 맥락입니다. 에이전트 기억, 대화 기록, 전체 보고서, 캐시된 Projection 본문, Reference 전체 본문이 아닙니다.

검색 비용 원칙:

- Reference 전체 묶음을 기본으로 주입하지 않습니다.
...
- later 후보, 향후 목록 항목, 접점별 recipe, 적합성 계획을 active 요구사항으로 승격하지 않습니다.
- 다음 행동에 필요한 담당 부분만 가져오고 멈춥니다.
- 일반 작업 prompt에서는 하나의 언어를 고릅니다. 같은 `doc_id`의 영어/한국어 대응 문서를 같은 prompt에 함께 넣는 에이전트 중복 주입 금지를 지킵니다. 이중 언어 검토는 대응 문서 전체를 주입하지 말고 필요한 부분만 비교합니다.
...
- 닫기 차단 사유
- 잔여 위험 요약
- 보장 수준. Core 또는 필요한 MCP가 답할 수 없으면 unavailable/capability 조건
- 출처 참조와 최신성

Reference 자료 전체, 전체 스키마, 전체 DDL, Projection 본문 전체, 아티팩트 본문 전체, 관련 없는 템플릿, 향후 목록, 최신이 아니거나 관련 없는 Task 이력, 과거 로그를 항상 주입되는 맥락에 넣지 않습니다.
...
행동에 엄격한 계약이 필요하면 담당 섹션을 연결하거나 가져옵니다. 일반 prompt에 담당 문서 전체를 붙여 넣지 않습니다.
...
| Capability insufficient | 요청한 쓰기, 캡처, guard, 격리, 보장이 지원하지 않는 필드에 의존할 때. | `CAPABILITY_INSUFFICIENT` 또는 구조화된 차단 사유를 반환하고 보장 label을 낮춥니다. |
| MCP unavailable | 접점 또는 호출 경로가 현재 Core 권한 경로에 닿지 못할 때. | 안정적인 공개 `MCP_UNAVAILABLE` 동작을 사용하고 상태 변경을 주장하지 않습니다. |
| Local access mismatch | 호출자 또는 전송 경로가 등록된 local profile 밖일 때. | 표시해도 안전한 진단과 함께 `LOCAL_ACCESS_MISMATCH`를 사용합니다. 접점별 `UNAUTHORIZED` code를 만들지 않습니다. |

`MCP_SERVER_UNAVAILABLE`과 `SURFACE_MCP_UNAVAILABLE`은 진단 조건입니다. `MCP_UNAVAILABLE`은 안정적인 공개 availability code로 남습니다.

Core에 닿을 수 없는 동안 대화 기억, 생성 파일, 캐시된 Projection, 오래된 status text, 운영자 서술에서 Core 상태, Write Authorization, gate 상태, 승인, 증거, 최종 수락, 잔여 위험 수락, Projection 복구, 닫기 준비 상태를 만들어 내지 않습니다.
...
접점별 recipe는 에이전트가 어떤 맥락을 포함할지 판단하게 돕는 짧은 통합 메모입니다. 별도 Reference 담당 문서가 아니며 긴 접점별 작업 흐름으로 커지면 안 됩니다.
...
일반 Core 규칙, 공개 API 스키마, Reference 문서 전체, 향후 커넥터 확장 구상, 호스팅 에이전트 가정, 감사 메모, 관련 없는 later 후보 항목, Projection 본문 전체, 긴 설정 튜토리얼을 넣지 않습니다. recipe가 later 자료를 가리킬 수는 있지만, 그 자료를 active MVP 필수사항으로 만들면 안 됩니다.
...
- 실제 `capability_profile` 필드에서 파생된 보장 표시
- `capability_profile`이 증명하지 못하면 `preventive` 또는 `isolated` 주장 없음
