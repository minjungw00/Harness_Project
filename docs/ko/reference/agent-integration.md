...
접점 이름 자체는 역량이 아닙니다. 커넥터는 실제 호스트, 버전/설정, 작업공간 정책, MCP 연결 태세, 활성 `stage_artifact` 스테이징 경로에 맞춘 `capability_profile`을 사용해야 합니다. 향후 캡처 경로, 가드 경로, 분리 경계가 승격된다면 그 경계도 프로필 범위에 포함해야 합니다.
...
`surface_status`, `local_access_posture`, `supported_access_classes`는 커넥터의 현재 API 호환성 태세를 보고합니다. `surface_status`는 저장된 `LocalSurfaceRegistration.status`와 일치해야 하며, 이 필드들 자체가 권한을 부여하지 않습니다. 서버는 커넥터 설명, 생성 파일, 대화 텍스트, Product Repository 파일, 에이전트 기억이 아니라 로컬 transport/session/binding 사실에서 현재 로컬 접점을 확인합니다. 현재 접근 분류 라벨, `LocalSurfaceRegistration`, `VerifiedSurfaceContext`는 [API Schema Core](api/schema-core.md#local-surface-access-values)가 담당하고, 최소 요청 조건은 [현재 MVP API](api/mvp-api.md#shared-request-rules)가 담당합니다. 기준 프로필에서 `artifact_read`는 담당 경로를 통한 등록된 `ArtifactRef` 읽기만 뜻합니다. 아티팩트 본문 읽기는 확인된 로컬 접점이 필요하며, `raw_artifact_path_read_supported=false`는 아티팩트 저장소 아래 로컬 파일시스템 경로만으로 아티팩트 바이트를 읽을 수 없다는 뜻입니다. `artifact_registration`에서는 `stage_artifact`가 문서화된 스테이징 핸들을 제공할 수 있습니다. `captured_artifact` 핸들과 접점 자체 아티팩트 캡처는 향후 담당 문서 지원이 필요하며, 활성 기준 프로필에는 그 지원이 없습니다.
...
접점 버전, MCP 설정, 권한, 작업공간 정책, 생성 파일, 관리형 블록, `stage_artifact` 경로, 가림 정책, 아티팩트 보존 방식, 로컬 접근 태세, 적합성 근거가 바뀌면 `capability_profile`을 갱신합니다. 하지만 프로필 갱신만으로 `LocalSurfaceRegistration`이 새로 고쳐지지는 않습니다. 등록 변경은 현재 로컬 접점 맥락을 확인하는 서버 담당 경로가 필요합니다. 후크, 접점 자체 캡처, 도구 실행 전 차단, 격리 지원은 승격 전까지 later/profile-gated로 남습니다.

생성된 rule, skill, MCP 스니펫, 어댑터 파일, 관리형 블록에는 커넥터 매니페스트가 필요합니다. 매니페스트는 생성 경로, 관리형 블록 ID와 해시, MCP 노출 태세, 표시해도 안전한 핸들, 프로필 최신성, drift, 대체 동작을 기록합니다. 원시 토큰, 비밀값, 비공개 설정값, 차단된 페이로드 바이트, 기준 Task 상태는 저장하지 않습니다.
...
| `detective` | 관련 역량 확인이 통과한 뒤에만 하네스가 행동 뒤에 지원되는 사실을 보고 상태를 `stale`, `partial`, `blocked`, `failed`로 표시할 수 있다고 말합니다. `reference-local-mcp`에서는 변경 경로 관찰로 제한됩니다. 명령, 네트워크, 비밀값 접근, 아티팩트 캡처, 차단, 격리 사실은 승격된 역량 프로필이 필요합니다. |
...
기준 로컬 MCP 프로필은 기본적으로 협력형입니다. 변경 경로 관찰이 지원되고 관련 역량 확인이 통과한 곳에서만 제한된 `detective` 동작을 표시할 수 있습니다. 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 접점 자체 아티팩트 캡처, 도구 실행 전 차단, 격리는 없습니다. `stage_artifact`를 통한 스테이징된 아티팩트 등록은 접점을 아티팩트 캡처 프로필로 바꾸지 않습니다. `preventive`나 `isolated` 동작을 주장하면 안 됩니다.
...
| `detective` | 관련 역량 확인이 통과한 뒤 하네스가 행동 뒤 지원되는 사실을 관찰할 수 있을 때. | 상태를 stale, partial, blocked, failed로 표시하고 활성 담당 행동, 새 증거, 또는 향후 승격된 reconcile/복구 경로를 요구합니다. |
...
Projection 최신성 문제는 Core 상태와 분리됩니다. 커넥터가 현재 Core 상태를 직접 읽을 수 있으면 그 상태에서 계속할 수 있습니다. 오래된 Projection에 의존하는 행동은 현재 Core 상태에서 먼저 새로 고쳐야 합니다. reconcile은 향후 담당 문서가 승격하기 전까지 이후 후보입니다.
...
기준 프로필에서 "hold" 표현은 협력형 범위 규율과, 관련 역량 확인이 통과했을 때의 탐지형 변경 경로 검증을 뜻합니다. `preventive` guard 동작, 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 아티팩트 캡처, 격리가 아닙니다.
...
활성 스모크 대상은 커넥터 마켓플레이스가 아니라 기준 `capability_profile`입니다. 런타임 fixture가 생기고 실행되기 전까지 `conformance_smoke_status`는 `planned_not_run`이어야 합니다.
...
- 요약, 변경 경로 호환성, 담당 경로에 등록된 아티팩트 참조가 있는 `record_run`
