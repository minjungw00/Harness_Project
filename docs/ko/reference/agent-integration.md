...
- Write Authorization, 민감 동작 permission refs, minimum MVP-1의 Approval 형태 Decision Packet refs, later Approval profile이 active일 때만 Approval refs, evidence summary refs, 해당 profile이 active일 때만 Evidence Manifest refs, Eval, 수동 QA, 작업 수락, Run, report, artifact, residual-risk refs
...
Evaluator는 더 좁은 verification bundle을 받아야 합니다. 여기에는 수용 기준, changed file, approval scope, relevant Decision Packet, residual risk summary, Autonomy Boundary, deferred decision, codebase stewardship ref, evidence summary refs, 해당 profile이 active일 때만 Evidence Manifest refs, required TDD trace ref, 수동 QA requirement, artifact ref, freshness state, forbidden pattern이 포함됩니다.
...
AFK, unattended, 또는 "내가 없는 동안 계속해" 지시는 connector 표시와 진행 자세에 관한 것이며 새 권한을 만들지 않습니다. Connector는 AFK 작업을 active Change Unit, active Autonomy Boundary, granted 민감 동작 permission, 실제 제품 파일 쓰기 전 compatible `prepare_write` / Write Authorization 안에 유지해야 합니다. Minimum MVP-1에서는 이 permission을 Approval 형태 Decision Packet으로 표현하고, later Approval profile은 Approval record를 사용할 수 있습니다.
...
|---|---|
| MVP-1 사용자 작업 루프 | User-readable status/next card, Decision Packet display, pending user judgment routing, 근거와 닫기 준비 상태 요약, work-acceptance separation, relevant한 residual-risk visibility. |
...
- sensitive-action permission의 request/granted/denied/expired path. Minimum MVP-1은 Approval 형태 Decision Packet을 사용하고, later Approval profile은 Approval record를 사용할 수 있음
