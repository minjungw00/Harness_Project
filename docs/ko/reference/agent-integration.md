...
이 문서는 에이전트가 쓰는 접점을 등록하고, 활성 접점 맥락으로 선택하며, 역량 선언으로 설명하는 방식을 담당합니다. 담당 결과의 하네스 맥락을 에이전트 접점에 전달할 때의 경계도 이 문서가 정합니다.
...
조건:
- 에이전트는 담당 결과의 상태 또는 호환되는 활성 접점 맥락을 통해서만 접점에 의존할 수 있습니다.
