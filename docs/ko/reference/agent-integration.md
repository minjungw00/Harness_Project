...
MCP는 동작하지만 pre-tool guard가 약하면 low-risk direct work는 cooperative `prepare_write`와 detective changed-path validation으로 진행할 수 있습니다. Medium/high-risk work에는 stricter validation, fixture로 입증된 sidecar guard, explicit approval, detached verification, isolation을 요구해야 합니다.
