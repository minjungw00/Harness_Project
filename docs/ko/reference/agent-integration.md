...
- Write Authorization, 민감 동작 permission refs, minimum MVP-1의 민감 동작 승인 user judgment refs, later Approval profile이 active일 때만 Approval refs, evidence summary refs, 해당 profile이 active일 때만 Evidence Manifest refs, Eval, 수동 QA, 작업 수락, Run, report, artifact, residual-risk refs
...
- 내부 엔지니어링 점검에 필요한 public tool/resource 하위 집합에 `T2 MCP` 사용 가능. 여기에는 첫 authority loop에 필요한 current project/current task/status resource만 포함되며, 전체 later-profile MCP surface가 내부 엔지니어링 점검 필수라는 뜻이 아닙니다.
