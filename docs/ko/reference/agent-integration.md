...
사용자에게 보이는 agent 절차는 [에이전트 가이드](../use/agent-guide.md)을 봅니다. 접점별 설정 메모는 [Surface Cookbook](surface-cookbook.md)을 봅니다.
...
사용자에게 보이는 절차는 [에이전트 가이드](../use/agent-guide.md)을 읽습니다. 정확한 권한 규칙은 현재 단계에 필요할 때만 해당 소유자 섹션을 봅니다. 쓰기와 닫기 권한은 [Core Model 참조](core-model.md), active MVP-1 method contract는 [MVP API](api/mvp-api.md), shared API shape는 [API Schema Core](api/schema-core.md), MCP exposure, generated-file, stale-context, artifact, secret, capability-overclaiming threat는 [보안 참조](security.md)를 사용합니다. 이 참조는 connector behavior와 capability 표시를 설명하며, kernel state transition을 정의하지 않습니다. 전체 참조 문서를 기본으로 모두 불러오라는 뜻이 아닙니다.
...
항상 적용되는 고정 규칙과 맥락은 짧고 최신이어야 하며, 그 자체가 권한 출처가 아닙니다. 항상 주입되는 운영 맥락의 예산은 현재 Task 요약, 작업 모양, 범위/하지 않을 일, 대기 중인 사용자 판단, 활성 blocker, 다음 안전한 행동, 근거 gap, close blocker, 잔여 위험 요약, guarantee level, source refs/freshness입니다. 항상 적용되는 고정 규칙은 언제 Harness를 쓰는지, current status 또는 에이전트 맥락 패킷을 어디서 읽는지, Journey Card는 해당 projection/profile이 활성화되어 있고 최신일 때만 쓴다는 점, product write에는 `prepare_write`가 필요하다는 점, 사용자 소유 판단은 사용자 판단 요청으로 라우팅한다는 점, 요구사항 구체화는 사용자에게 묻기 전에 repo/docs/current state에서 확인 가능한 사실을 먼저 확인한다는 점, status가 실행 전에 실제로 막을 수 있는 것과 실행 뒤에만 감지할 수 있는 것을 보여야 한다는 점, authoritative MCP를 사용할 수 없으면 product write를 보류한다는 점만 알려주면 충분합니다. schema dump, 전체 DDL, 오래된 작업 이력, evidence body 복사본, full artifact contents, 읽기용 요약 전체 본문, future catalog material, reference contract 복제본으로 늘리면 안 됩니다. 세션 절차 자체는 [에이전트 가이드](../use/agent-guide.md)이 담당합니다.

## Use 문서와 이 참조 문서의 경계

| 영역 | 담당 문서 |
|---|---|
| 사용자 세션에서 agent가 무엇을 보여주고, 묻고, 말해야 하는지 | [에이전트 가이드](../use/agent-guide.md) |
...
| Kernel state transition과 write/close rule | [Core Model 참조](core-model.md) |
| Guarantee level 의미와 security control expectation | [보안 참조](security.md#정직한-guarantee-display) |
| Guarantee display의 runtime placement | [런타임 아키텍처 참조](runtime-architecture.md#보장-수준) |
| Security asset, trust boundary, threat category, control category | [보안 참조](security.md) |
...
일반적인 interactive Harness 사용은 `T2` 이상에서 가장 자연스럽습니다. Reliable 분리 검증에는 보통 `T3` capture와 실제 independence boundary가 필요합니다. High-risk work에는 가능하면 fixture로 입증된 `T4` guard 또는 `T5` isolation을 사용해야 합니다. `T6`는 UI/UX evidence를 보강하지만 수동 QA judgment, 작업 수락, 분리 검증을 대체하지 않으며, human 수동 QA note와 수동으로 제공된 artifact를 기록할 수 있다면 내부 엔지니어링 점검/default reference posture나 에이전시 보증 팩 / 운영과 인계 팩의 staged 수동 QA 적용 범위의 필수 조건이 아닙니다.

내부 엔지니어링 점검과 MVP-1에서 connector는 구체적인 profile이 다르게 증명하지 않는 한 cooperative/detective behavior를 전제로 삼아야 합니다. `T4`와 `T5` 행은 더 강한 향후 또는 profile별 capability를 설명할 뿐이며, 첫 사용자 가치 조각가 기본으로 OS 수준 격리, 임의 도구 sandbox 격리, 변조 불가능한 로컬 파일, 도구 실행 전 차단을 제공한다는 뜻이 아닙니다.
...
- 내부 엔지니어링 점검 baseline과 staged-delivery `local_only` 자세가 적용되는지 여부
...
Integration은 [보안 참조](security.md#정직한-guarantee-display)의 guarantee level 정의를 사용하고, 이를 연결된 접점 프로필, 현재 적용 경로, fallback 선택지에 적용합니다.
...
내부 엔지니어링 점검과 MVP-1 사용자 작업 루프는 설명 중인 operation에 대해 fixture로 입증된 guard 또는 문서화된 separation boundary가 승격되고 증명되지 않는 한 reference surface를 cooperative/detective로 표시해야 합니다. Future preventive 또는 isolated profile을 문서화할 수는 있지만, owner 문서와 conformance가 승격하기 전까지는 향후 또는 profile별 범위로 label해야 합니다.

Stage display default는 [보안 위협 모델의 단계별 guarantee level](security.md#단계별-guarantee-level)을 따릅니다.

| 단계 | Connector display 기본값 |
|---|---|
| 내부 엔지니어링 점검 | `prepare_write`, Write Authorization, `record_run`, changed path, 최소 artifact/evidence ref 주변의 지시 기반/협력적 discipline과 제한된 탐지 가능 check를 표시합니다. 기본 도구 실행 전 사전 차단이나 격리를 암시하지 않습니다. |
| MVP-1 사용자 작업 루프 | 사용자에게 보이는 막힘, MCP availability, close readiness, decision/evidence gap, 그리고 surface가 지시로만 보류할 수 있는지 실행 뒤에만 탐지할 수 있는지 표시합니다. |
| 보증 프로필 | Verification, 수동 QA, waiver, residual risk, 작업 수락, stewardship finding의 더 강한 분리를 보여 주되, 더 강한 profile이 증명되지 않는 한 여전히 cooperative/detective로 표시합니다. |
| 운영 프로필 | Operator diagnostic, generated-file drift, projection freshness, artifact integrity, recover/export posture를 표시하고, exact coverage가 증명되지 않은 항목은 보장 한계를 정직하게 표시하며 탐지/보고 동작으로 설명합니다. |
| 로드맵 | Owner-doc promotion과 conformance proof가 있는 named covered operation 또는 separation boundary에 대해서만 preventive 또는 isolated로 표시합니다. |
...
Connector 맥락은 세 부분으로 나눕니다.

- 사용자 상태 카드: 사용자가 읽는 짧은 현재 상태 보기입니다. 작업 위치, 막힘, 다음 안전한 행동, 최신성을 보여줍니다.
- 에이전트 맥락 패킷: 다음 안전한 행동에 필요한 최소 현재 상태, 참조, 최신성, 소유자 섹션 포인터입니다. 항상 주입되는 connector payload의 목표입니다.
- Core 상태: 로컬 권한 기록이며 유일한 운영 기준입니다. 상태 변경, gate, 사용자 판단, 근거 참조, Write Authorization, 작업 수락, 잔여 위험 수용, 닫기를 소유합니다.

항상 주입되는 에이전트 맥락 패킷은 한 화면 이하를 목표로 합니다. 포함하는 항목은 다음뿐입니다.
...
항목이 없거나 관련 없으면 `none`, `unknown`, 또는 한 줄 ref summary로 표시할 수 있습니다. Connector는 더 넓은 역할 설명, 전체 phase history, schema detail, 절차 본문을 항상 주입되는 패킷에 기본으로 넣지 않습니다. 그런 내용은 connector 설정이나 단계별 pull context로 남깁니다.

항상 주입되는 맥락에는 전체 reference docs, 전체 public schema, 전체 Storage DDL, complete history, 전체 historical event log, 읽기용 요약 전체 본문, full artifact contents, raw log/screenshot/diff/trace, 전체 template, 관련 없는 template, Future Fixtures 또는 다른 future catalog 내용, 오래된 작업 이력, 관련 없는 Roadmap 자료를 넣으면 안 됩니다. 이런 자료는 해당 단계와 다음 행동이 정확히 필요로 할 때만 pull-on-demand로 가져옵니다.

오래된 agent memory, 오래된 chat history, remembered recommendation, pull한 context, 오래된 상태 카드, 렌더링된 template, 오래된 projection은 agent가 살펴볼 ref를 찾는 데 도움을 줄 수 있지만, write를 허가하거나, gate를 충족하거나, Task를 close하거나, 근거를 만들거나, 민감 동작 승인을 만들거나, 결과를 수락하거나, QA 또는 verification을 면제하거나, 잔여 위험을 받아들이거나, 닫기 준비 상태를 만들거나, current owner record를 대체하거나, stale projection을 고칠 수 없습니다. 상태 카드와 Projection은 state와 ref를 요약할 수 있지만 authority는 아닙니다. 상태가 중요하면 connector는 current Core state 또는 current Core state에서 파생된 에이전트 맥락 패킷을 가져와야 합니다. 권한에 영향을 주는 오래된 context는 먼저 담당 Core path로 refresh 또는 reconcile되어야 합니다.

항상 적용되는 고정 규칙 compass는 다음 10개 이하로 유지합니다.

1. 중요한 작업이나 재개 전에는 current status 또는 에이전트 맥락 패킷을 읽습니다. Journey Card는 해당 projection/profile이 활성화되어 있고 최신일 때만 사용합니다.
2. Product/runtime/code write에는 compatible `prepare_write`와 Write Authorization이 필요합니다.
3. 사용자 소유 제품 판단 또는 기술 판단은 사용자 판단 요청으로 라우팅합니다.
4. Approval은 product judgment, 작업 수락, 잔여 위험 수용이 아닙니다.
5. 사용자 상태 카드와 Projection은 읽기용 보기이지 기준 상태가 아닙니다.
...
10. 다음 action에 필요할 때만 Reference docs, schema, historical record, template, later-profile resource body, large artifact를 pull합니다.

토큰을 아끼더라도 올바른 동작에 필요한 사용자 소유 판단, blocker, scope limit, safety boundary, 근거 gap, close blocker, close-relevant residual-risk 정보를 빼면 안 됩니다. 사용자에게 보이는 판단 요청은 집중되어야 합니다. 판단 내용, 간결한 options 또는 chosen outcome, consequence, uncertainty, agent가 사용자 대신 판단하지 않는 것을 보여줍니다. 판단을 식별하는 데 필요한 최소 affected-scope refs만 더하고, 더 넓은 schema detail, evidence body, log, template, route metadata는 active profile이 요구할 때 pull/drill-down 또는 full-format presentation 뒤에 둡니다.

다음은 compact current-state packet field입니다. Public schema가 아니며 context API가 구현되어 있음을 증명하지 않습니다. 향후 connector payload와 prompt-sized context가 지켜야 할 compact 목표를 정의합니다.
...
| 대기 중인 사용자 판단 | User judgment refs와 한 줄 question, 또는 `none`. |
...
- Write Authorization, 민감 동작 permission refs, minimum MVP-1의 민감 동작 승인 user judgment refs, later Approval profile이 active일 때만 Approval refs, `evidence_ref` ref와 파생 evidence summary, 해당 profile이 active일 때만 Evidence Manifest refs, Eval, 수동 QA, 작업 수락, Run, report, artifact, residual-risk refs
...
Refs-first는 connector가 default prompt에 큰 본문을 붙여 넣지 않고 stable id, path, hash, summary, outcome, freshness를 push해야 한다는 뜻입니다. 다음 safe action이 content inspection을 요구할 때만 excerpt를 embed하고, 그 excerpt는 source ref와 연결해 둡니다. Retrieved, indexed, remembered, summarized context도 같은 규칙을 따릅니다. Agent가 다음에 무엇을 살펴볼지 알려 줄 수는 있지만, owner path가 실제 state change를 기록하기 전까지는 pull-only context로 남습니다. Write를 허가하거나 Write Authorization을 만들거나, user judgment를 해소하거나, Approval을 부여하거나, gate를 충족하거나, evidence를 만들거나, verification을 수행 또는 기록하거나, QA를 기록하거나, QA 또는 verification을 면제하거나, 결과를 수락하거나, 잔여 위험을 받아들이거나, projection freshness를 바꾸거나, Task를 close하면 안 됩니다.

Agent가 전체 문서 세트를 읽지 않도록 단계별 맥락을 사용합니다. 각 프로필은 항상 주입되는 현재 상태 패킷과 필요할 때 불러오는 소유자 섹션을 함께 좁힙니다. MCP resource pull은 staged [Read-only resources](api/schema-core.md#read-only-resources) map을 따릅니다. 내부 엔지니어링 점검은 current project/current task/status subset만 사용하고, MVP-1은 사용자 판단 맥락이 필요할 때만 user-judgment read를 더하며, 보증 프로필/운영 프로필/future resource는 profile-gated 또는 pull-on-demand로 남깁니다. Connector는 enabled resource에서 ref, 한 줄 summary, freshness marker를 push할 수 있지만 full resource output을 기본으로 주입하면 안 됩니다. 다음 항목은 해당 단계와 다음 행동이 특정 섹션을 요구할 때가 아니면 기본으로 불러오지 않습니다. 전체 [Storage](storage.md) DDL, 전체 [Conformance Fixtures 참조](conformance-fixtures.md) 또는 [향후 Fixtures](../later/future-fixtures.md), 모든 future catalog, 전체 [Template 참조](templates/README.md) 세트, 관련 없는 [Roadmap](../roadmap.md) 항목, 오래된 작업 이력, historical event log, complete history, 읽기용 요약 전체 본문, 오래된 projection, full artifact contents, raw log/screenshot/diff/trace, 전체 MCP schema, 전체 참조 문서가 여기에 속합니다.

| Profile | 최소 현재 상태 | 필요할 때 불러오는 문서 또는 소유자 참조 | 기본으로 불러오지 않는 것 | 사용자에게 보이는 요약 | 권한과 최신성 |
|---|---|---|---|---|---|
| 세션 시작 | 현재 Task 요약 또는 명시적인 `none`/`unknown`, 예상 작업 모양, 알려진 범위/하지 않을 일, 활성 blocker, 대기 중인 사용자 판단, 다음 안전한 행동, 근거 gap, close blocker, 잔여 위험 요약, guarantee level, source/freshness refs. | [에이전트 가이드: 세션 시작](../use/agent-guide.md#세션-시작), [이어가기](../use/agent-guide.md#이어가기), 현재 `harness.status` / `harness.status.next_actions`, 해당 profile이 포함할 때만 later/compatibility `harness.next`, projection 최신성이 다음 행동에 영향을 줄 때만 [문서 Projection: Freshness and failure rules](projection-and-templates.md#freshness-and-failure-rules). | 전체 작업 이력, 전체 참조 문서, 전체 schema, 오래된 projection, 관련 없는 template, 관련 없는 Roadmap, future catalog. | 사용자는 쉬운 현재 위치, primary blocker, 대기 중인 사용자 판단, 다음 안전한 행동, 접점이 실제로 막을 수 있는 것 또는 감지만 할 수 있는 것을 봅니다. | 상태가 중요하면 current Core status 또는 state-derived 에이전트 맥락 패킷을 읽습니다. Journey Card는 해당 projection/profile이 활성화되어 있고 최신일 때만 사용합니다. Core/MCP를 사용할 수 없으면 memory를 상태처럼 쓰지 말고 unavailable이라고 말합니다. |
| 계획/구체화 (Discovery) | 목표, 알려진 경우 사용자 가치, 범위와 비목표, 수용 기준 단서, repo/docs/current state에서 확인한 확인 가능한 사실, 빠진 정보, 막히는 질문, 참고 질문, assumption, 추적되는 불확실성, 사용자 소유 판단 후보, 활성 blocker, 다음 허용 구체화 action. | [사용자 가이드: 에이전트가 처음 답해야 할 것](../use/user-guide.md#에이전트가-처음-답해야-할-것), [에이전트 가이드: 요청 정리](../use/agent-guide.md#요청-정리), [범위와 쓰기 경계](../use/agent-guide.md#범위와-쓰기-경계), 관련 현재 Task/Change Unit/Shared Design 참조. 사용자 소유 route를 나눠야 할 때만 [커널: 판단 경로 경계](core-model.md#판단-경로-경계). | 현재 질문에 필요하지 않은 전체 module map, 오래된 PRD/design, design-policy catalog, 관련 없는 template, 전체 Storage DDL, 전체 Conformance 카탈로그, future catalog. | 사용자는 확인 가능한 사실, 있을 때 다음 막히는 질문, 나중에 볼 참고 질문, 추천, 불확실성, 미룰 때 영향, 안전하게 계속할 수 있는 일을 봅니다. | 질문하기 전에 current repo/docs/state 참조를 확인합니다. Unavailable 또는 stale source는 표시하고, stale design prose, chat memory, projection을 authority로 바꾸지 않습니다. |
| 쓰기 준비 | Active Task와 Change Unit, intended paths/tools/commands/network/secrets summary, scope와 out-of-bounds, Autonomy Boundary, pending judgments 또는 민감 동작 permission needs, later Approval profile이 active일 때만 Approval needs, baseline/state freshness, Write Authority Summary, guarantee/MCP availability. | [에이전트 가이드: 제품 파일 쓰기](../use/agent-guide.md#제품-파일-쓰기), [커널: prepare_write](core-model.md#prepare_write), [커널: Write Authorization](core-model.md#write-authorization), 관련 메서드인 [`harness.prepare_write`](api/mvp-api.md#harnessprepare_write). Sensitive category가 문제일 때만 해당 보안 소유자 섹션. | 전체 Kernel/reference 문서, 관련 없는 schema, historical event log, 큰 diff/log, 전체 Storage DDL, future catalog. | 사용자는 무엇이 바뀔 수 있는지, 무엇이 막혔는지, write-authority status, 가장 작은 unblocker를 봅니다. | Current Core state와 정확한 `prepare_write` input을 사용합니다. Stale baseline, stale projection, state conflict, intended path 변경, sensitive category 변경이 있으면 refresh 또는 reconcile합니다. |
| 실행 / Run 기록 | Consumed Write Authorization 또는 no-write basis, changed-path summary, command/tool summary, run outcome, integrity/freshness가 있는 artifact refs, redaction/omission/block notes, 즉시 필요한 다음 action. | [에이전트 가이드: 근거와 확인](../use/agent-guide.md#근거와-확인), [커널: record_run](core-model.md#record_run), [`harness.record_run`](api/mvp-api.md#harnessrecord_run), display 또는 artifact-ref 질문이 있을 때만 [문서 Projection: Artifact reference 렌더링](projection-and-templates.md#artifact-reference-렌더링). | 전체 log, raw diff, screenshot, trace, bundle, artifact inventory, 읽기용 요약 전체 본문, 전체 Template 세트, future catalog. | 사용자는 무엇이 실행됐고, 무엇이 바뀌었고, 무엇이 기록됐으며, 무엇이 redacted, omitted, blocked, stale, unsupported인지 봅니다. Raw artifact body는 pull-only로 둡니다. | Current run/artifact refs와 current state에서 기록합니다. Audit 또는 violation Run을 evidence, QA, verification, 작업 수락, close를 충족한 것처럼 보여주지 않습니다. |
| 근거 검토 | 알려진 근거 요약, 있을 때 `evidence_ref`, Run refs, ArtifactRefs, 보이는 evidence gaps, stale 또는 insufficient support, redaction/integrity notes, 영향을 받는 acceptance criteria 또는 claim, 다음 evidence action. Full Evidence Manifest profile이 active일 때만 Evidence Manifest ref를 포함합니다. | [에이전트 가이드: 근거와 확인](../use/agent-guide.md#근거와-확인), [`harness.record_run`](api/mvp-api.md#harnessrecord_run), artifact-ref 표시 규칙, 해당 profile이 active일 때만 [커널: Evidence Manifest](core-model.md#evidence-manifest). | 전체 evidence body, 전체 log, raw diff, screenshot, trace, bundle, artifact inventory, 읽기용 요약 전체 본문, 전체 Template 세트, future catalog. | 사용자는 criterion 또는 claim에 연결된 알려진 coverage와 gap, ref, redaction/integrity note를 봅니다. | Baseline, paths, 민감 동작 permission / Approval, artifact integrity, relevant owner record가 바뀌면 evidence를 stale로 표시합니다. 보이는 artifact ref만으로 evidence sufficiency가 되지는 않습니다. |
| 사용자 판단 요청 | 정확한 사용자 소유 판단, `judgment_type`, `presentation`, display label, 간결한 options 또는 chosen outcome, consequence, uncertainty, agent가 사용자 대신 판단하지 않는 것. 판단을 식별하는 데 필요한 최소 affected-scope refs만 포함합니다. | [에이전트 가이드: 사용자 소유 판단으로 막힐 때](../use/agent-guide.md#사용자-소유-판단으로-막힐-때), [커널: User Judgment](core-model.md#user-judgment), [Decision Gate](core-model.md#decision-gate), 정확한 API field가 필요할 때만 [`harness.request_user_judgment`](api/mvp-api.md#harnessrequest_user_judgment). | 포괄적 approval 표현, 관련 없는 judgment, 전체 근거 본문, 전체 log, 전체 schema reference, 전체 Template 세트, future catalog. | 사용자는 집중된 판단을 봅니다. Choice, options, consequence, uncertainty, agent가 판단하지 않는 것이 중심입니다. 더 넓은 trade-off, recommendation, affected gates/criteria, route metadata, evidence body는 active일 때 pull/drill-down 또는 full-format presentation으로 둡니다. | Current user judgment 또는 current state-derived judgment candidate에서 가져옵니다. Ref가 stale 또는 missing이면 묻기 전에 표시합니다. Replay id, routing metadata, internal enum detail은 경계를 설명할 때만 보조로 둡니다. 토큰을 아낀다는 이유로 사용자가 판단하는 데 필요한 최소 맥락을 제거하면 안 됩니다. |
| 닫기 준비 상태 | Scope match, 수용 기준, evidence coverage, verification status, 수동 QA status, 작업 수락 필요/상태, 잔여 위험 표시와 필요할 때 accepted refs, close blockers, projection freshness, smallest unblocker. | [에이전트 가이드: 닫기](../use/agent-guide.md#닫기), [검증, 수동 QA, 잔여 위험, 작업 수락](../use/agent-guide.md#검증-수동-qa-잔여-위험-작업-수락), [커널: close_task](core-model.md#close_task), [Gates](core-model.md#gates), [Waiver semantics](core-model.md#waiver-semantics), [`harness.close_task`](api/mvp-api.md#harnessclose_task), readable close context를 쓸 때 [문서 Projection: Freshness and failure rules](projection-and-templates.md#freshness-and-failure-rules). | 일반적인 all-done rollup, 전체 report 본문, 전체 historical log, 관련 없는 template, 전체 Conformance 카탈로그, 읽기용 요약 전체 본문. | Acceptance 또는 close 전에 close basis를 보여줍니다. Scope, user judgments, evidence, verification, 수동 QA, residual risk, acceptance status, primary blocker, smallest unblocker를 분리합니다. | Current Core gates, owner records, evidence/artifact refs, projection freshness를 읽습니다. Stale projection은 blocker를 요약할 수 있어도 authority가 될 수 없습니다. Exact `close_task` payload는 blocker 설명에 필요할 때만 보여줍니다. |
| 복구/오류 | Primary error 또는 blocker, owner, 마지막으로 안전하게 아는 현재 상태, stale 또는 unavailable source, 영향을 받는 authority claim, 다음 recovery action, write/close 보류 여부. | [에이전트 가이드: 이어가기](../use/agent-guide.md#이어가기), [상태와 막힘 읽기](../use/agent-guide.md#상태와-막힘-읽기), [Fallback Semantics](#fallback-semantics), [API Errors: Error taxonomy](api/errors.md#error-taxonomy), [State Conflict 동작](api/errors.md#state-conflict-behavior), stale readable context가 관련될 때 [문서 Projection: Freshness and failure rules](projection-and-templates.md#freshness-and-failure-rules). Operator repair가 다음 행동일 때만 특정 운영/복구 소유자 섹션. | 과거 event log, stack trace, 전체 artifact, 관련 없는 status, 전체 Storage DDL, 전체 Conformance 카탈로그, 관련 없는 Roadmap. | 사용자는 쉬운 blocker, owner, 영향을 받는 authority claim, 가장 작은 recovery step, write 또는 close 보류 여부를 봅니다. | 가능하면 Core를 다시 읽습니다. Core에 닿을 수 없으면 agent memory, chat history, cached projection, old status text, operator prose에서 상태를 만들어 내면 안 됩니다. |
...
Context profile은 context discipline이지 새 schema나 gate가 아닙니다. Phase가 바뀌면 connector가 기본으로 push하는 항목이 바뀔 뿐이며, write를 허가하거나, decision을 해소하거나, evidence를 만들거나, verification을 수행하거나, 민감 동작 승인을 만들거나, 작업을 수락하거나, risk를 받아들이거나, Task를 close하지 않습니다.

MVP-1은 다섯 가지 작은 보기를 사용합니다. 상태 카드는 "현재 어디인가"를 보여주고, 에이전트 맥락 패킷은 다음 안전한 행동에 필요한 맥락을 담으며, 판단 요청은 사용자 판단이 필요할 때만 쓰고, 실행/근거 요약은 최소 실행/근거 뒷받침을 보여주며, 닫기 결과는 닫기 준비 상태와 막힘을 보여줍니다. 각 보기는 작고 profile에 맞게 걸러야 합니다. Route metadata, 넓은 refs, recommendation, deferral effect는 active profile이 요구할 때 pull/drill-down 또는 full-format material로 둡니다. 전체 evidence나 artifact body는 항상 주입되는 맥락이 되면 안 됩니다.

Status, next-action, recommendation, rendered-template, recommended-playbook output은 read-only guidance입니다. `prepare_write`, 사용자 판단 요청, 선택적 full-format Decision Packet presentation, Change Unit update, evidence collection, verification, QA, reconcile, close attempt를 추천할 수는 있지만, 그 output 자체가 state를 mutate하거나, gate를 충족하거나, write를 허가하거나, evidence를 만들거나, 민감 동작 승인을 만들거나, verification을 수행하거나, 수동 QA를 기록하거나, QA 또는 verification을 면제하거나, 작업 수락을 기록하거나, 잔여 위험 수용을 기록하거나, 닫기 준비 상태를 만들거나, Task를 닫거나, assurance를 올리면 안 됩니다. 추천된 action이 기존 MCP/Core mutation path를 거친 뒤에만 state effect가 생깁니다.

Evaluator는 더 좁은 verification bundle을 받아야 합니다. 여기에는 수용 기준, changed file, approval scope, relevant user judgment, residual risk summary, Autonomy Boundary, deferred judgment, codebase stewardship ref, `evidence_ref` ref와 파생 evidence summary, 해당 profile이 active일 때만 Evidence Manifest refs, required TDD trace ref, 수동 QA requirement, artifact ref, freshness state, forbidden pattern이 포함됩니다.
...
- user judgment candidate 또는 existing user judgment route
- 실제 validator/check가 낼 validator finding candidate 또는 suggested `ValidatorResult` route
- evidence requirement
- Eval 또는 verification 필요
- 수동 QA requirement
- 민감 동작 permission 필요, 또는 해당 profile이 active일 때 later Approval 필요
- residual-risk candidate
- 필요한 경우 Change Unit update recommendation
- release handoff 보고서 input
- recommended next playbook

이 항목들은 기존 Core/MCP state-changing path가 실제 동작을 기록하기 전까지 display 및 routing output일 뿐입니다. Role Lens output은 status/next recommendation output과 마찬가지로 schema나 기준 record를 도입하거나, 그 자체로 기준 상태를 변경하거나, gate를 충족하거나, write를 허가하거나, Approval을 부여하거나, user judgment를 충족하거나, evidence를 만들거나, verification을 수행하거나, 수동 QA를 기록하거나, QA 또는 verification을 면제하거나, 작업 수락을 기록하거나, 잔여 위험 수용을 기록하거나, Task를 닫거나, assurance를 올리면 안 됩니다. Lens가 상태 변경이 필요한 일을 찾아내면 접점은 normal MCP tool과 Core path로 라우팅합니다.

Two-stage review display는 두 stage가 분명히 분리되어 보이게 해야 합니다.

| Stage | 질문 |
|---|---|
| Spec Compliance Review | 현재 Harness 권한 안에서 요청한 작업이 완료되었는가: 수용 기준 충족, Change Unit completion condition, scope/write authority 호환성, user judgment compatibility, evidence coverage, Residual Risk 표시? |
...
AFK, unattended, 또는 "내가 없는 동안 계속해" 지시는 connector 표시와 진행 자세에 관한 것이며 새 권한을 만들지 않습니다. Connector는 AFK 작업을 active Change Unit, active Autonomy Boundary, granted 민감 동작 permission, 실제 제품 파일 쓰기 전 compatible `prepare_write` / Write Authorization 안에 유지해야 합니다. Minimum MVP-1에서는 이 permission을 민감 동작 승인 user judgment로 표현하고, later Approval profile은 Approval record를 사용할 수 있습니다.

Surface는 scope expansion, Autonomy Boundary breach, compatible 민감 동작 permission 없는 새 sensitive action, 잔여 위험 수용, 작업 수락, QA 또는 verification waiver, public API 또는 module contract 변경, release/support promise, 문서 독자가 의존할 내용을 바꾸는 documentation promise, 사용자 소유 제품 판단 또는 기술 판단이 필요한 다른 public commitment 전에 멈추고 가장 작은 unblocker를 보여줘야 합니다.
...
내부 엔지니어링 점검은 local project registration 하나와 Core 권한 경로를 실행하는 데 필요한 reference-surface support만 사용합니다. 이 경로는 넓은 ecosystem 지원이 아니라 kernel을 증명해야 합니다. 이 section의 later bullet은 profile target이지 내부 엔지니어링 점검 requirement가 아닙니다.

내부 엔지니어링 점검 minimum reference expectations:

- 내부 엔지니어링 점검에 필요한 public tool/resource 하위 집합에 `T2 MCP` 사용 가능. 여기에는 첫 authority loop에 필요한 current project/current task/status resource만 포함되며, [API Schema Later](api/schema-later.md)의 전체 later-profile MCP surface가 내부 엔지니어링 점검 필수라는 뜻이 아닙니다.
...
|---|---|
| MVP-1 사용자 작업 루프 | User-readable status/next card, compact user judgment display, pending user judgment routing, 근거와 닫기 준비 상태 요약, work-acceptance separation, relevant한 residual-risk visibility. |
| 보증 프로필 | Evidence Manifest support, manual verification bundle 또는 fresh evaluator instruction, 수동 QA note/artifact support, captured 또는 manually supplied artifact의 artifact integrity status, assurance/QA/waiver display. |
| 운영 프로필 | Generated file, managed block, MCP config snippet, profile freshness용 connector manifest; projection freshness와 reconcile flow; [운영과 Conformance 참조](operations-and-conformance.md#doctor)가 이름 붙인 MCP availability, surface capability mismatch, generated-file drift, artifact integrity, artifact/capture fallback, stale context, evaluator bundle freshness, projection freshness, security/threat-model category에 대한 operator smoke. |
...
Connector conformance는 profile이 선언한 capability tier에서 공통 contract를 지킬 수 있음을 입증해야 합니다. 아래 scenarios는 aggregate profile map이지 단일 내부 엔지니어링 점검 checklist가 아닙니다.

내부 엔지니어링 점검 connector checks:
...
- Autonomy Boundary breach가 stop하거나 structured blocker를 보고합니다. Full-format Decision Packet presentation은 해당 profile이 enabled될 때의 later-profile입니다.
...
- status recommendation이 추천된 action이 기존 Core mutation path를 따르기 전까지 read-only guidance로 남는 동작. 별도 `harness.next` recommendation은 그 method가 active profile에 포함된 later/compatibility behavior일 때만 적용됩니다.
- connected 내부 엔지니어링 점검 surface가 Role Lens 또는 recommended-playbook output을 함께 노출한다면, 그 output은 read-only 안내일 뿐이며 내부 엔지니어링 점검 필수 기능이 아님

Later profile scenarios:

- intake를 `advisor`, `direct`, `work`로 분류하고, 사용자에게 보일 때는 읽기/조언, 작은 변경, 추적되는 작업으로 표시
- 확인 가능한 사실을 먼저 살피고, 막히는 질문과 참고 질문을 분리하며, 사용자 소유 판단을 선택지와 결과와 함께 라우팅하는 work shaping
- full Change Unit vertical/horizontal exception handling
- 가능하면 한 번에 하나의 막히는 질문을 묻고, 가능한 경우 추천과 불확실성, 미룰 때 영향을 함께 보여주며, 구체화를 긴 질문지로 만들지 않는 동작
- 막힘을 만드는 사용자 소유 판단에 broad approval 대신 사용자 판단 요청 표시
- public commitment가 사용자 소유 제품 판단 또는 기술 판단을 필요로 하면 user judgment 또는 다른 기존 owner path로 route
- AFK work가 active Change Unit scope, Autonomy Boundary latitude, 적용되는 허가된 민감 동작 permission, 실제 product write 전 compatible `prepare_write` / Write Authorization 안에 머무르고, stop wording이 입증된 guarantee level과 맞음
- sensitive-action permission의 request/granted/denied/expired path. Minimum MVP-1은 민감 동작 승인 user judgment를 사용하고, later Approval profile은 Approval record를 사용할 수 있음
...
- product/user risk가 있는 QA 면제를 user judgment로 route
...
- risk-accepted close에는 MVP-1에서 residual-risk acceptance user judgment와 관련 blocker/evidence ref가 필요하고, rich Residual Risk ref는 해당 profile이 active일 때만 적용
