...
에이전트 접점은 사용자가 에이전트와 대화하는 곳입니다. 하네스는 범위, 사용자 판단, 쓰기 확인, 근거 참조, 최종 수락, 잔여 위험 수락, 닫기 준비 상태를 대화 기록 밖에 두는 로컬 권한 계층입니다.
...
상태 카드, 읽기용 요약, 렌더링된 template, 추천, 검색된 맥락, 대화 기억은 읽기 전용입니다. 무엇을 확인할지 가리킬 수 있지만 쓰기를 허가하거나, gate를 만족하거나, 근거를 만들거나, 사용자 판단을 해소하거나, 민감 동작 승인을 주거나, 검증을 수행하거나, QA를 기록하거나, 작업을 수락하거나, 잔여 위험을 수락하거나, 읽기용 요약 최신성을 복구하거나, Task를 닫을 수 없습니다.
