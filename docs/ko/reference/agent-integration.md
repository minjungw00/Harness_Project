...
토큰을 아끼더라도 올바른 동작에 필요한 사용자 소유 결정, blocker, scope limit, safety boundary, close-relevant residual-risk 정보를 빼면 안 됩니다. 특히 사용자 결정 요청은 사용자가 충분히 결정할 수 있도록 정확한 질문, 선택지, 추천, 불확실성, 영향을 받는 범위나 gate, 결정을 미룰 때의 영향, 관련 refs를 포함해야 합니다.
