...
| `isolated` | Risky work 또는 verification에 쓰는 별도 worktree, sandbox, process, evaluator bundle 또는 동등한 경계를 보여줍니다. Isolation만으로 approval, 작업 수락, verification이 된 것처럼 보여주면 안 됩니다. |
...
| Verification | Acceptance criteria, changed file, evidence ref, artifact ref, approval scope, active 또는 relevant Decision Packet, residual-risk summary, 수동 QA requirement, independence/freshness profile, forbidden pattern. | Evaluator가 확인해야 할 때만 full evaluator bundle material, source file, log, exact Eval/수동 QA contract, verification guidance. |
| Close | Close-readiness summary, close blocker, evidence/verification/QA/작업 수락 상태, residual-risk summary 또는 accepted refs, projection freshness, smallest unblocker. | Blocker 또는 close attempt가 exact contract나 source content에 의존할 때만 `close_task`, 작업 수락과 잔여 위험, 수동 QA, verification, artifact detail. |
...
| Isolated | Risk가 separation을 요구할 때. | 별도 worktree, sandbox, process, manual evaluator bundle에서 work 또는 verification을 실행합니다. 관련 owner path가 결과를 기록하지 않는 한 separation을 approval, 작업 수락, verification으로 취급하지 않습니다. |
