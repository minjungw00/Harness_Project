...
접점 이름 자체는 역량이 아닙니다. 커넥터는 실제 호스트, 버전/설정, 작업공간 정책, MCP 연결 태세, 수동 아티팩트 첨부 경로에 맞춘 `capability_profile`을 사용해야 합니다. 향후 캡처 경로, guard 경로, 분리 경계가 승격된다면 그 경계도 프로필 범위에 포함해야 합니다.
...
`surface_status`, `local_access_posture`, `supported_access_classes`는 커넥터의 현재 API 호환성 태세를 보고합니다. `surface_status`는 저장된 `surfaces.status`와 일치해야 하며, 이 필드들 자체가 권한을 부여하지 않습니다. 현재 접근 분류 라벨과 접점 값 집합은 [API Schema Core](api/schema-core.md#local-surface-access-values)가 담당하고, 최소 요청 조건은 [현재 MVP API](api/mvp-api.md#shared-request-rules)가 담당합니다. 기준 프로필에서 `artifact_read`는 담당 경로를 통한 등록된 `ArtifactRef` 읽기만 뜻합니다. `raw_artifact_path_read_supported=false`는 아티팩트 저장소 아래 로컬 파일시스템 경로만으로 아티팩트 바이트를 읽을 수 없다는 뜻입니다. `artifact_registration`에서 수동 첨부는 문서화된 스테이징 핸들을 제공할 수 있습니다. `captured_artifact` 핸들은 담당 문서가 문서화한 캡처 핸들 지원이 필요한데, 기준 프로필에는 그 지원이 없습니다.

기준 `reference-local-mcp` 프로필에는 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 접점 자체 아티팩트 캡처, 도구 실행 전 차단, 격리 역량이 없습니다. 이런 역량 필드와 프로필 종류는 [Later 후보 색인](../later/index.md)의 later/profile-gated 자료입니다. 활성 프로필에 없다는 것은 지원하지 않는다는 뜻이지, 알 수 없거나 암묵적으로 사용할 수 있다는 뜻이 아닙니다.

접점 버전, MCP 설정, 권한, 작업공간 정책, 생성 파일, 관리형 block, 수동 아티팩트 첨부 경로, redaction 정책, 아티팩트 보존 방식, 로컬 접근 태세, 적합성 근거가 바뀌면 `capability_profile`을 갱신합니다. 후크, 접점 자체 캡처, 도구 실행 전 차단, 격리 지원은 승격 전까지 later/profile-gated로 남습니다.

생성된 rule, skill, MCP snippet, adapter file, 관리형 block에는 connector manifest가 필요합니다. Manifest는 생성 경로, 관리형 block id와 hash, MCP 노출 태세, 표시해도 안전한 핸들, 프로필 최신성, drift, 대체 동작을 기록합니다. 원시 token, secret, 비공개 설정값, 차단된 payload byte, 기준 Task state는 저장하지 않습니다.
...
| `detective` | 하네스가 행동 뒤에 지원되는 사실을 보고 상태를 stale, partial, blocked, failed로 표시할 수 있다고 말합니다. `reference-local-mcp`에서는 변경 경로 관찰로 제한됩니다. 명령, 네트워크, 비밀값 접근, 아티팩트 캡처, 차단, 격리 사실은 승격된 역량 프로필이 필요합니다. |

`preventive`와 `isolated` 같은 profile-gated 표시 이름은 향후 담당 문서가 정확한 프로필 필드, 대상 동작, 대체 동작, 오류, 증명 경로를 승격하기 전까지 [Later 후보 색인](../later/index.md)에 남습니다. 에이전트는 사용자가 더 강한 안전성을 요청했거나, guard/freeze/careful mode를 요청했거나, 대화에서 더 강한 표현을 썼다는 이유만으로 그 라벨을 고르면 안 됩니다. 활성 프로필이 더 강한 주장을 뒷받침하지 못하면 커넥터는 표시되는 `guarantee_display.level` 값을 낮추거나 `CAPABILITY_INSUFFICIENT`를 반환해야 합니다.

기준 로컬 MCP 프로필은 기본적으로 협력형입니다. 변경 경로 관찰이 뒷받침하는 곳에서만 제한된 `detective` 동작을 표시할 수 있습니다. 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 접점 자체 아티팩트 캡처, 도구 실행 전 차단, 격리는 없습니다. 담당 경로가 승인한 아티팩트 등록을 통한 수동 아티팩트 첨부는 가능할 수 있지만, 이것이 접점을 아티팩트 캡처 프로필로 바꾸지는 않습니다. `preventive`나 `isolated` 동작을 주장하면 안 됩니다.
...
- 활성 표시 형식. 현재 MVP에서는 `presentation=short`를 사용하며, `presentation=full`과 `Decision Packet`은 승격 전까지 later 후보 자료로 남습니다.
...
| 로컬 접근 불일치 | 호출자, 경로, 태세가 등록된 로컬 프로필 밖이거나 로컬 접근이 철회되었을 때. | 표시해도 안전한 진단과 함께 `LOCAL_ACCESS_MISMATCH`를 사용합니다. 접점별 `UNAUTHORIZED` code를 만들지 않습니다. |
...
기준 프로필에서 "hold" 표현은 협력형 범위 규율과, 가능할 때의 탐지형 변경 경로 검증을 뜻합니다. `preventive` guard 동작, 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 아티팩트 캡처, 격리가 아닙니다.
