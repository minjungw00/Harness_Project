...
- Write Authorization, 민감 동작 permission refs, minimum v0.2의 Approval 형태 Decision Packet refs, later Approval profile이 active일 때만 Approval refs, evidence summary refs, 해당 profile이 active일 때만 Evidence Manifest refs, Eval, 수동 QA, 작업 수락, Run, report, artifact, residual-risk refs
...
- sensitive-action permission의 request/granted/denied/expired path. Minimum v0.2는 Approval 형태 Decision Packet을 사용하고, later Approval profile은 Approval record를 사용할 수 있음
