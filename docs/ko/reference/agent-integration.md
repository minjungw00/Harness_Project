...
- 오래된 멤버십 행이 있어도 inactive, 무효, 실행 부적격 프로젝트는 실행 시점에 계속 사용할 수 없습니다.
...
`harness.list_projects`는 읽기 전용 MCP 어댑터 유틸리티 도구입니다. 이 도구는 통합에 명시적으로 허용된 프로젝트만 나열하고, 프로젝트 가용성과 기본값 상태를 보여 주며, 에이전트가 유효한 `project_id`를 선택할 수 있을 만큼의 프로젝트 식별 정보를 제공합니다. 이는 공개 하네스 Core API 메서드 아홉 개 밖에 있으며 공개 메서드 목록에 추가되면 안 됩니다.
...
- 공개 `ToolEnvelope.surface_id`는 스키마 담당 문서가 정의하는 API 요청 래퍼 필드로 남습니다. MCP 어댑터는 선택된 통합에서 호출 접점 식별성을 파생해야 하며 호출자 텍스트가 통합의 `surface_id`나 `surface_instance_id`를 덮어쓰게 하면 안 됩니다.
