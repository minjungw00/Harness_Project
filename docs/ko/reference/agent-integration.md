...
사용자에게 보이는 절차는 [에이전트 세션 흐름](../use/agent-session-flow.md)을 읽습니다. 정확한 권한 규칙은 현재 단계에 필요할 때만 해당 소유자 섹션을 봅니다. 쓰기와 닫기 권한은 [커널 참조](kernel.md), active MVP-1 method contract는 [MVP API](api/mvp-api.md), shared API shape는 [API Schema Core](api/schema-core.md), MCP exposure, generated-file, stale-context, artifact, secret, capability-overclaiming threat는 [보안 위협 모델 참조](security-threat-model.md)를 사용합니다. 이 참조는 connector behavior와 capability 표시를 설명하며, kernel state transition을 정의하지 않습니다. 전체 참조 문서를 기본으로 모두 불러오라는 뜻이 아닙니다.
...
항상 주입되는 맥락에는 전체 reference docs, 전체 public schema, 전체 Storage DDL, complete history, 전체 historical event log, 읽기용 요약 전체 본문, full artifact contents, raw log/screenshot/diff/trace, 전체 template, 관련 없는 template, Future Fixture Catalog 또는 다른 future catalog 내용, 오래된 작업 이력, 관련 없는 Roadmap 자료를 넣으면 안 됩니다. 이런 자료는 해당 단계와 다음 행동이 정확히 필요로 할 때만 pull-on-demand로 가져옵니다.
...
Compact status card는 "현재 어디이고 다음은 무엇인가?"를 위한 사용자 상태 보기입니다. 에이전트 맥락 패킷은 별도이며 더 작을 수 있습니다. Judgment-context도 별도입니다. Judgment-context는 사용자 판단이 필요할 때만 사용하며, 기본 prompt는 판단 내용, options, consequence, uncertainty, agent가 사용자 대신 판단하지 않는 것에 집중합니다. Route metadata, 넓은 refs, recommendation, deferral effect는 active profile이 요구할 때 pull/drill-down 또는 full-format material로 둡니다. 전체 evidence나 artifact body는 항상 주입되는 맥락이 되면 안 됩니다.
