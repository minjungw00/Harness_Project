...
| 사용자 판단 요청 | 정확한 사용자 소유 판단, `judgment_type`, `presentation`, display label, options 또는 chosen outcome, consequence, uncertainty, 영향을 받는 범위, 관련 참조 또는 명시적 absence, agent가 사용자 대신 판단하지 않는 것, 답변이 확정하지 않는 것, 답변 뒤의 next action. High-risk 또는 close-affecting prompt는 trade-offs, 추천, affected gates/acceptance criteria, deferral consequence도 보여줍니다. | [에이전트 세션 흐름: 사용자 소유 판단으로 막힐 때](../use/agent-session-flow.md#사용자-소유-판단으로-막힐-때), [커널: User Judgment](kernel.md#user-judgment), [Decision Gate](kernel.md#decision-gate), 정확한 API field가 필요할 때만 [`harness.request_user_judgment`](api/mvp-api.md#harnessrequest_user_judgment). | 포괄적 approval 표현, 관련 없는 judgment, 전체 근거 본문, 전체 log, 전체 schema reference, 전체 Template 세트, future catalog. | 사용자가 충분히 판단할 수 있게 exact choice, options, consequence, uncertainty, scope, 참조, agent가 판단하지 않는 것, 답변이 확정하지 않는 것을 보여줍니다. | Current user judgment 또는 current state-derived judgment candidate에서 가져옵니다. Ref가 stale 또는 missing이면 묻기 전에 표시합니다. Replay id, routing metadata, internal enum detail은 경계를 설명할 때만 보조로 둡니다. 토큰을 아낀다는 이유로 사용자가 판단하는 데 필요한 맥락을 제거하면 안 됩니다. |
