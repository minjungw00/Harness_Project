...
기준 로컬 MCP 프로필은 기본적으로 협력형입니다. 변경 경로 관찰이 뒷받침하는 곳에서만 제한된 `detective` 동작을 표시할 수 있습니다. 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 접점 자체 아티팩트 캡처, 도구 실행 전 차단, 격리는 없습니다. 담당 경로가 승인한 아티팩트 등록을 통한 수동 아티팩트 첨부는 가능할 수 있지만, 이것이 접점을 아티팩트 캡처 프로필로 바꾸지는 않습니다. `pre_tool_blocking_supported=false`와 `isolation_supported=false`이므로 `preventive`나 `isolated` 동작을 주장하면 안 됩니다.
...
`pre_tool_blocking_supported=false`이므로 "hold" 표현은 협력형 범위 규율과, 가능할 때의 탐지형 변경 경로 검증을 뜻합니다. `preventive` guard 동작, 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 아티팩트 캡처, 격리가 아닙니다.
