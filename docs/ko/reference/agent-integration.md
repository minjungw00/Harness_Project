...
Native capture가 없으면 connector는 manual artifact capture로 fallback해야 합니다. 즉 diff, log, screenshot, workflow note, command output, QA note를 사용자나 operator가 제공한 named artifact ref로 기록합니다. Native isolation 또는 fresh evaluator support가 없으면 acceptance criteria, changed file, relevant ref, artifact ref, freshness state, forbidden pattern을 담은 manual verification bundle로 fallback합니다. 이런 fallback은 명시적 evidence route일 뿐 preventive 또는 isolated guarantee level로 올려 주지 않습니다.
