...
`surface_status`, `local_access_posture`, `supported_access_classes`는 커넥터의 현재 API 호환성 태세를 보고합니다. 그 자체로 권한을 부여하지 않습니다. 현재 접근 분류 라벨과 접점 값 집합은 [API Schema Core](api/schema-core.md#local-surface-access-values)가 담당하고, 최소 요청 조건은 [현재 MVP API](api/mvp-api.md#shared-request-rules)가 담당합니다. 기준 프로필에서 `artifact_read`는 담당 경로를 통한 등록된 `ArtifactRef` 읽기만 뜻합니다. `raw_artifact_path_read_supported=false`는 아티팩트 저장소 아래 로컬 파일시스템 경로만으로 아티팩트 바이트를 읽을 수 없다는 뜻입니다.
...
| 로컬 접근 불일치 | 호출자 또는 전송 경로가 등록된 로컬 프로필 밖이거나 로컬 접근이 철회되었을 때. | 표시해도 안전한 진단과 함께 `LOCAL_ACCESS_MISMATCH`를 사용합니다. 접점별 `UNAUTHORIZED` code를 만들지 않습니다. |
