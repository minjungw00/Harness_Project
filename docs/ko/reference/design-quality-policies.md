...
설계 품질 정책은 finding, 근거 필요성, 수동 QA 요구, 검증 필요성, 잔여 위험 후보, 사용자 판단 필요성, close blocker를 만들 수 있지만 각 범주는 자기 owner path에 남습니다. 근거, 검증, 수동 QA, 작업 수락, 잔여 위험의 정확한 대체 불가능성 계약은 [Core Model 참조: 근거, 검증, 수동 QA, 작업 수락, 위험](core-model.md#근거-검증-수동-qa-작업-수락-위험)이 담당합니다.
