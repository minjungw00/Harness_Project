...
`VOLICORD_BIN`은 문서 예시가 사용하는 튜토리얼용 셸 변수일 뿐입니다. Volicord는 `VOLICORD_BIN`을 설정으로 읽지 않습니다. setup profile은 관리 호스트 설정과 generic MCP 설정 내보내기가 `volicord-mcp`를 시작할 때 사용할 명령을 기록합니다.
...
호스트 설정은 `volicord setup`이 기록한 MCP 명령을 사용합니다.
...
- 미래의 MCP 호스트 프로세스는 기록된 명령을 시작할 수 있어야 하며, 기본 Runtime Home이 의도한 위치가 아니라면 의도한 `VOLICORD_HOME`을 받아야 합니다.
...
| Codex | `shared` | `.codex/config.toml`을 적용할 때 선택한 `Product Repository`에 쓸 수 있어야 합니다. 미래의 Codex 호스트는 setup profile의 MCP 명령을 시작할 수 있어야 합니다. Codex 프로젝트 신뢰가 여전히 필요할 수 있습니다. |
| Claude Code | `personal`, `global` | Volicord가 `claude mcp` 명령을 사용할 수 있도록 관리 프로세스가 `claude` 실행 파일을 시작할 수 있어야 합니다. |
| Claude Code | `shared` | `.mcp.json`을 적용할 때 선택한 `Product Repository`에 쓸 수 있어야 합니다. 미래의 Claude Code 호스트는 setup profile의 MCP 명령을 시작할 수 있어야 합니다. 프로젝트 MCP 승인이 여전히 필요할 수 있습니다. |
...
- shared-intent 호스트 설정이 미래의 호스트 환경에서 setup profile에 기록된 MCP 명령을 시작할 수 없습니다.
