...
직접 호스트 설정을 구성할 때는 선택한 호스트와 범위가 필요로 할 때 관리 프로세스가 대상 호스트 설정을 조사하고 관리 설정을 쓸 수 있어야 합니다.
...
호스트 설정을 썼다는 사실은 호스트가 `volicord-mcp`를 신뢰, 승인, 로드, 초기화, 노출했다는 증거가 아닙니다. `managed host configuration state`의 의미와 호스트 신뢰 경계는 [에이전트 통합](agent-integration.md)이 담당합니다.
