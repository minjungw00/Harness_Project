...
| 패키지 관리자 설치 | 지원 범위 밖입니다. | 설치 문서는 소스 빌드와 별도로 설치된 실행 파일 찾기를 문서화하지만, 저장소 담당 문서에는 패키지 관리자 절차나 릴리스 배치가 정의되어 있지 않습니다. | 소스 빌드 경로를 사용하거나, `volicord`를 포함하는 이미 설치된 실행 파일 디렉터리를 사용합니다. |
...
소스 빌드에서는 디버그 실행 파일이 `target/debug` 아래에, 릴리스 실행 파일이 `target/release` 아래에 있어야 합니다. 별도로 설치된 실행 파일을 사용할 때는 명시적 setup 옵션이나 `PATH`로 setup이 `volicord`를 찾을 수 있는 설치 배치를 선택합니다.
...
Setup 안내, 링크, 또는 별도 설치로 명령이 보이게 된 뒤에는 일반 명령 찾기를
확인합니다.
...
호스트 설정은 `volicord setup`이 마련한 MCP 명령 정보를 사용합니다. 정확한
...
- Setup은 `volicord`를 찾을 수 있어야 합니다.
...
- 기본 `$HOME/.volicord`가 의도한 위치가 아니라면 미래의 `volicord mcp --stdio` 호스트 프로세스도 같은 Runtime Home 선택을 받아야 합니다. shared 프로젝트 호스트 설정은 개인 Runtime Home 경로를 담으면 안 되므로, 각 사용자는 기본값이 아닌 Runtime Home을 자신의 로컬 setup이나 환경으로 제공해야 합니다.
