...
| 셸 문법 | 유지되는 POSIX 스타일 예시에 대해 지원됩니다. 다른 셸은 이 예시에 대해 미검증입니다. | 설치 예시는 `export VOLICORD_BIN="$(pwd)/target/debug"`, 따옴표로 감싼 변수 확장, `PATH="$VOLICORD_BIN:$PATH"`, `test -x`를 사용합니다. | 셸이 이 문법을 실행할 수 없다면 직접 명령을 옮기고, 옮긴 각 명령을 확인한 뒤 계속합니다. |
...
| Codex와 Claude Code 호스트 최소 버전 | 안정적인 호스트 최소 버전은 정의되어 있지 않습니다. 호스트 호환성은 문서화된 버전 하한이 아니라 운영 점검으로 확인합니다. | Codex 검증은 `PATH`에서 `codex`를 찾고 `codex --version`을 실행합니다. Claude Code 검증은 `claude mcp get <server_name>`으로 호스트 상태를 조사합니다. 관리 검증은 최종 결과 상태를 담당합니다. | 설치 후 `volicord agent verify --connection-id <id>`를 사용합니다. 문서화되지 않은 Codex 또는 Claude Code 최소 버전에 의존하지 않습니다. |
...
- `export NAME=value`
- `$(pwd)` 명령 치환
- `"$VOLICORD_BIN/volicord"` 같은 따옴표로 감싼 변수 확장
- `PATH="$VOLICORD_BIN:$PATH" command ...` 같은 인라인 환경 변수 지정
- `test -x`
- 예시의 슬래시 경로
- 콜론으로 구분하는 `PATH`

`VOLICORD_BIN`은 문서 예시가 사용하는 튜토리얼용 셸 변수일 뿐입니다. Volicord는 `VOLICORD_BIN`을 설정으로 읽지 않습니다. 호스트 설정에는 범위에 따라 절대 실행 파일 경로나 이식 가능한 명령 이름 `volicord-mcp`가 들어갑니다.
...
POSIX 스타일 셸에서는 `test -x "$VOLICORD_BIN/volicord"`와 `test -x "$VOLICORD_BIN/volicord-mcp"`가 유지되는 실행 파일 확인 예시입니다.

호스트 설정에는 범위별 명령 요구사항이 있습니다.

- Codex `user` 범위와 Claude Code `local` 또는 `user` 범위는 `--mcp-command`를 제공할 때 절대 `volicord-mcp` 명령 경로가 필요합니다. 생략하면 관리 CLI는 `volicord` 실행 파일의 형제 디렉터리와 `PATH`를 차례로 시도합니다.
- Codex와 Claude Code `project` 범위는 이식 가능한 명령 이름 `volicord-mcp`를 사용해야 합니다. 나중의 호스트 프로세스는 자기 `PATH`에서 `volicord-mcp`를 찾을 수 있어야 합니다.
...
- 선택한 사용자가 `volicord init`, 프로젝트 등록, 에이전트 설치, 검증을 실행할 때 디렉터리를 만들거나 그 안에 쓸 수 있어야 합니다.
- 기본 `$HOME/.volicord`가 의도한 위치가 아니라면 미래의 `volicord-mcp` 호스트 프로세스도 같은 Runtime Home 선택을 받아야 합니다.
...
`Product Repository`는 프로젝트 등록과 프로젝트 범위 호스트 설정에 쓰이는 기존 로컬 디렉터리여야 합니다. `Volicord Runtime Home`과 분리되어 있어야 합니다.
...
직접 관리 호스트 설정 상태에서는 선택한 호스트와 범위가 필요로 할 때 관리 프로세스가 대상 호스트 설정을 조사하고 관리 설정을 쓸 수 있어야 합니다.
...
| Codex | `project` | `.codex/config.toml`을 적용할 때 `Product Repository`에 쓸 수 있어야 합니다. 미래의 Codex 호스트는 `PATH`에서 `volicord-mcp`를 찾아야 합니다. Codex 프로젝트 신뢰가 여전히 필요할 수 있습니다. |
| Claude Code | `local`, `user` | Volicord가 `claude mcp` 명령을 사용할 수 있도록 관리 프로세스가 `claude` 실행 파일을 시작할 수 있어야 합니다. |
| Claude Code | `project` | `.mcp.json`을 적용할 때 `Product Repository`에 쓸 수 있어야 합니다. 미래의 Claude Code 호스트는 `PATH`에서 `volicord-mcp`를 찾아야 합니다. 프로젝트 MCP 승인이 여전히 필요할 수 있습니다. |
| Generic | `export` | 내보내기 파일을 쓸 때만 쓰기 가능한 내보내기 대상이 필요합니다. 외부 호스트는 호스트별 방식으로 로드되고 점검되기 전까지 사용자 관리 상태이며 미검증입니다. |

호스트 설정을 설치했다는 사실은 호스트가 `volicord-mcp`를 신뢰, 승인, 로드, 초기화, 노출했다는 증거가 아닙니다. managed host configuration state 의미와 호스트 신뢰 경계는 [에이전트 통합](agent-integration.md)이 담당합니다.

## MCP 호스트 환경 요구사항

기준 MCP 호스트 환경은 `volicord-mcp --connection <connection_id>`를 로컬 자식 프로세스로 시작하고 stdin/stdout으로 통신할 수 있어야 합니다. 이것은 네트워크 리스너 요구사항이 아닙니다.

호스트 프로세스 환경은 아래를 제공해야 합니다.

- 설정된 명령 경로나 `PATH`에 따른 실행 가능한 `volicord-mcp` 명령
- 의도한 Runtime Home이 기본 홈에서 유도되는 위치가 아닐 때의 `VOLICORD_HOME`
- Runtime Home과 명시적으로 허용된 각 `Product Repository`에 대한 로컬 파일시스템 접근

`volicord-mcp --check --connection <connection_id>`는 시작 검증 점검입니다. 전체 호스트 통합 검증이 아닙니다. 전체 호스트 검증에는 [관리 CLI](admin-cli.md)가 정의한 관리 결과 게이트가 필요합니다.
...
- 프로젝트 범위 호스트 설정이 미래의 호스트 `PATH`에서 `volicord-mcp`를 찾는 방식에 의존할 수 없습니다.
