...
새로 커밋되는 non-dry-run 상태 변경에서는 현재 행 쓰기, `task_events` 추가, 영향을 받은
...
`project_state.state_version`은 모든 Task 변경을 세는 전역 카운터가 아닙니다. Core가 주
Task를 선택하지 않은 변경에 쓰는 프로젝트 범위 상태 시계입니다. `tasks.state_version`은
Task 하나와 그 담당 행에 대한 Task 범위 상태 시계입니다. 현재 활성 MVP의 공개 변경은
영향받는 상태 시계 하나를 고릅니다. 하나의 호출이 둘 이상의 `expected_state_version`을
요구하는 다중 범위 최신성은 담당 문서가 승격하기 전까지 later입니다.
