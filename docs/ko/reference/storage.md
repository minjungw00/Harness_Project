...
- Projection 렌더링, 상태 보기 템플릿 본문, 보고서 형식, 대시보드,
  내보내기, reconcile 동작, 영속 Projection 작업, reconcile queue,
  managed block drift repair, 운영 진입점, 적합성 실행기, 향후 fixture 저장소.
...
읽는 시점의 상태/Projection 출력은 커밋된 기록과 지속 아티팩트 참조에서 파생됩니다.
렌더링된 Projection, Markdown `status-card`, 생성된 문서를 편집해도 저장소가 갱신되거나
Core 상태가 바뀌지 않습니다. 현재 MVP에는 `projection_jobs` 테이블, 영속 Projection
캐시, reconcile queue, managed-output outbox, managed block drift repair용 저장소가 없습니다.
...
- `artifacts/`는 프로젝트 아티팩트 저장소입니다. 그 아래 경로는 Core가 지속 아티팩트
  승격/연결 경계를 적용한 뒤에만 지속 증거 바이트 또는 안전한 메타데이터를 저장합니다.
...
`harness.stage_artifact`는 활성 로컬 아티팩트 유틸리티이지만, 저장소가 소유하는
임시 스테이징 바이트 또는 알림과 `StagedArtifactHandle` 뒤의 `artifact_staging` 행
또는 동등한 스테이징 기록만 만듭니다. 성공한 호출은
`base.effect_kind=staging_created`인 `StageArtifactResult`를 반환합니다. Core 기록,
`task_events`, `tool_invocations` 재실행 행, `project_state.state_version` 증가를
만들지 않습니다.

저장소는 데이터 구조와 영속 저장을 분리합니다. `CloseReadinessBlocker`는 닫기 준비
상태를 평가할 때 쓰는 데이터 구조이지 저장 행, 이벤트, replay row 표시,
...
`blockers`를 통해 저장합니다. 별도의 커밋된 Discovery Brief, Shared Design,
Question Queue, Assumption Register, First Safe Change Unit Candidate 테이블을 만들지
않습니다. 증거는 간결한
증거 요약, Task 또는 Change Unit의 `CompletionPolicy`, 필수 `EvidenceCoverageItem`,
아티팩트 참조를 통해 저장합니다. 전체 Evidence Manifest 저장소를 요구하지 않습니다.

Projection은 현재 MVP에서 별도 테이블 계열로 영속하지 않습니다. `status-card`,
`judgment-request`, `run-evidence-summary`, `close-result`, `agent-context-packet`은 활성
기록 위에 읽는 시점에 만든 보기이며, 저장된 상태나 저장소 변경 경로가 아닙니다.

최소 활성 구체화 정보도 이 기존 기록들에 저장합니다. 여기에는 현재 목표 요약,
활성 범위 요약, 허용 경로 또는 영향 영역, 범위 밖 항목, 수락 기준, 자율성 경계,
필요한 사용자 소유 판단, 필요할 때 막히는 질문 하나, 다음 안전한 행동 하나,
`CompletionPolicy`, 필수 증거 기대 또는 증거 공백, 닫기 차단 사유가 포함됩니다. 빠졌거나 알 수 없는 항목은 `unknown`,
...
| `runs` | `state.sqlite` | 제품 쓰기가 있었다면 호환되는 Write Authorization 소비까지 포함하는 확정된 실행 또는 관찰 기록입니다. | `run_id`, `task_id`, `change_unit_id`, `write_authorization_id`, `surface_id`, `kind`, `status`, `product_write`, `baseline_ref`, `summary`, 관찰/증거 JSON 열, `created_at`, `completed_at`. |
...
| `artifacts` | `state.sqlite`와 아티팩트 저장소 | 아티팩트 무결성, 가림, 생산자, 보존, 가용성 사실을 가진 지속 증거 바이트 또는 안전한 메타데이터입니다. | `artifact_id`, `project_id`, `task_id`, `run_id`, `kind`, `uri`, `sha256`, `size_bytes`, `content_type`, `redaction_state`, `retention_class`, `produced_by`, `status`, `created_at`, `updated_at`. |
...
| `blockers` | `state.sqlite` | 다음 행동, 쓰기 호환성, 증거 공백, 닫기 준비, 복구를 위한 구조화된 차단 사유 상태를 저장합니다. `close_task` 계약이 커밋된 닫기 차단 효과를 허용할 때만 닫기 준비 상태 차단 사유 상태를 저장할 수 있습니다. `CloseReadinessBlocker`는 API 데이터 구조이며 그 자체가 행이나 저장 신호가 아닙니다. `prepare_write`의 `write_decision_reasons`는 응답과 재실행 페이로드에 담기는 prepare_write 판단 사유이지 닫기 준비 상태 차단 사유 행이 아닙니다. | `blocker_id`, `task_id`, `blocked_action`, `blocker_kind`, `status`, `message`, `owner_ref_json`, `related_refs_json`, `required_next_action`, `created_at`, `resolved_at`. |
...
| `tool_invocations` | `state.sqlite` | API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된 `dry_run=false` Core `MethodResult` 응답만 저장하는 재실행 행입니다. `ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 결과, 스테이징 생성 응답은 여기에 저장하지 않습니다. `dry_run=true`인 `harness.close_task intent=check`의 `CloseTaskResult`도 읽기 전용 결과이므로 이 저장 제외 대상입니다. | `invocation_id`, `project_id`, `tool_name`, `idempotency_key`, `request_hash`, `task_id`, `basis_state_version`, `response_json`, `status`, `created_at`. |
...
- 활성 테이블은 불투명하고 안정적인 id를 기본 키 또는 동등한 고유 키로
  ...
  `handle_id`, `artifact_id`, `artifact_link_id`, `evidence_summary_id`, `blocker_id`,
  `event_id`, `invocation_id`입니다.
...
- `write_authorizations.consumed_by_run_id`는 null이 아닐 때 고유합니다.
  ...
  한 번만 소비되는 관계를 보존합니다.
- `artifact_staging`에는 고유한 `(project_id, handle_id)`가 필요합니다. 핸들이
  존재하는 동안 `tmp_uri` 또는 동등한 스테이징 객체 식별자도 프로젝트 안에서
  고유해야 합니다. `consumed_by_run_id`와 `promoted_artifact_id`는 null이 아닐 때
  고유해야 하므로 스테이징 핸들 하나가 두 번 소비되거나 두 개의 영속 아티팩트로
  승격될 수 없습니다.
...
- `project_state.project_id`, `surfaces.project_id`, `tasks.project_id`,
  `artifact_staging.project_id`, `artifacts.project_id`, `task_events.project_id`,
  `tool_invocations.project_id`는 프로젝트 등록에 속합니다.
...
- `artifact_staging.task_id`는 담당 Task를 가리킵니다.
  `artifact_staging.consumed_by_run_id`와 `artifact_staging.promoted_artifact_id`는
  값이 있으면 소비하는 `harness.record_run` 트랜잭션이 만든 같은 프로젝트/같은 Task의
  `runs`와 `artifacts` 행을 가리킵니다.
...
- 아직 소비되지 않았거나 만료된 `artifact_staging` 행과 `artifacts/tmp/`의
  스테이징 바이트 또는 알림은 `expired` 또는 `discarded`로 표시할 수 있고, 등록
  전에는 임시 바이트를 정리할 수 있습니다. 이것들은 증거 권한이 아니기 때문입니다.
  그러나 `artifacts` 행이 커밋된 뒤 보존 정책에 따른 삭제, 프로젝트 해체, 파괴적
  정리는 일반 현재 MVP 상태 변경 밖이며 담당 문서가 정의한 경로가 필요합니다.
...
| `artifact_staging.status` | `staged`, `consumed`, `expired`, `discarded` | 저장소가 소유하는 임시 핸들 생명주기입니다. `harness.record_run`이 소비할 수 있는 값은 `staged`뿐이며, 종료 값은 다시 `staged`가 될 수 없습니다. |
| `artifact_staging.redaction_state` | `none`, `redacted`, `secret_omitted`, `blocked` | 임시 스테이징은 `ArtifactRef`와 같은 안전한 바이트 또는 안전한 알림 가림 어휘를 사용합니다. 숨겨진 원본을 설명하지 않습니다. |
...
| `tool_invocations.status` | `committed` | 재실행 행은 API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된 `dry_run=false` Core `MethodResult` 응답에만 존재합니다. `ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 결과, 성공한 `StageArtifactResult` 스테이징 결과에는 재실행 행이 없습니다. `dry_run=true`인 `harness.close_task intent=check`는 `ToolDryRunResponse`가 아니라 읽기 전용 결과입니다. |
...
- 차단된 `prepare_write`, `dry_run`, 잘못된 요청, 커밋 전 실패는 소비 가능한
  `write_authorizations` 행을 만들지 않습니다. 커밋된 `decision=allowed`가 아닌
  `PrepareWriteResult`는 응답과 재실행 페이로드에 `write_decision_reasons`를 담을 수 있지만,
  이 사유는 `WriteDecisionReason[]`이지 `CloseReadinessBlocker[]`가 아니며,
  닫기 준비 상태 차단 사유 행도 아니고 `close_state` 변경도 뜻하지 않습니다.

`SensitiveActionScope` 저장소는 `write_authorizations`와 분리됩니다.
`judgment_kind=sensitive_approval`에서는 `user_judgments`가 승인되었거나 대기 중인
민감 동작 범위를 `sensitive_action_scope_json` 또는 동등하게 검증된 판단 본문 필드에
저장합니다. 이 범위는 사용자가 판단하는 이름 붙은 민감 동작을 설명합니다.
`AuthorizedAttemptScope`가 아니며, `write_authorizations.attempt_scope_json`에 저장하지
않고, Write Authorization을 만들거나 소비하지도 않습니다. 제품 파일 쓰기에는 호환되는
`SensitiveActionScope` 판단과 별도의 호환 Write Authorization이 둘 다 필요할 수 있지만,
두 기록은 서로를 대신할 수 없습니다.
...
`VerifiedSurfaceContext`는 파생된 요청 맥락입니다. 사용자가 제공한 데이터도 아니고,
저장된 등록 행도 아니며, 오래 남는 권한 기록도 아닙니다.
...
증거 공백, 차단된 닫기, 닫기 차단 사유는 각 담당 기록이나 현재 Task 상태에 남습니다.
`tasks.close_reason`은 커밋된 `close_task` intent와 호환되어야 합니다.
`completed_self_checked`와 `completed_with_risk_accepted`는 필수 증거와 필요한 최종 수락을
요구하는 성공 완료 사유입니다. `cancelled`와 `superseded`는 종료 상태이지만 성공 완료 사유가
아닙니다. `cancelled`와 `superseded`를 `CompletionPolicy`의 증거, 최종 수락, 잔여 위험 수락
요구를 만족시킨 값처럼 저장하면 안 됩니다. 커밋된 supersession이 활성
...
- `success_criteria_json`, `acceptance_criteria_json`, `scope_boundary_json`,
  ...
  `constraints_json`, `autonomy_boundary_json`, `completion_policy_json` 같은
  Task와 Change Unit의 구체화 열.
- `user_judgments`의 요청, 맥락, 선택지, 영향받는 참조, 아티팩트 참조,
  `sensitive_action_scope_json` 또는 `resolution_json` 열.
...
`tool_invocations.response_json`은 재실행 행을 만드는 상태 효과에 대해 커밋된
`dry_run=false` Core `MethodResult` 응답 본문만 정확히 저장합니다. `StatusResult`,
`ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 `MethodResult` 결과, 성공한
`StageArtifactResult` 스테이징 결과는 저장하지 않습니다. 커밋된 차단 결과는 API
메서드별 상태 효과 표가 그 차단 커밋을 허용하고 응답이 커밋된 `MethodResult`일 때만
저장합니다.

`decision=blocked`, `decision=approval_required`, `decision=decision_required`인 커밋된
`PrepareWriteResult` 응답은 현재 API 계약이 허용하는 대로
`tool_invocations.response_json`에 `write_decision_reasons: WriteDecisionReason[]`를
담을 수 있습니다. 이 값은 응답과 재실행 페이로드에 담기는 prepare_write 판단 사유이지
닫기 준비 상태 차단 사유가 아닙니다. `prepare_write`는 `close_state`를 바꾸지 않고,
닫기 가능성 평가를 실행하지 않으며, `CloseReadinessBlocker` 저장을 만들지 않습니다.

`CloseTaskResult(close_state=blocked)`는 유효한 닫기 가능성 평가에서 나온 커밋된
`dry_run=false` Core `MethodResult`이고 메서드 상태 효과 표가 커밋된 닫기 차단 효과를
허용할 때만 재실행 행으로 저장합니다. `harness.close_task intent=check`가 반환하는
`CloseTaskResult`는 요청에 `dry_run=true`가 있어도 `effect_kind=read_only`인 읽기 전용
`MethodResult`이며 재실행 행으로 저장하지 않습니다. `STATE_VERSION_CONFLICT`를 담은
`ToolRejectedResponse`는 재실행 행으로 저장하지 않고 `CloseReadinessBlocker`도 만들지
않습니다.
...
`tasks.completion_policy_json` 또는 `change_units.completion_policy_json`은 Task 또는
Change Unit에서 `close_task intent=complete`에 쓰는 간결한 `CompletionPolicy`를
저장합니다. `intent=cancel`이나 `intent=supersede` 정책이 아닙니다.
`evidence_summaries` 행은 `task_id`와, 값이 있으면 `change_unit_id`를 통해 그 담당 정책에
묶입니다. `EvidenceSummary.completion_policy`를 포함하는 응답 형태는 증거 범위를 정책
담당자로 보지 않고 담당 Task 또는 Change Unit에서 정책을 읽어 구성합니다.

`evidence_summaries.coverage_items_json`은 명시적인 `claim`, `required_for_close`,
`coverage_state`, 검증된 뒷받침 참조 또는 공백 참조를 가진 경계 있는
`EvidenceCoverageItem` 항목을 저장합니다. 저장소는 필수/선택 구분이 빠졌거나, 알 수
없는 `coverage_state`를 쓰거나, 호환되는 `project_id`/`task_id` 범위 밖 참조를 이름
붙이는 `coverage_items_json`을 거부해야 합니다. 담당
`completion_policy_json.evidence_required=true`이면 저장된 요약은 필수
`EvidenceCoverageItem`이 `unsupported`, `partial`, `stale`,
`blocked`, 또는 빠진 상태일 때 `harness.close_task`가 증거 차단 사유를 도출할 수 있을
만큼 충분한 정보를 남겨야 합니다.
...
`existing_artifact` 입력은 이미 지속된 `ArtifactRef`를 가리켜야 하며, 같은
...
원시 캡처 어댑터 출력, 접점 자체 캡처 주장은 현재 MVP의 아티팩트 권한이 아닙니다.
...
이 순서의 어느 검증이라도 커밋 전에 실패하면 저장소는 `artifact_staging.status`,
`consumed_by_run_id`, `promoted_artifact_id`, `artifacts`, `artifact_links`,
`evidence_summaries`, `write_authorizations.status`, `task_events`, `tool_invocations`,
`project_state.state_version`을 바꾸면 안 됩니다. 아티팩트 본문 읽기는
`access_class=artifact_read`가 필요한 별도 읽기입니다.
...
제출된 핸들의 모양만 믿지 말고 스테이징 행과 대조해야 합니다. 성공한
`harness.stage_artifact`는 `base.effect_kind=staging_created`인 `StageArtifactResult`를
반환합니다. 안전한 바이트 또는 안전한 알림을 `artifacts/tmp/` 아래에 쓰고 임시
스테이징 행을 만들 수 있지만, 이는 저장소가 소유하는 임시 스테이징 부수 효과입니다.
Core 기록, `artifacts` 행, `artifact_links` 행, `evidence_summaries` 행, `task_events`
행, `tool_invocations` 재실행 행, `project_state.state_version` 증가를 만들지 않습니다.

만료되지 않았고 같은 프로젝트/같은 Task에 속하며 `artifact_staging.status=staged`인
핸들을 소비해 지속 `ArtifactRef`로 승격할 수 있는 활성 경로는 호환되는
`harness.record_run`뿐입니다. 또한 현재 확인된 `surface_id`와 `surface_instance_id`가
`created_by_surface_id`와 `created_by_surface_instance_id`와 일치해야 합니다. 현재 MVP는 접점 간(cross-surface)
staged artifact handoff를 지원하지 않으며, `StagedArtifactHandle`은 어떤 로컬 호출자나
사용할 수 있는 bearer token이 아닙니다. 소비 트랜잭션은 저장된 `project_id`,
`task_id`, `created_by_surface_id`, `created_by_surface_instance_id`, 만료 여부, 소비 상태, `sha256`,
...
`created_by_surface_id`/`created_by_surface_instance_id`/`sha256`/`size_bytes`/`redaction_state`가 맞지 않거나,
...
- 아티팩트 저장소 아래 지속 바이트 또는 안전한 메타데이터 알림,
...
증거로 쓸 수 있는 자격, 아티팩트 가용성, 증거 충분성은 서로 다릅니다.
`artifacts.status=available`인 행에 유효한 담당 연결이 있으면 `EvidenceCoverageItem`을 뒷받침할 수
있지만, 필수 `EvidenceCoverageItem`이 그 아티팩트를 주장에 연결하고 항목 상태가 `supported` 또는
`not_applicable`일 때만 `EvidenceSummary.status=sufficient`가 될 수 있습니다. 아티팩트가
없거나, 사용할 수 없거나, 무결성에 실패했거나, 그 밖의 이유로 쓸 수 없으면 아티팩트
가용성 문제로 남고, 필수 증거 항목도 충분하지 않을 수 있습니다.

`artifacts.status`는 가용성 상태입니다.

| 값 | 저장 의미 |
|---|---|
| `available` | 지속 안전 바이트 또는 안전한 메타데이터 알림이 있고 저장된 무결성 메타데이터와 맞습니다. |
| `missing` | 아티팩트 행은 남아 있지만 지속 바이트 또는 안전한 메타데이터 알림을 찾을 수 없습니다. |
| `integrity_failed` | 사용할 수 있는 바이트 또는 메타데이터가 `sha256`, `size_bytes` 같은 저장된 무결성 사실과 맞지 않습니다. |
| `unavailable` | 아티팩트 저장소 또는 필요한 조회 경로가 지속 바이트나 안전한 메타데이터 알림을 현재 제공할 수 없습니다. |
...
읽으려면 지속 `ArtifactRef`, 같은 프로젝트의 일치하는 `task_id`, 필요한
...
새로 커밋되는 `dry_run=false` 상태 변경에서는 현재 행 쓰기, `task_events` 추가, 프로젝트 전체
상태 버전 증가, `tool_invocations` 재실행 행 삽입이 하나의 트랜잭션으로 커밋되어야 합니다.
`harness.record_run`에서는 `artifact_staging`의 스테이징 핸들 소비, 아티팩트
승격/연결, 증거 업데이트, Write Authorization 소비, 이벤트 추가, 재실행 행 삽입,
`project_state.state_version` 정확히 한 번 증가가 같은 트랜잭션에 속합니다. 어느 하나라도
실패하면 부분적인 Write Authorization 행, 스테이징 소비, 아티팩트 승격/연결, Write Authorization 소비,
증거 업데이트, 이벤트, 닫기 효과, 재실행 행, 상태 버전 증가가 남지 않아야 합니다.

`tool_invocations`는 API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된
`dry_run=false` Core `MethodResult` 응답의 정확한 재실행만 저장합니다.
`ToolRejectedResponse`와 `ToolDryRunResponse`는 저장하지 않으며, 이 두 분기는
재실행 행을 만들거나 예약하지 않습니다. 키 범위는
[API Errors: Idempotency](api/errors.md#idempotency)가 담당합니다.
같은 키와 요청 해시가 재실행되면 Core는 이벤트를 추가하거나, 아티팩트를 승격/연결하거나,
Write Authorization을 소비하거나, 상태를 다시 바꾸지 않고 원래 커밋된 응답을
반환합니다. 같은 키가 다른 요청 해시로 재사용되면 Core는
[API Errors](api/errors.md#state-conflict-behavior)가 정의한 `STATE_VERSION_CONFLICT`를
반환합니다. 저장소 고유 키는
`(project_id, tool_name, idempotency_key)`입니다. `request_hash`는 그 행에 저장하는
충돌 판별자입니다.

`ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 결과, 성공한 `StageArtifactResult`
스테이징 결과는 `tool_invocations`에 저장하지 않습니다. `harness.status`와
`harness.close_task intent=check` 같은 읽기 전용 호출, 잘못된 요청, 커밋 전 검증 실패,
커밋 전 상태 버전 충돌, `ToolDryRunResponse` 미리보기, 상태 변경을 만들지 않는 거부된
`record_run` 시도는 현재 행을 만들거나, `artifact_staging.status`를 바꾸거나,
...
유효한 `ToolDryRunResponse` 미리보기는
`DryRunSummary.would_blockers: PlannedBlocker[]`를 담을 수 있습니다. 이
`PlannedBlocker[]` 항목은 미리보기 데이터일 뿐입니다. `task_event` 없음,
`task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음,
`close_state` 변경 없음, Write Authorization 변경 없음, 스테이징된 핸들 생성 또는 소비
없음, 아티팩트 효과 없음, 증거 업데이트 없음, `CloseReadinessBlocker` 저장 없음,
`project_state.state_version` 증가 없음입니다.

### 읽기 전용 결과 저장 효과

읽기 전용 결과는 응답에만 남으며 재실행 행이 아닙니다. `harness.status`와
`harness.close_task intent=check`는 응답을 위해 차단 사유, `CloseReadinessBlocker[]`,
증거 요약, 아티팩트 참조, 진단, 다음 행동을 계산할 수 있습니다. 하지만 읽기가
일어났다는 이유만으로 그 계산값을 저장하면 안 됩니다.

`harness.status`가 `close_blockers: CloseReadinessBlocker[]`를 반환하는 경우도 읽기 전용
관찰입니다. `task_event` 없음, `task_events` 추가 없음, 재실행 행 없음,
`tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 변경
없음, 스테이징된 핸들 소비 없음, 아티팩트 효과 없음, 증거 업데이트 없음,
`project_state.state_version` 증가 없음입니다.

`harness.close_task intent=check`는 `base.effect_kind=read_only`인 `CloseTaskResult`를
반환합니다. 같은 선택 동작에 `dry_run=true`가 있어도 응답은 `base.dry_run=true`,
`base.effect_kind=read_only`인 `CloseTaskResult`로 유지됩니다. `ToolDryRunResponse`가
아닙니다.

`harness.close_task intent=check`의 두 형태는 `blockers: CloseReadinessBlocker[]`를
반환하더라도 읽기 전용입니다. `task_event` 없음, `task_events` 추가 없음, 재실행 행 없음,
`tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 변경
없음, 스테이징된 핸들 소비 없음, 아티팩트 효과 없음, 증거 업데이트 없음,
`project_state.state_version` 증가 없음입니다. `tool_invocations.response_json`은 재실행 행을
만드는 상태 효과가 있는 커밋된 `dry_run=false` Core `MethodResult` 응답만 저장하므로,
읽기 전용 결과는 재실행 행으로 저장하지 않습니다.

### `prepare_write` 저장 효과

`harness.prepare_write`의 저장 효과는 응답 분기와 맞아야 합니다. 커밋된 `dry_run=false`
`PrepareWriteResult(decision=allowed)`는 API 계약이 허용한 호환 활성 Write Authorization을
만들거나 반환할 수 있고, 이벤트 추가, 재실행 행 생성, `project_state.state_version`
정확히 한 번 증가를 만들 수 있습니다.

`decision=blocked`, `decision=approval_required`, `decision=decision_required`인 커밋된
`dry_run=false` `PrepareWriteResult`는 메서드 상태 효과 계약이 그 판단 커밋을 허용할
때 응답과 재실행 페이로드에 `write_decision_reasons: WriteDecisionReason[]`를 담을 수
있습니다. 이 사유는 prepare_write 판단 사유이지 닫기 준비 상태 차단 사유도 아니고
`CloseReadinessBlocker[]`도 아니며, 닫기 준비 상태 차단 사유 기록도 아닙니다. 이 분기는
소비 가능한 Write Authorization을 만들지 않고, `close_state`를 바꾸지 않고, 닫기 가능성
평가를 실행하지 않고, `CloseReadinessBlocker` 저장을 만들지 않으며, 증거를 갱신하거나,
아티팩트를 바꾸거나, 스테이징된 핸들을 소비하거나, `close_task` 효과를 수행하면 안 됩니다.
메서드 계약이 허용한 이벤트, 재실행 행, prepare_write 판단 사유 페이로드,
`project_state.state_version` 효과만 가질 수 있습니다.

거절된 `prepare_write`는 `effect_kind=no_effect`인 `ToolRejectedResponse`를 반환합니다.
재실행 행 없음, `tool_invocations.response_json` 없음, 이벤트 없음, Write Authorization 생성
없음, Write Authorization 소비 없음, `close_state` 변경 없음, `CloseReadinessBlocker`
없음, 닫기 준비 상태 차단 사유 기록 없음, 아티팩트 효과 없음, 증거 업데이트 없음,
스테이징된 핸들 소비 없음, `project_state.state_version` 증가 없음입니다.

유효한 dry-run `prepare_write`는 `ToolDryRunResponse`를 반환하며 재실행 행이나
상태/저장소 효과를 만들지 않습니다. 예상 prepare_write 판단 사유는 미리보기 전용
`PlannedBlocker` 항목일 뿐이며, 저장된 `WriteDecisionReason` 객체도 아니고
`CloseReadinessBlocker` 저장도 아닙니다.

### `close_task` 저장 효과

`harness.close_task`의 저장 효과는 응답 분기와 맞아야 합니다. 닫기 사전 확인 거절은
`effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환합니다.
`CloseTaskResult`가 아니며 `CloseTaskResult(close_state=blocked)`도 아닙니다.

닫기 사전 확인 거절에는 `expected_state_version` 불일치, 다른 `request_hash`로
`idempotency_key`를 재사용한 경우, 오래된 `WriteAuthorization.basis_state_version`,
닫기 가능성 평가 전 Core 상태를 읽을 수 없는 경우, 유효한 Project 또는 Task를
선택하기 전 요청 식별 정보 실패, 닫기 가능성 평가 전 로컬 접근 또는 역량 실패,
닫기 가능성 평가 전 검증 실패가 포함됩니다. 이 커밋 전 실패는 저장소 효과 없음입니다.
`CloseReadinessBlocker` 없음, `task_event` 또는 `task_events` 추가 없음,
...
`CloseTaskResult(close_state=blocked)`는 닫기 가능성 평가가 실행되었고
`harness.close_task` 메서드 계약이 차단 결과 커밋을 허용할 때만 저장소 효과가 있는
커밋된 닫기 차단 결과입니다. `blockers: CloseReadinessBlocker[]`를 포함할 수 있고,
API/저장소 계약이 명시적으로 허용한 차단 사유 상태, `task_events`, 재실행 행,
`project_state.state_version` 효과만 만들 수 있습니다. `CloseReadinessBlocker`가 있다는
사실만으로 저장 효과를 뜻하지 않으며 Task는 열린 상태로 남겨야 합니다.
`STATE_VERSION_CONFLICT`에는 이 분기를 사용하면 안 됩니다. 그 코드는 사전 확인의
`ToolRejectedResponse` 분기에 속하며 재실행으로 저장하지 않습니다.

커밋된 차단 결과는 담당 메서드의 형태를 따라야 합니다. 커밋된 `decision=allowed`가 아닌
`PrepareWriteResult`는 응답과 재실행 페이로드의 `write_decision_reasons`를 사용합니다.
저장소는 이 prepare_write 판단 사유를 `CloseReadinessBlocker` 값이나 닫기 준비 상태 차단
사유 기록으로 바꾸면 안 됩니다. 커밋된 차단 `CloseTaskResult`는 응답 데이터로
`CloseReadinessBlocker[]`를 사용하며, `close_task` 계약이 명시적으로 허용한 차단 사유 상태,
이벤트, 재실행 행, `project_state.state_version` 효과만 만들 수 있습니다. 어떤 커밋된 차단
결과도 차단 사유가 없거나 부족하다고 지적한 권한을 만들면 안 됩니다. 차단된
`prepare_write`는 소비 가능한 `write_authorizations`를 만들지 않습니다.
...
새 `dry_run=false` 상태 변경 API 호출은 커밋 전에
...
`expected_state_version`을 요구하거나 받지 않습니다.

커밋된 모든 `dry_run=false` 상태 변경은 `project_state.state_version`을 정확히 1 올립니다.
...
`harness.status`, `harness.close_task intent=check`, `dry_run=true`인
`harness.close_task intent=check`, `ToolDryRunResponse` 미리보기 호출, 잘못된 요청,
커밋 전 검증 실패, 커밋 전 상태 버전 충돌, 멱등 재실행은 `project_state.state_version`을
올리지 않습니다.
응답 분기의 `state_version` 값은 항상 프로젝트 전체 버전을 사용합니다. 커밋된 상태
변경에서는 커밋 뒤 결과 버전이고, 읽기 전용 결과, `ToolDryRunResponse` 미리보기,
임시 스테이징 응답에서는 그 응답이 관찰한 현재 프로젝트 전체 버전입니다.

활성 첫 스키마에서는 `tasks.state_version`을 생략해야 합니다. 구현이 레거시 또는
프로토타입 `tasks.state_version` 열을 만나더라도 그 값은 비활성 메타데이터일 뿐입니다.
승인, `STATE_VERSION_CONFLICT`, 오래된 상태 판단, Write Authorization, 멱등성, 잠금, 동시성
기준으로 쓰면 안 됩니다.
...
그 불일치를 공개 API로 드러낼 때는 `STATE_VERSION_CONFLICT`를 사용합니다. 호출은 소비 전에
거절됩니다. 다른 현재 계약이 명시적으로 말하지 않는 한 이 거절된 호출은 Write Authorization
상태를 `consumed`, `invalid`, `stale` 또는 다른 어떤 상태로도 바꾸지 않으며,
`close_task`는 그 오래된 근거 조건에 대한 닫기 차단 사유를 저장하면 안 됩니다. 저장소
예시는 오래된 상태나 버전 불일치를 설명할 수 있지만, 저장소가 다른 공개 오류 이름을
만들거나 내부 데이터베이스 예외 이름을 공개하지 않습니다.
...
- 상태 카드, 간결한 상태 보기, Projection 최신성, 닫기 준비 상태, 보고서 문장은 현재
  레코드에서 읽는 시점에 파생합니다. 마이그레이션 권한, 복구 입력, 저장소 변경 경로가
  아닙니다.
...
- Projection 작업(`projection job`), 영속 Projection 캐시, managed-output outbox, managed block 해시와 drift
  복구 기록, Projection 대시보드.
...
- `captured_artifact` 핸들, `native capture` 저장소, 캡처 어댑터 출력 테이블, 원시
  캡처 queue, 접점 자체 아티팩트 캡처 기록.
...
활성 상태, 닫기 준비 상태, 실행/증거 요약, 다음 행동, 읽기용 카드,
`agent-context-packet`, 보장 표시는 위 활성 영속 레코드 위에 읽는 시점에 만드는 보기입니다.
이 출력은 오래되었거나, 없거나, 실패했을 수 있고 다시 계산될 수 있습니다. 그래도 저장소
권한을 바꾸지 않습니다.
