...
projection이 없습니다. 현재 저장소 단계와 인계 상태는 [구현 개요](../build/implementation-overview.md#문서-수락-상태)에서
확인합니다.
...
| Stage sequence와 implementation readiness | [Build: MVP-1 사용자 작업 루프](../build/mvp-user-work-loop.md), [내부 엔지니어링 점검](../build/engineering-checkpoint.md), [구현 개요](../build/implementation-overview.md) |
...
- `tool_invocations`는 committed idempotency replay를 지원합니다. 별도 user-facing
  domain record가 아닙니다.
- `artifacts`는 registered evidence byte 또는 safe metadata를 저장합니다. Core가
  owner-valid row로 연결하기 전까지 sufficiency를 증명하지 않습니다.
- `artifact_links`는 artifact를 owner record에 연결합니다. Link 자체는 report,
  projection, QA result, Eval, work acceptance, residual-risk acceptance가 아닙니다.
...
artifact registration boundary를 적용한 뒤 registered file 또는 safe metadata를
보관합니다.

`project.yaml`은 current Task state, current gate, Write Authorization state, evidence
sufficiency, work acceptance, residual-risk acceptance를 저장하면 안 됩니다.
...
| Registered artifact hash, size, owner link, path가 storage metadata와 더 이상 맞지 않습니다. | Projection drift가 아니라 evidence integrity failure 또는 recovery input으로 취급합니다. |
...
| `tasks` | User-value work unit과 task-scoped state clock. | `task_id`, `project_id`, `title`, `user_request`, `mode`, `lifecycle_phase`, `result`, `summary`, `active_change_unit_id`, `state_version`, `created_at`, `updated_at`, `closed_at`. |
...
|---|---|---|
| `artifacts` | Durable byte 또는 safe metadata와 integrity fact를 등록합니다. | Evidence sufficiency, QA, Eval, acceptance, risk acceptance, export bundle, report prose. |
...
| Close readiness | Task lifecycle, scope state, pending user judgments, evidence coverage, artifact availability, open blockers, work-acceptance와 residual-risk user judgment | 파생 check입니다. Active storage는 check에 쓰는 owner record와 blocker를 저장하며 별도 `close_readiness` 기준 source를 만들지 않습니다. |
...
| Full Manual QA matrix, including `manual_qa_records` | Human inspection workflow, finding, setup, QA evidence ref | Full QA profile이 active가 아닌데 QA가 required이면 user judgment, blocker, evidence summary visibility로 표시합니다. |
...
- `domain_terms`, `module_map_items`, `interface_contracts`, `shared_designs`,
  `change_unit_dependencies` 같은 design/stewardship record
...
| `evidence_summaries.coverage_state` | Core evidence gate와 API evidence summary owners |
