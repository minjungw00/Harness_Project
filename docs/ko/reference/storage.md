...
| `tool_invocations` | `state.sqlite` | API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된 non-dry-run Core `MethodResult` 응답만 저장하는 재실행 행입니다. 거절, `dry_run`, 읽기 전용, 스테이징 생성 응답은 여기에 저장하지 않습니다. | `invocation_id`, `project_id`, `tool_name`, `idempotency_key`, `request_hash`, `task_id`, `basis_state_version`, `response_json`, `status`, `created_at`. |
...
| `tool_invocations.status` | `committed` | 재실행 행은 API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된 non-dry-run Core `MethodResult` 응답에만 존재합니다. `ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 결과, 성공한 `StageArtifactResult` 스테이징 결과에는 재실행 행이 없습니다. |
...
`tool_invocations.response_json`에 저장하면 안 됩니다.
...
스테이징 결과는 `tool_invocations`에 저장하지 않습니다. `dry_run`, 잘못된 요청,
커밋 전 검증 실패, 커밋 전 상태 버전 충돌, `harness.status`와
`harness.close_task intent=check` 같은 읽기 전용 호출, 상태 변경을 만들지 않는 거부된
...
검증 실패, 커밋 전 상태 버전 충돌, 멱등 재실행은 `project_state.state_version`을 올리지
않습니다. 응답 분기의 `state_version` 값은 항상 프로젝트 전체 버전을 사용합니다.
커밋된 상태 변경에서는 커밋 뒤 결과 버전이고, 읽기 전용과 `dry_run` 응답에서는 그
응답이 관찰한 현재 프로젝트 전체 버전이며, 임시 스테이징 응답도 같은 관찰 버전을 반환합니다.
