...
현재 MVP는 활성 메서드 집합에 필요한 레코드만 영속화합니다.
...
| `project_state` | `state.sqlite` | 프로젝트별 로컬 상태 헤더, 상태 시계, 활성 Task 포인터, 기본 접점 포인터를 저장합니다. | `project_id`, `schema_version`, `storage_profile`, `state_version`, `active_task_id`, `default_surface_id`, `created_at`, `updated_at`. |
...
Task 하나와 그 담당 행에 대한 Task 범위 상태 시계입니다. 현재 MVP의 공개 변경은
영향받는 상태 시계 하나를 고릅니다. 하나의 호출이 둘 이상의 `expected_state_version`을
요구하는 다중 범위 최신성은 담당 문서가 승격하기 전까지 이후 후보입니다.
