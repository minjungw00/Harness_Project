...
찾은 primary Task가 없는 프로젝트 범위 변경은 `project_state.state_version`을 올립니다.
...
Task 수준 범위와 프로젝트 수준 범위를 섞으면 안 됩니다. Core가 상태 변경의 primary Task를
찾거나 만들면 최신성 확인과 응답 상태 버전은 그 Task의 `tasks.state_version`을 사용합니다.
상태 변경에 primary Task가 없으면 최신성 확인과 응답 상태 버전은
