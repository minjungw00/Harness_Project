...
| `tool_invocations` | `state.sqlite` | API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된 non-dry-run Core `MethodResult` 응답만 저장하는 재실행 행입니다. `ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 결과, 스테이징 생성 응답은 여기에 저장하지 않습니다. 여기에는 `dry_run=true`인 `harness.close_task intent=check`의 `CloseTaskResult`도 포함됩니다. | `invocation_id`, `project_id`, `tool_name`, `idempotency_key`, `request_hash`, `task_id`, `basis_state_version`, `response_json`, `status`, `created_at`. |
...
| `tool_invocations.status` | `committed` | 재실행 행은 API 메서드별 상태 효과 행이 재실행 행 생성을 허용한, 커밋된 non-dry-run Core `MethodResult` 응답에만 존재합니다. `ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 결과, 성공한 `StageArtifactResult` 스테이징 결과에는 재실행 행이 없습니다. `dry_run=true`인 `harness.close_task intent=check`는 `ToolDryRunResponse`가 아니라 읽기 전용 결과입니다. |
...
non-dry-run Core `MethodResult` 응답 본문만 정확히 저장합니다. 커밋된 차단 결과는
API 메서드별 상태 효과 표가 그 차단 커밋을 허용하고 응답이 커밋된 `MethodResult`일
때만 저장합니다. 저장소는 `ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용
`MethodResult` 결과, 성공한 `StageArtifactResult` 스테이징 결과를
`tool_invocations.response_json`에 저장하면 안 됩니다. `harness.close_task intent=check`가
반환하는 `CloseTaskResult`는 요청에 `dry_run=true`가 있어도
`effect_kind=read_only`인 읽기 전용 `MethodResult`이며, 재실행 행으로 저장하지
않습니다.
...
non-dry-run Core `MethodResult` 응답의 정확한 재실행만 저장합니다. 키 범위는
...
소비하거나, `write_authorizations.status`를 바꾸거나, 닫기 상태를 바꾸거나, `tool_invocations`
재실행 행을 만들거나, 상태 버전을 올리면 안 됩니다. 성공한 `harness.stage_artifact`는
...
`task_event` 없음, 닫기 상태 변경 없음, 아티팩트 갱신 또는 아티팩트 연결 없음,
스테이징된 핸들 소비 없음, Write Authorization 생성 또는 소비 없음,
`project_state.state_version` 상태 버전 증가 없음입니다. `tool_invocations.response_json`은
재실행 행을 만드는 상태 효과가 있는 커밋된 non-dry-run Core `MethodResult` 응답만
...
차단 `MethodResult`에 차단 사유 또는 다른 현재 행 상태 변경 저장을 허용하면, 그 응답은
이벤트, 재실행 행, 상태 버전 목적에서도 커밋된 non-dry-run Core `MethodResult`이며
...
그 불일치를 공개 API로 드러낼 때는 `STATE_VERSION_CONFLICT`를 사용합니다. 저장소 예시는
오래된 상태나 버전 불일치를 설명할 수 있지만, 저장소가 다른 공개 오류 이름을 만들거나
내부 데이터베이스 예외 이름을 공개하지 않습니다.
