...
- 공개 MCP 요청/응답, 공유 스키마, active enum 값, error, replay
...
대해서만 현재 하네스 레코드의 기준이 됩니다. 대화, 생성된 Markdown, status card,
Projection, 커넥터 출력, 운영자 출력, 보고서 문장은 저장소 권한이 아닙니다.

## 2. Runtime Home identity

하네스는 로컬 Runtime Home 하나와 등록된 project별 project-local 상태 데이터베이스 하나를
사용합니다. 기본 기준 루트는 `~/.harness`입니다. 구현은 같은 역할의 설정된 루트를
선택할 수 있습니다.
...
`registry.sqlite`는 Runtime Home 식별 정보와 최소 project 등록 데이터를
저장합니다. `project.yaml`은 정적 project 설정만 저장합니다.
`state.sqlite`는 project-local Core 상태를 저장합니다. 아티팩트 디렉터리는 Core가
아티팩트 등록 경계를 적용한 뒤 등록된 증거 바이트 또는 안전한 메타데이터를
저장합니다.

`project.yaml`은 현재 Task 상태, gate, Write Authorization 상태, 증거 충분성,
최종 수락, 잔여 위험 수락, close 상태를 저장하면 안 됩니다.

Runtime Home 식별은 파일시스템 경로에만 의존하면 안 됩니다. 복사되거나 이동된
Runtime Home은 같은 저장된 `runtime_home_id`를 가질 수 있습니다. 새 Runtime Home은 새 id를
가져야 합니다. 이 id는 의심스러운 복사본, 중복 등록, 경로 drift를 감지하는 데
...
hash, diagnostic은 방어적 확인입니다. 그 자체로 OS 수준 샌드박스, 임의 도구 제어,
변조 방지 저장소, 실행 전 차단을 만들지 않습니다.
...
Lock은 동시 상태 쓰기를 보호합니다. OS 샌드박스, 아티팩트 무결성,
