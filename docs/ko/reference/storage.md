...
| `blockers` | `state.sqlite` | 다음 행동, 쓰기 호환성, 증거 공백, close 가능성 평가, 복구를 위한 구조화된 차단 사유 상태를 저장합니다. `close_task` 계약이 커밋된 닫기 차단 효과를 허용할 때만 close 가능성 평가 차단 사유 상태를 저장할 수 있습니다. `CloseReadinessBlocker`는 API 데이터 구조이며 그 자체가 행이나 저장 신호가 아닙니다. `prepare_write`의 `write_decision_reasons`는 응답과 재실행 페이로드에 담기는 prepare_write 판단 사유이지 close 가능성 평가 차단 사유 행이 아닙니다. | `blocker_id`, `task_id`, `blocked_action`, `blocker_kind`, `status`, `message`, `owner_ref_json`, `related_refs_json`, `required_next_action`, `created_at`, `resolved_at`. |
...
- 차단된 `prepare_write`, `dry_run`, 잘못된 요청, 커밋 전 실패는 소비 가능한
  ...
  close 가능성 평가 차단 사유 행도 아니고 `close_state` 변경도 뜻하지 않습니다.
...
close 가능성 평가 차단 사유가 아닙니다. `prepare_write`는 `close_state`를 바꾸지 않고,
...
있습니다. 이 사유는 prepare_write 판단 사유이지 close 가능성 평가 차단 사유도 아니고
`CloseReadinessBlocker[]`도 아니며, close 가능성 평가 차단 사유 기록도 아닙니다. 이 분기는
...
없음, close 가능성 평가 차단 사유 기록 없음, 아티팩트 효과 없음, 증거 업데이트 없음,
...
저장소는 이 prepare_write 판단 사유를 `CloseReadinessBlocker` 값이나 close 가능성 평가 차단
