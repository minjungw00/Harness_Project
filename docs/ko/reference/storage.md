...
- Runtime Home identity와 project-local 저장소 배치.
- 활성 영속 레코드와 테이블별 저장 역할.
- 저장소가 소유하는 JSON `TEXT` 규칙.
- Artifact persistence와 artifact 담당 연결.
- Event와 멱등성 저장 의미.
- 상태 버전 저장 규칙.
- 잠금 정책과 마이그레이션 경계.
...
- Public MCP 요청/응답, shared schema, active enum value, error, replay
  behavior. [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md),
  [API Errors](api/errors.md)를 봅니다.
- Projection rendering, 상태 보기 템플릿 본문, report format, dashboard, export,
...
있습니다. 넓은 읽기 접근은 secret, PII, token, log, screenshot, diff, artifact content를
노출할 수 있습니다. 넓은 쓰기 접근은 tampering과 evidence poisoning risk입니다. File
...
| `project.yaml` | Project directory | Static project configuration. | `project_id`, `repo_root`, 표시/config default. |
