...
`dry_run=false` Core `MethodResult` 응답 본문만 정확히 저장합니다. 커밋된 차단 결과는
API 메서드별 상태 효과 표가 그 차단 커밋을 허용하고 응답이 커밋된 `MethodResult`일
때만 저장합니다. `CloseTaskResult(close_state=blocked)`는 유효한 닫기 차단 사유 행렬
평가에서 나온 커밋된 닫기 차단 결과일 때만 저장합니다. 저장소는
`ToolRejectedResponse`, `ToolDryRunResponse`, 읽기 전용 `MethodResult` 결과, 성공한
`StageArtifactResult` 스테이징 결과를 `tool_invocations.response_json`에 저장하면 안
됩니다. `STATE_VERSION_CONFLICT`를 담은 `ToolRejectedResponse`는 재실행 행으로 저장하지
않습니다. `harness.close_task intent=check`가 반환하는 `CloseTaskResult`는 요청에
`dry_run=true`가 있어도 `effect_kind=read_only`인 읽기 전용 `MethodResult`이며,
재실행 행으로 저장하지 않습니다.
...
`harness.close_task intent=check`는 응답을 위해 차단 사유, 닫기 차단 사유, 증거 요약,
아티팩트 참조, 진단, 다음 행동을 계산할 수 있습니다. 하지만 읽기가 일어났다는 이유만으로
그 계산값을 저장하면 안 됩니다.
...
닫기 차단 사유 행렬 평가 전 Core 상태를 읽을 수 없는 경우, 유효한 Project 또는 Task를
선택하기 전 요청 식별 정보 실패, 닫기 차단 사유 행렬 평가 전 로컬 접근 또는 역량 실패,
...
커밋된 닫기 차단 결과입니다. API/저장소 계약이 명시적으로 허용한 `CloseBlocker`,
`task_events`, 재실행 행, `project_state.state_version` 효과만 만들 수 있으며 Task는 열린
상태로 남겨야 합니다. `STATE_VERSION_CONFLICT`에는 이 분기를 사용하면 안 됩니다. 그 코드는
사전 확인의 `ToolRejectedResponse` 분기에 속하며 재실행으로 저장하지 않습니다.

어떤 메서드든 커밋된 차단 `MethodResult`는 API 메서드별 상태 효과 표가 허용한 차단 사유
또는 다른 상태 변경만 저장할 수 있습니다. 차단 사유가 없거나 부족하다고 지적한 권한을
만들면 안 됩니다. 예를 들어 차단된 `prepare_write` 응답은 소비 가능한
`write_authorizations`를 만들지 않습니다. API 담당 문서가 커밋된 차단 `MethodResult`에
차단 사유 또는 다른 현재 행 상태 변경 저장을 허용하면, 그 응답은 이벤트, 재실행 행,
상태 버전 목적에서도 커밋된 `dry_run=false` Core `MethodResult`이며
`tool_invocations.response_json`에 저장합니다.
...
값이 맞지 않으면 `STATE_VERSION_CONFLICT`를 반환하고 현재 기록, 이벤트, 아티팩트,
증거 요약, Write Authorization 생성 또는 소비, 닫기 상태, 재실행 행, 상태 버전 증가를 만들지
않습니다. 프로젝트 전체 상태 버전 불일치에 쓰는 현재 MVP의 유일한 공개 `ErrorCode`는
