...
읽는 시점의 상태/Projection 출력은 커밋된 기록과 등록된 아티팩트 참조에서 파생됩니다.
...
또는 동등한 스테이징 기록만 만듭니다. 현재 Core 권한 기록을 만들거나 프로젝트
상태 시계를 올리지 않습니다.
...
`harness.record_run`에서는 `artifact_staging`의 스테이징 핸들 소비도 같은 트랜잭션에
속합니다. 어느 하나라도 실패하면 부분적인 권한 행, 스테이징 소비, 이벤트, 아티팩트
등록, Write Authorization 소비, 증거 업데이트, 닫기 효과, 재실행 행이 남지 않아야 합니다.
...
`dry_run`, 잘못된 요청, 커밋 전 검증 실패, 커밋 전 상태 버전 충돌,
`harness.status`와 `harness.close_task intent=check` 같은 읽기 전용 호출,
상태 변경을 만들지 않는 거부된 `record_run` 시도는 현재 행을 만들거나,
`artifact_staging` 핸들을 소비하거나 갱신하거나, `task_events`를 추가하거나,
아티팩트를 등록하거나, 증거 요약을 갱신하거나, Write Authorization을 만들거나, 닫기
상태를 바꾸거나, `tool_invocations` 재실행 행을 만들거나, 상태 버전을 올리면 안 됩니다.
...
증거 요약, Write Authorization, 닫기 상태, 재실행 행, 상태 버전 증가를 만들지
않습니다. 프로젝트 전체 상태 버전 불일치에 쓰는 현재 MVP의 유일한 공개 `ErrorCode`는
`STATE_VERSION_CONFLICT`입니다. `STATE_CONFLICT`는 활성 공개 코드, 별칭, 폐기된 표기,
저장소 계층의 공개 오류 이름이 아닙니다. 현재 MVP의 공개 호출은 둘 이상의 공개
