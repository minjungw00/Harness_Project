...
- `projects/{project_id}/` 아래 프로젝트 디렉터리는 등록된 프로젝트 하나의 Harness
  project home입니다. `repo_root`와 같은 뜻이 아닙니다.
...
| `tool_invocations` | `state.sqlite` | `dry_run=false`인 상태 변경 도구 응답에 대한 커밋된 멱등 재실행 행입니다. | `invocation_id`, `project_id`, `tool_name`, `idempotency_key`, `request_hash`, `task_id`, `basis_state_version`, `response_json`, `status`, `created_at`. |
...
- 활성 테이블은 불투명하고 안정적인 id를 기본 키 또는 동등한 고유 키로
  사용합니다. 대상은 `project_id`, `surface_id`, `task_id`, `change_unit_id`,
  `user_judgment_id`, `write_authorization_id`, `run_id`, `artifact_id`,
  `artifact_link_id`, `evidence_summary_id`, `blocker_id`, `event_id`,
  `invocation_id`입니다.
...
- `surfaces`에는 고유한 `(project_id, surface_id)`가 필요합니다.
...
- `artifacts/tmp/`의 스테이징 바이트는 등록 전에는 증거 권한이 아니므로 정리할 수
  있습니다. 그러나 `artifacts` 행이 커밋된 뒤 보존 정책에 따른 삭제, 프로젝트 해체,
  파괴적 정리는 일반 현재 MVP 상태 변경 밖이며 담당 문서가 정의한 경로가 필요합니다.
- 향후 retention 또는 migration 경로는 아티팩트 hash, 담당 연결, 이벤트, 재실행 행을
  보존하거나 영향을 받은 ref를 복구 대상으로 유효하지 않게 표시해야 합니다. 현재
  기록이 아직 이름 붙이는 증거 뒷받침을 조용히 연쇄 삭제하면 안 됩니다.

닫힌 현재 MVP 저장 값 집합은 다음과 같습니다.
...
경계 없는 배열, 호환되는 project/task 범위 밖 기록을 가리키는 JSON은 커밋 전에 실패해야
...
`evidence_summary`, `blocker` 중 하나인지, `owner_record_id`가 대응 active 테이블에
...
차단된 응답은 메서드 담당 문서가 허용한 blocker 또는 다른 상태 변경만 저장할 수 있습니다.
차단 사유가 없거나 부족하다고 지적한 권한을 만들면 안 됩니다. 예를 들어 차단된 `prepare_write`
응답은 소비 가능한 `write_authorizations`를 만들지 않습니다. API 담당 문서가 커밋된
차단 응답에 blocker 또는 다른 현재 행 상태 변경 저장을 허용하면, 그 응답은 이벤트,
재실행 행, 상태 버전 목적에서도 커밋된 non-dry-run 상태 변경입니다.

## 8. 상태 버전 관리

상태 버전은 범위별 상태 시계입니다. 커밋된 상태 변경 호출은 영향을 받은 범위의 상태
시계를 정확히 한 번 올립니다. Task 범위 변경은 `tasks.state_version`을 올립니다. Core가
찾은 주 Task가 없는 프로젝트 범위 변경은 `project_state.state_version`을 올립니다.
...
범위의 결과 버전이고, 읽기 전용과 `dry_run` 응답에서는 현재 읽을 수 있는 버전
또는 영향을 받을 버전입니다.

`harness.status`, `harness.close_task intent=check`, `dry_run` 호출, 잘못된 요청, 커밋 전
검증 실패, 커밋 전 상태 충돌, 멱등 재실행은 상태 시계를 올리지 않습니다. 커밋된
차단 응답은 메서드 담당 문서가 Core에 blocker 또는 다른 현재 행 상태 변경 저장을
허용할 때만 영향을 받은 시계를 올립니다.

Task 수준 범위와 프로젝트 수준 범위를 섞으면 안 됩니다. Core가 상태 변경의 주 Task를
찾거나 만들면 최신성 확인과 응답 상태 버전은 그 Task의 `tasks.state_version`을 사용합니다.
상태 변경에 주 Task가 없으면 최신성 확인과 응답 상태 버전은
`project_state.state_version`을 사용합니다. 하나의 공개 호출에 둘 이상의
`expected_state_version`이 필요해지려면 향후 담당 문서가 명시적 다중 범위 버전
동작을 먼저 정의해야 합니다.
...
- Runtime Home 메타데이터와 `project_state`, 또는 유지보수자가 수락한 동등한 메커니즘에
  스키마/프로필 버전을 저장합니다.
- 향후 migration은 수락되기 전에 출발 버전, 대상 버전, 저장소 프로필, 담당 문서,
  되돌림 또는 복구 기대치를 선언해야 합니다.
- 향후 migration은 `registry.sqlite` 또는 `state.sqlite` 하나를 기준으로 트랜잭션 안에서
...
- 아티팩트 해시와 담당 연결을 보존하거나 영향을 받은 ref를 복구 대상으로 유효하지 않게
  표시합니다.
- 커밋된 `tool_invocations` 재실행 행을 보존해 migration 뒤 멱등성이 갈라지지 않게
  합니다.
...
- `doctor` 묶음, recover, export, release handoff, artifact dashboard, reconcile queue,
  운영 보고서를 위한 operations-profile 저장소.
...
- `user_judgments`와 `blockers`에서 분리된 상세 Approval 테이블과 상세 잔여 위험 생명주기
  테이블.
- 대시보드, 메트릭, 분석, 팀 작업 흐름, 호스팅 connector registry, connector
  marketplace, connector 분석, 접점 간 조율 저장소.
