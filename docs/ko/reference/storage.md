...
런타임 상태, 구현 완료된 DDL이 있다는 뜻이 아닙니다. 현재 저장소 상태는
...
- Runtime Home 식별과 project-local 저장소 배치.
- 활성 영속 레코드와 테이블별 저장 역할.
...
- 공개 MCP 요청/응답, 공유 스키마, active enum value, error, replay
  동작. [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md),
  [API Errors](api/errors.md)를 봅니다.
- Projection 렌더링, 상태 보기 템플릿 본문, report format, dashboard, export,
  reconcile 동작, operations entrypoint, conformance runner, future fixture
  storage.
- OS 권한, 샌드박스, 변조 방지 파일, 도구 실행 전 차단, 보안 격리 주장.
  [보안 참조](security.md)를 봅니다.

저장소는 Core가 커밋하고 담당 Core/API/storage 계약에 맞게 검증한 row에
대해서만 현재 하네스 레코드의 기준이 됩니다. Chat, 생성된 Markdown, status card,
projection, connector output, operator output, report prose는 저장소 권한이
아닙니다.

## 2. Runtime Home identity

하네스는 로컬 Runtime Home 하나와 등록된 project별 project-local state database 하나를
사용합니다. 기본 기준 root는 `~/.harness`입니다. 구현은 같은 역할의 configured root를
선택할 수 있습니다.
...
`registry.sqlite`는 Runtime Home 식별 정보와 최소 project registration data를
저장합니다. `project.yaml`은 정적 project configuration만 저장합니다.
...
Runtime Home 식별은 filesystem path에만 의존하면 안 됩니다. 복사되거나 이동된
...
도움이 됩니다. 하지만 storage를 변조 방지 상태로 만들지는 않습니다.

Runtime Home 파일은 로컬 운영 제어 데이터이고 민감한 지원 데이터를 담을 수 있습니다.
넓은 읽기 접근은 secret, PII, token, log, screenshot, diff, artifact content를 노출할 수
있습니다. 넓은 쓰기 접근은 변조와 증거 오염 위험입니다. File permission, owner check,
hash, diagnostic은 방어적 확인입니다. 그 자체로 OS-level sandboxing, arbitrary-tool
control, tamper-proof storage, pre-execution blocking을 만들지 않습니다.
...
- `registry.sqlite`의 최소 project registration.
- `project.yaml`의 정적 project configuration.
- `project_state`.
...
아래 표는 active 저장소 테이블과 최소 저장 역할을 이름 붙입니다. Full DDL이 아니며 API
schema를 복사하지 않습니다.
...
`display_label`은 active 저장소 identity column이 아닙니다. Display label은
`judgment_kind` 같은 stable identifier와 locale에서 파생합니다.

## 5. JSON TEXT 열

JSON을 저장하는 SQLite `TEXT` column은 저장 표현 선택입니다. 임의 JSON을
...
또는 이 문서가 이름 붙인 담당 문서에 맞게 validate합니다. `'{}'`, `'[]'` 같은 SQLite
default는 storage default일 뿐입니다. API field를 optional로 만들지 않습니다.

활성 JSON `TEXT` column은 active record에 필요한 compact 담당 형태 data로 제한합니다.
예시는 다음과 같습니다.
...
Status-like `TEXT` value는 open string이 아니라 닫힌 담당 value set입니다. Active value는
Core/API 담당 문서와 이 문서의 storage note가 담당합니다. Defensive `CHECK` constraint나
...
Artifact link는 담당 기록을 만들지 않습니다. 그 자체로 gate를 충족하거나, evidence
sufficiency를 증명하거나, QA를 수행하거나, final acceptance를 만들거나, residual risk를
수락하거나, Task를 close하지 않습니다.

## 7. 멱등성과 이벤트 의미

`task_events`는 committed Core mutation을 순서대로 기록합니다. 감사와 ordering trail이지,
...
반환합니다. 같은 key가 다른 request hash로 재사용되면 Core는 API 담당 문서가 정의한 state
conflict 동작을 반환합니다.
...
Blocked response는 메서드 담당 문서가 허용한 blocker 또는 다른 mutation만 저장할 수 있습니다.
...
Runtime mutation은 Core-owned state-changing path를 통해 직렬화합니다. 일반 SQLite
...
metadata는 담당 문서가 승격하기 전까지 later 운영 자료입니다.
...
- Commit 전과 constraint tightening 전에 담당 형태 JSON을 validate합니다.
- Unknown owner-bound status 또는 enum value는 담당 문서가 정의하기 전까지 invalid로
  취급합니다.
- `task_events`를 유지한다면 `task_events.event_seq` ordering을 보존합니다.
- Artifact hash와 담당 연결을 보존하거나 affected ref를 recovery 대상으로 invalid
  표시합니다.
...
Later/profile storage는 담당 문서가 범위, 대체 동작, 증명 기대치와 함께 좁은
...
있고 다시 계산될 수 있습니다. 그래도 저장소 권한을 바꾸지 않습니다.
