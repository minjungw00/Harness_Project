...
증거 요약, `CompletionPolicy`, 필수 `EvidenceCoverageItem`, 아티팩트 참조를 통해 저장합니다.
전체 Evidence Manifest 저장소를 요구하지 않습니다.
...
| `evidence_summaries` | `state.sqlite` | 상태, 실행/증거 요약, 차단 사유, 닫기에 쓰는 간결한 증거 범위, `CompletionPolicy`, 공백 기록을 저장합니다. | `evidence_summary_id`, `task_id`, `change_unit_id`, `status`, `completion_policy_json`, `coverage_items_json`, `summary`, `supporting_run_ids_json`, `supporting_artifact_link_ids_json`, `gap_blocker_ids_json`, `updated_at`. |
...
- `evidence_summaries.completion_policy_json`, `coverage_items_json`, 뒷받침/공백 참조 배열.
...
`evidence_summaries.completion_policy_json`은 Task 또는 Change Unit의 간결한
`CompletionPolicy`를 저장합니다. `coverage_items_json`은 명시적인 `claim`,
`required_for_close`, `coverage_state`, 검증된 뒷받침 참조 또는 공백 참조를 가진 경계 있는
`EvidenceCoverageItem` 항목을 저장합니다. 저장소는 필수/선택 구분이 빠졌거나, 알 수 없는
`coverage_state`를 쓰거나, 호환되는 `project_id`/`task_id` 범위 밖 참조를 이름 붙이는
`coverage_items_json`을 거부해야 합니다. `completion_policy_json.evidence_required=true`이면 저장된 요약은 필수
`EvidenceCoverageItem`이 `unsupported`, `partial`, `stale`, `blocked`, 또는 빠진 상태일 때
`harness.close_task`가 증거 차단 사유를 도출할 수 있을 만큼 충분한 정보를 남겨야 합니다.
