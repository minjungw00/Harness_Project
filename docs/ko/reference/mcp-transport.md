...
연결이 준비 상태가 된 뒤 `tools/list`는 묶인 Agent Connection 모드에 따라 도구를
노출합니다.

| 모드 | MCP에 보이는 공개 메서드 도구 |
|---|---|
...
MCP에 보이는 메서드 도구는 공개 Volicord Core API 메서드 목록과 같은 것이 아닙니다.
`volicord.record_user_judgment`는 User Channel 경로를 위한 공개 Core API 메서드이지만
Agent Connection MCP 도구로 노출되지 않습니다. 공개 메서드 담당 표는
[API 메서드](api/methods.md)를 봅니다.
...
공개 Volicord 메서드 도구에서 `tools/list`는 공개 메서드 payload 형태에 Agent
Connection 바인딩을 적용해 만든 MCP에 보이는 입력 스키마를 노출합니다. 보이는 스키마는
내부 요청 래퍼, 프로토콜 메타데이터, 프로젝트 ID, 연결 ID, `actor_source`,
`operation_category`, 검증 근거 필드를 숨겨야 합니다. 원시 공개 메서드 도구 인자가
최상위나 래퍼처럼 보이는 객체 안에 호출자 소유 호출 필드를 포함하면 어댑터는 Core 실행
전에 호출을 거절합니다.
