...
- `one_of:`는 branch object를 정의합니다. 나열된 child field 중 정확히 하나만 present이고, 그 value는 해당 child field의 stated type과 일치해야 합니다. Branch prose는 selected branch가 `decision_profile` 또는 `decision_kind` 같은 discriminator field와 일치해야 한다고 추가로 요구할 수 있습니다. 선택되지 않은 모든 branch는 `null`로 present가 아니라 absent여야 합니다.
...
6. 적용되는 경우 맞는 public tool 또는 Decision Packet path로 evidence/eval/QA/작업 수락을 기록합니다.
7. Blocker가 사라지면 `harness.close_task`로 close합니다.

v0.1 Core Authority Slice는 Core-owned authority를 증명하는 데 필요한 좁은 subset만 사용합니다. Status/blocker read, Task와 scoped boundary setup, `prepare_write`, 한 번만 쓰는 Write Authorization 하나, `record_run`, artifact/evidence ref 하나, structured blocker/status response가 전부입니다. Evidence Manifest, Eval, 수동 QA, 작업 수락, 잔여 위험 수용, full close semantics는 이후 profile behavior입니다.

### 단계별 요구사항 경계

`required`는 다음 세 뜻을 분리해서 읽습니다.

- **Schema-required**: 이 API schema에서 해당 method, schema object, profile이 구현되어 payload를 받거나 내보낼 때 required라는 뜻입니다.
- **Stage-required**: v0.1 Core Authority Slice 또는 v0.2 User-Facing Harness MVP 같은 특정 delivery stage를 종료하는 데 required라는 뜻입니다.
- **Reference target**: reference profile의 목표 동작입니다. Staged plan이 승격하기 전까지 later-profile, optional, diagnostic, future 범위일 수 있습니다.

Request 또는 response schema에서 required로 표시된 field는 해당 tool, record, profile이 구현되고 사용될 때 schema-required입니다. 이 field들이 모두 v0.1 Core Authority Slice exit criteria라는 뜻은 아닙니다. 단계별 전달 계획은 Build가 담당합니다. Stage scope는 [Build: MVP 계획](../build/mvp-plan.md#계약-필드-단계-구분)과 [첫 실행 가능한 조각](../build/first-runnable-slice.md)을 사용합니다. 이 문서가 schema block 또는 branch rule 안에서 별도 수식 없이 "required"라고 말하면 stage-required가 아니라 schema-required를 뜻합니다.

이 reference는 tool 또는 profile이 범위에 들어왔을 때의 valid API shape를 담당합니다. 예를 들어 어느 stage든 Decision Packet을 만들면 여기서 required인 Decision Packet field가 그대로 적용됩니다. Stage가 full user-facing quality를 미룰 수는 있지만 invalid public shape를 낼 수는 없습니다.

Tool section의 "Projection jobs enqueued" 문구는 해당 projection kind support가 범위에 있고 committed non-dry-run behavior가 일어날 때를 설명합니다. Dry run, pre-commit state conflict, rejected pre-commit request는 projection job을 enqueue하지 않습니다. v0.1은 status/blocker output을 반환할 수 있고, owner path가 이미 만든 경우에만 freshness/read fact를 보존할 수 있습니다. Full Markdown rendering이나 여러 projection kind는 v0.1 요구사항이 아닙니다. Projection은 파생 view이며 authority가 아닙니다.

v0.1 Core Authority Slice에서는 이 API를 하나의 authority loop에 필요한 최소 subset으로 읽습니다. Status/blocker read, owner-valid path를 통한 local project와 Task setup, scoped work boundary 하나, `prepare_write`, single-use Write Authorization 하나, compatible `record_run` 하나, artifact/evidence ref 하나, structured blocker/status response가 범위입니다. Profile별 Decision Packet 품질, full Evidence Manifest, 수동 QA, 분리 검증, 작업 수락, 잔여 위험 수용, recover/export, broad operator flow, rich projection rendering을 위한 tool과 response 의미는 future-profile capability이지 v0.1 exit criteria가 아닙니다.

API surface는 stage가 모든 범주를 요구하지 않을 때에도 close-support 범주를 분리해 보존해야 합니다. 근거, 검증, 수동 QA, 작업 수락, 잔여 위험 표시, 잔여 위험 수용은 서로 다른 response 의미입니다. Status, next-action, decision, close response는 어떤 범주가 `not_required`라고 말할 수 있지만, test pass, Eval verdict, QA waiver, 사용자의 작업 수락 note, 수용된 잔여 위험을 다른 범주의 generic substitute로 쓰면 안 됩니다.

사용자에게는 다음처럼 설명합니다.

- `harness.status`는 "지금 어디에 있고, 무엇이 중요하며, 표시가 최신인가?"에 답합니다.
- `harness.next`는 "다음 안전한 행동 또는 가장 작은 해소 방법은 무엇인가?"에 답합니다.
- `harness.prepare_write`는 "현재 active Change Unit, Autonomy Boundary, baseline, 민감 동작 승인, Decision Packet, design-policy, capability check 아래에서 지금 이 정확한 제품 파일 쓰기를 해도 되는가?"에 답합니다.
- `harness.record_run`은 "무슨 일이 일어났고, 하나의 implementation/direct Run을 위해 어떤 compatible Write Authorization이 consume되었으며, 무엇이 바뀌었고 어떤 근거 또는 artifact가 기록되었는가?"에 답합니다.
...
|---|---|---|
| `ToolEnvelope`, `ToolResponseBase`, `ToolError`, `StateSummary`, `EventRef` | v0.1 Core Authority Slice | 공통 contract shape입니다. Required field는 이 object를 사용하는 모든 emitted 또는 accepted payload에서 schema-required입니다. |
...
Decision Packet 품질은 아래 public field, `decision_profile`, [Decision Packet](kernel.md#decision-packet), [Decision Gate](kernel.md#decision-gate)의 kernel authority rule로 판단하며, private 표시 label이나 추가 권한 hint로 판단하지 않습니다. 충분한 packet은 `what_user_is_deciding`에 사용자가 무엇을 결정하는지 이름 붙이고, schema가 소유하는 `judgment_domain`을 포함하며, lifecycle route를 `decision_kind`에 기록하고, prompt 깊이를 `decision_profile`에 기록하고, relevant scope와 state/artifact ref를 식별하며, chosen outcome 또는 pending option label을 보여줍니다. 상세 option trade-off, recommendation, uncertainty, deferral consequence, affected acceptance criteria, user context, expiry, approval scope, reconcile target은 profile별 품질 field입니다. 선택한 profile이 요구하는 field로 선택, source, evidence, impact, agent latitude를 솔직히 보여줄 수 없다면 broad approval처럼 제시하지 말고 prompt를 blocked 또는 narrowed로 다루거나 충분한 context가 생길 때까지 decision을 pending으로 둬야 합니다.

`decision_kind`, `decision_profile`, `judgment_domain`은 서로 다른 schema 소유 field입니다. `decision_kind`는 lifecycle, 사용자가 기록하는 답변 route, resolution payload branch, gate 의미, state transition semantics를 제어합니다. `decision_profile`은 질문 깊이, profile별 필수 정보, Decision Packet `profile_payload` branch를 고릅니다. `judgment_domain`은 사용자에게 결정을 어떻게 설명하고 묶어 보여줄지 정하는 판단 영역입니다. `affected_gates`는 그 결정이 어떤 gate나 막힌 행동에 영향을 줄 수 있는지 기록하는 별도 field입니다. 별도 owner rule이 명시하지 않는 한 `judgment_domain`이나 `decision_profile`은 close gate aggregation, Approval behavior, waiver behavior, 잔여 위험 수용, 권한 경로를 직접 override하면 안 됩니다. 하나의 결정은 표시되는 판단 영역과 prompt profile과 별개로 하나 이상의 gate에 영향을 줄 수 있으며, 여러 영역에 걸친 결정은 `mixed`를 쓰거나 options, affected gates, risk, evidence, follow-up에 부차적인 고려사항을 보여줘야 합니다.
...
| --- | --- | --- |
| `minimal_decision` | full trade-off object가 결정 자체보다 무거운 간단한 제품, 범위, 기술 unblocker. | 공통 field, 간결한 pending option label, 별도 option list가 중요하지 않을 때의 명시적 empty options list, 또는 chosen outcome, 그리고 relevant scope와 related state/artifact refs. 상세 pros/cons, recommendation, uncertainty, deferral consequence는 중요하지 않으면 empty일 수 있고, schema가 `null`을 허용하는 곳에서는 `null`일 수 있습니다. |
| `product_ux_tradeoff` | 사용자 영향이 의미 있는 제품, UX, 문구, 접근성, workflow 선택. | benefits/costs/risks가 있는 하나 이상의 상세 options, recommendation 또는 recommendation이 없는 이유, uncertainty, deferral consequence, 필요한 경우 affected acceptance criteria, 관련 evidence 또는 design refs. |
| `architecture_tradeoff` | 아키텍처, dependency, API/interface, migration, storage, 보안 민감 기술 방향. | 하나 이상의 상세 options와 trade-off, compatibility와 rollback/risk notes, recommendation 또는 recommendation이 없는 명시적 이유, uncertainty, deferral consequence, affected gates/criteria, source/evidence refs. |
...
| `reconcile` | managed drift, proposal/state mismatch, generated/projection edit에 대한 사용자 결정. | `decision_kind=reconcile`, `reconcile_item_id`, reconcile target, 하나 이상의 candidate outcome options, reconcile되는 state/projection refs. |
...
| `decision_kind` | 허용 또는 기본 `decision_profile` |
| --- | --- |
| `approval` | 반드시 `approval_shaped`. |
| `scope_confirmation` | 기본은 `minimal_decision`; 같은 기록된 결정이 실제로 여러 고려사항을 함께 다룰 때만 `mixed`. |
| `design_choice` | 중요도에 따라 보통 `product_ux_tradeoff` 또는 `minimal_decision`; 하나의 결정이 실제로 여러 영역을 함께 다룰 때만 `mixed`. |
| `architecture_choice` | 보통 `architecture_tradeoff`; 사소하고 범위가 제한된 기술 unblocker에만 `minimal_decision`; 하나의 결정이 실제로 여러 영역을 함께 다룰 때만 `mixed`. |
| `product_tradeoff` | 보통 `product_ux_tradeoff`; 사소하고 범위가 제한된 제품 또는 문구 선택에만 `minimal_decision`; 하나의 결정이 실제로 여러 영역을 함께 다룰 때만 `mixed`. |
| `autonomy_boundary` | 보통 `minimal_decision`; 같은 기록된 결정 안에 scope/autonomy 복잡성과 다른 중요한 고려사항이 함께 있을 때 `mixed`. |
| `verification_waiver` | 반드시 `waiver`. |
| `qa_waiver` | 반드시 `waiver`. |
| `acceptance` | 반드시 `acceptance`. |
...
`decision_kind=approval`은 stable public enum value로 유지됩니다. `DecisionPacket`과 `DecisionPacketCandidate` 모두에서 이 값은 `decision_profile=approval_shaped`를 사용해야 하며, 민감 동작 승인만을 위한 Approval 형태의 judgment context를 뜻합니다. 제품 장단점, 설계 방향, 아키텍처 판단이나 기술 구조 판단, 해결되지 않은 security 또는 product-security 판단, QA 면제 판단, 검증 면제 판단, 검증 위험, 작업 수락, 잔여 위험 수용 같은 사용자 소유 판단은 별도의 compatible Decision Packets와 gate updates로 표현되지 않는 한 이 값으로 해소할 수 없습니다.
...
| `NO_ACTIVE_CHANGE_UNIT`, `SCOPE_REQUIRED`, `SCOPE_VIOLATION`, `AUTONOMY_BOUNDARY_EXCEEDED`, `BASELINE_STALE` | 범위, 사용자 결정 경계 또는 baseline 문제 | Scope를 확인하거나 줄이고, Change Unit 또는 baseline을 갱신하거나, 진행 전에 필요한 Decision Packet을 요청합니다. |
| `DECISION_REQUIRED`, `DECISION_UNRESOLVED` | 사용자 판단 필요 | 관련 Decision Packet 또는 decision prompt를 profile에 맞는 options 또는 chosen outcome, refs, required일 때 full-profile recommendation, uncertainty, deferral effect와 함께 보여줍니다. |
...
| 10 | `APPROVAL_DENIED` | relevant 민감 동작 승인이 denied됨 |
| 11 | `APPROVAL_EXPIRED` | relevant 민감 동작 승인이 expired되었거나 scope 또는 baseline에서 drift됨 |
| 12 | `APPROVAL_REQUIRED` | sensitive change에 민감 동작 승인이 필요하지만 compatible granted 민감 동작 승인이 없음 |
...
| 17 | `EVIDENCE_INSUFFICIENT` | required 근거 coverage가 absent, partial, `stale`, 또는 blocked임 |
| 18 | `VERIFY_NOT_DETACHED` | verification이 분리 검증으로 count될 수 없음 |
| 19 | `QA_REQUIRED` | required 수동 QA가 pending, failed, missing, 또는 validly waived되지 않음 |
| 20 | `RESIDUAL_RISK_NOT_VISIBLE` | known close-relevant 잔여 위험이 작업 수락 또는 close 전에 visible하지 않음. `ResidualRiskSummary.status=none`이 no known close-relevant risk를 confirm한 경우에는 선택하지 않음 |
| 21 | `ACCEPTANCE_REQUIRED` | 잔여 위험 표시가 satisfied된 뒤에도 required 작업 수락이 pending 또는 rejected임 |
...
Public method는 staged surface별로 묶어 읽습니다. 같은 method가 later profile에서 payload 의미를 확장하면 여러 stage에 나타날 수 있습니다. 그렇다고 later profile이 earlier stage에서 stage-required가 되는 것은 아닙니다. 아래 tool section은 method/profile이 범위에 들어왔을 때의 정확한 request/response contract입니다.

#### v0.1 Core Authority Slice surface

| Method 또는 capability | Stage requirement | Scope boundary |
|---|---|---|
| `harness.status` | Stage-required minimal status/blocker read. | Current Core state, write-authority summary, structured blocker를 반환하며, future-profile 값은 schema가 허용하는 `null`, empty array, `unknown`, `not_required`로 표현합니다. Projection rendering은 요구하지 않습니다. |
| `harness.prepare_write` | Stage-required product-write authorization check. | 하나의 allowed/blocked path와, allowed일 때 durable single-use Write Authorization 하나를 만듭니다. Approval-shaped Decision Packet과 Approval record lifecycle은 later-profile입니다. |
| `harness.record_run` | Stage-required Run recording and Write Authorization consumption. | Compatible implementation/direct Run 하나를 기록하고 authorization 하나를 한 번 consume하며, owner path를 통해 artifact/evidence ref 하나를 등록합니다. Full Evidence Manifest, TDD, Eval, Manual QA update는 later-profile입니다. |
| `harness.intake` 또는 owner-valid setup path | Stage-required setup capability이지만 v0.1에서 method 자체는 optional입니다. | v0.1은 local project, Task, scoped work boundary가 필요합니다. 이 public method를 사용한다면 minimal setup shape만 사용하고, full natural-language intake/discovery는 v0.2입니다. |
...
| Method 또는 capability | Stage requirement | Scope boundary |
|---|---|---|
| `harness.launch_verify`와 `harness.record_eval` | Detached verification profile이 enabled될 때 stage-required입니다. | Launch는 detached candidate 또는 bundle을 만들 뿐입니다. Recorded qualifying Eval만 verification/assurance를 update할 수 있습니다. |
| `harness.record_manual_qa` | Manual QA profile이 enabled될 때 stage-required입니다. | Human QA judgment와 valid waiver path는 screenshot, browser capture, final acceptance, verification과 구분됩니다. |
| `harness.request_user_decision`과 `harness.record_user_decision` assurance profiles | Approval-shaped decision, QA/verification waiver, full residual-risk acceptance, final acceptance separation이 enabled될 때 stage-required입니다. | Approval은 Write Authorization이 아닙니다. Waiver와 accepted risk는 detached verification 또는 generic acceptance를 만들지 않습니다. |
| `harness.record_run` assurance payloads | Evidence manifest, feedback-loop, TDD profile이 enabled될 때 stage-required입니다. | Rich evidence, feedback-loop, TDD record는 해당 enabled profile에서만 schema-required입니다. |
| `ValidatorResult` emitting paths | Active profile이 enable한 agency assurance validators에 대해 stage-required입니다. | Core check는 별도로 block할 수 있습니다. 나열된 stable validator ID만 validator result입니다. |

#### v0.4 Operations surface

| Method 또는 capability | Stage requirement | Scope boundary |
|---|---|---|
| `harness.status`, `harness.next`, `harness.close_task`, response `projection_jobs`의 projection freshness | Operations projection support가 enabled될 때만 stage-required입니다. | Projection job과 ref는 Core state에서 파생된 view입니다. Evidence, acceptance, risk acceptance, close authority, write authority를 만들지 않습니다. |
| `harness.request_user_decision` / `harness.record_user_decision`의 Reconcile profile | Reconcile storage/operations support가 enabled될 때만 stage-required입니다. | Reconcile candidate는 existing Decision Packet/owner path가 outcome을 commit하기 전까지 state가 되지 않습니다. |
...
| [`harness.record_user_decision`](#harnessrecord_user_decision) | Decision Packet, 민감 동작 승인, 면제 판단, 작업 수락, 잔여 위험 decision 해결 |
...
|---|---|---|
| `ask_user` | 이름 붙은 경로를 계속하기 전에 사용자 소유 결정, 민감 동작 승인, QA 판단, 작업 수락, 잔여 위험 수용, 또는 다른 답변이 필요합니다. | Prompt는 사용자가 무엇을 결정하는지와 관련 refs를 말해야 하며, 그 자체로 write나 close를 허가하지 않습니다. |
...
`RecommendedPlaybook.route`, `decision_packet_route`, 선택적 구현 `decision_requests` 같은 routing metadata는 그 자체로 authority가 아닙니다. Caller를 existing Decision Packet 또는 owner-record path로 안내할 수는 있지만 민감 동작 승인, 사용자 소유 판단, 작업 수락, 면제 판단, 잔여 위험 수용, Write Authorization, close를 충족할 수 없습니다.
...
Stage/profile: v0.1 core authority loop에서 stage-required입니다. Approval candidate는 Approval state를 만들지 않고 반환할 수 있습니다. Committed Approval record와 approval-shaped Decision Packet lifecycle은 later Approval/Agency Assurance profile과 matching storage가 필요합니다.
...
Stage/profile: v0.1 stage-required가 아닙니다. v0.2는 user-owned judgment 또는 final acceptance가 progress나 close를 막을 때 사용자 대상 Decision Packet prompt를 도입합니다. Approval-shaped decision, QA/verification waiver, full residual-risk acceptance, reconcile은 matching v0.3/v0.4 owner profile과 storage가 범위에 있을 때만 enabled됩니다. `decision_profile`, `judgment_domain` 같은 field는 packet 또는 candidate가 만들어질 때 schema-required이지 v0.1 requirement가 아닙니다.
...
상태 전이 요약: pending Decision Packet을 record하고 보통 Task를 `waiting_user`로 옮깁니다. 사용자 소유 제품 장단점 판단 또는 중요한 기술/아키텍처 선택 같은 `decision_gate` 대상 판단은 `decision_gate=pending`을 set합니다. 민감 동작 승인 요청은 pending Approval 기록을 생성하고 `approval_gate=pending`을 set하며, scope confirmation은 `scope_gate=pending`을 set합니다. 작업 수락은 required일 때 `acceptance_gate=pending`을 set하거나 유지합니다. 잔여 위험 수용은 별도의 `decision_kind=residual_risk_acceptance` 경로를 사용하며, 이름 붙고 표시된 risk에 대한 사용자 수용이 아직 필요하면 `residual_risk_acceptance` close blocker category를 사용합니다.
...
Projection job 대기열 추가: source record를 변경하는 committed non-dry-run Decision Packet creation에서는 Task projection support가 범위에 있으면 `TASK`; Core가 기준 Approval 형태 Decision Packet과 연결된 pending Approval 기록을 만든 뒤 `decision_kind=approval`이고 APR projection support가 범위에 있을 때만 `APR`; reconcile/projection support가 범위에 있을 때만 affected reconcile projection. Dry-run, pre-commit state-conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다.

위 committed non-dry-run path에서 standalone Decision Packet projection이 켜져 있을 때만 optional `DEC` job을 대기열에 넣습니다.
...
Purpose: pending Decision Packet에 대한 user's answer를 record하고 optional accepted 잔여 위험을 기록합니다.

Stage/profile: v0.1 stage-required가 아닙니다. v0.2는 user-owned decision과 final acceptance가 MVP scope에 있을 때 이를 기록합니다. Approval grant/deny, waiver, residual-risk acceptance metadata, reconcile outcome은 later owner profile이 필요합니다. 이 method는 existing canonical Decision Packet에 대한 사용자 답을 기록하며 broad approval이나 write authority를 만들지 않습니다.

Allowed actor: `user`, `operator`.

이 request는 existing canonical Decision Packet에 답하는 것입니다. Caller가 저장된 값을 바꾸는 replacement `decision_profile`을 넘길 수 없습니다. Core는 stored packet의 `decision_kind`, `decision_profile`, status, refs, 선택된 `profile_payload` branch, 그리고 `decision_kind`가 고른 `RecordUserDecisionPayload` branch에 맞춰 answer를 validate합니다.
...
RecordUserDecisionPayload:
  # 정확히 하나의 branch만 present이고, branch는 decision_kind와 일치해야 한다.
  one_of:
    approval:
...
`RecordUserDecisionPayload.one_of`는 recorded answer의 resolution payload branch object입니다. 나열된 branch 중 정확히 하나만 present이고, selected branch는 `decision_kind`와 일치해야 하며, 선택되지 않은 모든 branch는 absent입니다. `decision_profile`은 Decision Packet의 질문/profile `profile_payload` branch를 고르며, 이 resolution branch를 고르지 않습니다. `judgment_domain`과 `decision_profile`은 저장된 Decision Packet에서 읽어 표시, grouping, recorded answer validation에 쓰며, resolution branch로 사용하지 않습니다. 선택된 resolution branch, selected option, compatible Decision Packet이 기록되는 judgment를 결정합니다. "approved", "go ahead", "proceed", "looks good", "좋아", "진행해" 같은 자유로운 note text는 이를 민감 동작 승인, 작업 수락, 면제 판단, 잔여 위험 수용, write authority로 넓힐 수 없습니다. `accepted_risks`는 Decision Packet과 current Judgment Context가 user decision 전에 close-relevant 잔여 위험을 보이게 만든 경우에만 allowed입니다. `decision_kind=acceptance`에서 Core는 close-relevant 잔여 위험이 보이거나 `ResidualRiskSummary.status=none`이 no known close-relevant risk를 confirm한 경우에만 작업 수락을 record할 수 있습니다. 작업 수락은 사용자의 result judgment를 기록합니다. 민감 동작을 승인하거나, QA 또는 검증을 면제하거나, 잔여 위험 수용 route가 식별된 위험을 함께 기록하지 않는 한 known risk를 accepted로 표시하지 않습니다. Core는 `decision_packet_id`가 식별하는 기준 `DecisionPacket`에 answer를 record합니다. 모든 `decision_requests` row는 routing/replay metadata로만 update되며 linked compatible Decision Packet과 owner-record update 없이는 `decision_gate`, 민감 동작 승인, 작업 수락, 면제 판단, 잔여 위험 수용, close를 충족할 수 없습니다. Core는 Residual Risk record를 update하고 residual-risk 상태 참조를 반환하여 accepted-risk metadata를 기록하며, 잔여 위험 수용을 분리 검증으로 취급하지 않습니다. `AcceptedRiskInput.residual_risk_ref=null`은 current Decision Packet과 Judgment Context가 해당 close-relevant risk를 이미 사용자에게 보이게 만들고, Core가 같은 기록된 transition 안에서 Residual Risk record를 생성하거나 associate할 수 있을 만큼 충분한 source/evidence context를 포함할 때만 allowed입니다. Visibility 또는 context가 없으면 Core는 hidden risk를 조용히 create하고 accept하지 말고 reject 또는 block해야 합니다.

`decision_kind=verification_waiver`에서 `decision.verification_waiver.value=waived`는 위 Decision Packet과 Residual Risk path를 통해서만 수용된 검증 위험을 기록합니다. 그 risk가 close-relevant라면 `accepted_risks`는 나중의 `completed_with_risk_accepted` close에 필요한 visible Residual Risk refs를 식별하거나 만들 수 있어야 합니다. Verification-waiver decision은 assurance를 update하면 안 되고, 분리 검증을 만들면 안 되며, `completed_verified` close를 허용하면 안 됩니다.
...
Projection job 대기열 추가: source record를 변경하는 committed non-dry-run 사용자 소유 결정 record에서는 Task projection support가 범위에 있으면 `TASK`; targeted Decision Packet이 Approval 형태이고 연결된 Approval 기록이 update되며 APR projection support가 범위에 있을 때 `APR`; QA 면제가 QA 기록으로 represented되고 수동 QA projection support가 범위에 있을 때 `MANUAL-QA`; reconcile/projection support가 범위에 있을 때만 affected design/task projection. Dry-run, pre-commit state-conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다. Decision Packet visibility는 여전히 `TASK` projection, status/next response, judgment-context resource, decision-packet resource를 통해 나타납니다.
...
Stage/profile: Manual QA policy/profile이 enabled된 v0.3 Agency Assurance behavior입니다. v0.1 stage-required가 아니며, browser capture나 note는 human QA record 또는 valid waiver path를 대체하지 않습니다.
...
Stage/profile: v0.1은 implementation이 그 경로를 선택한 경우 narrow structured blocker/status smoke로만 사용할 수 있습니다. v0.2는 user-facing close readiness와 blocker display를 요구하며, 민감 동작 승인, 작업 수락 분리, 잔여 위험 표시, 잔여 위험 수용은 관련 있을 때 그 안에 나타납니다. v0.3은 full assurance, QA, waiver, accepted-risk close semantics를 enabled합니다. v0.4는 enabled Operations profile에서 projection/report freshness blocker를 추가할 수 있습니다.
...
`CloseTaskRequest`는 accepted-risk refs를 전달하지 않습니다. `completed_with_risk_accepted`에서는 Core가 close-relevant Residual Risk records에 이미 기록된 수용 상태를 읽으며, visible accepted residual-risk 상태가 없으면 block합니다. `verification_gate=waived_by_user`이면 completion은 `completed_with_risk_accepted`를 request해야 합니다. Waiver는 분리 검증이 아니므로 `completed_verified`는 계속 blocked입니다.
...
Close blockers에는 해소되지 않았거나 missing, deferred-without-coverage, blocked, rejected, `stale`, incompatible 상태인 blocking Decision Packets와, successful close 전에 표시되지 않은 known close-relevant 잔여 위험이 포함됩니다. Known close-relevant 잔여 위험이 없으면 `ResidualRiskSummary.status=none`이 잔여 위험 표시를 충족하며 close blocker가 아닙니다. 잔여 위험 수용 close에는 표시되고 수용된 Residual Risk refs가 추가로 필요합니다. 작업 수락이 required인 경우 close-relevant 잔여 위험이 표시되거나 `ResidualRiskSummary.status=none`으로 confirmed된 뒤에만 작업 수락을 record할 수 있습니다.

Close-relevant risk가 표시되었지만 수용되지 않았고 requested close path가 잔여 위험 수용을 요구하면 blocker category는 `residual_risk_acceptance`입니다. 잔여 위험 수용 decision이 missing이면 `DECISION_REQUIRED`, existing 잔여 위험 수용 Decision Packet이 unresolved이면 `DECISION_UNRESOLVED`를 blocker code로 사용합니다.

`requested_close_reason=completed_verified`의 blockers에는 same-session review, invalid 또는 insufficient independence, stale evaluator bundle, baseline drift, missing 또는 failed-integrity reviewed artifacts, 검증 면제 판단이 포함됩니다. `requested_close_reason=completed_with_risk_accepted`의 blockers에는 waived 또는 remaining 검증 위험에 대한 visible accepted Residual Risk refs 누락이 포함됩니다.
...
`CloseBlockerCategory`는 `CloseTaskResponse.blockers[].category`의 안정적인 public category value set입니다. 값은 `open_run`, `scope`, `user_decision`, `sensitive_action_approval`, `design_policy`, `evidence`, `verification`, `manual_qa`, `residual_risk_visibility`, `residual_risk_acceptance`, `final_acceptance`, `projection_freshness`, `artifact_availability`입니다. 사용자 표시에서는 이 값에 자연스러운 label을 붙일 수 있지만, primary `ErrorCode`는 secondary blocker를 합치지 않으며, 여러 범주가 unresolved인데 `closed=false`를 하나의 일반적인 "not done" 상태로 설명하면 안 됩니다.
...
Projection job 대기열 추가: committed non-dry-run successful close 또는 close-blocker state change에서는 task-summary projection support가 범위에 있으면 `TASK`; active projection profile이 해당 reports를 포함할 때만 final freshness에 필요한 latest profile-required reports. Dry-run, pre-commit state-conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다. Projection freshness는 final report/export freshness requirement를 막을 수 있지만, projection text가 Core close state 또는 structured close blocker를 대체하지는 않습니다.
