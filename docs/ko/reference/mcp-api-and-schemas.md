...
- `one_of:`는 branch object를 정의합니다. 나열된 child field 중 정확히 하나만 present이고, 그 value는 해당 child field의 stated type과 일치해야 합니다. Branch prose는 selected branch가 `judgment_type` 또는 profile-specific discriminator field와 일치해야 한다고 추가로 요구할 수 있습니다. 선택되지 않은 모든 branch는 `null`로 present가 아니라 absent여야 합니다.
...
Harness는 로컬 권한 기록이자 사용자 판단 라우팅 계층입니다. Core-owned local state가 기준 권한입니다. Chat, Markdown 읽기용 요약(Projection), connector output, tool output은 권한이 아닙니다.

Shared schema를 읽기 전에 이 manifest를 먼저 봅니다. 내부 엔지니어링 점검 / MVP-1 구현 경로는 아래 active API surface와 [ArtifactRef](#artifactref)의 stage-specific value set만 사용합니다. Later-profile method detail과 future schema appendix는 정확한 참조 shape를 보존하지만 더 이른 stage의 requirement를 늘리지 않습니다.

| Stage/profile | Active API slice | 해당 slice에서 active가 아닌 것 |
|---|---|---|
| 내부 엔지니어링 점검 | Current status/state read, owner-valid setup path 하나, `harness.prepare_write`, compatible `harness.record_run` 하나, artifact/evidence ref 하나, structured status/blocker response. Optional smoke read는 `harness.next` 또는 좁은 `harness.close_task` blocker response를 쓸 수 있습니다. | Full natural-language intake, stored user-judgment packet, full Evidence Manifest, detached verification, Manual QA, work acceptance, residual-risk acceptance, rich projection, export/recover, advanced connector, operations surface. |
| MVP-1 사용자 작업 루프 | Ordinary-language intake, next-safe-action output이 있는 status, `harness.prepare_write`, `harness.record_run`, `harness.request_user_judgment`, `harness.record_user_judgment`, blocker summary가 있는 `harness.close_task`. | Detached verification launch/eval, full Manual QA matrix, user-facing approval route를 넘는 approval hardening, export/recover, advanced connector API, broad operations, detailed diagnostic projection. |
| 보증 프로필 이후 Later profiles/future | Assurance, verification, Manual QA, waiver, full residual-risk acceptance, reconcile, validator emission, projection/report/export/recover, operations, advanced connector profile. Owner doc이 승격할 때만 active입니다. | 내부 엔지니어링 점검 smoke 또는 minimum MVP-1 user-value slice가 아닙니다. |

### First Implementable Calls

내부 엔지니어링 점검 implementer는 이 set 안에 머물 수 있어야 합니다.

1. `harness.status` 또는 내부 엔지니어링 점검 read-only resource로 status를 읽습니다.
...
MVP-1 implementer는 later assurance/operations를 떠안지 않고 user value를 추가합니다.
...
4. Committed `user_judgment`에 대한 사용자 답은 `harness.record_user_judgment`로 기록합니다.
...
| Method or capability | 내부 엔지니어링 점검 | MVP-1 | 보증 프로필 이후 / later profiles |
|---|---|---|---|
...
| `harness.next` | Optional smoke read. | Optional expanded/compatibility read입니다. 작업 수락이 progress나 close를 막을 때 `request_acceptance`를 반환할 수 있고, preferred MVP-1 path는 `harness.status.next_actions`입니다. | Verification, Eval, Manual QA, reconcile, operations, profile-expanded work-acceptance 또는 residual-risk next-action context는 owner profile이 active일 때만 포함합니다. |
...
| `harness.request_user_judgment` | Not active. | 사용자 소유 판단, 민감 동작 승인, work-acceptance prompt에 active. | Committed Approval record, approval hardening, waiver, full residual-risk acceptance, reconcile profile을 추가할 수 있습니다. |
| `harness.record_user_judgment` | Not active. | 민감 동작 승인 판단을 포함해 `user_judgment`에 대한 사용자 답 기록에 active. | Committed Approval lifecycle, waiver, risk-acceptance, reconcile record update를 추가할 수 있습니다. |
...
| `harness.launch_verify`, `harness.record_eval`, `harness.record_manual_qa` | Not active. | 기본 MVP-1에는 not active. | 보증 프로필 또는 later owner profile에서만 active입니다. |
...
| Field 또는 schema family | 내부 엔지니어링 점검 | MVP-1 | 보증 프로필 이후 / later profiles |
|---|---|---|---|
...
| `UserJudgment`, `JudgmentContext`, `judgment_type`/`presentation`/`display_label` fields | Owner path가 이미 가진 seeded/display ref를 제외하면 inactive. | User-facing judgment에 active. | Approval, waiver, risk, reconcile profile이 확장합니다. |
| `approval_request_candidate`, `approval_refs`, `ApprovalScope` | `prepare_write`가 approval을 요구할 때 candidate display는 가능하지만 committed Approval lifecycle은 inactive. | `ApprovalScope`는 민감 동작 승인 `user_judgment`에서 active입니다. `approval_refs`는 Approval owner profile이 켜지기 전까지 비어 있습니다. | Committed Approval record, `approval_refs`, hardened approval/profile behavior. |
...
Preferred MVP-1 public method name은 `harness.request_user_judgment`와 `harness.record_user_judgment`입니다. 기존 `harness.request_user_decision`과 `harness.record_user_decision`은 compatibility alias로만 남습니다. Alias는 one-to-one mapping입니다. 추가 method, state path, authority를 만들지 않습니다.

`UserJudgment` / `user_judgment`가 canonical public record family입니다. "Decision Packet"은 복잡한 판단을 위한 full-format presentation 또는 legacy label입니다. 기본 사용자 표시 메커니즘도, 별도 authority path도 아닙니다.

`harness.status.next_actions`가 preferred MVP-1 next-safe-action output입니다. `harness.next`는 expanded next-action payload와 기존 문서 compatibility를 위한 optional read method입니다. MVP-1 implementation은 `harness.status.next_actions`만으로도 minimum next-action requirement를 만족할 수 있습니다. 단 caller가 next safe action과 smallest unblocker를 찾을 수 있어야 합니다.
...
내부 엔지니어링 점검 resource subset은 의도적으로 작습니다. 첫 authority loop에 필요한 current project, active/current Task, current state와 blocker, write-authority status, Core가 chat memory나 generated Markdown보다 권한 있는 기록임을 보여 주는 Run/artifact/evidence ref만 지원합니다.
...
내부 엔지니어링 점검은 Journey, Journey Spine, full user judgment storage/presentation, Evidence Manifest, bundle, design map, module map, interface contract, report, projection job, projection renderer를 요구하지 않습니다.

### MVP-1 사용자 작업 루프 resources

MVP-1은 내부 엔지니어링 점검 current-status resource를 유지하고, 일반 사용자가 현재 작업 상태, 대기 중인 사용자 판단, 근거 요약, 닫기 준비 상태, 닫기 막힘, 관련될 때의 작업 수락 필요/상태와 잔여 위험 표시를 이해할 수 있게 그 summary를 확장할 수 있습니다. Minimum MVP-1 path에는 detailed report resource가 필요하지 않습니다.

| Resource | Profile 의미 |
|---|---|
| `harness://task/{task_id}/user-judgments` | 해당 Task의 active, resolved, deferred, blocked `user_judgment` summary입니다. 사용자 판단 표시를 위해 사용합니다. |
...
`harness://task/{task_id}/decision-packets`는 legacy full-format label을 쓰는 client를 위한 compatibility alias로 남을 수 있습니다. 새 client는 `user-judgments`를 우선해야 합니다.

MVP-1의 근거와 닫기 준비 상태 resource path는 current Core state와 ref에서 파생된 `harness://task/{task_id}/summary`, `harness://status/card`, `harness.status`, 또는 `harness.next`로 충족할 수 있습니다. `harness://task/{task_id}/evidence-manifest`를 요구하지 않습니다.

### 보증 프로필 resources

이 resource들은 profile-gated assurance read입니다. 첫 조각이나 minimum MVP-1 사용자 작업 루프 요구사항이 아닙니다.
...
| `harness://task/{task_id}/evidence-manifest` | Assurance/evidence profile이 켜졌을 때 evidence coverage와 manifest-oriented read입니다. MVP-1 evidence summary에는 필요하지 않으며, detailed manifest projection body는 pull-on-demand 또는 diagnostic 범위에 남습니다. |
...
이 resource들은 projection-oriented, design/reference, 또는 diagnostic read입니다. Owner가 승격한 profile을 통해서만 켜거나 진단을 위해 필요할 때만 가져옵니다. 내부 엔지니어링 점검 또는 minimum MVP-1 요구사항으로 취급하면 안 됩니다.
...
Public MCP 계약의 내부 엔지니어링 점검/default reference posture는 등록된 project surface에 대한 local-only 노출입니다. Local-only란 기대되는 local user/profile에 대한 로컬 프로세스, 로컬 socket, 또는 localhost loopback 연결을 뜻합니다. 인증되지 않은 shared endpoint, non-loopback bind, forwarded/tunneled endpoint, cloud/CI relay, cross-user socket 또는 directory, 등록된 connector profile로 설명되지 않는 remote caller는 제외됩니다. MCP server를 이 로컬 경계 밖에 노출하면 위협 모델이 달라지며, owner documentation과 conformance가 특정 connector posture를 승격하기 전까지 내부 엔지니어링 점검/default reference posture 밖에 남습니다. 그런 더 강한 profile이 없다면 MCP endpoint에 닿을 수 있는 호출자도 Core가 검증해야 하는 claim을 보낸 출처일 뿐, 자동으로 신뢰되는 권한이 아닙니다.
...
- `actor_kind`는 routing과 policy check를 위한 claimed actor role입니다. 그 자체만으로 민감 동작 승인, 작업 수락, user judgment resolution, 수동 QA judgment, 분리 검증 독립성를 충족하면 안 됩니다.
...
지원 계층 label은 enum value가 아닙니다. 내부 엔지니어링 점검에는 owner path가 이미 만든 freshness/read fact를 보존하는 것 외의 projection rendering exit requirement가 없습니다. MVP-1 사용자 작업 루프는 사용자가 현재 작업, 사용자 판단, 근거, 닫기 막힘을 이해할 만큼의 파생 output을 제공하지만 broad template polish를 요구하지 않습니다. 작업 수락과 잔여 위험 사실은 관련 있을 때 distinct하게 남지만 필수 projection kind를 늘리지는 않습니다. Agency assurance와 operations/export support는 later profile class입니다. Future/diagnostic projections는 자동으로 로드맵 전용이라는 뜻은 아니며 owner가 승격한 profile에서 켤 수 있지만, 내부 엔지니어링 점검이나 최소 MVP-1 요구사항은 아닙니다.

ProjectionKind extensibility가 projection을 기준 상태로 만들지는 않습니다. 모든 projection job은 여전히 owner 기록 및 아티팩트 참조에서 파생된 보기를 렌더링합니다. 어떤 ProjectionKind 지원 계층도 state, 근거, 수동 QA, 검증, 작업 수락, 잔여 위험 수용, close authority, Write Authorization을 만들지 않습니다. `DEC`는 해당 기능이 켜졌을 때 standalone full-format Decision Packet Markdown에만 유효합니다. Standalone `DEC` job이 없어도 active stage/profile이 요구하는 user judgment visibility가 줄어들면 안 되며, 이 visibility는 status/next responses, judgment-context resources, user-judgment resources, 최소 `TASK` 또는 card display를 통해 제공되어야 합니다. 첫 사용자 가치 조각에 필요한 것은 standalone `DEC` `ProjectionKind`가 아니라 판단 요청 display shape입니다. Persisted `JOURNEY-CARD` Markdown과 Journey Spine-style output은 future/diagnostic 범위입니다. `harness.status`, `harness.next`, significant resume flow의 현재 위치 맥락은 간결한 상태 출력으로 충족할 수 있습니다.
...
주요 schema object는 담당 section과 아래 표를 통해 stage/profile 의미를 가집니다. `active_from/profile` label은 문서 metadata일 뿐 payload field나 새 schema member가 아닙니다.

| Schema object 또는 family | active_from/profile | Activation 의미 |
|---|---|---|
| `ToolEnvelope`, `ToolResponseBase`, `ToolError`, `StateSummary`, `EventRef` | 내부 엔지니어링 점검 | Active payload에서 쓰는 공통 contract shape입니다. |
| `WriteAuthorizationSummary`, `WriteAuthoritySummary`, `ApprovalScope` | 내부 엔지니어링 점검 core write authority. Approval record는 보증 프로필 | Write Authorization summary는 allowed write가 만들어질 때 내부 엔지니어링 점검 범위입니다. Minimum MVP-1에서는 민감 동작 승인 user judgment coverage가 `user_judgment` ref로 표현됩니다. Approval-specific refs와 lifecycle은 Approval profile이 켜지기 전까지 later-profile입니다. |
| `ArtifactRef`, `ArtifactInput`, `EvidenceRefs`, `StateRecordRef` | 내부 엔지니어링 점검 minimal artifact/evidence ref. 더 풍부한 owner relation은 later profile | 내부 엔지니어링 점검은 registered artifact/evidence ref 하나와 compatible owner link 하나가 필요합니다. Later owner record kind는 해당 storage/API profile이 있을 때만 valid합니다. |
| `UserJudgment`, `UserJudgmentCandidate`, `JudgmentContext`, `AcceptanceVisibilityContext` | MVP-1 사용자 작업 루프. Full approval/waiver/risk/reconcile profile은 보증 프로필/운영 프로필 | User judgment 또는 candidate가 만들어질 때 required field가 적용됩니다. 내부 엔지니어링 점검은 user judgment storage를 요구하지 않습니다. |
| `ResidualRiskSummary`와 residual-risk refs | MVP-1 visibility. 보증 프로필 full residual-risk acceptance semantics | `status=none`은 summary claim일 수 있습니다. Accepted risk는 standalone record kind가 아니라 `residual_risk` ref의 state입니다. |
| `ValidatorResult` | 보증 프로필과 운영 프로필. Owner가 승격한 capability/status check는 예외 | Validator result가 emitted될 때 required field가 적용됩니다. Core check는 validator ID가 되지 않고도 block할 수 있습니다. |
| `ProjectionJobRef`, `ProjectionKind`, projection freshness object | 내부 엔지니어링 점검에서 required인 `ProjectionKind`는 없음. MVP-1은 minimal `TASK`/card output을 사용할 수 있고, assurance, operations/export, future/diagnostic class는 profile-gated | Projection ref와 job은 derived-view metadata이며 authority가 아닙니다. Kind는 해당 projection support를 enabled한 stage/profile에서만 valid reference target입니다. |
| `JourneyCardSummary`, `RecommendedPlaybook`, Role Lens routing field | profile에 따라 MVP-1 display guidance 또는 later diagnostic view | 읽기 전용 display/routing schema입니다. 그 자체로 write를 authorize하거나, gate를 충족하거나, risk를 accept하거나, Task를 close하지 않습니다. |
...
Sensitive category는 명령어처럼 외우는 체계가 아니라 민감 동작 permission이 필요한 민감 위험을 설명하는 label입니다. 하나의 intended write에는 여러 category가 함께 붙을 수 있습니다. Category는 민감 동작 permission이 왜 필요한지 설명하지만 제품, 아키텍처, 보안, QA, verification, 작업 수락, 잔여 위험 관련 판단, policy 판단을 대신 해결하지 않습니다. 정확한 write-state 동작은 [커널 참조](kernel.md#prepare_write)가 담당하고, public request와 lifecycle shape은 [`harness.prepare_write`](#harnessprepare_write)와 [Sensitive-Action Approval Path](#sensitive-action-approval-path)가 담당합니다.

| Category | 보통 뜻하는 것 | 민감 동작 승인, 판단 요청, evidence, redaction 지침 |
|---|---|---|
| `auth_change` | 로그인, session, password, OAuth, account recovery, lockout, authentication policy 변경. | Approval은 민감한 auth 단계를 포괄합니다. Auth model, lockout behavior, recovery UX, user notice, 해소되지 않은 security trade-off에는 기술 판단 요청을 사용합니다. Evidence는 test 또는 review 결과를 보여주되 credential, token, secret value를 노출하지 않아야 합니다. |
| `permission_model_change` | Role, ACL/RBAC rule, authorization check, admin capability, ownership check, access boundary 변경. | Approval은 permission-sensitive mutation을 포괄합니다. Role design, migration, audit expectation, default access, compatibility에는 기술 판단 요청을 사용합니다. Evidence는 보호 대상 subject data를 노출하지 않고 covered path와 policy check를 식별해야 합니다. |
| `schema_change` | Database, state, API, event, fixture, data-model shape 변경과 migration. | Approval은 민감한 schema 또는 migration side effect를 포괄합니다. Additive path와 breaking path 선택, backfill, rollback, compatibility window, maintenance cost에는 기술 판단 요청을 사용합니다. Evidence에는 migration/test coverage를 포함하고 production-like record는 redaction해야 합니다. |
| `dependency_change` | Runtime/build dependency 또는 dependency lock의 install, upgrade, removal, 변경. | Approval은 install, lockfile edit, dependency-file write를 포괄합니다. Dependency 채택이 architecture, compatibility, cost, license posture, rollback, maintenance를 바꾸면 기술 판단 요청을 사용합니다. Evidence에는 lockfile diff, test output, security 또는 license scan ref를 둘 수 있습니다. |
| `public_api_change` | CLI flag, HTTP endpoint, SDK contract, exported function, public config, documented behavior, module-boundary commitment 변경. | Approval은 민감한 write를 포괄할 수 있지만 compatibility와 breaking-change 판단에는 기술 판단 요청이 필요합니다. Evidence에는 caller-impact check, docs update, migration note, relevant test를 포함해야 합니다. |
| `destructive_write` | Delete, overwrite, irreversible migration step, data loss, reset operation, history/state removal. | Approval은 destructive side effect와 affected scope를 이름 붙여야 합니다. Rollback, backup, user impact, irreversibility, 잔여 위험 수용에는 판단 요청을 사용합니다. Evidence에는 applicable한 dry-run, backup, diff, recovery ref가 있어야 합니다. |
| `network_write` | POST/PUT/PATCH/DELETE 또는 그에 준하는 네트워크 write operation. | Approval은 network target, method/class, payload class, expiry를 포괄합니다. External user impact, rollback, data selection, target ownership이 불확실한 경우 판단 요청을 사용합니다. Evidence는 request를 안전하게 요약하고 secret 또는 PII payload를 생략해야 합니다. |
| `external_service_write` | Third-party service 또는 external account의 resource 생성, 변경, 삭제, configuration. | Approval은 external service action과 account/tenant boundary를 포괄합니다. Ownership, lifecycle, retention, cost, rollback, user notice, support impact에는 판단 요청을 사용합니다. Evidence는 token이나 private raw payload 대신 service ref, id, redacted log를 사용해야 합니다. |
| `secret_access` | Credential, token, certificate, key, secret handle의 read, write, rotation, copy, use. | Approval은 named secret scope와 access kind를 포괄합니다. Secret choice, rotation plan, retention, audit trail, exposure risk에는 기술 판단 요청을 사용합니다. Evidence는 handle, `secret_omitted`, `blocked` artifact를 사용해야 하며 secret value를 log, projection, export, screenshot, summary에 넣으면 안 됩니다. |
| `production_config_change` | Production flag, environment variable, config file, runtime limit, operational default, safety switch 변경. | Approval은 production-sensitive config write를 포괄합니다. Rollout, rollback, user impact, monitoring, operational trade-off에는 기술 판단 요청을 사용합니다. Evidence에는 config diff 또는 plan ref를 두고 secret과 tenant-specific value는 redaction 또는 omission해야 합니다. |
| `ci_cd_change` | CI workflow, release pipeline, deployment automation, runner permission, signing, publishing, test gate 변경. | Approval은 pipeline 또는 automation mutation을 포괄합니다. Release policy, required gate, runner trust, rollback, deployment authority에는 기술 판단 요청을 사용합니다. Evidence에는 workflow diff와 secret value가 omitted된 run log를 포함해야 합니다. |
| `infra_or_deployment_change` | Cloud, container, Kubernetes, Terraform, provisioning, routing, scaling, deployment, operational topology 변경. | Approval은 infrastructure 또는 deployment side effect를 포괄합니다. Topology, rollout, rollback, availability, security boundary, cost, ownership choice에는 기술 판단 요청을 사용합니다. Evidence에는 plan/apply summary, affected resource, redacted provider output을 포함해야 합니다. |
| `privacy_or_pii_change` | PII 또는 privacy-sensitive data의 collection, storage, display, transformation, retention, deletion. | Approval은 privacy-sensitive action을 포괄합니다. Data minimization, field, retention, user notice, consent, access, redaction strategy에는 기술 판단 요청을 사용합니다. Evidence는 sanitized sample을 사용하고 PII는 artifact 등록 전에 redacted, omitted, blocked 상태가 되어야 합니다. |
| `data_export` | Report, file, external system, support bundle, user/operator download 등으로 현재 boundary 밖에 data를 내보내는 작업. | Approval은 어떤 data가 어디로 나가는지, destination, retention, expiry를 포괄합니다. Field selection, recipient authority, redaction, omitted value, audit trail, export와 관련해 받아들이는 위험에는 판단 요청을 사용합니다. Evidence에는 evidence summary ref, export artifact ref, redaction state를 보존하는 ref를 포함하고, full Evidence Manifest profile이 active일 때만 Evidence Manifest를 포함합니다. |
| `telemetry_or_logging_change` | Event, log, metric, trace, sampling, correlation id, log retention 추가, 삭제, 변경. | Approval은 behavior, user data, cost, operational risk를 노출할 수 있는 telemetry를 포괄합니다. Event semantics, privacy posture, retention, opt-out, observability trade-off, support burden에는 기술 판단 요청을 사용합니다. Evidence는 sanitized sample을 보여주고 raw secret 또는 PII를 피해야 합니다. |
| `license_or_compliance_change` | License file, notice, compliance control, audit evidence, legal commitment, policy-governed obligation 변경. | Approval은 compliance-sensitive write를 포괄합니다. Acceptable license posture, obligation handling, exception path, 위험을 받아들이는 판단에는 판단 요청을 사용합니다. Evidence는 scan, notice, policy ref를 가리키되 restricted audit material을 불필요하게 노출하지 않아야 합니다. |
| `billing_or_cost_change` | Paid resource use, quota, billing configuration, plan, cost-bearing model call, usage limit 변경. | Approval은 비용이 발생하는 action과 budget boundary를 포괄합니다. Cost/performance trade-off, quota policy, user impact, rollback, 비용 지출과 관련해 받아들이는 위험에는 판단 요청을 사용합니다. Evidence에는 estimate, limit, observed usage ref를 포함해야 합니다. |
| `model_or_prompt_policy_change` | Model selection, system/developer prompt, safety policy, tool policy, routing, evaluation policy, generated-output policy 변경. | Approval은 민감한 policy 또는 prompt write를 포괄합니다. Product tone, safety trade-off, data exposure, model cost, eval threshold, user-facing behavior에는 제품/UX 판단 또는 기술 판단 요청을 사용합니다. Evidence에는 eval ref와 필요한 경우 redacted prompt/policy artifact를 포함해야 합니다. |
| `policy_override` | Harness, project, security, QA, verification, compliance policy를 우회, 약화, waiver, exception 처리하는 작업. | Approval은 scope 안의 민감한 override step만 허가할 수 있습니다. 왜 exception이 받아들일 만한지, 어떤 위험을 받아들이는지, 어떤 follow-up이 남는지, close에 어떤 영향이 있는지에는 판단 요청을 사용합니다. Evidence는 policy, waiver, Residual Risk, follow-up ref를 연결해야 합니다. |

Approval prompt는 일반 사용자가 이해하는 side effect를 먼저 말하고 identifier를 뒤에 붙여야 합니다. 예: "redaction된 billing CSV를 vendor X로 export해도 될까요? (`data_export`, `external_service_write`)." 같은 단계가 사용자 소유 제품/UX 판단, 기술 판단, QA, verification, 작업 수락, 잔여 위험 관련 판단, policy 판단도 결정한다면 그 판단은 compatible user judgment로 연결해야 합니다. 그 판단이 write authority를 막고 있다면 `prepare_write`가 `allowed`를 반환하기 전에 applicable한 owner gate 의미에 따라 resolved, deferred, waived 또는 그 밖의 호환되는 방식으로 기록되어야 합니다.
...
아래 표는 staged implementation에서 실제로 active한 validation set입니다. 이어지는 full schema block은 참조 안정성을 위해 later/future enum value를 보존하지만, caller와 validator는 active stage/profile이 켠 값만 받아들여야 합니다.

| Field | 내부 엔지니어링 점검 / MVP-1 active values | 보증 프로필/운영 프로필 later-profile values | 로드맵 / future candidate values |
|---|---|---|---|
...
`bundle`과 `manifest`는 verification/evidence bundle profile에서 사용합니다. `qa_capture`는 Manual QA/capture profile에서 사용합니다. `export_component`는 operations/export profile에서만 active입니다. Design probe, prototype, architecture scan, decision-context artifact는 future 또는 owner-promoted candidate이며 내부 엔지니어링 점검이나 minimum MVP-1 artifact requirement가 아닙니다.

| Field | 내부 엔지니어링 점검 active owner record kinds | MVP-1 active owner record kinds | 보증 프로필/운영 프로필 later-profile record kinds | 로드맵 / future candidate record kinds |
|---|---|---|---|---|
...
MVP-1 sensitive-action approval은 user-facing route에서 `user_judgment` ref로 표현할 수 있습니다. Committed `approval` ref는 Approval owner profile이 켜지기 전까지 later-profile입니다. `projection` ref는 해당 owner profile의 projection support가 있을 때만 valid합니다. Projection은 계속 derived view이며 authority가 아닙니다.

`evidence_summary`와 `close_readiness`는 이름이 같은 MVP-1 storage row를 가리키는 public `StateRecordRef` kind입니다. 둘은 authority ref 위의 Core-owned summary/check record에 대한 public ref일 뿐이며 full Evidence Manifest, projection, verification, close-report profile을 active로 만들지 않습니다. Minimum summary는 artifact/run ref를 저장할 뿐 artifact link를 소유하지 않으므로 둘은 MVP-1 `ArtifactInput.relation.record_kind` 값이 아닙니다. `change_unit_dependency`는 `change_unit_dependencies`가 MVP-1 storage가 아니므로 future/diagnostic으로 남습니다.

향후 Browser QA Capture는 새 reference schema가 아니라 이 artifact 경계를 사용합니다. 화면 capture는 보통 `screenshot`을 사용하고, 묶음 QA output은 `qa_capture`를 사용할 수 있습니다. Console log와 network trace는 `log` 또는 `qa_capture`를 사용할 수 있고, accessibility snapshot과 workflow recording은 명확한 description과 함께 `qa_capture` 또는 `other`를 사용할 수 있습니다. 이러한 artifact는 모두 redaction, secret/PII handling, Task-scoped ownership, 수동 QA record 또는 Feedback Loop attachment rules를 따라야 합니다. Capture artifact는 근거를 보강할 수 있지만 작업 수락을 만들거나, 수동 QA judgment를 대체하거나, 분리 검증을 충족하거나, 내부 엔지니어링 점검에 필요한 capture schema를 추가하지 않습니다.
...
ArtifactInput:
  ...
  relation:
    task_id: string
    run_id: string | null
    record_kind: task | change_unit | run | user_judgment | decision_packet | shared_design | residual_risk | evidence_manifest | eval | manual_qa_record | feedback_loop | tdd_trace | journey_spine_entry | projection
...
이후 consumer도 같은 의미를 유지해야 합니다. Evidence Manifest, 수동 QA, Eval, projection, export, Release Handoff, doctor, artifact integrity display는 ref, hash, 안전한 omission note, handle, blocked notice를 보여줄 수 있지만 생략되었거나 차단된 원본 값을 inline 표시하거나 재구성하거나 요약하거나 export하면 안 됩니다. `secret_omitted`는 secret이 아닌 근거가 보이는 주장만 충족할 수 있으며, 생략된 값이 필요한 주장은 unsupported 또는 insufficient로 남겨야 합니다. `blocked`는 replacement artifact, compatible waiver, user judgment outcome, Residual Risk record의 accepted-risk 상태, 또는 다른 documented resolution이 그 경로를 해소하기 전까지 evidence, QA, verification, projection, export, Release Handoff에서 시도된 input을 사용할 수 없는 것으로 취급한다는 뜻입니다.

Record 또는 projection references는 `ArtifactRef`가 아니라 `StateRecordRef`를 사용합니다.

```yaml
StateRecordRef:
  record_kind: task | change_unit | run | approval | write_authorization | user_judgment | decision_packet | journey_spine_entry | shared_design | domain_term | module_map_item | interface_contract | feedback_loop | residual_risk | evidence_summary | close_readiness | evidence_manifest | eval | manual_qa_record | tdd_trace | change_unit_dependency | reconcile_item | projection
  record_id: string
  projection_path: string | null
```

`record_kind=user_judgment`는 민감 동작 승인 judgment를 포함한 사용자 소유 판단의 canonical MVP-1 reference kind입니다. `record_kind=decision_packet`은 migration 중 같은 record family 또는 standalone full-format Decision Packet projection을 가리키는 legacy compatibility alias입니다. 새 payload는 `user_judgment`를 사용해야 합니다.

`record_kind=approval`은 later-profile reference kind입니다. Storage/DDL이 `approvals` table을 명시적으로 승격하고 Approval owner profile이 active인 경우가 아니라면 minimum MVP-1 implementation은 이 값을 반환하면 안 됩니다. Minimum MVP-1의 민감 동작 승인은 `record_kind=user_judgment`로 참조합니다.
...
Feedback-loop profile이 enabled되면 기준 feedback-loop records에 대한 public refs는 `feedback_loops.feedback_loop_id`와 함께 `record_kind=feedback_loop`를 사용합니다. Red/green/refactor TDD evidence row에는 `record_kind=tdd_trace`만 사용합니다. Feedback Loop는 execution evidence로 TDD Trace를 cite할 수 있지만, TDD Trace가 selected-loop definition을 대체하지는 않습니다.
...
`WriteAuthorizationSummary.approval_refs`는 minimum MVP-1에서 비어 있습니다. Resolved 민감 동작 승인 user judgment가 민감 동작 coverage를 제공하면 compatible judgment는 `user_judgment_refs`에 들어갑니다. Committed Approval refs는 Approval owner profile이 active일 때만 반환합니다.
...
`DEC`, `DESIGN`, `EXPORT`, `JOURNEY-CARD`, `RUN-SUMMARY`, `EVIDENCE-MANIFEST`, `EVAL`, `TDD-TRACE`, `MODULE-MAP`, `INTERFACE-CONTRACT`처럼 active stage/profile 밖에 있는 `ProjectionKind` 값은 해당 projection 기능 또는 profile이 켜졌을 때만 projection job kind로 유효합니다. Active stage/profile이 요구하는 사용자 판단 visibility는 status/next responses, judgment-context resources, user-judgment resources, 최소 `TASK` 또는 card display를 통해 제공됩니다. 이는 standalone `DEC` `ProjectionKind`가 아니라 판단 요청 display shape에 대한 요구사항입니다. Persisted `JOURNEY-CARD` Markdown과 Journey Spine-style output은 future/diagnostic 범위이며 status, next, significant resume flow의 현재 위치 맥락은 간결한 상태 출력으로 충족할 수 있습니다. 전체 projection template text는 [Template 참조](templates/README.md)에 있으며, 이 API schema file이 담당하지 않습니다.

User Judgment, Write Authorization, Write Authority Summary, Journey Card, Judgment Context, Autonomy Boundary, Recommended Playbook, acceptance visibility, residual-risk summaries는 public MCP schemas입니다. 이 schemas는 API payload만 설명합니다. 기준 kernel records는 owner docs가 정의합니다. 이 목록에서 `RecommendedPlaybook`은 표시 전용 예외입니다. 자체 기준 kernel record, DDL table, task event, projection job이 없습니다.

Role Lens behavior는 이 기존 표시 및 routing schema를 사용합니다. Role lens는 `RecommendedPlaybook`으로 나타날 수 있고, existing user judgment로 route할 수 있으며, `UserJudgmentCandidate`를 propose할 수 있습니다. 별도의 public payload schema, 권한 기록, 상태 전이를 도입하지 않습니다.

Judgment 품질은 아래 public field와 [User Judgment](kernel.md#user-judgment), [Decision Gate](kernel.md#decision-gate)의 kernel authority rule로 판단합니다. 충분한 요청은 사용자가 무엇을 판단하는지, compact judgment type, relevant scope/ref, 기록될 option 또는 outcome, 답변이 해소하지 않는 것을 보여줘야 합니다. 선택, source, evidence, impact, agent latitude를 솔직히 보여줄 수 없으면 broad approval처럼 제시하지 말고 blocked 또는 narrowed로 다룹니다.

공통 judgment model은 의도적으로 작게 둡니다. 사용자에게 보이는 display label은 다섯 가지이고, API payload는 compact `judgment_type`과 `presentation` 값을 가집니다.

| User-facing display label | Internal `judgment_type` examples | 의미 |
|---|---|---|
| 제품/UX 판단 | `product_choice` | 제품 동작, 문구, interaction, 취향, 사용자 가치, release-facing promise. |
| 기술 판단 | `technical_choice` | Architecture, public API, dependency, migration, compatibility, security/privacy trade-off, QA/verification expectation, scope/autonomy choice, 중요한 implementation direction. |
| 민감 동작 승인 | `sensitive_action_approval` | 이름 붙은 민감 단계에 대한 scoped permission. |
| 작업 수락 | `work_acceptance` | 작업 수락이 필요한 경우 사용자의 결과 판단. |
| 잔여 위험 수용 | `residual_risk_acceptance` | 이름 붙고 visible한 close-relevant remaining risk의 수용. |

`presentation=short`는 작은 unblocker와 한 화면 prompt의 기본값입니다. `presentation=full`은 복잡하거나 high-risk이거나 close에 영향을 주거나 reconcile/later-profile 판단을 위한 full-format Decision Packet-style presentation입니다. Presentation은 렌더링되는 context 양만 바꾸며 authority를 바꾸지 않습니다.

이전 field와의 compatibility mapping은 임시로 명시합니다.

| Legacy field 또는 method | Canonical replacement |
|---|---|
...
| `judgment_domain` | `judgment_type`과 ordinary display label. |
| `decision_kind` | `judgment_type`과 route-specific payload validation. |
| `decision_profile` | `presentation`; full-format Decision Packet display는 별도 record family가 아니라 presentation입니다. |

Legacy field는 compatibility layer 또는 migration tool에서만 받아들일 수 있습니다. 새 example과 future fixture는 judgment 이름과 `user_judgment` ref를 사용해야 합니다.

`UserJudgmentCandidate`, `RecommendedPlaybook.route`, `judgment_path`, optional implementation prompt-routing rows는 request/display/routing metadata입니다. [`harness.request_user_judgment`](#harnessrequest_user_judgment)로 caller를 안내할 수 있지만, owner path가 compatible `UserJudgment`와 linked owner record를 commit 또는 update하기 전에는 authority가 아닙니다. Stored user judgment가 committed `judgment_type`, `presentation`, `judgment_payload`, status, refs를 소유합니다.

아래 schema block은 이 문서의 YAML-like notation만 사용합니다. `judgment_payload.approval_scope`는 `judgment_type=sensitive_action_approval`에서만 필요합니다. `acceptance`는 `judgment_type=work_acceptance`에서만 필요하고, `residual_risk_acceptance`는 `judgment_type=residual_risk_acceptance`에서만 필요합니다. Later-profile branch는 해당 owner profile이 enabled될 때만 적용합니다. `presentation=full`은 informed judgment에 충분한 option, ref, risk, consequence, "does not cover" text를 요구합니다. `presentation=short`는 extra trade-off text가 중요하지 않으면 concise option과 `null` detail을 쓸 수 있습니다.
...
UserJudgmentCandidate:
  # Candidate 공통 field는 canonical id/status/timestamp를 제외하고 UserJudgment 공통 field와 맞춘다.
  task_id: string
  change_unit_id: string | null
  judgment_type: product_choice | technical_choice | sensitive_action_approval | work_acceptance | residual_risk_acceptance
  presentation: short | full
...
NextActionSummary.action_kind의 stage별 active values:

| Stage/profile | Active `action_kind` values |
|---|---|
| 내부 엔지니어링 점검 | `prepare_write`, `implement`, `close_task`, `idle` |
| MVP-1 사용자 작업 루프 | `ask_user`, `prepare_write`, `implement`, `request_acceptance`, `close_task`, `idle` |
| 보증 프로필/운영 프로필 later profiles | matching owner profile이 enabled될 때만 `launch_verify`, `record_eval`, `record_manual_qa`, `reconcile` |
...
`AcceptanceVisibilityContext.evidence_summary_refs`는 `record_kind=evidence_summary`인 `StateRecordRef` entry를 포함합니다. Public ref가 필요한 close-readiness display는 `record_kind=close_readiness`를 사용합니다. 둘은 MVP-1 summary/check ref입니다. Full `evidence_manifest`, verification, 수동 QA, projection ref는 owner profile이 enabled될 때만 later-profile value로 active입니다.
...
`judgment_type=sensitive_action_approval`은 민감 동작 승인 judgment만 뜻합니다. 제품/UX 판단, 아키텍처나 보안/개인정보 고려사항을 포함한 기술 판단, QA waiver, verification waiver, verification risk, 작업 수락, 잔여 위험 수용은 별도의 compatible user judgment와 owner-record update가 있어야 해소됩니다.
...
보증 프로필과 운영 프로필의 ValidatorResult ID는 다음과 같습니다.
...
| `WRITE_AUTHORIZATION_INVALID` | supplied Write Authorization이 absent, expired, `stale`, revoked, idempotent replay 밖에서 already consumed, 또는 Task, Change Unit, baseline, intended operation, sensitive-action permission refs, user judgment refs와 incompatible합니다 |
| `DECISION_REQUIRED` | 사용자 소유 판단이 requested action 진행을 막고 있어 user judgment request가 필요함 |
| `DECISION_UNRESOLVED` | relevant user judgment가 pending, deferred without coverage, rejected, blocked, `stale` 또는 requested action과 incompatible함 |
| `AUTONOMY_BOUNDARY_EXCEEDED` | the intended operation exceeds the active Change Unit Autonomy Boundary |
| `APPROVAL_REQUIRED` | 진행하려면 민감 동작 permission이 필요함 |
| `APPROVAL_DENIED` | 관련 민감 동작 permission이 denied됨 |
| `APPROVAL_EXPIRED` | 민감 동작 permission이 expired되었거나 baseline/scope에서 drift됨 |
...
`WRITE_AUTHORIZATION_REQUIRED`와 `WRITE_AUTHORIZATION_INVALID`는 missing 또는 invalid Write Authorization에만 사용합니다. Observed paths, tools, commands, network targets, secrets, sensitive categories가 authorized 또는 active scope를 넘는 경우 scope violations는 계속 `SCOPE_VIOLATION`을 사용합니다. `ToolError` object를 사용할 수 있으면 write-authority failure에는 `ToolErrorWriteAuthorityDetails`를 넣습니다. `WRITE_AUTHORIZATION_REQUIRED`는 `write_authorization_issue=missing`을 사용합니다. `WRITE_AUTHORIZATION_INVALID`는 `expired`, `stale`, `revoked`, `already_consumed`, `incompatible`, `absent`, `unknown` 중 하나를 사용합니다.
...
- `LOCAL_ACCESS_MISMATCH`: Core 또는 operator가 reachable local endpoint 또는 caller path를 registered local access/profile boundary 밖으로 분류할 수 있습니다. Off-profile 또는 unauthorized local access도 여기에 포함합니다. Detail은 `ToolErrorLocalAccessDetails`를 사용하며 raw tokens, private config, sensitive file contents를 노출하면 안 됩니다. 이 reference는 local-profile mismatch에 별도 `ACCESS_DENIED` code를 추가하지 않습니다.
...
| `MCP_UNAVAILABLE`(`details.mcp_unavailable_kind=server_unavailable`) 또는 진단상 `MCP_SERVER_UNAVAILABLE` | MCP 사용 불가: Core에 닿을 수 없음 | 기준 상태 변경, 민감 동작 permission 변경, gate update, projection repair, close를 주장하기 전에 Core 연결을 복구하거나 진단합니다. |
...
| `NO_ACTIVE_CHANGE_UNIT`, `SCOPE_REQUIRED`, `SCOPE_VIOLATION`, `AUTONOMY_BOUNDARY_EXCEEDED`, `BASELINE_STALE` | 범위, 사용자 판단 경계 또는 baseline 문제 | Scope를 확인하거나 줄이고, Change Unit 또는 baseline을 갱신하거나, 진행 전에 필요한 user judgment를 요청합니다. |
| `DECISION_REQUIRED`, `DECISION_UNRESOLVED` | 사용자 판단 필요 | 관련 user judgment prompt를 options 또는 chosen outcome, refs, 필요할 때 recommendation, uncertainty, deferral effect와 함께 보여줍니다. |
| `APPROVAL_REQUIRED`, `APPROVAL_DENIED`, `APPROVAL_EXPIRED` | 민감 동작 permission 필요 또는 사용할 수 없음 | Minimum MVP-1에서는 민감 동작 승인 user judgment를 요청, 해소, 갱신합니다. Later Approval profile이 active일 때만 committed Approval record를 다룹니다. 민감 동작 승인을 Write Authorization이나 product judgment처럼 다루면 안 됩니다. |
...
Taxonomy table의 모든 code는 이 reference의 public `ErrorCode` values입니다. Validator-specific detail은 `ValidatorResult.findings`에 둡니다. Write-authority, MCP availability, local-access/profile detail은 해당 `ToolError.details` diagnostic object에 둡니다.
...
| 12 | `APPROVAL_DENIED` | relevant 민감 동작 permission이 denied됨 |
| 13 | `APPROVAL_EXPIRED` | relevant 민감 동작 permission이 expired되었거나 scope 또는 baseline에서 drift됨 |
| 14 | `APPROVAL_REQUIRED` | sensitive change에 민감 동작 permission이 필요하지만 compatible granted 민감 동작 승인 user judgment 또는 later Approval record가 없음 |
| 15 | `DECISION_UNRESOLVED` | existing relevant user judgment가 pending, deferred without coverage, rejected, blocked, `stale`, 또는 incompatible함 |
| 16 | `AUTONOMY_BOUNDARY_EXCEEDED` | intended operation이 active Change Unit Autonomy Boundary를 초과하며, next step이 user judgment request여도 이 code를 사용함 |
| 17 | `DECISION_REQUIRED` | 사용자 소유 판단이 action 진행을 막고 있어 user judgment request가 필요함 |
...
표시되었지만 수용되지 않은 close-relevant risk는 `RESIDUAL_RISK_NOT_VISIBLE`로 반환하지 않습니다. Requested close path가 잔여 위험 수용을 요구하면 public close/API response는 잔여 위험 수용 user judgment를 요청해야 할 때 primary `DECISION_REQUIRED`를 사용하고, 관련 잔여 위험 수용 user judgment가 pending, rejected, blocked, stale, deferred without coverage, incompatible일 때 `DECISION_UNRESOLVED`를 사용합니다. Structured close blocker category는 `residual_risk_acceptance`여야 하며, `related_refs`는 관련 `residual_risk` refs 또는 pending `user_judgment` refs를 인용해야 합니다.
...
#### 내부 엔지니어링 점검 active API surface
...
| `harness.prepare_write` | Active product-write authorization check. | 하나의 allowed/blocked path와, allowed일 때 durable single-use Write Authorization 하나를 만듭니다. Minimum MVP-1은 resolved 민감 동작 승인 user judgment coverage를 사용할 수 있고, committed Approval record는 later-profile입니다. |
...
| `harness.intake` 또는 owner-valid setup path | Active setup capability입니다. 내부 엔지니어링 점검에서 method 자체는 optional입니다. | 내부 엔지니어링 점검은 local project, Task, scoped work boundary가 필요합니다. 이 public method를 사용한다면 minimal setup shape만 사용하고, full natural-language intake/discovery는 MVP-1입니다. |
| `harness.next` | Optional 내부 엔지니어링 점검 read. | Smoke path에서 구현한다면 다음 minimal authority-loop action 또는 smallest unblocker만 반환합니다. 내부 엔지니어링 점검은 `ask_user`, `request_acceptance`, verification, QA, reconcile action kind를 반환하지 않습니다. `ask_user`와 `request_acceptance`는 MVP-1에서 들어오고, verification, QA, reconcile action kind는 action-kind stage table에 따라 later owner profile이 enabled될 때만 들어옵니다. |
| `harness.close_task` | Optional narrow blocker/status smoke only. | Structured blocker를 보여주는 가장 단순한 방법일 때만 사용할 수 있습니다. 내부 엔지니어링 점검은 작업 수락, 잔여 위험 수용, full close semantics를 증명하지 않습니다. |

#### MVP-1 active API surface

| Method 또는 capability | Activation | Scope boundary |
|---|---|---|
| `harness.status.next_actions`; optional `harness.next` | Active fuller user-facing status/next display. | 현재 위치, pending user judgments, evidence summary, close readiness/blocker, smallest unblocker를 사용자 언어로 보여줍니다. 작업 수락이 progress나 close를 막을 때 `request_acceptance`를 반환할 수 있습니다. 작업 수락 필요/상태와 잔여 위험 표시는 관련 있을 때 구분해 보여주며 필수 projection kind를 늘리지는 않습니다. |
...
| `harness.request_user_judgment`와 `harness.record_user_judgment` | User-owned judgment와 work acceptance가 progress 또는 close를 막을 때 active입니다. | User judgment/candidate가 만들어질 때 `UserJudgment` field가 active입니다. Full-format Decision Packet presentation은 명시적으로 켜진 complex/later-profile 판단에서만 사용합니다. `harness.request_user_decision`과 `harness.record_user_decision`은 compatibility alias입니다. Approval hardening, waiver, full residual-risk acceptance, reconcile profile은 owner profile이 켜질 때 들어옵니다. |
| `harness.record_run` | Active evidence/artifact summary path. | MVP-1 사용자 가치 경로에서 evidence가 충분히 visible해야 합니다. Full Evidence Manifest projection과 assurance record는 명시적으로 enabled되기 전까지 later-profile입니다. |
...
| Minimal `TASK` projection 또는 compact status/card output | Active display capability입니다. Persisted projection job일 필요는 없습니다. | Status/next/card output이 MVP-1의 최소 사용자 읽기용 summary를 충족하면 persisted Markdown rendering은 optional입니다. |
...
| `harness.request_user_judgment` / `harness.record_user_judgment`의 Reconcile profile | Reconcile storage/operations support가 enabled될 때만 active입니다. | Reconcile candidate는 existing user judgment/owner path가 outcome을 commit하기 전까지 state가 되지 않습니다. |
| Operator diagnostics, recover/export, artifact integrity, conformance commands | 운영 프로필 owner surface이며, 현재 이 문서의 public MCP tool은 아닙니다. | 정확한 command semantics는 [Operations And Conformance Reference](operations-and-conformance.md)가 담당합니다. 이 capability를 future MCP tool로 승격하면 matching storage profile support와 함께 이 profile에 추가합니다. |
...
내부 엔지니어링 점검 / MVP-1 경로에서 쓰는 current staged method index:

| Tool | 이 section에서 찾는 것 |
|---|---|
| [`harness.status`](#harnessstatus) | status, active stage의 gate, write authority, guarantee, residual-risk visibility, next-action display |
...
| [`harness.record_run`](#harnessrecord_run) | run recording, active artifact/evidence ref, Write Authorization consumption |
| [`harness.request_user_judgment`](#harnessrequest_user_judgment) | `user_judgment` creation과 user-owned judgment prompt |
| [`harness.record_user_judgment`](#harnessrecord_user_judgment) | active `user_judgment`에 대한 사용자 답변 기록 |
| [`harness.close_task`](#harnessclose_task) | close request/response, close state, blocker, close result |

Later-profile method index입니다. 이 method section들은 reference stability를 위해 남아 있지만 내부 엔지니어링 점검 또는 minimum MVP-1에서는 active가 아닙니다.

| Tool | Later-profile use |
|---|---|
| [`harness.launch_verify`](#harnesslaunch_verify) | 보증 프로필이 enabled될 때 verification launch request/response와 bundle ref |
| [`harness.record_eval`](#harnessrecord_eval) | Eval recording이 enabled될 때 Eval recording, verification verdict, independence qualifier, artifact ref |
| [`harness.record_manual_qa`](#harnessrecord_manual_qa) | Manual QA policy가 enabled될 때 수동 QA result, waiver link, residual-risk ref, QA artifact |

### `harness.status`

Purpose: project, surface, active Task, 간결한 현재 위치 요약, gate, guarantee, 범위에 있을 때의 projection freshness, active user judgment, Autonomy Boundary, Write Authority Summary, residual-risk, pending-judgment status를 반환합니다.

Stage/profile: 내부 엔지니어링 점검은 minimal status/blocker profile을 사용합니다. MVP-1은 `next_actions`가 있는 사용자 대상 status/card profile을 추가합니다. 보증 프로필 이후 assurance, projection, reconcile, operations field는 위 activation table을 통해서만 들어옵니다.

사용자에게 보이는 의미: 현재 위치를 보여줍니다. Status 표시는 active Task, 작업 모양, 알려진 범위/하지 않을 일, 가장 먼저 해소할 막힘이 있으면 그 막힘, 대기 중인 사용자 판단, 가장 작은 해소 방법, 근거 공백, 닫기 막힘, 잔여 위험 요약, 보장 수준, 출처/최신성 refs를 먼저 보여야 합니다. Ref와 추가 막힘을 함께 보여줄 수 있지만, 사용자가 계속 진행할 수 있는지 이해하려고 raw schema field만 읽게 만들면 안 됩니다. 전체 reference docs, 전체 schema, 전체 DDL, 전체 historical event log, 읽기용 요약 전체 본문, full artifact contents, 관련 없는 template, future catalog 자료를 embed하면 안 됩니다.
...
|---|---|
| 내부 엔지니어링 점검 minimal | Present하면 active Task/status, compact `status_card`, current state version, 요청되고 사용 가능할 때 write-authority summary, guarantee display, primary structured blocker를 반환합니다. `next_actions`는 empty이거나 minimal blocker/action 하나를 담을 수 있습니다. Future-profile array는 empty이고 future-profile object는 schema에 따라 `null` 또는 `unknown`입니다. |
| MVP-1 user-facing | `next_actions`, pending-judgment visibility, evidence/status summary, close-readiness/blocker display를 추가합니다. 작업 수락과 잔여 위험 사실은 관련 있을 때 나타나지만 별도 projection kind를 요구하지 않습니다. |
| 보증 프로필/운영 프로필 assurance and operations | Verification, Manual QA, validator, projection freshness, reconcile, diagnostic/export-related ref는 해당 profile과 storage가 enabled될 때만 추가합니다. |
...
`status_card`와 agent compact context/reference payload는 작은 파생 display이지 그 자체로 구현된 authority path가 아닙니다. 기본으로 담을 수 있는 것은 현재 Task 요약, 작업 모양, 범위/하지 않을 일, 대기 중인 사용자 판단, 활성 막힘, 다음 안전한 행동, 근거 공백, 닫기 막힘, 잔여 위험 요약, 보장 수준, 출처 refs/최신성뿐입니다. 추가 본문은 정확한 단계와 다음 행동이 필요로 할 때만 pull-on-demand로 둡니다.
...
Stage/profile: 내부 엔지니어링 점검은 implementation이 public API setup을 선택한 경우 minimal owner-valid Task/scope setup path로만 이 method를 사용할 수 있습니다. Full natural-language intake, discovery quality, procedural budget display, ordinary-user resume behavior는 MVP-1 사용자 작업 루프 범위입니다.
...
Stage/profile: optional 내부 엔지니어링 점검 minimal next-action read입니다. 내부 엔지니어링 점검은 `request_acceptance`를 반환하지 않습니다. MVP-1은 더 풍부한 사용자 대상 next action과 smallest-unblocker display를 제공하며, 작업 수락이 progress나 close를 막을 때 `request_acceptance`를 반환할 수 있습니다. 보증 프로필/운영 프로필은 verification, Eval, Manual QA, reconcile, operations, profile-expanded work-acceptance 또는 residual-risk action을 owner profile이 enabled될 때만 추가합니다.
...
| Profile | Profile-scoped response meaning |
|---|---|
| 내부 엔지니어링 점검 minimal | 구현된 경우 scoped write 준비, authorized Run 기록, missing scope/write authority 보고, missing artifact/evidence support 보고 같은 다음 authority-loop action 또는 smallest blocker만 반환합니다. `recommended_playbooks`와 future-profile refs는 empty일 수 있고, owner state가 없으면 `judgment_context`와 `autonomy_boundary`는 `null`일 수 있습니다. |
| MVP-1 user-facing | Ordinary-language next action, user-owned judgment 또는 work acceptance가 필요할 때 pending user judgment refs와 judgment context, evidence/close-readiness context를 반환합니다. 작업 수락이 progress나 close를 막으면 `next_action.action_kind`가 `request_acceptance`일 수 있습니다. 작업 수락 필요/상태와 잔여 위험 표시는 서로 구분되며 별도 projection kind를 요구하지 않습니다. |
| 보증 프로필/운영 프로필 assurance and operations | Verification, Eval, Manual QA, reconcile, projection freshness, operations-oriented next action, profile-expanded work-acceptance 또는 residual-risk behavior는 해당 profile이 enabled되고 matching storage가 있을 때만 반환합니다. |
...
`pending_user_judgments`는 해소되지 않은 user-action judgment를 포함합니다. 현재 phase 또는 requested action에 아직 영향을 주는 deferred, blocked, recently resolved judgment는 `judgment_context.active_user_judgment_refs`를 통해 나타납니다. `pending_decisions`와 `active_decision_packet_refs`는 compatibility alias일 뿐입니다.
...
`recommended_playbooks`는 반환된 next safe action에 맞는 절차를 호출자가 선택하도록 돕습니다. 이는 현재 상태와 policy/playbook context에서 계산되는 API/display guidance일 뿐입니다. `playbook_id`는 display/routing string identifier로 남으며 기준 kernel enum이 아닙니다. 그 자체로 state transition, event, projection, gate, write, evidence, verification, QA, risk, 작업 수락, close 효과를 만들지 않습니다. 사용자 소유 판단을 새로 요구하는 playbook recommendation은 affected write 또는 close가 진행되기 전에 user judgment candidate/request path 또는 existing user judgment로 라우팅해야 합니다. `route.display_route` 값은 display route이지 public tool name이 아니며 상태 변경 tool call 지시도 아닙니다. 전체 Role Lens/playbook 경계는 [Agent Integration](agent-integration.md#role-lens-동작)이 담당합니다.

`RecommendedPlaybook.route`, `judgment_path`, 선택적 구현 prompt-routing rows 같은 routing metadata는 그 자체로 authority가 아닙니다. Caller를 existing user judgment 또는 owner-record path로 안내할 수는 있지만 민감 동작 승인, 사용자 소유 판단, 작업 수락, 면제 판단, 잔여 위험 수용, Write Authorization, close를 충족할 수 없습니다.
...
Stage/profile: 내부 엔지니어링 점검 core authority loop에서 active입니다. Approval candidate는 state를 만들지 않고 반환할 수 있습니다. Minimum MVP-1의 민감 동작 승인은 canonical 민감 동작 승인 user judgment로 해소합니다. Committed Approval record는 later Approval/보증 프로필과 matching storage가 필요합니다.

사용자에게 보이는 의미: 지금 이 정확한 product write를 해도 되는지 답합니다. 이 답은 현재 active Task, active Change Unit scope, Autonomy Boundary, baseline freshness, 민감 동작 permission, user judgment coverage, design policy, surface capability에 기반합니다. `decision=allowed`이면 허용된 작업, 범위 근거, durable single-use Write Authorization ref 또는 summary, 보장 수준의 한계, detective/cooperative limitation을 보여줍니다. Harness 권한 상태상 쓰기가 허용되지 않으면 가장 먼저 해소할 이유와 가장 작은 해소 방법을 보여줍니다. Approval 또는 user judgment candidate payload는 담당 tool path로 commit되기 전까지 candidate일 뿐입니다.
...
`approval_request_candidate`는 `decision=approval_required`이거나 Core가 새 Approval 요청을 제안할 수 있을 때만 포함합니다. 그 외에는 `null`입니다. 이는 이후 `harness.request_user_judgment(judgment_type=sensitive_action_approval)` 호출의 `approval_scope`에 사용할, 상태를 변경하지 않는 candidate입니다. `prepare_write`가 이를 반환해도 Approval 기록, user judgment, Write Authorization, `APR` projection job은 생성되지 않습니다. UI, status response, next-action response가 Approval 요청 commit 전에 이 payload를 표시한다면 이를 candidate 표시로 label해야 하며 `APR` projection이라고 부르면 안 됩니다.
...
Write Authorization은 intended operation과 현재 state, baseline, active Change Unit scope, 민감 동작 coverage ref, user judgment ref, sensitive categories, 보장 수준에 한정됩니다. Approval ref는 Approval owner profile이 active일 때만 포함합니다. 이는 `write_authorization_id`를 통해 `harness.record_run`이 consume하며 재사용 가능한 grant가 아닙니다. 하나의 authorization은 같은 committed `record_run` request의 idempotent replay를 제외하면 하나의 committed implementation 또는 direct Run과만 호환됩니다.

`active_user_judgment_refs`는 intended write와 relevant한 모든 user judgments를 포함합니다. Pending, deferred, blocked, recently resolved judgments가 포함됩니다.

`user_judgment_candidate`는 `decision=decision_required`이고 compatible user judgment가 아직 없을 때 present합니다. Field는 envelope 이후의 `RequestUserJudgmentRequest`와 일치합니다. 여기에는 `judgment_type`, `presentation`, `judgment_payload`가 포함됩니다. 이는 나중에 `harness.request_user_judgment`를 호출하기 위한 non-mutating candidate payload입니다. `prepare_write`가 이를 반환해도 user judgment가 생성되거나 update되지는 않습니다.

상태 전이 요약: Task를 `executing`, `waiting_user`, `blocked`로 옮길 수 있습니다. Allowed일 때 Write Authorization을 생성하거나 idempotent replay에 대해 already committed response를 반환할 수 있습니다. `scope_gate=pending/blocked`, `decision_gate=required/pending/blocked`, `approval_gate=required/expired`, `stale` evidence/Approval marker를 set할 수 있습니다. Minimum MVP-1에서 `approval_gate=pending`은 `harness.request_user_judgment(judgment_type=sensitive_action_approval)`이 민감 동작 승인 user judgment를 만들 때 시작됩니다. Later Approval profile은 linked pending Approval record를 추가로 만들 수 있습니다.
...
Projection job 대기열 추가: Task display나 blocker에 영향을 주는 committed non-dry-run state change에 대해, Task projection support가 범위에 있으면 `TASK`. `prepare_write`는 `decision=approval_required` 또는 `approval_request_candidate`를 반환했다는 이유만으로 `APR`을 대기열에 넣으면 안 됩니다. `APR`은 Approval profile이 active일 때 committed Approval record lifecycle에만 reserved됩니다. Dry-run, state-conflict, pure pre-commit rejection path는 projection job을 대기열에 넣지 않습니다. Idempotent replay는 새 work를 enqueue하지 않고 original job refs를 반환합니다.

Active profile이 validator emission을 켤 때 ValidatorResults emitted: `autonomy_boundary_check`, `decision_gate_check`, `decision_quality_check`, `feedback_loop_check`, `tdd_trace_required`, `codebase_stewardship_check`, applicable design-quality validators, `surface_capability_check`.

Later design/TDD profile이 `tdd_trace_required`를 enabled하면 `prepare_write`는 actual RED evidence와 valid TDD 면제 판단이 없는 non-test implementation write에 design-policy blocker를 보고할 수 있습니다. Intended operation이 failing RED check를 만드는 test-path write라면 scope, baseline, 민감 동작 승인, Autonomy Boundary, other required check가 통과할 때 계속 진행할 수 있습니다. RED target 또는 plan은 그 test-path write를 뒷받침할 수 있지만, non-test implementation write를 위한 RED-evidence precondition이나 Evidence Manifest coverage를 충족하면 안 됩니다. Blocker는 validator 결과, blocked reason, 필요한 경우 secondary error/details로 표현합니다. Primary `ToolError.code`는 API precedence table에 따라 선택합니다.
...
민감 동작 승인은 다음 절차를 따릅니다.

1. `harness.prepare_write`가 intended product write의 sensitive categories를 감지합니다.
2. Scope, baseline, sensitive categories, paths, tools, commands, network targets, secret access, capability 요구사항을 포괄하는 compatible 민감 동작 permission이 없으면 `prepare_write`는 `decision=approval_required`를 반환하고, `approval_request_candidate`를 포함하며, 두 Write Authorization field를 `null`로 두고 `authorization_effect=none`을 사용하며, Task blocker를 update할 수 있습니다. Task display 또는 blocker에 영향을 주는 committed non-dry-run state change이고 Task projection support가 범위에 있을 때만 `TASK`를 대기열에 넣습니다. 이 상태를 변경하지 않는 candidate 때문에 Approval 기록, user judgment, Write Authorization, `APR` projection job을 생성하면 안 됩니다.
3. 호출자는 candidate와 current intended write에서 파생한 `approval_scope`로 `harness.request_user_judgment`를 `judgment_type=sensitive_action_approval`과 함께 호출합니다.
4. Minimum MVP-1에서 Core는 민감 동작 승인을 위한 canonical `user_judgment` record를 생성합니다. Response는 `user_judgment_ref`를 포함하고 `approval_id=null`로 두며, `approval_gate=pending`을 set할 수 있습니다. Approval row, Approval `StateRecordRef`, `approval_refs`, `APR` projection job은 만들지 않습니다.
5. User 또는 operator는 해당 user judgment에 대해 `harness.record_user_judgment`를 호출합니다.
6. Minimum MVP-1에서 Core는 user judgment 해소를 기록하고, 그 민감 동작 승인 judgment에서 `approval_gate`를 granted, denied, expired 중 하나로 다시 계산합니다. 그래도 Write Authorization은 만들지 않습니다. Standalone full-format user judgment projection이 enabled된 경우 `DEC`를 대기열에 넣을 수 있지만 `APR`은 넣지 않습니다.
7. 민감 동작 permission이 granted이면 호출자는 fresh idempotency key와 current `expected_state_version`으로 `harness.prepare_write`를 다시 호출합니다.
8. 그 retry만 Write Authorization을 만들 수 있습니다. Minimum MVP-1에서는 resolved 민감 동작 승인 user judgment의 scope, baseline, sensitive categories, paths, tools, commands, network targets, secret scope, user judgment refs, capability checks가 current intended write와 compatible할 때만 성공합니다.

Later Approval/보증 프로필이 active이면 4단계와 6단계에서 committed Approval record를 추가로 만들거나 update할 수 있고, non-null `approval_id`, `StateRecordRef.record_kind=approval`, `approval_refs`, active APR projection support의 `APR` enqueue가 가능해집니다. 이 behavior는 minimum MVP-1 requirement가 아닙니다.

민감 동작 승인은 정해진 scope 안의 sensitive categories를 허가합니다. 제품 장단점, 설계 방향, 아키텍처 판단이나 기술 판단, 해결되지 않은 security 또는 product-security 판단, 검증 위험, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용 같은 사용자 소유 판단을 해소하지 않습니다. Sensitive action이 사용자 소유의 제품/UX 판단, 기술 판단, 또는 해결되지 않은 security/product-security 판단도 포함하면 Core는 `prepare_write`가 `allowed`를 반환하기 전에 별도의 compatible user judgment를 요구해야 합니다. 민감 동작 승인은 Write Authorization이 아닙니다. 실제 제품 쓰기에는 여전히 allowed `prepare_write` result와 반환된 Write Authorization을 compatible하게 consume하는 `harness.record_run`이 필요합니다. Minimum MVP-1의 민감 동작 승인 prompt와 later Approval profile이 active일 때만의 Approval record는 broad agreement를 묻지 말고 민감 동작, scope, expiry, 아직 결정되지 않은 것을 보여줘야 합니다.
...
Stage/profile: 내부 엔지니어링 점검은 Write Authorization 하나를 consume하는 implementation/direct Run 하나와 artifact/evidence ref 하나를 등록하는 데 active입니다. MVP-1은 같은 method를 user-readable evidence summary에 사용합니다. Evidence Manifest, feedback-loop, TDD, verification-input, 더 풍부한 artifact relation은 보증 프로필 이후 owner profile이 enabled될 때만 active입니다.

Stage별 active request subset:

| Request part | 내부 엔지니어링 점검 active | MVP-1 active | Later-profile only |
|---|---|---|---|
| `RecordRunRequest.kind` | `implementation`, `direct`; 선택한 setup path가 쓰는 경우에만 `shaping_update` | `shaping_update`, `implementation`, `direct` | `verification_input` |
| `artifact_inputs.kind` | [Stage-Specific Active Value Sets](#stage-specific-active-value-sets)의 active values | 같은 표의 active values | 같은 표의 profile-gated artifact kinds |
| `artifact_inputs.relation.record_kind` | `task`, `change_unit`, `run` | 해당 owner record가 있을 때 `task`, `change_unit`, `run`, `user_judgment`, `residual_risk` | 같은 표의 later-profile owner kinds |
| `payload` branches | `implementation`, `direct`; setup에 필요한 minimal `shaping_update` | `shaping_update`, `implementation`, `direct` | `verification_input`, `FeedbackLoopUpdate`, `TddTraceUpdate` branches |
...
Minimum 내부 엔지니어링 점검 / MVP-1 request schema:
...
다음 Feedback Loop와 TDD detail은 later-profile schema branch입니다. 해당 owner profile이 enabled될 때 exact payload를 안정적으로 쓰기 위해 여기에 보존하지만, 내부 엔지니어링 점검 또는 minimum MVP-1 requirement는 아닙니다.
...
Core는 consumed authorization을 observed changed paths, created/deleted paths, artifact inputs와 resolved artifact refs, command results, run summary, baseline, active Change Unit, 민감 동작 permission refs, user judgment refs, sensitive categories, surface guarantee와 비교해 validate합니다. Run summary는 Run을 설명하는 데 도움을 주지만, compatible observed paths, artifacts, authorization basis 없이는 authorized changes의 proof로 받아들이면 안 됩니다.
...
Minimum 내부 엔지니어링 점검 / MVP-1 response schema:
...
`evidence_summary_ref`는 MVP-1 active evidence response ref입니다. Non-null이면 `StateRecordRef.record_kind=evidence_summary`를 사용하고 `evidence_summaries.evidence_summary_id`를 가리킵니다. Minimum 내부 엔지니어링 점검 response에서는 보통 `null`입니다. `evidence_manifest_ref`는 full Evidence Manifest owner profile이 enabled될 때만 `record_kind=evidence_manifest`를 쓰는 later-profile extension입니다. 내부 엔지니어링 점검 / MVP-1 minimum implementation은 이를 validate, store, return할 필요가 없습니다. 내부 엔지니어링 점검 / MVP-1에서 `run_summary_ref`와 `direct_result_ref`가 non-null이면 별도 public record kind를 만들지 않고 committed Run을 `record_kind=run`으로 가리킵니다.
...
Active profile이 validator emission을 켤 때 ValidatorResults emitted: `decision_quality_check`, `autonomy_boundary_check`, `feedback_loop_check`, `tdd_trace_required`, `codebase_stewardship_check`, applicable design-quality validators, `surface_capability_check`.
...
Purpose: progress, write, close를 막거나 MVP-1 work acceptance에 필요한 사용자 판단을 위한 structured `user_judgment` record를 create합니다. Waiver, residual-risk, reconcile path는 active profile이 켜졌을 때 같은 record family를 사용합니다.

Stage/profile: 내부 엔지니어링 점검에서는 active가 아닙니다. MVP-1은 user-owned judgment, 민감 동작 승인, work acceptance가 progress나 close를 막을 때 사용자 대상 판단 요청을 도입합니다. Committed Approval record, approval hardening, QA/verification waiver, full residual-risk acceptance, full-format Decision Packet presentation, reconcile은 owner profile이 active일 때의 later-profile extension입니다.
...
이 method/profile이 active이면 Core는 canonical `UserJudgment`를 저장합니다. 내부 엔지니어링 점검은 이 storage path를 요구하지 않습니다. Public request와 response schema는 optional interaction row가 아니라 `user_judgment`를 중심으로 유지됩니다. 구현이 prompt-routing row를 만들면 그 row는 routing, interaction, replay, compatibility metadata일 뿐입니다. 어떤 gate도 canonical `user_judgment_id`로 다시 연결되기 전에는 그것을 고려하면 안 됩니다.

`state_summary_at_request=null`이면 Core가 같은 transaction 안에서 request-time summary를 파생합니다. `expires_at`은 값이 있을 때 canonical user judgment state에 복사됩니다. `judgment_payload`는 [Shared schemas](#shared-schemas)의 설명처럼 `judgment_type`과 일치해야 합니다. "go ahead", "proceed", "looks good", "좋아", "진행해" 같은 넓은 자연어 답변은 schema branch가 아닙니다. Request는 사용자가 무엇을 판단하는지, judgment type, relevant refs, 답변이 해결하지 않는 것을 이름 붙여야 합니다.

`judgment_type=sensitive_action_approval`에서는 `judgment_payload.approval_scope`가 required입니다. Minimum MVP-1에서 Core는 canonical 민감 동작 승인 user judgment를 만들고 committed Approval record는 만들지 않으므로 response의 `approval_id`는 `null`입니다. Later Approval owner profile은 linked pending Approval record를 추가로 만들 수 있지만, 민감 동작 permission은 `harness.record_user_judgment`가 judgment를 해소하기 전에는 granted가 아닙니다. 이 judgment는 별도 표현이 없는 제품/UX 판단, 기술 판단, 작업 수락, 잔여 위험 수용을 해소하지 않습니다.

`judgment_type=work_acceptance` 또는 `residual_risk_acceptance`에서는 사용자가 acceptance 전에 close-relevant risk를 볼 수 있을 만큼 acceptance visibility 또는 residual-risk refs가 있어야 합니다. Context가 부족하면 request를 block하거나 좁혀야 합니다.
...
`RequestUserDecisionRequest`와 `RequestUserDecisionResponse`는 이 shape의 compatibility alias입니다. Alias payload는 `judgment_domain`, `decision_kind`, `decision_profile`을 `judgment_type`, `presentation`, route-specific payload validation으로 map합니다. 새 example과 future fixture는 judgment 이름을 써야 합니다.

`RequestUserJudgmentResponse.approval_id`는 minimum MVP-1에서 `null`입니다. Later Approval owner profile이 committed Approval record를 만들 때만 값이 채워집니다.

Status와 next-action response가 반환하는 `pending_user_judgments`는 `record_kind=user_judgment`인 해소되지 않은 user-action `StateRecordRef` entry를 포함합니다. `active_user_judgment_refs` field는 pending, deferred, blocked, recently resolved record를 포함해 current phase 또는 requested action과 relevant한 모든 user judgment를 포함합니다.

상태 전이 요약: pending user judgment를 record하고 보통 Task를 `waiting_user`로 옮깁니다. Product/technical judgment는 `decision_gate=pending`을 set할 수 있습니다. Sensitive-action approval judgment는 `approval_gate=pending`을 set할 수 있습니다. Scope/autonomy detail은 technical judgment payload로 표현될 때 `scope_gate` 또는 active Change Unit boundary에 영향을 줄 수 있습니다. Work-acceptance judgment는 `acceptance_gate=pending`을 set하거나 유지할 수 있습니다. Risk acceptance는 이름 붙은 visible risk가 accepted될 때까지 `residual_risk_acceptance` close blocker category를 사용합니다.

implementation-local detail/audit를 위해 반환될 수 있는 non-stable EventRef values: `user_judgment_requested`, `sensitive_action_approval_requested`, `product_judgment_requested`, `technical_judgment_requested`, `work_acceptance_requested`, `residual_risk_acceptance_requested`, `reconcile_judgment_requested`.

Projection job 대기열 추가: source record를 변경하는 committed non-dry-run user judgment creation에서는 Task projection support가 범위에 있으면 `TASK`; standalone full-format user judgment projection이 enabled될 때만 `DEC`; later Approval owner profile이 committed Approval record를 만들거나 바꾸고 APR projection support가 active일 때만 `APR`. Minimum MVP-1 민감 동작 승인 user judgment 자체는 `APR`을 enqueue하지 않습니다. Dry-run, pre-commit state conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다.

ValidatorResults emitted: `decision_quality_check`, active Change Unit boundary에 영향을 주는 judgment에서는 `autonomy_boundary_check`, residual-risk acceptance judgment에서는 `residual_risk_visibility_check`.

Core checks/preconditions: `state_envelope`, `user_judgment_validity`, sensitive-action approval judgment의 `approval_scope`, later reconcile judgment의 `reconcile_required`.
...
Idempotency behavior: repeated request는 같은 user judgment, related records, events, projection jobs를 반환합니다. 같은 key에 다른 judgment payload를 사용하면 `STATE_CONFLICT`입니다.
...
Purpose: pending `user_judgment` record에 대한 사용자 답을 record합니다. Active judgment type/profile이 허용할 때만 accepted residual risk도 기록합니다.

Stage/profile: 내부 엔지니어링 점검에서는 active가 아닙니다. MVP-1은 user-owned judgments, 민감 동작 승인 judgment resolution, work acceptance가 MVP scope에 있을 때 이를 기록합니다. Committed Approval record update, waiver, residual-risk acceptance metadata, reconcile outcome은 later owner profile이 active일 때 필요합니다. 이 method는 existing canonical `UserJudgment`에 대한 답을 기록합니다. Broad approval이나 write authority를 만들지 않습니다.

Allowed actor: `user`, `operator`.

이 request는 existing canonical `UserJudgment`에 답합니다. Caller가 대체 judgment context를 넘겨 저장된 judgment를 바꿀 수 없습니다. Core는 stored judgment의 `judgment_type`, `presentation`, status, refs, `judgment_payload`에 맞춰 answer를 validate합니다.
...
`RecordUserJudgmentRequest.judgment_type`은 stored `UserJudgment.judgment_type`과 일치해야 합니다. Allowed `value` 의미는 judgment type별로 다릅니다. `product_choice`와 `technical_choice`는 `selected`, `rejected`, `deferred`를 씁니다. `sensitive_action_approval`은 `granted`, `denied`, `expired`를 씁니다. `work_acceptance`는 `accepted`, `rejected`를 씁니다. `residual_risk_acceptance`는 `accepted`, `rejected`, `deferred`를 씁니다. Later owner profile은 technical-judgment payload로 waiver 또는 reconcile outcome을 기록하게 할 수 있지만, stored payload branch가 그 path를 명시해야 합니다.

Core는 existing canonical `UserJudgment`에 대해 answer를 validate합니다. Request는 `judgment_type`, `presentation`, `display_label`, `judgment_payload`, affected refs, stored branch를 바꿀 수 없습니다. Free-form note text는 답변을 민감 동작 승인, 작업 수락, waiver, 잔여 위험 수용, write authority로 넓힐 수 없습니다. "proceed", "looks good", "go ahead", "좋아", "진행해" 같은 표현은 pending judgment가 그 judgment type을 명시하고 recorded answer가 allowed value와 일치할 때가 아니면 민감 동작 승인, 작업 수락, 잔여 위험 수용으로 자동 해석되지 않습니다.

`accepted_risks`는 stored residual-risk acceptance judgment와 current Judgment Context가 close-relevant residual risk를 이미 사용자에게 보이게 만든 경우에만 allowed입니다. `AcceptedRiskInput.residual_risk_ref=null`은 current visible context가 같은 transition에서 Core가 Residual Risk record를 만들거나 연결하기에 충분할 때만 allowed입니다. Visibility 또는 context가 없으면 Core는 hidden risk를 조용히 create하고 accept하지 말고 reject 또는 block해야 합니다.
...
상태 전이 요약: targeted user judgment는 resolved, deferred, rejected, blocked 상태로 처리됩니다. Affected gate 또는 later-profile reconcile item을 업데이트합니다. Minimum MVP-1에서 민감 동작 승인 grant/deny는 `UserJudgment`와 `approval_gate`를 업데이트하지만 Write Authorization이나 committed Approval record를 만들지 않습니다. Approval owner profile이 active이면 같은 judgment가 linked Approval record도 업데이트할 수 있습니다. User-resolved product 또는 technical judgment는 `decision_gate` 또는 관련 owner state를 업데이트합니다. Verification 또는 QA waiver는 relevant owner profile이 accepted한 active technical-judgment waiver payload로만 표현될 수 있습니다. Work acceptance는 `acceptance_gate`를 업데이트합니다. Accepted residual risk는 assurance를 높이지 않고 Residual Risk records를 업데이트합니다.

implementation-local detail/audit를 위해 반환될 수 있는 non-stable EventRef values: `user_judgment_recorded`, `user_judgment_resolved`, `user_judgment_deferred`, `user_judgment_rejected`, `sensitive_action_approval_granted`, `sensitive_action_approval_denied`, `product_judgment_recorded`, `technical_judgment_recorded`, `work_acceptance_recorded`, `residual_risk_accepted`, `reconcile_resolved`.

Projection job 대기열 추가: source record를 변경하는 committed non-dry-run user-owned judgment record에서는 Task projection support가 범위에 있으면 `TASK`; standalone full-format Decision Packet presentation이 enabled될 때만 `DEC`; later Approval owner profile이 committed Approval record를 update하고 APR projection support가 active일 때만 `APR`; QA waiver가 QA record로 represented되고 Manual QA projection support가 active일 때 `MANUAL-QA`. Minimum MVP-1 민감 동작 승인 judgment 자체는 `APR`을 enqueue하지 않습니다. Dry-run, pre-commit state conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다.

ValidatorResults emitted: `decision_quality_check`, `autonomy_boundary_check`, `residual_risk_visibility_check`.

Core checks/preconditions: `state_envelope`, `pending_user_judgment_exists`, `approval_scope`, `qa_waiver_reason`, `reconcile_target_validity`.
...
Idempotency behavior: repeated judgment는 같은 user judgment resolution, accepted-risk refs, updated records, events를 반환합니다. 같은 key로 이미 recorded judgment를 바꾸려 하면 `STATE_CONFLICT`를 반환합니다.
...
Stage/profile: 내부 엔지니어링 점검은 implementation이 그 경로를 선택한 경우 narrow structured blocker/status smoke로만 사용할 수 있습니다. MVP-1은 close state와 blocker display를 요구합니다. 보증 프로필 이후는 profile이 active일 때 full assurance, QA, waiver, accepted-risk, projection/report freshness semantics를 추가합니다.
...
Close response block은 profile-gated superset입니다. Minimum MVP-1 close는 core close state, blocker, residual-risk, acceptance, 그리고 기존 ref array에 반환되는 관련 `evidence_summary` 또는 `close_readiness` `StateRecordRef` entry를 사용합니다. Verification, 수동 QA, projection/report, operations ref는 matching profile이 enabled될 때만 active입니다.
...
Close blockers에는 unresolved, missing, deferred-without-coverage, blocked, rejected, stale, incompatible user judgments, missing evidence, active Runs, active profile에서만 요구되는 verification, active profile에서만 요구되는 Manual QA, active일 때 missing work acceptance, visible하지 않은 known close-relevant residual risk, requested close reason 때문에 acceptance가 아직 필요한 visible close-relevant risk가 포함됩니다. Known close-relevant residual risk가 없으면 `ResidualRiskSummary.status=none`이 residual-risk visibility를 충족하며 close blocker가 아닙니다.
...
다음 method들은 future/later-profile public API details입니다. 내부 엔지니어링 점검 requirement가 아니며, owner profile이 명시적으로 active하지 않는 한 minimum MVP-1 user-value slice에도 포함되지 않습니다.
...
Stage/profile: 보증 프로필 behavior입니다. 내부 엔지니어링 점검에는 required가 아니며, 그 자체로 detached assurance를 만들지 않습니다. `harness.record_eval`이 나중에 qualify할 수 있는 candidate/bundle을 만듭니다.
...
Stage/profile: 보증 프로필 behavior입니다. Eval recording이 enabled될 때 required field가 적용됩니다. Same-session check, self-check summary, passed command는 이 method가 qualifying Eval을 기록하고 Core가 gate/assurance state를 update하기 전까지 detached verification이 아닙니다.
...
Stage/profile: Manual QA policy/profile이 enabled된 보증 프로필 behavior입니다. 내부 엔지니어링 점검에서는 active가 아니며, browser capture나 note는 human QA record 또는 valid waiver path를 대체하지 않습니다.
...
`result=waived`에서 product/user risk 또는 policy-required judgment가 있으면 `waiver_user_judgment_ref`가 reference하는 compatible technical user judgment가 필요합니다. `waiver_reason`만으로 가능한 경우는 policy가 허용한 low-risk waiver에 한정됩니다. 복잡한 waiver에는 full-format Decision Packet presentation을 사용할 수 있지만 canonical record는 `user_judgment`입니다.
...
Projection job 대기열 추가: source record를 변경하는 committed non-dry-run 수동 QA record에서는 task-summary projection support가 범위에 있으면 `TASK`; Manual QA profile이 active일 때 `MANUAL-QA`; standalone full-format Decision Packet projection이 enabled되고 waiver user judgment가 visibility에 영향을 줄 때만 `DEC`; active profile이 해당 diagnostic projection을 포함할 때 optional `EVIDENCE-MANIFEST`. Dry-run, pre-commit state-conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다. Waiver user judgment visibility는 여전히 status/next response, judgment-context resource, user-judgment resource, 최소 `TASK` 또는 card display를 통해 나타납니다.
