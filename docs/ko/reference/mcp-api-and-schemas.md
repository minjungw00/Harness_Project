...
Shared schema를 읽기 전에 이 manifest를 먼저 봅니다. v0.1/v0.2 구현 경로는 아래 active API surface와 [ArtifactRef](#artifactref)의 stage-specific value set만 사용합니다. Later-profile method detail과 future schema appendix는 정확한 참조 shape를 보존하지만 더 이른 stage의 requirement를 늘리지 않습니다.
...
| `harness.next` | Optional smoke read. | Optional expanded/compatibility read입니다. 작업 수락이 progress나 close를 막을 때 `request_acceptance`를 반환할 수 있고, preferred v0.2 path는 `harness.status.next_actions`입니다. | Verification, Eval, Manual QA, reconcile, operations, profile-expanded work-acceptance 또는 residual-risk next-action context는 owner profile이 active일 때만 포함합니다. |
...
### 코어 권한 스모크(v0.1 Core Authority Smoke) resources
...
### 첫 사용자 가치 조각(v0.2 First User-Value Slice) resources

v0.2는 v0.1 current-status resource를 유지하고, 일반 사용자가 현재 작업 상태, 대기 중인 사용자 판단, 근거 요약, 닫기 준비 상태, 닫기 막힘, 관련될 때의 작업 수락 필요/상태와 잔여 위험 표시를 이해할 수 있게 그 summary를 확장할 수 있습니다. Minimum v0.2 path에는 detailed report resource가 필요하지 않습니다.
...
이 resource들은 profile-gated assurance read입니다. 첫 조각이나 minimum First User-Value Slice 요구사항이 아닙니다.
...
|---|---|---|
| `ToolEnvelope`, `ToolResponseBase`, `ToolError`, `StateSummary`, `EventRef` | v0.1 Core Authority Smoke | Active payload에서 쓰는 공통 contract shape입니다. |
| `WriteAuthorizationSummary`, `WriteAuthoritySummary`, `ApprovalScope` | v0.1 core write authority. Approval record는 v0.3 Agency Assurance | Write Authorization summary는 allowed write가 만들어질 때 v0.1 범위입니다. Minimum v0.2에서는 Approval 형태 Decision Packet coverage가 Decision Packet ref로 표현됩니다. Approval-specific refs와 lifecycle은 Approval profile이 켜지기 전까지 later-profile입니다. |
...
| `DecisionPacket`, `DecisionPacketCandidate`, `JudgmentContext`, `AcceptanceVisibilityContext` | v0.2 First User-Value Slice. Full approval/waiver/risk/reconcile profile은 v0.3/v0.4 | Decision Packet 또는 candidate가 만들어질 때 required field가 적용됩니다. v0.1은 Decision Packet storage를 요구하지 않습니다. |
...
`bundle`과 `manifest`는 verification/evidence bundle profile에서 사용합니다. `qa_capture`는 Manual QA/capture profile에서 사용합니다. `export_component`는 operations/export profile에서만 active입니다. Design probe, prototype, architecture scan, decision-context artifact는 future 또는 owner-promoted candidate이며 v0.1이나 minimum v0.2 artifact requirement가 아닙니다.
...
`record_kind=approval`은 later-profile reference kind입니다. Storage/DDL이 `approvals` table을 명시적으로 승격하고 Approval owner profile이 active인 경우가 아니라면 minimum v0.2 implementation은 이 값을 반환하면 안 됩니다. Minimum v0.2의 Approval 형태 민감 동작 승인은 `record_kind=decision_packet`으로 참조합니다.
...
`WriteAuthorizationSummary.approval_refs`는 minimum v0.2에서 비어 있습니다. Resolved Approval 형태 Decision Packet이 민감 동작 coverage를 제공하면 compatible packet은 `decision_packet_refs`에 들어갑니다. Committed Approval refs는 Approval owner profile이 active일 때만 반환합니다.
...
| v0.3/v0.4 later profiles | matching waiver, residual-risk acceptance, reconcile owner profile이 enabled될 때만 `waive`, `accept-risk`, `reconcile` |

v0.2 route set은 사용자 소유 선택, deferral, sensitive-action approval prompt, work acceptance를 다룹니다. Full QA/verification waiver, full residual-risk acceptance, reconcile machinery는 정확한 field가 shared schema에 남아 있어도 later-profile route입니다.
...
| `harness.prepare_write` | Active product-write authorization check. | 하나의 allowed/blocked path와, allowed일 때 durable single-use Write Authorization 하나를 만듭니다. Minimum v0.2는 resolved Approval 형태 Decision Packet coverage를 사용할 수 있고, committed Approval record는 later-profile입니다. |
...
| `harness.next` | Optional v0.1 read. | Smoke path에서 구현한다면 다음 minimal authority-loop action 또는 smallest unblocker만 반환합니다. v0.1은 `ask_user`, `request_acceptance`, verification, QA, reconcile action kind를 반환하지 않습니다. `ask_user`와 `request_acceptance`는 v0.2에서 들어오고, verification, QA, reconcile action kind는 action-kind stage table에 따라 later owner profile이 enabled될 때만 들어옵니다. |
...
| `harness.record_run` | Active evidence/artifact summary path. | v0.2 사용자 가치 경로에서 evidence가 충분히 visible해야 합니다. Full Evidence Manifest projection과 assurance record는 명시적으로 enabled되기 전까지 later-profile입니다. |
...
| Minimal `TASK` projection 또는 compact status/card output | Active display capability입니다. Persisted projection job일 필요는 없습니다. | Status/next/card output이 v0.2의 최소 사용자 읽기용 summary를 충족하면 persisted Markdown rendering은 optional입니다. |
...
v0.1/v0.2 경로에서 쓰는 current staged method index:
...
Later-profile method index입니다. 이 method section들은 reference stability를 위해 남아 있지만 v0.1 또는 minimum v0.2에서는 active가 아닙니다.

| Tool | Later-profile use |
|---|---|
| [`harness.launch_verify`](#harnesslaunch_verify) | Agency Assurance가 enabled될 때 verification launch request/response와 bundle ref |
...
Stage/profile: v0.1은 implementation이 public API setup을 선택한 경우 minimal owner-valid Task/scope setup path로만 이 method를 사용할 수 있습니다. Full natural-language intake, discovery quality, procedural budget display, ordinary-user resume behavior는 v0.2 First User-Value Slice 범위입니다.
...
Stage/profile: optional v0.1 minimal next-action read입니다. v0.1은 `request_acceptance`를 반환하지 않습니다. v0.2는 더 풍부한 사용자 대상 next action과 smallest-unblocker display를 제공하며, 작업 수락이 progress나 close를 막을 때 `request_acceptance`를 반환할 수 있습니다. v0.3/v0.4는 verification, Eval, Manual QA, reconcile, operations, profile-expanded work-acceptance 또는 residual-risk action을 owner profile이 enabled될 때만 추가합니다.
...
| v0.2 user-facing | Ordinary-language next action, user-owned judgment 또는 work acceptance가 필요할 때 pending Decision Packet refs와 judgment context, evidence/close-readiness context를 반환합니다. 작업 수락이 progress나 close를 막으면 `next_action.action_kind`가 `request_acceptance`일 수 있습니다. 작업 수락 필요/상태와 잔여 위험 표시는 서로 구분되며 별도 projection kind를 요구하지 않습니다. |
| v0.3/v0.4 assurance and operations | Verification, Eval, Manual QA, reconcile, projection freshness, operations-oriented next action, profile-expanded work-acceptance 또는 residual-risk behavior는 해당 profile이 enabled되고 matching storage가 있을 때만 반환합니다. |
...
Stage/profile: v0.1 core authority loop에서 active입니다. Approval candidate는 state를 만들지 않고 반환할 수 있습니다. Minimum v0.2의 민감 동작 승인은 기준 Approval 형태 Decision Packet으로 해소합니다. Committed Approval record는 later Approval/Agency Assurance profile과 matching storage가 필요합니다.
...
4. Minimum v0.2에서 Core는 Approval 형태 사용자 판단을 위한 기준 Decision Packet을 생성합니다. Response는 `decision_packet_ref`를 포함하고 `approval_id=null`로 두며, `approval_gate=pending`을 set할 수 있습니다. Approval row, Approval `StateRecordRef`, `approval_refs`, `APR` projection job은 만들지 않습니다.
5. User 또는 operator는 해당 Decision Packet에 대해 `harness.record_user_judgment`를 호출합니다.
6. Minimum v0.2에서 Core는 Decision Packet 해소를 기록하고, 그 Approval 형태 packet에서 `approval_gate`를 granted, denied, expired 중 하나로 다시 계산합니다. 그래도 Write Authorization은 만들지 않습니다. Standalone Decision Packet projection이 enabled된 경우 `DEC`를 대기열에 넣을 수 있지만 `APR`은 넣지 않습니다.
...
8. 그 retry만 Write Authorization을 만들 수 있습니다. Minimum v0.2에서는 resolved Approval 형태 Decision Packet의 scope, baseline, sensitive categories, paths, tools, commands, network targets, secret scope, Decision Packet refs, capability checks가 current intended write와 compatible할 때만 성공합니다.

Later Approval/Agency Assurance profile이 active이면 4단계와 6단계에서 committed Approval record를 추가로 만들거나 update할 수 있고, non-null `approval_id`, `StateRecordRef.record_kind=approval`, `approval_refs`, active APR projection support의 `APR` enqueue가 가능해집니다. 이 behavior는 minimum v0.2 requirement가 아닙니다.

민감 동작 승인은 정해진 scope 안의 sensitive categories를 허가합니다. 제품 장단점, 설계 방향, 아키텍처 판단이나 기술 구조 판단, 해결되지 않은 security 또는 product-security 판단, 검증 위험, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용 같은 사용자 소유 판단을 해소하지 않습니다. Sensitive action이 사용자 소유의 제품 판단, 기술 구조 판단이나 아키텍처 판단, 또는 해결되지 않은 security/product-security 판단도 포함하면 Core는 `prepare_write`가 `allowed`를 반환하기 전에 별도의 compatible Decision Packet을 요구해야 합니다. 민감 동작 승인은 Write Authorization이 아닙니다. 실제 제품 쓰기에는 여전히 allowed `prepare_write` result와 반환된 Write Authorization을 compatible하게 consume하는 `harness.record_run`이 필요합니다. Minimum v0.2의 Approval 형태 prompt와 Decision Packet, later Approval profile이 active일 때만의 Approval record는 broad agreement를 묻지 말고 민감 동작, scope, expiry, 아직 결정되지 않은 것을 보여줘야 합니다.
...
Purpose: progress, write, close를 막거나 v0.2 work acceptance에 필요한 user judgment를 위한 structured Decision Packet을 create합니다. Waiver, residual-risk, reconcile path는 active profile이 켜졌을 때 같은 route를 사용합니다.

Stage/profile: v0.1에서는 active가 아닙니다. v0.2는 user-owned judgment, 민감 동작 승인, work acceptance가 progress나 close를 막을 때 사용자 대상 Decision Packet prompt를 도입합니다. Committed Approval record, approval hardening, QA/verification waiver, full residual-risk acceptance, reconcile은 owner profile이 active일 때의 later-profile extension입니다.
...
`RequestUserJudgmentResponse.approval_id`는 minimum v0.2에서 `null`입니다. Later Approval owner profile이 committed Approval record를 만들 때만 값이 채워집니다.
...
Projection job 대기열 추가: source record를 변경하는 committed non-dry-run Decision Packet creation에서는 Task projection support가 범위에 있으면 `TASK`; standalone Decision Packet projection이 enabled될 때만 `DEC`; later Approval owner profile이 committed Approval record를 만들거나 바꾸고 APR projection support가 active일 때만 `APR`. Minimum v0.2 Approval 형태 Decision Packet 자체는 `APR`을 enqueue하지 않습니다. Dry-run, pre-commit state conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다.
