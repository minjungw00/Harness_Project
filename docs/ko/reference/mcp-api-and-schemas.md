...
Public tool response는 local-security claim failure를 existing response shape로 보이게 해야 합니다.

| Condition | Response guidance |
|---|---|
| `project_id`, `task_id`, `surface_id`가 resolve되지 않거나, addressed project 밖으로 resolve되거나, tool-specific Task 또는 owner record와 충돌합니다. | Mutation 전에 reject합니다. Primary `ErrorCode`는 existing precedence table에서 선택하고, 구체적인 claim mismatch는 `ToolError.details`, blocked reason, state summary, validator/check output에 둡니다. Public spoofing-specific code를 추가하지 않습니다. |
| `actor_kind`가 `user`, `operator`, `evaluator`라고 claim하지만 request path가 user acceptance, Manual QA, Approval, detached verification independence를 충족할 수 없습니다. | Relevant gate를 unsatisfied로 유지하고, tool에 따라 `ACCEPTANCE_REQUIRED`, `QA_REQUIRED`, `APPROVAL_REQUIRED`, `DECISION_REQUIRED`, `VERIFY_NOT_DETACHED`, `CAPABILITY_INSUFFICIENT`, 또는 validator result 같은 existing blocker를 사용합니다. Actor claim은 audit context이지 judgment의 proof가 아닙니다. |
| MCP endpoint가 off-profile, weak, stale, forwarded, tunneled, 또는 unknown access mode를 통해서만 reachable합니다. | Core 또는 operator가 condition을 classify할 수 있으면 existing `MCP_UNAVAILABLE` 또는 `CAPABILITY_INSUFFICIENT`를 사용하고, 가능한 access-mode fact를 `details` 또는 guarantee display에 포함합니다. Core에 닿을 수 없다면 authoritative Core response나 mutation을 주장할 수 없습니다. |
...
같은 key를 다른 canonical request payload와 함께 reuse하면 `STATE_CONFLICT` response는 민감한 request body를 노출하지 않으면서 replay 문제를 진단할 수 있게 해야 합니다. `ToolError.details`에는 idempotency scope, stored/received request hash 또는 그에 준하는 opaque comparison, caller가 original request를 replay하거나 fresh key로 retry해야 한다는 사실을 포함할 수 있습니다.
...
최신이 아닌 `expected_state_version`은 caller identity의 proof가 아니라 concurrency drift로 보고합니다. Diagnostic display는 어떤 scope가 stale이었는지, Core가 관찰한 current version이 무엇인지, retry 전에 caller가 refresh해야 한다는 점을 말해야 합니다. Caller가 제공했다는 이유로 오래된 Task 또는 project view를 받아들이면 안 됩니다.
