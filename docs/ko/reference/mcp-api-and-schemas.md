...
| `harness.next` | Optional v0.1 read. | Smoke path에서 구현한다면 다음 minimal authority-loop action 또는 smallest unblocker만 반환합니다. v0.1은 `ask_user`, `request_acceptance`, verification, QA, reconcile action kind를 반환하지 않습니다. 이 값들은 action-kind stage table에 따라 v0.2 또는 later owner profile에서만 사용됩니다. |
...
Stage/profile: optional v0.1 minimal next-action read입니다. v0.2는 더 풍부한 사용자 대상 next action과 smallest-unblocker display를 제공하며, 작업 수락이 progress나 close를 막을 때 `request_acceptance`를 반환할 수 있습니다. v0.3/v0.4는 verification, Eval, Manual QA, reconcile, operations, profile-expanded acceptance/risk behavior를 owner profile이 enabled될 때만 추가합니다.
...
| v0.2 user-facing | Ordinary-language next action, user-owned judgment가 필요할 때 pending Decision Packet refs와 judgment context, evidence/close-readiness context를 반환합니다. 작업 수락이 progress나 close를 막으면 `next_action.action_kind`가 `request_acceptance`일 수 있습니다. 작업 수락과 잔여 위험 사실은 acceptance visibility와 함께 나타나며 별도 projection kind를 요구하지 않습니다. |
| v0.3/v0.4 assurance and operations | Verification, Eval, Manual QA, reconcile, projection freshness, operations-oriented next action, profile-expanded acceptance/risk behavior는 해당 profile이 enabled되고 matching storage가 있을 때만 반환합니다. |
...
Purpose: progress, write, close를 막는 user judgment를 위한 structured Decision Packet을 create합니다. Active profile이 켜져 있으면 acceptance, waiver, risk, reconcile path도 다룹니다.
