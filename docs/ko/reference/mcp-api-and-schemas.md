...
v0.2의 user-facing MVP surface는 `judgment_domain`을 display/grouping에 일관되게 보여 주면서 `decision_kind`를 lifecycle과 gate route로 유지해야 합니다. 또한 사용자가 close readiness를 이해할 수 있을 만큼 작업 수락과 잔여 위험 표시 field가 필요합니다. 최소 사용자 읽기용 output set은 현재 작업 상태, 판단 요청, 근거 요약, 닫기 준비 상태 / blocker 요약입니다. 분리 검증 독립성, full 수동 QA 정책 매트릭스, stewardship validators, export/recover, release handoff, detailed projection kinds는 owner doc이 subset을 명시적으로 승격하지 않는 한 이후 profile입니다.
...
Purpose: project, surface, active Task, 간결한 현재 위치 요약, gate, guarantee, projection freshness, active Decision Packet, Autonomy Boundary, Write Authority Summary, residual-risk, pending-decision status를 반환합니다.
