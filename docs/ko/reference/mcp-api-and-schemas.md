...
Public MCP 계약의 MVP 기본값은 등록된 project surface에 대한 local-only 노출입니다. Local-only란 로컬 프로세스, 로컬 socket, 또는 localhost loopback 연결을 뜻합니다. 인증되지 않은 shared endpoint, non-loopback bind, forwarded/tunneled endpoint, 등록된 connector profile로 설명되지 않는 remote caller는 제외됩니다. MCP server를 이 로컬 경계 밖에 노출하면 위협 모델이 달라지므로 문서화된 connector capability profile, 접근 제어 계약, secret/PII 처리 정책, 보장 수준 표시, conformance coverage가 필요합니다. 그런 더 강한 profile이 없다면 MCP endpoint에 닿을 수 있는 호출자도 Core가 검증해야 하는 claim을 보낸 출처일 뿐, 자동으로 신뢰되는 권한이 아닙니다.

접근 제어 계약은 localhost-only binding, local file permission으로 제한된 Unix-domain socket 또는 다른 local socket, in-process 또는 stdio transport, per-project token, process-scoped configuration material, 또는 이에 준하는 로컬 제어 같은 여러 방식으로 구현될 수 있습니다. 이 예시는 schema enum이 아니며 하나의 CLI syntax를 요구하지 않습니다. Public API 계약에서 중요한 점은 호출자의 access mode가 등록된 surface profile과 맞아야 하고, 변경 요청 전에 Core가 모든 envelope claim을 계속 검증한다는 것입니다.
