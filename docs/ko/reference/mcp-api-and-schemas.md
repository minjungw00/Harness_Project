...
- `one_of:`는 branch object를 정의합니다. 나열된 child field 중 정확히 하나만 present이고, 그 value는 해당 child field의 stated type과 일치해야 합니다. Branch prose는 selected branch가 `decision_profile` 같은 discriminator field와 일치해야 한다고 추가로 요구할 수 있습니다. 선택되지 않은 모든 branch는 `null`로 present가 아니라 absent여야 합니다.
...
v0.1 Core Authority Slice에서는 이 API를 하나의 authority loop에 필요한 최소 subset으로 읽습니다. Status/blocker read, owner-valid path를 통한 local project와 Task setup, scoped work boundary 하나, `prepare_write`, single-use Write Authorization 하나, compatible `record_run` 하나, artifact/evidence ref 하나, structured blocker/status response가 범위입니다. Full Decision Packet quality, full Evidence Manifest, 수동 QA, 분리 검증, 작업 수락, 잔여 위험 수용, recover/export, broad operator flow, rich projection rendering을 위한 tool과 response 의미는 future-profile capability이지 v0.1 exit criteria가 아닙니다.
...
이 request는 existing canonical Decision Packet에 답하는 것입니다. Caller가 저장된 값을 바꾸는 replacement `decision_profile`을 넘길 수 없습니다. Core는 stored packet의 `decision_kind`, `decision_profile`, status, refs, compatible payload branch에 맞춰 answer를 validate합니다.
