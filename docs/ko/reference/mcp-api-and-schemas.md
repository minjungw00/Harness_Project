...
- `harness.prepare_write`는 "지금 이 정확한 제품 파일 쓰기를 해도 되는가?"에 답합니다.
- `harness.record_run`은 "무슨 일이 일어났고, 어떤 evidence 또는 artifact가 기록됐으며, 다음은 무엇인가?"에 답합니다.
- `harness.close_task`는 "이 Task를 지금 끝내거나 취소할 수 있는가, 아니라면 무엇이 막는가?"에 답합니다.
...
Public MCP 계약은 등록된 project surface를 위한 로컬 프로세스 또는 localhost 연결을 전제로 합니다. MCP server를 이 로컬 경계 밖에 노출하면 위협 모델이 달라지므로, 문서화된 connector capability profile, 접근 제어 계약, 보장 수준 표시가 필요합니다. 그런 더 강한 profile이 없다면 MCP 엔드포인트에 닿을 수 있는 호출자도 Core가 검증해야 하는 claim을 보낸 출처일 뿐, 자동으로 신뢰되는 권한이 아닙니다.

접근 제어 계약은 localhost-only binding, local file permission으로 제한된 Unix-domain socket, per-project token, process-scoped configuration material, 또는 이에 준하는 로컬 제어 같은 여러 방식으로 구현될 수 있습니다. 이 예시는 schema enum이 아니며 하나의 CLI syntax를 요구하지 않습니다. Public API 계약에서 중요한 점은 호출자의 access mode가 등록된 surface profile과 맞아야 하고, 변경 요청 전에 Core가 모든 envelope claim을 계속 검증한다는 것입니다.
...
- `idempotency_key`는 committed mutation의 중복을 막습니다. Authorization token이 아니며, 같은 `(project_id, tool_name, idempotency_key)` scope에서 같은 canonical request payload일 때만 replay로 유효합니다.
- `expected_state_version`은 호출자의 concurrency claim입니다. 최신이 아니거나 잘못된 version은 `STATE_CONFLICT`를 반환합니다. 이를 이용해 오래된 Task 또는 project view가 이기게 만들면 안 됩니다.
...
State-changing tool에서 Core는 `expected_state_version`을 현재 project/Task state와 비교합니다. 일치하지 않으면 `STATE_CONFLICT`를 반환하고 `details`에 현재 state version과 status summary를 포함합니다. 호출자는 상태를 새로 읽은 뒤 새 idempotency key로 retry하거나 exact previous request를 replay해야 합니다.
...
사용자에게 보이는 의미: 지금 이 정확한 product write를 해도 되는지 답합니다. `decision=allowed`이면 허용된 작업, 범위 근거, Write Authorization ref 또는 summary, 보장 수준의 한계, detective/cooperative limitation을 보여줍니다. 쓰기가 blocked이면 가장 먼저 해소할 이유와 가장 작은 해소 방법을 보여줍니다. Approval 또는 Decision Packet candidate payload는 담당 tool path로 commit되기 전까지 candidate일 뿐입니다.
...
Write Authorization은 intended operation과 현재 state, baseline, active Change Unit scope, Approval ref, Decision Packet ref, sensitive categories, 보장 수준에 한정됩니다. 이는 `write_authorization_id`를 통해 `harness.record_run`이 consume하며 재사용 가능한 grant가 아닙니다.
