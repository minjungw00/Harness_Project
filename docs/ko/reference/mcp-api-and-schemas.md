...
Request 또는 response schema에서 required로 표시된 field는 해당 tool, record, profile이 구현되고 사용될 때 required입니다. 이 field들이 모두 v0.1 Core Authority Slice exit criteria라는 뜻은 아닙니다. 단계별 전달 계획은 Build가 담당합니다. Stage scope는 [Build: MVP 계획](../build/mvp-plan.md#계약-필드-단계-구분)과 [첫 실행 가능한 조각](../build/first-runnable-slice.md)을 사용합니다.
...
Stage/profile requiredness: 어떤 stage 또는 profile이 `DecisionPacket`이나 사용자 표시 `DecisionPacketCandidate`를 만들면 `judgment_domain`과 `decision_profile`은 모두 required이며 아래 값 중 하나여야 합니다. Minimal 코어 권한 조각(v0.1) 구현은 optional `decision_requests` table을 생략할 수 있지만, committed Decision Packet state에서 `judgment_domain` 또는 `decision_profile`을 생략하면 안 됩니다. 사용자 대상 하네스 MVP(v0.2) display는 저장된 값에서 자연스러운 label을 렌더링합니다.
