...
Projection job 대기열 추가: `TASK`. `prepare_write`는 `decision=approval_required` 또는 `approval_request_candidate`를 반환했다는 이유만으로 `APR`을 대기열에 넣으면 안 됩니다. `APR`은 기록된 Approval 기록과 approval-shaped Decision Packet lifecycle에만 reserved됩니다.
