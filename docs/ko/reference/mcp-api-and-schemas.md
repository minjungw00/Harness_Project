...
| `harness.request_user_judgment` | Not active. | User-owned judgment, Approval 형태 민감 동작 승인, work-acceptance prompt에 active. | Committed Approval record, approval hardening, waiver, full residual-risk acceptance, reconcile profile을 추가할 수 있습니다. |
| `harness.record_user_judgment` | Not active. | Approval 형태 packet을 포함해 Decision Packet에 대한 사용자 답 기록에 active. | Committed Approval lifecycle, waiver, risk-acceptance, reconcile record update를 추가할 수 있습니다. |
...
내부 엔지니어링 점검은 Journey, Journey Spine, Decision Packet storage, Evidence Manifest, bundle, design map, module map, interface contract, report, projection job, projection renderer를 요구하지 않습니다.
...
| `WriteAuthorizationSummary`, `WriteAuthoritySummary`, `ApprovalScope` | 내부 엔지니어링 점검 core write authority. Approval record는 보증 프로필 | Write Authorization summary는 allowed write가 만들어질 때 내부 엔지니어링 점검 범위입니다. Minimum MVP-1에서는 Approval 형태 Decision Packet coverage가 Decision Packet ref로 표현됩니다. Approval-specific refs와 lifecycle은 Approval profile이 켜지기 전까지 later-profile입니다. |
...
| `DecisionPacket`, `DecisionPacketCandidate`, `JudgmentContext`, `AcceptanceVisibilityContext` | MVP-1 사용자 작업 루프. Full approval/waiver/risk/reconcile profile은 보증 프로필/운영 프로필 | Decision Packet 또는 candidate가 만들어질 때 required field가 적용됩니다. 내부 엔지니어링 점검은 Decision Packet storage를 요구하지 않습니다. |
...
| `data_export` | Report, file, external system, support bundle, user/operator download 등으로 현재 boundary 밖에 data를 내보내는 작업. | Approval은 어떤 data가 어디로 나가는지, destination, retention, expiry를 포괄합니다. Field selection, recipient authority, redaction, omitted value, audit trail, export와 관련해 받아들이는 위험에는 Decision Packet을 사용합니다. Evidence에는 evidence summary ref, export artifact ref, redaction state를 보존하는 ref를 포함하고, full Evidence Manifest profile이 active일 때만 Evidence Manifest를 포함합니다. |
...
MVP-1 sensitive-action approval은 user-facing route에서 Decision Packet ref로 표현할 수 있습니다. Committed `approval` ref는 Approval owner profile이 켜지기 전까지 later-profile입니다. `projection` ref는 해당 owner profile의 projection support가 있을 때만 valid합니다. Projection은 계속 derived view이며 authority가 아닙니다.
...
`record_kind=approval`은 later-profile reference kind입니다. Storage/DDL이 `approvals` table을 명시적으로 승격하고 Approval owner profile이 active인 경우가 아니라면 minimum MVP-1 implementation은 이 값을 반환하면 안 됩니다. Minimum MVP-1의 Approval 형태 민감 동작 승인은 `record_kind=decision_packet`으로 참조합니다.
...
`WriteAuthorizationSummary.approval_refs`는 minimum MVP-1에서 비어 있습니다. Resolved Approval 형태 Decision Packet이 민감 동작 coverage를 제공하면 compatible packet은 `decision_packet_refs`에 들어갑니다. Committed Approval refs는 Approval owner profile이 active일 때만 반환합니다.
...
공통 judgment model은 세 축입니다. Full schema는 참조 안정성을 위해 later route를 보존하지만, staged implementation은 active profile이 켠 route value만 validate합니다.
...
|---|---|
| MVP-1 사용자 작업 루프 | `choose`, `defer`, `approve-sensitive-action`, `accept-result` |
| 보증 프로필/운영 프로필 later profiles | matching waiver, residual-risk acceptance, reconcile owner profile이 enabled될 때만 `waive`, `accept-risk`, `reconcile` |

MVP-1 route set은 사용자 소유 선택, deferral, sensitive-action approval prompt, work acceptance를 다룹니다. Full QA/verification waiver, full residual-risk acceptance, reconcile machinery는 정확한 field가 shared schema에 남아 있어도 later-profile route입니다.
...
| `decision_kind` | `judgment_route`. Product, architecture, design, scope choice는 `choose`; sensitive-action approval은 `approve-sensitive-action`; work acceptance는 `accept-result`입니다. QA/verification waiver의 `waive`, residual-risk acceptance의 `accept-risk`, reconcile의 `reconcile`은 해당 later route가 enabled될 때만 사용합니다. |
...
아래 schema block은 이 문서의 YAML-like notation만 사용합니다. `JudgmentPayload`의 route-specific field는 active `judgment_route`로 validate합니다. `approval_scope`는 `approve-sensitive-action`에서만 필요하고, `acceptance`는 `accept-result`에서만 필요합니다. Later-profile branch는 해당 route가 enabled될 때만 적용합니다. `waiver`는 `waive`, `residual_risk_acceptance`는 `accept-risk`, `reconcile`은 `reconcile`에서만 필요합니다. `display_depth=tradeoff`, `high-risk`, `close-affecting`은 informed judgment에 충분한 option, ref, risk, consequence, "does not cover" text를 요구합니다. `display_depth=simple`은 extra trade-off text가 중요하지 않으면 concise option과 `null` detail을 쓸 수 있습니다.
...
| `WRITE_AUTHORIZATION_INVALID` | supplied Write Authorization이 absent, expired, `stale`, revoked, idempotent replay 밖에서 already consumed, 또는 Task, Change Unit, baseline, intended operation, sensitive-action permission refs, Decision Packet refs와 incompatible합니다 |
...
| `APPROVAL_REQUIRED`, `APPROVAL_DENIED`, `APPROVAL_EXPIRED` | 민감 동작 permission 필요 또는 사용할 수 없음 | Approval 형태 Decision Packet을 요청, 해소, 갱신합니다. Later Approval profile이 active일 때만 committed Approval record를 다룹니다. 민감 동작 승인을 Write Authorization이나 product judgment처럼 다루면 안 됩니다. |
...
| 14 | `APPROVAL_REQUIRED` | sensitive change에 민감 동작 permission이 필요하지만 compatible granted Approval 형태 Decision Packet 또는 later Approval record가 없음 |
...
| `harness.prepare_write` | Active product-write authorization check. | 하나의 allowed/blocked path와, allowed일 때 durable single-use Write Authorization 하나를 만듭니다. Minimum MVP-1은 resolved Approval 형태 Decision Packet coverage를 사용할 수 있고, committed Approval record는 later-profile입니다. |
...
| [`harness.request_user_judgment`](#harnessrequest_user_judgment) | Decision Packet creation과 user-owned judgment prompt |
| [`harness.record_user_judgment`](#harnessrecord_user_judgment) | active Decision Packet에 대한 사용자 답변 기록 |
...
| MVP-1 user-facing | Ordinary-language next action, user-owned judgment 또는 work acceptance가 필요할 때 pending Decision Packet refs와 judgment context, evidence/close-readiness context를 반환합니다. 작업 수락이 progress나 close를 막으면 `next_action.action_kind`가 `request_acceptance`일 수 있습니다. 작업 수락 필요/상태와 잔여 위험 표시는 서로 구분되며 별도 projection kind를 요구하지 않습니다. |
...
Stage/profile: 내부 엔지니어링 점검 core authority loop에서 active입니다. Approval candidate는 state를 만들지 않고 반환할 수 있습니다. Minimum MVP-1의 민감 동작 승인은 기준 Approval 형태 Decision Packet으로 해소합니다. Committed Approval record는 later Approval/보증 프로필과 matching storage가 필요합니다.

사용자에게 보이는 의미: 지금 이 정확한 product write를 해도 되는지 답합니다. 이 답은 현재 active Task, active Change Unit scope, Autonomy Boundary, baseline freshness, 민감 동작 permission, Decision Packet coverage, design policy, surface capability에 기반합니다. `decision=allowed`이면 허용된 작업, 범위 근거, durable single-use Write Authorization ref 또는 summary, 보장 수준의 한계, detective/cooperative limitation을 보여줍니다. Harness 권한 상태상 쓰기가 허용되지 않으면 가장 먼저 해소할 이유와 가장 작은 해소 방법을 보여줍니다. Approval 또는 Decision Packet candidate payload는 담당 tool path로 commit되기 전까지 candidate일 뿐입니다.
...
상태 전이 요약: Task를 `executing`, `waiting_user`, `blocked`로 옮길 수 있습니다. Allowed일 때 Write Authorization을 생성하거나 idempotent replay에 대해 already committed response를 반환할 수 있습니다. `scope_gate=pending/blocked`, `decision_gate=required/pending/blocked`, `approval_gate=required/expired`, `stale` evidence/Approval marker를 set할 수 있습니다. Minimum MVP-1에서 `approval_gate=pending`은 `harness.request_user_judgment(judgment_route=approve-sensitive-action)`이 Approval 형태 Decision Packet을 만들 때 시작됩니다. Later Approval profile은 linked pending Approval record를 추가로 만들 수 있습니다.
...
4. Minimum MVP-1에서 Core는 Approval 형태 사용자 판단을 위한 기준 Decision Packet을 생성합니다. Response는 `decision_packet_ref`를 포함하고 `approval_id=null`로 두며, `approval_gate=pending`을 set할 수 있습니다. Approval row, Approval `StateRecordRef`, `approval_refs`, `APR` projection job은 만들지 않습니다.
5. User 또는 operator는 해당 Decision Packet에 대해 `harness.record_user_judgment`를 호출합니다.
6. Minimum MVP-1에서 Core는 Decision Packet 해소를 기록하고, 그 Approval 형태 packet에서 `approval_gate`를 granted, denied, expired 중 하나로 다시 계산합니다. 그래도 Write Authorization은 만들지 않습니다. Standalone Decision Packet projection이 enabled된 경우 `DEC`를 대기열에 넣을 수 있지만 `APR`은 넣지 않습니다.
...
8. 그 retry만 Write Authorization을 만들 수 있습니다. Minimum MVP-1에서는 resolved Approval 형태 Decision Packet의 scope, baseline, sensitive categories, paths, tools, commands, network targets, secret scope, Decision Packet refs, capability checks가 current intended write와 compatible할 때만 성공합니다.
...
민감 동작 승인은 정해진 scope 안의 sensitive categories를 허가합니다. 제품 장단점, 설계 방향, 아키텍처 판단이나 기술 구조 판단, 해결되지 않은 security 또는 product-security 판단, 검증 위험, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용 같은 사용자 소유 판단을 해소하지 않습니다. Sensitive action이 사용자 소유의 제품 판단, 기술 구조 판단이나 아키텍처 판단, 또는 해결되지 않은 security/product-security 판단도 포함하면 Core는 `prepare_write`가 `allowed`를 반환하기 전에 별도의 compatible Decision Packet을 요구해야 합니다. 민감 동작 승인은 Write Authorization이 아닙니다. 실제 제품 쓰기에는 여전히 allowed `prepare_write` result와 반환된 Write Authorization을 compatible하게 consume하는 `harness.record_run`이 필요합니다. Minimum MVP-1의 Approval 형태 prompt와 Decision Packet, later Approval profile이 active일 때만의 Approval record는 broad agreement를 묻지 말고 민감 동작, scope, expiry, 아직 결정되지 않은 것을 보여줘야 합니다.
...
| `artifact_inputs.relation.record_kind` | `task`, `change_unit`, `run` | 해당 owner record가 있을 때 `task`, `change_unit`, `run`, `decision_packet`, `residual_risk` | 같은 표의 later-profile owner kinds |
...
Core는 consumed authorization을 observed changed paths, created/deleted paths, artifact inputs와 resolved artifact refs, command results, run summary, baseline, active Change Unit, 민감 동작 permission refs, Decision Packet refs, sensitive categories, surface guarantee와 비교해 validate합니다. Run summary는 Run을 설명하는 데 도움을 주지만, compatible observed paths, artifacts, authorization basis 없이는 authorized changes의 proof로 받아들이면 안 됩니다.
...
Purpose: progress, write, close를 막거나 MVP-1 work acceptance에 필요한 user judgment를 위한 structured Decision Packet을 create합니다. Waiver, residual-risk, reconcile path는 active profile이 켜졌을 때 같은 route를 사용합니다.

Stage/profile: 내부 엔지니어링 점검에서는 active가 아닙니다. MVP-1은 user-owned judgment, 민감 동작 승인, work acceptance가 progress나 close를 막을 때 사용자 대상 Decision Packet prompt를 도입합니다. Committed Approval record, approval hardening, QA/verification waiver, full residual-risk acceptance, reconcile은 owner profile이 active일 때의 later-profile extension입니다.
...
`judgment_route=approve-sensitive-action`에서는 `judgment_payload.approval_scope`가 required입니다. Minimum MVP-1에서 Core는 기준 Approval 형태 Decision Packet을 만들고 committed Approval record는 만들지 않으므로 response의 `approval_id`는 `null`입니다. Later Approval owner profile은 linked pending Approval record를 추가로 만들 수 있지만, 민감 동작 permission은 `harness.record_user_judgment`가 packet을 해소하기 전에는 granted가 아닙니다. 이 route는 별도 표현이 없는 product, architecture, security/privacy, QA/verification, work-acceptance, residual-risk judgment를 해소하지 않습니다.
...
Projection job 대기열 추가: source record를 변경하는 committed non-dry-run Decision Packet creation에서는 Task projection support가 범위에 있으면 `TASK`; standalone Decision Packet projection이 enabled될 때만 `DEC`; later Approval owner profile이 committed Approval record를 만들거나 바꾸고 APR projection support가 active일 때만 `APR`. Minimum MVP-1 Approval 형태 Decision Packet 자체는 `APR`을 enqueue하지 않습니다. Dry-run, pre-commit state conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다.
...
상태 전이 요약: targeted Decision Packet은 resolved, deferred, rejected, blocked 상태로 처리됩니다. Affected gate 또는 reconcile item을 업데이트합니다. Minimum MVP-1에서 Approval 형태 grant/deny는 Decision Packet과 `approval_gate`를 업데이트하지만 Write Authorization이나 committed Approval record를 만들지 않습니다. Approval owner profile이 active이면 같은 route가 linked Approval record도 업데이트할 수 있습니다. User-resolved product, scope, autonomy, technical judgment는 `decision_gate` 또는 관련 owner state를 업데이트합니다. Verification waiver는 `verification_gate=waived_by_user`를 set할 수 있습니다. QA waiver는 policy가 waiver path를 accept할 때 `qa_gate=waived`를 set할 수 있습니다. Work acceptance는 `acceptance_gate`를 업데이트합니다. Accepted residual risk는 assurance를 높이지 않고 Residual Risk records를 업데이트합니다.
...
Projection job 대기열 추가: source record를 변경하는 committed non-dry-run user-owned judgment record에서는 Task projection support가 범위에 있으면 `TASK`; standalone Decision Packet projection이 enabled될 때만 `DEC`; later Approval owner profile이 committed Approval record를 update하고 APR projection support가 active일 때만 `APR`; QA waiver가 QA record로 represented되고 Manual QA projection support가 active일 때 `MANUAL-QA`. Minimum MVP-1 Approval 형태 Decision Packet resolution 자체는 `APR`을 enqueue하지 않습니다. Dry-run, pre-commit state conflict, rejected pre-commit request는 projection job을 대기열에 넣지 않습니다.
