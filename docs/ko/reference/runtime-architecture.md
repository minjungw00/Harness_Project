...
중요한 규칙은 분리입니다. 기준 운영 상태를 변경하는 것은 Core뿐입니다. 제품 소스 파일, 대화 텍스트, 생성된 Markdown, connector 파일, operator output, MCP caller claim은 system에 정보를 줄 수 있지만 기준 운영 상태는 `state.sqlite` current records에 있습니다. `state.sqlite.task_events`는 audit와 ordering history이며 일반적인 current-state source가 아닙니다. 원본 근거는 artifact store에 있습니다.
