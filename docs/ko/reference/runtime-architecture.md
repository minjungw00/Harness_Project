...
Artifact store는 지속 보관되는 근거 파일과 안전한 메타데이터 알림을 보관하지만, loose file dump가 아닙니다. Diff, log, screenshot, trace, checkpoint, bundle, captured manifest, exported bundle component, 기타 evidence file은 Harness staging, approved capture adapter, 또는 이미 commit된 artifact ref에서 등록되고 integrity metadata와 owner 관계를 가진 뒤에만 저장됩니다.
...
Large logs, diffs, screenshots, traces 같은 큰 근거는 등록된 아티팩트 참조로 link해야 합니다. Markdown 보고서와 export는 ref가 무엇을 뒷받침하는지 요약하고 redaction 및 availability state와 안전한 메모를 표시할 수 있지만, 큰 evidence 본문을 붙여 넣거나 생략된 secret value를 다시 만들면 안 됩니다.
...
이 named 보고서 kind는 기본적으로 상태 기록과 artifact refs에서 생성되는 projections입니다. Artifact store의 evidence file을 참조할 수 있고 export가 snapshots를 포함할 수 있지만, 그렇다고 Markdown 보고서가 기준 근거나 기준 상태가 되지는 않습니다.
