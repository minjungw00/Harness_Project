...
정확한 상태 전이는 [커널 참조](kernel.md)를, public tool envelope는 [API Schema Core](api/schema-core.md)를, replay 동작은 [API Errors](api/errors.md)를, storage layout과 lock은 [Storage와 DDL](storage-and-ddl.md)을, security asset, trust boundary, threat, control은 [보안 위협 모델 참조](security-threat-model.md)를, operator entrypoint 의미는 [운영과 Conformance 참조](operations-and-conformance.md)를 사용합니다.
...
| Markdown 보고서 | 기록과 artifact refs에서 만든 사람이 읽을 수 있는 projection | TASK, compact status card, optional full-format Decision Packet views, APR, DIRECT-RESULT, 그리고 later diagnostic view인 Journey Card/Spine, RUN-SUMMARY, EVAL, EVIDENCE-MANIFEST |
