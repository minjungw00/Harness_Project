...
정확한 상태 전이는 [Core Model 참조](core-model.md)를, public tool envelope는 [API Schema Core](api/schema-core.md)를, replay 동작은 [API Errors](api/errors.md)를, storage layout과 lock은 [Storage](storage.md)를, security asset, trust boundary, threat, control은 [보안 참조](security.md)를, operator entrypoint 의미는 [운영과 Conformance 참조](operations-and-conformance.md)를 사용합니다.
...
- SQLite DDL. [Storage](storage.md)를 봅니다.
...
- threat-model asset, trust boundary, threat category, control category, guarantee-level 의미. [보안 참조](security.md)를 봅니다.
...
- kernel transition table. 자세한 내용은 [Core Model 참조](core-model.md)를 봅니다.
...
하네스 서버/설치:
  ...
  reconcile worker, operator tools
...
하네스는 로컬 권한 계층으로 설계되며, 일반적인 운영체제 보안 경계를 대신하지 않습니다. 초기 로컬 하네스는 OS 권한을 자동으로 제공하거나, 임의 도구를 sandbox 격리하거나, 로컬 파일을 변조 불가능하게 만들거나, 지시 기반 agent behavior를 사전 차단 보안으로 바꾸지 않습니다. 전체 asset map, trust-boundary map, threat category, control category는 [보안 참조](security.md)가 담당합니다.
...
Architecture는 이 경계를 계속 보이게 합니다. 제품 저장소 파일과 projection은 input 또는 readable view이고, MCP와 connected surface는 Core로 들어오는 caller path이며, 런타임 홈은 local control data이고, artifact store의 bytes는 registration과 integrity check 전까지 untrusted입니다. External tool/network는 기존 scope, Approval, connector, operator control로 제한되는 side-effecting path입니다. 전체 boundary matrix는 [보안 참조](security.md#신뢰-경계)가 담당합니다.
...
Remote, shared, tunneled, forwarded, non-loopback, cross-user, cloud/CI relay 노출은 owner docs가 connector posture를 승격하고 증명하기 전까지 내부 엔지니어링 점검 baseline과 staged delivery 밖에 남습니다. 전체 asset, trust-boundary, threat, control model은 [보안 참조](security.md#mcp-local-access와-caller-boundary)가 담당하고, connector profile reporting은 [Agent 통합 참조](agent-integration.md#capability-profiles), API validation은 [API Schema Core](api/schema-core.md#mcp-boundary-and-caller-trust), operator diagnostic은 [운영과 Conformance 참조](operations-and-conformance.md#serve-mcp)가 담당합니다.
...
제품 저장소는 생성된 읽기용 요약과, active profile이 켠 경우 생성된 `TASK`, `APR`, `RUN-SUMMARY`, `EVAL`, `DIRECT-RESULT`, `EVIDENCE-MANIFEST`, `TDD-TRACE`, `MANUAL-QA`, `DOMAIN-LANGUAGE`, `MODULE-MAP`, `INTERFACE-CONTRACT`, `JOURNEY-CARD`, `EXPORT`, 그 밖의 report projection Markdown 보고서를 담을 수 있습니다. 내부 엔지니어링 점검은 전체 catalog가 아니라 structured status/blocker output부터 시작해야 합니다. 사용자 판단 요청 표시, 증거 요약, 닫기 준비 상태 output은 MVP-1와 이후 profile에서 자랍니다. 이 파일들은 사람과 agent가 작업을 읽는 데 도움을 주지만 기준 상태가 아닙니다. 사람이 편집할 수 있는 영역은 입력 접점입니다. 사람이 남긴 edit은 reconcile이 Core 상태 변경 action으로 라우팅할 때만 상태 기록이 됩니다.

## Harness Server / Installation

한국어 표현: 하네스 서버/설치.

하네스 서버/설치는 제어 계층입니다. 내부 엔지니어링 점검은 여러 service의 fleet 대신 내부 모듈을 가진 하나의 로컬 프로세스로 구현할 수 있습니다.
...
- 단계별로 도입되는 운영자 진입점을 제공합니다. 초기 단계에는 최소 local 진단/상태를 두고, 운영 프로필 또는 나중 profile이 범위에 넣을 때 recover, export, artifact, handoff, conformance surface를 추가합니다.
...
하네스 런타임 홈은 로컬 운영 권한을 저장합니다. Reference location은 `~/.harness`이지만 정확한 layout은 [Storage](storage.md)이 담당합니다.
...
- 지속 보관되는 증거 파일을 위한 artifact directories
...
대화 접점은 사용자 의도, decision, 민감 동작 승인 prompt, QA 판단, acceptance를 모읍니다. Agent 접점은 읽기, 편집, 확인을 수행합니다. 하네스 rules와 skills는 agent가 현재 상태를 놓치지 않게 합니다. MCP server는 tool 경계를 제공합니다. Core는 상태 모델을 담당합니다. Validator, artifact 수집, projection, reconcile은 증거와 읽기용 출력을 상태 전이에 붙입니다.

Native hooks, sidecars, command wrappers, file watchers, worktree isolation은 capability에 따라 달라지는 통제 계층입니다. 구체적인 capability profile이 covered operation에 대해 fixture로 더 강한 통제를 증명하지 않는 한 내부 엔지니어링 점검과 초기 첫 사용자 가치 조각는 reference 접점에서 cooperative/detective behavior에 의존합니다.


### Core modules

Core는 첫 조각에서 단일 로컬 프로세스로 실행할 수 있습니다. 전체 server는 아래 내부 책임으로 자랄 수 있지만, 내부 엔지니어링 점검이 이들을 별도 모듈로 모두 구현해야 하는 것은 아닙니다. 내부 엔지니어링 점검 구현은 local registration, 등록된 reference `capability_profile` 하나, Task 하나, 활성 Change Unit 또는 범위가 정해진 작업 경계 하나, `prepare_write`, 한 번만 쓰는 Write Authorization 하나, `record_run` 하나, artifact/evidence ref 하나, structured status/blocker response 하나, 좁은 close-blocker check 밖의 기능을 stub, absent path 또는 later-profile scope로 둘 수 있습니다.
...
| Connector adapter | `surface_id=reference-local-mcp`인 reference surface registration, capability reporting, capture hints. Broad connector ecosystem support는 later/profile 범위 |


Core만 기준 운영 상태를 업데이트합니다. Agents, MCP tools, CLI commands, projectors, reconnect/recovery flows는 Core 로직을 거치거나 같은 상태 compatibility rules를 보존하는 recovery code를 사용해야 합니다. 이들은 Core record를 표시, 진단, 복구, 파생할 수 있지만 두 번째 기준 상태 모델을 유지하면 안 됩니다. Operator command name과 flag는 표시/entrypoint 선택입니다. 동작은 Core state record, `state.sqlite.task_events`의 audit/order fact, evidence/artifact ref, 해당 profile이 active일 때의 projection job, API-owned errors 또는 문서화된 diagnostics가 정의합니다.

Decision, Journey, Autonomy/Boundary modules는 새로운 권한 tier를 만들지 않습니다. 기준 기록은 `state.sqlite` 현재 기록과 `state.sqlite.task_events`에 있습니다. Artifact evidence는 등록된 아티팩트 기록와 artifact-store byte 또는 안전한 메타데이터 알림에 있고, Markdown views는 projections 또는 proposal 접점으로 남습니다.
...
보증 프로필과 운영 프로필의 ValidatorResult ID set은 API가 소유하며 [API Schema Later](api/schema-later.md#validatorresult-stable-ids)에 나열됩니다. 이 runtime reference는 해당 validator가 Core와 adapter 옆에 어디에 놓이는지 담당하며, 두 번째 ID registry를 만들지 않습니다.
...
State/envelope validation, active Task, active Change Unit, changed paths, baseline freshness, Approval 범위, evidence sufficiency, artifact integrity, verification independence, same-session verification guard, evaluator bundle freshness, projection 최신성 같은 Core preconditions와 mechanical checks는 이 validators 전이나 옆에서 실행될 수 있습니다. 이 값들은 이 section, [API Schema Core](api/schema-core.md#validatorresult), [API Schema Later](api/schema-later.md#validatorresult-stable-ids), [Storage](storage.md)이 stable ValidatorResult-emitting set으로 명시적으로 승격하지 않는 한 대체 validator ID가 아닙니다. Surface capability는 `ValidatorResult`로 emit될 때 의도적으로 `surface_capability_check` capability validator로 model됩니다.


Adapters와 sidecars는 접점 capability를 observable facts로 번역합니다. Capability에 대한 kernel gate를 만들지는 않습니다. Capability는 `surface_capability_check` validator, `prepare_write` blocked reasons, fallback behavior, guarantee display를 통해 나타납니다. 활성 reference `capability_profile`에서 unsupported field는 표시되는 guarantee를 낮추거나 claim을 막아야 합니다. 필요한 surface capability가 없으면 product write가 조용히 계속되면 안 됩니다. 구체적인 host/profile의 capability declaration과 refresh trigger는 [Agent 통합 참조](agent-integration.md#capability-profiles)가 담당하고, 접점별 path는 [Surface Cookbook](surface-cookbook.md)이 이름 붙입니다.
...
Projection freshness는 파생 read fact입니다. Status, next-action, export, operator command가 이를 확인해 readable view가 stale, failed, unknown이라고 보고할 수는 있지만, Core state, structured blockers, evidence records, 최종 수락, 잔여 위험 수락, Write Authorization의 authority는 각 owner record에 남습니다. 내부 엔지니어링 점검은 full projection worker를 증명하지 않고 freshness 또는 read fact를 노출할 수 있습니다. MVP-1은 현재 작업 상태, 사용자 판단 요청, 증거 요약, 닫기 준비 상태/blocker를 사용자가 이해할 만큼의 derived output이 필요하고, hardened 또는 operational profile은 complete projection/reconcile 및 diagnostic report path를 담당합니다.

## Artifact store architecture

Artifact store는 지속 보관되는 증거 파일과 안전한 메타데이터 알림을 보관하지만, loose file dump가 아닙니다. Diff, log, screenshot, trace, checkpoint, bundle, captured manifest, exported bundle component, 기타 evidence file은 Harness staging, approved capture adapter, 또는 이미 commit된 artifact ref에서 등록되고 integrity metadata와 owner 관계를 가진 뒤에만 저장됩니다.

Artifact는 두 부분으로 이루어집니다.

- artifact store 안의 등록된 파일 또는 안전한 메타데이터 알림
- kind, `uri`, `sha256`, `size_bytes`, `content_type`, `redaction_state`, `produced_by`, `retention_class`, Task-scoped owner relation을 이름 붙이는 등록된 아티팩트 참조와 `state.sqlite`의 artifact 상태 기록


Core는 runs, evidence manifests, Eval records, 수동 QA records, user judgments, 렌더링된 Task-scoped projection refs 같은 기존 Task-scoped owner record에 artifact refs를 기록합니다. Current Task-scoped artifact model에서 렌더링된 projection ref로 향하는 `artifact_links`는 artifact의 `task_id` 안에 머뭅니다. Project-level projection job은 owner docs가 허용하는 곳에서 `projection_jobs` metadata로 track될 수 있지만, project-scoped artifact links는 아닙니다. Exact relation rules는 [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md), [Storage](storage.md), [Projection과 Template 참조](projection-and-templates.md), [운영과 Conformance 참조](operations-and-conformance.md)가 담당합니다. Large logs, diffs, screenshots, traces, patches는 허용될 때 등록된 아티팩트 참조로 남기고, 민감도 때문에 전체 본문을 저장할 수 없으면 redacted, `secret_omitted`, `blocked` ref로 남깁니다. Markdown 보고서는 제한 없는 evidence 본문을 포함하는 대신 ref로 link해야 합니다.

Raw secret, token, full sensitive log는 artifacts로 저장하면 안 됩니다. Secret-related evidence가 required라면 Core는 redacted artifact, `secret_omitted` 또는 `blocked` metadata notice, secret handle, relevant validator를 통과한 operator note를 기록합니다.

Critical evidence는 Markdown prose나 path 모양의 caller claim만으로 sufficient가 될 수 없습니다. Required artifact가 missing이거나 `sha256` / `size_bytes` / `content_type` / `redaction_state` metadata가 없거나 owner relation이 없거나 `hash_mismatch` 같은 integrity validation에 실패하면 Core는 related evidence를 `stale` 또는 `blocked`로 표시합니다. 그 evidence가 required이면 close는 계속 blocked입니다.

Large logs, diffs, screenshots, traces 같은 큰 증거는 등록된 아티팩트 참조로 link해야 합니다. Markdown 보고서와 export는 ref가 무엇을 뒷받침하는지 요약하고 redaction 및 availability state와 안전한 메모를 표시할 수 있지만, 큰 evidence 본문을 붙여 넣거나 생략된 secret value를 다시 만들면 안 됩니다.

Export는 파생 bundle이며 네 번째 권한 공간이 아닙니다. Export는 Core state snapshot, 안전한 state/event version fact, report projection snapshot, artifact ref, 허용된 등록된 아티팩트 파일, artifact integrity result, `redaction_state`, omitted-secret note, retained, expired, unavailable, `secret_omitted`, `blocked` artifact의 retention/availability fact를 포함할 수 있습니다. Durable file이 되는 export component는 valid owner record 또는 Task-scoped projection ref에 연결된 artifact로 남습니다. Export는 recovery artifact, stale projection, Markdown prose, chat text, staging path, operator console output에서 성공을 추론하면 안 됩니다.

### Registered artifacts, 상태 기록, Markdown 보고서

경계는 다음과 같습니다.

| Item | Authority | Examples |
|---|---|---|
| 등록된 아티팩트 파일 | `ArtifactRef` metadata가 있는 artifact store의 durable evidence file 또는 안전한 메타데이터 알림 | diff, log, screenshot, checkpoint, bundle, manifest file, metadata-only notice |
| 상태 기록 | `state.sqlite`의 기준 structured record | Task, Change Unit, User Judgment, Journey Spine Entry, Residual Risk, Run, Approval, Eval, 수동 QA record, Evidence Manifest, Shared Design, Artifact record |
| Markdown 보고서 | 기록과 artifact refs에서 만든 사람이 읽을 수 있는 projection | MVP-1의 작은 보기와 later/full-profile 보고서는 [Projection과 Template 참조](projection-and-templates.md)와 [Template 참조](templates/README.md)가 담당합니다. |


이 named 보고서 kind는 기본적으로 상태 기록과 artifact refs에서 생성되는 projections입니다. Artifact store의 evidence file을 참조할 수 있고 export가 snapshots를 포함할 수 있지만, 그렇다고 Markdown 보고서가 기준 증거나 기준 상태가 되지는 않습니다.
...
| artifact file missing 또는 `hash_mismatch` | artifact와 dependent evidence, projection, export, close-readiness view를 `stale` 또는 blocked로 표시합니다. Recovery를 통해 다시 scan하거나, 등록된 정확한 bytes를 restore하거나, replacement를 등록합니다 |
