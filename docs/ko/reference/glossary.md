...
하네스 개념을 처음 이해하려면 [시작하기](../start.md)를 읽습니다. 정확한 동작이 필요하면 아래 owner link나 개별 정의 안의 link를 따라갑니다.
...
구현 계획 전에 에이전트가 사용하는 가정을 정리한 Discovery 또는 later/profile Shared Design 보조 상태 보기/요약 목록입니다. Source, confidence, owner, 가정이 틀릴 때 바뀌는 일을 이름 붙여야 합니다. 이는 권장 표시 또는 보조 내용이지 독립 schema나 기준 record 필드 목록이 아닙니다. 활성 MVP-1에서 assumption context는 Task shaping, Change Unit shaping, user judgment 후보를 돕습니다. 하지만 사용자 동의, 민감 동작 승인, 최종 수락, 잔여 위험 수락, 증거, 닫기 준비 상태, 범위 권한, 쓰기 승인 기록은 아닙니다.
...
이는 scope grant나 쓰기 전 범위 확인 / Write Authorization이 아니며 active Change Unit 밖의 paths, tools, commands, network targets, secret access, sensitive categories를 승인하지 않습니다. User judgment가 Autonomy Boundary update나 Change Unit update proposal을 승인할 수는 있습니다. 하지만 resulting write에는 여전히 compatible Change Unit scope와 sensitive categories에 필요한 민감 동작 승인이 필요합니다. 정확한 kernel behavior는 [Autonomy Boundary](core-model.md#autonomy-boundary)가 담당하고, close-impact placement는 [설계 품질](design-quality.md#when-a-finding-blocks-close)에 요약되어 있습니다.
...
Enabled design-quality finding을 라우팅하는 kernel gate surface입니다. 활성 MVP에서 write 또는 close를 차단하는 finding은 [설계 품질: 닫기 blocker가 되는 조건](design-quality.md#when-a-finding-blocks-close)의 작은 Core-backed 집합뿐입니다. 더 넓은 domain-language, TDD, module/interface, stewardship, feedback-loop, Manual QA, detached-verification catalog finding은 active owner path가 좁은 동작을 승격하지 않는 한 later 후보입니다.

### 설계 품질

활성 현재 MVP 설계 품질 경계를 담당하는 문서입니다. 활성 역할, 발견 사항 심각도 해석, 닫기 blocker 조건, 면제 경계, 증거 기대치, validator ID 경계, later 정책 후보 경계를 정의합니다. 전체 policy-to-validator mapping을 제공하지 않고 kernel state machine을 재정의하지 않습니다.
...
Run, Eval, 수동 QA 기록, validator, review display, operator diagnostic, conformance check에서 나온 관찰된 issue, gap, risk, blocker, noteworthy result입니다. Finding은 독립 권한 경로가 아니며 chat이나 report prose에 남아 있는 것만으로 `gate` 또는 닫기에 영향을 주지 않습니다. 기존 소유자 기록 또는 구조화된 결과를 통해 라우팅될 때만 상태와 관련됩니다. 예: active일 때 Evidence Manifest gap, user judgment 후보 또는 record, Change Unit update, active일 때 수동 QA 또는 Eval record, 잔여 위험 record, reconcile item, close blocker, 후속 Task/Change Unit record. 활성 close impact는 [설계 품질](design-quality.md#when-a-finding-blocks-close)과 [Core Model 참조](core-model.md#finding-라우팅)가 담당합니다.

### First Safe Change Unit Candidate

제품 쓰기가 가까워졌을 때 안전한 다음 작업 후보를 내부 Change Unit 모양으로 표현한 것입니다. 해소되지 않은 사용자 소유 판단을 숨기지 않고 포함되는 동작, 범위 밖 동작, 완료 조건, 알려진 민감 영역, 중지 조건을 이름 붙여야 합니다. Discovery는 에이전트가 확인할 수 있는 사실과 사용자 소유 판단을 분리하고 안전한 다음 작업이 충분히 분명해진 뒤 이것을 제안할 수 있습니다. Later/profile Shared Design도 enabled일 때 이를 포함할 수 있습니다. Discovery가 오직 이 후보를 찾기 위해 존재하는 것은 아닙니다. 이는 권장 표시 또는 보조 내용이지 독립 schema나 기준 record 필드 목록이 아닙니다. 이것은 후보일 뿐입니다. 제품 쓰기 전에는 여전히 활성 Change Unit, 호환되는 범위 `gate` 상태, 이후 `prepare_write`가 필요합니다.
...
`state.sqlite.task_events`, runs, validator results, projection jobs, reconcile items 같은 local record에서 파생되는 later diagnostic-only metric입니다. Owner 문서로 승격되기 전까지 metric 표시는 rate, count, duration, guard-trigger summary를 읽기 전용 진단으로만 보여줄 수 있습니다. Local Derived Metrics는 로드맵 후보로 남습니다. [로드맵: 후보 항목 목록](../later/index.md#roadmap-candidates)을 봅니다.

### Manual QA

한국어 기준 표현: 수동 QA.

수동 QA는 UX, 작업 흐름, 문구, 시각 결과, 접근성, 제품 적합성처럼 사람이 판단해야 하는 경험 품질을 사람이 확인한 기록입니다. 필수일 때는 수동 QA 기록 또는 유효한 QA 면제 판단 경로로 기록됩니다. 브라우저 간단 확인, 스크린샷, 브라우저 QA 아티팩트, 테스트, 검증자 메모는 맥락을 뒷받침할 수 있지만 그 자체로 수동 QA 판단이 아닙니다. 정확한 `gate` 동작은 [QA Gate](core-model.md#qa-gate)가 담당합니다. 활성 설계 품질 면제와 증거 경계는 [설계 품질](design-quality.md#waiver-boundary)에 요약되어 있습니다.
...
기록 수준의 수동 QA 결과입니다. 수행자, 프로필, 결과, 아티팩트, 발견사항, 해당하는 경우 면제 이유, 다음 행동을 포함합니다. 결과 값 집합은 [QA Gate](core-model.md#qa-gate)와 later/profile-gated [`harness.record_manual_qa`](../later/index.md#later-schema-candidates)가 담당합니다. 대기 중인 필수 QA는 `qa_gate=pending`으로 표현하며 수동 QA 기록 결과가 아닙니다.
...
Future design-quality policy family를 위한 later 후보 shape입니다. 활성 현재 MVP의 설계 품질은 full policy contract table이나 전체 policy-to-validator mapping을 제공하지 않습니다. [설계 품질](design-quality.md#later-policy-catalog-boundary)을 봅니다.
...
Projection job과 template kind를 나타내는 API enum입니다. Support class, active value, extension rule은 [API Schema Core](api/schema-core.md#projectionkind-support)가 담당하고, later/profile-gated value는 [API Schema Later](../later/index.md#later-schema-candidates)에 남습니다. Support class label은 내부 엔지니어링 점검 run obligation이 아닙니다. 내부 엔지니어링 점검에는 소유자 경로가 이미 만든 freshness/read fact를 보존하는 것 외의 projection rendering exit requirement가 없습니다. 어떤 ProjectionKind도 Projection을 기준 상태로 만들지 않습니다.
...
열린 질문을 blocking, useful-but-not-blocking, codebase-answerable로 분류한 Discovery 또는 later/profile Shared Design 보조 상태 보기/요약 목록입니다. 이는 권장 표시 또는 보조 내용이지 독립 schema나 기준 record 필드 목록이 아닙니다. Blocking question은 사용자 소유 판단이 필요할 때 user judgment 후보로 라우팅될 수 있습니다. Useful-but-not-blocking question은 남겨 두거나, defer하거나, 후속 작업으로 바꿀 수 있습니다. Codebase-answerable question은 사용자에게 묻지 말고 현재 저장소, 문서, 하네스 상태, 출처 참조에서 답해야 합니다. Queue는 user judgment, `gate`, 민감 동작 승인, 증거, 최종 수락, 닫기, 쓰기 승인 기록이 아닙니다.
...
Spec Compliance Review와 Code Quality / Stewardship Review를 분리하는 later 표시/절차 구분입니다. Spec Compliance Review는 현재 하네스 권한 안에서 요청한 작업이 완료됐는지 묻습니다. Code Quality / Stewardship Review는 구현이 코드베이스 안에서 유지보수하기 좋은지 묻습니다. Review Stages는 활성 현재 MVP requirement, 기준 기록, `ProjectionKind` value, 민감 동작 승인 / Approval, evidence, verification, QA, 최종 수락, 잔여 위험 수락, close, 쓰기 승인 기록이 아닙니다. 이름만 남긴 later 정책 위치는 [설계 품질](design-quality.md#later-policy-catalog-boundary)과 [Later 후보 색인](../later/index.md#assurance-candidates)에 있습니다. Same-session Review Stages는 `assurance_level=detached_verified`를 만들지 않습니다.
...
더 약한 발견 사항을 숨기지 않고 finding severity를 해석하기 위한 활성 설계 품질 규칙입니다. Same concern은 전체 Task나 단순히 같은 validator ID가 아니라 같은 close/write/judgment/evidence/risk target입니다. 서로 다른 concern은 분리하고, 같은 concern에서 경쟁하는 findings에만 가장 강한 valid active action을 사용합니다. Public primary `ToolError` 선택은 API가 소유합니다. 활성 동작은 [설계 품질: 발견 사항 심각도](design-quality.md#finding-severity)가 담당합니다.

### Shared Design

구현이 계획으로 굳어지기 전에 공유된 이해를 기록하기 위한 later/profile design-support record 또는 projection label입니다. 활성 MVP-1에서 요구사항 구체화는 committed Shared Design record를 만들지 않고 Task, Change Unit, User Judgment owner path로 지속됩니다. Discovery Brief, Question Queue, Assumption Register, 안전한 다음 작업 후보 또는 작업 분할 제안, First Safe Change Unit Candidate는 owner profile이 별도 record를 명시적으로 켜기 전까지 support/display 이름입니다. Shared Design은 enabled일 때 shaping과 `design_gate` 준비 상태를 도울 수 있지만 민감 동작 승인, 최종 수락, 잔여 위험 수락, QA 판단, 증거, 닫기 준비 상태, 쓰기 승인 기록은 아닙니다. Shared Design의 Markdown 렌더링 결과는 파생 보기(Projection)이자 제안용 접점입니다. 활성 설계 품질 경계는 [설계 품질](design-quality.md#active-current-mvp-design-quality-role)이 담당합니다.
...
Kernel state 안의 기준 structured record입니다. Active MVP-1 state record는 active schema/storage owner set으로 제한됩니다. 예를 들면 Task, Change Unit, User Judgment, Run, 쓰기 승인 기록, Artifact record, Evidence Summary, Blocker입니다. Journey Spine Entry, 잔여 위험, Approval, Evidence Manifest, Eval, 수동 QA 기록, Shared Design record, Domain Term, Module Map Item, Interface Contract, Feedback Loop, TDD Trace, Reconcile Item 같은 later/profile record는 owner profile이 켜졌을 때만 state record입니다.
