...
이 용어집은 선별된 하네스 핵심 용어를 사람이 읽기 쉽게 설명하는 간결한 안내입니다. 포함된 용어마다 짧은 뜻과 주 담당 문서를 보여 주는 선별 독자용 하위 집합입니다.

완전한 구조화 용어 메타데이터는 [`docs/terminology-map.yaml`](../../terminology-map.yaml)에 있습니다. 여기에는 용어 기록, 한영 표현 통제, 식별자 보존 규칙, 주 담당 경로, 관련 참조 메타데이터가 포함됩니다. 이 표의 각 행은 대응하는 용어 지도 항목의 집중 주 담당 문서를 따르며, 주변 참조는 용어 지도의 `related_references`에 남깁니다. 주제별 담당 문서 찾기는 [참조 색인](README.md)을 사용합니다. `doc_id` 기준의 정확한 기계 판독 경로는 [`docs/doc-index.yaml`](../../doc-index.yaml)을 사용합니다.

계약 세부사항은 이 표가 아니라 집중 담당 문서에서 확인합니다.
