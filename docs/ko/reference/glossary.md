...
| reserved value | 값 집합에 남겨 둔 어휘나 향후 표면입니다. 현재 담당 문서가 말하기 전까지 현재 MVP 동작이 아닙니다. | 예약된 값 | 예약된 값 | 정확한 값 문자열 | 기본값, 필수값, 지원됨, 강제됨, 수락됨, 검증됨, 닫기 준비 상태, 현재 보장처럼 쓰는 표현 | [현재 MVP 범위](active-mvp-scope.md), [API 값 집합](api/schema-value-sets.md) |
| profile-gated value | 문서화된 profile-gated 조건과 담당 동작이 활성일 때만 사용할 수 있는 값입니다. | profile-gated 값 | profile-gated 값 | 정확한 값 문자열 | 값 집합에 있다는 이유만으로 profile-gated 값을 현재 MVP 동작처럼 쓰는 표현 | [현재 MVP 범위](active-mvp-scope.md), [API 값 집합](api/schema-value-sets.md) |
...
| promotion-time owner update | 승격 시점에 담당 문서를 만들거나 지정하고, 필요한 대응 담당 문서를 함께 갱신하는 작업입니다. | 승격 시점의 담당 문서 갱신 | 승격 시점의 담당 문서 갱신 | 파일 경로와 앵커 | 향후 담당 문서가 이미 현재 기준 담당 문서인 것처럼 이름 붙이는 표현 | [작성 가이드](../maintain/authoring-guide.md), [이후 후보 색인](../later/index.md), [현재 MVP 범위](active-mvp-scope.md) |
| future owner placeholder | 승격 시점에 담당 문서를 만들거나 지정해야 함을 표시하는 자리표시자 표현입니다. 현재 담당 문서가 아닙니다. | 향후 담당 문서 자리표시자 | 향후 담당 문서 자리표시자 | - | 자리표시자를 현재 기준 담당 문서처럼 독자에게 안내하는 표현 | [작성 가이드](../maintain/authoring-guide.md), [이후 후보 색인](../later/index.md) |
...
| active guarantee | 현재 범위와 보안 담당 문서가 모두 현재 동작으로 문서화한 보장입니다. | 현재 보장 | 현재 보장 | 정확한 보장 라벨 값 | 예약된 값이나 profile-gated 값을 현재 보장처럼 쓰는 표현 | [보안](security.md), [현재 MVP 범위](active-mvp-scope.md), [API 값 집합](api/schema-value-sets.md) |
...
| `isolated` guarantee label | 이후/profile-gated 보장 라벨로 예약된 값입니다. 현재 MVP의 현재 보장이 아닙니다. | `isolated` | `isolated` | `isolated` | 격리 보장이 제공됩니다, 현재 격리됩니다, 현재 MVP가 isolated 보장을 제공합니다 | [보안](security.md), [API 값 집합](api/schema-value-sets.md) |
