...
Projection job과 template kind를 나타내는 API enum입니다. Support class, active value, extension rule은 [API Schema Core](api/schema-core.md#projectionkind-support)가 담당하고, later/profile-gated value는 [API Schema Later](api/schema-later.md#later-profile-ref-and-artifact-values)에 남습니다. Support class label은 내부 엔지니어링 점검 run obligation이 아닙니다. 내부 엔지니어링 점검에는 소유자 경로가 이미 만든 freshness/read fact를 보존하는 것 외의 projection rendering exit requirement가 없습니다. 어떤 ProjectionKind도 Projection을 기준 상태로 만들지 않습니다.
