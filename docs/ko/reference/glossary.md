...
| `complete` intent value | `harness.close_task`의 `intent` 값이거나 다른 명시적 식별자일 때 보존하는 값입니다. 전체나 전체 평가라는 뜻의 산문 표현은 아닙니다. | `complete` | `complete` | `complete`, `intent=complete` | 전체, 전체 평가, 전체 평가 순서를 뜻하는 산문에서 `complete`를 보존하는 표현 | [MVP API](api/mvp-api.md), [API 값 집합](api/schema-value-sets.md) |
| full evaluation order | 평가 순서 전체를 가리키는 일반 산문 표현입니다. 닫기 준비 상태 맥락에서는 전체 닫기 준비 상태 평가 순서라고 씁니다. | 전체 평가 순서 | 전체 평가 순서 | - | `complete` 평가 순서, complete 평가 순서, `complete` 닫기 준비 상태 순서, complete 닫기 준비 상태 순서 | [번역 가이드](../maintain/translation-guide.md), [용어 지도](../../terminology-map.yaml) |
...
| `isolated` | 이후/profile-gated 보장 라벨로 예약된 값입니다. 현재 MVP의 활성 보장이 아닙니다. | `isolated` | `isolated` | `isolated` | 격리 보장이 제공됩니다, 현재 격리됩니다, 현재 MVP가 isolated 보장을 제공합니다 | [보안](security.md), [API 값 집합](api/schema-value-sets.md) |
