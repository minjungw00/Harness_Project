...
아래 용어는 오래된 문서, payload, label과의 혼동을 막을 때만 남깁니다. 새 active 문서의 primary concept로 쓰지 않습니다.

| 용어 | 호환성 메모 | 현재 경로 |
|---|---|---|
| Decision Packet | Full-format user-judgment presentation이자 legacy label입니다. Active MVP는 compact `presentation=short`를 쓰며, `presentation=full`은 later/profile material입니다. | [API Schema Core](api/schema-core.md#userjudgment), [Later](../later/index.md#assurance-candidates) |
...
| `judgment_type`, `judgment_domain`, `decision_kind`, `decision_profile` | 오래된 judgment-shape alias입니다. `judgment_kind`, route-specific payload validation, `presentation`을 우선합니다. | [API Schema Core](api/schema-core.md#userjudgment) |
| `judgment_category`, `judgment_route`, `display_depth` | 오래된 Decision Packet draft의 legacy 또는 implementation routing term입니다. | [API Schema Core](api/schema-core.md#userjudgment) |
...
| Historical delivery labels | `v0.1 Core Authority Smoke`, `v0.2 First User-Value Slice`, `v0.3 Agency Assurance Pack`, `v0.4 Operations & Handoff Pack`, `v1+ Expansion`은 현재 stage name이 아닙니다. | [번역 가이드](../maintain/translation-guide.md) |
| Full later schemas, template bodies, fixture bodies, old roadmap narratives, archive copies, scratch migration files | Active documentation에서 폐기되었습니다. 필요하면 promoted owner document를 통해서만 다시 들여옵니다. | [Later](../later/index.md#explicitly-retired-material) |
