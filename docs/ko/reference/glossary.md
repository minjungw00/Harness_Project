...
Current state, evidence, relevant references는 context에 유지하고, 오래된 chat, old PRDs, closed issues, oversized raw artifacts는 명시적으로 필요할 때만 가져오는 policy입니다.
...
Bilingual parity, links, owner 경계, stable catalogs, glossary terms, 기준 기록 표현, TODO usage, non-owner duplicate contracts의 drift를 감지하는 read-only documentation maintenance check profile입니다. Rule bodies는 [문서 작성 가이드](../maintain/authoring-guide.md#docs-maintenance-checks)가 담당하고, operator 보고와 entrypoint expectation은 [운영과 Conformance 참조](operations-and-conformance.md#docs-maintenance-프로필)가 담당합니다. Core fixture conformance, runtime validator, projection 새로고침, QA, acceptance, evidence, Residual Risk 수용, 기준 상태 전이가 아닙니다. `harness conformance run` 또는 다른 operator command로 제공되면 명시적으로 선택하는 separate docs-only profile이며, 관례적 이름은 `docs-maintenance`입니다. Console 또는 ephemeral result는 runtime fixture pass/fail, 구현 준비 상태, close readiness, Task state에 영향을 주지 않습니다.
...
Required acceptance criteria 또는 completion conditions가 Evidence Manifest와 관련 state records 및 artifact refs로 뒷받침되는지에 대한 close-relevant judgment입니다. Chat text나 Markdown 보고서 prose만으로 판단하지 않습니다.
...
연결된 접점 또는 runtime path에서 available한 enforcement strength입니다.
...
`state.sqlite.task_events`, runs, validator results, projection jobs, reconcile items 같은 local record에서 파생되는 later diagnostic-only metric입니다. Metric 표시는 rate, count, duration, guard-trigger summary를 보여줄 수 있지만 상태 변경, gate 충족, 쓰기 권한, 승인 부여, 근거 생성, projection 대기열 추가 또는 새로고침, projection 최신성 변경, close readiness 또는 구현 준비 상태 변경, verification 수행 또는 기록, QA 기록, QA 또는 verification 면제, Residual Risk 수용, 결과 수용, assurance level 상승, Task close를 하면 안 됩니다.
...
Product의 modules, responsibilities, public interfaces, dependency direction, internal complexity, test 경계, owner 결정, watchpoints를 정리한 map입니다. 기준 기록은 `module_map_items`입니다.
...
Implementation이 plan으로 굳어지기 전에 Task에 대해 최소한으로 기록한 shared understanding입니다. goal, scope, non-goals, acceptance criteria, assumptions, decisions, rejected options, domain/module/interface impact, first Change Unit shape를 포함합니다. Shared Design의 Markdown 렌더링 결과는 projections이자 proposal 접점입니다.

### Source-of-truth

어떤 fact에 대한 기준 정보입니다. Harness에서 운영 상태의 기준 기록은 `state.sqlite` current records와 `state.sqlite.task_events`이고, raw evidence의 기준 위치는 artifact store이며, Markdown documents는 projections입니다.
