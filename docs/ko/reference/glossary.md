...
이 용어집은 하네스 문서의 용어를 자연스러운 한국어로 안내합니다. 용어의 뜻과 이어서 읽을 주 담당 문서를 빠르게 확인할 때 사용합니다.
...
| Product Repository | 정확한 라벨은 `Product Repository`, 일반 산문은 제품 저장소 | 하네스 런타임 상태와 구분되는 사용자의 프로젝트 작업 공간입니다. | [런타임 경계](runtime-boundaries.md) |
| Harness Runtime Home | 정확한 라벨은 `Harness Runtime Home`, 일반 산문은 런타임 홈 | 하네스 기록과 아티팩트를 담는 운영 데이터 공간입니다. | [런타임 경계](runtime-boundaries.md) |
| documentation | 문서 | 유지되는 원천 자료입니다. 런타임 출력, 제품 구현, 수락 상태와 구분합니다. | [작성 가이드](../maintain/authoring-guide.md) |
...
| out-of-scope capability | 지원 범위 밖 기능 | 적용되는 담당 문서들이 승격하기 전까지 기준 지원 경계 밖에 있는 기능입니다. | [기준 범위](scope.md) |
| evidence collection workflow | 증거 수집 흐름 | 지원 여부를 기준 범위가 담당하는 증거 흐름 기능 표현입니다. | [기준 범위](scope.md) |
| expanded or additional evidence collection workflows | 확장 또는 추가 증거 수집 흐름 | 기준 범위와 영향받는 담당 문서가 승격하기 전까지 지원 범위 밖인 증거 수집 흐름 묶음입니다. | [기준 범위](scope.md) |
...
| owner placeholder | 담당 문서 자리표시자 | 지원 전 담당 문서를 만들거나 지정해야 함을 표시합니다. | [작성 가이드](../maintain/authoring-guide.md) |
| `Task` | `Task` | 작업 범위, 권한 맥락, 판단, 증거, 닫기 준비 상태를 묶는 하네스 개체입니다. | [Core 모델](core-model.md) |
...
| close task behavior | `Task` 닫기 동작 | 메서드별 요청, 평가, 결과를 묶어 가리키는 표현입니다. | [Task 닫기 메서드](api/method-close-task.md) |
...
| complete intent | `complete` | 일반 산문의 "전체"가 아니라 `complete` 의도 값입니다. | [API 값 집합](api/schema-value-sets.md) |
| full evaluation order | 전체 평가 순서 | `complete` 값이 아니라 평가 순서 전체를 가리키는 일반 산문 표현입니다. | [번역 가이드](../maintain/translation-guide.md) |
| artifact | 아티팩트 | 참조되거나 스테이징되는 작업 자료를 가리키는 하네스 개념입니다. | [API 아티팩트 스키마](api/schema-artifacts.md) |
...
| error routing | 오류 처리 경로 | 거부 응답, 차단 결과, dry-run 응답 분기를 나누는 처리 경로입니다. | [API 오류 처리 경로](api/error-routing.md) |
...
| error detail helper values | 오류 세부사항 보조 값 | 기계 판독용 오류 세부사항의 보조 값입니다. | [API 오류 세부사항](api/error-details.md) |
| dry-run | dry-run 미리보기 | `dry_run`을 사용하는 API 호출의 미리보기 모드입니다. | [API 코어 스키마](api/schema-core.md) |
| dry-run preview routing | dry-run 미리보기 처리 경로 | `dry_run` 미리보기 응답 분기의 처리 경로입니다. | [API 오류 처리 경로](api/error-routing.md) |
| blocked result | 차단 결과 | 거부 응답이 아니라 차단을 보고하는 API 결과입니다. | [API 오류 처리 경로](api/error-routing.md) |
...
- 한국어 용어: 정확한 라벨은 `Product Repository`, 일반 산문은 제품 저장소
...
- 한국어 용어: 정확한 라벨은 `Harness Runtime Home`, 일반 산문은 런타임 홈
...
- 용어: documentation
- 한국어 용어: 문서
- 의미: 유지되는 원천 자료입니다. 런타임 출력, 제품 구현, 수락 상태와 구분합니다.
...
- 한국어 용어: 의미 골격
- 의미: 중요한 참조 섹션을 다듬기 전에 정하는 의미 단위 구조입니다.
- 주 담당 문서: [작성 가이드](../maintain/authoring-guide.md)
...
- 용어: baseline scope
- 한국어 용어: 기준 범위
- 의미: 하네스가 안정적으로 지원한다고 문서화한 경계입니다.
- 주 담당 문서: [기준 범위](scope.md)
...
- 한국어 용어: 지원 동작
- 의미: 기준 범위와 영향받는 담당 문서가 지원한다고 문서화한 동작입니다.
- 주 담당 문서: [기준 범위](scope.md)
- 함께 볼 문서: [API 값 집합](api/schema-value-sets.md)
...
- 한국어 용어: 지원되는 API 값
- 의미: 이름만 있거나 예약된 값이 아니라 지원된다고 문서화된 API 값입니다.
...
- 한국어 용어: 지원 범위 밖 기능
- 의미: 적용되는 담당 문서들이 승격하기 전까지 기준 지원 경계 밖에 있는 기능입니다.
- 주 담당 문서: [기준 범위](scope.md)
...
- 한국어 용어: 증거 수집 흐름
- 의미: 지원 여부를 기준 범위가 담당하는 증거 흐름 기능 표현입니다.
- 주 담당 문서: [기준 범위](scope.md)
...
- 의미: 기준 범위와 영향받는 담당 문서가 승격하기 전까지 지원 범위 밖인 증거 수집 흐름 묶음입니다.
- 주 담당 문서: [기준 범위](scope.md)
...
- 용어: existing owner
- 한국어 용어: 기존 담당 문서
- 의미: 이미 존재하고 규범 의미를 담을 수 있는 담당 문서입니다.
- 주 담당 문서: [작성 가이드](../maintain/authoring-guide.md)
- 함께 볼 문서: [참조 색인](README.md)
...
- 한국어 용어: 담당 문서 자리표시자
- 의미: 지원 전 담당 문서를 만들거나 지정해야 함을 표시합니다.
- 주 담당 문서: [작성 가이드](../maintain/authoring-guide.md)
- 함께 볼 문서: [기준 범위](scope.md)
...
- 용어: `Task`
- 한국어 용어: `Task`
- 의미: 작업 범위, 권한 맥락, 판단, 증거, 닫기 준비 상태를 묶는 하네스 개체입니다.
...
- 용어: active scope
- 한국어 용어: 현재 적용 범위
- 의미: `Task` 또는 Change Unit 맥락에서 현재 적용되는 범위입니다.
- 주 담당 문서: [Core 모델](core-model.md)
- 함께 볼 문서: [범위 갱신 메서드](api/method-update-scope.md)
...
- 한국어 용어: 사용자 소유 판단
- 의미: 하네스가 기록하지만 Core 소유 사실로 바꾸지 않는 사용자 결정이나 평가입니다.
- 주 담당 문서: [Core 모델](core-model.md)
...
- 용어: close task
- 한국어 용어: `Task` 닫기
- 의미: 사용자 또는 API가 `Task` 닫기를 시도하는 동작입니다.
- 주 담당 문서: [Task 닫기 메서드](api/method-close-task.md)
...
- 한국어 용어: `Task` 닫기 동작
- 의미: 메서드별 요청, 평가, 결과를 묶어 가리키는 표현입니다.
- 주 담당 문서: [Task 닫기 메서드](api/method-close-task.md)
...
- 용어: complete intent
- 한국어 용어: `complete`
- 의미: 일반 산문의 "전체"가 아니라 `complete` 의도 값입니다.
- 주 담당 문서: [API 값 집합](api/schema-value-sets.md)
...
- 용어: artifact
- 한국어 용어: 아티팩트
- 의미: 참조되거나 스테이징되는 작업 자료를 가리키는 하네스 개념입니다.
...
- 용어: evidence
- 한국어 용어: 증거
- 의미: 주장, 검증 결과, 사용자 판단 맥락을 뒷받침하는 기록 자료입니다.
- 주 담당 문서: [Core 모델](core-model.md)
...
- 의미: 저장된 프로젝트 상태의 순서를 나타내는 식별자입니다.
...
- 한국어 용어: 쓰기 권한 부여
- 의미: 하네스 쓰기 권한 부여 개념의 정확한 제품 라벨입니다.
- 주 담당 문서: [Core 모델](core-model.md)
...
- 용어: reserved value
- 한국어 용어: 예약된 값
- 의미: 그 자체만으로 기준 동작을 뜻하지 않는 예약 어휘나 노출 지점입니다.
- 주 담당 문서: [기준 범위](scope.md)
- 함께 볼 문서: [API 값 집합](api/schema-value-sets.md)
...
- 용어: error routing
- 한국어 용어: 오류 처리 경로
- 의미: 거부 응답, 차단 결과, dry-run 응답 분기를 나누는 처리 경로입니다.
...
- 의미: 공개 오류 코드 표현이 차단 사유 데이터에 나타나는 경우를 가리키는 경계 용어입니다.
- 주 담당 문서: [API 차단 사유 처리 경로](api/blocker-routing.md)
...
- 의미: `dry_run` 미리보기 응답 분기의 처리 경로입니다.
- 주 담당 문서: [API 오류 처리 경로](api/error-routing.md)
- 함께 볼 문서: [API 코어 스키마](api/schema-core.md)
...
- 용어: blocked result
- 한국어 용어: 차단 결과
- 의미: 거부 응답이 아니라 차단을 보고하는 API 결과입니다.
- 주 담당 문서: [API 오류 처리 경로](api/error-routing.md)
