...
| `ToolRejectedResponse` | 커밋 전 실패에 쓰는 거절 응답 분기입니다. 비어 있지 않은 `ToolRejectedResponse.errors: ToolError[]`를 담고, `response_kind=rejected`와 `effect_kind=no_effect`를 사용합니다. 메서드별 결과 필드, 재실행 행, 상태 버전 증가, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비를 만들지 않습니다. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md) |
...
| `ToolError` | 공개 오류 식별자, 재시도 안내, 구조화된 세부정보를 담는 공통 오류 형태입니다. `ToolRejectedResponse.errors` 안에서는 커밋 전 실패 오류를 담습니다. 정확한 공개 코드와 우선순위는 API Errors가 담당합니다. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md) |
| `WriteDecisionReason` | `PrepareWriteResult.write_decision_reasons`에 담기는 `prepare_write` 판단 사유입니다. `decision=blocked`, `decision=approval_required`, `decision=decision_required`를 설명합니다. `CloseBlocker`도, dry-run 예상 차단 사유도, 커밋 전 실패 컨테이너도 아닙니다. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [Core Model](core-model.md) |
| `PlannedBlocker` | `DryRunSummary.would_blockers`에만 쓰는 dry-run 예상 차단 사유입니다. 저장되지 않으며, `WriteDecisionReason`도 아니고 `CloseBlocker`도 아니며 커밋된 차단 결과도 아닙니다. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [API Errors](api/errors.md) |
| `STATE_VERSION_CONFLICT` | 현재 MVP에서 프로젝트 전체 최신성 충돌과 멱등성 충돌에 쓰는 공개 `ErrorCode`입니다. 오래된 `project_state.state_version` 기대값, 소비 전 오래된 `WriteAuthorization.basis_state_version`, 멱등성 `request_hash` 충돌이 여기에 속합니다. `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답으로만 반환되며, `WriteDecisionReason.code`, `CloseBlocker.code`, `PlannedBlocker.code`가 될 수 없습니다. | [API Errors](api/errors.md), [Storage](storage.md) |
...
| `CloseBlocker` | 닫기 사전 확인이 성공한 뒤 `close_task`의 닫기 차단 사유 행렬이 만든 커밋된 닫기 차단 결과입니다. `CloseTaskResult.blockers`에 담기며, 일반 차단 사유 타입이 아닙니다. `prepare_write`에서 쓰지 않고, `DryRunSummary.would_blockers`에서도 쓰지 않으며, 커밋 전 실패를 담는 거절 응답 컨테이너도 아닙니다. `STATE_VERSION_CONFLICT`는 `CloseBlocker.code`가 될 수 없습니다. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md), [Core Model](core-model.md) |
