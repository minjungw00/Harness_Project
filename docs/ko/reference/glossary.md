...
구현 계획 전에 agent가 사용하는 assumptions를 정리한 Discovery 또는 Shared Design support/projection 목록입니다. Source, confidence, owner, assumption이 틀릴 때 바뀌는 일을 이름 붙여야 합니다. 이는 권장 display/support 내용이지 standalone schema나 canonical record field list가 아닙니다. Assumption Register는 Discovery Brief, 첫 구현 후보, 작업 분할 제안, 또는 First Safe Change Unit Candidate를 구체화하는 데 도움을 주지만 user approval, sensitive-action Approval, acceptance, residual-risk acceptance, evidence, 닫기 준비 상태, scope authority, 쓰기 허가 기록은 아닙니다.
