...
| 읽기용 투영 문서(projection), managed block, projection freshness, Markdown 보고서, template body | [문서 Projection 참조](document-projection.md); [Template 참조](templates/README.md) |
...
Close-relevant 잔여 위험이 보였거나 없다고 확인된 뒤, 결과와 남은 trade-offs가 받아들일 만하다는 사용자의 판단입니다. Required final acceptance는 결정 패킷 user decision, `task_gates.acceptance_gate`, `state.sqlite.task_events`를 포함하는 kernel acceptance path를 통해 기록됩니다. Acceptance는 approval, assurance, verification, 수동 QA, evidence sufficiency, residual-risk acceptance와 구분됩니다. 추가 write를 허가하거나 빠진 check를 나중에 충족된 것으로 만들지 않습니다.
...
Capability는 validator results, blocked reasons, display에 영향을 주지만 Approval, 쓰기 허가 기록, verification, QA, acceptance, 잔여 위험을 받아들이는 판단, close readiness, kernel gate는 아닙니다. 정확한 level meanings는 [런타임 아키텍처](runtime-architecture.md#보장-수준)가 담당합니다.
