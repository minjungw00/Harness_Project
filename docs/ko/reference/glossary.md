...
`judgment_domain`은 결정 패킷에서 스키마가 소유한 판단 영역입니다. 값은 `product_ux`, `technical_architecture`, `security_privacy`, `qa_acceptance`, `residual_risk`, `scope_autonomy`, `mixed`이며, 표시는 제품/UX 또는 보안/개인정보 같은 자연스러운 라벨로 바꿔 보여줄 수 있습니다. `decision_kind`는 lifecycle, 사용자가 기록하는 답변 route, resolution payload branch, gate 의미, 상태 전이 의미를 제어합니다. `decision_profile`은 질문 깊이, Decision Packet `profile_payload` branch, profile별 필수 정보를 제어합니다. 영향을 받는 `gate`나 막힌 행동은 `affected_gates`와 관련 소유자 기록으로 별도 기록합니다. `judgment_domain`과 `decision_profile`은 사용자가 어떤 종류와 깊이의 판단을 요구받는지 이해하도록 돕지만 상태, `gate`, 소유자 기록, 검증기 입력, 닫기 집계 규칙, 권한 경로가 아닙니다. 여러 영역에 걸친 결정은 영역을 배타적으로 다루지 말고 부차적인 고려사항을 장단점, 영향받는 `gate`, 위험, 근거, 후속 작업에 보여줘야 합니다. 표시는 결정 유형, 결정 제목, 프로필에 맞는 선택 맥락, 사용자가 정확히 결정하는 것, 왜 지금 필요한지, 해당하는 경우 잔여 위험을 보이게 하되 `decision_kind`, 민감 동작 승인, 작업 수락, QA, 잔여 위험 수용, 닫기, 쓰기 허가 기록의 소유자 계약을 바꾸면 안 됩니다.
...
Decision Packet의 schema 필드인 `judgment_domain`입니다. `product_ux`, `technical_architecture`, `security_privacy`, `qa_acceptance`, `residual_risk`, `scope_autonomy`, `mixed`처럼 사용자에게 보이는 판단 영역을 묶습니다. 사용자가 어떤 판단을 요청받는지 이해하도록 돕지만 전체 결정 유형은 아니며 `gate`, status, 권한 경로, 검증기 입력, 닫기 집계 규칙, 영향받는 gate 관계, `decision_kind`의 대체물이 아닙니다. 사용자 표시에서는 자연스러운 한국어로 판단 영역을 보여주되, 해당 경로가 대기 중이면 민감 동작 승인, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용을 계속 구분해야 합니다.
