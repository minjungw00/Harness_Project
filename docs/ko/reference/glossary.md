...
| `Agent Connection` | 에이전트 연결 | 내부 `connection_id`와 함께 저장되는 로컬 MCP 호스트 연결 단위입니다. | [Agent Connection 참조](agent-connection.md) |
...
| `Connection Projects` | 연결 프로젝트 | Agent Connection에 명시적으로 허용된 내부 `project_id` 목록입니다. | [Agent Connection 참조](agent-connection.md) |
