...
| 식별자 | 짧은 설명 | 담당 경로 |
|---|---|---|
| 활성 MCP 메서드 | `harness.intake`, `harness.status`, `harness.update_scope`, `harness.prepare_write`, `harness.stage_artifact`, `harness.record_run`, `harness.request_user_judgment`, `harness.record_user_judgment`, `harness.close_task`. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md) |
...
| `ToolRejectedResponse` | 커밋 전 실패에 쓰는 상태 효과 없는 거절 응답 분기입니다. 비어 있지 않은 `ToolRejectedResponse.errors: ToolError[]`를 담고, `response_kind=rejected`와 `effect_kind=no_effect`를 사용합니다. 메서드별 결과 필드, 재실행 행, 상태 버전 증가, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, 닫기 상태 변경, 다른 응답 효과를 만들지 않습니다. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md) |
...
| `ToolError` | `ToolRejectedResponse.errors`에 담겨 커밋 전 실패를 전달하는 공개 오류 정보입니다. 공개 오류 식별자, 재시도 안내, 구조화된 세부정보를 유지합니다. 정확한 공개 코드와 우선순위는 API Errors가 담당합니다. | [API Schema Core](api/schema-core.md), [API Errors](api/errors.md) |
| `WriteDecisionReason` | `PrepareWriteResult.write_decision_reasons`에 담기는 `prepare_write` 판단 사유입니다. `decision=blocked`, `decision=approval_required`, `decision=decision_required`일 때 `prepare_write` 판단 결과를 설명합니다. 커밋 전 실패도, close 가능성 평가의 닫기 차단 사유도, dry-run 미리보기 데이터도 아닙니다. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [Core Model](core-model.md) |
| `CloseReadinessBlocker` | close 가능성 평가에서 닫기 가능성을 판단할 때 발견한 닫기 차단 사유의 데이터 구조입니다. 활성 사용 위치는 `StatusResult.close_blockers`와 `CloseTaskResult.blockers`입니다. 이 타입 자체는 저장 효과를 뜻하지 않는다. 지속 저장, `close_state` 변경, 다른 상태 효과를 뜻하지 않습니다. `prepare_write`, `DryRunSummary.would_blockers`, `ToolRejectedResponse.errors`에는 쓰지 않으며, `STATE_VERSION_CONFLICT`는 `CloseReadinessBlocker.code`가 될 수 없습니다. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [API Errors](api/errors.md), [Core Model](core-model.md), [Storage](storage.md) |
| `PlannedBlocker` | `DryRunSummary.would_blockers`에만 쓰는 dry-run 예상 차단 사유 데이터입니다. 저장되지 않으며, `WriteDecisionReason`도 아니고 `CloseReadinessBlocker`도 아니며 커밋된 차단 결과도 아닙니다. | [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [API Errors](api/errors.md) |
| `STATE_VERSION_CONFLICT` | 현재 MVP에서 프로젝트 전체 최신성 충돌과 멱등성 충돌에 쓰는 공개 `ErrorCode`입니다. 오래된 `project_state.state_version` 기대값, 소비 전 오래된 `WriteAuthorization.basis_state_version`, 멱등성 `request_hash` 충돌이 여기에 속합니다. `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답으로만 반환되며, `WriteDecisionReason.code`, `CloseReadinessBlocker.code`, `PlannedBlocker.code`가 될 수 없습니다. | [API Errors](api/errors.md), [Storage](storage.md) |
...
| `CloseTaskResult.close_state` | `harness.close_task`가 돌려주는 응답 수준의 닫기 상태입니다. 값은 `ready`, `blocked`, `closed`, `cancelled`, `superseded`입니다. 지속 저장되는 `Task.lifecycle_phase`와는 별도입니다. | [MVP API](api/mvp-api.md) |
