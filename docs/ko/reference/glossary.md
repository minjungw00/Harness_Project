...
Bilingual parity, links, owner boundaries, stable catalogs, glossary terms, source-of-truth phrasing, TODO usage, non-owner duplicate contracts의 drift를 detect하는 read-only documentation maintenance check profile입니다. Rule bodies는 [문서 작성 가이드](../maintain/authoring-guide.md#docs-maintenance-checks)가 담당하고, operator reporting과 entrypoint expectation은 [운영과 Conformance 참조](operations-and-conformance.md#docs-maintenance-프로필)가 담당합니다. Core fixture conformance, runtime validator, projection refresh, QA, acceptance, evidence, residual-risk acceptance, canonical state transition이 아닙니다. `harness conformance run` 또는 다른 operator command로 expose되면 명시적으로 선택하는 separate docs-only profile이며, 관례적 이름은 `docs-maintenance`입니다. Console 또는 ephemeral result는 runtime fixture pass/fail, implementation readiness, close readiness, Task state에 영향을 주지 않습니다.
...
Projection job과 template kind를 나타내는 API enum입니다. Tier, values, extension rules는 [Shared schemas](mcp-api-and-schemas.md#shared-schemas)가 담당합니다. 어떤 ProjectionKind도 projection을 canonical state로 만들지 않습니다.
...
External PR, review, deployment, rollback, monitoring process를 위한 release readiness를 summarize하는 optional report/export profile입니다. Close readiness, blocker, evidence ref, verification ref, Manual QA ref, residual-risk ref, changed file, projection freshness, redaction note, suggested checklist item을 포함합니다. Deployment authority, merge authority, approval, evidence, verification, QA, acceptance, residual-risk acceptance, assurance upgrade, production monitoring, Task close가 아닙니다.
