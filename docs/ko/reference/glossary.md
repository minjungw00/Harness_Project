...
기준 Decision Packet을 가리킬 수 있는 선택적 라우팅, 상호작용, 멱등 재시도, 호환성 인계 메타데이터입니다. Minimal 내부 엔지니어링 점검 구현은 이를 생략할 수 있습니다. Decision Request는 결정 권한이 아니며 그 자체로 `decision_gate`, 민감 동작 승인, 작업 수락, 면제 판단, 잔여 위험 수용, close를 절대 충족하지 않습니다. 관문 집계에는 호환되는 연결 `decision_packet_id`가 있을 때만 관련됩니다.
