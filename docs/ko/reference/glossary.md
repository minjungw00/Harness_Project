...
Decision Packet의 스키마 필드인 `decision_kind`입니다. 상태 흐름, payload 분기, 관문 의미, 상태 전이 의미를 제어합니다. 사용자에게 보이는 묶음인 `judgment_domain`과 구분됩니다. 표시는 쉬운 말로 결정 경로를 설명할 수 있지만 스키마/API 문맥에서는 field name과 enum value를 정확히 유지합니다.

### Decision Profile

한국어 기준 표현: 결정 프로필.

Decision Packet의 스키마 필드인 `decision_profile`입니다. `minimal_decision`, `product_ux_tradeoff`, `architecture_tradeoff`, `approval_shaped`, `waiver`, `acceptance`, `residual_risk_acceptance`, `reconcile`, `mixed`처럼 결정 기록이 요구하는 질문 깊이를 제어합니다. `decision_kind`, `judgment_domain`과 구분됩니다. `decision_kind`는 상태 흐름의 의미를 제어하고, `decision_profile`은 필요한 세부사항의 양을 제어하며, `judgment_domain`은 독자가 이해하기 쉽게 판단을 묶습니다.
...
`judgment_domain`은 결정 패킷에서 스키마가 소유한 판단 영역입니다. 값은 `product_ux`, `technical_architecture`, `security_privacy`, `qa_acceptance`, `residual_risk`, `scope_autonomy`, `mixed`이며, 표시는 제품/UX 또는 보안/개인정보 같은 자연스러운 라벨로 바꿔 보여줄 수 있습니다. 영향을 받는 `gate`나 막힌 행동은 `affected_gates`와 관련 소유자 기록으로 별도 기록합니다. `judgment_domain`과 `decision_profile`은 사용자가 어떤 종류와 깊이의 판단을 요구받는지 이해하도록 돕지만 상태, `gate`, 소유자 기록, 검증기 입력, 닫기 집계 규칙, 권한 경로가 아닙니다. 여러 영역에 걸친 결정은 영역을 배타적으로 다루지 말고 부차적인 고려사항을 장단점, 영향받는 `gate`, 위험, 근거, 후속 작업에 보여줘야 합니다. 표시는 결정 유형, 결정 제목, 프로필에 맞는 선택 맥락, 사용자가 정확히 결정하는 것, 왜 지금 필요한지, 해당하는 경우 잔여 위험을 보이게 하되 `decision_kind`, 민감 동작 승인, 작업 수락, QA, 잔여 위험 수용, 닫기, 쓰기 허가 기록의 소유자 계약을 바꾸면 안 됩니다.
