...
| Public error, precedence, idempotency, blocked behavior, stale-state behavior | [API Errors](errors.md) |
| Core state semantics와 lifecycle 의미 | [Core Model 참조](../core-model.md) |
| Storage table, JSON `TEXT`, enum hardening, artifact persistence | [Storage](../storage.md) |
| Security guarantee 의미 | [보안 참조](../security.md) |
...
이 문서의 YAML-like block은 예시라고 표시하지 않는 한 normative schema notation입니다.

- `field: Type`은 field가 required이고 non-null이라는 뜻입니다.
- `field: Type | null`은 field가 required이고 JSON `null`을 허용한다는 뜻입니다.
- `Type[]`은 field가 present이고 array를 담는다는 뜻입니다. Empty array는 `[]`로 씁니다.
- `a | b | c`는 closed active enum입니다.
- 명시되지 않은 field는 명시적인 extension container 밖에서 reject됩니다.

Storage validation은 별도 owner 경계입니다. API payload와 API-shaped stored JSON은 먼저 이 API reference로 validate합니다. DDL, storage-only JSON, default, lock, migration은 [Storage](../storage.md)가 담당합니다.
...
Envelope field는 call을 route하고 audit합니다. `surface_id`는 capability, write authority, local access, user judgment, 민감 동작 승인, final acceptance, residual-risk acceptance, close를 부여하지 않습니다.
...
`ToolResponseBase.state_version`은 committed mutation에서는 affected scope의 resulting version이고, read-only와 dry-run response에서는 current readable version 또는 would-be affected version입니다. `dry_run=true`는 current record, event, artifact, evidence summary, Write Authorization, close state, idempotency replay row를 만들지 않습니다.
...
`ArtifactRef`는 Harness storage에 등록된 durable evidence file을 가리킵니다. Caller가 임의로 준 path가 아닙니다.
...
`uri`는 Harness storage를 통해 resolve되며 보통 `harness-artifact://{project_id}/{artifact_id}`입니다. Raw secret, token, full sensitive log를 evidence로 저장하면 안 됩니다. Content가 redacted, omitted, blocked이면 `sha256`와 `size_bytes`는 hidden original이 아니라 committed safe bytes를 설명합니다.

<a id="artifactinput"></a>

## ArtifactInput

`ArtifactInput`은 `harness.record_run`에서 staging, capture-adapter, existing-artifact handle로만 받습니다. Arbitrary file read authority를 부여하지 않습니다.
...
`source_kind`에 맞는 source field 하나만 있어야 합니다. Invalid source shape, caller-supplied arbitrary path, raw secret, token, full sensitive log는 mutation 전에 reject됩니다.
...
`AuthorizedAttemptScope`는 `write_authorizations.attempt_scope_json`에 저장되고 나중에 `harness.record_run`에서 비교하는 exact scope입니다. `WriteAuthorizationSummary.status`는 durable authorization lifecycle입니다. `blocked`는 Write Authorization status가 아닙니다. Blocked write는 consumable authorization 없이 blocker를 반환합니다.
...
`judgment_kind`는 canonical decision-type field입니다. 렌더링된 label과 localized label은 schema value가 아닙니다. `presentation=short`가 활성 MVP presentation입니다. Expanded presentation body는 활성 API schema가 아닙니다.
...
Sensitive category는 왜 민감 동작 승인이 필요할 수 있는지 설명합니다. Product, technical, scope, QA, verification, acceptance, residual-risk, policy question을 결정하지 않습니다.
