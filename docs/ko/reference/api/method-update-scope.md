...
이 문서는 현재 MVP에서 `harness.update_scope` 메서드가 담당하는 동작을 담당합니다.
...
params:
  ...
  scope_update:
    ...
    exclude:
      - "계정 삭제 동작"
      - "청구 내보내기 동작"
  scope_boundary: "계정 내보내기 확인 UI를 추가하고 계정 내보내기 확인 테스트를 갱신한다."
  non_goals:
    - "계정 삭제 동작"
    - "청구 내보내기 동작"
  acceptance_criteria:
    - "다운로드 전에 계정 내보내기 확인 단계가 필요하다."
  autonomy_boundary: "계정 내보내기 확인 UI와 계정 내보내기 확인 테스트 범위 안에서만 작업한다."
