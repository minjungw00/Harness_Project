...
확인이 허용되면 소비 가능한 단일 사용 쓰기 승인(`Write Authorization`)을 만듭니다. 확인이 허용되지 않으면 그 쓰기 승인 경로를 거부하거나 미룹니다.
...
- 쓰기 승인은 경로 수준 `AuthorizedAttemptScope`에 묶입니다.
...
계정 데이터 내보내기 확인 문구에 대한 사용자 소유 판단은 사용자 소유 판단 요청과 기록 메서드에서 별도로 처리되며, 이 민감 동작 승인과 같은 승인이 아닙니다.
