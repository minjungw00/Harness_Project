...
- 활성 목록 밖의 값은 [범위 참조](../scope.md)와 영향받는 의미 담당 문서가 지원 동작을 정의하기 전까지 기준 범위 API 값이 아닙니다.

비주장:

- 활성 목록 밖의 이름을 적는 것만으로 활성 범위가 넓어지지 않습니다.
...
예약된 값이나 프로필 조건부 값은 기준 범위의 기본 활성 값이 아닙니다. 이 문서는 지원되지 않는 값 이름을 활성 값 집합으로 공개하지 않습니다.
