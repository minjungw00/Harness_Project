...
주 오류 우선순위, API 오류와 차단 사유 경로, `ToolError.details`, 응답 분기 형태, 표시 라벨, 저장 효과, 보안 보장은 정의하지 않습니다.
...
- 거부 응답, 차단 결과, `dry_run`, `close_task` 경로: [API 오류 경로](error-routing.md).
...
허용되지 않는 것:
- 더 구체적인 지원 코드가 있으면 이 대체 코드를 사용하지 않습니다.
