...
이 메서드는 간결한 증거 범위를 갱신하고, 제품 쓰기를 기록할 때 호환되는 쓰기 승인을 소비하며, 기존 아티팩트를 연결하고, 허용되는 경우 적격 스테이징 핸들을 지속 `ArtifactRef`로 승격할 수도 있습니다.
...
오래된 `expected_state_version`과 오래된 승인 기준 상태는 활성 쓰기 승인을 소비하기 전에 거절됩니다.
...
- 커밋된 차단 결과는 유효하지 않은 스테이징 핸들, 누락된 쓰기 승인, 오래된 상태, 오래된 승인 기준 상태, 로컬 접근 실패를 숨기면 안 됩니다.
