...
- 현재 상태 버전이 권한 부여 기준 상태와 여전히 맞습니다.
- 관찰된 변경 경로가 권한 부여된 시도와 호환됩니다.

오래된 `expected_state_version`과 오래된 권한 부여 기준 상태는 `Write Authorization`을 소비하기 전에 거절됩니다.
...
- 커밋된 차단 결과는 유효하지 않은 스테이징 핸들, 누락된 `Write Authorization`, 오래된 상태, 오래된 권한 부여 기준 상태, 로컬 접근 실패를 숨기면 안 됩니다.
...
커밋 시 실행, 증거, 차단 사유, 권한 부여 소비, 아티팩트 연결 결과를 지속할 수 있습니다. 정확한 저장 효과와 아티팩트 승격 세부사항은 아래 저장 담당 문서가 담당합니다.
...
params:
  ...
  summary: "인보이스 다운로드 확인 테스트가 통과했습니다."
  observed_changes:
    changed_paths: []
  ...
  artifact_inputs: []
  evidence_updates:
    - claim: "인보이스 다운로드 확인 테스트가 통과했습니다."
