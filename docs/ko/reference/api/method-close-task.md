...
- 지속 저장은 메서드 분기와 저장 효과 담당 문서가 허용할 때만 가능합니다. `CloseReadinessBlocker`가 있다는 사실만으로는 충분하지 않습니다.
...
`intent=check`와 `dry_run=true`는 읽기 전용 `CloseTaskResult` 분기에 남습니다. 상태 변경 `intent`의 `dry_run=true`는 유효할 때 공통 미리보기 분기를 사용합니다.
