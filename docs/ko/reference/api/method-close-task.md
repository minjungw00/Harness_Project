...
`intent=check`와 `dry_run=true`는 읽기 전용 `CloseTaskResult` 분기에 남습니다. 상태 변경 `intent`의 `dry_run=true`는 유효할 때 공통 미리보기 분기를 사용합니다. 분기 형태는 [API 코어 스키마](schema-core.md)가 담당하고, 계획 차단 사유 응답 분기 경로는 [API 오류 경로](error-routing.md)가 담당합니다. 닫기 준비 상태 blocker 처리 경로는 [API blocker 처리 경로](blocker-routing.md)가 담당합니다.
...
- 공개 오류와 닫기 준비 상태 blocker 처리 경로: [API 오류 코드](error-codes.md), [`close_task` blocker 매핑](blocker-routing.md#harnessclose_task-close-blockers).
