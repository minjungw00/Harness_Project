...
현재 MVP의 public error code, primary-error precedence, blocked/dry-run behavior, idempotency replay, state conflict behavior, close blocker behavior, 사용자 표시 label guidance를 확인할 때 이 참조를 사용합니다.

이 문서는 향후 하네스 서버 동작을 계획하고 검토하기 위한 참조입니다. 현재 문서 저장소에 MCP server가 구현되어 있다는 뜻이 아닙니다.

<a id="mvp-1-guarantee-and-status-taxonomy"></a>

## MVP-1 guarantee와 상태 taxonomy
...
| `preventive` | 승격된 profile이 covered operation을 실행 전에 막는 증명된 control을 가지고 있습니다. Exact operation과 proof가 이름 붙은 경우에만 사용합니다. |
| `isolated` | 승격된 profile이 해당 claim에 맞는 문서화되고 증명된 separation boundary를 가지고 있습니다. Boundary를 이름 붙이며, isolation만으로 더 넓은 authority를 추론하지 않습니다. |

활성 MVP behavior는 기본적으로 협력형 확인입니다. 연결된 surface가 사실을 정직하게 관찰할 수 있을 때만 제한된 사후 확인을 함께 표시합니다.

| Condition | Public path | Agent rule |
|---|---|---|
| `core_unavailable` | `MCP_UNAVAILABLE` | 하네스 상태를 만들어 내지 않습니다. Core에 다시 닿거나 사용자가 하네스 밖 진행을 명시적으로 선택하기 전까지 하네스 의존 write와 close를 보류합니다. |
| `local_access_denied` | `LOCAL_ACCESS_MISMATCH` 또는 `CAPABILITY_INSUFFICIENT` | 로컬 파일이나 명령 사실을 추측하지 않습니다. 가능한 local surface를 쓰거나, profile을 고치거나, scope를 줄이거나, 입력을 unverified로 표시합니다. |
...
## Blocked와 dry-run behavior

Blocked response는 pre-commit failure와 다릅니다. Method owner가 blocker recording을 허용하는 경우에만 Core가 blocked response를 commit할 수 있습니다. Committed blocked response는 `blockers`, events, state version, idempotency replay를 업데이트할 수 있지만, blocker가 missing이라고 말하는 authority를 만들면 안 됩니다.
...
## 사용자 표시 label guidance

아래 label은 display guidance이지 새 public error code가 아닙니다.

| API condition | User-facing label | 가장 작은 unblocker |
|---|---|---|
| `VALIDATION_FAILED` | invalid request | Retry 전에 payload, enum value, activation rule, field set을 고칩니다. |
| `STATE_CONFLICT` | state conflict | Current status를 refresh하고 current state version으로 retry하거나 original idempotent request를 replay합니다. |
| `MCP_UNAVAILABLE` | Core unavailable | State change, gate update, write compatibility, close를 주장하기 전에 Core access를 reconnect 또는 diagnose합니다. |
| `LOCAL_ACCESS_MISMATCH` | local access denied or off-profile | Registered local surface/profile을 사용하거나, local access를 repair하거나, capable surface로 옮깁니다. |
...
| `NO_ACTIVE_TASK` | no active Task | Task-scoped action 전에 Task를 select 또는 create합니다. |
...
| `DECISION_REQUIRED`, `DECISION_UNRESOLVED` | judgment needed | Kind, refs, options, consequences와 함께 focused `UserJudgment`를 보여 주거나 resolve합니다. |
...
| `EVIDENCE_INSUFFICIENT` | evidence needed | Missing check를 record/rerun하거나 evidence gap과 가장 작은 unblocker를 보여 줍니다. |
| `ACCEPTANCE_REQUIRED` | final acceptance needed | Visible result basis에 대해 `judgment_kind=final_acceptance`를 request 또는 resolve합니다. |
| `RESIDUAL_RISK_NOT_VISIBLE` | residual risk not visible | Final acceptance 또는 close 전에 close-relevant risk를 보여 줍니다. |
| `PROJECTION_STALE` | stale readable view | 그 view에 의존하기 전에 refresh합니다. Canonical close state로 취급하지 않습니다. |
| `ARTIFACT_MISSING` | artifact issue | Missing/failed artifact를 reattach, regenerate, replace한 뒤 의존합니다. |
| `VALIDATOR_FAILED` | check or blocker failed | Specific validator 또는 blocker를 보여 줍니다. Typed blocker가 없을 때만 fallback으로 사용합니다. |
