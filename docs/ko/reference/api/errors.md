...
| `LOCAL_ACCESS_MISMATCH` | local access denied or off capability | Registered local surface를 사용하거나, local access를 repair하거나, capable surface로 옮깁니다. |
| `CAPABILITY_INSUFFICIENT` | unsupported or insufficient surface | Capable surface를 사용하거나, operation을 줄이거나, missing capability가 필요 없는 path를 선택합니다. |
