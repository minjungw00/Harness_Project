...
| `out_of_scope` | `SCOPE_REQUIRED`, `SCOPE_VIOLATION`, `NO_ACTIVE_CHANGE_UNIT`, `AUTONOMY_BOUNDARY_EXCEEDED`, `BASELINE_STALE` | 영향을 받는 행동을 보류하고, 불일치를 보여 주며, current scope로 줄이거나 구체적인 사용자 소유 범위 판단을 요청합니다. |
| `missing_judgment` | `DECISION_REQUIRED`, `DECISION_UNRESOLVED`, `APPROVAL_REQUIRED`, `APPROVAL_DENIED`, `APPROVAL_EXPIRED`, `ACCEPTANCE_REQUIRED` | 집중된 `UserJudgment`를 묻거나 해결합니다. 제품 판단, 기술 판단, 범위 판단, 민감 동작 승인, 최종 수락, 잔여 위험 수락, QA 면제 판단, 검증 위험 수락, 취소 판단을 넓은 approval 하나로 합치지 않습니다. |
| `missing_evidence` | `EVIDENCE_INSUFFICIENT`, `ARTIFACT_MISSING` | 영향을 받는 주장, refs, evidence status, 가장 작은 unblocker를 보여줍니다. Test result, artifact integrity, evidence sufficiency를 만들어 내지 않습니다. |
| `close_blocked` | `CloseTaskResponse.close_state=blocked`와 primary `ErrorCode` | 구조화된 blocker와 next action을 반환합니다. Task를 terminal로 표시하지 않습니다. |
...
| Code | 의미 |
|---|---|
| `VALIDATION_FAILED` | payload 형태, enum 값, activation rule, profile-specific validation이 mutation 전에 실패했습니다. |
...
| `NO_ACTIVE_CHANGE_UNIT` | 쓰기 가능한 또는 닫기 관련 operation에 active scoped Change Unit이 없습니다. |
| `SCOPE_REQUIRED` | 요청한 쓰기나 action 전에 scope confirmation이 필요합니다. |
| `SCOPE_VIOLATION` | 의도했거나 관찰된 paths, tools, commands, network targets, secret access, sensitive categories가 active scope 또는 stored `AuthorizedAttemptScope`를 넘었습니다. |
| `WRITE_AUTHORIZATION_REQUIRED` | 쓰기 가능한 Run에 `harness.prepare_write`의 required Write Authorization이 없습니다. |
| `WRITE_AUTHORIZATION_INVALID` | 제공된 Write Authorization이 missing, expired, stale, revoked, replay 밖에서 consumed, 또는 incompatible입니다. |
...
| `AUTONOMY_BOUNDARY_EXCEEDED` | 의도한 operation이 active Change Unit Autonomy Boundary를 넘었습니다. |
...
필요한 authorization이 supplied되지 않았으면 `authorization_reason=missing`과 함께 `WRITE_AUTHORIZATION_REQUIRED`를 사용합니다. 기존 authorization을 consume할 수 없으면 `WRITE_AUTHORIZATION_INVALID`를 사용합니다.
...
Committed replay row가 없는 새 state-changing attempt에서 Core는 freshness check 전에 primary Task를 찾습니다. Resolution order는 tool-specific `task_id`, `ToolEnvelope.task_id`, active Task 순서입니다.

Task-scoped mutation은 `expected_state_version`을 `tasks.state_version`과 비교합니다. 찾은 primary Task가 없는 project-scoped mutation은 `project_state.state_version`과 비교합니다. Mismatch는 `STATE_CONFLICT`를 반환하고 current record, event, artifact, evidence summary, Write Authorization, close state, replay row를 만들지 않습니다.
...
| `MCP_UNAVAILABLE` | Core 사용 불가 | State change, gate update, write compatibility, close를 주장하기 전에 Core access를 다시 연결하거나 진단합니다. |
