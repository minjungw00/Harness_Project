...
| `CAPABILITY_INSUFFICIENT` | Connected surface는 valid하지만 해당 `capability_profile`이 required observation, capture, local access, blocking/isolation condition, guarantee claim, active MVP behavior를 정직하게 충족할 수 없습니다. |
...
| `PROJECTION_STALE` | Requested readable status/projection view가 stale 또는 failed입니다. Active MVP에서는 그 자체로 close blocker가 아닙니다. |
...
`harness.record_run`은 write authorization과 observation failure를 public error로 이렇게 매핑합니다.

| `record_run` path | Primary public error | Required handling |
|---|---|---|
| Product-write Run에 required authorization이 없음 | Details에 reason을 담을 때 `authorization_reason=missing`을 가진 `WRITE_AUTHORIZATION_REQUIRED` | Commit 전에 reject합니다. Run, artifact, evidence summary, authorization consumption, event, replay row, close-relevant state를 만들지 않습니다. |
| Authorization이 stale, expired, revoked, consumed, incompatible임 | Matching `authorization_reason`을 가진 `WRITE_AUTHORIZATION_INVALID` | Commit 전에 reject하고 authorization을 consumed로 표시하지 않습니다. |
| Observed attempt가 stored `AuthorizedAttemptScope` 또는 active scope 밖임 | `SCOPE_VIOLATION` | Mismatched path/tool/command/network/secret/sensitive-category class를 display-safe detail로 보고합니다. 해당 write를 compatible로 취급하지 않습니다. |
| Required comparison에 필요한 observed field가 unsupported이거나 surface capability가 부족함 | `CAPABILITY_INSUFFICIENT` | Claim을 줄이거나 막습니다. Observed되지 않은 command, network access, secret access, artifact capture, pre-tool blocking, isolation fact를 verified로 표시하지 않습니다. |
| Forbidden secret 또는 artifact handling | Mutation 전 invalid artifact input shape/source 또는 forbidden raw secret payload에는 `VALIDATION_FAILED`; observed secret/artifact handling이 authorized scope를 넘으면 `SCOPE_VIOLATION`; existing committed artifact ref가 missing이거나 integrity/redaction metadata check에 실패하면 `ARTIFACT_MISSING` | Raw secret, token, full sensitive log를 evidence로 저장하지 않습니다. Required evidence가 failed artifact/ref에 의존하면 evidence를 stale 또는 blocked로 유지하고 close도 blocked로 유지합니다. |
...
- `CAPABILITY_INSUFFICIENT`: caller는 recognized surface/profile에 있지만 required observation, capture, local access, blocking/isolation condition, validator, guarantee claim을 충족할 수 없습니다. Unsupported `capability_profile` field는 가능하면 display를 낮추고, write, observation, claim에 필요하면 막습니다.
...
| `EVIDENCE_INSUFFICIENT` | evidence needed | Missing check를 record/rerun하거나 specific evidence gap과 가장 작은 해소 방법을 보여 줍니다. |
| `ACCEPTANCE_REQUIRED` | final acceptance needed | Visible result basis에 대해 `judgment_kind=final_acceptance` user judgment를 request 또는 resolve합니다. |
| `RESIDUAL_RISK_NOT_VISIBLE` | residual risk not visible | Final acceptance 또는 close 전에 close-relevant risk를 보여 줍니다. 이는 residual-risk acceptance와 별개입니다. |
| `PROJECTION_STALE` | stale readable view | 그 readable status/projection view에 의존하기 전에 refresh합니다. 이를 canonical close state로 취급하지 않습니다. |
...
| 20 | `RESIDUAL_RISK_NOT_VISIBLE` | Known close-relevant residual risk가 visible하지 않습니다. |
| 21 | `ACCEPTANCE_REQUIRED` | Residual-risk visibility가 충족된 뒤 required final acceptance가 pending 또는 rejected입니다. |
| 22 | `PROJECTION_STALE` | Requested readable status/projection view가 stale 또는 failed입니다. |
| 23 | `ARTIFACT_MISSING` | Referenced artifact file이 missing이거나 integrity check가 failed입니다. |
| 24 | `VALIDATOR_FAILED` | 더 specific한 typed blocker가 없을 때만 generic validator fallback으로 선택합니다. |
...
Unresolved user judgment blocker는 `user_judgment`를 사용하고, missing kind를 알 수 있으면 `required_judgment_kind`를 포함해야 합니다. Active MVP close blocker는 `CloseTaskResponse.blockers.required_judgment_kind`에 있는 judgment kind만 사용합니다. 값은 `product_decision`, `technical_decision`, `scope_decision`, `sensitive_approval`, `final_acceptance`, `residual_risk_acceptance`, `cancellation`입니다. Final acceptance는 residual-risk acceptance를 해소하지 않고, residual-risk acceptance는 final acceptance를 해소하지 않습니다. Sensitive approval은 product decision이나 technical decision을 해소하지 않습니다. Broad approval text는 prompt, kind, affected object, scope, recorded user intent가 맞지 않으면 어떤 판단도 해소하지 않습니다.
...
`PROJECTION_STALE`은 readable-view freshness error입니다. Active MVP close-blocker category가 아닙니다. Requested status/projection view가 display에 쓰기에는 너무 오래됐거나 failed이면 나타날 수 있지만, later/profile owner가 extension을 활성화하기 전에는 `CloseTaskResponse.blockers`에 projection/report freshness를 포함하면 안 됩니다.
