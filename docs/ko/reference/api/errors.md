...
| `isolated` | 승격된 profile이 해당 주장에 맞는 문서화되고 증명된 separation boundary를 가지고 있습니다. | Boundary를 이름 붙입니다. Isolation만으로 민감 동작 승인, 근거, 최종 수락, 잔여 위험 수락, close, 더 강한 authority를 추론하지 않습니다. |
...
Core unavailable rule: Harness/Core authority를 사용할 수 없으면 agent는 Task 상태, 민감 동작 승인, 사용자 판단, 근거, 최종 수락, 잔여 위험 수락, 닫기 준비 상태를 만들어 내면 안 됩니다. Authority를 사용할 수 없다고 보고할 수만 있으며, 사용자가 그 방식을 명시적으로 선택한 경우에만 하네스 밖에서 진행할 수 있습니다.
