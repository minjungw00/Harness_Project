...
- 예시는 계정 데이터 내보내기 전에 명시적 확인 단계를 추가하고, 계정 데이터 내보내기 확인 테스트를 갱신하며, 계정 데이터 내보내기 확인 테스트 출력을 대표 실행과 증거 데이터로 기록하는 오래 유지되는 작업을 사용합니다.
- 이 샘플을 바꾸거나 API 예시를 검토하는 규칙은 [작성 가이드](../../maintain/authoring-guide.md)와 [문서 점검](../../maintain/checks.md)이 담당합니다.
