...
메서드 담당 문서의 예시는 오래 유지되는 계정 데이터 내보내기 확인 시나리오를 사용합니다.

- `Task` 요약: 계정 데이터 내보내기 확인.
- 범위: 계정 데이터 내보내기 확인 UI와 계정 내보내기 확인 테스트.
- 범위 밖: 계정 삭제 동작과 청구 내보내기 동작.
- 수락 기준: 다운로드 전에 명시적 확인 단계가 필요합니다.
- 확장: 메서드 예시는 계정 내보내기 확인 테스트와 대표 실행 및 증거 데이터를 더할 수 있습니다.
