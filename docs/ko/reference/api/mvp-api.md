...
`dry_run=true`는 기준 권한이 아닙니다. 유효한 dry run은 `ToolDryRunResponse.dry_run_summary`에 설명용 `PlannedEffect` 예상 효과, 후보 차단 사유, 예상 오류, 다음 행동을 담습니다. 현재 기록, `task_events` 행, 지속 `ArtifactRef`, Write Authorization, 증거 요약, 닫기 상태, `tool_invocations` 재실행 행, 상태 버전 증가를 만들지 않으며, 아직 존재하지 않는 기록의 가짜 ref를 미리보기 설명에 넣으면 안 됩니다.
...
`dry_run=true`는 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 만들거나 갱신하거나 재개할 Task 또는 Change Unit을 `PlannedEffect` 항목으로 설명할 수 있습니다. 하지만 실제 Task나 Change Unit을 만들지 않고 실제 `task_ref`나 `change_unit_ref`를 반환하지 않습니다.
...
`dry_run=true`는 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 호환되는 Write Authorization이 오래된 상태가 될 수 있음을 `PlannedEffect` 항목으로 설명할 수 있습니다. 하지만 실제 Write Authorization을 `stale`로 표시하지 않고 실제 `stale_write_authorization_refs`를 반환하지 않습니다.
...
- **`dry_run` 분기:** `dry_run=true`는 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 non-dry-run 경로가 Write Authorization을 만들지, 기존 권한을 사용할지, 만들지 않을지를 미리 보여 줄 수 있습니다. 하지만 소비 가능한 Write Authorization을 만들지 않고 실제 `write_authorization_ref`를 반환하면 안 됩니다.
...
- **상태 효과:** 호환되는 커밋 호출은 `runs`를 만들고, 유효한 `artifact_staging` handle을 소비하고, 그 스테이징된 핸들을 지속 `artifacts`로 승격하고, `artifact_links`를 추가하고, `evidence_summaries`를 만들고, Run 관련 차단 사유를 업데이트하고, `write_authorizations.status=active`를 소비하고, 이벤트와 커밋된 재실행 행을 만들며, `project_state.state_version`을 정확히 한 번 올릴 수 있습니다. 제품 쓰기 Run은 현재 `project_state.state_version`이 Write Authorization의 프로젝트 전체 `basis_state_version`과 맞고, 관찰된 변경 경로가 저장된 승인 시도와 호환될 때만 활성 Write Authorization을 소비합니다. 권한의 근거 버전이 현재 `project_state.state_version`과 더 이상 맞지 않으면 그 시도는 소비 전에 `STATE_VERSION_CONFLICT`를 반환합니다. 거부된 호출, dry run, 커밋 전 실패는 Run 생성, 아티팩트 승격 또는 연결, 스테이징된 핸들 소비 또는 갱신, 증거 업데이트, 유효하지 않은 Write Authorization 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하면 안 됩니다.
- **`dry_run` 분기:** `dry_run=true`는 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 Run, 아티팩트, 증거, 차단 사유, Write Authorization 예상 효과를 미리 보여 줄 수 있습니다. 하지만 `run_summary`를 만들거나 반환하지 않고, 스테이징된 아티팩트를 소비하거나 승격하지 않으며, 아티팩트 연결, 증거 업데이트, Write Authorization 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하지 않습니다.
- **커밋 전 거절:** 유효하지 않은 스테이징된 핸들, Write Authorization 없음 또는 유효하지 않음, 오래된 Write Authorization 근거, 오래된 `expected_state_version`은 `ToolRejectedResponse`를 반환합니다. 거절 응답은 `run_summary` 생성, 아티팩트 승격, 스테이징된 핸들 소비, 증거 업데이트, Write Authorization 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하면 안 됩니다.
...
- **`dry_run` 분기:** `dry_run=true`는 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 판단 해결과 그에 따른 차단 사유 또는 다음 행동 예상 효과를 보여 줄 수 있습니다. 하지만 판단을 해결하거나 판단 상태를 바꾸지 않고, 차단 사유, 이벤트, 재실행 행, `state_version`을 업데이트하지 않습니다.
