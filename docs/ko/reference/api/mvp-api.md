...
4. `dry_run=false`로 커밋된 동작 또는 성공한 스테이징 동작은 메서드별 `MethodResult`를 반환합니다.
...
메서드에 도구별 `task_id`가 있으면 Core는 도구별 `task_id`, `ToolEnvelope.task_id`, 활성 Task 순서로 주 Task를 찾습니다. 이 해석은 담당 기록을 고르는 일이며 별도 상태 시계를 고르지 않습니다. 새 `dry_run=false` 상태 변경은 모두 커밋 전에 `ToolEnvelope.expected_state_version`을 현재 `project_state.state_version`과 비교합니다.
...
`dry_run=true`는 권한 근거가 아닙니다. 상태 효과가 있는 선택 동작 또는 스테이징 선택 동작의 유효한 `dry_run` 미리보기는 `ToolDryRunResponse.dry_run_summary`에 설명용 `PlannedEffect` 예상 효과, `PlannedBlocker` 후보 차단 사유, 미리보기 가능한 진단, 다음 행동을 담습니다. 현재 기록, `task_events` 행, 지속 `ArtifactRef`, 스테이징된 핸들, Write Authorization 생성 또는 소비, 증거 요약, 닫기 상태, `tool_invocations` 재실행 행, 상태 버전 증가를 만들지 않으며, 아직 존재하지 않는 기록의 가짜 ref를 미리보기 설명에 넣으면 안 됩니다.
...
이 상태 효과가 있는 메서드에서는 요청이 그 외에는 유효하고 미리보기 가능할 때 `dry_run=true`가 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 만들거나 갱신하거나 재개할 Task 또는 Change Unit을 `PlannedEffect` 항목으로 설명할 수 있습니다. 하지만 실제 Task나 Change Unit을 만들지 않고 실제 `task_ref`나 `change_unit_ref`를 반환하지 않습니다.

- **상태 효과:** 커밋된 `dry_run=false` 호출은 `tasks`를 만들거나 재개하고, `project_state.active_task_id`를 설정하며, 쓰기 가능한 확정된 `direct` 또는 `work`에 초기 범위 후보를 `change_units`에 만들고, 차단 사유를 업데이트하고, 이벤트와 커밋된 `tool_invocations` 재실행 행을 만들며, `project_state.state_version`을 정확히 한 번 올릴 수 있습니다. 요청이 아직 쓰기 가능한 상태가 아니라면 Task는 `lifecycle_phase=shaping`으로 남거나 그 상태가 되고, 현재 목표 요약, 알려진 범위와 범위 밖 항목, 영향 영역 또는 경로, 수락 기준, 자율성 경계, 첫 Change Unit 상태, 다음 안전한 행동을 막는 사용자 소유 blocker 범주가 있을 때 그 범주, 다음 안전한 행동 하나를 활성 Task, Change Unit, 사용자 판단, 증거, 차단 사유 필드로 표현합니다. 요청이 이미 쓰기 가능한 작업으로 충분히 구체적이면 `harness.intake`가 준비 경로에 필요한 초기 범위를 만들 수 있지만, 첫 제품 쓰기는 여전히 `harness.prepare_write`가 필요합니다. 활성 목표, 범위 경계, 범위 밖 항목, 수락 기준, 자율성 경계, baseline, 활성 Change Unit의 이후 변경은 `harness.update_scope`가 담당합니다. 메서드 이름은 지속 저장되는 생명주기 값이 아닙니다. 새로 만들거나 재개한 Task는 [API Schema Core](schema-core.md#current-mvp-value-sets)의 활성 `Task.lifecycle_phase` 값 집합을 사용해야 합니다. `dry_run`과 커밋 전 실패는 이를 만들지 않고 `state_version`도 올리지 않습니다.
...
이 상태 효과가 있는 메서드에서는 요청이 그 외에는 유효하고 미리보기 가능할 때 `dry_run=true`가 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 호환되는 Write Authorization이 오래된 상태가 될 수 있음을 `PlannedEffect` 항목으로 설명할 수 있습니다. 하지만 실제 Write Authorization을 `stale`로 표시하지 않고 실제 `stale_write_authorization_refs`를 반환하지 않습니다.

- **상태 효과:** 커밋된 `dry_run=false` 호출은 활성 Task 구체화 필드, 활성 `change_units` 행 생성 또는 교체, `tasks.active_change_unit_id`, 관련 `scope_decision` 사용자 판단 참조 연결, 차단 사유, 이벤트, 커밋된 재실행 행을 업데이트하고 `project_state.state_version`을 정확히 한 번 올릴 수 있습니다. 이 갱신은 현재 목표 요약, 활성 범위 요약, 허용 경로 또는 영향 영역, 범위 밖 항목, 수락 기준, 자율성 경계, 필요한 사용자 소유 판단, 막히는 질문이 있다면 그 질문, 다음 안전한 행동, 증거 기대 또는 증거 공백, 닫기 차단 사유가 담당 상태에 표현됐을 때 모호한 요청을 쓰기 가능한 첫 Change Unit으로 바꾸는 활성 경로입니다. 프로젝트 전체 상태 버전이나 갱신된 Task, Change Unit, baseline, 범위 경계, 범위 밖 항목, 수락 기준, 자율성 경계가 활성 Write Authorization과 더 이상 맞지 않으면 Core는 해당 Write Authorization을 `status=stale`로 표시합니다. 소비하거나, 취소하거나, 만료시키거나, 조용히 재사용하지 않습니다. `dry_run`과 커밋 전 실패는 현재 기록, 범위 변경, `stale` 처리된 Write Authorization, 이벤트, 아티팩트, 증거 요약, 재실행 행, 상태 버전 증가를 만들지 않습니다.
...
- **오류:** `VALIDATION_FAILED`, `STATE_VERSION_CONFLICT`, `NO_ACTIVE_TASK`, `NO_ACTIVE_CHANGE_UNIT`, `SCOPE_REQUIRED`, `SCOPE_VIOLATION`, `DECISION_REQUIRED`, `DECISION_UNRESOLVED`, `AUTONOMY_BOUNDARY_EXCEEDED`, `CAPABILITY_INSUFFICIENT`, `MCP_UNAVAILABLE`, `LOCAL_ACCESS_MISMATCH`, `BASELINE_STALE`, `VALIDATOR_FAILED`.
...
- **상태 효과:** 없습니다. `include.close=true`일 때 `harness.status`는 현재 close 가능성 평가를 읽기 전용으로 수행할 수 있습니다. `StatusResult.close_blockers`는 `CloseReadinessBlocker[]`를 담습니다. 이 항목은 읽기 전용 관찰이며 저장된 `close_task` 결과가 아닙니다. `harness.status`는 `task_event`, `task_events` 추가, 재실행 행, `tool_invocations.response_json`, `close_state` 변경, Write Authorization 변경, 스테이징된 핸들 소비, 아티팩트 효과, 증거 업데이트, `project_state.state_version` 증가를 만들지 않습니다. `dry_run=true`이면 공통 읽기 전용 분기 규칙에 따라 같은 읽기 전용 `StatusResult` 형태를 반환하고 `base.dry_run=true`, `base.effect_kind=read_only`를 사용합니다.
...
- **prepare_write 판단 사유:** `PrepareWriteResult.write_decision_reasons`는 prepare_write 판단 사유를 담습니다. `decision=allowed`이면 `[]`입니다. `decision=blocked`, `decision=approval_required`, `decision=decision_required`이면 비어 있으면 안 됩니다. 이 사유는 `WriteDecisionReason` 값이며, close 가능성 평가의 닫기 차단 사유도 아니고 close 가능성 평가 결과도 아닙니다.
- **닫기 경계:** `harness.prepare_write`는 close 가능성 평가의 닫기 차단 사유를 만들지 않고, `close_state`를 바꾸지 않고, close 가능성 평가를 실행하지 않고, `close_task` 재실행 행을 만들지 않습니다.
...
- **`dry_run` 분기:** 이 상태 효과가 있는 메서드에서는 요청이 그 외에는 유효하고 미리보기 가능할 때 `dry_run=true`가 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 `dry_run=false` 경로가 Write Authorization을 만들지, 기존 권한을 사용할지, 만들지 않을지를 미리 보여 줄 수 있습니다. prepare_write dry-run 예상 차단 사유는 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로만 표현하며 `source_kind=write_decision`을 사용합니다. 이 미리보기는 실제 `WriteDecisionReason`을 저장하거나 반환하지 않습니다. 소비 가능한 Write Authorization을 만들지 않고 실제 `write_authorization_ref`를 반환하면 안 됩니다.
...
- **ArtifactInput 검증 순서:** `record_run`은 커밋 응답을 만들기 전에 아래 순서를 결정적으로 적용합니다.
  ...
  9. `ArtifactInput.source_kind=existing_artifact`인 각 입력은 `existing_artifact_ref`가 같은 프로젝트와 허용된 Task 범위의 지속 아티팩트인지 검증합니다.
  10. `record_run` 중에는 아티팩트 본문을 읽지 않습니다. 아티팩트 본문 읽기는 `VerifiedSurfaceContext.access_class=artifact_read`가 필요한 별도 읽기 API가 처리합니다.
...
- **상태 효과:** 호환되는 커밋 호출은 `runs`를 만들고, 유효한 `artifact_staging` 핸들을 소비하고, 그 스테이징된 핸들을 지속 `artifacts`로 승격하고, `artifact_links`를 추가하고, `evidence_summaries`를 만들고, Run 관련 차단 사유를 업데이트하고, `write_authorizations.status=active`를 소비하고, 이벤트와 커밋된 재실행 행을 만들며, `project_state.state_version`을 정확히 한 번 올릴 수 있습니다. 제품 쓰기 Run은 현재 `project_state.state_version`이 Write Authorization의 프로젝트 전체 `basis_state_version`과 맞고, 관찰된 변경 경로가 저장된 승인 시도와 호환될 때만 활성 Write Authorization을 소비합니다. 권한의 근거 버전이 현재 `project_state.state_version`과 더 이상 맞지 않으면 그 시도는 소비 전에 `STATE_VERSION_CONFLICT`를 반환합니다. 거부된 호출, `dry_run` 호출, 커밋 전 실패는 Run 생성, 아티팩트 승격 또는 연결, 스테이징된 핸들 소비 또는 갱신, 증거 업데이트, Write Authorization 생성 또는 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하면 안 됩니다.
- **`dry_run` 분기:** 이 상태 효과가 있는 메서드에서는 요청이 그 외에는 유효하고 미리보기 가능할 때 `dry_run=true`가 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 Run, 아티팩트, 증거, 차단 사유, Write Authorization 예상 효과를 미리 보여 줄 수 있습니다. 하지만 `run_summary`를 만들거나 반환하지 않고, 스테이징된 아티팩트를 소비하거나 승격하지 않으며, 아티팩트 연결, 증거 업데이트, Write Authorization 생성 또는 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하지 않습니다.
- **커밋 전 거절:** 유효하지 않은 스테이징된 핸들, Write Authorization 없음 또는 유효하지 않음, 오래된 Write Authorization 근거, 오래된 `expected_state_version`은 `ToolRejectedResponse`를 반환합니다. 거절 응답은 `run_summary` 생성, 아티팩트 승격, 스테이징된 핸들 소비, 증거 업데이트, Write Authorization 생성 또는 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하면 안 됩니다.
...
- **상태 효과:** 커밋된 `dry_run=false` `RequestUserJudgmentResult`는 대기 중인 `user_judgments` 행 하나를 만들고, 영향을 받는 차단 사유를 연결하거나 업데이트할 수 있으며, 이벤트와 재실행 행을 만들고, `project_state.state_version`을 정확히 한 번 올립니다. 실제 대기 중인 판단은 이 결과 분기가 커밋된 뒤에만 생깁니다. 다른 메서드가 반환한 후보는 이 메서드가 커밋되기 전까지 대기 중인 판단이 아닙니다. `dry_run`과 커밋 전 실패는 대기 중인 판단, 차단 사유 갱신, 이벤트, 재실행 행, 상태 버전 증가를 만들지 않습니다.
- **`dry_run` 분기:** 이 상태 효과가 있는 메서드에서는 요청이 그 외에는 유효하고 미리보기 가능할 때 `dry_run=true`가 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 대기 중인 사용자 판단의 예상 효과를 미리 보여 줄 수 있지만 대기 중인 판단을 만들지 않고 실제 `user_judgment_ref`를 반환하면 안 됩니다.
- **오류:** `VALIDATION_FAILED`, `STATE_VERSION_CONFLICT`, `NO_ACTIVE_TASK`, `DECISION_REQUIRED`, `DECISION_UNRESOLVED`, `MCP_UNAVAILABLE`, `LOCAL_ACCESS_MISMATCH`, `CAPABILITY_INSUFFICIENT`, `VALIDATOR_FAILED`.
...
- **상태 효과:** 커밋된 `dry_run=false` 호출은 `user_judgments.status`를 업데이트하고, 요청 수준의 `selected_option_id`, 요청 메모, 답변 세부정보를 기록하고, 관련 차단 사유와 판단에 의존하는 요약만 업데이트하며, 이벤트와 재실행 행을 만들고, `project_state.state_version`을 정확히 한 번 올립니다. 활성 Task 범위 필드나 활성 Change Unit은 직접 바꾸지 않습니다. 해결된 `scope_decision` 때문에 범위를 바꿔야 하면 응답의 다음 행동은 `harness.update_scope`를 가리킵니다. 활성 MVP에서는 독립적인 위험 수락 행을 만들지 않습니다. `dry_run`과 커밋 전 실패는 판단 해결, 차단 사유 갱신, 이벤트, 재실행 행, 상태 버전 증가를 만들지 않습니다.
- **`dry_run` 분기:** 이 상태 효과가 있는 메서드에서는 요청이 그 외에는 유효하고 미리보기 가능할 때 `dry_run=true`가 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 판단 해결과 그에 따른 차단 사유 또는 다음 행동 예상 효과를 보여 줄 수 있습니다. 하지만 판단을 해결하거나 판단 상태를 바꾸지 않고, 차단 사유, 이벤트, 재실행 행, `state_version`을 업데이트하지 않습니다.
- **오류:** `VALIDATION_FAILED`, `STATE_VERSION_CONFLICT`, `NO_ACTIVE_TASK`, `DECISION_UNRESOLVED`, `APPROVAL_DENIED`, `APPROVAL_EXPIRED`, `ACCEPTANCE_REQUIRED`, `RESIDUAL_RISK_NOT_VISIBLE`, `MCP_UNAVAILABLE`, `LOCAL_ACCESS_MISMATCH`, `VALIDATOR_FAILED`.
- **저장소 담당 문서:** `user_judgments`, `blockers`, `task_events`, `tool_invocations`.
...
Core는 close 가능성 평가나 종료 닫기 커밋 전에 `close_task` 사전 확인을 먼저 수행합니다. 이 단계에서는 요청 검증, 로컬 접근, 선택한 intent에서 요구되거나 제공된 `expected_state_version`의 최신성, 재실행 행을 만들 수 있는 intent의 `idempotency_key` 재실행 조회와 `request_hash` 호환성, Project/Task 식별 정보, 닫기와 관련된 `WriteAuthorization.basis_state_version`이 현재 `project_state.state_version`과 맞는지 확인합니다. 사전 확인 실패는 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환하며 `CloseTaskResult`가 아닙니다. 이 커밋 전 실패는 `CloseTaskResult.blockers`를 반환하지 않고, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 변경 없음, 스테이징된 핸들 소비 없음, 아티팩트 효과 없음, 증거 업데이트 없음, `project_state.state_version` 증가 없음입니다. `STATE_VERSION_CONFLICT`는 `harness.close_task`에서 이 사전 확인의 `ToolRejectedResponse.errors` 오류로만 사용할 수 있으며 절대 `CloseReadinessBlocker.code`가 될 수 없습니다.

`CloseTaskResponse`의 응답 분기는 아래처럼 사용합니다.

- `ToolRejectedResponse`: close 가능성 평가 전 실패.
- `CloseTaskResult(close_state=blocked)`: 유효한 close 가능성 평가에서 남은 닫기 차단 사유를 `CloseTaskResult.blockers: CloseReadinessBlocker[]`로 반환하는 결과. 커밋된 닫기 차단 효과는 이 응답 branch와 메서드 상태 효과 표가 허용할 때만 생깁니다.
...
`intent=complete`에서는 `close_task` 사전 확인이 성공한 뒤 API 응답이 Core의 순서 있는 close 가능성 평가를 `CloseTaskResult.blockers: CloseReadinessBlocker[]`로 반영합니다. 처음 실패한 행이 기본 닫기 차단 근거가 됩니다. 뒤 행의 차단 사유도 보조 차단 사유로 함께 보일 수 있지만, 뒤 행이 앞 행을 만족시키거나 숨길 수는 없습니다.

| 순서 | `CloseReadinessBlocker.category` | 일반적인 공개 오류 경로 |
|---:|---|---|
| 1 | `task` | 사전 확인이 같은 프로젝트의 Task를 확정한 뒤, 그 Task의 생명주기나 현재 상태가 complete 전이에 들어갈 수 없으면 `VALIDATION_FAILED`를 사용합니다. Project/Task 식별 정보가 없거나, 읽을 수 없거나, 다른 프로젝트에 속하면 커밋 전 실패이며 닫기 차단 사유가 아닙니다. |
| 2 | `open_run` | 열린 Run, interrupted/violation Run, 호환되지 않거나 복구되지 않은 Run이 닫기 근거를 막고 더 구체적인 타입 코드가 없으면 `VALIDATOR_FAILED`를 사용합니다. |
...
| 6 | `write_compatibility` | 사전 확인이 현재 프로젝트 최신성과 `WriteAuthorization.basis_state_version` 최신성을 확정한 뒤에만 적용됩니다. 필요한 Write Authorization 또는 Run 조건이 없거나, 사용할 수 없거나, 유효하지 않거나, 이미 소비되었거나, 범위·경로·baseline·관찰된 쓰기와 호환되지 않으면 `WRITE_AUTHORIZATION_REQUIRED`, `WRITE_AUTHORIZATION_INVALID`, `SCOPE_VIOLATION`, `BASELINE_STALE`, `VALIDATOR_FAILED`를 사용합니다. 오래된 `expected_state_version`이나 오래된 `WriteAuthorization.basis_state_version`을 다루지 않습니다. |
| 7 | `baseline` 또는 `surface_capability` | 로컬 접근 사전 확인이 성공한 뒤에도 baseline이나 확인된 로컬 접점 역량이 닫기 주장이나 필요한 보장 표시를 정직하게 뒷받침할 수 없으면 `BASELINE_STALE`, `CAPABILITY_INSUFFICIENT`, `VALIDATOR_FAILED`를 사용합니다. 현재 로컬 접근을 사용할 수 없거나 불일치하면 커밋 전 실패이며 닫기 차단 사유가 아닙니다. |
...
| 10 | `final_acceptance` | 필요한 `final_acceptance`가 없거나, 거절되었거나, 오래됐거나, 보이는 닫기 근거와 연결되지 않았으면 `ACCEPTANCE_REQUIRED`를 사용합니다. |
| 11 | `residual_risk_visibility` | 닫기에 영향을 주는 알려진 잔여 위험이 사용자가 판단할 만큼 보이지 않으면 `RESIDUAL_RISK_NOT_VISIBLE`를 사용합니다. |
| 12 | `residual_risk_acceptance` | 보이는 닫기 관련 잔여 위험에 아직 호환되는 `residual_risk_acceptance`가 필요하면 `DECISION_REQUIRED` 또는 `DECISION_UNRESOLVED`를 사용합니다. |
| 13 | `recovery` | 현재 상태를 읽을 수 있고 사전 확인 단계의 최신성 또는 멱등성 충돌이 없을 때, 재실행, 복구, 손상, 해결되지 않은 차단 상태, 그 밖의 수리 제약을 닫기 전에 처리해야 하면 `VALIDATOR_FAILED`를 사용합니다. `idempotency_key`의 `request_hash` 충돌을 다루지 않습니다. |
| 14 | 없음 | 차단 사유가 없으면 complete 전이를 커밋합니다. 하나라도 남아 있으면 `close_state=blocked`를 반환하고 Task를 열린 상태로 둡니다. |

`STATE_VERSION_CONFLICT`는 complete close 가능성 평가 밖에 있습니다. `write_compatibility`, `recovery`, 어떤 `CloseReadinessBlocker.code`에서도 공개 오류 경로로 쓰지 않습니다. 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`, `idempotency_key`의 `request_hash` 충돌은 모두 `ToolRejectedResponse` 사전 확인 경우일 뿐입니다.
...
- **상태 효과:** `intent=check`는 `dry_run=true`가 있어도 항상 읽기 전용입니다. `task_event` 없음, `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `project_state.state_version` 증가 없음, `close_state` 변경 없음, 아티팩트 효과 없음, 스테이징된 핸들 소비 없음, Write Authorization 변경 없음입니다. 커밋된 `dry_run=false` 최종 닫기는 `tasks.lifecycle_phase`, `tasks.close_reason`, `tasks.result`, `tasks.closed_at`, 영향을 받는 `change_units`, 차단 사유, 필요한 경우 프로젝트 활성 Task 상태, 이벤트, 재실행, `project_state.state_version`을 정확히 한 번 업데이트합니다. 커밋된 닫기 차단 결과인 `CloseTaskResult`는 메서드 계약이 허용한 차단 사유 상태, 이벤트, 재실행 행, `project_state.state_version` 효과만 만들 수 있고 Task는 열린 상태로 둬야 합니다. `CloseReadinessBlocker`가 있다는 사실만으로 저장 효과를 뜻하지 않는다. `close_task` 사전 확인 거절은 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환하며 `CloseTaskResult.blockers`를 반환하지 않고, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 변경 없음, 스테이징된 핸들 소비 없음, 아티팩트 효과 없음, 증거 업데이트 없음, `project_state.state_version` 증가 없음입니다.
- **`dry_run` 분기와 응답 선택:** `harness.close_task`는 혼합 intent 메서드이므로 메서드 이름이 아니라 선택된 intent의 상태 효과로 응답 분기를 고릅니다. `intent=check`와 `dry_run=true`는 `base.dry_run=true`, `base.effect_kind=read_only`인 `CloseTaskResult`를 반환하며 `ToolDryRunResponse`를 반환하면 안 됩니다. `intent=complete`, `intent=cancel`, `intent=supersede`와 `dry_run=true`는 요청이 그 외에는 유효하며 미리보기 가능하면 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 종료 또는 차단된 닫기 예상 효과, close 가능성 평가 발견 사항, 다음 행동을 미리 보여 줄 수 있습니다. `intent=complete`, `intent=cancel`, `intent=supersede`에 대한 close_task dry-run 미리보기의 예상 닫기 차단 사유는 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로만 표현하며 `source_kind=close_readiness`를 사용하고, `CloseReadinessBlocker`로 표현하지 않습니다. 이 미리보기는 닫기 상태, Task 생명주기, 차단 사유, 이벤트, 재실행 행, 아티팩트 생성·갱신·연결, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `state_version`을 바꾸지 않습니다.
- **오류:** `STATE_VERSION_CONFLICT`가 이 메서드 목록에 남아 있는 것은 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`, `idempotency_key`의 `request_hash` 충돌 같은 `ToolRejectedResponse` 사전 확인 오류로만 허용된다는 뜻입니다. 커밋된 닫기 차단 결과의 오류가 아닙니다. 그 밖의 메서드 오류에는 `VALIDATION_FAILED`, `NO_ACTIVE_TASK`, `DECISION_REQUIRED`, `DECISION_UNRESOLVED`, `SCOPE_REQUIRED`, `SCOPE_VIOLATION`, `APPROVAL_REQUIRED`, `APPROVAL_DENIED`, `APPROVAL_EXPIRED`, `EVIDENCE_INSUFFICIENT`, `ARTIFACT_MISSING`, `ACCEPTANCE_REQUIRED`, `RESIDUAL_RISK_NOT_VISIBLE`, `CAPABILITY_INSUFFICIENT`, `MCP_UNAVAILABLE`, `LOCAL_ACCESS_MISMATCH`, `VALIDATOR_FAILED`가 있습니다.
