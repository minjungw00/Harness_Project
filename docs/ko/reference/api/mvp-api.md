...
메서드 담당 문서의 예시는 오래 유지되는 계정 데이터 내보내기 확인 작업을 사용합니다. 이 작업은 계정 데이터 내보내기 전에 명시적 확인 단계를 추가하고, 계정 데이터 내보내기 확인 테스트를 갱신하며, 계정 데이터 내보내기 확인 테스트 출력을 대표 실행 및 증거 데이터로 기록합니다.
