...
| 메서드 | 활성 역할 | 담당 문서 |
|---|---|---|
| `harness.intake` | 일반 사용자 작업을 시작, 재개, 분류합니다. | [접수 메서드](method-intake.md) |
| `harness.update_scope` | `harness.intake` 이후 활성 Task 범위와 활성 Change Unit을 갱신합니다. | [범위 갱신 메서드](method-update-scope.md) |
| `harness.status` | 현재 상태와 다음 안전한 행동을 반환합니다. | [상태 메서드](method-status.md) |
| `harness.prepare_write` | 제품 파일 쓰기 호환성을 Write Authorization 전에 확인합니다. | [쓰기 준비 메서드](method-prepare-write.md) |
| `harness.stage_artifact` | 안전한 바이트나 안전한 알림을 스테이징합니다. | [아티팩트 스테이징 메서드](method-stage-artifact.md) |
| `harness.record_run` | 작업, 증거, 아티팩트 참조를 기록합니다. | [실행 기록 메서드](method-record-run.md) |
| `harness.request_user_judgment` | 대기 중인 사용자 소유 판단 요청 하나를 만듭니다. | [사용자 판단 메서드](method-user-judgment.md#harnessrequest_user_judgment) |
| `harness.record_user_judgment` | 대기 중인 판단에 대한 사용자의 답을 기록합니다. | [사용자 판단 메서드](method-user-judgment.md#harnessrecord_user_judgment) |
| `harness.close_task` | 닫기 준비 상태를 확인하거나 허용될 때 닫습니다. | [Task 닫기 메서드](method-close-task.md) |

## 메서드 담당 문서 경로

메서드 동작을 확인할 때는 아래 담당 문서를 먼저 엽니다. 메서드 페이로드 필드 질문은 영향을 받는 메서드 담당 문서에서 시작합니다. 공통 응답 분기 스키마, 공통 요청 래퍼 필드, 중첩 스키마 필드, 저장 효과, 공개 오류 코드는 아래 담당 문서 링크를 따릅니다.
...
| 메서드 페이로드 필드 | 위의 영향을 받는 메서드 담당 문서 |
| 공통 요청 래퍼 필드와 중첩 스키마 필드 | [스키마 담당 문서 링크](#스키마-담당-문서-링크) |
...
| 스키마 영역 | 담당 문서 |
|---|---|
| 공통 요청 래퍼, 응답 분기, 오류, 이벤트 | [API 코어 스키마](schema-core.md) |
...
- 범위: 계정 내보내기 확인 흐름과 계정 내보내기 확인 테스트.
- 범위 밖: 계정 삭제 동작.
- 수락 기준: 계정 데이터 내보내기 파일을 다운로드하기 전에 명시적 확인 단계가 필요하다.
- 확장: 메서드 예시는 계정 내보내기 확인 테스트의 대표 실행과 증거 데이터를 더할 수 있습니다.
...
API 예시 정합성 질문과 필드 이름 일관성 질문은 [작성 가이드](../../maintain/authoring-guide.md)와 [점검](../../maintain/checks.md)으로 보냅니다. 구체적인 예시 필드 정의는 그다음 영향을 받는 메서드, 스키마, 또는 저장소 담당 문서로 보냅니다.
