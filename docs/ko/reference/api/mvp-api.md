...
- 각 메서드의 전체 method 동작 세부사항. 여기에는 메서드별 필수 입력, 접근 요구사항, 결과 필드, `dry_run` 동작, 대표 요청과 응답 본문이 포함됩니다.
...
method 동작을 확인할 때는 아래 담당 문서를 먼저 엽니다. 공통 응답 branch, 스키마 필드, 저장 효과, 공개 오류 코드는 아래 담당 문서 링크를 따릅니다.
...
메서드 담당 문서의 예시는 오래 유지되는 계정 내보내기 확인 시나리오를 사용합니다. `Task` 요약은 계정 내보내기 확인이고, 범위는 계정 내보내기 확인 UI와 계정 내보내기 확인 테스트를 포함하며, 계정 삭제 동작과 청구 내보내기 동작은 제외합니다. 수락 기준은 다운로드 전에 명시적 확인 단계가 필요하다는 것입니다. 다른 메서드 예시는 같은 시나리오에 계정 내보내기 확인 테스트와 대표 실행 및 증거 데이터를 더할 수 있습니다.
