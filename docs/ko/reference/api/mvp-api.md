...
- **상태 효과:** 호환되는 커밋 호출은 `runs`를 만들고, 유효한 `artifact_staging` handle을 소비하고, 그 스테이징된 핸들을 지속 `artifacts`로 승격하고, `artifact_links`를 추가하고, `evidence_summaries`를 만들고, Run 관련 차단 사유를 업데이트하고, `write_authorizations.status=active`를 소비하고, 이벤트와 커밋된 재실행 행을 만들며, `project_state.state_version`을 정확히 한 번 올릴 수 있습니다. 제품 쓰기 Run은 현재 `project_state.state_version`이 Write Authorization의 프로젝트 전체 `basis_state_version`과 맞고, 관찰된 변경 경로가 저장된 승인 시도와 호환될 때만 활성 Write Authorization을 소비합니다. 권한의 근거 버전이 현재 `project_state.state_version`과 더 이상 맞지 않으면 그 시도는 소비 전에 `STATE_VERSION_CONFLICT`를 반환합니다. 거부된 호출, dry run, 커밋 전 실패는 Run 생성, 아티팩트 승격 또는 연결, 스테이징된 핸들 소비 또는 갱신, 증거 업데이트, Write Authorization 생성 또는 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하면 안 됩니다.
- **`dry_run` 분기:** `dry_run=true`는 `ToolDryRunResponse`를 반환합니다. `DryRunSummary`는 Run, 아티팩트, 증거, 차단 사유, Write Authorization 예상 효과를 미리 보여 줄 수 있습니다. 하지만 `run_summary`를 만들거나 반환하지 않고, 스테이징된 아티팩트를 소비하거나 승격하지 않으며, 아티팩트 연결, 증거 업데이트, Write Authorization 생성 또는 소비, 이벤트 추가, 재실행 행 생성, `state_version` 증가를 하지 않습니다.
