...
메서드 담당 문서의 예시는 오래 유지되는 계정 내보내기 확인 시나리오를 사용합니다. `Task` 요약은 계정 내보내기 확인이고, 범위는 계정 내보내기 확인 UI와 계정 내보내기 테스트를 포함하며, 계정 삭제 동작과 청구 내보내기 동작은 제외합니다. 수락 기준은 다운로드 전에 명시적 확인 단계가 필요하다는 것입니다. 다른 메서드 예시는 같은 시나리오에 계정 내보내기 테스트와 대표 실행 및 증거 데이터를 더할 수 있습니다.
