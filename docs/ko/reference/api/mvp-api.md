...
현재 MVP의 활성 API 표면을 확인할 때 이 참조를 사용합니다. 이 문서는 [API 값 집합](schema-value-sets.md)이 담당하는 활성 메서드 이름 값 집합에 대해 메서드별 요청, 응답, 상태 효과, 저장소 담당 문서, 오류, 보안 경계를 담당합니다.
...
정확한 활성 메서드 이름 값 집합은 [API 값 집합](schema-value-sets.md)이 담당합니다. 이 문서는 그 현재 메서드들의 동작을 담당합니다.
...
- **상태 효과:** 없습니다. `include.close=true`일 때 `harness.status`는 현재 닫기 준비 상태 평가를 읽기 전용으로 수행할 수 있습니다. `StatusResult.close_blockers`는 `CloseReadinessBlocker[]`를 담습니다. 이 항목은 읽기 전용 관찰이며 저장된 `close_task` 결과가 아닙니다. `harness.status`는 `task_event`, `task_events` 추가, 재실행 행, `tool_invocations.response_json`, `close_state` 변경, Write Authorization 변경, 스테이징된 핸들 소비, 아티팩트 효과, 증거 업데이트, `project_state.state_version` 증가를 만들지 않습니다. `dry_run=true`이면 공통 읽기 전용 분기 규칙에 따라 같은 읽기 전용 `StatusResult` 형태를 반환하고 `base.dry_run=true`, `base.effect_kind=read_only`를 사용합니다.
...
- **prepare_write 판단 사유:** `PrepareWriteResult.write_decision_reasons`는 prepare_write 판단 사유를 담습니다. `decision=allowed`이면 `[]`입니다. `decision=blocked`, `decision=approval_required`, `decision=decision_required`이면 비어 있으면 안 됩니다. 이 사유는 `WriteDecisionReason` 값이며, 닫기 준비 상태 평가의 닫기 차단 사유도 아니고 닫기 준비 상태 평가 결과도 아닙니다.
- **닫기 경계:** `harness.prepare_write`는 닫기 준비 상태 평가의 닫기 차단 사유를 만들지 않고, `close_state`를 바꾸지 않고, 닫기 준비 상태 평가를 실행하지 않고, `close_task` 재실행 행을 만들지 않습니다.
...
Core는 닫기 준비 상태 평가나 종료 닫기 커밋 전에 `close_task` 사전 확인을 먼저 수행합니다. 이 단계에서는 요청 검증, 로컬 접근, 선택한 intent에서 요구되거나 제공된 `expected_state_version`의 최신성, 재실행 행을 만들 수 있는 intent의 `idempotency_key` 재실행 조회와 `request_hash` 호환성, Project/Task 식별 정보, 닫기와 관련된 `WriteAuthorization.basis_state_version`이 현재 `project_state.state_version`과 맞는지 확인합니다. 사전 확인 실패는 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환하며 `CloseTaskResult`가 아닙니다. 이 커밋 전 실패는 `CloseTaskResult.blockers`를 반환하지 않고, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 변경 없음, 스테이징된 핸들 소비 없음, 아티팩트 효과 없음, 증거 업데이트 없음, `project_state.state_version` 증가 없음입니다. `STATE_VERSION_CONFLICT`는 `harness.close_task`에서 이 사전 확인의 `ToolRejectedResponse.errors` 오류로만 사용할 수 있으며 절대 `CloseReadinessBlocker.code`가 될 수 없습니다.
...
`intent=complete`에서는 `close_task` 사전 확인이 성공한 뒤 API 응답이 Core의 순서 있는 닫기 준비 상태 평가를 `CloseTaskResult.blockers: CloseReadinessBlocker[]`로 반영합니다. 처음 실패한 행이 기본 닫기 차단 근거가 됩니다. 뒤 행의 차단 사유도 보조 차단 사유로 함께 보일 수 있지만, 뒤 행이 앞 행을 만족시키거나 숨길 수는 없습니다.
...
`STATE_VERSION_CONFLICT`는 complete 닫기 준비 상태 평가 밖에 있습니다. `write_compatibility`, `recovery`, 어떤 `CloseReadinessBlocker.code`에서도 공개 오류 경로로 쓰지 않습니다. 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`, `idempotency_key`의 `request_hash` 충돌은 모두 `ToolRejectedResponse` 사전 확인 경우일 뿐입니다.
