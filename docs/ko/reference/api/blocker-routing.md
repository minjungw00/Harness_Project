...
이 문서는 닫기 차단 사유와 API 응답 분기 사이의 경계를 담당합니다. 독자는 이 문서에서 차단 사유 처리 경로를 확인합니다. 메서드 동작이나 스키마 형태는 여기서 정의하지 않습니다.
...
공통 경계 규칙을 먼저 적용한 뒤에만 이 표를 사용합니다. 공개 오류 코드 묶음은 담당 문서가 정의한 닫기 차단 사유 데이터가 있을 때만 닫기 차단 사유와 관련됩니다. 스키마나 메서드 담당 문서가 공개 `ErrorCode` 값의 사용을 명시적으로 허용하지 않는 한 그 값을 `CloseReadinessBlocker.code`에 복사하지 않습니다.
