...
이 문서는 기준 범위의 `harness.request_user_judgment`와 `harness.record_user_judgment` 메서드 동작을 담당합니다.
...
이 메서드는 사용자의 답에 따라 특정 대기 판단을 `resolved`, `rejected`, `deferred`, `blocked` 중 하나로 표시합니다. 답변을 관련 없는 승인, 범위 확장, 수락, 잔여 위험 수락, 쓰기 승인으로 넓히지 않습니다.
...
결과는 포함된 차단 사유와 판단에 의존하는 요약만 갱신합니다.
