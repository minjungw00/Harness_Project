...
무엇을 렌더링하거나 가져올지 판단할 때는 다음 모델로 나눕니다.

- 사용자 상태 카드: 사용자가 읽는 짧은 현재 상태 보기입니다.
- 에이전트 맥락 패킷: 다음 안전한 행동에 필요한 최소 상태, 참조, 최신성, 소유자 섹션 포인터입니다.
- MVP-1 지원 보기: 현재 단계가 필요로 할 때만 쓰는 판단 요청, 실행/근거 요약, 닫기 결과입니다.
- Core 상태: 로컬 권한 기록이며 유일한 운영 기준입니다.

## Projection을 쉽게 말하면

하네스 projection은 이미 기준 상태나 evidence/artifact storage에 기록된 작업을 사람이 읽기 쉽게 보여주는 읽기용 요약입니다. MVP-1은 current Core record와 ref에서 다섯 가지 작은 보기를 직접 파생할 수 있습니다. 상태 카드(`status-card`), 에이전트 맥락 패킷(`agent-context-packet`), 판단 요청(`judgment-request`), 실행/근거 요약(`run-evidence-summary`), 닫기 결과(`close-result`)입니다. Later profile은 `state.sqlite` record, `state.sqlite.task_events`, 등록된 evidence/artifact 참조를 읽는 projector를 추가할 수 있고, 그때 [Template later-profile](templates/later-profile/README.md)에 있는 `TASK`, `APR`, `RUN-SUMMARY`, `EVIDENCE-MANIFEST`, `EVAL`, `DIRECT-RESULT` 같은 Markdown 보고서를 생성합니다.
...
| MVP-1 사용자 작업 루프 보기 | MVP-1 사용자 작업 루프 | 활성 MVP-1 템플릿/보기 세트는 정확히 [상태 카드](templates/status-card.md), [에이전트 맥락 패킷](templates/agent-context-packet.md), [판단 요청](templates/judgment-request.md), [실행/근거 요약](templates/run-evidence-summary.md), [닫기 결과](templates/close-result.md)입니다. 이 작은 보기들은 current Task state, 범위/하지 않을 일, 대기 중인 판단, 활성 막힘, 다음 안전한 행동, 근거 공백, 닫기 막힘, 보이는 잔여 위험, 보장 수준, 출처/최신성 참조를 보여주되 전체 report body를 요구하지 않습니다. |
| Agent context packet/reference payload | MVP-1 지원 접점과 이후 단계 | 같은 Core 상태와 ref에서 파생한 에이전트용 간결한 payload입니다. 다음 행동에 필요한 id, ref, blocker label, owner-section pointer, freshness를 담을 수 있지만 사용자 대상 권한이 아니며 기본으로 full projection body를 넣지 않습니다. |
...
MVP-1 사용자 작업 루프의 projection 표면은 의도적으로 작습니다. 정확히 다섯 가지 보기를 사용합니다.

| 보기 | 목적 |
|---|---|
| [상태 카드](templates/status-card.md) | 사용자가 보는 짧은 현재 상태. |
| [에이전트 맥락 패킷](templates/agent-context-packet.md) | 다음 안전한 행동을 위한 compact agent context. |
| [판단 요청](templates/judgment-request.md) | 사용자 소유 판단 요청. |
| [실행/근거 요약](templates/run-evidence-summary.md) | 최소 실행과 근거 요약. |
| [닫기 결과](templates/close-result.md) | 닫기 준비 상태, 최종 수락, 잔여 위험, 막힘. |
...
MVP-1의 다섯 보기는 모두 compact해야 합니다. Schema field, DDL, event log, full artifact, full artifact contents, full reference doc, full Evidence Manifest, full report body, 관련 없는 template, future catalog material을 쏟아내면 안 됩니다. 최종 수락 필요 여부/상태와 잔여 위험 표시는 관련 있을 때 distinct Core meaning으로 남지만, 별도 필수 detailed template이 아니라 상태 카드, 판단 요청, 닫기 결과 안에 나타납니다.

Projection audience는 분리합니다.

| Audience | Shape | Rule |
|---|---|---|
| 사용자 상태 | 평범한 말과 ref를 담은 상태 카드, 판단 요청, 실행/근거 요약, 닫기 결과 | MVP-1 사용자 작업 루프 보기입니다. 표시 전용이며 Core 상태와 ref에서 파생됩니다. |
...
닫기 막힘: 근거 공백. 최종 수락은 아직 요청하지 않았습니다.
...
Managed block을 직접 편집한 내용은 수락된 상태가 아니라 drift입니다. 권한처럼 보이는 문구를 직접 편집해도 write를 허가하거나, decision을 해결하거나, 필수 근거 조건을 충족하지 않습니다. 또한 verification 또는 수동 QA를 대체하거나, 잔여 위험을 수락하거나, assurance를 높이거나, 작업을 닫거나, owner 기록을 변경하지 않습니다.
...
3. Registered artifact byte와 안전한 메타데이터 알림은 `ArtifactRef`, owner relation, integrity/redaction metadata가 함께 있을 때만 기준 근거가 됩니다.
...
| Evidence coverage | Minimum MVP-1: `evidence_ref` record, Run refs, ArtifactRefs, 파생된 visible gap summary. Full Evidence Manifest profile: `evidence_manifests` plus artifact refs | MVP-1에서는 실행/근거 요약 또는 상태 카드의 evidence summary/gap display. `EVIDENCE-MANIFEST` projection은 full evidence profile이 active일 때만 사용 | MVP-1에서는 `record_run` evidence update. Full Evidence Manifest profile이 active일 때 evidence module update, then projector |
...
- 모든 등록된 근거 아티팩트 참조는 `artifact_id`, Task 또는 equivalent owner scope, kind, `uri`, `sha256`, `size_bytes`, `content_type`, `redaction_state`, `produced_by`, relation owner, `retention_class`, availability metadata를 가져야 합니다.
...
| MVP-1 실행/근거 표시 | [실행/근거 요약](templates/run-evidence-summary.md) | 최소 Run과 근거 요약입니다. 상세 Run Summary와 Evidence Manifest는 later/full-profile입니다. |
| MVP-1 닫기 표시 | [닫기 결과](templates/close-result.md) | 닫기 준비 상태, 최종 수락, 잔여 위험, 막힘을 보여줍니다. |
