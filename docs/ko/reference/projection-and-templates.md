...
| [닫기 결과](templates/close-result.md) | 닫기 준비 상태, 작업 수락, 잔여 위험, 막힘. |
...
| Write Authorization | `state.sqlite.write_authorizations`와 관련 Task, Change Unit, approval, user judgment, baseline, consumed Run ref | `TASK` Write Authority Summary, Journey Card Write Authority Summary line, `RUN-SUMMARY` relation | `prepare_write.decision=allowed`가 active row를 만들거나 반환함. Blocked/required/conflict decision은 authorization row를 만들지 않습니다. Idempotent replay는 already committed response를 반환합니다. `record_run`이 authorization을 consume한 뒤 projector |
...
- User Judgment: `judgment_type`, `presentation`, `display_label`을 포함한 `state.sqlite.user_judgments`와 관련 ref -> 상태 카드의 판단 줄, 판단 요청 보기, status/next-action responses, judgment-context resources, user-judgment resources; later/standalone full-format projection이 켜진 경우 optional `TASK`, Journey Card, `DEC` projection
...
- Write Authorization: `state.sqlite.write_authorizations`는 specific `decision=allowed` write attempt와 authorization lifecycle status를 기록합니다. Scope, approval, evidence, verification, QA, 작업 수락, 잔여 위험 수용이 아닙니다.
...
| MVP-1 닫기 표시 | [닫기 결과](templates/close-result.md) | 닫기 준비 상태, 작업 수락, 잔여 위험, 막힘을 보여줍니다. |
...
어떤 template class도 projection을 기준 상태로 만들거나 evidence, QA, verification, 민감 동작 승인, 작업 수락, 잔여 위험 수용, 닫기 준비 상태, close authority, Write Authorization을 만들지 않습니다. Source record가 없으면 projector는 template completeness를 맞추기 위해 placeholder state를 만들어내면 안 됩니다.
