...
사용자가 Projection을 편집해도 그 내용은 입력일 뿐입니다. reconcile과 Core 상태 변경 작업 같은 명시적 담당 경로를 거쳐야 상태가 될 수 있습니다. 관리 텍스트, front matter, 표시된 상태, 아티팩트 참조, 닫기 상태, 최종 수락 상태, 잔여 위험 상태, 템플릿 텍스트를 직접 고쳐도 수락된 상태가 되지 않습니다.
...
이 보기들은 접점에 따라 짧은 텍스트, 카드, Markdown 조각, 구조화된 페이로드로 렌더링될 수 있습니다. 영속 Markdown Projection 작업이나 전체 보고서 렌더러가 필요하지 않습니다. 첫 내부 smoke 목표는 렌더링된 카드 대신 구조화된 상태/차단 사유 출력을 반환해도 됩니다.
...
- 관리 블록 안을 직접 편집하면 drift입니다. 수락된 상태가 아닙니다.
- Projection 작업 저장소가 active이면 projector는 저장소 담당 경로를 통해 원천 상태 버전, Projection 버전 또는 상태, 렌더링 시각, 작업 상태, 관리 hash를 기록합니다.
- 현재 MVP의 작은 보기는 영속 Projection 작업 없이 읽는 시점의 출처/최신성 문구를 담을 수 있습니다.
- 관리 hash는 marker line을 제외한 projector 소유 관리 블록 본문에서 계산합니다. 줄 끝은 정규화하고 projector가 의미 있다고 정한 공백 규칙을 따릅니다.
- 관리 hash는 drift를 감지할 뿐입니다. Markdown을 상태로 만들지 않습니다.
- 렌더링 전에 관리 블록 hash가 마지막 Projection hash와 다르면 projector는 drift를 보고하거나 담당 경로로 가는 복구 후보를 만듭니다. 편집된 블록을 조용히 수락된 상태로 받아들이지 않습니다.
...
구현 계층: 현재 MVP 사용자 작업 루프 보기입니다. 첫 내부 smoke 목표는 이 카드 대신 구조화된 상태/차단 사유 출력을 반환해도 됩니다.
...
- 보장 수준 줄은 항상 렌더링합니다. 현재 MVP 기본 동작에서는 실제 한계가 협력형 보류이면 그렇게 적고, 사후 보고라면 그 한계를 메모에 적어야 합니다. Core/MCP가 unavailable이면 최신이 아니거나 추측한 보장 대신 unavailable 조건을 렌더링합니다.
...
- 증거 충분성은 양이 아니라 포괄성입니다. 현재 뒷받침하는 참조가 없거나 중요한 아티팩트 참조에 담당 관계, 무결성 메타데이터, `redaction_state`, 가용성이 없으면 공백과 현재 증거 상태를 보여줘야 합니다. 긴 아티팩트 목록이나 보고서 문장을 증명처럼 취급하면 안 됩니다.
...
구현 계층: 현재 MVP 사용자 작업 루프 보기입니다. 상세 연속성, release-handoff, export 보고서는 later 후보 범위입니다.
...
- 닫기에 영향을 줄 때 설계 품질 담당 경로로 전달된 조치. later 후보가 active가 아니면 현재 MVP 차단 집합으로 제한합니다.
...
- 현재 MVP `close-result` 출력은 현재 MVP 닫기 의미만 보여줍니다. later 후보인 보장 정보와 상세 QA 줄은 later 후보 범위에 남습니다.
...
later 후보 템플릿 본문은 active 문서가 아니며 이 참조 문서에 저장하지 않습니다. later 후보 템플릿 이름은 본문 없이 [Later template 후보](../later/index.md#later-template-candidates)에만 둘 수 있습니다.

later 후보 목록은 현재 MVP 요구사항, 활성 `ProjectionKind`, 스키마 계약, 런타임 동작, 템플릿 본문, 생성된 Projection, 증거, 검증, QA, 최종 수락, 잔여 위험 수락, 닫기 준비 상태, 구현 작업, 수락 증거를 만들지 않습니다.

later 템플릿을 승격하려면 향후 담당 문서가 좁은 범위, 원천 기록, 대체 동작, 대체 불가능 규칙, 최신성 동작, 증명 기대치, 정확한 담당 문서 위치를 정의해야 합니다. 그 전까지 현재 MVP 활성 출력은 이 문서의 다섯 템플릿으로 제한됩니다.
