...
- 사용자가 이해하는 데 필요한 현재 범위, 허용 경로 또는 영향 영역, 범위 밖 항목, 수락 기준, 에이전트가 스스로 판단해도 되는 범위, 이번에 바꿀 가장 작은 작업 단위 요약
...
- 영향을 받는 Task, 이번에 바꿀 가장 작은 작업 단위, 쓰기 범위, 닫기 범위, 민감 동작 범위, 기준 또는 다른 대상
- 선택지 또는 선택된 결과
...
- 닫기 가능 여부, 닫기를 막는 이유, 가장 작은 해결 방법
- 원천 상태 버전, 최신성, 역량 상태
...
닫기를 막는 이유: {reasons_this_cannot_be_closed_yet|none}
닫기를 가능하게 하는 가장 작은 해결 방법: {smallest_unblocker|none}
