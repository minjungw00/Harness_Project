...
- Projection 권한 경계
- 읽는 시점에 파생되는 Projection
- 읽기 전용 렌더링과 비변경 규칙
- 현재 MVP compact Projection/템플릿 렌더링 규칙
- `ArtifactRef` 렌더링 규칙
- 작은 보기의 출처/최신성 표시 규칙
- 현재 MVP 활성 템플릿 세트
...
- SQLite DDL, 저장소 배치, 아티팩트 저장소, 영속 Projection 작업 저장소. [Storage](storage.md)를 봅니다.
...
렌더링된 Projection, Markdown `status-card`, 생성된 문서, managed block, front matter, 표시된 상태, 아티팩트 참조, 닫기 상태, 최종 수락 상태, 잔여 위험 상태, 템플릿 텍스트를 편집해도 Core 상태가 직접 바뀌지 않습니다. 사용자가 상태 변경을 원하면 범위 변경, 사용자 판단, 실행/증거 기록, 닫기 확인처럼 담당 Core/API 경로가 처리하는 일반 자연어 요청이나 활성 API 흐름으로 들어와야 합니다.

현재 MVP에는 활성 reconcile queue, 편집 가능한 Projection 입력 경로, Projection에서 Core로 되돌리는 복구 경로, 영속 Projection 작업, managed block drift repair가 없습니다. 이런 항목은 향후 담당 문서가 범위, 대체 동작, 대체 불가 규칙, 증명 기대치를 갖춰 승격하기 전까지 이후 후보로 남습니다.

## Projection은 파생 표시

Projection은 현재 Core 소유 기록과 등록된 `ArtifactRef` 메타데이터에서 읽는 시점에 만드는 파생 보기입니다. 사람이 작업, 출처 참조, 차단 사유, 증거 공백, 닫기를 막는 이유, 최신성, 다음 안전한 행동을 읽게 돕습니다. 두 번째 상태 저장소가 아닙니다.

현재 MVP Projection 세트는 정확히 다음 다섯 개입니다.

- 사용자용 작은 보기 네 가지: `status-card`, `judgment-request`, `run-evidence-summary`, `close-result`
- 에이전트용 지원 패킷 하나: `agent-context-packet`

사용자용 보기 네 가지와 `agent-context-packet`은 서로 구분합니다. 사용자용 Projection은 사용자의 판단과 확인을 돕는 평범한 말을 씁니다. `agent-context-packet`은 다음 에이전트 행동을 위한 작은 지원 맥락이지, 사용자용 문장도 숨은 권한 경로도 아닙니다.

이 보기들은 접점에 따라 짧은 텍스트, 카드, Markdown 조각, 구조화된 페이로드로 렌더링될 수 있습니다. 읽을 때 현재 원천 기록에서 계산하며, 영속 Markdown Projection 작업, 영속 Projection 캐시, reconcile queue, managed block 복구, 전체 보고서 렌더러가 필요하지 않습니다. 첫 내부 스모크 목표는 렌더링된 카드 대신 구조화된 상태/차단 사유 출력을 반환해도 됩니다.
...
## 편집 가능한 Projection 상태 없음

현재 MVP에는 Core로 되돌아 쓰는 사람이 편집 가능한 Projection 영역이 없습니다. 메모, 수정, 제안은 사용자가 일반 자연어 요청이나 활성 API 흐름으로 보낼 때만 일반 입력입니다. 렌더링된 Markdown `status-card`, 생성된 문서, 템플릿 본문, front matter 필드, 표시된 아티팩트 참조, 표시된 닫기/최종 수락/잔여 위험 줄은 읽기 전용 파생 표시입니다.

향후 managed block, reconcile, Projection 작업, drift repair 설계가 편집 입력 처리를 정의할 수는 있습니다. 승격 전까지 이런 설계는 활성 Core 변경 경로, 저장소 행, queue, 복구 후보, 최신성 복구, 닫기/증거/최종 수락 권한으로 존재하지 않습니다. 렌더링된 보기의 위쪽에는 짧은 경계 안내가 있어야 합니다. 표시 전용이고, Core 상태와 참조에서 파생되었으며, Write Authorization이나 닫기 결과가 아니라는 점을 보여줍니다.
...
임시로 스테이징된 아티팩트 자료와 등록된 `ArtifactRef`는 다릅니다. 사용자용 렌더링은 첨부가 스테이징 중인지, 만료 예정인지, 기록을 기다리는지 말할 수 있습니다. 하지만 호환되는 `harness.record_run`이 스테이징 핸들을 소비하고 등록 아티팩트를 주장에 연결하기 전까지는 지속 증거라고 부르면 안 됩니다. 원시 파일 경로, 복사한 로컬 경로, 원시 로그, "화면 캡처가 이 경로에 있다"는 문장은 현재 MVP에서 아티팩트 권한이 아닙니다.
...
사용자용 출력 네 가지는 평범한 말을 씁니다. 사용자가 판단하거나, 차단 사유를 이해하거나, 증거를 살피거나, 닫기를 이해하는 데 도움이 될 때만 출처 참조와 최신성을 보여줍니다. 현재 단계, 에이전트가 스스로 판단해도 되는 범위, 하네스가 확인할 수 있는 수준, 닫기를 막는 이유처럼 사용자가 바로 읽을 수 있는 라벨을 씁니다. 스키마, DDL, 이벤트 로그, 전체 아티팩트, 전체 보고서 본문, 전체 증거 목록, 향후 목록을 쏟아내지 않습니다.

사용자용 출력은 Discovery, Change Unit, `EvidenceSummary`, `CloseBlocker` 같은 하네스 어휘를 알아야 이해되는 형태가 되면 안 됩니다. 먼저 평범한 결과를 보여줍니다. 무엇이 알려졌는지, 사용자가 무엇을 결정해야 하는지, 이 행동 하나에 대해 무엇이 승인됐는지, 어떤 증거가 있거나 빠졌는지, 어떤 위험이 보이는지, 무엇이 닫기를 막는지를 먼저 말합니다. 정확한 스키마 이름은 참조 모델을 설명하거나 에이전트용 패킷을 렌더링할 때만 씁니다.
...
- 현재 Task 요약, 작업 모양, 현재 단계, 필요할 때 막히는 질문 하나, 다음 안전한 행동
- 사용자가 이해하는 데 필요한 현재 범위, 허용 경로 또는 영향 영역, 범위 밖 항목, 수락 기준, 에이전트가 스스로 판단해도 되는 범위, 첫 안전한 작업 단위 요약
- 사용자에게 읽히는 라벨로 렌더링한 대기 중인 판단
- 진행 또는 닫기가 보류된 평이한 이유와 활성 차단 사유
- 확인 근거 요약, 뒷받침 참조, 가림 처리 또는 가용성 메모, 증거 공백
- 관련 있을 때 닫기를 막는 이유, 최종 수락 필요 여부, 잔여 위험 표시, 잔여 위험 수락 상태
- 보이는 다음 행동을 바꿀 때만 설계 품질 담당 경로로 전달된 조치
- 하네스가 확인할 수 있는 수준 또는 사용 불가/역량 상태
...
현재 단계: {current_stage}; {current_stage_reason|none}
범위: {scope_summary}
허용 경로/영역: {allowed_paths_or_affected_areas|unknown}
범위 밖: {non_goals|none}
수락 기준: {acceptance_criteria|unknown}
에이전트가 스스로 판단해도 되는 범위: {agent_decision_boundary|default}
...
확인 근거 요약: {evidence_status}. {evidence_summary|none}
증거 공백: {evidence_gaps|none}
확인: {check_summary|none}
닫기: {close_readiness_summary}
닫기를 막는 이유: {reasons_this_cannot_be_closed_yet|none}
설계 품질 조치: {design_quality_routed_action|none}
잔여 위험: {residual_risk_visibility|none}
다음 안전한 행동: {next_safe_action}
하네스가 확인할 수 있는 수준: {harness_verification_level_or_unavailable}; {harness_verification_note}
...
- "하네스가 확인할 수 있는 수준" 줄은 항상 렌더링합니다. 현재 MVP 기본 동작에서는 실제 한계가 협력형 보류이면 그렇게 적고, 탐지형 보고는 관찰 가능한 지원 사실과 통과한 역량 확인이 그 한계를 뒷받침할 때만 적습니다. Core/MCP를 사용할 수 없으면 최신이 아니거나 추측한 확인 수준 대신 `unavailable` 조건을 렌더링합니다.
...
- 정확한 질문, 이유, 현재 근거로 이미 좁혀진 선택지, 불확실성, 사용자가 결정하지 않을 때의 결과
- 영향을 받는 Task, 첫 안전한 작업 단위, 쓰기 범위, 닫기 범위, 민감 동작 범위, 기준 또는 다른 대상
- 선택지 또는 선택된 결과
...
현재 범위에 맞는 선택지: {bounded_option|none}
왜 중요한가: {rationale}
불확실한 점: {uncertainty}
영향받는 작업: {affected_scope_summary}
민감 동작 범위: {sensitive_action_scope_plain_summary|not_applicable}
명시적으로 승인하지 않는 것: {sensitive_action_not_authorized|not_applicable}
역량 주장: {sensitive_action_capability_claim|not_applicable}
...
- 민감 동작 승인에서는 활성 범위 필드에서 평이한 요약을 렌더링합니다. 이름 붙은 행동, 명령 또는 도구, 의도한 경로, 호스트, 의존성, 비밀값 핸들, 시간 창, 범위 제한, 명시적으로 승인하지 않는 행동, 정직한 역량 주장을 보여줍니다. 다른 판단 종류에서는 이 줄들을 `not_applicable`로 둡니다.
...
- 증거 참조, 스테이징된 아티팩트 상태, 등록 아티팩트 참조, 가림 처리, 가용성 메모
...
행동 또는 변경: {run_or_action_summary}
변경 경로: {changed_paths|none}
확인: {checks_run_or_reason_not_run}
쓰기 확인: {write_check_summary|no product write}
확인 근거 요약: {evidence_status}. {evidence_summary}
뒷받침하는 증거 참조: {evidence_refs|none}
스테이징된 첨부: {staged_artifact_summary|none}
등록 아티팩트 참조: {artifact_ref_summary|none}
가림 처리 또는 가용성: {redaction_availability_summary|none}
뒷받침하는 주장: {supported_claims_or_criteria|none}
...
- 스테이징된 첨부는 임시 입력 스테이징일 뿐입니다. 등록 아티팩트 참조와 따로 보여주고, 실행 기록이 이를 소비해 등록 아티팩트를 주장에 연결하기 전까지 지속 증거로 세지 않습니다.
...
사용자가 닫기 준비 상태, 닫기를 막는 이유, 또는 닫기 결과를 간결하게 봐야 할 때 `close-result`를 사용합니다. 최종 수락, 잔여 위험, 증거, 아티팩트 가용성, 자체 확인 근거, 차단 사유를 서로 분리해 보여줍니다.
...
- 닫기 가능 여부, 닫기를 막는 이유, 닫기를 가능하게 하는 다음 행동
- 원천 상태 버전, 최신성, 역량 상태
...
닫기를 막는 이유: {reasons_this_cannot_be_closed_yet|none}
닫기를 가능하게 하는 다음 행동: {smallest_unblocker|none}
...
- 최종 수락과 잔여 위험 수락은 서로 다른 줄로 유지합니다. 넓은 "좋아 보여"는 이름 붙은 결과에 대한 대기 중인 최종 수락 질문을 해결할 때만 최종 수락으로 보일 수 있습니다. 이름 붙은 위험을 묻고 기록한 경우가 아니라면 잔여 위험 수락이 아닙니다.
- 잔여 위험 수락은 빠진 필수 증거나 닫기 관련 아티팩트 가용성 문제를 고치지 않습니다. 담당 경로가 해결할 때까지 증거 또는 아티팩트 차단 사유를 계속 보입니다.
...
경계: 에이전트 맥락 패킷은 행동을 돕는 맥락일 뿐입니다. 쓰기를 승인하거나, gate를 충족하거나, 증거를 만들거나, 민감 동작 승인을 부여하거나, 최종 수락을 기록하거나, 잔여 위험을 수락하거나, 닫기 준비 상태를 만들거나, Task를 닫을 수 없습니다. Core 상태 입력도 아닙니다. 생성된 상태 카드, Projection, 렌더링된 템플릿, 대화 요약, 패킷 필드를 담당 상태로 다시 입력하면 안 됩니다.

기준 기록:

- 표시해도 안전한 확인된 접점 상태. 상태 변경 접근이나 아티팩트 본문 접근이 현재 가능한지 포함합니다.
- 프로젝트 전체 `state_version`과 원천 참조
- 활성 Task와 활성 Change Unit 요약
- 활성 범위, 허용 경로 또는 영향 영역, 범위 밖 항목, 수락 기준, `autonomy_boundary`
- 다음 안전한 행동에 영향을 주는 `ShapingReadiness` 공백
- 해결되지 않은 사용자 판단
- 관련 있을 때의 활성 `SensitiveActionScope` 요약
- 관련 있을 때의 Write Authorization 요약
- 관련 있을 때의 스테이징된 아티팩트 핸들 상태와 등록된 `ArtifactRef` 상태
- `EvidenceSummary` 상태와 참조, 또는 증거 공백
- 활성 차단 사유
- 닫기 차단 사유
- 활성일 때 잔여 위험 요약
- 보장 표시 조건. 특히 현재 주장이 `cooperative`인지, 확인된 범위의 `detective`인지, 또는 사용 불가/역량 제한인지 보여줍니다.
- 정확히 하나의 다음 안전한 행동
...
- 확인된 접점 상태
- 프로젝트 전체 `state_version`과 원천 참조
- 활성 Task와 활성 Change Unit
- 활성 범위
- 구체화 준비 상태 공백
- 해결되지 않은 사용자 판단
- 활성 민감 동작 범위
- Write Authorization
- 스테이징된 아티팩트와 등록된 아티팩트
- 증거 요약 또는 공백
- 차단 사유
- 다음 안전한 행동
- 닫기 차단 사유
...
- `state_version`은 프로젝트 전체 Core/API 상태 버전입니다. Task별 로컬 시계가 아닙니다.
- `verified_surface`는 표시해도 안전한 현재 접근 맥락입니다. `LocalSurfaceRegistration`을 새로 고치거나, 권한을 부여하거나, 물리적 차단을 증명하지 않습니다.
- 이 에이전트용 패킷은 `change_unit_ref`, `autonomy_boundary`, `close_blockers`, `SensitiveActionScope`, `WriteAuthorizationSummary`, `EvidenceSummary` 같은 기계 판독용 필드 이름을 유지할 수 있습니다. 이런 이름을 사용자용 카드의 표시 라벨로 다시 쓰면 안 됩니다.
...
- `guarantee_display` 필드는 필수 보장 표시 맥락입니다. Core/MCP를 사용할 수 없으면 `GuaranteeDisplay.level` 값을 만들어 내지 말고 `unavailable`/역량 조건을 넣습니다. 새로 확인하기 전까지 하네스에 의존하는 상태, 쓰기, 증거, 최종 수락, 잔여 위험, 닫기 주장은 사용할 수 없는 것으로 다룹니다. 관련 역량 확인이 통과했고 확인된 관찰 범위 안일 때만 `detective`를 씁니다.
- 새 아티팩트 바이트는 현재 MVP에서 `harness.stage_artifact`로 들어오며, 담당 `harness.record_run` 경로가 소비하기 전까지는 스테이징 상태입니다. `captured_artifact`, 접점 자체 캡처, 원시 경로, 원시 로그, 캡처 어댑터 출력을 활성 아티팩트 권한처럼 렌더링하지 않습니다.
