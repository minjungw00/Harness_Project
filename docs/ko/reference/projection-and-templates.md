...
| User Judgment | `judgment_kind`, `presentation`, `display_label`을 포함한 `state.sqlite.user_judgments`, 관련 `decision_gate` state, judgment event, 관련 approval 또는 reconcile record, artifact ref, MVP-1 residual-risk blocker ref, later profile이 active일 때만 연결되는 `state.sqlite.residual_risks` | MVP-1 판단 요청, 상태 카드의 판단 줄, status/next-action responses, user-judgment resources, 또는 dedicated prompt. Later/standalone projection이 켜진 경우 optional full-format Decision Packet, `TASK`, Journey Card, `DEC` display | `request_user_judgment` / `record_user_judgment`, then projector |
...
| Stewardship Impact Summary | `domain_terms`, `module_map_items`, `interface_contracts`, `feedback_loops`, TDD가 selected된 경우 TDD records, MVP-1 blocker/user judgment 또는 later `state.sqlite.residual_risks`, policy validator 결과, related refs | `TASK` Stewardship Impact와 status/resume stewardship display | Owner 기록 업데이트, validator 결과, reconcile, close flow, then projector |
