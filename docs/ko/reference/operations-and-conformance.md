...
- MCP exposure posture를 기본 local-only로, 문서화된 access-control contract가 있으면 함께 connector manifest에 기록합니다
...
| Surface security, guard, freeze, MCP access honesty | `CONN-guard-display-matches-capability`, `CONN-cooperative-freeze-does-not-claim-prevention`, `CONN-mcp-unavailable-holds-product-runtime-code-writes`, `CONN-local-only-mcp-default-and-off-profile-remote-held`, `CONN-careful-mode-does-not-create-authority`, freeze와 Autonomy Boundary rows | Cooperative/detective honesty와 MCP-unavailable hold는 Agency-Hardened MVP. 더 강한 blocking을 주장하는 preventive `T4` 또는 remote/shared connector profile은 later candidate | `status`, `next`, `prepare_write`, `connect`, `serve mcp`, 또는 operator diagnostic | `guarantee_level`이 있는 registered surface/capability profile, MCP availability 또는 off-profile facts, write-capable case의 active Task/Change Unit/gates, fixture가 persistent state를 검증할 때 optional Autonomy Boundary 또는 freeze 관련 owner records |
