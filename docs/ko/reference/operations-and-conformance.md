...
이 문서는 향후 operator와 conformance behavior를 위한 참조 문서입니다. 현재 저장소는 문서 전용이며 runnable Harness Server conformance test를 담고 있지 않습니다. 현재 단계와 인계 상태는 [구현 개요](../build/implementation-overview.md#문서-수락-상태)에 있습니다.
...
| Fixture 작성 순서와 suite coverage | [Conformance staging](#conformance-staging), 그다음 [검증 프로파일별 증명 동작](conformance-fixtures.md#검증-프로파일별-증명-동작), [내부 엔지니어링 점검 Fixture Set](conformance-fixtures.md#v01-core-authority-smoke-fixture-set), [MVP-1 사용자 작업 루프 Fixture Set](conformance-fixtures.md#v02-first-user-value-slice-fixture-set), [Clarification Quality Fixture Group](conformance-fixtures.md#clarification-quality-fixture-group), [커널 스모크(Kernel Smoke) Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue), [향후 Fixtures: Fixture Suites](../later/future-fixtures.md#fixture-suites) | Kernel gate와 event name은 [Core Model 참조](core-model.md)에 남습니다. Future catalog suite는 listing만으로 early-stage requirement가 되지 않습니다. |
| Concern별 fixture 예시 | [향후 Fixtures: Fixture 예시 지도](../later/future-fixtures.md#fixture-예시-지도), 그다음 해당 예시 section | 예시 `input`은 계속 담당 public tool schema를 통과해야 합니다. |
...
| 보증 프로필 | 이 단계에서 active인 owner path를 통해 verification, Manual QA, residual-risk, 작업 수락, stewardship, context-hygiene profile을 위한 assurance-oriented support를 제공합니다. | Operator recovery/export completeness, broad projection/reconcile operations, release handoff, full operations conformance profile. |
...
| 보증 프로필 | Assurance diagnostic은 verification independence, 수동 QA, 잔여 위험 수용, 작업 수락, stewardship evidence의 누락을 보고할 수 있지만, 격리나 사전 차단을 암시하면 안 됩니다. |
...
|---|---|---|
| [`harness connect`](#connect) | 내부 엔지니어링 점검 최소 registration, 더 넓은 connector-profile 동작은 승격된 뒤 | repository/runtime registration 의미와 first-connection expectation이 필요할 때 |
...
Status/next recommendation, Role Lens output, recommended playbook, operator diagnostic은 이후 기존 Core/MCP mutation path가 underlying action을 기록하지 않는 한 read-only guidance입니다. 사용자 판단 요청, `prepare_write`, evidence collection, verification, QA, reconcile, repair, export, close attempt를 제안할 수는 있지만, 그 자체로 state를 mutate하거나, write를 허가하거나, gate를 충족하거나, 결과를 수락하거나, 잔여 위험을 받아들이거나, Task를 close하지 않습니다.
...
Build 문서는 내부 엔지니어링 점검과 stage exit를 계획하기 위한 문서 수준 수락 점검을 제공할 수 있습니다. 이 점검은 reviewer가 내부 엔지니어링 점검을 좁게 유지하도록 돕지만 fixture field, suite metadata, public request schema, storage row, primary error, runner comparison mode가 아닙니다. 향후 runtime pass/fail은 [Conformance Fixtures 참조](conformance-fixtures.md)의 exact body shape와 assertion semantics를 사용하는 executable fixture에서만 나옵니다.

내부 엔지니어링 점검은 첫 실행 가능한 authority-loop target이며, 커널 스모크(Kernel Smoke)는 그 좁은 경로를 실행하는 향후 smoke-check label입니다. Stage exit criteria는 Build의 [내부 엔지니어링 점검](../build/engineering-checkpoint.md)이 담당합니다. Exact future runtime fixture queue는 [Conformance Fixtures 참조: Kernel Smoke Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue)가 담당합니다. 최소 Kernel Smoke subset 통과는 첫 내부 Core 권한 경로를 증명하지만 전체 conformance suite를 요구하지 않고 MVP-1 사용자 작업 루프 fixtures, 보증 프로필 fixtures, operations conformance를 주장하지 않습니다.

MVP-1 사용자 작업 루프는 [Conformance Fixtures 참조](conformance-fixtures.md#v02-first-user-value-slice-fixture-set)의 작은 user-value fixture set과 [Clarification Quality Fixture Group](conformance-fixtures.md#clarification-quality-fixture-group)을 사용합니다. MVP-1은 broad future catalog, full Manual QA, Eval systems, TDD trace, module map, interface contract, Journey/Spine projection, export/recover, dashboard/team/orchestration, advanced connector/security fixture를 실행해야 하는 단계가 아닙니다.

이후 conformance profile은 [MVP-1 사용자 작업 루프 계획](../build/mvp-user-work-loop.md)의 stage name을 따릅니다. 보증 프로필 fixtures는 보증 프로필에, 운영 프로필 또는 promoted-expansion fixtures는 운영 프로필과 승격된 로드맵 candidate에 대응합니다. Exact policy, API, storage, projection, connector, fixture requirement는 각 Reference owner에 남습니다. Suite catalog metadata는 runner selection과 reporting을 위해 scenario를 suite, delivery stage, tag로 group할 수 있지만 Core에 전달되지 않습니다. 향후 executable fixture는 여전히 Core state, events, artifacts, projections/freshness, errors를 통해 검증해야 합니다.
...
Docs-maintenance 요약: 이 smoke path는 Markdown을 읽고 임시 report만 냅니다. Harness runtime state, artifact, projection, QA, 작업 수락, 위험, close fact를 만들지 않습니다.
...
Connect 순서 요약: registration은 Product Repository, Runtime Home, surface profile, 필요한 MCP 도달성 확인, stage/profile check를 연결하지만 generated file을 state로 취급하지 않습니다.
...
Doctor 요약: `doctor`는 readiness category를 읽고 diagnostic level을 보고합니다. 이 도식만으로 repair, mutation, runtime 인증이 일어나지는 않습니다.
...
Recovery 요약: recover는 failure를 분류하고 필요하면 compensating fact를 기록합니다. Failed projection, interrupted write, stale baseline, missing artifact가 성공 주장으로 바뀌지 않게 합니다.
...
Export 요약: export bundle은 Core state, event-version fact, registered artifact, projection snapshot, redaction/omission metadata에서 파생됩니다. 네 번째 권한 공간이 아닙니다.
...
- Suggested checklist item은 advisory입니다. 차단하는 사용자 소유 판단, 잔여 위험을 받아들이는 판단, 수동 QA, evidence, verification, 민감 동작 permission 필요성, 또는 해당 profile이 active일 때 later Approval 필요성을 드러내면, 그 need는 기존 User Judgment, evidence, 수동 QA, Eval, residual-risk, 민감 동작 permission / Approval, close path로 라우팅됩니다.
...
Artifact check 요약: artifact integrity는 registered record와 stored file을 비교하고, 신뢰할 수 없는 bytes가 있으면 dependent evidence, projection freshness, close readiness를 stale 또는 blocked로 표시합니다.
...
Artifact check가 `secret_omitted` 또는 `blocked`를 관찰하면 이후 operation은 그 영향을 숨기지 않고 보고합니다. Evidence Manifest와 QA view는 omitted 또는 blocked ref를 표시하고, 분리 검증은 Eval path가 omission을 받아들이거나 다른 documented resolution이 적용되지 않는 한 사용할 수 없는 원본 bytes를 missing input으로 취급합니다. Projection 표시는 포함된 content 대신 redaction state를 보여주고, export/Release Handoff summary는 값을 노출하지 않고 omission 또는 block을 나열합니다. `secret_omitted`는 secret이 아닌 evidence가 보이는 주장만 뒷받침할 수 있습니다. `blocked`는 attempted capture를 audit 가능하게 보존하지만, replacement, waiver, User Judgment outcome, accepted risk, documented fallback이 path를 해소하기 전까지 dependent evidence, QA, Eval, projection, export, Release Handoff input을 blocked, insufficient, unavailable, unresolved 중 적절한 상태로 남깁니다.
...
[향후 Fixtures: Agency, Stewardship, Context, Design-Quality Suite](../later/future-fixtures.md#agency-stewardship-context-design-quality-suite)로 이동했습니다. Suite responsibility와 read-only recommendation boundary는 승격 전까지 future catalog guidance입니다.
...
[향후 Fixtures: Intake와 Decision Catalog Entries](../later/future-fixtures.md#intake와-decision-catalog-entries)로 이동했습니다. Catalog row는 exact-shape fixture로 구체화되기 전까지 guidance입니다.

### Staged Fixture Coverage

[향후 Fixtures: Staged Fixture Coverage](../later/future-fixtures.md#staged-fixture-coverage)로 이동했습니다. Staged suite coverage map은 승격 전까지 future catalog guidance입니다.

### Fixture 예시 지도

[향후 Fixtures: Fixture 예시 지도](../later/future-fixtures.md#fixture-예시-지도)로 이동했습니다. Concern-specific fixture example은 그 문서가 담당합니다.

### Core Fixture 예시

[향후 Fixtures: Core Fixture 예시](../later/future-fixtures.md#core-fixture-예시)로 이동했습니다.

### Agency Fixture 예시

[향후 Fixtures: Agency Fixture 예시](../later/future-fixtures.md#agency-fixture-예시)로 이동했습니다.

### Connector Fixture 예시

[향후 Fixtures: Connector Fixture 예시](../later/future-fixtures.md#connector-fixture-예시)로 이동했습니다.
...
[향후 Fixtures: Design-Quality Fixture 예시](../later/future-fixtures.md#design-quality-fixture-예시)로 이동했습니다.

### Stewardship Fixture 예시

[향후 Fixtures: Stewardship Fixture 예시](../later/future-fixtures.md#stewardship-fixture-예시)로 이동했습니다.
...
[향후 Fixtures: Context Hygiene Fixture 예시](../later/future-fixtures.md#context-hygiene-fixture-예시)로 이동했습니다.
...
[향후 Fixtures: 로드맵 Browser QA Capture Candidate Entries](../later/future-fixtures.md#v1-expansion-browser-qa-capture-candidate-entries)로 이동했습니다. Owner docs가 승격하고 증명하기 전까지 catalog-only future candidate로 남습니다.

### Fixture Suites

[향후 Fixtures: Fixture Suites](../later/future-fixtures.md#fixture-suites)로 이동했습니다. Suite catalog grouping과 fixture-family summary는 승격 전까지 future catalog guidance입니다.
