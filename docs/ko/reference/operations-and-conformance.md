...
Agency-Hardened MVP는 final reference conformance target입니다. Decision Packet quality, sensitive approval lifecycle separation, 결과 수락과 close 전 residual-risk visibility, same-session verification guard coverage를 포함한 detached verification guards, Manual QA, stewardship 및 context-hygiene validators, full feedback-loop checks, codebase stewardship coverage, projection/reconcile completeness, recover/export/artifact integrity behavior, connector guard/freeze honesty, later 경계 checks, broader fixture coverage를 추가로 증명해야 합니다. Suite catalog metadata는 runner selection과 reporting을 위해 scenario를 suite, stage, tag로 group할 수 있지만 Core에 전달되지 않습니다. Executable fixture는 여전히 Core state, events, artifacts, projections, errors를 통해 검증해야 합니다.
