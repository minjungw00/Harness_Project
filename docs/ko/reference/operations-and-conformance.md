...
| Fixture 작성 순서와 suite coverage | [Conformance staging](#conformance-staging), 그다음 [검증 프로파일별 증명 동작](conformance-fixtures.md#검증-프로파일별-증명-동작), [커널 스모크(Kernel Smoke) Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue), [Staged Fixture Coverage](conformance-fixtures.md#staged-fixture-coverage), [Fixture Suites](conformance-fixtures.md#fixture-suites) | Kernel gate와 event name은 [커널 참조](kernel.md)에 남습니다. |
...
코어 권한 조각(v0.1 Core Authority Slice)은 첫 실행 가능한 conformance target이며, 커널 스모크(Kernel Smoke)는 first runnable kernel slice fixtures를 위한 authoring profile입니다. Stage exit criteria는 Build의 [첫 실행 가능한 조각](../build/first-runnable-slice.md)이 담당합니다. Exact future runtime fixture queue는 [Conformance Fixtures 참조: Kernel Smoke Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue)가 담당합니다. 커널 스모크(Kernel Smoke) 통과는 첫 runnable kernel 권한 경로를 증명하지만 user-facing MVP fixtures, agency-hardened fixtures, operations/future fixtures conformance를 주장하지 않습니다.

이후 conformance profile은 [MVP 계획](../build/mvp-plan.md)의 stage name을 따릅니다. first runnable kernel slice fixtures는 코어 권한 조각(v0.1 Core Authority Slice)에, user-facing MVP fixtures는 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)에, agency-hardened fixtures는 보증과 스튜어드십 팩(v0.3 Assurance & Stewardship Pack)에, operations/future fixtures는 운영과 인계 팩(v0.4 Operations & Handoff Pack)과 승격된 v1+ Expansion candidate에 대응합니다. Exact policy, API, storage, projection, connector, fixture requirement는 각 Reference owner에 남습니다. Suite catalog metadata는 runner selection과 reporting을 위해 scenario를 suite, delivery stage, tag로 group할 수 있지만 Core에 전달되지 않습니다. 향후 executable fixture는 여전히 Core state, events, artifacts, projections/freshness, errors를 통해 검증해야 합니다.
...
- active projection profile에 포함된 projection view만 렌더링하거나 대기열에 넣습니다. 첫 runnable kernel slice에는 persisted Markdown projection이 필요하지 않습니다
...
[Conformance Fixtures 참조: Staged Fixture Coverage](conformance-fixtures.md#staged-fixture-coverage)로 이동했습니다. Staged suite coverage map은 그 문서가 담당합니다.
