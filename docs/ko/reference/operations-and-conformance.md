...
Docs-maintenance 요약: 이 문서 점검 layer는 Markdown을 읽고 임시 report만 냅니다. Harness runtime state, artifact, projection, QA, 작업 수락, 위험, close fact를 만들지 않습니다. 위 목록이 보고 형식입니다.
...
Export 요약: export bundle은 Core state, event-version fact, registered artifact, projection snapshot, redaction/omission metadata에서 파생됩니다. 네 번째 권한 공간이 아닙니다. 위 목록이 bundle 내용을 이름 붙입니다.
...
Artifact check 요약: artifact integrity는 registered record와 stored file을 비교하고, 신뢰할 수 없는 bytes가 있으면 dependent evidence, projection freshness, close readiness를 stale 또는 blocked로 표시합니다. 위 점검 목록이 무결성 계약입니다.
