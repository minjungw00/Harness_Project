...
요약: 내부 엔지니어링 점검 conformance scope는 좁은 Core 권한 루프입니다. Project/Task setup, 하나의 scoped boundary, `prepare_write`와 Write Authorization, `record_run`과 evidence link, status/blocker output만 봅니다. 이후 프로필이 user-facing, assurance, operations coverage를 더하지만 broad future catalog를 초기 단계 요구사항으로 만들지는 않습니다.
