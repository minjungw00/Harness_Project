...
Harness 운영자 절차, Conformance staging과 향후 run entrypoint behavior, 문서 점검, active MVP structured draft, 향후 runtime conformance 사이의 경계를 찾아볼 때 이 참조 문서를 사용합니다.
...
- 향후 fixture의 구조화된 draft body shape나 assertion semantics가 필요할 때. Overview는 여기서 시작하고 core detail과 활성 state-assertion fixture draft는 [Conformance Fixtures 참조](conformance-fixtures.md)를, 간결한 향후 scenario family 목록은 [향후 Fixtures](../later/index.md#future-fixture-families)를 사용합니다.

## 읽기 전에

핵심 적합성 모델, 활성 state-assertion fixture draft, future fixture의 구조화된 body shape, execution, assertion semantics, 현재 단계 상태, Kernel Smoke 작성 순서는 [Conformance Fixtures 참조](conformance-fixtures.md)를 사용합니다. Catalog listing만으로 stage-required가 되지 않는 간결한 향후 scenario family 목록은 [향후 Fixtures](../later/index.md#future-fixture-families)를 사용합니다. Core mutation authority placement와 런타임 경계 공간은 [Runtime Boundaries](runtime-boundaries.md)를, security asset, trust boundary, threat, control은 [보안 참조](security.md)를, public tool schema와 replay 동작은 [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md), [API Errors](api/errors.md)를, storage layout은 [Storage](storage.md)를, 상태 전이 의미는 [Core Model 참조](core-model.md)를 사용합니다.
...
- 문서 점검은 Markdown 문서에 대한 읽기 전용 편집 점검입니다. Link integrity, terminology consistency, stage boundary, security wording, user-language check, owner-boundary drift, 영어/한국어 의미 일치를 봅니다. Documentation finding만 만들 수 있습니다.
- Active MVP structured draft는 내부 엔지니어링 점검과 MVP-1의 기대 동작을 향후 assertion field로 설명합니다. 아직 실행 가능한 fixture가 아니며 generated runtime artifact도 아니고 현재 runtime pass/fail 결과도 아닙니다.
- runtime conformance는 future server implementation work입니다. Runtime implementation과 fixture suite materialization 이후 conformance는 실행 가능한 fixture로 Harness behavior를 증명합니다. Passing fixture는 Core 또는 operator request를 실행하고 captured response fact, Core state, storage row, event, artifact, blocker, error, forbidden side effect를 비교해야 합니다.
...
Rendered prose, status text, Journey Card text, close report, agent summary는 독자에게 도움이 될 수 있지만, 그것만 맞춰서는 conformance를 통과할 수 없습니다. Runtime finding과 close blocker는 prose-only report text가 아니라 structured Core/API result, owner-record ref, storage row, event, artifact, blocker, error, forbidden-side-effect check, 승격된 경우 projection freshness로 assert해야 합니다. docs-maintenance finding은 자체 read-only report label을 사용하며 runtime conformance 밖에 남습니다.

이 문서는 향후 구현을 위한 operator-facing procedure와 conformance overview를 담당합니다. [Conformance Fixtures 참조](conformance-fixtures.md)는 핵심 적합성 모델, exact structured fixture draft shape, assertion semantics, suite catalog metadata boundary, 검증 프로파일별 증명 동작, 활성 Kernel Smoke, MVP-1 user-loop, security/capability, artifact/evidence fixture draft, 축소된 Kernel Smoke queue를 담당합니다. [향후 Fixtures](../later/index.md#future-fixture-families)는 승격 전까지 catalog-only로 남는 간결한 향후 scenario family 목록을 담당합니다.
...
- [Conformance Fixtures 참조](conformance-fixtures.md) 또는 [향후 Fixtures](../later/index.md#future-fixture-families)로 이동한 fixture-detail anchor의 compatibility stub

## 여기서 다루지 않는 것

이 참조 문서는 runtime implementation readiness를 주장하지 않습니다. Future implementation과 Conformance work에 필요한 단계별 semantics를 정의합니다.

Conformance fixture draft body shape, fixture assertion semantics, 향후 scenario 목록, public MCP schema, SQLite DDL, projection template body, Start/Use workflow, long-term analytics도 이 문서가 담당하지 않습니다. Core fixture mechanics는 [Conformance Fixtures 참조](conformance-fixtures.md)가 담당하고, 향후 scenario family 목록은 [향후 Fixtures](../later/index.md#future-fixture-families)가 담당합니다. docs-maintenance rule body는 [문서 작성 가이드](../maintain/authoring-guide.md#docs-maintenance-checks)가 담당하며, 이 reference는 아래 operator profile 경계만 담당합니다.

## 계약 위치 지도

| 필요한 것 | 먼저 볼 곳 | 관련 owner |
|---|---|---|
| Operator command 의미 | [운영자 entrypoint](#운영자-entrypoint), 그리고 해당 command section: [connect](#connect), [doctor](#doctor), [serve mcp](#serve-mcp), [projection refresh](#projection-refresh), [reconcile](#reconcile), [recover](#recover), [export](#export), [artifacts check](#artifacts-check), [conformance run](#conformance-run) | Core 상태 권한은 [Core Model 참조](core-model.md)에 남고, authority placement는 [Runtime Boundaries](runtime-boundaries.md)에 남습니다. |
...
| Fixture draft body shape와 runner behavior | [Conformance Fixtures 참조: Conformance Fixture Format](conformance-fixtures.md#conformance-fixture-format), [Conformance Execution](conformance-fixtures.md#conformance-execution), [Fixture Assertion Semantics](conformance-fixtures.md#fixture-assertion-semantics) | Public request schema는 [MVP API](api/mvp-api.md)와 [API Schema Core](api/schema-core.md)에 남습니다. Idempotency와 state conflict behavior는 [API Errors](api/errors.md)에 남습니다. Storage seeding과 row-effect detail은 [Storage](storage.md)에 남습니다. |
| 활성 동작 예시, future fixture 작성 순서, future suite label | [Conformance staging](#conformance-staging), 그다음 [검증 프로파일별 증명 동작](conformance-fixtures.md#검증-프로파일별-증명-동작), [Kernel Smoke 동작 예시](conformance-fixtures.md#engineering-checkpoint-behavior-examples), [MVP-1 사용자 작업 루프 동작 예시](conformance-fixtures.md#mvp-1-user-work-loop-behavior-examples), [Security And Capability 동작 예시](conformance-fixtures.md#security-and-capability-behavior-examples), [Artifact And Evidence 동작 예시](conformance-fixtures.md#artifact-and-evidence-behavior-examples), [Kernel Smoke Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue), [향후 Fixtures: Fixture Suites](../later/index.md#future-fixture-families) | Kernel gate와 event name은 [Core Model 참조](core-model.md)에 남습니다. Future catalog suite label은 listing만으로 early-stage requirement가 되지 않습니다. |
| Concern별 향후 scenario 목록 | [향후 Fixtures: 시나리오 묶음 목록](../later/index.md#future-fixture-families예시-지도), 그다음 해당 inventory section | Catalog row는 fixture body, public input example, 현재 runtime conformance case가 아닙니다. |
...
|---|---|
| 내부 엔지니어링 점검 | Active Core path에 대한 지시 기반/협력적 local diagnostic 표현과 제한된 탐지 가능 보고. 구조화된 차단 사유는 Core/API 결과이지 도구 실행 전 사전 차단 증명이 아닙니다. |
| MVP-1 사용자 작업 루프 | 사용자가 보는 status와 차단 사유 표현은 Harness authority 안에서 무엇이 진행될 수 없는지, 무엇을 사용자만 결정할 수 있는지 설명할 수 있습니다. 그래도 surface가 지시로만 보류할 수 있는지, 실행 뒤에만 탐지할 수 있는지 표시해야 합니다. |
...
Status/next recommendation, Role Lens output, recommended playbook, operator diagnostic은 이후 기존 Core/MCP mutation path가 underlying action을 기록하지 않는 한 read-only guidance입니다. 사용자 판단 요청, `prepare_write`, evidence collection, verification, QA, reconcile, repair, export, close attempt를 제안할 수는 있지만, 그 자체로 state를 mutate하거나, write를 승인하거나, gate를 충족하거나, 결과를 수락하거나, 잔여 위험을 수락하거나, Task를 close하지 않습니다.

## Conformance staging

Conformance는 runtime implementation이 존재한 뒤 단계적으로 실행할 수 있지만, staged execution이 exact structured fixture body shape를 바꾸거나 향후 reference conformance 요구사항을 줄이면 안 됩니다. 현재 문서 전용 단계에서 이 section은 향후 runtime conformance 계획이며, fixture file, conformance runner, 실행 가능한 Harness Server conformance test가 이미 존재한다는 뜻으로 읽으면 안 됩니다.
...
문서 점검은 현재 문서 작업의 active layer입니다. Markdown docs의 link integrity, terminology consistency, stage boundary, security wording, user-language quality, documentation drift, owner mismatch, 영어/한국어 file-structure 또는 semantic-section parity gap, owner 밖의 중복 규범 문구, 깨진 link나 anchor, future fixture/request schema drift, enum/API/event/validator/projection drift, glossary/source-of-truth phrasing drift, TODO hygiene 문제를 확인합니다. 이런 finding은 문서 유지보수 finding일 뿐입니다. 이 profile은 Markdown docs에 대한 read-only maintenance check이지 Core fixture conformance, runtime validator, evidence, 잔여 위험을 수락하는 판단, close readiness, 기준 상태 전이가 아닙니다. Enum value, API shape, owner boundary에 대한 docs drift를 찾을 수 있지만 서버가 동작한다는 증명은 아닙니다. `task_events`에 추가하거나, artifact를 만들거나, projection을 refresh하거나, QA 또는 acceptance state를 만들거나, close readiness에 영향을 주거나, runtime implementation readiness를 주장하거나, runtime fixture pass/fail 산정에 포함하면 안 됩니다.
...
향후 operator wiring 계약: 나중 구현이 `harness conformance run` 또는 다른 operator entrypoint로 이 점검을 제공한다면 docs-maintenance는 명시적으로 선택하는 docs-only profile이며, 관례적 profile name은 `docs-maintenance`입니다. runtime conformance run은 operator가 이 profile을 선택하지 않는 한 포함하면 안 됩니다. 명시적으로 선택하더라도 runtime Core fixture suite와 별도로 보고하고 runtime fixture pass/fail 또는 implementation readiness로 계산하지 않습니다. 이 profile의 `PASS`, `WARN`, `FAIL` label은 manual review에 도움을 줄 수 있는 docs-maintenance report label입니다. Core fixture result, manual acceptance, final acceptance, close readiness, implementation readiness가 아니며, 위의 read-only runtime-effect 경계가 그대로 적용됩니다.
...
docs-maintenance 요약: 이 문서 점검 layer는 Markdown을 읽고 임시 report만 냅니다. Harness runtime state, artifact, projection, QA, acceptance, 위험, close fact를 만들지 않습니다. 위 목록이 보고 형식입니다.
...
- Connector Manifest에 connector profile 최신성, capability profile version, detected version, last verification time, conformance 또는 operator-check 증거를 기록합니다
...
- Suggested checklist item은 advisory입니다. 차단하는 사용자 소유 판단, 잔여 위험을 수락하는 판단, 수동 QA, evidence, verification, 민감 동작 승인 필요성, 또는 해당 profile이 active일 때 later Approval 필요성을 드러내면, 그 need는 기존 User Judgment, evidence, 수동 QA, Eval, residual-risk, 민감 동작 승인 / Approval, close path로 라우팅됩니다.

진단 및 보고 경계: future Local Derived Metrics는 owner 문서가 승격하기 전까지 보고서 또는 operator diagnostic에서 읽기 전용 파생 표시로만 나타날 수 있습니다. Operational authority를 만들지 않으며, 후보 경계는 [로드맵: 후보 항목 목록](../later/index.md#roadmap-candidates)을 봅니다.

이 경계의 향후 fixture catalog row는 [향후 Fixtures: 운영 프로필 Catalog Entries](../later/index.md#future-fixture-families)에 둡니다. 이 참조 문서는 operator behavior를 소유합니다. Catalog row는 승격 전까지 MVP-1 요구사항 아님이고, 실행 가능한 항목도 아닙니다.
...
- secret/PII handling이 `redaction_state` 및 export 또는 capture note와 호환됩니다. Raw secret, token, full sensitive log는 증거 바이트로 저장하지 않습니다
...
향후 `conformance run`은 보통 runtime suite가 materialized된 뒤 운영 프로필 또는 나중 단계에서 들어오는 operations-profile surface입니다. 선택한 fixture suite 또는 명시적으로 선택한 docs-only maintenance profile을 실행합니다. Runtime suite는 MCP tool과 operator command가 쓰는 것과 같은 Core entrypoint를 사용하며, exact-shape fixture가 captured response fact, Core state, storage row, event, artifact, blocker, 승격된 경우 projection/freshness, error, forbidden side effect를 비교할 때만 pass/fail이 결정됩니다. docs-maintenance는 별도의 read-only profile로 남으며 runtime fixture pass/fail과 implementation readiness에 포함하지 않습니다.
...
| 정확한 structured fixture draft field, runner loading/execution, default comparison mode | [Conformance Fixtures 참조](conformance-fixtures.md#conformance-탐색-지도) |
| Suite intent와 작성 순서 | [Conformance staging](#conformance-staging), 그다음 [검증 프로파일별 증명 동작](conformance-fixtures.md#검증-프로파일별-증명-동작), [Kernel Smoke 동작 예시](conformance-fixtures.md#engineering-checkpoint-behavior-examples), [MVP-1 사용자 작업 루프 동작 예시](conformance-fixtures.md#mvp-1-user-work-loop-behavior-examples), [Security And Capability 동작 예시](conformance-fixtures.md#security-and-capability-behavior-examples), [Artifact And Evidence 동작 예시](conformance-fixtures.md#artifact-and-evidence-behavior-examples), [Kernel Smoke Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue), [향후 Fixtures: Fixture Suites](../later/index.md#future-fixture-families) |
| 향후 scenario 목록과 catalog-only future candidate | [향후 Fixtures: 시나리오 묶음 목록](../later/index.md#future-fixture-families예시-지도) |

Operator boundary: 이 문서는 operator entrypoint, runtime/docs-maintenance profile 분리, conformance overview를 담당합니다. [Conformance Fixtures 참조](conformance-fixtures.md)는 fixture body shape, assertion semantics, suite catalog metadata boundary, 검증 프로파일, 활성 structured state-assertion behavior draft, 축소된 Kernel Smoke queue를 담당합니다. [향후 Fixtures](../later/index.md#future-fixture-families)는 간결한 향후 scenario 목록과 catalog-only candidate를 담당합니다. runtime conformance가 구체화되면 runtime suite pass/fail은 계속 실행 상태 기반이며, rendered prose만으로는 conformance를 통과할 수 없습니다.

### Conformance Fixture Format

[Conformance Fixtures 참조: Conformance Fixture Format](conformance-fixtures.md#conformance-fixture-format)으로 이동했습니다. 이 stub은 이전 anchor를 보존합니다. Structured fixture draft body shape, canonical active value boundary, `request.payload` public request schema boundary는 그 문서가 담당합니다. Later-profile shorthand detail은 승격 전까지 [향후 Fixtures: Later-Profile Fixture Shorthand Notes](../later/index.md#future-fixture-families)에 남습니다.
...
[Conformance Fixtures 참조: Fixture Assertion Semantics](conformance-fixtures.md#fixture-assertion-semantics)로 이동했습니다. Response fact, state change, storage row, event, artifact, blocker, error, validator, 승격된 경우 projection/freshness fact, forbidden side effect의 assertion mode는 그 문서가 담당합니다.

### Agency, Stewardship, Context, Design-Quality Suite

[향후 Fixtures: Agency, Stewardship, Context, Design-Quality Suite](../later/index.md#future-fixture-families)로 이동했습니다. Suite responsibility와 read-only recommendation boundary는 승격 전까지 catalog-only 목록입니다.

#### Catalog-Only Fixture Skeleton Guidance

[Conformance Fixtures 참조: Catalog-Only Fixture Skeleton Guidance](conformance-fixtures.md#catalog-only-fixture-skeleton-guidance)로 이동했습니다. Catalog skeleton guidance는 실행 가능한 fixture body가 아닙니다. Later-profile shorthand detail은 [향후 Fixtures: Later-Profile Fixture Shorthand Notes](../later/index.md#future-fixture-families)에 남습니다.

#### Kernel Smoke Authoring Queue

[Conformance Fixtures 참조: Kernel Smoke Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue)로 이동했습니다. 이 queue는 fixture 작성 순서이지 fixture-body metadata가 아닙니다.

#### Intake와 Decision Catalog Entries

[향후 Fixtures: Intake와 Decision Catalog Entries](../later/index.md#future-fixture-families와-decision-catalog-entries)로 이동했습니다. Catalog row는 owner가 승격하기 전까지 시나리오 목록입니다.

<a id="staged-fixture-coverage"></a>

### 시나리오 묶음 목록

[향후 Fixtures: 시나리오 묶음 목록](../later/index.md#future-fixture-families)으로 이동했습니다. 예전 staged coverage map은 지금 compact catalog inventory이며 fixture checklist가 아닙니다.

<a id="fixture-예시-지도"></a>

### 시나리오 목록 지도

[향후 Fixtures: 시나리오 묶음 목록](../later/index.md#future-fixture-families예시-지도)으로 이동했습니다. 예전 fixture example map은 실행 가능한 예시가 아니라 compact catalog inventory입니다.

<a id="core-fixture-예시"></a>

### Core 시나리오 목록

[향후 Fixtures: Core, Evidence, Verification, Close Catalog Entries](../later/index.md#future-fixture-families예시)로 이동했습니다.

<a id="agency-fixture-예시"></a>

### Agency 시나리오 목록

[향후 Fixtures: Agency Catalog Entries](../later/index.md#future-fixture-families예시)로 이동했습니다.

<a id="connector-fixture-예시"></a>

### Connector 시나리오 목록

[향후 Fixtures: Connector Catalog Entries](../later/index.md#future-fixture-families예시)로 이동했습니다.

#### Connector Agency Catalog Entries

[향후 Fixtures: Connector Agency Catalog Entries](../later/index.md#future-fixture-families)로 이동했습니다.

<a id="design-quality-fixture-예시"></a>

### Design-Quality 시나리오 목록

[향후 Fixtures: Design-Quality와 Stewardship Catalog Entries](../later/index.md#future-fixture-families예시)로 이동했습니다.
...
[향후 Fixtures: Stewardship Catalog Entries](../later/index.md#future-fixture-families)로 이동했습니다.

<a id="context-hygiene-fixture-예시"></a>

### Context Hygiene 시나리오 목록

[향후 Fixtures: Context Hygiene Catalog Entries](../later/index.md#future-fixture-families예시)로 이동했습니다.

#### Context Hygiene Catalog Entries

[향후 Fixtures: Context Hygiene Catalog Entries](../later/index.md#future-fixture-families)로 이동했습니다.

#### Core, Projection, Reconcile, Verification Boundary Catalog Entries

[향후 Fixtures: Core, Projection, Reconcile, Verification Boundary Catalog Entries](../later/index.md#future-fixture-families)로 이동했습니다.

#### 로드맵 Browser QA Capture Candidate Entries

[향후 Fixtures: 로드맵 Browser QA Capture Candidate Entries](../later/index.md#future-fixture-families)로 이동했습니다. Owner docs가 승격하고 증명하기 전까지 catalog-only future candidate로 남습니다.

### Fixture Suites

[향후 Fixtures: Fixture Suites](../later/index.md#future-fixture-families)로 이동했습니다. Suite label은 planning label이며 승격 전까지 필수 file set이 아닙니다.
