...
- 자세한 future fixture body shape나 assertion semantics가 필요할 때. Overview는 여기서 시작하고 core detail과 작은 내부 엔지니어링 점검 / MVP-1 동작 예시는 [Conformance Fixtures 참조](conformance-fixtures.md)를, stage-required가 아닌 detailed future scenario candidate는 [향후 Fixtures](../later/future-fixtures.md)를 사용합니다.
...
| MVP 동작 예시, future fixture 작성 순서, future suite coverage | [Conformance staging](#conformance-staging), 그다음 [검증 프로파일별 증명 동작](conformance-fixtures.md#검증-프로파일별-증명-동작), [내부 엔지니어링 점검 동작 예시](conformance-fixtures.md#v01-core-authority-smoke-fixture-set), [MVP-1 사용자 작업 루프 동작 예시](conformance-fixtures.md#v02-first-user-value-slice-fixture-set), [Clarification Quality 동작 예시](conformance-fixtures.md#clarification-quality-fixture-group), [커널 스모크(Kernel Smoke) Authoring Queue](conformance-fixtures.md#kernel-smoke-authoring-queue), [향후 Fixtures: Fixture Suites](../later/future-fixtures.md#fixture-suites) | Kernel gate와 event name은 [Core Model 참조](core-model.md)에 남습니다. Future catalog suite는 listing만으로 early-stage requirement가 되지 않습니다. |
...
MVP-1 사용자 작업 루프는 [Conformance Fixtures 참조](conformance-fixtures.md#v02-first-user-value-slice-fixture-set)의 작은 user-value 동작 예시와 [Clarification Quality 동작 예시](conformance-fixtures.md#clarification-quality-fixture-group)을 사용합니다. 이 예시는 아직 실행 가능한 fixture가 아님. MVP-1은 broad future catalog, full Manual QA, Eval systems, TDD trace, module map, interface contract, Journey/Spine projection, export/recover, dashboard/team/orchestration, advanced connector/security fixture를 실행해야 하는 단계가 아닙니다.
...
Docs-maintenance 요약: 이 문서 점검 layer는 Markdown을 읽고 임시 report만 냅니다. Harness runtime state, artifact, projection, QA, 작업 수락, 위험, close fact를 만들지 않습니다.

```mermaid
flowchart LR
  Start["문서 점검"] --> Check["Markdown docs 확인"]
...
| Concern별 향후 실행 가능한 예시와 catalog-only future candidate | [향후 Fixtures: Fixture 예시 지도](../later/future-fixtures.md#fixture-예시-지도) |
