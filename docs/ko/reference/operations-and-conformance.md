...
Harness 운영자 절차, Conformance staging과 향후 run entrypoint behavior, 문서 점검, 대표 동작 예시, 향후 runtime conformance 사이의 경계를 찾아볼 때 이 참조 문서를 사용합니다.
...
- 향후 fixture 형태나 주장 권한이 필요할 때. Overview는 여기서 시작하고 core detail은 [적합성 참조](conformance.md)를, 이름만 남긴 future fixture family 목록은 [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)를 사용합니다.

## 읽기 전에

현재 적합성 상태, 적합성의 의미, 아직 없는 것, future fixture 형태, 주장 권한, 대표 예시, catalog-only future 경계, metrics 경계는 [적합성 참조](conformance.md)를 사용합니다. Catalog listing만으로 stage-required가 되지 않는 이름만 남긴 future fixture family 목록은 [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)를 사용합니다. Core mutation authority placement와 런타임 경계 공간은 [Runtime Boundaries](runtime-boundaries.md)를, security asset, trust boundary, threat, control은 [보안 참조](security.md)를, public tool schema와 replay 동작은 [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md), [API Errors](api/errors.md)를, storage layout은 [Storage](storage.md)를, 상태 전이 의미는 [Core Model 참조](core-model.md)를 사용합니다.
...
- 대표 동작 예시는 내부 엔지니어링 점검과 MVP-1의 기대 동작을 향후 assertion category로 설명합니다. 실행 가능한 fixture가 아니며, full fixture body도 아니고, generated runtime artifact도 아니며, 현재 runtime pass/fail 결과도 아닙니다.
...
이 문서는 향후 구현을 위한 operator-facing procedure와 conformance overview를 담당합니다. [적합성 참조](conformance.md)는 현재 적합성 경계, future fixture 형태, 주장 권한, 대표 예시, catalog-only future 경계, metrics 경계를 담당합니다. [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)는 owner가 좁은 동작을 승격하기 전까지 이름만 남긴 future fixture family 후보를 담당합니다.
...
- 향후 conformance run과 docs-maintenance reporting의 operator-facing 경계

## 여기서 다루지 않는 것

이 참조 문서는 runtime implementation readiness를 주장하지 않습니다. Future implementation과 Conformance work에 필요한 단계별 semantics를 정의합니다.

Conformance fixture 형태, 주장 권한, future fixture family 이름, public MCP schema, SQLite DDL, projection template body, Start/Use workflow, long-term analytics도 이 문서가 담당하지 않습니다. 적합성 계획 경계는 [적합성 참조](conformance.md)가 담당하고, future fixture family 이름은 [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)가 담당합니다. docs-maintenance rule body는 [문서 작성 가이드](../maintain/authoring-guide.md#docs-maintenance-checks)가 담당하며, 이 reference는 아래 operator profile 경계만 담당합니다.
...
| fixture 형태와 주장 권한 | [적합성 참조: fixture 형태](conformance.md#4-fixture-형태)와 [주장 권한](conformance.md#5-주장-권한) | Public request schema는 [MVP API](api/mvp-api.md)와 [API Schema Core](api/schema-core.md)에 남습니다. Idempotency와 state conflict behavior는 [API Errors](api/errors.md)에 남습니다. Storage row-effect detail은 [Storage](storage.md)에 남습니다. |
| 대표 동작 예시와 future family 이름 | [Conformance staging](#conformance-staging), [적합성 참조: 대표 예시](conformance.md#6-대표-예시), [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families) | 예시는 fixture file이 아니며, future family 이름은 listing만으로 early-stage requirement가 되지 않습니다. |
| Concern별 future family 후보 | [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families) | Catalog name은 fixture body, public input example, 현재 runtime conformance case가 아닙니다. |
...
Conformance는 runtime implementation과 fixture materialization이 생긴 뒤에만 실행됩니다. 현재 문서 전용 단계에서 이 section은 향후 runtime conformance 계획입니다. Fixture file, conformance runner, generated conformance artifact, 현재 Harness Server conformance test가 제공된다는 뜻으로 읽으면 안 됩니다.

Build 문서는 첫 implementation slice와 stage exit를 위한 planning review check를 제공할 수 있습니다. 이 check는 acceptance state, fixture field, suite metadata, public request schema, storage row, primary error, generated conformance artifact, runner comparison mode, runtime pass/fail result가 아닙니다.

내부 엔지니어링 점검은 첫 향후 내부 authority-loop target입니다. Kernel Smoke는 이 target 아래의 future smoke-check authoring label일 뿐이며, stage도 아니고 현재 실행 가능한 suite도 아닙니다. 좁은 향후 proof는 local project/surface 하나, active Task 또는 scoped boundary 하나, `prepare_write`, 필요한 경우 Write Authorization, compatible `record_run` 하나, evidence/link visibility, status/blocker output, 좁은 close-blocker check에 집중합니다.

MVP-1 사용자 작업 루프 conformance는 사용자에게 보이는 work-loop behavior까지만 넓어집니다. 평소 요청이 tracked work가 되는지, 사용자 소유 판단이 분리되어 남는지, broad approval text가 특정 판단을 대신하지 않는지, evidence가 final acceptance나 residual-risk acceptance를 대신하지 않는지, close blocker가 계속 보이는지를 증명해야 합니다. Compact 예시는 [적합성 참조: 대표 예시](conformance.md#6-대표-예시)에 있습니다.

[Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)의 이름은 catalog-only입니다. Owner가 좁은 동작을 exact contract와 proof expectation과 함께 승격하기 전까지 Engineering Checkpoint, MVP-1, Assurance Profile, Operations Profile, Roadmap 요구사항이 아닙니다.

Runtime fixture result가 나중에 생기면 실행 상태 기반이어야 합니다. Rendered prose, status text, Journey Card text, close report prose, generated summary, documentation-check label만으로 conformance를 통과할 수 없습니다.
...
이 경계의 future fixture family 이름은 [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)에 둡니다. 이 참조 문서는 operator behavior를 소유합니다. Family 이름은 승격 전까지 MVP-1 요구사항 아님이고, 실행 가능한 항목도 아닙니다.
...
향후 `conformance run`은 보통 runtime suite가 materialized된 뒤 운영 프로필 또는 나중 단계에서 들어오는 operations-profile surface입니다. 선택한 runtime fixture suite 또는 명시적으로 선택한 docs-only maintenance profile을 실행합니다. Runtime suite는 MCP tool과 operator command가 쓰는 것과 같은 Core entrypoint를 사용합니다. Pass/fail은 captured response fact, Core state, storage row, 승격된 stable event, artifact, blocker, 승격된 projection/freshness, error, forbidden side effect에 대한 structured comparison으로만 결정됩니다.
...
| Future fixture 형태와 주장 권한 | [적합성 참조: fixture 형태](conformance.md#4-fixture-형태)와 [주장 권한](conformance.md#5-주장-권한) |
| 대표 예시 | [적합성 참조: 대표 예시](conformance.md#6-대표-예시) |
| Future family 이름과 catalog-only candidate | [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families) |
| Metrics 경계 | [적합성 참조: Metrics 경계](conformance.md#8-metrics-경계) |

docs-maintenance는 별도 read-only profile로 남으며 runtime fixture pass/fail과 implementation readiness에 포함하지 않습니다. Operator report가 docs-maintenance label을 보여줄 수는 있지만, 그 label은 문서 유지보수 finding일 뿐입니다.
