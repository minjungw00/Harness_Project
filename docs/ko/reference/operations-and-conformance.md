...
Operations는 Core 주변의 operator-facing command입니다. Repository를 연결하고, readiness를 진단하고, MCP를 serve하고, projection을 refresh하고, human edit을 reconcile하고, interrupted state를 recover하고, bundle을 export하고, artifact를 check할 수 있습니다.

중요한 규칙은 operations가 agent와 같은 Core authority 위에 놓인 surface라는 점입니다. 진단, repair, export, fixture 실행은 할 수 있지만 두 번째 state model을 만들거나 Markdown을 authoritative하게 만들면 안 됩니다.
...
Public MCP schema, SQLite DDL, projection template body, Learn/Use workflow, long-term analytics도 이 문서가 담당하지 않습니다. Docs-maintenance는 [문서 작성 가이드](../maintain/authoring-guide.md#docs-maintenance-checks)가 rule을 담당하는 별도의 read-only documentation profile입니다. Runtime fixture conformance, implementation readiness, evidence, QA, acceptance, close readiness, canonical state transition이 아닙니다.
...
Conformance는 incremental하게 실행할 수 있지만, staged execution이 fixture body shape를 바꾸거나 final MVP 요구사항을 줄이면 안 됩니다.

Kernel Smoke는 MVP-0부터 early MVP-3 capabilities까지를 가로지르는 selected smoke slice에서 나온 첫 runnable conformance target입니다. Project와 Task state, scoped Change Unit behavior, `prepare_write` allow/block behavior, durable Write Authorization creation, `record_run` authorization consumption, artifact와 evidence manifest basics, minimal projection enqueue/current behavior, write authority가 없을 때 blocked writes 또는 runs, evidence 또는 decision 요구사항이 없을 때 blocked close, basic Core fixture execution을 증명해야 합니다. Kernel Smoke 통과는 첫 runnable kernel authority path를 증명하지만 final MVP conformance를 주장하지 않습니다.
...
Browser QA Capture conformance는 v1 priority candidate이지 MVP smoke 요구사항이 아닙니다. Future fixtures는 capability profile fields, redaction 및 secret/PII handling, browser test environment, artifact retention, capture artifact mapping, unsupported-surface fallback behavior가 정의된 뒤에만 declared `T6 QA Capture` behavior를 prove해야 합니다. MVP fixtures는 automated browser capture를 요구하지 않고 Manual QA records, artifact refs, QA waiver behavior, close blockers를 계속 prove합니다.
...
Diagnostic and reporting boundary: future [Local Derived Metrics](../roadmap.md#local-derived-metrics)는 report 또는 operator diagnostic에 나타날 수 있지만, roadmap은 이를 later diagnostic-only analytics로 유지합니다. Metric readout은 state mutate, gate satisfaction, write authorization, approval grant, evidence creation, projection enqueue 또는 refresh, projection freshness change, close readiness 또는 implementation readiness 변경, verification perform 또는 record, QA recording, QA 또는 verification waiver, residual risk acceptance, result acceptance, assurance upgrade, Task close를 하면 안 됩니다.
...
Fixture가 설계 품질 severity를 assert할 때는 모든 관련 validator 결과를 `expected_state.validators` 아래 보이게 유지하고, policy-owned [Severity Composition Rule](design-quality-policies.md#severity-composition-rule)이 산출한 합성된 gate, write-blocker, close-blocker, waiver, Decision Packet outcome도 assert해야 합니다. Fixture는 policy schema를 추가하거나 더 강한 merged blocker가 있다는 이유만으로 lower-severity finding을 숨기면 안 됩니다.
...
Fixture runner는 `request_hash`, baseline `tree_hash`, projection `managed_hash`에 대해 reference implementation과 같은 canonicalization rule을 사용해야 합니다. 세부 알고리즘은 MCP API, Storage와 DDL, Document Projection 문서가 계속 담당합니다. Conformance fixture는 그 source-of-truth boundary를 다시 정의하지 않고 deterministic behavior를 검증합니다.
...
| stewardship | 설계 품질 validator와 codebase-stewardship validator는 canonical owner record, ref, policy-owned severity composition 규칙을 통해 `design_gate`, `decision_gate`, `qa_gate`, close blocker, waiver eligibility에 영향을 줍니다. Public interface, module, domain, feedback-loop, TDD, Manual QA, waiver check는 schema나 DDL을 duplicate하지 않습니다. Review Stage display는 새 authority를 만들지 않고 Spec Compliance Review와 Code Quality / Stewardship Review를 분리합니다. |
...
| `CONN-role-lens-non-authoritative-routing` | `next` | `next`는 `product-review`, `eng-review`, `design-review`, `security-review`, `qa-review`, `release-handoff` 같은 role-lens playbooks를 recommend할 수 있습니다. 이 read는 state를 mutate하거나, gate를 satisfy하거나, write를 authorize하거나, evidence를 만들거나, verification을 perform 또는 record하거나, QA를 record하거나, QA 또는 verification을 waive하거나, residual risk를 accept하거나, result를 accept하거나, Task를 close하거나, assurance를 upgrade하지 않습니다. Action이 필요한 lens output은 existing Decision Packet refs, `DecisionPacketCandidate` routes, validator/evidence/Manual QA/residual-risk candidates, release-handoff input, recommended next playbook으로 표현됩니다. |
| `CONN-freeze-narrows-current-boundary` | `prepare_write` or `next` | Freeze request는 display guidance, held write, stricter next action, 또는 existing scope가 incompatible할 때 `prepare_write` block/hold로 반영됩니다. Fixture가 persistent Change Unit, allowed-path, Autonomy Boundary, AFK stop-condition, related owner-record update를 assert한다면, 그 update는 existing Core state-changing path, Decision Packet route, owner-record update path를 통해 일어나야 합니다. Freeze label은 그 자체로 owner records를 mutate하지 않습니다. |
| `CONN-guard-display-matches-capability` | `status` or `prepare_write` | Guard display는 connected profile의 actual `guarantee_level`과 limitation notes를 report합니다. Cooperative guard는 prevention을 claim하지 않습니다. Detective guard에는 changed-path/log/artifact validation assertion이 필요합니다. Proven pre-tool blocking path가 없는 한 preventive guard는 MVP 요구사항이 아닙니다. |
| `CONN-careful-mode-no-authority-upgrade` | `next` or `prepare_write` | Careful mode는 fresher status, narrower scope, explicit `prepare_write`, evidence mapping, one blocking user question을 요구해 next action을 더 strict하게 만들 수 있습니다. 그 자체로 state를 mutate하거나, gate를 satisfy하거나, approval을 grant하거나, write를 authorize하거나, evidence를 create하거나, verification을 perform 또는 record하거나, QA를 record하거나, QA 또는 verification을 waive하거나, residual risk를 accept하거나, result를 accept하거나, Task를 close하거나, assurance를 upgrade하지 않습니다. |
...
이 catalog entries는 future candidates이지 MVP fixture bodies 또는 MVP smoke 요구사항이 아닙니다. Browser QA Capture capability profile, redaction policy, test environment, artifact retention, fixture target, fallback semantics가 정의된 뒤에만 executable이 됩니다.
...
| `BROWSER-QA-capture-not-final-acceptance-or-detached-verification` | `record_manual_qa` 또는 `record_eval` | Browser QA artifacts는 evidence를 support할 수 있지만 final acceptance를 record하지 않고, required human Manual QA judgment를 대체하지 않으며, separate Eval path가 independence 요구사항을 satisfy하지 않는 한 `assurance_level=detached_verified`를 set하지 않습니다. |
...
- connector guard/freeze: cooperative/detective freeze와 guard display, careful-mode non-authority behavior. Preventive `T4` pre-tool blocking은 surface-specific fixture가 hook, wrapper, sidecar, permission layer가 covered operation을 실행 전에 block할 수 있음을 prove할 때만 포함합니다.
