...
| state | current state 읽기 가능성, JSON field parse와 shape 유효성, locks, active Task 일관성 |
...
| Runtime Home에 광범위한 쓰기 권한이 있습니다. | 영향을 받는 영역에 따라 `security/threat model`과 `state`, `surface`, 또는 `artifacts`; write-capable 준비 상태에서는 보통 `FAIL`. | `state.sqlite`, `registry.sqlite`, `project.yaml`, connector manifest, artifact 저장소, staging file, 생성 운영 파일에 대한 변조 위험. 직접 편집은 Core/recover/artifact check가 검증하기 전까지 유효하지 않습니다. |
...
- MCP가 호출자에게 노출될 때 문서화된 접근 제어 계약을 보고합니다. 예: localhost-only binding, Unix-domain socket, per-project token, process-scoped configuration, 이에 준하는 로컬 제어
