...
| 로컬 파일, 생성된 표시, 복사된 식별자, 대화 텍스트, 에이전트 기억이 권한이 아니라는 규칙. | 런타임 위치 정의. 해당 정의는 [런타임 경계](runtime-boundaries.md)가 담당합니다. |

## 지원되는 보안 보장

<a id="honest-guarantee-display"></a>
하네스가 어떤 보장을 설명하려면 [범위](scope.md)와 이 보안 담당 문서가 모두 그 보장 수준을 지원해야 합니다. 주장이 접점 역량에 의존한다면 해당 접점과 관찰 범위에 대한 역량 확인도 통과해야 합니다.
...
조건:
- 호출자, 에이전트, 접점, 커넥터가 문서화된 하네스 경로를 따릅니다.
- 주장이 담당 문서가 정한 Core, API, 저장소, 런타임, 사용자 판단 경계 안에 머뭅니다.

주장할 수 있는 것:
- 하네스 기록, 쓰기 호환성, 증거 요약, 사용자 소유 판단, 닫기 준비 상태 결과는 담당 문서가 정한 하네스 경로의 지배를 받습니다.
- 담당 경로가 현재 상태와 호환되지 않는다고 말하면 하네스는 거부하거나, 진행을 막는 결과를 반환하거나, 집중된 사용자 소유 판단을 요구할 수 있습니다.
...
- 관련 역량 확인이 통과했습니다.
- 관찰 범위가 문서화되어 있습니다.
- 변경 경로 표현은 해당 접점이 관련 동작의 변경 경로를 보고할 때만 사용합니다.

주장할 수 있는 것:
- 확인된 접점과 확인된 역량은 문서화된 관찰 범위 안에서 제한적인 관찰이나 불일치 보고를 뒷받침합니다.
- 보고 조건이 맞으면 관찰된 변경 경로에 대한 제한적 탐지 주장을 할 수 있습니다.
- 역량이 없거나 부족하면 `CAPABILITY_INSUFFICIENT` 같은 담당 문서가 정한 오류 동작으로 이어집니다.
...
주장할 수 있는 것:
- 관련 담당 문서가 요구사항을 정의하면 쓰기 호환성, Run 기록, 닫기 전에 민감 동작 승인이 필요할 수 있습니다.
...
하네스 보안 주장은 로컬 행위자가 하네스 상태, 기록, 아티팩트, 쓰기 호환성, 사용자 소유 판단에 대해 담당 문서가 정한 하네스 경로를 사용한다는 가정에 놓입니다.
...
- 담당 문서가 정한 하네스 경로 밖의 직접 로컬 수정이 유효한 하네스 기록, 증거, 수락, 잔여 위험 수락, `Write Authorization`, 아티팩트 권한을 만든다는 주장.
...
하네스 기록은 그 기록을 만들고, 검증하고, 갱신하는 담당 문서의 하네스 경로를 통해서만 권한을 가집니다.
...
- 호환되는 제품 파일 쓰기는 활성 범위, Change Unit 호환성, 사용자 소유 판단, 그리고 쓰기 담당 문서가 요구하는 `Write Authorization`의 지배를 받을 수 있습니다.
...
- 복사된 아티팩트, Run, 증거, 판단 텍스트에서 생기는 검증이나 수락.
