...
- connector `capability_profile` 필드, 생성된 manifest, 대체 동작, 접점별 recipe. [Agent 통합 참조](agent-integration.md)를 봅니다.
...
탐지적 동작은 무엇을 관찰했고 무엇이 아직 미확인인지 말해야 합니다. 지원되지 않는 명령, 네트워크, secret, 외부 시스템 효과는 근처의 하네스 확인이 성공했다는 이유만으로 통과로 보고하면 안 됩니다.
