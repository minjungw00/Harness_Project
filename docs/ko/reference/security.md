...
- 보안 자산 범주와 신뢰 경계 범주
- `cooperative`, `detective`의 의미와 later/profile-gated `preventive` / `isolated` 라벨에 대한 현재 MVP 비보장 경계
...
현재 MVP 값 집합에서 `GuaranteeDisplay.level` 값은 `cooperative`와 `detective`뿐입니다. `preventive`와 `isolated`는 [Later 후보 색인](../later/index.md)에 남는 later/profile-gated 표시 이름입니다. 현재 MVP 스키마 값이나 활성 보장이 아닙니다.
...
기준 `capability_profile`에는 예방형 또는 격리형 태세가 없습니다. 에이전트와 커넥터는 사용자 의도, guard/freeze/careful-mode 문구, 향후 프로필 아이디어에서 더 강한 보장 라벨을 추론하면 안 됩니다. 향후 지원 필드, 대상 동작, 대체 동작, 오류, 증명 경로는 담당 문서가 승격하기 전까지 [Later 후보 색인](../later/index.md)에 속합니다.
...
로컬 접근 태세도 하네스 호환성 사실입니다. `registered_local`은 API 담당 문서가 요청한 접근 분류에 대해 호출자/전송 경로를 등록된 로컬 접점과 맞는 것으로 다룰 수 있다는 뜻입니다. OS 계정, 편집기, shell, package manager, 임의 로컬 프로세스가 제한된다는 뜻이 아닙니다. API 접근에는 같은 프로젝트에 등록된 `surface_id`, `surfaces.status=active`, 적용되는 경우 호환되는 `project_id`/`surface_id`/`task_id`/`expected_state_version`, 활성 접점 역량이 여전히 필요합니다. `unavailable`, `mismatch`, `revoked` 태세는 공개 API 오류와 표시해도 안전한 진단으로 라우팅됩니다. 더 강한 보안 경계의 증거가 아닙니다.
...
| 비밀값, 토큰, PII, 표시해도 안전한 핸들 | 아티팩트, 로그, prompt, Projection, manifest, export를 통해 누출될 수 있습니다. | 담당 경로는 가림, 생략, blocked-payload metadata, 표시해도 안전한 핸들을 우선해야 합니다. |
...
| 비밀값 또는 PII 노출 | 로그, 스크린샷, trace, prompt, artifact, Projection, manifest, export가 민감 값을 담습니다. | 가림, 생략, blocked-payload notice, 표시해도 안전한 핸들, 담당 경로가 승인한 증거 요약을 우선합니다. |
...
- 담당 경로가 요구하는 등록된 `ArtifactRef`의 `sha256`, `size_bytes`, `content_type`, 소유 관계, 가용성, 가림, 생략, blocked-payload 확인. 이런 확인은 접점 자체 아티팩트 캡처가 아닙니다.
...
예방형 프로필은 later/profile-gated 자료입니다. 현재 MVP에는 기본 도구 실행 전 차단 프로필도, 활성 `preventive` 보장도 없습니다. `prepare_write`, Write Authorization, `allowed`, `blocked`, 파일 잠금, hash, 상태 카드, Projection, 문서 점검, fixture 초안, guard 문구, freeze 문구, careful-mode 문구를 실행 전 차단으로 설명하지 않습니다. 향후 예방형 프로필 필드, 대상 동작, 대체 동작, 오류, 증명 기대치는 담당 문서가 승격하기 전까지 [Later 후보 색인](../later/index.md)에 남습니다.

## 10. Later 격리 경계

격리형 프로필은 later/profile-gated 자료입니다. 현재 MVP에는 기본 `isolated` 보장도, 활성 보안 격리 경계도 없습니다.

분리된 worktree, 새로운 세션, 새로운 evaluator bundle, 별도 프로세스는 최신성, 검증 독립성, 영향 범위 축소를 도울 수 있습니다. 하지만 자동으로 운영체제 샌드박싱, 권한 격리, 변조 불가능 저장소, 보안 격리가 되지는 않습니다.

파일이 로컬이라는 이유, bundle이 최신이라는 이유, 커넥터에 친근한 mode name이 있다는 이유, tool이 다른 directory에서 실행된다는 이유, 문서가 조심하라고 말한다는 이유만으로 `isolated`를 쓰지 않습니다. 향후 격리형 프로필 필드, 경계, 대상 동작, 대체 동작, 오류, 증명 기대치는 담당 문서가 승격하기 전까지 [Later 후보 색인](../later/index.md)에 남습니다.
