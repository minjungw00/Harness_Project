...
이 문서는 Volicord의 보안 보장 표현, 로컬 접근 가정, 민감 동작 승인 경계, 명시적 보안 비주장을 담당합니다.
...
| 코딩 에이전트 통합의 호스트 신뢰, 호스트 승인, 안내 비보장. | Codex 또는 Claude Code 호스트 설정 문법. 해당 문법은 [관리 CLI](admin-cli.md)가 담당합니다. |

## 지원되는 보안 보장

<a id="honest-guarantee-display"></a>
Volicord가 어떤 보장을 설명하려면 [범위](scope.md)와 이 보안 담당 문서가 모두 그 보장 수준을 지원해야 합니다. 보장 표시는 프로젝트 강제 프로필, 확인된 묶인 접점 등록, 활성화된 강제 메커니즘, 지원되는 기준 범위에서 파생됩니다. 주장이 접점 역량에 의존한다면 그 접점과 관찰 범위에 대한 역량 확인도 통과해야 합니다.

보장 표시는 그 표시를 정당화하는 접점 또는 증거 관찰에 묶여 있어야 합니다. 협력형 Run 보고나 협력적 `agent_report` 관찰은 별도 지원 접점 관찰 또는 외부 결과가 기록되고 인용되지 않는 한 `detective`나 외부 관찰 사실이 아닙니다.
...
조건:
- 호출자, 에이전트, 접점, 커넥터가 문서화된 Volicord 계약을 따릅니다.
...
조건:
- 주장이 접점을 이름으로 밝힙니다.
- 프로젝트 강제 프로필과 확인된 묶인 접점 등록이 그 주장을 지원합니다.
- 관련 역량 또는 강제 확인이 통과했고 관찰된 동작에 대해 지원되는 사실을 만들었습니다.
- 관찰 범위가 문서화되어 있습니다.
...
- 역량 선언만으로 보장이 `cooperative`보다 높아진다는 주장.
...
- 검증 근거는 통제된 등록 값과 어댑터 바인딩 값으로 구성되는 진단 메타데이터입니다. 접근 허용, 감사 증명, 호출자 권한이 아닙니다.
- 기준 확인된 행위자 보증은 협력적 등록 접점 출처이지 암호학적 인간 신원 증명이 아닙니다.
...
- 호출자가 제공한 `verified` 플래그, 요청된 `access_class`, `capability_profile`, 복사된 `verification_basis`, 공개 요청 필드, 환경 변수가 로컬 접근을 부여하거나 신뢰된 검증 근거 문구를 제공한다는 주장.
- `ToolEnvelope.actor_kind=user`가 호환되는 `VerifiedActorContext` 없이 인간 신원을 증명하거나 사용자 권한을 제공한다는 주장.
- 호스트 설정 설치가 호스트가 MCP 서버를 신뢰, 승인, 로드, 초기화, 노출했다는 사실을 증명한다는 주장.
...
- `VerifiedActorContext`는 에이전트 통합과 메서드 담당 문서가 현재 권한 해결 동작에 대해 묶인 접점 역할, 접점 인스턴스, 검증 근거, 보증 수준을 받아들일 때만 권한 출처를 제공할 수 있습니다.
- `surfaces.local_access_json.authorized_access_classes`가 문서화된 여러 접근 등급을 담고 있으면 등록된 접점 인스턴스 하나가 그 여러 접근 등급 허용을 가질 수 있습니다.
...
- `ToolEnvelope.actor_kind`가 신원이나 권한의 증명이라는 주장.
...
주장할 수 있는 것:
- 관리 CLI가 필요한 확인을 관찰할 수 있으면 Host Installation 검증은 `complete`를 `action_required`, `partial_failure`, `failed`와 구분할 수 있습니다.
...
- 에이전트 instructions, `AGENTS.md` 블록, `CLAUDE.md`, `.claude/rules/` 파일, MCP 서버 instructions가 접근 제어, 보안 강제, 사용자 판단, `Write Authorization`, 또는 모델이 이를 따랐다는 증명이라는 주장.
...
- [런타임 경계](runtime-boundaries.md): Volicord 소스 저장소/설치 파일, 실행 파일 프로세스, `Product Repository`, `Volicord Runtime Home`, 외부 MCP 호스트 설정 경계.
