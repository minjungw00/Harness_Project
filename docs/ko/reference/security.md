...
| 아티팩트 저장소 | 아티팩트 바이트는 등록되고, 담당 기록과 연결되고, 필요한 무결성/가림 메타데이터가 확인되기 전까지 신뢰하지 않습니다. staged bytes는 적격 `record_run` 승격으로 persistent `ArtifactRef`가 만들어지기 전까지 임시 staging에 남습니다. |
...
| 아티팩트 또는 증거 변조 | 바이트, 경로, 해시, 스테이징 핸들, 메타데이터가 바뀌었거나 `stale`, 만료, 이미 소비됨, `missing`, `redacted`, `blocked`, 다른 Task, 다른 접점, `unrelated` 상태입니다. | 필요한 경우 스테이징 핸들의 project, task, surface instance, 만료 여부, 소비 상태, checksum, size, 등록, 무결성, 가림, 담당 관계 확인이 통과할 때까지 증거를 `insufficient` 또는 `blocked`로 취급합니다. 원시 경로, 원시 로그, 임의 로컬 경로 문자열, 접점 자체 캡처 주장은 아티팩트 권한이 아닙니다. |
