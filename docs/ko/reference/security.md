...
- `harness.stage_artifact`가 호출자가 제공한 안전한 바이트 또는 안전한 알림에 대해 같은 프로젝트/같은 Task에 묶인 임시 스테이징 핸들만 만듭니다. 임의 로컬 파일이 안전하거나 허가되었다는 증명은 아닙니다.
...
- `record_run`은 `run_recording`으로 유효한 staging handle을 소비해 persistent `ArtifactRef`로 승격합니다. `artifact_registration`은 `harness.stage_artifact`에만 남습니다.
