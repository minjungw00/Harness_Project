...
| `isolated` | Work 또는 verification이 별도 worktree, sandbox, process, evaluator bundle, 또는 equivalent boundary를 가로질러 실행됩니다. Isolation은 blast radius를 줄이지만 그 자체로 approval, verification, 작업 수락, close를 만들지 않습니다. |
