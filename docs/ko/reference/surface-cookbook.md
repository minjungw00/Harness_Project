...
Generic kernel rule, public API schema, policy contract를 여기서 반복하지 않습니다. Guard, freeze, careful-mode label은 connected profile의 실제 cooperative, detective, preventive, isolated capability 위에 얹힌 label로만 쓸 수 있습니다. Recipe가 이런 label을 쓰면 그 동작이 범위 보류인지, 사후 감지인지, 입증된 실행 전 차단인지, isolation인지 밝혀야 합니다.
...
Claude Code recipe는 해당 hook이 configured되어 있고 conformance가 covered operation을 실행 전에 차단할 수 있음을 입증한 경우에만 "guard"를 `PreToolUse`에 매핑할 수 있습니다. 그렇지 않으면 freeze와 careful mode는 cooperative 범위 보류 또는 다음 행동을 더 엄격하게 제한하는 instruction과 available post-tool capture로 남습니다.
