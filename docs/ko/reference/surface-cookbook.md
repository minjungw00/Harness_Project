...
- 캡처·가드·격리 선택지
- 접점별 맥락 전략: 항상 주입되는 운영 맥락을 역할, 현재 단계/context profile, 현재 상태 요약, 활성 blocker, 대기 중인 사용자 소유 결정, 다음 허용 행동으로 제한하는 방법과 세션 시작, Discovery, 사용자 결정 요청, 쓰기 준비, 실행/근거, 닫기 준비 상태, 오류/복구 profile 사이를 전환하는 방법
