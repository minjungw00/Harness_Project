...
접점 이름만으로 보장 수준을 추론하면 안 됩니다. 모든 connector는 실제 사용하는 host/profile/configuration에 대한 capability profile을 선언해야 하며, profile이 입증한 capability가 guarantee level을 결정합니다.

Generic capability profile 규칙은 [Agent 통합 참조](agent-integration.md#capability-profiles)를 봅니다.

Surface recipe는 로컬 접근용 public error code나 OS 수준 보안 보장을 새로 정의하지 않습니다. [런타임 아키텍처](runtime-architecture.md), [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md), [API Errors](api/errors.md), [보안 참조](security.md), [Agent 통합](agent-integration.md)의 내부 엔지니어링 점검/default reference local-only MCP posture를 그대로 따릅니다. 초기 recipe는 cooperative/detective 수준의 표현을 기본으로 삼아야 하며, preventive와 isolated option은 구체적인 profile이 승격되고 증명되기 전까지 향후 또는 profile별 범위로 남습니다. Recipe는 접점별 local transport, config snippet, setup hint, access-control material class, hook, wrapper, isolation option을 이름 붙일 수 있지만 recipe text만으로 보장 수준을 올리지 않습니다. Agent Integration capability profile과 conformance가 해당 surface가 cooperative, detective, preventive, isolated 중 무엇인지 결정합니다. Recipe는 raw token, secret, private configuration value를 노출하면 안 되며, surface name만으로 remote, shared, non-loopback, forwarded, tunneled MCP exposure가 암시되지는 않습니다. MCP/Core에 닿을 수 없으면 `MCP_UNAVAILABLE` 또는 `MCP_SERVER_UNAVAILABLE` 같은 operations diagnostic으로 처리합니다. 도달 가능한 local caller 또는 access mode가 registered profile 밖이면 표시해도 안전한 진단 세부 정보를 포함한 `LOCAL_ACCESS_MISMATCH`를 사용합니다. Recognized surface/profile이 required capability를 충족할 수 없으면 `CAPABILITY_INSUFFICIENT`를 사용하고, project, Task, surface, Run, actor claim mismatch에는 일반 state-conflict/scope/capability check를 사용합니다. MCP server에 닿을 수 없으면 해당 call path에서 authoritative Core response가 없으며, cooperative recipe는 write를 지시에 따라 보류하고, preventive 표현은 fixture로 입증된 guard가 해당 operation을 cover할 때만 허용되며, isolated 표현은 connector profile이 exact separation boundary를 이름 붙이고 증명한 경우에만 허용됩니다. Surface별 `UNAUTHORIZED` public code를 도입하지 않습니다. 현재 reference API는 분류 가능한 local-access/profile mismatch에 `LOCAL_ACCESS_MISMATCH`를 사용하며, 그 경우에 별도 `UNAUTHORIZED` code를 추가하지 않습니다.
...
- 캡처·가드·격리 선택지
- 접점별 맥락 전략: 항상 주입되는 운영 맥락을 현재 Task 요약, 작업 모양, 범위/하지 않을 일, 대기 중인 사용자 판단, 활성 blocker, 다음 안전한 행동, 근거 gap, close blocker, 잔여 위험 요약, guarantee level, source refs/freshness로 제한하는 방법과 계획/구체화, 쓰기 준비, 실행/Run 기록, 근거 검토, 닫기 준비 상태, 사용자 판단 요청, 복구/오류 프로필 사이를 전환하는 방법, 그리고 [Agent 통합: Context Push/Pull Principles](agent-integration.md#context-pushpull-principles)에 따라 각 프로필에서 불러올 최소 소유자 섹션
