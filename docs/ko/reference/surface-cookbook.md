...
- 캡처·가드·격리 선택지
- 접점별 맥락 전략: 항상 주입되는 운영 맥락을 역할, 현재 단계와 맥락 프로필, 현재 상태 요약, 활성 blocker, 대기 중인 사용자 소유 결정, 다음 허용 행동으로 제한하는 방법과 세션 시작, 요구사항 구체화 (Discovery), 사용자 결정 요청, 쓰기 준비, 실행/근거, 닫기 준비 상태, 복구/오류 프로필 사이를 전환하는 방법, 그리고 [Agent 통합: Context Push/Pull Principles](agent-integration.md#context-pushpull-principles)에 따라 각 프로필에서 불러올 최소 소유자 섹션
