...
| `ENG-CHECK-prepare-write-in-scope-allowed` | `harness.prepare_write` | Compatible in-scope product-write request가 `decision=allowed`와 primary error 없음으로 반환되고, Task, scope/Change Unit, intended operation, basis state version에 묶인 durable active Write Authorization 하나를 만듭니다. |
...
| 2 | `ENG-CHECK-prepare-write-in-scope-allowed` | `harness.prepare_write` | Active Task, compatible scope, 필요한 경우 compatible baseline, compatible surface guarantee, unresolved required judgment 없음 | `decision=allowed`; Durable Write Authorization 하나가 compatible Task, scope/Change Unit, intended operation, `basis_state_version`, `status=active`, `consumed_by_run_id=null`로 생성됨 | Stable event로 promoted된 경우에만 `prepare_write_allowed`, `write_authorization_created` | None | Projection requirement 없음. Owner path가 이미 projection을 invalidate할 때만 `TASK` stale/enqueued 허용 | None |
