...
Harness conformance material의 세 층을 구분해 볼 때 이 참조 문서를 사용합니다. 세 층은 문서 점검, 활성 structured fixture draft, 향후 runtime conformance입니다. 이 문서는 향후 conformance가 무엇을 증명하는지, 활성 Kernel Smoke, MVP-1 user-loop, artifact/evidence draft family, canonical active fixture value rule, exact structured fixture draft shape, future runner execution behavior, fixture assertion semantics, 현재 단계 상태, 향후 fixture catalog와의 경계를 설명합니다.
...
- 정확한 fixture body field, canonical active value boundary, `request.payload` public-schema rule, 향후 runner isolation behavior가 필요할 때.
...
- 활성 Kernel Smoke, MVP-1 사용자 작업 루프, artifact/evidence fixture draft, 또는 이 draft와 향후 fixture catalog 사이의 경계가 필요할 때.
...
이 draft family는 내부 엔지니어링 점검과 MVP-1을 위한 활성 향후 작성 target입니다. 아직 실행 가능한 fixture가 아니며 generated runtime artifact도 아니고 현재 pass/fail 기준도 아닙니다. 아래 structured draft body는 활성 `scenario_id`, 증명 의도, public request owner, 예상 Core/storage effect, owner link를 보존합니다. 향후 구현에서 명시적으로 materialize하기 전까지는 문서 초안입니다.
...
활성 초안은 하나의 같은 shape를 사용합니다. Review하기에 너무 커지지 않게 유지하되, 향후 materialized fixture가 확장하고 검증해야 할 public request field, active storage row, public error code, blocker category를 이름 붙입니다. Evidence summary family는 두 request path를 둡니다. Insufficient 상태는 read/close-visible state이고, sufficient 상태는 committed active `record_run`으로 만들어지기 때문입니다.
...
purpose: Shaping update가 active state에 persist됨.
initial_state:
  project_state:
    project_id: PROJ-001
    state_version: 2
...
purpose: prepare_write allowed 결과가 Write Authorization을 만듦.
initial_state:
  project_state:
    project_id: PROJ-001
    active_task_id: TASK-001
...
purpose: prepare_write blocked 결과가 Write Authorization을 만들지 않음.
initial_state:
  project_state:
    project_id: PROJ-001
    active_task_id: TASK-001
...
purpose: Idempotent replay가 original committed prepare_write response를 반환함.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: ready
...
purpose: 같은 idempotency key와 다른 canonical request hash가 conflict를 반환함.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: ready
...
purpose: record_run이 compatible Write Authorization을 소비함.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: ready
...
purpose: record_run이 authorization 없는 product write를 거부함.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: ready
...
purpose: Authorized scope 밖에서 관찰된 attempt가 거부됨.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: ready
...
purpose: Raw secret artifact storage가 mutation 전에 차단됨.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: executing
...
purpose: Evidence summary insufficient 상태가 active blocker로 계속 보임.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: blocked
...
expected_errors: []
forbidden_side_effects:
  - Status prose, Markdown evidence text, projection output, or agent summary does not repair missing evidence refs, create evidence, create artifacts, create final acceptance, accept residual risk, or close the Task.
schema_owners:
...
purpose: Evidence summary sufficient 상태가 active Run과 artifact ref로 뒷받침됨.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: executing
...
purpose: Final acceptance 누락이 close blocker가 됨.
initial_state:
  tasks:
    - task_id: TASK-001
      mode: work
...
purpose: 보이는 residual risk가 accepted되지 않으면 close blocker가 됨.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: executing
...
purpose: Accepted-risk close는 active residual-risk acceptance state에서만 성공함.
initial_state:
  tasks:
    - task_id: TASK-001
      mode: work
...
expected_errors: []
forbidden_side_effects:
  - Accepted risk does not create Approval, final acceptance, non-active risk/assurance records, detached verification, or an assurance upgrade beyond active `assurance_level` values.
schema_owners:
  api: docs/*/reference/api/mvp-api.md#harnessclose_task
  schema: docs/*/reference/api/schema-core.md#acceptedriskinput
  core: docs/*/reference/core-model.md#close_task
...
purpose: Display label이 canonical state가 아님을 증명하는 judgment fixture.
initial_state:
  tasks:
    - task_id: TASK-001
      lifecycle_phase: ready
...
향후 fixture file과 suite catalog는 fixture body 밖에 metadata를 가질 수 있습니다. Fixture body 자체는 위 field만 사용해야 향후 conformance runner가 behavior를 일관되게 비교할 수 있습니다. `purpose`는 fixture가 제한하는 동작을 설명하고, `schema_owners`는 public request shape, schema value, Core transition, storage row, error를 검증할 때 쓰는 active owner docs를 이름 붙입니다. 둘 다 public MCP request field가 아니며 Core에 전달되지 않습니다. Suite delivery stage, assertion mode, docs-maintenance result, prose status, rendered Markdown, authoring note를 표현하기 위해 fixture body field를 추가하지 않습니다. 그런 정보는 suite catalog metadata, docs-maintenance report, display owner, 주변 문서에 둡니다.
...
향후 runner 순서 요약: 위 번호 목록이 계약 요약입니다. 향후 runner는 exact fixture body를 읽고 fixture-only runtime home을 seed한 뒤 Core를 통해 request를 실행하고, response/state/storage/events/artifacts/blockers/errors/forbidden side effects를 비교해 report를 냅니다.
...
| 5 | `MVP-ACTIVE-prepare-write-idempotent-replay` | `harness.prepare_write` replay | Existing committed replay row, original stored `request_hash`, original active authorization | Original response, original stored `request_hash`, original `write_authorization_ref`, `authorization_effect=returned` 반환. | None | Duplicate authorization, event, artifact, replay update, non-active effect, state-version increment 없음. |
| 6 | `MVP-ACTIVE-idempotency-key-hash-conflict` | 같은 idempotency key와 다른 hash를 쓰는 state-changing tool | Existing committed replay row | `STATE_CONFLICT`; original replay row와 stored `request_hash` unchanged. | `STATE_CONFLICT` | Merged response, new authorization, event, artifact, non-active effect, owner relation, replay row update 없음. |
...
| 15 | `MVP-ACTIVE-accepted-risk-close` | `harness.close_task` (`intent=complete`, `requested_close_reason=completed_with_risk_accepted`) | Sufficient evidence, visible risk, compatible `judgment_kind=residual_risk_acceptance`가 있는 Task | Task가 `tasks.lifecycle_phase=completed`, accepted-risk close reason, user judgment ref로 close됨. | None | Accepted risk가 Approval, final acceptance, non-active assurance state, assurance upgrade를 만들지 않음. |
