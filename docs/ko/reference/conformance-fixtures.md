...
|---|---|---|---|
| 내부 엔지니어링 점검 fixtures, 작성 label은 커널 스모크(Kernel Smoke) | 내부 엔지니어링 점검 | Minimal 권한 루프만 증명합니다. Project/Task setup, registered reference `capability_profile` 하나, active Change Unit/scope boundary, `dry_run=false`인 in-scope `prepare_write.decision=allowed`, Harness 권한 상태에 따른 out-of-scope write block, durable single-use Write Authorization, compatible `record_run` consumption/linking, missing artifact/evidence blocker/status 또는 좁은 close blocker, non-mutating status read가 포함됩니다. | MVP-1 사용자 가치, natural-language intake, profile별 user judgment 품질, full Evidence Manifest, projection renderer support, 여러 projection kind, residual-risk acceptance semantics, final-acceptance semantics, 수동 QA, 분리 검증, export/recover, release handoff, 전체 conformance suite, 향후 fixture catalog, broad connector ecosystem, hosted connector registry, cross-surface orchestration, higher guard guarantee, broad operations. |
| MVP-1 사용자 작업 루프 fixtures | MVP-1 사용자 작업 루프 | 평소 요청이 Harness vocabulary 없이 tracked work가 되고, clarification quality, judgment separation, evidence blocker, 잔여 위험 표시, 같은 reference `capability_profile`에서 나오는 정직한 authority/fallback behavior, derived-summary non-authority, 작은 design-quality blocking set이 Core-owned state와 structured response로 보임을 증명합니다. | Full agency assurance hardening, detached verification independence, full 수동 QA matrix, stewardship policy suite, full TDD/module/interface/domain-language catalog, full feedback-loop audit, export/recover, release handoff, broad connector ecosystem, hosted connector registry, cross-surface orchestration, MVP-1 사용자 가치 경로 밖의 automation. |
...
| `ENG-CHECK-reference-surface-profile-honest` | `harness.status`, `harness://project/current`, 또는 owner profile read | `surface_id=reference-local-mcp`인 registered reference `capability_profile` 하나가 `mcp_available=true`, `cooperative_prepare_write_supported=true`, `changed_path_detection_supported=true`, `artifact_capture_supported=false`, `manual_artifact_attachment_supported=true`, `command_observation_supported=false`, `secret_access_observation_supported=false`, `pre_tool_blocking_supported=false`, `isolation_supported=false`, `max_guarantee_level=detective`, `conformance_smoke_status=planned_not_run` 또는 equivalent planned/not-run status를 보고합니다. 이 profile은 Write Authorization을 만들거나 Core gate를 대체하지 않으며 `preventive`, `isolated`, native artifact capture, command observation, secret-access observation을 주장하지 않습니다. Unsupported field는 표시되는 guarantee를 낮추거나, 필요할 때 `CAPABILITY_INSUFFICIENT` / structured blocked reason을 만듭니다. |
...
| `ENG-CHECK-task-scoped-stale-state-version` | state-changing task-scoped tool | Stale `expected_state_version`은 Core-resolved primary Task의 `tasks.state_version`과 비교됩니다. Response는 `STATE_CONFLICT`이며 conflicting new attempt에 대해 current record, event, artifact, projection job, replay row가 생기지 않습니다. |
| `ENG-CHECK-project-scoped-stale-state-version` | primary Task가 없는 state-changing project-scoped tool | Stale `expected_state_version`은 `project_state.state_version`과 비교됩니다. Response는 `STATE_CONFLICT`이며 conflicting new attempt에 대해 Task row, project current record, event, artifact, projection job, replay row가 생기지 않습니다. |
| `ENG-CHECK-idempotency-same-key-same-hash-replay` | committed state-changing tool replay | 같은 `idempotency_key`와 같은 canonical `request_hash`를 다시 사용하면 original committed response를 반환합니다. Freshness check를 다시 실행하거나 event append, artifact 등록, projection enqueue, replay row update를 하지 않습니다. |
| `ENG-CHECK-idempotency-same-key-different-hash-conflict` | committed state-changing tool replay | 같은 `idempotency_key`를 다른 canonical `request_hash`와 함께 다시 사용하면 `STATE_CONFLICT`를 반환합니다. Original replay row를 보존하고 새 artifact, event, projection job, response field, owner relation을 old result에 merge하지 않습니다. |
| `ENG-CHECK-dry-run-idempotency-non-consumption` | `dry_run=true`인 state-changing tool | Dry-run은 diagnostic 또는 would-change effect를 반환할 수 있지만 current record, `task_events` row, artifact, consumable Write Authorization, projection job, `tool_invocations` replay row를 만들지 않고, `idempotency_key`를 예약하거나 사용된 것으로 처리하지 않습니다. |
| `ENG-CHECK-prepare-write-replay-no-duplicate-authorization` | `harness.prepare_write` replay | 같은 committed allowed `prepare_write`를 replay하면 original response와 original `write_authorization_ref`를 반환합니다. Durable authorization은 정확히 하나만 존재하며 duplicate authorization, event, projection job, state-version increment가 기록되지 않습니다. |
| `ENG-CHECK-record-run-replay-no-double-consumption` | `harness.record_run` replay | 같은 committed `record_run`을 replay하면 original response를 반환합니다. Linked Write Authorization은 한 번만 consumed로 남고 두 번째 consumption, Run, artifact registration, event, projection job, state-version increment가 기록되지 않습니다. |
...
| `MVP1-product-technical-scope-judgments-separated` | 제품 판단, 기술 판단, 범위 판단을 별도 user-owned judgment request 또는 candidate로 표현하고, 민감 동작 승인, QA 면제 판단, 검증 위험 수락, 최종 수락, 잔여 위험 수락, 취소 판단과 구분합니다. |
...
| `MVP1-unresolved-user-judgment-blocks-write-or-close` | 관련 `product_decision`, `technical_decision`, `scope_decision`, `sensitive_approval`, `qa_waiver`, `verification_risk_acceptance`, `final_acceptance`, `residual_risk_acceptance`, `cancellation` 판단이 missing, pending, rejected, blocked, stale, incompatible이면 affected write 또는 close가 structured Core/API result로 막히고 `required_judgment_kind`와 action이 필요한 judgment ref를 보여줍니다. |
| `MVP1-missing-evidence-blocks-close-when-required` | Evidence가 required이고 `evidence_summary.status`가 `none`, `partial`, `stale`, `blocked`이면 close는 applicable하게 `category=evidence` 또는 `artifact_availability`로 막힙니다. Report prose, status-card text, Markdown은 gap을 충족하지 않습니다. |
| `MVP1-artifact-integrity-metadata-required-for-sufficient-evidence` | Critical artifact evidence는 supporting `ArtifactRef`가 current owner relation, `sha256`, `size_bytes`, `content_type`, `redaction_state`, `produced_by`, `retention_class`, availability metadata를 가져야 `evidence_summary.status=sufficient`를 만들 수 있습니다. Missing metadata, missing bytes, unresolved owner link, raw secret/full sensitive-log storage, `hash_mismatch`는 affected evidence를 `stale` 또는 `blocked`로 표시합니다. Required evidence가 affected이면 close는 blocked입니다. |
| `MVP1-residual-risk-visible-before-final-acceptance-risk-close` | Known close-relevant residual risk는 successful final acceptance 또는 close 전에 보여야 합니다. Hidden 또는 stale risk는 `residual_risk_visibility`를 통해 해당 route를 막습니다. |
| `MVP1-accepted-risk-close-requires-explicit-risk-acceptance` | `completed_with_risk_accepted`는 risk가 visible이고 compatible `judgment_kind=residual_risk_acceptance` user judgment가 관련 blocker/evidence ref와 함께 수락을 기록할 때만 성공합니다. `judgment_kind=final_acceptance`나 broad "go ahead" text는 이를 충족하지 않습니다. |
| `MVP1-ambiguous-go-ahead-does-not-resolve-route` | "yes, do it", "go ahead", "looks good", "좋아", "진행해" 같은 ambiguous consent phrase는 ambiguous user-judgment route를 해소하거나, QA를 면제하거나, 검증 위험을 수락하거나, 잔여 위험을 수락하거나, 취소 판단을 기록하거나, 범위 밖 작업을 허가하지 않습니다. |
| `MVP1-mcp-core-unavailable-does-not-fabricate-authority` | MCP/Core가 unavailable이면 surface는 권한 상태를 읽거나 바꿀 수 없다고 보고하고 Task state, Write Authorization, evidence, close readiness, 민감 동작 승인, 최종 수락을 만들어내지 않습니다. |
| `MVP1-unsupported-surface-lowers-or-blocks-claim` | Requested write, evidence claim, guarantee display가 unsupported reference `capability_profile` field에 의존하면 result는 표시되는 guarantee를 낮추거나 `CAPABILITY_INSUFFICIENT` / structured blocked reason을 반환합니다. Product write는 조용히 진행되지 않으며 unsupported native capture, command observation, secret-access observation, pre-tool blocking, isolation을 available로 취급하지 않습니다. |
| `MVP1-projection-template-output-not-state` | Projection, template, status-card, Markdown output은 derived 상태로 남습니다. 이를 읽거나 편집해도 state 생성, gate 충족, write authorize, evidence attach, 최종 수락 기록, 잔여 위험 수락, Task close가 일어나지 않습니다. |
...
| `MVP1-design-quality-blocking-set-limited` | Design-quality finding은 Autonomy Boundary exceeded, unresolved user judgment, missing active scope, missing required evidence, stale context affecting write/close, surface capability insufficient for a claimed guarantee일 때만 기본적으로 write 또는 close를 차단합니다. Full domain-language, module/interface, TDD, stewardship, feedback-loop, Manual QA, detached-verification catalog finding은 active owner path가 승격하지 않는 한 candidate/advisory로 라우팅합니다. |
| `MVP1-policy-finding-one-next-action` | 각 design-quality finding은 정확히 하나의 routed action을 가집니다. Action은 block write, block close, ask one focused user judgment, request evidence, mark residual risk, show advisory next action, no action 중 하나입니다. Blocker 하나는 next action 하나를 만들며 무한 review loop를 만들지 않습니다. |
...
| `CLARIFY-product-and-technical-decisions-separated` | 사용자가 소유해야 하는 판단의 종류가 다르면 product decision과 material technical architecture decision을 분리합니다. |
...
|---|---|---|---|---|---|---|---|---|
| 1 | `ENG-CHECK-project-task-scope-setup` | Owner setup path 또는 validated seed path | Registered local project, 또는 setup action이 등록하는 경우 비어 있는 fixture-only runtime home | Local project 하나, active Task 하나, active Change Unit 또는 scoped work boundary 하나가 존재함. Setup만으로 Write Authorization이나 product-write Run이 생기지 않음 | Owner-promoted setup event만 | None | Projection requirement 없음 | None |
| 1a | `ENG-CHECK-reference-surface-profile-honest` | `harness.status`, `harness://project/current`, 또는 owner profile read | Registered local project와 `surface_id=reference-local-mcp`인 reference `capability_profile` | Profile field가 active reference surface contract와 일치함. `conformance_smoke_status`는 planned/not run이지 passed가 아님. Unsupported field는 guarantee display를 낮추거나 required claim을 막음. Profile은 Write Authorization을 만들거나 Core gate를 대체하지 않음 | None | None | Projection requirement 없음 | Scenario가 unsupported required capability를 요청할 때만 `CAPABILITY_INSUFFICIENT` |
