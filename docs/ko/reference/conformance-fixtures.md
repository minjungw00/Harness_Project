...
- Projection output은 freshness, source-state-version 표시, readability, availability를 확인할 수 있지만 renderer output이 Core state를 대체하거나, evidence를 충족하거나, write를 authorize하거나, close/accept/risk acceptance를 수행하거나, conformance truth의 source가 되면 안 됩니다.
...
향후 runtime conformance는 fixture 기반입니다. Scenario table만으로는 충분하지 않습니다. 구체화된 각 test fixture는 action을 실행하고 state, events, artifacts, projections, errors를 검증해야 합니다.
...
향후 `harness conformance run`은 MCP tool과 operator command가 사용하는 것과 같은 Core entrypoint를 통해 fixture를 실행합니다. 동작을 prose output만 검사해서 검증하면 안 됩니다. Core entrypoint를 실행하고 그 결과의 state, events, artifacts, projection, error를 비교해야 합니다.
...
이 queue는 코어 권한 조각(v0.1 Core Authority Slice) fixture candidate를 처음 작성할 때의 authoring order입니다. 커널 스모크(Kernel Smoke)는 제품 MVP가 아니라 이 내부 실행 목표에 대응하는 좁은 conformance authoring profile입니다. 이 row들은 향후 작성 지침이며 executable fixture file이 이미 존재한다고 암시하지 않습니다. 이 순서는 fixture author를 위한 것이며 executable fixture 사이의 dependency가 아닙니다. 향후 각 executable fixture는 isolated하게 유지하고, 필요한 최소 owner record를 `initial_state`에 seed하며, 하나의 public Core 또는 operator action을 사용하고, fixture body shape는 그대로 유지해야 합니다.
...
템플릿 구현 계층과 최신성 동작은 [문서 Projection](document-projection.md#템플릿-구현-계층) 및 [Freshness and failure rules](document-projection.md#freshness-and-failure-rules)가 담당합니다. Kernel Smoke는 target behavior를 증명할 때 projection requirement 없음, projection freshness, enqueue/failure fact를 검증할 수 있지만, projection-template polish나 detailed report template을 요구하면 안 됩니다.
...
| 2 | `CORE-project-registration-reference-surface` | `connect`, project registration, 또는 owner registration action | 비어 있거나 등록되지 않은 isolated runtime home과 임시 Product Repository | Project 하나와 reference surface 하나가 honest guarantee level로 idempotent하게 등록됨. Registration만으로 active Task는 생기지 않음 | Owner stable event catalog가 정의할 때만 registration event | 없음 | Projection requirement 없음 | 없음 |
| 3 | `CORE-task-one-active-record` | `harness.intake`, task creation owner path, 또는 validated seed path | 등록된 project와 reference surface. Active Task 없음 | Lifecycle phase, state version, minimal gate/status state를 가진 active Task 하나가 생김. Task creation만으로 Write Authorization이 생기지 않음 | Owner stable event catalog가 promote하지 않는 ordinary create에는 없음 | 없음 | Projection requirement 없음. Owner path가 이미 projection을 invalidate할 때만 `TASK` enqueue 허용 | 없음 |
...
| 9 | `CORE-record-run-consumes-authorization-registers-artifact-evidence` | `harness.record_run`의 `kind=direct` 또는 `kind=implementation` | Active Task, active scope, compatible unconsumed Write Authorization, baseline, staged artifact 또는 evidence input, support가 필요한 completion condition 하나 | Run committed. Write Authorization이 한 번 consumed됨. Artifact/evidence ref가 Run 또는 evidence relation에 link됨. Minimal evidence state가 supported, partial, insufficient support를 기록함 | `run_recorded`, `write_authorization_consumed`, evidence/artifact event는 owner catalog가 promote할 때만 | Integrity/redaction metadata와 same-Task owner link를 가진 registered `ArtifactRef` 또는 owner evidence ref | Projection requirement 없음. State가 바뀌면 `TASK` stale/enqueued 허용 | 없음 |
...
향후 suite family는 위 검증 프로파일 아래에 묶습니다.
