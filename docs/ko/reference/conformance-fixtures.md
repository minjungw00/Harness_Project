...
- 작은 v0.1 Core Authority Slice fixture set, v0.2 User-Facing Harness MVP fixture set, clarification-quality group, 또는 이들과 향후 fixture catalog 사이의 경계가 필요할 때.
...
| 작은 단계별 fixture set | [v0.1 Core Authority Slice Fixture Set](#v01-core-authority-slice-fixture-set), [v0.2 User-Facing Harness MVP Fixture Set](#v02-user-facing-harness-mvp-fixture-set), [Clarification Quality Fixture Group](#clarification-quality-fixture-group) |
...
Clarification-quality fixture는 Harness가 사용자 판단을 대신하지 않으면서 필요한 판단을 잘 요청한다는 점을 증명할 때 user-facing MVP path에 속합니다. 더 깊은 policy-specific Decision Packet coverage는 v0.2 path에 minimal blocker가 필요한 경우를 제외하고 v0.3에 남습니다.
...
이 queue는 [v0.1 Core Authority Slice Fixture Set](#v01-core-authority-slice-fixture-set)을 위한 향후 작성 지침입니다. 커널 스모크(Kernel Smoke)는 첫 내부 권한 루프를 위한 좁은 작성 label이지 제품 MVP, full conformance suite, 향후 fixture catalog가 아닙니다. 이 row들은 executable fixture file이 이미 존재한다고 암시하지 않습니다. Compact authoring order일 뿐이며, 첫 구현 계획은 Build가 이름 붙인 하나의 authority loop를 증명하는 가장 작은 subset만 구체화할 수 있습니다.
