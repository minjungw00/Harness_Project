...
향후 Harness Server runtime test를 위한 핵심 적합성 모델을 찾아볼 때 이 참조 문서를 사용합니다. 여기에는 conformance가 무엇을 증명하는지, 작은 내부 엔지니어링 점검 / MVP-1 fixture set, exact fixture body shape, runner execution behavior, fixture assertion semantics, 현재 단계 상태, 향후 fixture catalog와의 경계가 포함됩니다.
...
- 작은 내부 엔지니어링 점검 fixture set, MVP-1 사용자 작업 루프 fixture set, clarification-quality group, 또는 이들과 향후 fixture catalog 사이의 경계가 필요할 때.
...
Conformance에는 두 conceptual layer가 있습니다. 이 파일의 핵심 적합성 모델 및 작은 전달 fixture set과, detailed later scenario를 담는 [향후 Fixture Catalog](future-fixture-catalog.md)입니다. 핵심 모델은 내부 엔지니어링 점검 Kernel Smoke와 MVP-1 사용자 가치를 설명할 만큼 작게 유지하여 later catalog coverage가 early implementation requirement처럼 보이지 않게 합니다.
...
- 검증 프로파일별 증명 동작, 축소된 내부 엔지니어링 점검 / MVP-1 fixture set, 축소된 Kernel Smoke 작성 순서
...
| 작은 전달 fixture set | [내부 엔지니어링 점검 Fixture Set](#v01-core-authority-smoke-fixture-set), [MVP-1 사용자 작업 루프 Fixture Set](#v02-first-user-value-slice-fixture-set), [Clarification Quality Fixture Group](#clarification-quality-fixture-group) |
...
| 운영 프로필 / promoted-expansion fixtures | 운영 프로필 및 로드맵 | Export/recover, artifact integrity, release handoff, operator readiness, reconcile, broader conformance coverage, 승격된 future higher guarantee level 또는 automation profile을 증명합니다. | Owner 문서가 mechanism을 정의하고 fixture가 covered behavior를 증명하기 전의 stronger security, isolation, preventive guard, browser-capture, remote/shared MCP, automation claim. |

## 작은 전달 Fixture Set

아래 fixture set은 향후 executable fixture를 위한 문서상 specification target입니다. 초기 conformance가 Harness의 차별점, 즉 local authority state, user-owned judgment routing, evidence와 risk visibility, 정직한 guarantee wording에 집중하도록 의도적으로 짧고 testable하게 둡니다. 현재 fixture file은 없으며, 내부 엔지니어링 점검 또는 MVP-1을 통과하기 위해 넓은 future catalog가 필요하지도 않습니다.

<a id="v01-core-authority-smoke-fixture-set"></a>

### 내부 엔지니어링 점검 Fixture Set

내부 엔지니어링 점검 fixture는 첫 local authority loop만 증명합니다. 각 candidate는 Core-owned state, stable owner event가 있을 때의 event, 관련 artifact ref, structured error 또는 blocker를 assert해야 합니다. Projection assertion은 기본적으로 요구하지 않습니다.
...
### MVP-1 사용자 작업 루프 Fixture Set

MVP-1 fixture는 broad assurance 또는 operations catalog로 커지지 않고 사용자에게 보이는 Harness 가치를 증명합니다. Candidate는 해당 stage에서 active한 경우 `harness.intake`, `harness.status`, `harness.next`, `harness.request_user_judgment`, `harness.record_user_judgment`, `harness.prepare_write`, `harness.record_run`, `harness.close_task`를 사용할 수 있습니다.
...
Clarification-quality fixture는 Harness가 사용자 판단을 대신하지 않으면서 필요한 판단을 잘 요청한다는 점을 증명할 때 MVP-1 사용자 작업 루프 path에 속합니다. 더 깊은 policy-specific Decision Packet coverage는 MVP-1 path에 minimal blocker가 필요한 경우를 제외하고 보증 프로필에 남습니다.
...
Fixture body type notation은 API의 [Schema notation convention](mcp-api-and-schemas.md#schema-notation-convention)을 따릅니다. 위 top-level fixture body field는 모두 required입니다. Fixture가 empty object, object map, array를 의도적으로 제공할 때는 `{}` 또는 `[]`를 사용합니다. Required top-level field를 생략하는 것은 invalid fixture body이며 "not asserted"가 아닙니다. 내부 엔지니어링 점검 check에서 `expected_projection`은 `{}`이거나 explicit no-requirement assertion일 수 있습니다. Projection rendering은 내부 엔지니어링 점검 exit criterion이 아니기 때문입니다.
...
Fixture shorthand는 두 번째 API가 아닙니다. Main 내부 엔지니어링 점검 / MVP-1 path에서는 shorthand가 `initial_state` seeding 또는 suite catalog metadata를 간단히 표현할 수 있을 뿐이며 owner-defined record와 public schema를 보존해야 합니다. Public mutation은 `ToolEnvelope` expansion 이후에도 선택된 `action`에 대한 documented public request branch를 `input` 아래에서 사용해야 합니다. Later-profile shorthand detail은 [향후 Fixture Catalog: Later-Profile Fixture Shorthand Notes](future-fixture-catalog.md#later-profile-fixture-shorthand-notes)에 두며 내부 엔지니어링 점검 또는 MVP-1의 active requirement가 아닙니다.
...
이 queue는 [내부 엔지니어링 점검 Fixture Set](#v01-core-authority-smoke-fixture-set)을 위한 향후 작성 지침입니다. 커널 스모크(Kernel Smoke)는 첫 내부 권한 루프를 위한 좁은 작성 label이지 제품 MVP, 전체 conformance suite, 향후 fixture catalog가 아닙니다. 이 row들은 executable fixture file이 이미 존재한다고 암시하지 않습니다. Compact authoring order일 뿐이며, 첫 구현 계획은 Build가 이름 붙인 하나의 authority loop를 증명하는 가장 작은 subset만 구체화할 수 있습니다.
...
그 catalog entry는 promoted owner path가 exact-shape executable fixture로 구체화하기 전까지 design inventory일 뿐입니다. 내부 엔지니어링 점검 requirement가 아니며, 그 자체로 MVP-1을 확장하지 않고, 이 저장소가 문서 전용인 동안 runtime conformance로 계산하지 않습니다.
