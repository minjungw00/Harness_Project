...
| 작은 MVP 동작 예시 | [내부 엔지니어링 점검 동작 예시](#v01-core-authority-smoke-fixture-set), [MVP-1 사용자 작업 루프 동작 예시](#v02-first-user-value-slice-fixture-set), [Clarification Quality 동작 예시](#clarification-quality-fixture-group) |
...
아래 동작 예시는 active MVP path를 위한 설계 target입니다. 향후 conformance가 Harness의 차별점, 즉 local authority state, user-owned judgment routing, evidence와 risk visibility, 정직한 guarantee wording에 집중하도록 의도적으로 짧고 testable하게 둡니다. 현재 실행 가능한 fixture가 아님. Generated runtime artifact도 아니며 현재 pass/fail 기준도 아닙니다. 내부 엔지니어링 점검 또는 MVP-1을 만족하기 위해 넓은 future catalog가 필요하지도 않습니다.

<a id="v01-core-authority-smoke-fixture-set"></a>

### 내부 엔지니어링 점검 동작 예시

내부 엔지니어링 점검 동작 예시는 첫 local authority loop만 설명합니다. 향후 owner가 이를 fixture로 구체화하면 각 fixture는 Core-owned state, stable owner event가 있을 때의 event, 관련 artifact ref, structured error 또는 blocker를 assert해야 합니다. Projection assertion은 기본적으로 요구하지 않습니다.
...
MVP-1 동작 예시는 broad assurance 또는 operations catalog로 커지지 않고 사용자에게 보이는 Harness 가치를 설명합니다. 향후 fixture가 이 예시를 구체화하면 해당 stage에서 active한 경우 `harness.intake`, `status.next_actions`를 포함한 `harness.status`, `harness.request_user_judgment`, `harness.record_user_judgment`, `harness.prepare_write`, `harness.record_run`, `harness.close_task`를 사용할 수 있습니다. 별도 `harness.next` fixture는 later/compatibility material에 속합니다.
...
Fixture 형태 요약: suite metadata는 fixture를 묶을 수 있지만, fixture body는 향후 실행 가능한 conformance를 위한 하나의 정확한 action-and-expectation shape를 유지합니다.
...
MVP 동작 예시, fixture 예시, queue는 향후 작성 계획입니다. 문서 수락, 별도의 구현 계획 준비 결정, Harness Server 구현, 명시적인 fixture materialization step이 있은 뒤에야 실행 가능한 상태가 됩니다. 문서 점검은 Markdown drift를 보고할 수 있지만 runtime conformance가 아니며 Core fixture result를 만들지 않습니다.
...
이 queue는 [내부 엔지니어링 점검 동작 예시](#v01-core-authority-smoke-fixture-set)를 위한 향후 작성 지침입니다. 커널 스모크(Kernel Smoke)는 첫 내부 권한 루프를 위한 좁은 작성 label이지 제품 MVP, 전체 conformance suite, 향후 fixture catalog가 아닙니다. 이 row들은 실행 가능한 fixture file이 이미 존재한다고 암시하지 않습니다. Compact authoring order일 뿐이며, 첫 구현 계획은 Build가 이름 붙인 하나의 authority loop를 증명하는 가장 작은 subset만 구체화할 수 있습니다.
