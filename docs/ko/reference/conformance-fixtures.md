...
|---|---|---|
| `ENG-CHECK-project-task-scope-setup` | owner setup path, `harness.intake`, 또는 validated seed path | Core-owned state 안에 local project 하나, active Task 하나, scoped work boundary 하나가 존재합니다. Setup만으로 Write Authorization이나 product-write Run이 생기지 않습니다. |
| `ENG-CHECK-prepare-write-in-scope-allowed` | `harness.prepare_write` | Compatible in-scope product-write request가 primary error 없이 반환되고, Task, scope/Change Unit, intended operation, basis state version에 묶인 durable Write Authorization 하나를 만듭니다. |
...
| `ENG-CHECK-write-authorization-single-use` | `harness.record_run` | 첫 compatible product-write Run은 unconsumed authorization을 consume할 수 있습니다. 같은 authorization을 쓰는 두 번째 distinct Run은 `WRITE_AUTHORIZATION_CONSUMED`, `WRITE_AUTHORIZATION_INVALID`, 또는 owner-equivalent error로 막히며 두 번째 consumption은 기록되지 않습니다. |
