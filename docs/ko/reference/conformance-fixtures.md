...
| `MVP-v02-product-ux-and-architecture-judgments-separated` | 제품/UX 판단과 기술 판단을 별도 user-owned judgment request 또는 candidate로 표현하고, 민감 동작 승인, 작업 수락, 잔여 위험 수용과 구분합니다. |
...
| `MVP-v02-ambiguous-feature-enters-clarification` | Ambiguous feature request는 premature implementation이나 broad approval이 아니라 clarification 또는 user judgment routing으로 들어갑니다. |
...
Clarification-quality fixture는 Harness가 사용자 판단을 대신하지 않으면서 필요한 판단을 잘 요청한다는 점을 증명할 때 MVP-1 사용자 작업 루프 path에 속합니다. 더 깊은 policy-specific user judgment coverage는 MVP-1 path에 minimal blocker가 필요한 경우를 제외하고 보증 프로필에 남습니다.
