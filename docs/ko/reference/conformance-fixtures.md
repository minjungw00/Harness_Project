...
| `CORE-v01-prepare-write-out-of-scope-blocked` | `harness.prepare_write` | Out-of-scope intended write는 Harness 권한 상태가 structured blocker 또는 `SCOPE_VIOLATION` equivalent primary error로 거부합니다. Write Authorization, Run, artifact, projection job, state-authorizing side effect가 생기지 않습니다. |
...
| `MVP-v02-small-typo-direct-change-stays-light` | 작은 typo 또는 direct change는 가벼운 procedural budget을 유지하되 scope, product write에 필요한 쓰기 전 범위 확인 / Write Authorization, evidence/self-check support, 관련 user-owned judgment를 보존합니다. |
...
| `MVP-v02-mcp-core-unavailable-does-not-fabricate-authority` | MCP/Core가 unavailable이면 surface는 권한 상태를 읽거나 바꿀 수 없다고 보고하고 Task state, Write Authorization, evidence, close readiness, approval, acceptance를 만들어내지 않습니다. |
...
| 3 | `CORE-v01-prepare-write-out-of-scope-blocked` | `harness.prepare_write` | Requested path/tool/operation을 제외하는 scoped boundary가 있는 active Task | Out-of-scope intended write가 Harness 권한 상태에 의해 blocked됨. Write Authorization, Run, artifact, state-authorizing side effect가 생기지 않음 | Owner catalog가 promote하지 않는 한 pre-commit rejection에는 stable event 없음 | None | Pre-commit rejection에는 projection job 없음 | `SCOPE_VIOLATION` 또는 owner-equivalent structured blocker/error |
