...
- Active `expected_storage_rows`는 Storage가 소유한 active record 영역만 사용할 수 있습니다. 범위는 `project_state`, `surfaces` 또는 같은 역할의 reference-surface registration record, `tasks`, `task_events`, `change_units`, `user_judgments`, `write_authorizations`, `runs`, `artifacts`, `artifact_links`, `evidence_summaries` 또는 같은 역할의 minimal evidence coverage record, `blockers`, `tool_invocations`입니다. `shared_designs`, Discovery Brief row, Question Queue row, Assumption Register row, First Safe Change Unit Candidate row, committed `approvals`, `manual_qa_records`, `evidence_manifests`, `evals`, durable `projection_jobs` 또는 projection cache, export/recover table, validator-run archive, rich residual-risk table, close-readiness/status-card cache, 그 밖의 later/profile table family를 나열하면 안 됩니다.
- Requirements-shaping fixture assertion은 shaping output을 active owner row에만 저장합니다. 해당 row는 필요에 따라 `tasks`, `change_units`, committed judgment request가 있을 때의 `user_judgments`, Core가 blocker를 commit할 때의 `blockers`, minimal evidence coverage effect가 있을 때의 `evidence_summaries`, 그리고 committed shaping run을 위한 `runs.kind=shaping_update`입니다. Shaping fixture가 Shared Design storage, Discovery Brief, Question Queue, Assumption Register, 별도 First Safe Change Unit Candidate record를 요구하면 안 됩니다.
- `expected_storage_rows.write_authorizations`는 active `AuthorizedAttemptScope`를 `attempt_scope_json` 아래 보존해야 합니다. 포함되는 값은 `task_id`, `change_unit_id`, `basis_state_version`, `surface_id`, intended operation, intended paths/tools/commands와 command classes, product-file-write intent, intended network targets, intended secret handles/scope, sensitive categories, `baseline_ref`, related user judgment refs, `guarantee_level`입니다. Committed non-dry-run `prepare_write.decision=allowed` fixture는 `request.payload`의 proposed attempt-scope field, `expected_response.write_authorization.attempt_scope`, `expected_storage_rows.write_authorizations.attempt_scope_json`이 같은 resolved `AuthorizedAttemptScope`를 가리킨다고 assert해야 합니다. Request나 증명 claim이 command, tool, network, secret, baseline, sensitive category, surface, guarantee fact를 포함하는데 path만 assert하면 안 됩니다. Committed non-dry-run `decision=allowed`만 `write_authorizations.status=active`를 만듭니다. `blocked`, `approval_required`, `decision_required`, `state_conflict`는 response/blocker/error outcome이지 authorization row가 아닙니다.
...
- `expected_storage_rows.user_judgments`는 active user-judgment schema의 `judgment_kind`, `presentation`, `status`, owner ref, payload JSON을 사용해야 합니다. `display_label`과 localized label은 rendered display text일 뿐입니다. Storage column, canonical row identity, validator input이나 key, state-compatibility assertion, blocker key, gate key, compatibility input, close aggregation key로 나타나면 안 됩니다.
...
- `forbidden_side_effects`는 documentation draft에서는 읽기 쉬운 문장일 수 있습니다. 하지만 materialized executable fixture에서는 가능한 곳마다 owner-record absence, row effect, artifact, event, projection, generated-output assertion으로 확장해야 합니다. Failed operation에서는 `expected_storage_rows`와 `forbidden_side_effects`가 서로 맞아야 합니다. Fixture가 Run, artifact, replay row, evidence mutation, authorization consumption, projection job, later/profile record를 금지하면서 동시에 그 row나 effect를 기대하면 안 됩니다. Absence 자체가 증명 대상이면 `expected_storage_rows` table-effect assertion과 compatible exact-mode metadata 또는 명시적인 negative side-effect assertion을 사용합니다.
...
| `MVP-ACTIVE-prepare-write-allowed-authorization` | `harness.prepare_write` / `PrepareWriteRequest`. | Public schema가 정의한 `product_file_write_intended`, `intended_paths`, `intended_tools`, `intended_commands`, `intended_network`, `intended_secret_scope`, `sensitive_categories`, `baseline_ref`를 사용합니다. 이 request field가 `AuthorizedAttemptScope`의 proposed 부분입니다. `dry_run`과 `expected_state_version`은 `envelope` 안에 있습니다. | `decision=allowed`, `authorization_effect=created`, `tasks.lifecycle_phase=ready`, active `write_authorizations.status=active`, 같은 resolved `AuthorizedAttemptScope`를 담은 `WriteAuthorizationSummary.attempt_scope`와 `write_authorizations.attempt_scope_json`, committed non-dry-run response에만 replay row, OS permission/sandbox/preventive/isolated/evidence/close claim 없음. |
| `MVP-ACTIVE-prepare-write-blocked-no-authorization` | `harness.prepare_write` / `PrepareWriteRequest`. | Allowed case와 같은 public request shape를 사용합니다. Incompatible path, missing active scope, active Change Unit 없음은 public field와 active Core state로 표현합니다. | `SCOPE_REQUIRED`, `NO_ACTIVE_CHANGE_UNIT`, `SCOPE_VIOLATION` 같은 structured blocker/error. Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. `write_authorization_ref=null`, `write_authorization=null`이며 consumable Write Authorization row/Run/artifact/pre-commit failure replay row/projection job 없음. |
| `MVP-ACTIVE-prepare-write-idempotent-replay` | `harness.prepare_write` / `PrepareWriteRequest`. | 반복 call은 같은 public request payload, `envelope.idempotency_key`, canonical request hash를 사용합니다. `canonical_request_hash`를 `request.payload`에 넣지 않습니다. Hash는 `tool_invocations` 아래의 stored/captured assertion입니다. | Original committed response, original stored `request_hash`, original `write_authorization_ref`를 `authorization_effect=returned`로 반환. Duplicate authorization/event/artifact/replay update/projection job/state-version increment 없음. |
| `MVP-ACTIVE-idempotency-key-hash-conflict` | Active state-changing public tool. 보통 `harness.prepare_write`입니다. | Conflicting call은 같은 `envelope.idempotency_key`와 다른 canonical public payload를 사용하므로 canonical request hash가 다릅니다. Stored/observed hash fact는 public request field가 아니라 storage/comparison field에서 assert합니다. | Primary `STATE_CONFLICT`, original replay row와 stored `request_hash` 보존. Merged response field, event, artifact, projection job, owner relation, new authorization, replay row update 없음. |
| `MVP-ACTIVE-record-run-consumes-authorization` | `harness.record_run` / `RecordRunRequest`. | `kind=implementation`, `payload.kind=implementation`을 사용하고 `payload.implementation`만 non-null branch입니다. `observed_changes.changed_paths[]`는 bare path string이 아니라 `ChangedPath` object입니다. Observed path, command, tool, network, secret, sensitive category, `baseline_ref`, Task, Change Unit, surface, guarantee fact는 active surface가 관찰하거나 attest할 수 있는 범위에서 stored `AuthorizedAttemptScope`와 compatible해야 합니다. | Compatible Run 하나가 기록되고 `runs.observed_attempt_json` / active payload comparison이 compatible로 표시됩니다. Active Write Authorization은 정확히 한 번 consumed됩니다. Task execution assertion은 `tasks.lifecycle_phase=executing`을 사용합니다. Registered evidence/artifact ref는 active schema를 사용하며 final acceptance, residual-risk acceptance, later/profile 보증 상태, close state를 만들지 않음. |
...
| `MVP-ACTIVE-record-run-observed-out-of-scope` | `harness.record_run` / `RecordRunRequest`. | `kind=implementation`, `payload.kind=implementation`을 사용하고 `payload.implementation`만 non-null branch입니다. Observed path, command, network, secret, sensitive category, baseline, Task, Change Unit, surface는 active `ImplementationPayload` observation field를 사용하고 stored `AuthorizedAttemptScope`를 의도적으로 넘거나 다릅니다. | Primary `SCOPE_VIOLATION`. Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. Authorization은 success로 consumed되지 않습니다. Fixture가 owner-defined violation/audit 예외를 명시적으로 고르지 않는 한 Run/artifact/evidence/replay mutation이 없음을 assert합니다. 그 예외를 고르더라도 completion evidence나 close readiness가 아닙니다. |
...
| `MVP-ACTIVE-evidence-summary-sufficient` | `harness.record_run` / `RecordRunRequest`. | `kind=implementation`, `payload.kind=implementation`을 사용하고 `payload.implementation`만 non-null branch입니다. Product-write evidence이면 observed attempt와 맞는 stored `AuthorizedAttemptScope`를 가진 compatible active Write Authorization을 seed합니다. Artifact evidence는 `input_id`, `source_kind`, `kind`, `redaction_state`, `produced_by`, `retention_class`, `relation`을 가진 active `ArtifactInput`을 사용합니다. `artifact_inputs`에 bare `staged_uri` entry를 넣지 않습니다. | Registered `ArtifactRef` value와 `artifact_links`가 active `evidence_summaries.status=sufficient` update를 뒷받침함. Task는 close 전까지 `tasks.lifecycle_phase=executing`으로 남습니다. Full Evidence Manifest, later/profile 보증 상태, final acceptance, residual-risk acceptance 없음. |
...
| `MVP-ACTIVE-display-label-not-canonical` | `harness.request_user_judgment` / `RequestUserJudgmentRequest`. | `judgment_kind=product_decision`, `presentation=short`, 전체 `RequestUserJudgmentRequest` field를 담은 committed `dry_run=false` public request를 사용합니다. 렌더링된 `user_visible_summary`에는 locale에서 파생한 문구가 들어갈 수 있지만, fixture state와 assertion key는 rendered label이 아니라 `request.payload.judgment_kind`와 `expected_response.user_judgment.judgment_kind`를 사용합니다. | Pending `expected_storage_rows.user_judgments` row 하나가 `judgment_kind=product_decision`, `presentation=short`, `status=pending_user`를 가짐. `expected_response.user_judgment`도 같은 canonical value를 가짐. Task lifecycle은 `tasks.lifecycle_phase=waiting_user`를 사용합니다. `display_label`은 request payload, `expected_response.user_judgment`, expected storage row, validator/state-compatibility assertion, blocker 또는 gate key, close aggregation에 없음. Request만으로 decision이 resolved되지 않음. |
...
Fixture body type notation은 API의 [Schema notation convention](api/schema-core.md#schema-notation-convention)을 따릅니다. 위 top-level fixture body field는 모두 required입니다. Fixture가 empty object, object map, array를 의도적으로 제공할 때는 `{}` 또는 `[]`를 사용합니다. Required top-level field를 생략하는 것은 invalid fixture body이며 "not asserted"가 아닙니다. 내부 엔지니어링 점검과 MVP-1 active draft에서는 projection rendering이 보통 없고, active `expected_storage_rows`가 `projection_jobs`를 요구하면 안 됩니다. 나중에 승격된 owner가 projection freshness를 요구하면, 그 promoted later/profile fixture가 rendered Markdown matching이 아니라 `expected_state_changes.checks`, `expected_storage_rows.projection_jobs`, 또는 owner가 정의한 structured location에 Core/storage fact를 assert합니다.
...
| 5 | `MVP-ACTIVE-prepare-write-idempotent-replay` | `harness.prepare_write` replay | Existing committed replay row, original stored `request_hash`, original active authorization | Original response, original stored `request_hash`, original `write_authorization_ref`, `authorization_effect=returned` 반환. | None | Duplicate authorization, event, artifact, replay update, projection job, state-version increment 없음. |
| 6 | `MVP-ACTIVE-idempotency-key-hash-conflict` | 같은 idempotency key와 다른 hash를 쓰는 state-changing tool | Existing committed replay row | `STATE_CONFLICT`; original replay row와 stored `request_hash` unchanged. | `STATE_CONFLICT` | Merged response, new authorization, event, artifact, projection job, owner relation, replay row update 없음. |
| 7 | `MVP-ACTIVE-record-run-consumes-authorization` | `kind=implementation`, `payload.kind=implementation`, `payload.implementation`만 non-null인 `harness.record_run` | Task `tasks.lifecycle_phase=ready`, compatible scope, observed attempt와 맞는 stored `AuthorizedAttemptScope`를 가진 active Write Authorization | Compatible `observed_attempt_json`이 있는 Run 하나가 기록되고 authorization이 정확히 한 번 consumed됨. Task execution assertion은 `tasks.lifecycle_phase=executing`을 사용합니다. | None | Second consumption, final acceptance, residual-risk acceptance, later/profile 보증 상태, close 없음. |
...
| 10 | `MVP-ACTIVE-record-run-observed-out-of-scope` | `kind=implementation`, `payload.kind=implementation`, `payload.implementation`만 non-null인 `harness.record_run` | Stored `AuthorizedAttemptScope`가 observed path, command, network target, secret, sensitive category, baseline, Task, Change Unit, 또는 surface를 제외하는 active Write Authorization | Out-of-scope observation은 rejected되거나 owner-defined violation/audit path로만 기록되며 success로 authorization을 consume하지 않음. Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. | `SCOPE_VIOLATION` | Invalid authorization이 consumed되지 않음. 명시적 violation/audit 예외를 고른 경우가 아니면 Run/artifact/evidence/replay mutation 없음. Observation은 completion evidence나 close readiness가 아님. |
...
| 14 | `MVP-ACTIVE-evidence-summary-sufficient` | `kind=implementation`, `payload.kind=implementation`, `payload.implementation`만 non-null이고 active `ArtifactInput`을 쓰는 `harness.record_run` | Task `tasks.lifecycle_phase=executing`, observed attempt와 맞는 stored `AuthorizedAttemptScope`를 가진 compatible authorization, 그리고 redaction이나 omission이 필요하지 않아 `redaction_state=none`으로 허용되는 non-secret staged artifact | Registered artifact refs와 evidence summary가 owner records에서 sufficient가 됨. Task는 close 전까지 `tasks.lifecycle_phase=executing`으로 남습니다. | None | Full Evidence Manifest, later/profile 보증 상태, final acceptance, residual-risk acceptance 없음. |
