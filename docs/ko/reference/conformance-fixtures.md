...
- `expected_state_changes`는 [Core Model 참조](core-model.md), [MVP API](api/mvp-api.md), [API Schema Core](api/schema-core.md)가 정의한 active Core-owned field와 value를 assert해야 합니다. Core owner가 정확히 정의하지 않은 한 `tasks.lifecycle_phase`에 `active` 같은 status word를 쓰면 안 됩니다.
...
|---|---|---|---|
| `MVP-ACTIVE-task-change-unit-setup` | Owner-valid setup path. Public surface로 노출한다면 `harness.intake` / `IntakeRequest`를 사용합니다. | `request.payload`는 `envelope`, `user_request`, `requested_mode`, `resume_policy`, `acceptance_criteria`, `constraints`, `initial_context_refs`를 포함해야 합니다. Scope는 `initial_scope`가 아니라 `constraints.allowed_paths`, `change_units.allowed_paths_json`, 또는 owner-defined scope field 같은 active Task/Change Unit field에 둡니다. | Active Task pointer 하나, active Change Unit 또는 scope boundary 하나, valid `tasks.mode` / `tasks.lifecycle_phase`, 그리고 Write Authorization, Run, artifact, evidence, acceptance, residual-risk acceptance, close, projection-as-authority effect 없음. |
| `MVP-ACTIVE-shaping-update-persists` | `harness.record_run` / `RecordRunRequest`. | `kind=shaping_update`, `payload.kind=shaping_update`를 사용하고 `payload.shaping_update`만 non-null branch입니다. `kind=shaping`이나 top-level `shaping_update`를 `payload` 밖에서 쓰지 않습니다. | Shaping update가 active Task/Change Unit owner field와 valid `runs.kind=shaping_update` row에 persist되고 product-write authority를 만들지 않음. |
| `MVP-ACTIVE-prepare-write-allowed-authorization` | `harness.prepare_write` / `PrepareWriteRequest`. | Public schema가 정의한 `product_file_write_intended`, `intended_paths`, `intended_tools`, `intended_commands`, `intended_network`, `intended_secret_scope`, `sensitive_categories`, `baseline_ref`를 사용합니다. `dry_run`과 `expected_state_version`은 `envelope` 안에 있습니다. | `decision=allowed`, `authorization_effect=created`, active `write_authorizations.status=active`, stored `AuthorizedAttemptScope`, committed non-dry-run response에만 replay row, OS permission/sandbox/preventive/isolated/evidence/close claim 없음. |
| `MVP-ACTIVE-prepare-write-blocked-no-authorization` | `harness.prepare_write` / `PrepareWriteRequest`. | Allowed case와 같은 public request shape를 사용합니다. Incompatible path, missing active scope, active Change Unit 없음은 public field와 active Core state로 표현합니다. | `SCOPE_REQUIRED`, `NO_ACTIVE_CHANGE_UNIT`, `SCOPE_VIOLATION` 같은 structured blocker/error, consumable Write Authorization/Run/artifact/pre-commit failure replay row/projection job 없음. |
...
| 10 | `MVP-ACTIVE-raw-secret-artifact-blocked` | `ArtifactInput`이 있는 `harness.record_run` | Forbidden raw-secret evidence를 시도하는 active Task/Run path와 active `ArtifactInput` shape | Raw secret bytes는 mutation 전에 reject되거나 owner-approved safe blocked/omitted metadata로만 표현됨. | Forbidden input shape/source 또는 raw secret payload before mutation은 `VALIDATION_FAILED`; missing 또는 integrity-failed committed artifact ref에만 `ARTIFACT_MISSING`. | Raw secret storage, rendering, export, evidence sufficiency, authorization consumption, close 없음. |
