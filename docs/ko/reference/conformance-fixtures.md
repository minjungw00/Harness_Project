...
- `expected_blockers`는 [MVP API: harness.close_task](api/mvp-api.md#harnessclose_task), [API Errors: harness.close_task Close Blockers](api/errors.md#harnessclose_task-close-blockers), Core/storage owner가 정의한 active blocker category, `required_judgment_kind`, related ref, close-blocker shape를 사용해야 합니다.
...
| `MVP-ACTIVE-record-run-consumes-authorization` | `harness.record_run` / `RecordRunRequest`. | `kind=implementation`, `payload.kind=implementation`을 사용하고 `payload.implementation`만 non-null branch입니다. `observed_changes.changed_paths[]`는 bare path string이 아니라 `ChangedPath` object입니다. | Compatible Run 하나가 기록되고 active Write Authorization이 정확히 한 번 consumed됨. Task execution assertion은 `tasks.lifecycle_phase=executing`을 사용합니다. Registered evidence/artifact ref는 active schema를 사용하며 final acceptance, residual-risk acceptance, detached verification, close state를 만들지 않음. |
| `MVP-ACTIVE-record-run-missing-authorization-blocked` | `harness.record_run` / `RecordRunRequest`. | Product-write implementation/direct payload는 public branch shape를 사용합니다. Missing authorization을 test할 때는 `write_authorization_id=null`입니다. | Primary `WRITE_AUTHORIZATION_REQUIRED`. Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. Run, artifact link, evidence update, authorization consumption, projection job, event, state-version advance, replay row 없음. |
| `MVP-ACTIVE-record-run-observed-out-of-scope` | `harness.record_run` / `RecordRunRequest`. | Observed path, command, network, secret, sensitive category는 active `ImplementationPayload` / `DirectPayload` observation field를 사용합니다. | Primary `SCOPE_VIOLATION` 또는 owner-equivalent structured blocker. Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. Invalid authorization은 consumed되지 않고 observation은 completion evidence나 close readiness가 아님. |
...
| `MVP-ACTIVE-evidence-summary-sufficient` | `harness.record_run` / `RecordRunRequest`. | Artifact evidence는 `input_id`, `source_kind`, `kind`, `redaction_state`, `produced_by`, `retention_class`, `relation`을 가진 active `ArtifactInput`을 사용합니다. `artifact_inputs`에 bare `staged_uri` entry를 넣지 않습니다. | Registered `ArtifactRef` value와 `artifact_links`가 active `evidence_summaries.status=sufficient` update를 뒷받침함. Task는 close 전까지 `tasks.lifecycle_phase=executing`으로 남습니다. Full Evidence Manifest, Manual QA, detached verification, final acceptance, risk acceptance 없음. |
| `MVP-ACTIVE-final-acceptance-missing-close-blocker` | `harness.close_task` / `CloseTaskRequest`. | `intent=complete`와 active `requested_close_reason`을 사용합니다. 이 blocker case에서는 보통 `completed_self_checked`입니다. `intent=completed_with_risk_accepted`를 쓰지 않습니다. Accepted risk는 close reason이지 intent enum이 아닙니다. | Close가 structured final-acceptance blocker와 primary `ACCEPTANCE_REQUIRED`로 blocked 상태를 유지함. Task lifecycle assertion은 `tasks.lifecycle_phase=blocked`를 사용하며 final_acceptance judgment를 fabricate하지 않음. |
| `MVP-ACTIVE-residual-risk-visible-not-accepted-blocker` | `harness.close_task` / `CloseTaskRequest`. | `intent=complete`를 사용합니다. Residual-risk acceptance는 rich residual-risk record가 아니라 active blocker와 `user_judgments`에서 읽습니다. | `required_judgment_kind=residual_risk_acceptance`가 있는 structured residual-risk-acceptance blocker. Primary `DECISION_REQUIRED` 또는 `DECISION_UNRESOLVED`. Task lifecycle assertion은 `tasks.lifecycle_phase=blocked`를 사용하며 close state나 accepted-risk claim을 fabricate하지 않음. |
| `MVP-ACTIVE-accepted-risk-close` | `harness.close_task` / `CloseTaskRequest`. | `intent=complete`, `requested_close_reason=completed_with_risk_accepted`를 사용합니다. Request가 accepted-risk ref를 운반하지 않습니다. Core가 compatible `judgment_kind=residual_risk_acceptance` state와 blocker ref를 읽습니다. | Task가 `tasks.lifecycle_phase=completed`와 `close_reason=completed_with_risk_accepted`로 close됨. Accepted-risk ref는 active `user_judgment` / `blocker` ref를 가리킴. Detached verification, Manual QA, Approval, final acceptance, rich residual-risk row, assurance upgrade 없음. |
...
| 7 | `MVP-ACTIVE-record-run-consumes-authorization` | `harness.record_run` | Task `tasks.lifecycle_phase=ready`, compatible scope, active compatible Write Authorization | Run 하나가 기록되고 authorization이 정확히 한 번 consumed됨. Task execution assertion은 `tasks.lifecycle_phase=executing`을 사용합니다. | None | Second consumption, final acceptance, residual-risk acceptance, detached verification, close 없음. |
| 8 | `MVP-ACTIVE-record-run-missing-authorization-blocked` | `harness.record_run` | Task `tasks.lifecycle_phase=ready`와 authorization 없는 product-write Run request | Product-write Run이 commit 전에 blocked되고 Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. | `WRITE_AUTHORIZATION_REQUIRED` | Run, consumption, completion evidence, artifact link, projection job, replay row 없음. |
| 9 | `MVP-ACTIVE-record-run-observed-out-of-scope` | `harness.record_run` | Stored scope가 observed path를 제외하는 active compatible Write Authorization | Out-of-scope observation은 blocked되거나 owner violation/audit path로만 기록되며 success로 authorization을 consume하지 않음. Task lifecycle은 `tasks.lifecycle_phase=blocked`를 사용합니다. | `SCOPE_VIOLATION` 또는 owner-equivalent structured blocker | Invalid authorization이 consumed되지 않음. Observation은 completion evidence나 close readiness가 아님. |
| 10 | `MVP-ACTIVE-raw-secret-artifact-blocked` | `ArtifactInput`이 있는 `harness.record_run` | Forbidden raw-secret evidence를 시도하는 Task `tasks.lifecycle_phase=executing`, Run path, active `ArtifactInput` shape | Raw secret bytes는 mutation 전에 reject되거나 owner-approved safe blocked/omitted metadata로만 표현됨. | Forbidden input shape/source 또는 raw secret payload before mutation은 `VALIDATION_FAILED`; missing 또는 integrity-failed committed artifact ref에만 `ARTIFACT_MISSING`. | Raw secret storage, rendering, export, evidence sufficiency, authorization consumption, close 없음. |
| 11 | `MVP-ACTIVE-evidence-summary-insufficient` | `harness.status` 또는 evidence owner read | Partial/missing evidence summary가 있는 Task `tasks.lifecycle_phase=blocked` | Evidence summary가 insufficient/partial로 남고 close-relevant blocker가 structured임. | Close/write path가 의존할 때 `EVIDENCE_INSUFFICIENT` blocker | Status prose 또는 Markdown evidence list가 missing refs를 repair하지 않음. |
| 12 | `MVP-ACTIVE-evidence-summary-sufficient` | `harness.record_run` | Task `tasks.lifecycle_phase=executing`, compatible authorization, visible staged artifact | Registered artifact refs와 evidence summary가 owner records에서 sufficient가 됨. Task는 close 전까지 `tasks.lifecycle_phase=executing`으로 남습니다. | None | Full Evidence Manifest, Manual QA, detached verification, final acceptance, risk acceptance 없음. |
...
| 14 | `MVP-ACTIVE-residual-risk-visible-not-accepted-blocker` | `harness.close_task` | Visible close-relevant residual risk가 있고 compatible acceptance judgment가 없는 Task | Residual-risk acceptance가 계속 required이고 Task lifecycle assertion은 `tasks.lifecycle_phase=blocked`를 사용합니다. | `required_judgment_kind=residual_risk_acceptance`가 있는 `DECISION_REQUIRED` 또는 `DECISION_UNRESOLVED` | Visible risk는 accepted risk가 아님. Rich Residual Risk record, detached verification, close state를 fabricate하지 않음. |
| 15 | `MVP-ACTIVE-accepted-risk-close` | `harness.close_task` | Sufficient evidence, visible risk, compatible `judgment_kind=residual_risk_acceptance`가 있는 Task | Task가 `tasks.lifecycle_phase=completed`, accepted-risk close reason, user judgment ref로 close됨. | None | Accepted risk가 detached verification, Manual QA, Approval, final acceptance, assurance upgrade를 만들지 않음. |
| 16 | `MVP-ACTIVE-display-label-not-canonical` | `harness.request_user_judgment` | Task `tasks.lifecycle_phase=ready`와 Change Unit | Response는 localized display label을 render할 수 있지만 storage와 blocker state는 canonical `judgment_kind`를 사용함. Pending user-owned judgment assertion은 `tasks.lifecycle_phase=waiting_user`를 사용합니다. | None | `display_label`과 localized label은 canonical state, gate key, storage identity, close aggregation key가 아님. |
