...
향후 Harness Server runtime test를 위한 핵심 적합성 모델을 찾아볼 때 이 참조 문서를 사용합니다. 여기에는 conformance가 무엇을 증명하는지, 작은 v0.1/v0.2 fixture set, exact fixture body shape, runner execution behavior, fixture assertion semantics, 현재 단계 상태, 향후 fixture catalog와의 경계가 포함됩니다.
...
- 작은 v0.1 Core Authority Smoke fixture set, v0.2 First User-Value Slice fixture set, clarification-quality group, 또는 이들과 향후 fixture catalog 사이의 경계가 필요할 때.
...
현재 이 문서는 runnable test 모음이 아니라 향후 적합성 검증 계획입니다. 이후 구현 계획에서 쓸 candidate fixture ID와 필요한 동작을 정의할 뿐이며 fixture file, runner code, generated output, runtime state, runnable Harness Server conformance suite를 만들지 않습니다. 문서 전용 단계에서는 이 예시에서 실제 fixture 파일을 만들지 않습니다.

Conformance에는 두 conceptual layer가 있습니다. 이 파일의 핵심 적합성 모델 및 작은 단계별 fixture set과, detailed later scenario를 담는 [향후 Fixture Catalog](future-fixture-catalog.md)입니다. 핵심 모델은 v0.1 Kernel Smoke와 v0.2 사용자 가치를 설명할 만큼 작게 유지하여 later catalog coverage가 early implementation requirement처럼 보이지 않게 합니다.
...
- Projection output은 projection support가 범위에 있을 때 freshness, source-state-version 표시, readability, availability를 확인할 수 있지만 renderer output이 Core state를 대체하거나, evidence를 충족하거나, write를 authorize하거나, close/accept/risk acceptance를 수행하거나, conformance truth의 source가 되면 안 됩니다. v0.1 Core Authority Smoke는 empty 또는 "no projection requirement" field를 넘는 projection assertion을 요구하지 않습니다.
...
- active v0.1/v0.2 path의 fixture shorthand boundary
...
- 검증 프로파일별 증명 동작, 축소된 v0.1/v0.2 fixture set, 축소된 Kernel Smoke 작성 순서
...
| 작은 단계별 fixture set | [v0.1 Core Authority Smoke Fixture Set](#v01-core-authority-smoke-fixture-set), [v0.2 First User-Value Slice Fixture Set](#v02-first-user-value-slice-fixture-set), [Clarification Quality Fixture Group](#clarification-quality-fixture-group) |
...
아래 fixture set은 향후 executable fixture를 위한 문서상 specification target입니다. 초기 conformance가 Harness의 차별점, 즉 local authority state, user-owned judgment routing, evidence와 risk visibility, 정직한 guarantee wording에 집중하도록 의도적으로 짧고 testable하게 둡니다. 현재 fixture file은 없으며, v0.1 또는 v0.2를 통과하기 위해 넓은 future catalog가 필요하지도 않습니다.

### v0.1 Core Authority Smoke Fixture Set

v0.1 fixture는 첫 local authority loop만 증명합니다. 각 candidate는 Core-owned state, stable owner event가 있을 때의 event, 관련 artifact ref, structured error 또는 blocker를 assert해야 합니다. Projection assertion은 기본적으로 요구하지 않습니다.

| Fixture ID | Primary action | Required behavior assertion |
|---|---|---|
| `CORE-v01-project-task-scope-setup` | owner setup path, `harness.intake`, 또는 validated seed path | Core-owned state 안에 local project 하나, active Task 하나, scoped work boundary 하나가 존재합니다. Setup만으로 Write Authorization이나 product-write Run이 생기지 않습니다. |
| `CORE-v01-prepare-write-in-scope-allowed` | `harness.prepare_write` | Compatible in-scope product-write request가 primary error 없이 반환되고, Task, scope/Change Unit, intended operation, basis state version에 묶인 durable Write Authorization 하나를 만듭니다. |
| `CORE-v01-prepare-write-out-of-scope-blocked` | `harness.prepare_write` | Out-of-scope intended write는 Harness authority state가 structured blocker 또는 `SCOPE_VIOLATION` equivalent primary error로 거부합니다. Write Authorization, Run, artifact, projection job, state-authorizing side effect가 생기지 않습니다. |
| `CORE-v01-write-authorization-single-use` | `harness.record_run` | 첫 compatible product-write Run은 unconsumed authorization을 consume할 수 있습니다. 같은 authorization을 쓰는 두 번째 distinct Run은 `WRITE_AUTHORIZATION_CONSUMED`, `WRITE_AUTHORIZATION_INVALID`, 또는 owner-equivalent error로 막히며 두 번째 consumption은 기록되지 않습니다. |
| `CORE-v01-record-run-consumes-and-links-authorization` | `harness.record_run` | Compatible Run이 observed work를 기록하고 `consumed_by_run_id` 또는 owner-equivalent relation으로 Write Authorization에 연결됩니다. Chat/tool output을 authority로 취급하지 않고 authorization basis를 보존합니다. |
| `CORE-v01-missing-artifact-evidence-ref-blocker` | `harness.status`, narrow `harness.close_task` smoke, 또는 owner blocker read | 필요한 artifact/evidence support가 없으면 `ARTIFACT_MISSING` 또는 `EVIDENCE_INSUFFICIENT` 같은 structured status/blocker state로 보고됩니다. Rendered prose 또는 Markdown은 missing ref를 충족할 수 없습니다. |
| `CORE-v01-status-read-no-mutation` | `harness.status` 또는 `harness.next` read | Status는 current Task, scope, write-authority summary, evidence/artifact support, blockers, state version을 반환하되 event append, artifact 생성, projection enqueue, write authorization, evidence satisfaction, close를 만들지 않습니다. |

### v0.2 First User-Value Slice Fixture Set

v0.2 fixture는 broad assurance 또는 operations catalog로 커지지 않고 사용자에게 보이는 Harness 가치를 증명합니다. Candidate는 해당 stage에서 active한 경우 `harness.intake`, `harness.status`, `harness.next`, `harness.request_user_judgment`, `harness.record_user_judgment`, `harness.prepare_write`, `harness.record_run`, `harness.close_task`를 사용할 수 있습니다.

| Fixture ID | Required behavior assertion |
|---|---|
| `MVP-v02-natural-language-starts-tracked-work` | 평범한 사용자 언어가 "Harness", `Task`, `Change Unit`, `Decision Packet`, 또는 startup phrase를 요구하지 않고 tracked work를 시작하거나 resume합니다. 요청 자체만으로 product write를 authorize하지 않습니다. |
| `MVP-v02-codebase-answerable-facts-checked-before-question` | Current seeded repo/codebase refs, Harness state refs, connector/session facts가 있으면 사용자가 이미 알 수 있는 사실을 반복 설명하게 하기 전에 그것을 사용합니다. 남은 user-owned judgment는 focused question으로 route합니다. |
| `MVP-v02-product-ux-and-architecture-judgments-separated` | Product/UX judgment와 material technical architecture judgment를 별도 user-owned route 또는 candidate로 표현하고, sensitive-action Approval, work acceptance, residual-risk acceptance와 구분합니다. |
| `MVP-v02-small-typo-direct-change-stays-light` | 작은 typo 또는 direct change는 가벼운 procedural budget을 유지하되 scope, product write에 필요한 write authority, evidence/self-check support, 관련 user-owned judgment를 보존합니다. |
| `MVP-v02-ambiguous-feature-enters-clarification` | Ambiguous feature request는 premature implementation이나 broad approval이 아니라 clarification 또는 Decision Packet routing으로 들어갑니다. |
| `MVP-v02-missing-user-judgment-blocks-write-or-close` | 관련 product, UX, architecture, work-acceptance, risk judgment가 없으면 affected write 또는 close가 structured Core/API result로 막힙니다. |
| `MVP-v02-missing-evidence-blocks-close-when-required` | Active profile이 evidence를 요구할 때 evidence 또는 artifact ref가 없으면 report prose가 아니라 structured status/blocker로 close가 막힙니다. |
| `MVP-v02-residual-risk-visible-before-acceptance-risk-close` | Known close-relevant residual risk는 successful work acceptance 또는 risk-accepted close 전에 보여야 합니다. Hidden 또는 stale risk는 해당 route를 막습니다. |
| `MVP-v02-ambiguous-go-ahead-does-not-resolve-route` | "go ahead", "looks good", "좋아", "진행해" 같은 ambiguous consent phrase는 ambiguous user-judgment route를 해소하거나, evidence를 waive하거나, residual risk를 accept하거나, out-of-scope work를 authorize하지 않습니다. |
| `MVP-v02-mcp-core-unavailable-does-not-fabricate-authority` | MCP/Core가 unavailable이면 surface는 authority state를 읽거나 바꿀 수 없다고 보고하고 Task state, Write Authorization, evidence, close readiness, approval, acceptance를 만들어내지 않습니다. |
| `MVP-v02-projection-template-output-not-state` | Projection, template, status-card, Markdown output은 derived 상태로 남습니다. 이를 읽거나 편집해도 state 생성, gate 충족, write authorize, evidence attach, work/risk acceptance, Task close가 일어나지 않습니다. |
| `MVP-v02-detached-verification-not-claimed-unless-recorded` | Active profile이 요구하고 compatible recorded Eval 또는 owner verification path가 있을 때만 detached verification을 주장합니다. Same-session review 또는 prose만으로 assurance를 올리지 않습니다. |

### Clarification Quality Fixture Group

Clarification-quality fixture는 Harness가 사용자 판단을 대신하지 않으면서 필요한 판단을 잘 요청한다는 점을 증명할 때 First User-Value Slice path에 속합니다. 더 깊은 policy-specific Decision Packet coverage는 v0.2 path에 minimal blocker가 필요한 경우를 제외하고 v0.3에 남습니다.
...
Fixture 형태 요약: suite metadata는 fixture를 묶을 수 있지만, fixture body는 향후 executable conformance를 위한 하나의 정확한 action-and-expectation shape를 유지합니다.
...
Fixture body type notation은 API의 [Schema notation convention](mcp-api-and-schemas.md#schema-notation-convention)을 따릅니다. 위 top-level fixture body field는 모두 required입니다. Fixture가 empty object, object map, array를 의도적으로 제공할 때는 `{}` 또는 `[]`를 사용합니다. Required top-level field를 생략하는 것은 invalid fixture body이며 "not asserted"가 아닙니다. v0.1 Core Authority Smoke check에서 `expected_projection`은 `{}`이거나 explicit no-requirement assertion일 수 있습니다. Projection rendering은 v0.1 exit criterion이 아니기 때문입니다.
...
Fixture shorthand는 두 번째 API가 아닙니다. Main v0.1/v0.2 path에서는 shorthand가 `initial_state` seeding 또는 suite catalog metadata를 간단히 표현할 수 있을 뿐이며 owner-defined record와 public schema를 보존해야 합니다. Public mutation은 `ToolEnvelope` expansion 이후에도 선택된 `action`에 대한 documented public request branch를 `input` 아래에서 사용해야 합니다. Later-profile shorthand detail은 [향후 Fixture Catalog: Later-Profile Fixture Shorthand Notes](future-fixture-catalog.md#later-profile-fixture-shorthand-notes)에 두며 v0.1 또는 v0.2의 active requirement가 아닙니다.
...
Runner 순서 요약: 향후 runner는 exact fixture body를 읽고 isolated runtime을 seed한 뒤 Core를 통해 action을 실행하고, state/events/artifacts/projection/errors를 비교해 report를 냅니다.
...
Conformance runner는 status-like `TEXT` field도 [Storage와 DDL](storage-and-ddl.md#canonical-enum-hardening)의 owner-bound hardening map을 통해 seed하고 검사해야 합니다. Main v0.1/v0.2 path에서 fixture seed loader는 active stage의 seeded record에 실제로 들어가는 owner value만 검증하고, artifact/ref enum assertion은 API [stage-specific active value sets](mcp-api-and-schemas.md#stage-specific-active-value-sets)를 사용합니다. 예를 들면 registry/project surface guarantee, Run kind/status, Write Authorization status/guarantee, 해당 owner path가 active일 때의 Approval status, evidence support가 active일 때의 Evidence Manifest status, risk visibility가 active일 때의 residual-risk visibility/status, projection assertion이 범위에 있을 때의 projection job kind/status, 그리고 해당 owner record를 사용할 때의 current Task 또는 Change Unit status입니다. Later-profile status field는 그 profile이 active가 되기 전까지 promoted owner docs와 future catalog에 남습니다. 유효하지 않은 state recovery를 명시적으로 test하는 scenario가 아닌 한 unknown status value는 계속 invalid입니다. Expected-state status assertion은 prose label이 아니라 captured owner value를 비교합니다.
...
이 queue는 [v0.1 Core Authority Smoke Fixture Set](#v01-core-authority-smoke-fixture-set)을 위한 향후 작성 지침입니다. 커널 스모크(Kernel Smoke)는 첫 내부 권한 루프를 위한 좁은 작성 label이지 제품 MVP, 전체 conformance suite, 향후 fixture catalog가 아닙니다. 이 row들은 executable fixture file이 이미 존재한다고 암시하지 않습니다. Compact authoring order일 뿐이며, 첫 구현 계획은 Build가 이름 붙인 하나의 authority loop를 증명하는 가장 작은 subset만 구체화할 수 있습니다.
...
|---|---|---|---|---|---|---|---|---|
| 1 | `CORE-v01-project-task-scope-setup` | Owner setup path, `harness.intake`, 또는 validated seed path | Registered local project, 또는 setup action이 등록하는 경우 비어 있는 isolated runtime home | Local project 하나, active Task 하나, scoped work boundary 하나가 존재함. Setup만으로 Write Authorization이나 product-write Run이 생기지 않음 | Owner-promoted setup event만 | None | Projection requirement 없음 | None |
| 2 | `CORE-v01-prepare-write-in-scope-allowed` | `harness.prepare_write` | Active Task, compatible scope, 필요한 경우 compatible baseline, compatible surface guarantee, unresolved required judgment 없음 | Durable Write Authorization 하나가 compatible Task, scope/Change Unit, intended operation, `basis_state_version`, status, `consumed_by_run_id=null`로 생성됨 | Stable event로 promoted된 경우에만 `prepare_write_allowed`, `write_authorization_created` | None | Projection requirement 없음. Owner path가 이미 projection을 invalidate할 때만 `TASK` stale/enqueued 허용 | None |
| 3 | `CORE-v01-prepare-write-out-of-scope-blocked` | `harness.prepare_write` | Requested path/tool/operation을 제외하는 scoped boundary가 있는 active Task | Out-of-scope intended write가 Harness authority state에 의해 blocked됨. Write Authorization, Run, artifact, state-authorizing side effect가 생기지 않음 | Owner catalog가 promote하지 않는 한 pre-commit rejection에는 stable event 없음 | None | Pre-commit rejection에는 projection job 없음 | `SCOPE_VIOLATION` 또는 owner-equivalent structured blocker/error |
| 4 | `CORE-v01-write-authorization-single-use` | `harness.record_run` | Active Task, compatible scope, prior Run이 이미 consume한 compatible Write Authorization | Consumed authorization reuse가 blocked됨. Original consumed relation은 유지되고 second Run은 commit되지 않음 | Owner catalog가 promote하지 않는 한 pre-commit reuse rejection에는 stable event 없음 | None | Pre-commit rejection에는 projection job 없음 | `WRITE_AUTHORIZATION_CONSUMED`, `WRITE_AUTHORIZATION_INVALID`, 또는 owner-equivalent error |
| 5 | `CORE-v01-record-run-consumes-and-links-authorization` | `harness.record_run`의 `kind=direct` 또는 `kind=implementation` | Active Task, compatible scope, compatible unconsumed Write Authorization, 필요한 경우 baseline | Run이 한 번 commit되고 observed work를 기록하며 supplied Write Authorization에 consumption을 link함 | Promoted된 경우에만 `run_recorded`, `write_authorization_consumed` | Run이 artifact도 등록할 때만 있음 | Projection requirement 없음. State가 바뀌면 `TASK` stale/enqueued 허용 | None |
| 6 | `CORE-v01-missing-artifact-evidence-ref-blocker` | `harness.status`, narrow `harness.close_task` smoke, 또는 owner blocker read | Current path가 artifact/evidence support를 요구하지만 compatible ref가 없는 active Task | Missing artifact/evidence support가 structured blocker/status로 보임. Report prose, Markdown, tool text는 ref를 충족하지 않음 | Close-task smoke가 owner path이고 stable event가 promoted된 경우에만 `close_blocked` | None | Projection requirement 없음 | `ARTIFACT_MISSING`, `EVIDENCE_INSUFFICIENT`, 또는 owner-equivalent blocker/error |
| 7 | `CORE-v01-status-read-no-mutation` | `harness.status` 또는 `harness.next` read | Current scope, write-authority summary, artifact/evidence summary가 있는 active Task | Read가 current state, blockers, state version을 반환하되 event append, artifact 생성, projection enqueue, write authorization, evidence satisfaction, close를 만들지 않음 | None | None | Read-only status/blocker output은 projection enqueue 없음 | None |

위 queue는 의도적으로 작습니다. v0.1은 전체 conformance suite, broad catalog family coverage, 작업 수락 의미, 수동 QA, 분리 검증, export/recover, reconcile, stewardship, context hygiene, Browser QA Capture, future guarantee-level check를 요구하지 않습니다.
