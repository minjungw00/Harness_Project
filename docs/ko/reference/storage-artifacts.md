...
- 사실이 불완전한 레거시 아티팩트는 `legacy_unknown`으로 남습니다. 마이그레이션은 메타데이터 열 형태가 맞다는 이유만으로 레거시 아티팩트를 `verified`로 표시하면 안 됩니다.
- `legacy_unknown`, `corrupt`, 삭제됨, 없음, 사용 불가, 수정된 아티팩트는 증거 또는 닫기 권한 요구사항을 만족할 수 없습니다.
