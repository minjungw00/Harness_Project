...
그림 역할: 저장 생명주기입니다. 구현자의 "스테이징된 자료는 언제 증거 자격을 가질
수 있는 지속 아티팩트 관계가 되는가?"라는 질문에 답합니다. 실선 화살표는 저장
생명주기 전이 또는 지속 연결 경로를 뜻하고, 점선 화살표는 스테이징만으로는 증거가
되지 않는 제외 경로를 표시합니다. 이 그림은 아티팩트 본문 읽기 점검이나 테이블
배치를 보여 주지 않습니다. 생명주기의 정확성은 이 문서가 담당하고, API 형태와
테이블 개요는 [API 아티팩트 스키마](api/schema-artifacts.md)와
[저장소 기록](storage-records.md)이 담당합니다.
