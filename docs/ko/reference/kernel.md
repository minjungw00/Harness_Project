...
8. Decision Packet은 사용자 소유 판단을 기록하고, Approval은 민감 동작 승인을 기록합니다.
...
3. 어떤 사용자 판단이 아직 진행을 막는가?
   ...
   차단하는 사용자 소유 판단은 Decision Packet state와 aggregate `decision_gate`로 표현합니다. 민감 동작 승인은 Approval state와 `approval_gate`로 따로 표현합니다.
...
| `approve-sensitive-action` | 사용자가 범위가 정해진 민감 동작 승인을 Approval로 부여합니다. | 제품 방향, 기술 방향, correctness proof, 작업 수락, 위험 수용, QA, verification, evidence, Write Authorization. |
...
Approval은 범위가 정해진 민감 동작 승인입니다. Paths, tools, commands, network targets, secret scope, sensitive categories, baseline, expiry, 해당 민감 동작에 대한 user judgment을 cover할 수 있습니다.
