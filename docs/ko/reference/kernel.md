...
Harness는 사용자가 무엇을 판단하는지와 내부에서 무엇을 기록하는지를 분리합니다. 사용자 대상 문서는 사용자가 여러 schema 축을 이해해야 하는 것처럼 노출하면 안 됩니다. 사용자는 다섯 가지 표시 라벨 중 하나를 봅니다.

### 사용자에게 보이는 판단 유형

| 표시 라벨 | 내부 `judgment_type` 예 | 사용자가 소유하는 판단 |
|---|---|---|
| 제품/UX 판단 | `product_choice` | 제품 동작, 문구, interaction, taste, 사용자 가치, release-facing promise. |
| 기술 판단 | `technical_choice` | Public API, module boundary, dependency, migration, compatibility, security/privacy trade-off, QA/verification expectation, scope/autonomy choice, 중요한 implementation direction. |
| 민감 동작 승인 | `sensitive_action_approval` | 이름 붙은 민감 동작 permission. |
| 작업 수락 | `work_acceptance` | Work acceptance가 required일 때 결과를 받아들일지. |
| 잔여 위험 수용 | `residual_risk_acceptance` | 보이는 close-relevant risk를 이번 close에서 받아들일지. |
...
flowchart LR
  Need["사용자 판단 필요"]
  Need --> Product["제품/UX 판단<br/>product_choice"]
  Need --> Technical["기술 판단<br/>technical_choice"]
  Need --> Sensitive["민감 동작 승인<br/>sensitive_action_approval"]
  Need --> Accept["작업 수락<br/>work_acceptance"]
  Need --> Risk["잔여 위험 수용<br/>residual_risk_acceptance"]

  Product --> Judgment["user_judgment"]
  Technical --> Judgment
  Sensitive --> Judgment
  Accept --> Judgment
  Risk --> Judgment
...
- `judgment_type`은 compact 내부 판단 유형을 저장합니다.
...
User Judgment는 사용자 소유 판단을 위한 기준 상태 기록입니다. 기준 record family는 `user_judgment`이고, 질문, `judgment_type`, `presentation`, `display_label`, status, options 또는 selected outcome, affected scope, related refs, 필요한 경우 deferral effect, 그리고 민감 동작 승인, 작업 수락, 잔여 위험 수용에 필요한 context를 기록합니다.
...
Journey Spine은 Task state, Change Unit, Run, User Judgment, Approval, evidence, verification, QA, acceptance state, residual risk, close event, artifact ref, `state.sqlite.task_events`에서 파생되는 continuity입니다. 별도 source of truth가 아닙니다.

### Journey Spine Entry

Journey Spine Entry는 기존 state와 event에서 재구성할 수 없는 continuity annotation이 필요할 때만 쓰는 durable annotation입니다. Owner record를 보완할 뿐 Task, Change Unit, Run, User Judgment, evidence, verification, QA, risk, acceptance, close, artifact state를 대체하지 않습니다.
