...
|---|---|---|
| Approval / 민감 동작 승인 | 정의된 scope와 expiry 안의 sensitive-action permission. | 막연한 동의, 제품 방향, 중요한 기술 방향, correctness proof, 작업 수락, 잔여 위험 수용, QA 면제 판단, 검증 면제 판단, 근거, Write Authorization. |
| Decision Packet | 사용자 소유 제품 판단, 중요한 기술 판단, 면제 판단, 작업 수락, 잔여 위험, reconcile 판단을 위한 기준 경로. | Approval 형태이고 Approval record에 연결된 경우를 제외한 민감 동작 승인, product-write authority, 분리 검증. |
...
| v0.2 User-Facing Harness MVP | 사용자에게 보이는 status는 scope, judgment, 근거, 닫기 준비 상태, required일 때 작업 수락, close-relevant risk가 있을 때 잔여 위험 표시를 보여야 합니다. 테스트가 통과해도 왜 close가 막히는지 설명해야 합니다. |
...
Journey Spine은 Task의 ordered work journey를 상태에서 파생해 이어 주는 continuity model입니다. Task, Change Unit, Run, Decision Packet, Approval, Evidence Manifest, Eval, 수동 QA, Residual Risk, `task_gates.acceptance_gate`, 작업 수락 Decision Packet user-decision state, close events, artifact references, `state.sqlite.task_events`에서 재구성됩니다.
...
Sensitive action이 product trade-off, architecture choice, 중요한 기술 선택, QA 면제 판단, 검증 면제 판단, 검증 위험, 작업 수락, 잔여 위험 수용, public interface commitment 같은 사용자 소유 제품 판단이나 중요한 기술 판단도 포함한다면 Approval record는 sensitive category만 허가할 수 있습니다. 사용자 소유 판단에는 여전히 호환되는 Decision Packet이 필요합니다.
...
- 사용자, caller, release/support 소비자, 문서 독자, 다른 system이 의존할 내용을 바꾸는 public commitment는 product direction, 중요한 technical direction, compatibility, risk, 작업 수락에 영향을 줄 때 사용자 소유 판단입니다. 민감 동작 승인은 그 Decision Packet 경로를 대신할 수 없습니다.
- 민감 동작 승인은 사용자 소유의 제품 판단 또는 중요한 기술 판단, correctness proof, QA, 검증, 작업 수락, 근거, Write Authorization을 대신하지 않습니다.
...
민감 동작 승인과 Decision Packet authority는 분리되어 있습니다. Approval은 defined scope 안의 sensitive categories를 허가하는 민감 동작 승인입니다. 사용자 소유 판단의 기록 source가 아닙니다. Sensitive action이 product trade-off, architecture choice, 중요한 기술 선택, QA 면제 판단, 검증 면제 판단, 검증 위험, 작업 수락, 잔여 위험 수용, public interface commitment 같은 사용자 소유 제품 판단이나 중요한 기술 판단도 포함한다면 Approval은 sensitive category만 허가할 수 있습니다. 사용자 소유 판단에는 여전히 호환되는 Decision Packet이 필요합니다.
...
Current reference model의 작업 수락은 기준 Decision Packet user-decision path, Task의 `acceptance_gate`, `state.sqlite.task_events`를 통해 저장됩니다. Kernel은 별도의 Acceptance state record를 정의하지 않습니다. 닫기에 영향을 주는 잔여 위험이 있으면 그 위험이 visible한 뒤에만 작업 수락을 기록할 수 있지만, 결과를 수락했다고 해서 해당 위험이 수용된 것은 아닙니다. 잔여 위험 수용 decision은 위험을 이름 붙이고 accepted Residual Risk refs를 기록해야 합니다.
