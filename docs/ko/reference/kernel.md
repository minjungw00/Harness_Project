...
8. Decision Packet은 사용자 소유 판단을 기록합니다. Minimum v0.2에서는 Approval 형태 Decision Packet이 민감 동작 승인을 기록할 수 있고, later Approval record는 더 hardened된 committed lifecycle을 추가합니다.
...
3. 어떤 사용자 판단이 아직 진행을 막는가?
   ...
   차단하는 사용자 소유 판단은 Decision Packet state와 aggregate `decision_gate`로 표현합니다. 민감 동작 승인은 `approval_gate`로 따로 표현합니다. Minimum v0.2에서는 Approval 형태 Decision Packet에서, later profile에서는 committed Approval state에서 gate를 계산할 수 있습니다.
...
|---|---|---|
| `choose` | 사용자가 제품, 기술, 보안/개인정보, 범위/자율성 option 중 선택합니다. | 민감 동작 permission, 쓰기 권한, 작업 수락, waiver, risk acceptance. |
| `defer` | 사용자가 판단을 의도적으로 미룹니다. 진행, close, risk, follow-up 영향을 기록해야 합니다. | 해결, waiver, 작업 수락, blocker 숨김. |
| `approve-sensitive-action` | 사용자가 Approval 형태 Decision Packet으로 범위가 정해진 민감 동작 permission을 부여합니다. Later Approval profile은 committed Approval record도 만들 수 있습니다. | 제품 방향, 기술 방향, correctness proof, 작업 수락, 위험 수용, QA, verification, evidence, Write Authorization. |
| `waive` | 허용된 경우 이름 붙은 requirement를 면제합니다. | 생략한 QA/check/verification 자체, assurance upgrade, generic consent. |
| `accept-result` | 작업 수락이 required일 때, close basis가 보인 뒤 사용자가 결과를 받아들입니다. | 근거, QA, verification, 민감 동작 permission, waiver, risk acceptance, 새 write authority. |
| `accept-risk` | 사용자가 requested close를 위해 이름 붙은 보이는 Residual Risk를 받아들입니다. | 위험 없는 close, 분리 검증, QA pass, evidence sufficiency, 작업 수락, 민감 동작 permission. |
...
flowchart LR
  ...
  Need --> Approve["approve-sensitive-action<br/>민감 동작 permission만"]
...
Decision Packet을 resolve하면 사용자 소유 판단을 기록합니다. `judgment_route=approve-sensitive-action`과 compatible `approval_scope`를 가진 packet일 때만 민감 동작 permission을 만듭니다. Later Approval profile은 linked Approval path를 추가로 요구할 수 있습니다. Write Authorization, evidence, close는 만들지 않습니다.
...
Approval은 범위가 정해진 민감 동작 permission입니다. Minimum v0.2에서는 `judgment_route=approve-sensitive-action`과 `judgment_payload.approval_scope`를 가진 resolved Approval 형태 Decision Packet으로 표현할 수 있습니다. Later Approval/Agency Assurance profile에서는 committed Approval record로도 표현할 수 있습니다. Paths, tools, commands, network targets, secret scope, sensitive categories, baseline, expiry, 해당 민감 동작에 대한 user judgment을 cover할 수 있습니다.
...
`approval_gate`는 sensitive categories가 있을 때만 적용됩니다. 이 gate는 민감 동작 permission이 필요한지, pending인지, granted인지, denied인지, expired인지 요약합니다. Minimum v0.2에서 `granted`는 compatible resolved Approval 형태 Decision Packet이 sensitive scope를 cover한다는 뜻입니다. Later Approval profile에서는 committed Approval record에서 파생될 수 있습니다. Write Authorization, product judgment, evidence, verification, QA, work acceptance, residual-risk acceptance가 아닙니다.
...
이 reference는 full state machine table을 중복하지 않습니다. Invariant는 write-capable transition이 `prepare_write`와 `record_run`을 통과하고, user-owned judgment가 Decision Packet으로, 필요한 민감 동작 permission이 Approval 형태 route로 처리되며, completion이 `close_task`를 통과한다는 것입니다.
...
| Sensitive-action approval은 별도입니다. | Approval 형태 Decision Packet, later Approval record, `approval_gate`는 sensitive permission만 cover합니다. |
...
| 민감 동작 승인을 product/UX 또는 technical judgment로 사용 | Reject하거나 Decision Packet으로 repair합니다. |
