...
필요한 gate는 active stage와 profile이 정합니다. 이 문서에 field나 gate가 있다는 사실만으로 v0.1, v0.2, 또는 작은 직접 변경에 full future behavior가 required가 되지는 않습니다.
...
| 사용자 판단 category와 route | [판단 경로 경계](#판단-경로-경계), [Decision Packet](#decision-packet), [Decision Gate](#decision-gate) | Public request field는 [`harness.request_user_judgment`](mcp-api-and-schemas.md#harnessrequest_user_judgment)이 담당합니다. |
...
8. Decision Packet은 사용자 소유 판단을 기록하고, Approval은 민감 동작 허가를 기록합니다.
...
2. 지금 허용된 작업은 무엇인가?
   ...
   허용된 작업은 active Task, 작업 모양, active Change Unit, scope, Autonomy Boundary, baseline freshness, 민감 동작 Approval, 사용자 소유 판단, 적용 policy, surface capability, requested operation으로 계산합니다.

3. 어떤 사용자 판단이 아직 진행을 막는가?

   차단하는 사용자 소유 판단은 Decision Packet state와 aggregate `decision_gate`로 표현합니다. 민감 동작 허가는 Approval state와 `approval_gate`로 따로 표현합니다.

4. 이 Task를 close할 수 있는가?

   `close_task`는 close intent를 open Run state, scope, required decision, 민감 동작 Approval, evidence, required verification, required Manual QA, residual-risk visibility, required residual-risk acceptance, required work acceptance, projection freshness, artifact availability와 비교합니다.
...
| 작은 직접 변경 | `direct` | 제품 파일 쓰기는 명시된 범위와 호환되는 `prepare_write` / Write Authorization을 통해서만 가능합니다. 요청이 명확하면 Change Unit은 작을 수 있습니다. 근거도 가벼울 수 있습니다. Decision Packet, 수동 QA, 분리 검증, 작업 수락, 잔여 위험 수용은 의식처럼 만들지 않습니다. Active profile, task type, user request, security/criticality profile, 감지된 위험, 명시 requirement가 요구할 때만 적용합니다. |
...
작은 직접 변경은 작게 유지해야 합니다. 범위가 불분명해지거나, changed path가 active scope를 넘거나, 여러 제품 영역이나 subsystem이 관련되거나, 제품/UX 판단 또는 중요한 기술 구조 판단이 나타나거나, public API나 module contract 영향이 나타나거나, 보안/개인정보 영향이 있거나, 민감 동작이 나타나거나, 근거 기대가 커지거나, QA 또는 verification이 required가 되거나, 잔여 위험이 작지 않거나, 여러 단계 delivery가 필요하면 같은 Task를 추적되는 작업으로 상향합니다.

Tiny direct profile은 `mode=direct` 안의 표시/profile 선택일 뿐입니다. 오탈자, 의미 변경 없는 문서 한 문장, obvious rename에 적합합니다. Scope, write authority, 민감 동작 Approval, 사용자 소유 판단, 실제로 적용되는 evidence requirement, residual-risk visibility, close rule을 우회할 수 없습니다.

## 판단 경로 경계

Harness는 사용자가 무엇을 판단하는지와 내부에서 어떤 route로 기록하는지를 분리합니다. 사용자 대상 문서는 세 개의 독립 축을 사용자가 이해해야 하는 것처럼 노출하면 안 됩니다. 사용자는 category를 보고, route는 맞는 동사를 사용하며, 표시 깊이는 위험도에 맞게 커집니다.

### 사용자에게 보이는 판단 묶음

| Category | 사용자가 소유하는 판단 | Schema category |
|---|---|---|
| 제품/UX | 제품 동작, 문구, interaction, taste, 사용자 가치, release-facing promise. | `judgment_category=product_ux` |
| 기술 구조 | Public API, module boundary, dependency, migration, compatibility, cost, maintainability, 중요한 implementation direction. | `judgment_category=technical_architecture` |
| 보안/개인정보 | Data exposure, secret, auth, permission, retention, logging, redaction, user notice, security/privacy trade-off. | `judgment_category=security_privacy` |
| 범위/자율성 | Scope expansion, non-goal, agent latitude, Change Unit update, Autonomy Boundary update. | `judgment_category=scope_autonomy` |
| QA/검증 | 이름 붙은 QA나 verification path를 요구, 수행, 미룸, 면제할지. | `judgment_category=qa_verification` |
| 작업 수락 | Work acceptance가 required일 때 결과를 받아들일지. | `judgment_category=work_acceptance`와 `judgment_route=accept-result` |
| 잔여 위험 | 보이는 close-relevant risk를 이번 close에서 받아들일지. | `judgment_category=residual_risk` |
| 복합 | 나누면 의미가 사라지는 하나의 cross-cutting judgment. | `judgment_category=mixed` |

### 내부 route

| Route | Kernel 의미 | 이렇게 취급하면 안 되는 것 |
|---|---|---|
| `choose` | 사용자가 제품, 기술, 보안/개인정보, 범위/자율성 option 중 선택합니다. | 민감 동작 Approval, 쓰기 권한, 작업 수락, waiver, risk acceptance. |
| `defer` | 사용자가 판단을 의도적으로 미룹니다. 진행, close, risk, follow-up 영향을 기록해야 합니다. | 해결, waiver, 작업 수락, blocker 숨김. |
| `approve-sensitive-action` | 사용자가 범위가 정해진 민감 동작 허가를 Approval로 부여합니다. | 제품 방향, 기술 방향, correctness proof, 작업 수락, 위험 수용, QA, verification, evidence, Write Authorization. |
| `waive` | 허용된 경우 이름 붙은 requirement를 면제합니다. | 생략한 QA/check/verification 자체, assurance upgrade, generic consent. |
| `accept-result` | 작업 수락이 required일 때, close basis가 보인 뒤 사용자가 결과를 받아들입니다. | 근거, QA, verification, Approval, waiver, risk acceptance, 새 write authority. |
| `accept-risk` | 사용자가 requested close를 위해 이름 붙은 보이는 Residual Risk를 받아들입니다. | 위험 없는 close, 분리 검증, QA pass, evidence sufficiency, 작업 수락, Approval. |
| `reconcile` | Human-editable 또는 generated/projection drift를 accepted state, note, reject, decision request, defer로 처리합니다. | Markdown, report prose, chat에서 곧바로 state mutation. |

이 route 도식은 사용자 소유 판단을 기록하는 설계 계약입니다. 어떤 route 동사를 쓰느냐에 따라 바뀔 수 있는 owner path가 달라지며, 포괄적인 "승인" route는 일부러 두지 않습니다.

```mermaid
flowchart LR
  Need["사용자 판단 필요"]
  Need --> Choose["choose<br/>제품, 기술, 보안, 범위"]
  Need --> Approve["approve-sensitive-action<br/>Approval만"]
  Need --> Waive["waive<br/>이름 붙은 requirement만"]
  Need --> Accept["accept-result<br/>작업 수락"]
  Need --> Risk["accept-risk<br/>보이는 Residual Risk"]
  Need --> Reconcile["reconcile<br/>projection 또는 generated drift"]
  Need --> Defer["defer<br/>영향 기록"]
  ...
  Reconcile --> CorePath["Core 또는 reconcile path"]
```

기록된 뒤에도 route는 서로 분리됩니다. Approval은 제품 방향을 고르지 않고, waiver는 생략한 check를 수행하지 않으며, 작업 수락은 risk acceptance가 아니고, reconcile은 Core 경로 없이 Markdown을 state로 바꾸지 않습니다.

### 표시 깊이

| 표시 깊이 | 쓸 때 | 최소 표시 |
|---|---|---|
| `simple` | 영향이 작은 좁은 unblocker. | 정확한 질문, scope, option 또는 requested outcome, 이 답이 확정하지 않는 것. |
| `tradeoff` | 결과가 중요한 제품/UX 또는 기술 선택. | Options, 가능한 recommendation, uncertainty, defer effect, affected scope와 criteria. |
| `high-risk` | 보안/개인정보, sensitive category, public API, migration, dependency, rollback cost가 큰 선택. | Trade-off에 risk, 가능한 evidence ref, 관련 Approval boundary, rollback/follow-up effect를 더합니다. |
| `close-affecting` | 작업 수락, waiver, residual-risk acceptance, 또는 close에 영향을 주는 decision. | Close basis, blocker, residual-risk visibility, affected gates, required refs, exact close impact. |

Canonical schema field는 이 모델에서 이렇게 쓰입니다.

- `judgment_category`는 사용자에게 보이는 판단 묶음을 저장합니다. Payload branch를 고르거나 gate를 혼자 재계산하거나 route 의미를 대체하지 않습니다.
- `judgment_route`는 internal owner path와 resolution branch를 저장합니다.
- `display_depth`는 선택한 route의 표시/검증 깊이를 저장합니다. Gate, Approval substitute, waiver, 작업 수락, residual-risk acceptance, close rule이 아닙니다.
- `affected_gates`, owner ref, Decision Packet status가 그 판단이 무엇에 영향을 줄 수 있는지 정합니다.

모호한 동의는 좁게 해석합니다. "proceed", "go ahead", "looks good", "좋아", "진행해" 같은 문구는 기본적으로 서로 맞지 않는 route를 해결할 수 없습니다. 하나의 사용자 답변이 여러 route를 만족하려면, 질문이 그 route들을 명시했고 답변이 각 route와 호환되어야 합니다. 기록 payload도 각 route의 scope, consequence, close/write impact를 이름 붙여야 합니다. 그렇지 않으면 Core나 agent가 다시 확인해야 합니다.
...
| Sensitive-action Approval | 제품/UX 판단, 기술 구조 판단, correctness, evidence, QA, verification, 작업 수락, residual-risk acceptance, Write Authorization. |
...
Decision Packet은 사용자 소유 판단을 위한 기준 상태 기록입니다. 질문, category, route, display depth, status, options 또는 selected outcome, affected scope, related refs, 필요한 경우 deferral effect, 그리고 approval, waiver, acceptance, risk acceptance, reconcile에 필요한 route-specific context를 기록합니다.

Decision Packet은 `decision_gate`에 반영됩니다. 차단하는 사용자 소유 판단은 chat text, broad approval, projection prose만으로 충족될 수 없습니다. 기록된 Decision Packet과 그 resolution, deferral, rejection, blocked state, supersession이 그 판단의 기준입니다.
...
Decision Packet을 resolve하면 사용자 소유 판단을 기록합니다. Approval route와 linked Approval path를 따르지 않는 한 sensitive-action Approval을 만들지 않습니다. Write Authorization, evidence, close도 만들지 않습니다.
...
Journey Spine은 Task state, Change Unit, Run, Decision Packet, Approval, evidence, verification, QA, acceptance state, residual risk, close event, artifact ref, `state.sqlite.task_events`에서 파생되는 continuity입니다. 별도 source of truth가 아닙니다.

### Journey Spine Entry

Journey Spine Entry는 기존 state와 event에서 재구성할 수 없는 continuity annotation이 필요할 때만 쓰는 durable annotation입니다. Owner record를 보완할 뿐 Task, Change Unit, Run, Decision Packet, evidence, verification, QA, risk, acceptance, close, artifact state를 대체하지 않습니다.
...
Approval은 범위가 정해진 민감 동작 허가입니다. Paths, tools, commands, network targets, secret scope, sensitive categories, baseline, expiry, 해당 민감 동작에 대한 user judgment을 cover할 수 있습니다.

Approval은 correctness를 증명하지 않습니다. 제품 방향, 기술 구조, evidence, QA, verification, 작업 수락, residual-risk acceptance를 대신하지 않습니다. 그 자체로 product write도 authorize하지 않습니다.

### Write Authorization

Write Authorization은 `prepare_write`가 제품 파일 쓰기를 허용할 때 만드는 durable single-use state record입니다. Task, Change Unit, compatibility basis, intended operation, intended write surface, related approvals and decisions, guarantee level, status, compatible Run에 의한 consumption을 기록합니다.
...
Command, Run, Eval, QA, review, validator, diagnostic에서 나온 finding은 별도 authority path가 아닙니다. Existing owner record를 통해 라우팅될 때만 state에 영향을 줍니다. 예를 들면 Evidence Manifest, Decision Packet, Change Unit, Approval, Eval, Manual QA, Residual Risk, Reconcile Item, structured close blocker, enabled owner path입니다.
...
Residual Risk record는 risk를 보이게 합니다. Work를 verify하거나, evidence를 대체하거나, QA를 waive하거나, Approval을 부여하거나, 작업 수락을 암시하거나, Task를 close하지 않습니다.
...
Reconcile Item은 human-editable 또는 generated/projection drift를 위한 candidate record입니다. Reconcile은 merge, reject, convert to note, Decision Packet create, defer를 할 수 있습니다. Markdown이나 generated text는 accepted reconcile/owner path를 통해서만 state가 됩니다.

### Design Support Records

Shared Design, Domain Term, Module Map Item, Interface Contract, Feedback Loop, TDD Trace는 profile이 켜졌을 때 scope, evidence, design policy를 support할 수 있습니다. Policy detail은 [설계 품질 정책](design-quality-policies.md)이 담당하고, storage shape는 [Storage와 DDL](storage-and-ddl.md)이 담당합니다.
...
- Approval과 Decision Packet authority는 분리됩니다.
...
`decision_gate`는 relevant Decision Packet과 현재 감지된 user-owned judgment need에서 recompute합니다. Precedence는 다음 순서입니다.

1. Relevant judgment가 incompatible, rejected without replacement, expired, blocked이면 `blocked`.
2. Relevant Decision Packet이 user를 기다리면 `pending`.
3. Blocking user-owned judgment가 감지됐지만 compatible Decision Packet이 없으면 `required`.
...
`approval_gate`는 sensitive categories가 있을 때만 적용됩니다. `granted`는 compatible Approval이 sensitive scope를 cover한다는 뜻입니다. Write Authorization, product judgment, evidence, verification, QA, work acceptance, residual-risk acceptance가 아닙니다.
...
`acceptance_gate`는 required일 때 final result acceptance를 기록합니다. Acceptance는 close basis가 보인 뒤에만 기록할 수 있습니다. Close basis에는 evidence status, applicable verification status, applicable Manual QA status, residual-risk visibility 또는 confirmed absence가 포함됩니다.

Residual-risk visibility는 별도입니다. Known close-relevant risk가 없으면 `ResidualRiskSummary.status=none`이 visibility를 만족합니다. Known close-relevant risk가 있으면 work acceptance 또는 successful close 전에 보여야 합니다. Risk-accepted close에는 `accept-risk` route를 통해 accepted Residual Risk refs가 추가로 필요합니다.
...
이 reference는 full state machine table을 중복하지 않습니다. Invariant는 write-capable transition이 `prepare_write`와 `record_run`을 통과하고, user-owned judgment가 Decision Packet과 필요한 Approval로 route되며, completion이 `close_task`를 통과한다는 것입니다.

#### Stable Event Catalog

Stable event name은 append-only state history label입니다. 그 자체가 authority는 아닙니다. Exact payload shape는 Storage와 API 문서가 담당합니다. Event name은 Task lifecycle update, `prepare_write` decision, Write Authorization creation/consumption/staling, Run recording, Decision Packet update, Approval update, gate recompute, evidence update, residual-risk visibility or acceptance, waiver recording, close attempt, close success, projection freshness change, reconcile outcome 같은 state change를 설명해야 합니다.
...
이 write-authority 순서는 Kernel 설계 계약입니다. Scope, required user judgment, sensitive-action Approval은 `prepare_write`의 입력이고, 허용된 `prepare_write`만 한 번 쓰는 Write Authorization을 만듭니다. `record_run`은 실제로 일어난 일을 기록할 뿐, 사후에 권한을 부여하지 않습니다.

```mermaid
flowchart LR
  Scope["범위가 정해진 Task / Change Unit"] --> Prepare["prepare_write"]
  Judgment["필요한 사용자 판단<br/>필요하면 Decision Packet"] --> Prepare
  Approval["sensitive-action Approval<br/>필요할 때"] --> Prepare
  ...
  Record --> Status["상태 또는 막힘"]
  Blocker --> Status
```

## prepare_write

`prepare_write`는 제품 파일 쓰기에 대한 유일한 권한 판단 지점입니다. Approval, Decision Packet resolution, `record_run`, `close_task`, report, projection, agent prose는 input 또는 context가 될 수 있습니다. 하지만 product-file write를 authorize하거나 consumable Write Authorization을 만들지는 않습니다.
...
8. Sensitive categories가 적용되면 sensitive-action Approval을 확인합니다.
9. User-owned judgment를 위한 required Decision Packet을 확인합니다.
...
`blocked`, `approval_required`, `decision_required`, `state_conflict` result는 consumable Write Authorization을 만들면 안 됩니다. `approval_required`는 sensitive-action Approval이 없거나 쓸 수 없다는 뜻입니다. Broad approval이나 product judgment로 바꾸면 안 됩니다. `decision_required`는 user-owned judgment가 필요하다는 뜻입니다. Approval로 바꾸면 안 됩니다.
...
External side effect도 authority 의미가 같습니다. 실행 전에는 `prepare_write`가 intended side effect를 평가합니다. 실행 뒤에는 `record_run`이 실제로 일어난 일을 기록합니다. `record_run`은 compatible scope, Approval, Decision Packet coverage, Write Authorization 없이 일어난 effect를 사후 authorize할 수 없습니다.
...
5. Scope를 확인합니다.
6. Blocking Decision Packet과 `decision_gate`를 확인합니다.
7. Sensitive categories가 적용됐으면 sensitive-action Approval을 확인합니다.
...
12. Residual-risk visibility를 확인합니다. Risk-accepted close가 requested 또는 required이면 accepted Residual Risk refs를 확인합니다.
...
```mermaid
flowchart TD
  Intent["close_task intent"] --> Safe["unsafe open Run 없음"]
  Safe --> Judgments["blocking judgment와 Approval"]
  Judgments --> Evidence["필요할 때 evidence"]
  Evidence --> Verify["필요할 때 verification"]
  Verify --> QA["필요할 때 수동 QA"]
  QA --> Acceptance["필요할 때 작업 수락"]
  Acceptance --> Risk["필요할 때 residual risk 표시와 수용"]
  Risk --> Ready{"required close fact가 모두 compatible?"}
  Ready -->|아니오| Blockers["close blocker 반환"]
  Ready -->|예| Close["close result 기록"]
  Close --> Events["close event 추가"]
```

Structured close blocker는 open Run, scope, user-owned judgment, sensitive-action Approval, design policy, evidence, verification, Manual QA, residual-risk visibility, residual-risk acceptance, work acceptance, projection freshness, artifact availability처럼 close를 막는 category를 이름 붙여야 합니다. Public response가 primary error code 하나를 고르더라도 secondary blocker와 refs는 보여야 합니다.
...
| Residual risk acceptance가 required인 tracked work | Active profile의 다른 required gate가 satisfied됨. Close-relevant residual risk가 visible하고, `accept-risk` route로 accepted Residual Risk refs가 기록됨. | Verification은 not required, passed, 또는 required-verification waiver/risk path가 satisfied된 waived일 수 있습니다. | `result=passed`, `close_reason=completed_with_risk_accepted`, `assurance_level=none` 또는 `self_checked`. `detached_verified`처럼 표시하지 않습니다. |
...
Verification waiver는 detached verification이 아닙니다. Waived verification gap이 close-relevant이면 close에는 visible and accepted Residual Risk refs가 필요하고 `completed_with_risk_accepted`를 사용합니다. QA waiver는 Manual QA pass, verification, work acceptance, assurance upgrade가 아닙니다. Decision deferral은 waiver가 아닙니다.
...
| User-owned judgment는 agent judgment로 대체될 수 없습니다. | Decision Packet과 `decision_gate`는 compatible resolution, deferral, risk handling이 기록될 때까지 affected write 또는 close를 막습니다. |
| Sensitive-action approval은 별도입니다. | Approval record와 `approval_gate`는 sensitive permission만 cover합니다. |
...
| Required sensitive-action Approval 없이 product write attempt | approval-required를 반환합니다. Write Authorization을 만들지 않습니다. |
...
| Approval을 product/UX 또는 technical judgment로 사용 | Reject하거나 Decision Packet으로 repair합니다. |
...
| Active profile 없이 future-profile validator를 current v0.1/v0.2 requirement로 취급 | Later-profile로 qualify하거나 current profile에서 disable합니다. |
