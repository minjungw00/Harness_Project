...
| `MVP-ACTIVE-close-task-state-effecting-dry-run-preview` | `harness.close_task`는 혼합 intent 메서드입니다. 응답 분기 선택은 메서드 이름만이 아니라 선택된 intent의 상태 효과를 따릅니다. `intent=complete`, `intent=cancel`, `intent=supersede`와 `dry_run=true`가 함께 있고 그 외에는 유효하며 미리보기 가능하면 상태 효과가 있는 dry-run 분기를 선택합니다. | 예상 응답 분기는 그런 유효한 상태 효과 dry-run 미리보기에 대한 `effect_kind=no_effect`인 `ToolDryRunResponse`입니다. 미리보기는 예상 닫기 차단 사유, 종료 닫기 효과, 다음 행동을 설명할 수 있지만 닫기 상태 변경, `task_events`, 재실행 행, 아티팩트 생성·갱신·연결, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version` 증가를 만들지 않습니다. |
