...
여러 메서드를 잇거나 끝까지 이어지는 시나리오는 적합성 또는 다른 시나리오 수준 문서에 둡니다. API 메서드 참조 예시에 두지 않습니다. 어떤 적합성 시나리오, 예시, 튜토리얼, 대표 시나리오도 API 메서드 참조 문서가 그 페이로드를 재사용해야 한다는 뜻이 아닙니다. 메서드 참조 예시는 메서드 안에서만 성립하는 최소 예시로 두며, 적용되는 담당 계약과 일관되어야 합니다.
...
아래 `scenario_id`는 작은 참조 기준입니다. 예시, 튜토리얼, 런타임 결과, 구현 계획, API 예시가 반드시 따라야 하는 페이로드가 아닙니다. 정확한 분기, 저장, 접근, 아티팩트, 보안, 닫기 준비 상태 계약은 위 담당 문서 링크를 사용합니다.
