...
- 시나리오 색인의 기대 동작 요약
- 적합성 기준의 주장 권한 경계
- 적합성 기준, 담당 문서, 예시, 튜토리얼, 메서드 안의 API 예시 사이의 관계
...
| 항목 | 경계 | 세부사항 |
|---|---|---|
| 시나리오 의미 | 안정적인 동작 기준 | [세부사항](#scenario-semantics) |
...
정의:
- 적합성 시나리오는 기준 범위 또는 분명한 담당 경계에 속하는 동작 기준 하나에 이름을 붙입니다.
...
적합성 기준은 담당 문서가 권한 있는 사실로 정한 것만 판단합니다. 시나리오 설명, 에이전트 요약, 렌더링된 보기, 상태 문구, 유지보수 점검 라벨, 상태 보기는 특정 담당 문서가 그 사실을 권한으로 정의할 때만 적합성 권한이 됩니다.

그 자체만으로는 권한으로 다루면 안 됩니다.

이 문서의 "해야 합니다", "필수", "항상"은 적합성 기준이나 담당 문서로 이어지는 요건을 뜻합니다. 인접 계약을 다시 정의한다는 뜻이 아닙니다.

<a id="criteria-vs-examples-and-tutorials"></a>
## 기준과 예시, 튜토리얼의 구분

적합성 기준은 참조 기준입니다. 예시와 튜토리얼은 독자가 시나리오를 알아보도록 도울 수 있지만, 권한 기록, API 분기, 저장 효과, 보안 보장, 닫기 준비 상태 결과, 수락 증거, 잔여 위험 수락을 만들지 않습니다.

참조 시나리오는 안정적인 동작 설명을 사용해야 합니다. 유지보수 흐름 라벨, 넓은 검토 단계, 오래 유지되지 않는 프로젝트 상태를 점검 대상 동작으로 삼으면 안 됩니다.

여러 메서드를 잇거나 끝까지 이어지는 시나리오는 이 문서나 다른 시나리오 수준 문서에서 시나리오 수준 기준으로만 다룰 수 있습니다. API 메서드 참조 문서들이 공유하는 페이로드, 픽스처, 예시 축이 되면 안 됩니다.

API 메서드 예시는 자신이 설명하는 개념을 밝히기 위해 적합성 시나리오에 개념적으로 링크할 수 있습니다. 그러나 일관성을 맞추려고 그 시나리오의 페이로드, 참조, 경로, `state_version`, 아티팩트 참조, 실행 참조, 판단 참조, 차단 사유 참조, 응답 스냅샷을 복사하면 안 됩니다. API 메서드 예시는 적합성 기준을 정의하지 않으며, 적합성 시나리오는 메서드 참조 예시가 그 페이로드를 재사용하도록 요구하지 않습니다.
...
- [사용자 소유 판단 요청 메서드](api/method-request-user-judgment.md)
- [사용자 소유 판단 기록 메서드](api/method-record-user-judgment.md)
