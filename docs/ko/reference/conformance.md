...
현재 활성 예시는 `cooperative`와 지원되는 `detective` 사실만 주장할 수 있습니다. `preventive` 또는 `isolated` 주장은 승격된 프로필과 그 프로필에 대해 담당 문서가 정의한 증명 경로가 있을 때만 유효합니다. 적합성 계획 문구만으로 이런 표시 값이 현재 실행 가능하거나 증명된 것이 되지 않습니다.
...
| 예시 | 동작 | 향후 주장 초점 |
|---|---|---|
| `MVP-ACTIVE-project-state-version-single-clock` | 커밋된 모든 non-dry-run 상태 변경은 단일 프로젝트 전체 `project_state.state_version`을 사용합니다. `tasks.state_version`은 활성 충돌 기준이나 동시성 기준이 아닙니다. | 새 상태 변경은 `ToolEnvelope.expected_state_version`을 `project_state.state_version`과 비교합니다. 커밋된 상태 변경은 이 값을 정확히 1 올립니다. `ToolResponseBase.state_version`, `tool_invocations.basis_state_version`, `task_events.state_version`은 모두 프로젝트 전체 모델을 따릅니다. `close_task intent=supersede`가 Task 생명주기와 `project_state.active_task_id`를 함께 바꾸는 경우도 같습니다. |
| `MVP-ACTIVE-prepare-write-blocked-or-dry-run-no-durable-authorization` | 차단되었거나 dry-run인 `prepare_write`는 지속되는 승인을 만들지 않습니다. | 응답에는 소비 가능한 Write Authorization이 없습니다. `write_authorizations`에는 활성 행이 추가되지 않습니다. Run, 아티팩트, 증거, 닫기, 최종 수락, 잔여 위험 상태가 바뀌지 않습니다. |
| `MVP-ACTIVE-prepare-write-committed-scoped-authorization` | 허용되어 committed 처리된 `prepare_write`는 범위가 정해진 1회용 Write Authorization을 기록합니다. | 응답의 승인 범위, Core 상태, `write_authorizations.attempt_scope_json`이 Task, Change Unit, 프로젝트 전체 `basis_state_version`, 접점, 의도한 제품 파일 경로, 민감 범주, 기준 참조, 관련 판단, 보장 표시 수준에서 같은 의미를 가집니다. |
| `MVP-ACTIVE-close-task-blocks-missing-acceptance-or-risk-condition` | 필요한 최종 수락이 없으면 `close_task`가 차단됩니다. 또한 닫기에 영향을 주는 잔여 위험이 필요한 수준으로 보이지 않거나, 그 위험이 활성 닫기 경로에서 요구될 때 수락되지 않은 경우에도 차단됩니다. | 응답 차단 사유는 담당 문서의 category와 필요한 경우 `required_judgment_kind`를 사용합니다. Task는 completed가 되지 않습니다. 닫기 상태가 누락된 수락이나 위험 수락을 대신하지 않습니다. 증거, 최종 수락, 잔여 위험 상태는 분리되어 남습니다. |
