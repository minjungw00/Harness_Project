...
| `MVP-ACTIVE-record-run-consumes-write-authorization-once` | 호환되는 제품 쓰기 `harness.record_run`은 맞는 활성 Write Authorization을 정확히 한 번 소비합니다. | Run은 호환되는 Write Authorization 하나와 연결되고, 그 Write Authorization은 해당 Run이 소비한 것으로 표시됩니다. 멱등 재실행은 다시 소비하지 않고 원래 응답을 반환합니다. Write Authorization이 없거나, 오래됐거나, 만료됐거나, 철회됐거나, 이미 소비됐거나, 호환되지 않거나, 승인 범위 밖 관찰 시도이면 성공 Run, 증거, 아티팩트 승격, 닫기 상태, 상태 버전 증가를 만들지 않습니다. |
| `MVP-ACTIVE-stage-artifact-temporary-handle-only` | `harness.stage_artifact`는 호출자가 제공한 안전한 바이트나 안전 공지를 같은 프로젝트/같은 Task의 임시 `StagedArtifactHandle`로 스테이징할 뿐입니다. | 스테이징은 Core 기록, 지속 `ArtifactRef`, 증거 요약, 차단 사유, 이벤트, `tool_invocations` 재실행 행, 닫기 효과, `project_state.state_version` 증가를 만들지 않습니다. 핸들은 범위가 정해져 있고 만료되며 한 번만 소비될 수 있어야 합니다. 증거 권한도 아니고 임의 로컬 파일 권한도 아닙니다. |
| `MVP-ACTIVE-record-run-promotes-staged-artifact-to-artifact-ref` | 호환되는 `harness.record_run`만 만료되지 않은 같은 프로젝트/같은 Task의 `StagedArtifactHandle`을 소비해 지속 `ArtifactRef`로 승격할 수 있습니다. | `record_run`은 누락, 만료, 불일치, 이미 소비됨, 다른 프로젝트, 다른 Task, 무결성 불일치 핸들을 `ARTIFACT_MISSING`과 `ToolError.details.staging_handle_reason`으로 거부하며, Run, 아티팩트, 증거 업데이트, 승인 소비, 재실행 행, 상태 버전 증가를 만들지 않습니다. 유효하게 소비된 핸들은 스테이징된 `sha256`, `size_bytes`, `content_type`, `redaction_state`, project, Task와 맞는 등록 아티팩트 메타데이터와 담당 연결을 만듭니다. |
