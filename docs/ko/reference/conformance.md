...
| `MVP-ACTIVE-close-task-complete-stale-state-version-rejected` | `harness.close_task intent=complete`에서 `ToolEnvelope.expected_state_version`이 현재 프로젝트 전체 `project_state.state_version`보다 오래됐으면, complete 닫기 차단 사유 행렬을 실행하기 전에 `close_task` 사전 확인에서 커밋 전 실패가 됩니다. | 예상 응답 분기는 `ToolRejectedResponse` 거절 응답이고 `errors[0].code=STATE_VERSION_CONFLICT`입니다. `CloseTaskResult`가 아닙니다. 거절된 시도는 `CloseBlocker` 없음, `tool_invocations` 재실행 행 없음, `close_state` 변경 없음, Write Authorization 생성 또는 소비 없음, 상태 버전 증가 없음(`state_version` 증가 없음)을 유지해야 합니다. |
| `MVP-ACTIVE-close-task-complete-stale-write-authorization-basis-rejected` | `harness.close_task intent=complete`의 사전 확인에서 닫기와 관련된 `WriteAuthorization.basis_state_version`이 현재 `project_state.state_version`보다 오래됐다고 확인되면, 닫기 차단 사유 행렬 커밋이나 Write Authorization 소비 전에 커밋 전 실패가 됩니다. | 예상 응답 분기는 `ToolRejectedResponse` 거절 응답이고 `errors[0].code=STATE_VERSION_CONFLICT`입니다. `CloseTaskResult`가 아닙니다. 거절된 시도는 `CloseBlocker` 없음, Write Authorization 소비 없음, `tool_invocations` 재실행 행 없음, 닫기 상태나 증거 변경 없음, 상태 버전 증가 없음(`state_version` 증가 없음)을 유지해야 합니다. |
