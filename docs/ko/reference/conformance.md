...
기준 범위의 기준 설명은 [범위](scope.md)를 확인하세요. 구현 경로는 [구현 가이드](../build/implementation-guide.md)가 설명합니다.

## 적합성 항목 요약

| 항목 | 경계 | 세부사항 |
|---|---|---|
| 문서 기준 | 지원되는 문서 기준 | [세부사항](#documentation-criteria) |
| 내부 스모크 목표 | 구현 담당 점검 대상 | [세부사항](#internal-smoke-target) |
| 픽스처 형식 | 지원 범위 밖 후보 형식 | [세부사항](#fixture-shape-boundary) |
| 실행 가능한 픽스처 | 문서 범위 밖 | [세부사항](#executable-fixtures) |
| 런타임 적합성 보고 | 지원 범위 밖 기능 | [세부사항](#runtime-conformance-report) |

<a id="documentation-criteria"></a>
### 문서 기준

경계:
- 문서 유지보수를 위한 지원 참조 기준입니다.
...
경계:
- 구현 담당 문서가 책임지는 점검 대상입니다.

실행 경계:
- 실행 동작은 구현 담당 문서가 맡습니다.

담당 문서:
- `build/implementation-guide.md`

허용되지 않는 것:
- 문서가 담당하는 적합성 모음처럼 설명하지 않습니다.

<a id="fixture-shape-boundary"></a>
### 픽스처 형식 경계

경계:
- 이 참조 문서가 설명하는 지원 범위 밖 후보 형식입니다.

실행 경계:
- 후보 형식은 픽스처 파일, 실행기 입력, 적합성 테스트 모음 진입점이 아닙니다.

담당 문서:
- `docs/ko/reference/conformance.md`

허용되지 않는 것:
- 후보 형식을 픽스처 파일, 실행기 입력, 실행 가능한 적합성 모음처럼 설명하지 않습니다.
...
실행 경계:
- 실행 가능한 픽스처 자료에는 실행기 담당 문서와 담당 문서가 승격한 픽스처가 필요합니다.

담당 문서:
- 실행기 담당 문서와 픽스처를 승격하는 담당 문서입니다.
...
실행 경계:
- 적합성 실행기, 적합성 테스트 모음 진입점, 생성된 적합성 보고서, 런타임 적합성 결과는 문서 범위 밖입니다.

담당 문서:
- [범위 참조](scope.md)
- [정책과 적합성: 적합성 실행 진입점](scope.md)

허용되지 않는 것:
- 지표, 생성된 글, 렌더링된 보고서, 문서 점검 라벨을 적합성 권한이나 현재 런타임 증명으로 제시하지 않습니다.

이 문서의 "해야 합니다", "필수", "항상"은 문서 기준이나 서버/실행기 계약의 요건을 뜻합니다. 문서가 실행 가능한 점검을 만든다는 뜻이 아닙니다.

## 적합성이 뜻하는 것

서버 적합성이란 실행 점검이 담당 문서가 정의한 동작 하나를 담당 문서의 권한 기록과 비교할 수 있다는 뜻입니다. 문서 점검은 링크, 용어, 담당 문서 경계, 범위 문구, 보안 표현, 한영 의미 일치를 보는 별도 유지보수 보조 도구입니다.

런타임 적합성 점검은 담당 문서가 권한 있는 사실로 정한 것만 판단해야 합니다. 특정 담당 문서가 그 사실을 승격하지 않았다면 생성된 글, 에이전트 요약, 렌더링된 보고서, 상태 문구, 문서 점검 라벨, 상태 보기를 권한으로 다루면 안 됩니다.

## 문서 경계

아래 항목은 런타임 또는 구현 산출물이며 문서 저장소 내용이 아닙니다.
...
- 기준 범위 동작이나 지원 범위 밖 기능에 대한 런타임 결과
- 도구 실행 전 차단, OS 권한 제어, 임의 도구 샌드박스, 변조 방지 저장소, 보안 격리, 프로필 조건부 더 강한 보장 주장에 대한 런타임 증명

이 문서의 예시는 적합성 개념을 설명합니다. 런타임 상태, 수락 증거, 닫기 준비 상태, 잔여 위험 수락, 생성된 보고서, 구현 경로 상태를 만들지는 않습니다.

## 픽스처 형식

픽스처 형식은 지원 범위 밖 후보 형식이며, 문서가 소유하는 픽스처 파일이 아닙니다. 승격된 픽스처는 아래 부분을 담은 작은 구조화 기록이어야 합니다.
...
목적:
- 담당 문서가 승격한 픽스처가 비교할 수 있는 구조화된 사실을 이름 붙입니다.
...
담당 문서가 승격한 구체화 픽스처는 공개 담당 스키마를 사용해야 합니다. 픽스처 전용 enum 값, 가짜 필드, 상태로 쓰는 지역화 표시 라벨, 글로만 된 기대값, 지원 범위 밖 기능 전용 값을 만들면 안 됩니다.

## 주장 권한

주장 권한은 담당 문서가 승격한 픽스처가 판단할 수 있는 사실의 좁은 범위입니다. 권한은 시나리오 설명이나 생성된 요약이 아니라 담당 문서가 정의한 사실에서 옵니다.

픽스처 주장은 담당 문서가 정의한 응답 사실, Core 상태, 저장 효과, 아티팩트 사실, 공개 `ErrorCode` 값, 구조화된 차단 사유, 보장 표시 사실, 금지된 부작용의 부재를 참조할 수 있습니다.
...
아래 `scenario_id`는 담당 문서가 승격할 픽스처 설계를 위한 작은 문서 기준입니다. 픽스처 본문, 런타임 결과, 생성된 런타임 객체, 구현 계획이 아닙니다. 정확한 분기, 저장, 접근, 아티팩트, 보안, 닫기 준비 상태 계약은 위 담당 문서 링크를 사용합니다.
...
담당 문서 링크:
- [에이전트 통합](agent-integration.md)
- [API 메서드 담당 문서 경로](api/methods.md#메서드-담당-문서-경로)
...
기준 범위 밖 픽스처 계열은 [정책과 적합성 픽스처 계열](scope.md)에 둡니다. 범위 참조는 이름만 보존하며, 이 문서는 그 목록을 반복하지 않습니다.
...
승격 조건: 담당 문서가 좁은 동작을 범위, 대체 동작, 정확한 계약, 증명 경로 기대치와 함께 승격해야 실행 가능한 픽스처 자료가 지원됩니다.

## 지표 경계

지표는 적합성 권한이 아닙니다. 로컬 지표와 지원 범위 밖 기능인 적합성 보고는 진단에 유용할 수 있지만, 담당 문서가 승격하기 전에는 읽기 전용 파생 표시나 지원 범위 밖 기능으로 남습니다.
...
승격 조건: 지표가 승격되면 담당 문서가 원천 기록, 최신성 경계, 표시 문구, 대체 불가 규칙을 정의해야 합니다.
