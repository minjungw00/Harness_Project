...
| `MVP-ACTIVE-project-state-version-stale-mutation-rejected` | 상태를 바꾸는 `dry_run=false` 요청의 `ToolEnvelope.expected_state_version`이 현재 프로젝트 전체 `project_state.state_version`보다 오래되었으면 요청은 커밋 전에 실패합니다. `tasks.state_version`은 활성 충돌 기준이나 동시성 기준이 아닙니다. | 예상 응답 분기는 `ToolRejectedResponse`이고 공개 `ErrorCode`는 `STATE_VERSION_CONFLICT`입니다. 실패한 시도는 현재 기록, 이벤트, 아티팩트, 증거, Write Authorization 생성 또는 소비, 닫기 상태, 재실행 행, 상태 버전 증가를 만들지 않습니다. 성공적으로 커밋된 변경만 프로젝트 전체 `project_state.state_version`을 정확히 1 올립니다. |
| `MVP-ACTIVE-dry-run-pre-commit-failure-rejected` | `dry_run=true` 요청이어도 검증, 로컬 접근, 역량, 상태 조회, 오래된 상태, 오래된 Write Authorization 근거 버전, 스테이징된 핸들 검증이 실패하면 읽기 전용 결과나 `dry_run` 미리보기 전에 멈춥니다. | 예상 응답 분기는 `dry_run=true`, `effect_kind=no_effect`인 `ToolRejectedResponse`입니다. `ToolDryRunResponse`도 아니고 메서드별 `MethodResult`도 아닙니다. 이 거절은 현재 기록, `task_events`, 재실행 행, 아티팩트, 증거, 닫기 상태, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version` 증가를 만들지 않습니다. |
...
| `MVP-ACTIVE-prepare-write-requires-compatible-scope-and-approval` | `harness.prepare_write`는 의도한 제품 파일 쓰기가 활성 Task, Change Unit, 범위, 기준선, Autonomy Boundary, 필요한 사용자 판단, 필요한 별도 민감 동작 승인, 접점 역량, 현재 프로젝트 전체 상태 버전과 호환될 때만 `decision=allowed`를 반환할 수 있습니다. | 커밋된 차단 결과의 예상 응답 분기는 `PrepareWriteResult`이고 `decision=blocked`, `approval_required`, 또는 `decision_required`를 씁니다. 프로젝트 전체 버전 불일치는 공개 `ErrorCode` `STATE_VERSION_CONFLICT`가 있는 `ToolRejectedResponse`이며 별도 `prepare_write` decision 값이 아닙니다. 차단 결과와 `dry_run` 미리보기는 소비 가능한 Write Authorization, Run, 아티팩트, 증거, 닫기, 최종 수락, 잔여 위험 상태를 만들지 않습니다. |
...
| `MVP-ACTIVE-close-task-complete-stale-state-version-rejected` | `harness.close_task intent=complete`에서 `ToolEnvelope.expected_state_version`이 현재 프로젝트 전체 `project_state.state_version`보다 오래됐으면, complete 닫기 차단 사유 행렬을 실행하기 전에 `close_task` 사전 확인에서 커밋 전 실패가 됩니다. | 예상 응답 분기는 `ToolRejectedResponse` 거절 응답이고 `errors[0].code=STATE_VERSION_CONFLICT`입니다. `CloseTaskResult`가 아닙니다. 거절된 시도는 `CloseBlocker` 없음, `tool_invocations` `replay row`(재실행 행) 없음, `close_state` 변경 없음, Write Authorization 생성 또는 소비 없음, 상태 버전 증가 없음(`state_version` 증가 없음)을 유지해야 합니다. |
| `MVP-ACTIVE-close-task-complete-stale-write-authorization-basis-rejected` | `harness.close_task intent=complete`의 사전 확인에서 닫기와 관련된 `WriteAuthorization.basis_state_version`이 현재 `project_state.state_version`보다 오래됐다고 확인되면, 닫기 차단 사유 행렬 커밋이나 Write Authorization 소비 전에 커밋 전 실패가 됩니다. | 예상 응답 분기는 `ToolRejectedResponse` 거절 응답이고 `errors[0].code=STATE_VERSION_CONFLICT`입니다. `CloseTaskResult`가 아닙니다. 거절된 시도는 `CloseBlocker` 없음, Write Authorization 소비 없음, `tool_invocations` `replay row`(재실행 행) 없음, 닫기 상태나 증거 변경 없음, 상태 버전 증가 없음(`state_version` 증가 없음)을 유지해야 합니다. |
| `MVP-ACTIVE-close-task-blocks-current-write-compatibility` | `harness.close_task intent=complete`에서 사전 확인이 현재 `ToolEnvelope.expected_state_version`과 닫기 관련 `WriteAuthorization.basis_state_version`의 최신성을 확정한 뒤에도, complete 닫기 차단 사유 행렬은 의미적 Write Authorization 또는 제품 쓰기 Run 비호환을 찾을 수 있습니다. | 예상 응답 분기는 커밋된 닫기 차단 결과인 `CloseTaskResult`입니다. `CloseTaskResult.close_state=blocked`, `CloseBlocker.category=write_compatibility`, `CloseBlocker.code != STATE_VERSION_CONFLICT`를 사용해야 합니다. `CloseBlocker.code`는 `WRITE_AUTHORIZATION_REQUIRED`, `WRITE_AUTHORIZATION_INVALID`, `SCOPE_VIOLATION`, `BASELINE_STALE`, `VALIDATOR_FAILED` 같은 의미적 차단 사유 코드여야 하며 Task는 열린 상태로 남습니다. |
...
| `MVP-ACTIVE-close-task-check-read-only` | `harness.close_task intent=check`는 `dry_run=true`가 있어도 상태를 바꾸지 않고 닫기 가능 여부와 차단 사유를 계산합니다. | 예상 응답 분기는 `CloseTaskResult`이고 `base.effect_kind=read_only`입니다. `dry_run=true`가 있으면 `ToolDryRunResponse`가 아니라 `base.dry_run=true`, `base.effect_kind=read_only`인 `CloseTaskResult`를 유지합니다. `tasks`, `blockers`, `task_events`, `tool_invocations`, 닫기 상태 변경, 증거 요약, 아티팩트, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version` 변경이 없습니다. |
| `MVP-ACTIVE-close-task-state-effecting-dry-run-preview` | `harness.close_task`는 혼합 intent 메서드입니다. 응답 분기 선택은 메서드 이름만이 아니라 선택된 intent의 상태 효과를 따릅니다. `intent=complete`, `intent=cancel`, `intent=supersede`와 `dry_run=true`가 함께 있고 그 외에는 유효하며 미리보기 가능하면 상태 효과가 있는 `dry_run` 분기를 선택합니다. | 예상 응답 분기는 그런 유효한 상태 효과 `dry_run` 미리보기에 대한 `effect_kind=no_effect`인 `ToolDryRunResponse`입니다. 미리보기는 예상 닫기 차단 사유, 종료 닫기 효과, 다음 행동을 설명할 수 있지만 닫기 상태 변경, `task_events`, 재실행 행, 아티팩트 생성·갱신·연결, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version` 증가를 만들지 않습니다. |
