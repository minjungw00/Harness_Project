...
- `harness.stage_artifact` 담당 문서가 범위에 있을 때의 임시 `StagedArtifactHandle` 응답 사실. 단, 지속 아티팩트 권한은 호환되는 `record_run` 승격 뒤에만 주장할 수 있습니다.
...
| `MVP-ACTIVE-stage-artifact-temporary-handle-only` | `harness.stage_artifact`는 `artifact_registration`으로 호출자가 제공한 안전한 bytes나 안전 공지를 같은 프로젝트/같은 Task의 임시 `StagedArtifactHandle`로 staging할 뿐입니다. | staging은 Core 기록, persistent `ArtifactRef`, 증거 요약, 차단 사유, 이벤트, `tool_invocations` 재실행 행, 닫기 효과, `project_state.state_version` 증가를 만들지 않습니다. handle은 범위가 정해져 있고 만료되며 한 번만 소비될 수 있어야 합니다. 증거 권한도 아니고 임의 로컬 파일 권한도 아닙니다. |
| `MVP-ACTIVE-record-run-promotes-staged-artifact-to-artifact-ref` | 호환되는 `run_recording`의 `harness.record_run`만 만료되지 않은 같은 프로젝트/같은 Task의 `StagedArtifactHandle`을 소비해 persistent `ArtifactRef`로 승격할 수 있습니다. | `record_run`은 누락, 만료, 불일치, 이미 소비됨, 다른 프로젝트, 다른 Task, 무결성 불일치 handle을 `ARTIFACT_MISSING`과 `ToolError.details.staging_handle_reason`으로 거부하며, Run, artifact, 증거 업데이트, 승인 소비, 재실행 행, 상태 버전 증가를 만들지 않습니다. 유효하게 소비된 handle은 staging된 `sha256`, `size_bytes`, `content_type`, `redaction_state`, project, Task와 맞는 등록 artifact 메타데이터와 담당 연결을 만듭니다. 이 승격은 `artifact_registration`을 요구하지 않습니다. 그 접근 분류는 앞선 `harness.stage_artifact` 요청에서만 사용됩니다. |
