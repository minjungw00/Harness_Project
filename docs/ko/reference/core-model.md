...
- 증거 요약과 차단 사유 기록: 증거 기대 또는 증거 공백, 활성 차단 사유, close 가능성 평가의 닫기 차단 사유.
...
명령, Run, 검토, validator, 진단, 향후 QA/검증 흐름에서 나온 발견 사항은 활성 담당 경로를 통해 라우팅될 때만 Core에 영향을 줍니다. 예를 들면 차단 사유, 증거 요약, 사용자 판단, `harness.update_scope`, close 가능성 평가의 닫기 차단 사유입니다. 대화나 보고서 문장에 남은 발견 사항은 상태가 아닙니다.
...
close 가능성 평가는 현재 Core 상태를 기준으로 현재 Task가 닫힐 수 있는지 평가하는 Core 평가입니다. 이 평가는 `CloseReadinessBlocker`로 표현되는 닫기 차단 사유를 만들 수 있습니다. `CloseReadinessBlocker`는 데이터 구조일 뿐이며, 타입 자체는 저장 효과를 뜻하지 않습니다. 이 타입이 있다는 사실만으로 `close_state` 변경, 재실행 행 생성, 이벤트 생성, 아티팩트 효과, Write Authorization 변경, 스테이징된 핸들 소비, 증거 업데이트, `project_state.state_version` 증가가 생기지 않습니다. 상태 효과는 응답 branch와 메서드 상태 효과 표가 결정합니다.

close 가능성 평가는 호출 경로마다 다른 응답 branch로 드러날 수 있습니다.

- `harness.status`는 close 가능성 평가를 읽기 전용 관찰로 보여 줄 수 있습니다. `StatusResult.close_blockers: CloseReadinessBlocker[]`는 응답에만 담기는 관찰 데이터이며 저장소나 상태 효과를 만들지 않습니다.
- `harness.close_task intent=check`는 close 가능성 평가를 읽기 전용으로 수행합니다. `CloseTaskResult.blockers: CloseReadinessBlocker[]`를 반환하며, `dry_run=true`가 있어도 `base.dry_run=true`, `effect_kind=read_only`인 `CloseTaskResult`입니다. 저장소나 상태 효과를 만들지 않습니다.
- `harness.close_task intent=complete`는 사전 확인을 먼저 수행한 뒤 close 가능성 평가를 실행합니다. 닫기 차단 사유가 남아 있으면 `CloseTaskResult(close_state=blocked)`와 `CloseTaskResult.blockers: CloseReadinessBlocker[]`를 반환합니다. 커밋된 효과는 이 응답 branch와 메서드 상태 효과 표가 허용할 때만 생깁니다.
- `harness.close_task intent=complete`, `intent=cancel`, `intent=supersede`에 `dry_run=true`가 있으면 요청이 그 밖에는 유효하고 미리보기 가능할 때 `ToolDryRunResponse`를 반환합니다. 예상 close 가능성 평가 발견 사항은 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로만 표현하며 `source_kind=close_readiness`를 사용합니다. 실제 `CloseReadinessBlocker` 객체가 아닙니다.
- `prepare_write`는 close 가능성 평가를 실행하지 않고, `close_state`를 바꾸지 않으며, `CloseReadinessBlocker`를 쓰지 않습니다. prepare_write 판단 사유에는 `PrepareWriteResult.write_decision_reasons: WriteDecisionReason[]`만 사용합니다.

`close_task`는 먼저 커밋 전 실패를 가르는 사전 확인을 수행합니다. 이 단계는 close 가능성 평가, 재실행 행 생성, Write Authorization 소비, 스테이징된 핸들 소비보다 앞섭니다. 실패하면 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환하며, `CloseTaskResult`나 `CloseTaskResult(close_state=blocked)`가 아닙니다. `close_task` 사전 확인의 거절 항목은 다음입니다.
...
- close 가능성 평가 전에 Core 상태를 읽을 수 없는 경우.
- 유효한 Project/Task를 선택하기 전에 요청 식별 정보가 실패한 경우.
- close 가능성 평가 전에 로컬 접근 또는 역량 확인이 실패한 경우.
...
`idempotency_key`의 `request_hash` 충돌, 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`은 complete close 가능성 평가 밖에 있습니다. 모두 사전 확인 거절이며 의미적 닫기 차단 사유가 아닙니다.
...
사전 확인이 성공한 뒤 `intent=complete`, `intent=cancel`, `intent=supersede`에 `dry_run=true`가 있으면 요청이 그 밖에는 유효하고 미리보기 가능할 때 `ToolDryRunResponse`를 반환합니다. 이 미리보기는 종료 또는 커밋된 닫기 차단 예상 효과, 예상되는 close 가능성 평가 발견 사항, 다음 행동을 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로 보여 줄 수 있으며 `source_kind=close_readiness`를 사용합니다. 하지만 실제 `CloseReadinessBlocker` 객체를 반환하지 않고, Task 생명주기, 닫기 필드, 차단 사유, 이벤트, 재실행 행, 아티팩트, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version`을 바꾸지 않습니다.

사전 확인이 성공한 `dry_run=false` 상태 변경 닫기 intent에서 Core는 종료 전이를 커밋하거나, 메서드 상태 효과 계약이 허용하는 커밋된 닫기 차단 결과를 반환합니다. `CloseTaskResult(close_state=blocked)`는 사전 확인이 성공한 뒤 발견한 의미적 닫기 차단 사유에 대해서만 `blockers: CloseReadinessBlocker[]`를 반환합니다. 오래된 상태, 오래된 권한 근거, `idempotency_key`의 `request_hash` 충돌, 읽을 수 없는 Core 상태, 요청 식별 정보 실패, close 가능성 평가 전 로컬 접근/역량 실패에 대한 응답이 아닙니다. 커밋된 닫기 차단 결과는 Task를 열린 상태로 둡니다.
...
| `Task.result` | 작업의 굵은 결과입니다. 값은 `none`, `advice_only`, `completed`, `cancelled`, `superseded`입니다. 실패한 Run, violation, 차단된 닫기, 증거 공백은 Run 상태, close 가능성 평가의 닫기 차단 사유 상태, 증거 상태, 현재 Task 상태에 남기며 종료 Task 결과가 아닙니다. |
...
닫기 가능성은 [API Schema Core](api/schema-core.md#current-mvp-value-sets)가 담당하는 활성 `CloseReadinessBlocker.category` 값만 사용합니다.
