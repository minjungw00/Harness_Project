...
- 공개 API 페이로드 스키마, 응답 분기 형태, 공통 요청 래퍼, 메서드 결과 구조. [MVP API 경로 문서](api/mvp-api.md), 메서드 담당 문서, [API 코어 스키마](api/schema-core.md), API 스키마 담당 문서를 봅니다.
...
| 이후 후보 | [이후 후보](#core-owner-later-candidates)를 참고합니다. |
...
담당 문서 링크:
- [MVP API 경로 문서](api/mvp-api.md)와 그 문서가 나열하는 메서드 담당 문서.
