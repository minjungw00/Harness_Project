...
모든 `close_task` 사전 확인 거절은 상태 효과 없음입니다. `CloseBlocker` 없음, `task_event` 또는 `task_events` 추가 없음, `tool_invocations` 재실행 행(`replay row`) 없음, `close_state` 변경 없음, Write Authorization 소비 없음, 스테이징된 핸들 소비 없음, 아티팩트 효과 없음, 증거 요약 갱신 없음, `project_state.state_version` 증가 없음입니다. `STATE_VERSION_CONFLICT`는 `close_task`의 사전 확인 거절에만 속합니다. `CloseBlocker.code`, `write_compatibility` 또는 `recovery` 행렬 행, 커밋된 닫기 차단 결과에 넣으면 안 됩니다.
...
| 순서 | 확인 | 실패했을 때의 차단 결과 |
|---:|---|---|
| 1 | 선택된 Task 생명주기 유효성 | 선택된 같은 프로젝트의 Task가 이미 종료되었거나 complete 전이에 들어갈 수 없으면 `task` 차단 사유를 반환합니다. Project/Task identity가 없거나, 읽을 수 없거나, 다른 프로젝트에 속하면 사전 확인 거절이며 닫기 차단 사유가 아닙니다. |
...
| 13 | 복구 제약 확인 | 현재 상태를 읽을 수 있고 사전 확인이 성공한 뒤에도 해결되지 않은 수리나 복구 상태, 손상, 해결되지 않은 차단 상태, 그 밖의 수리 제약을 닫기 전에 처리해야 하면 `recovery` 차단 사유를 반환합니다. `idempotency_key` request hash 충돌과 상태 버전 충돌은 사전 확인 거절이며 `recovery` 차단 사유가 아닙니다. |
...
`idempotency_key` request hash 충돌, 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`은 complete 닫기 차단 사유 행렬 밖에 있습니다. 모두 사전 확인 거절이며 의미적 차단 사유가 아닙니다.
...
사전 확인이 성공한 뒤 `intent=complete`, `intent=cancel`, `intent=supersede`에 `dry_run=true`가 있으면 요청이 그 밖에는 유효하고 미리보기 가능할 때 `ToolDryRunResponse`를 반환합니다. 이 미리보기는 종료 또는 커밋된 닫기 차단 예상 효과, 후보 닫기 차단 사유, 다음 행동을 보여 줄 수 있습니다. 하지만 Task 생명주기, 닫기 필드, 차단 사유, 이벤트, 재실행 행, 아티팩트, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version`을 바꾸지 않습니다.

사전 확인이 성공한 `dry_run=false` 상태 변경 닫기 intent에서 Core는 종료 전이를 커밋하거나, 메서드 상태 효과 계약이 허용하는 커밋된 닫기 차단 결과를 반환합니다. `CloseTaskResult(close_state=blocked)`는 커밋된 닫기 차단 결과에만 쓰며, 이는 사전 확인이 성공한 뒤 발견한 의미적 차단 사유를 뜻합니다. 오래된 상태, 오래된 권한 근거, `idempotency_key` request hash 충돌, 읽을 수 없는 Core 상태, 요청 식별 정보 실패, 닫기 차단 사유 행렬 평가 전 로컬 접근/역량 실패에 대한 응답이 아닙니다. 커밋된 닫기 차단 결과는 Task를 열린 상태로 둡니다.
...
| 활성 범주 | Core 의미 |
|---|---|
| `task` | Project/Task identity 사전 확인이 성공한 뒤에도 선택된 Task가 요청한 전이와 호환되지 않거나, 이미 종료되었거나, 그 밖의 이유로 사용할 수 없는 상태입니다. |
...
| `recovery` | 현재 상태를 읽을 수 있고 사전 확인이 성공한 뒤에도 해결되지 않은 수리 또는 복구 상태가 남아 있어 전이 전에 처리해야 합니다. `idempotency_key` request hash 충돌이나 상태 버전 충돌은 다루지 않습니다. |
