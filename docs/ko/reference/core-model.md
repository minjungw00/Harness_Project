...
- 정확한 활성 메서드 이름과 enum 형태 값 집합. [API 값 집합](api/schema-value-sets.md)을 봅니다.
- 저장소 테이블, DDL, Runtime Home 배치, 잠금, 마이그레이션, 지속 저장 JSON 배치. [저장소 기록](storage-records.md), [저장 효과](storage-effects.md), [아티팩트 저장소](storage-artifacts.md), [저장소 버전 관리](storage-versioning.md)를 봅니다.
- 렌더링된 Projection 본문이나 템플릿 본문. 권한은 [Projection과 템플릿 참조](projection-and-templates.md), 정확한 본문은 [템플릿 본문](template-bodies.md)을 봅니다.
- 커넥터 `capability_profile`과 접점별 사용법. [에이전트 통합 참조](agent-integration.md)와 [접점별 사용법](../use/surface-recipes.md)을 봅니다.
...
Core는 API 응답과 상태 효과 어휘를 일관되게 씁니다. `MethodResult`는 `ToolResultBase`를 바탕으로 한 메서드별 실제 메서드 결과 분기입니다. 여기에는 `effect_kind=read_only`인 실제 읽기 결과, `effect_kind=core_committed`인 Core 커밋 결과, `effect_kind=staging_created`인 성공한 스테이징 결과, 메서드 상태 효과 계약이 허용하는 커밋된 차단 결과가 포함됩니다. `ToolRejectedResponse`는 `effect_kind=no_effect`인 커밋 전 실패 분기입니다. 메서드별 성공 필드를 담지 않으며, 재실행 행 없음, 상태 버전 증가 없음, 스테이징된 핸들 소비 없음, Write Authorization 생성 없음, Write Authorization 소비 없음이 적용됩니다. `ToolDryRunResponse`는 `effect_kind=no_effect`인 유효한 `dry_run` 미리보기 분기입니다. 상태 효과가 없고, 실제 생성 참조나 소비 가능한 권한을 담지 않습니다. 오래된 프로젝트 전체 상태, 오래된 `WriteAuthorization.basis_state_version`, 유효하지 않은 스테이징된 핸들은 거절 응답이며 성공처럼 생긴 결과 필드로 표현하지 않습니다.

## 3. 엔티티 모델

아래 엔티티는 권한 관계를 정의합니다. 저장소 테이블이나 API 본문을 추가하지 않습니다.

- Task: 사용자 가치 단위입니다. 현재 구체적 `mode`, 범위 관계, 차단 사유, 판단 필요성, 증거 상태, 닫기 준비 상태, 최종 수락 상태, 잔여 위험 상태, 최근 Run 관계를 기록합니다. 활성 구체적 Task `mode` 값은 [API 값 집합](api/schema-value-sets.md)이 담당합니다. `harness.intake`의 `auto`는 분류 입력일 뿐 Task 상태가 아닙니다.
...
- 증거 요약과 차단 사유 기록: 증거 기대 또는 증거 공백, 활성 차단 사유, 닫기 준비 상태 평가의 닫기 차단 사유.
...
`ShapingReadiness`는 이 활성 상태에서 파생한 간결한 준비 상태 보기입니다. 현재 Task, 활성 또는 제안된 Change Unit, 대기 중인 `user_judgment` 후보나 기록, 증거 요약, 차단 사유, 다음 행동 상태에서 계산합니다. 별도 기록으로 저장하지 않으며, 지속되는 계획 아티팩트를 만들 수 있다는 허가도 아닙니다. 정확한 API 필드 이름은 [API 상태 스키마](api/schema-state.md)가 담당합니다.
...
명령, Run, 검토, validator, 진단, 향후 QA/검증 흐름에서 나온 발견 사항은 활성 담당 경로를 통해 라우팅될 때만 Core에 영향을 줍니다. 예를 들면 차단 사유, 증거 요약, 사용자 판단, `harness.update_scope`, 닫기 준비 상태 평가의 닫기 차단 사유입니다. 대화나 보고서 문장에 남은 발견 사항은 상태가 아닙니다.

## 4. 사용자 소유 판단 경계

사용자 소유 판단은 하네스가 추론하지 않고 사용자에게 묻거나 사용자의 선택으로 보존해야 하는 경계입니다. 정확한 `UserJudgment` 스키마와 API 필드는 [API 판단 스키마](api/schema-judgment.md)와 [MVP API](api/mvp-api.md)가 담당합니다. 이 섹션은 판단 경계의 의미를 담당합니다.
...
관문은 진행, 쓰기, Run 기록, 닫기를 위한 Core 호환성 요약입니다. 현재 MVP에서 공개 스키마에 노출되는 활성 관문 상태 필드는 [API 상태 스키마](api/schema-state.md)가 담당하고 그 값은 [API 값 집합](api/schema-value-sets.md)이 담당합니다. 계획 문장에 관문 이름이 있다는 이유만으로 활성 스키마 필드, 저장소 기록, validator, 닫기 차단 사유 category, 닫기 요구사항이 생기지 않습니다.
...
- <a id="capability-boundary"></a>Capability Boundary: 접점 역량은 차단 사유, validator 발견 사항, 보장 표시에 영향을 주지만 권한을 만드는 관문은 아닙니다. 역량이 부족하면 주장을 좁히거나, 담당 경로를 통해 동작을 보류하거나, `CloseReadinessBlocker.category=surface_capability`를 만들어야 합니다. 검증이나 사전 차단이 실제로 일어난 것처럼 꾸미면 안 됩니다.

예약된 gate 이름은 담당 문서가 승격하기 전까지 [이후 후보 색인](../later/index.md)에만 남습니다.

- <a id="design-gate"></a>Design Gate는 later/reserved gate 이름입니다. 설계 품질(Design Quality)은 현재 MVP의 활성 관문이 아니며, 현재 MVP에는 독립 설계 정책 닫기 관문이 없습니다. 설계 품질 관찰 사항은 제품/기술/범위 판단, 증거, 잔여 위험 표시, 접점 역량, 또는 활성 `CloseReadinessBlocker.category` 같은 담당 경로에 맞을 때만 닫기에 영향을 줍니다.
...
공개 응답에서 gate 상태를 어떻게 노출하는지는 [API 상태 스키마](api/schema-state.md), [API 값 집합](api/schema-value-sets.md), 메서드 담당 문서가 맡습니다. Core는 호환성 의미와 오래된 gate 요약을 쓰기 또는 닫기 전에 다시 계산해야 한다는 규칙을 담당합니다.
...
`decision=blocked`, `decision=approval_required`, `decision=decision_required`는 Core가 메서드 상태 효과 계약에 따라 해당 `prepare_write` 판단을 커밋할 때만 `PrepareWriteResult`로 반환됩니다. 이 `decision=allowed`가 아닌 결과는 `write_decision_reasons: WriteDecisionReason[]`로 prepare_write 판단 사유를 설명합니다. API 메서드 표가 허용한 이벤트, 재실행, prepare_write 판단 사유, 프로젝트 전체 상태 버전 효과만 가질 수 있으며, 소비 가능한 Write Authorization, 증거 기록, `close_state` 변경, `CloseReadinessBlocker`, 닫기 준비 상태 평가, 커밋된 닫기 결과를 만들면 안 됩니다.

`WriteDecisionReason`은 `prepare_write` 판단 출력을 설명합니다. `CloseReadinessBlocker`는 `StatusResult.close_blockers` 또는 `CloseTaskResult.blockers`로 반환되는 닫기 준비 상태 평가의 닫기 차단 사유 데이터 구조입니다. 두 타입은 서로 바꿔 쓸 수 없습니다. `prepare_write`는 prepare_write 판단 사유에 `WriteDecisionReason[]`만 쓰고 `CloseReadinessBlocker`를 쓰지 않습니다. 닫기 준비 상태 평가의 닫기 차단 사유도 `WriteDecisionReason`을 쓰지 않습니다.

이 상태 효과가 있는 메서드의 유효한 `dry_run=true` 호출은 `ToolDryRunResponse`를 반환합니다. `dry_run=false` 경로를 미리 보여 줄 수 있지만 Write Authorization을 만들지 않고 실제 `write_authorization_ref`를 반환하지 않습니다. 이 미리보기의 예상 prepare_write 판단 사유는 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로만 표현하며, 실제 `WriteDecisionReason`이나 `CloseReadinessBlocker` 객체가 아닙니다.
...
MCP 또는 연결된 접점이 필요한 협력형 확인을 수행할 수 없으면 정직한 결과는 보류, `WriteDecisionReason`을 담은 `decision=allowed`가 아닌 `PrepareWriteResult`, 낮아진 보장 표시, 또는 담당 API/오류 경계가 정한 커밋 전 역량 오류 중 하나입니다. 예방형 또는 격리형 표현은 later/profile-gated이며, 향후 담당 문서가 대상 동작의 정확한 경계를 승격하고 증명하기 전까지 사용할 수 없습니다.

현재 MVP의 `prepare_write`는 활성 접점이 제공할 수 없는 명령 관찰, 네트워크 관찰, 비밀값 접근 관찰, 아티팩트 캡처, 도구 실행 전 차단, 격리를 요구하는 요청을 거절하거나 `decision=allowed`가 아닌 `PrepareWriteResult`로 반환해야 합니다. 인식된 활성 접점에 요청한 역량이 없으면 `CAPABILITY_INSUFFICIENT`를 사용하고, 요청 형태나 요청한 보장이 활성 프로필에 유효하지 않으면 `VALIDATION_FAILED`를 사용합니다. `SensitiveActionScope` 세부사항이나 지원되지 않는 관찰을 활성 Write Authorization 안에 넣으면 안 됩니다.
...
닫기 준비 상태 평가는 현재 Core 상태를 기준으로 현재 Task가 닫힐 수 있는지 평가하는 Core 평가입니다. 이 평가는 `CloseReadinessBlocker`로 표현되는 닫기 차단 사유를 만들 수 있습니다. `CloseReadinessBlocker`는 데이터 구조일 뿐이며, 타입 자체는 저장 효과를 뜻하지 않습니다. 이 타입이 있다는 사실만으로 `close_state` 변경, 재실행 행 생성, 이벤트 생성, 아티팩트 효과, Write Authorization 변경, 스테이징된 핸들 소비, 증거 업데이트, `project_state.state_version` 증가가 생기지 않습니다. 상태 효과는 응답 branch와 메서드 상태 효과 표가 결정합니다.

닫기 준비 상태 평가는 호출 경로마다 다른 응답 branch로 드러날 수 있습니다.

- `harness.status`는 닫기 준비 상태 평가를 읽기 전용 관찰로 보여 줄 수 있습니다. `StatusResult.close_blockers: CloseReadinessBlocker[]`는 응답에만 담기는 관찰 데이터이며 저장소나 상태 효과를 만들지 않습니다.
- `harness.close_task intent=check`는 닫기 준비 상태 평가를 읽기 전용으로 수행합니다. `CloseTaskResult.blockers: CloseReadinessBlocker[]`를 반환하며, `dry_run=true`가 있어도 `base.dry_run=true`, `effect_kind=read_only`인 `CloseTaskResult`입니다. 저장소나 상태 효과를 만들지 않습니다.
- `harness.close_task intent=complete`는 사전 확인을 먼저 수행한 뒤 닫기 준비 상태 평가를 실행합니다. 닫기 차단 사유가 남아 있으면 `CloseTaskResult(close_state=blocked)`와 `CloseTaskResult.blockers: CloseReadinessBlocker[]`를 반환합니다. 커밋된 효과는 이 응답 branch와 메서드 상태 효과 표가 허용할 때만 생깁니다.
- `harness.close_task intent=complete`, `intent=cancel`, `intent=supersede`에 `dry_run=true`가 있으면 요청이 그 밖에는 유효하고 미리보기 가능할 때 `ToolDryRunResponse`를 반환합니다. 예상 닫기 준비 상태 평가 발견 사항은 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로만 표현하며 `source_kind=close_readiness`를 사용합니다. 실제 `CloseReadinessBlocker` 객체가 아닙니다.
- `prepare_write`는 닫기 준비 상태 평가를 실행하지 않고, `close_state`를 바꾸지 않으며, `CloseReadinessBlocker`를 쓰지 않습니다. prepare_write 판단 사유에는 `PrepareWriteResult.write_decision_reasons: WriteDecisionReason[]`만 사용합니다.

`close_task`는 먼저 커밋 전 실패를 가르는 사전 확인을 수행합니다. 이 단계는 닫기 준비 상태 평가, 재실행 행 생성, Write Authorization 소비, 스테이징된 핸들 소비보다 앞섭니다. 실패하면 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환하며, `CloseTaskResult`나 `CloseTaskResult(close_state=blocked)`가 아닙니다. `close_task` 사전 확인의 거절 항목은 다음입니다.

- `expected_state_version`이 현재 `project_state.state_version`과 맞지 않는 경우.
- 같은 `idempotency_key`를 다른 `request_hash`로 재사용한 경우.
- `WriteAuthorization.basis_state_version`이 현재 `project_state.state_version`보다 오래된 경우.
- 닫기 준비 상태 평가 전에 Core 상태를 읽을 수 없는 경우.
- 유효한 Project/Task를 선택하기 전에 요청 식별 정보가 실패한 경우.
- 닫기 준비 상태 평가 전에 로컬 접근 또는 역량 확인이 실패한 경우.

모든 `close_task` 사전 확인 거절은 상태 효과 없음입니다. `CloseReadinessBlocker` 없음, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 생성 없음, Write Authorization 소비 없음, 스테이징된 핸들 소비 없음, 아티팩트 승격 또는 연결 없음, 증거 요약 갱신 없음, `project_state.state_version` 증가 없음입니다. `STATE_VERSION_CONFLICT`는 `close_task`의 사전 확인 `ToolRejectedResponse.errors`에만 속합니다. `CloseReadinessBlocker.code`, `CloseTaskResult(close_state=blocked).errors[0]`, 커밋된 닫기 차단 결과에 넣으면 안 됩니다.

사전 확인이 성공하면 `intent=check`는 읽기 전용 닫기 준비 상태 평가입니다. 응답에만 현재 닫기 가능성, `CloseTaskResult.blockers: CloseReadinessBlocker[]`, 증거 요약, 아티팩트 참조, 다음 행동을 계산해 담을 수 있습니다. 요청에 `dry_run=true`가 함께 있어도 응답 분기는 `CloseTaskResult`로 유지되며 `base.dry_run=true`, `base.effect_kind=read_only`를 사용합니다. 두 경우 모두 상태 효과 없음: `close_state` 변경 없음, `task_events` 없음, 재실행 행 없음, 아티팩트 갱신 없음, 스테이징된 핸들 소비 없음, Write Authorization 생성 없음, Write Authorization 소비 없음, `project_state.state_version` 증가 없음입니다.

사전 확인이 성공하면 `intent=complete`는 결정적인 complete 닫기 준비 상태 평가를 사용합니다. Core는 `CloseReadinessBlocker`를 둘 이상 모을 수 있지만, 응답 순서와 기본 닫기 차단 사유의 근거는 아래 순서를 따릅니다.

| 순서 | 확인 | 실패했을 때의 닫기 준비 상태 평가 결과 |
|---:|---|---|
| 1 | 선택된 Task 생명주기 유효성 | 선택된 같은 프로젝트의 Task가 이미 종료되었거나 complete 전이에 들어갈 수 없으면 `task` 차단 사유를 반환합니다. Project/Task 식별 정보가 없거나, 읽을 수 없거나, 다른 프로젝트에 속하면 사전 확인 거절이며 `CloseReadinessBlocker`가 아닙니다. |
...
| 6 | Write Authorization과 Run 호환성 확인 | `write_compatibility`는 최신성이 확정된 뒤 현재 상태 전제에서 남는 의미적 차단 사유만 다룹니다. 필요한 Write Authorization 또는 제품 쓰기 Run 조건이 없거나, 사용할 수 없거나, 유효하지 않거나, 이미 소비되었거나, 범위/경로/baseline/관찰된 쓰기와 호환되지 않는 경우입니다. 오래된 `WriteAuthorization.basis_state_version`은 사전 확인 거절이며 이 행의 차단 사유가 아닙니다. |
| 7 | baseline과 접점 역량 확인 | 로컬 접근 사전 확인이 성공한 뒤에도 baseline이나 확인된 로컬 접점 역량이 닫기 주장 또는 필요한 보장 표시를 정직하게 뒷받침할 수 없으면 `baseline` 또는 `surface_capability` 차단 사유를 반환합니다. 현재 로컬 접근 실패는 사전 확인 거절이며 `CloseReadinessBlocker`가 아닙니다. |
...
| 13 | 복구 제약 확인 | 현재 상태를 읽을 수 있고 사전 확인이 성공한 뒤에도 해결되지 않은 수리나 복구 상태, 손상, 해결되지 않은 차단 상태, 그 밖의 수리 제약을 닫기 전에 처리해야 하면 `recovery` 차단 사유를 반환합니다. `idempotency_key`의 `request_hash` 충돌과 상태 버전 충돌은 사전 확인 거절이며 `recovery` 차단 사유가 아닙니다. |
| 14 | 닫기 전이 또는 차단 응답 | 차단 사유가 없으면 complete 전이를 커밋합니다. 하나라도 남아 있으면 `CloseTaskResult.close_state=blocked`를 반환하고 Task를 열린 상태로 둡니다. |

이 평가 순서는 뒤 단계가 앞 단계의 요구사항을 대신하게 허용하지 않습니다. 최종 수락과 잔여 위험 수락은 필수 증거를 대신할 수 없고, 뒷받침되지 않은 `EvidenceCoverageItem`을 충분한 증거로 바꿀 수 없으며, 필요한 아티팩트나 상태 참조를 대신할 수 없습니다.

`idempotency_key`의 `request_hash` 충돌, 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`은 complete 닫기 준비 상태 평가 밖에 있습니다. 모두 사전 확인 거절이며 의미적 닫기 차단 사유가 아닙니다.
...
사전 확인이 성공하면 `intent=cancel`과 `intent=supersede`는 해당 종료 경로의 전이 확인만 평가합니다. 둘 다 성공 완료가 아니므로 증거 충분성, 최종 수락, 잔여 위험 수락을 요구하지 않습니다. 의미적 차단 사유는 선택된 Task 생명주기, cancellation 또는 supersession 충돌, 해결되지 않은 복구 제약, 그리고 `intent=supersede`에서 활성 포인터를 옮길 때 필요한 같은 프로젝트의 열린 `superseding_task_id`가 없거나 유효하지 않은 경우처럼 그 종료 전이를 무효로 만드는 조건으로 제한합니다.

사전 확인이 성공한 뒤 `intent=complete`, `intent=cancel`, `intent=supersede`에 `dry_run=true`가 있으면 요청이 그 밖에는 유효하고 미리보기 가능할 때 `ToolDryRunResponse`를 반환합니다. 이 미리보기는 종료 또는 커밋된 닫기 차단 예상 효과, 예상되는 닫기 준비 상태 평가 발견 사항, 다음 행동을 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로 보여 줄 수 있으며 `source_kind=close_readiness`를 사용합니다. 하지만 실제 `CloseReadinessBlocker` 객체를 반환하지 않고, Task 생명주기, 닫기 필드, 차단 사유, 이벤트, 재실행 행, 아티팩트, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version`을 바꾸지 않습니다.

사전 확인이 성공한 `dry_run=false` 상태 변경 닫기 intent에서 Core는 종료 전이를 커밋하거나, 메서드 상태 효과 계약이 허용하는 커밋된 닫기 차단 결과를 반환합니다. `CloseTaskResult(close_state=blocked)`는 사전 확인이 성공한 뒤 발견한 의미적 닫기 차단 사유에 대해서만 `blockers: CloseReadinessBlocker[]`를 반환합니다. 오래된 상태, 오래된 권한 근거, `idempotency_key`의 `request_hash` 충돌, 읽을 수 없는 Core 상태, 요청 식별 정보 실패, 닫기 준비 상태 평가 전 로컬 접근/역량 실패에 대한 응답이 아닙니다. 커밋된 닫기 차단 결과는 Task를 열린 상태로 둡니다.
...
| `CloseTaskResult.close_state` | 응답 수준의 닫기 상태입니다. 값은 `ready`, `blocked`, `closed`, `cancelled`, `superseded`입니다. 지속 저장되는 생명주기 필드가 아닙니다. |
...
| `Task.result` | 작업의 굵은 결과입니다. 값은 `none`, `advice_only`, `completed`, `cancelled`, `superseded`입니다. 실패한 Run, violation, 차단된 닫기, 증거 공백은 Run 상태, 닫기 준비 상태 평가의 닫기 차단 사유 상태, 증거 상태, 현재 Task 상태에 남기며 종료 Task 결과가 아닙니다. |
...
닫기 준비 상태는 [API 값 집합](api/schema-value-sets.md)이 담당하는 활성 `CloseReadinessBlocker.category` 값만 사용합니다.

| 활성 범주 | Core 의미 |
|---|---|
| `task` | Project/Task 식별 정보 사전 확인이 성공한 뒤에도 선택된 Task가 요청한 전이와 호환되지 않거나, 이미 종료되었거나, 그 밖의 이유로 사용할 수 없는 상태입니다. |
...
| `write_compatibility` | 최신성이 확정된 뒤 현재 상태 전제에서 남는 의미적 차단 사유입니다. 필요한 Write Authorization 또는 제품 쓰기 Run 호환성이 없거나, 사용할 수 없거나, 유효하지 않거나, 이미 소비되었거나, 범위/경로/baseline/관찰된 쓰기와 맞지 않는 경우를 다룹니다. 오래된 `WriteAuthorization.basis_state_version`은 다루지 않습니다. |
...
| `recovery` | 현재 상태를 읽을 수 있고 사전 확인이 성공한 뒤에도 해결되지 않은 수리 또는 복구 상태가 남아 있어 전이 전에 처리해야 합니다. `idempotency_key`의 `request_hash` 충돌이나 상태 버전 충돌은 다루지 않습니다. |
...
- 저장소 테이블, DDL, Runtime Home 배치, 잠금, 마이그레이션, 아티팩트 저장소, enum 강화: [저장소 기록](storage-records.md), [저장 효과](storage-effects.md), [아티팩트 저장소](storage-artifacts.md), [저장소 버전 관리](storage-versioning.md).
- Projection 최신성, 읽는 시점의 보기, 읽기 전용 표시 경계: [Projection과 템플릿 참조](projection-and-templates.md). 정확한 활성 렌더링 템플릿 본문: [템플릿 본문](template-bodies.md).
...
- 커넥터 `capability_profile`과 접점별 대체 동작: [에이전트 통합 참조](agent-integration.md).
