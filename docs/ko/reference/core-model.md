...
`intent=check`는 읽기 전용입니다. 응답에 현재 complete 준비 상태 행렬, 닫기 차단 사유, 증거 요약, 아티팩트 참조, 다음 행동을 계산해 담을 수 있지만 차단 사유를 저장하거나, 이벤트를 추가하거나, 재실행 행을 만들거나, 닫기 상태를 바꾸거나, `project_state.state_version`을 올리면 안 됩니다.
