...
| 활성 method side effect | [Lifecycle and transitions](#lifecycle-and-transitions), [`prepare_write`](#prepare_write), [`record_run`](#record_run), [`close_task`](#close_task) | Method별 API/Core/storage/error 매트릭스는 [MVP API: 활성 MVP 전이 매트릭스](api/mvp-api.md#활성-mvp-전이-매트릭스)에 있습니다. |
| 닫기 의미 | [`close_task`](#close_task), [Active MVP-1 작업 모양별 닫기 matrix](#active-mvp-1-작업-모양별-닫기-matrix), [Close result semantics](#close-result-semantics) | Public close response는 [`harness.close_task`](api/mvp-api.md#harnessclose_task)이 담당합니다. |
...
### Evidence Summary와 Evidence Manifest

활성 MVP-1에서는 Core가 소유한 `evidence_summary`가 status와 close에 쓰이는 active evidence contract입니다. Close-relevant claim, criterion, completion condition을 supporting Run, blocker, user judgment, `ArtifactRef` value에 mapping합니다. Evidence sufficiency는 artifact 개수가 아니라 criterion coverage로 판단합니다.

Full Evidence Manifest는 owner profile이 활성화하기 전까지 later/profile material입니다. 활성화된 뒤에는 Run, artifact, Eval, Manual QA record, design record, 기타 owner record를 참조할 수 있습니다. 그래도 final acceptance, residual-risk acceptance, close를 대체하지 않습니다.
...
Minimum MVP-1에서 residual-risk state는 `ResidualRiskSummary`, close-relevant `blocker` ref, residual-risk acceptance용 `user_judgment` ref로 표현합니다. Rich Residual Risk record는 알려진 남은 불확실성, 한계, 확인하지 못한 조건, trade-off를 위한 later/profile material입니다. 그 profile이 활성화되면 source refs, affected scope, visibility, accepted-risk metadata, follow-up, close impact를 기록합니다.
...
Reconcile Item은 human-editable 또는 generated/projection drift를 위한 later/profile candidate record입니다. Reconcile은 해당 owner path가 active일 때만 merge, reject, convert to note, user judgment create, defer를 할 수 있습니다. Markdown이나 generated text는 accepted reconcile/owner path를 통해서만 state가 됩니다.
...
Capability는 first-class Kernel gate가 아닙니다. Surface capability는 blocked reason, validator result, guarantee display에 영향을 줄 수 있습니다. Cooperative/detective surface는 proven guard가 operation을 cover하지 않는 한 preventive blocking을 주장하면 안 됩니다. Required observation, capture path, local access, pre-tool blocking claim, isolation claim, 그 밖의 capability가 unsupported이면 Core는 그 write, status, close claim을 verified로 취급하지 않습니다. 대신 `CAPABILITY_INSUFFICIENT` 또는 equivalent structured blocker로 라우팅해야 합니다.
...
Public request input, Core owner, state-version basis, idempotency replay, storage row, event, response ref, dry-run behavior, error, status/close blocker impact를 묶은 활성 MVP method별 계약은 [MVP API: 활성 MVP 전이 매트릭스](api/mvp-api.md#활성-mvp-전이-매트릭스)를 사용합니다. 아래 Core transition section은 그 매트릭스 뒤의 state logic을 설명하며, active side effect를 그보다 넓히지 않습니다.
...
`missing required authorization`은 product-write Run에 required authorization ref가 없거나 current matching row가 없다는 뜻이며 `WRITE_AUTHORIZATION_REQUIRED`로 매핑됩니다. `stale authorization`은 row가 expired, revoked, already consumed, explicitly stale이거나 attempted write에 대해 더 이상 current가 아닌 state version, baseline, scope, surface, related judgment set을 기준으로 한다는 뜻이며 `WRITE_AUTHORIZATION_INVALID`로 매핑됩니다. `observed attempt outside authorized scope`는 observed paths, tools, commands, command classes, product-file-write behavior, network targets, secret handles, sensitive categories, baseline, Task, Change Unit, surface가 stored `AuthorizedAttemptScope`를 넘거나 다르다는 뜻이며 `SCOPE_VIOLATION`으로 매핑됩니다. `insufficient surface capability`는 connected surface에 필요한 observation, capture, blocking, isolation capability가 없어서 Core가 attempted scope의 required part를 정직하게 비교할 수 없다는 뜻이며 `CAPABILITY_INSUFFICIENT`로 매핑됩니다.

Out-of-scope change, missing Write Authorization, expired Write Authorization, stale Write Authorization, revoked Write Authorization, consumed Write Authorization, incompatible Write Authorization, insufficient surface capability는 case에 따라 rejection, violation, recovery, stale/blocker state가 됩니다. Core는 invalid authorization을 successfully consumed로 기록하면 안 됩니다. Active contract가 observed behavior 기록을 지원할 때 violation 또는 audit Run을 기록할 수는 있지만, completion evidence로 count하지 않습니다. Relevant owner record를 통해 repair되기 전까지 affected scope의 evidence sufficiency, final acceptance, residual-risk acceptance, close readiness 또는 later/profile QA/verification requirement를 만족하지 않습니다. Attempted authorization ref는 validator finding, violation payload, event payload에만 나타날 수 있습니다.

Pre-commit rejected `record_run`에서는 위의 명시적 committed violation/audit 예외가 적용되지 않는 한 Core가 Run, artifact, artifact link, evidence summary, authorization consumption, blocker/gate update, task event, projection job, state-version advance, replay row를 만들지 않습니다.
...
`close_task`의 failure, dry-run, idempotency, row mutation, response-ref behavior는 [MVP API: 활성 MVP 전이 매트릭스](api/mvp-api.md#활성-mvp-전이-매트릭스)가 제한합니다. 특히 `dry_run=true`는 would-close diagnostic을 반환할 수 있지만 Task를 terminal로 표시하거나 close blocker를 만들거나 업데이트하거나 close event를 append하거나 projection job을 enqueue하거나 state version을 advance하거나 replay state를 만들면 안 됩니다.
