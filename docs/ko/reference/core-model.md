...
흔한 차단 사유에는 활성 Task 없음, 활성 범위 없음, 범위 밖 쓰기 의도, 해결되지 않은 사용자 소유 판단, 민감 동작 승인 없음, 맞지 않는 Autonomy Boundary, 부족한 접점 역량, 없거나 유효하지 않은 Write Authorization, 오래된 baseline, 증거 없음, 오래됐거나 사용할 수 없는 아티팩트 지원, 최종 수락 없음, 숨겨진 잔여 위험, 닫기에 영향을 주는 수락되지 않은 잔여 위험, 안전하지 않은 열린 Run, 복구 제약, `cancellation` 충돌, `supersession` 충돌이 있습니다.
