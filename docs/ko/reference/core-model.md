...
오래된 프로젝트 전체 state_version은 커밋 전에 `STATE_VERSION_CONFLICT`를 담은 `ToolRejectedResponse`를 반환합니다. `STATE_VERSION_CONFLICT`는 `PrepareWriteResult.decision` 값이 아니며, Write Authorization, 차단 사유 커밋, 재실행 행, 증거 기록, 닫기 상태, `state_version` 증가를 만들지 않습니다. 유효한 `dry_run=true` 호출은 `ToolDryRunResponse`를 반환합니다. non-dry-run 경로를 미리 보여 줄 수 있지만 Write Authorization을 만들지 않고 실제 `write_authorization_ref`를 반환하지 않습니다.
...
응답 분기 정체성은 닫기 권한의 일부입니다. `intent=check`는 `effect_kind=read_only`인 읽기 전용 `MethodResult`로 `CloseTaskResult`를 반환합니다. 커밋된 차단 결과인 `intent=complete` 응답은 Core가 메서드 상태 효과 계약에 따라 닫기 행렬 결과를 커밋할 때만 `close_state=blocked`인 `CloseTaskResult`를 반환하며, Task는 열린 상태로 남습니다. 닫기 행렬 커밋 전 선행조건 실패는 성공처럼 생긴 `CloseTaskResult`가 아니라 `effect_kind=no_effect`인 `ToolRejectedResponse`를 반환합니다. 상태를 바꾸는 닫기 intent에서 유효한 `dry_run=true` 호출은 `ToolDryRunResponse`를 반환합니다. 종료 또는 차단된 닫기 예상 효과를 미리 보여 줄 수 있지만 Task 생명주기, 닫기 필드, 차단 사유, 이벤트, 재실행 행, `project_state.state_version`을 바꾸지 않습니다.
