...
- 렌더링된 projection body나 template text. [Projection과 Template 참조](projection-and-templates.md)와 [Template 참조](templates/README.md)를 봅니다.
- Connector capability profile과 surface recipe. [Agent 통합 참조](agent-integration.md)와 [Surface Cookbook](surface-cookbook.md)을 봅니다.
...
2. 하네스는 하네스 기록과 상태 전이를 다룹니다. OS permission, 임의 도구 실행, 권한 격리를 제공하지 않습니다.
...
5. Write Authorization은 호환되는 attempt 하나에 한 번만 쓰입니다. 재사용 가능한 범위도 아니고 OS permission도 아닙니다.
...
7. 사용자가 소유하는 판단은 agent 추론, 포괄적 동의, generated prose, evidence, projection text로 대체될 수 없습니다.
...
11. 현재 활성 MVP 범위와 later/profile 자료는 분리됩니다. Later candidate는 owner가 scope, fallback behavior, proof expectation과 함께 승격할 때만 active가 됩니다.
...
- Chat, generated Markdown, projection prose, report text는 Core state를 대신하지 않습니다.
- Evidence, log, screenshot, artifact, test output은 최종 수락, 수동 QA, 검증, 잔여 위험 수락을 대신하지 않습니다.
- QA는 최종 수락이 아닙니다. QA 면제 판단은 QA evidence나 QA pass가 아닙니다.
- 검증 위험 수락은 verification, detached verification, assurance upgrade가 아닙니다.
- 민감 동작 승인은 제품 방향, 기술 방향, 범위, correctness, evidence, QA, 최종 수락, 잔여 위험 수락, Write Authorization을 대신하지 않습니다.
- 제품 판단, 기술 판단, 범위 판단은 서로를 대신하지 않습니다.
- 최종 수락은 evidence를 만들거나, evidence gap을 지우거나, QA를 면제하거나, verification을 증명하거나, 민감 동작 승인을 부여하거나, scope를 바꾸거나, 잔여 위험을 수락하거나, blocker를 override하지 않습니다.
- 잔여 위험 수락은 work를 verify하거나, no-risk close를 만들거나, evidence를 만족하거나, QA를 만족하거나, 최종 수락을 암시하지 않습니다.
...
Write Authorization은 협력형 하네스 기록입니다. Connected agent나 surface에게 intended write가 현재 하네스 상태와 호환된다고 알려줄 수 있습니다. OS permission을 주거나, sandbox를 강제하거나, arbitrary tool을 막거나, storage를 tamper-proof하게 만들거나, operation을 격리하지 않습니다.

MCP 또는 connected surface가 필요한 cooperative check를 수행할 수 없으면 정직한 결과는 hold, blocker, degraded guarantee display, capability error 중 하나입니다. Preventive 또는 isolated wording은 해당 operation을 cover하는 정확한 boundary가 문서화되고 증명되었을 때만 사용할 수 있습니다.
