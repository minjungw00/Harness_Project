...
`close_task`는 먼저 커밋 전 실패를 가르는 사전 확인을 수행합니다. 이 단계는 `CloseBlocker` 생성, 닫기 차단 사유 행렬 평가, 재실행 행 생성, Write Authorization 소비, 스테이징된 핸들 소비보다 앞섭니다. 실패하면 `effect_kind=no_effect`인 `ToolRejectedResponse` 거절 응답을 반환하며, `CloseTaskResult`나 `CloseTaskResult(close_state=blocked)`가 아닙니다. `close_task` 사전 확인의 거절 항목은 다음입니다.
...
- 닫기 차단 사유 행렬 평가 전에 Core 상태를 읽을 수 없는 경우.
- 유효한 Project/Task를 선택하기 전에 요청 식별 정보가 실패한 경우.
- 닫기 차단 사유 행렬 평가 전에 로컬 접근 또는 역량 확인이 실패한 경우.

모든 `close_task` 사전 확인 거절은 상태 효과 없음입니다. `CloseBlocker` 없음, `task_event` 또는 `task_events` 추가 없음, 재실행 행 없음, `tool_invocations.response_json` 없음, `close_state` 변경 없음, Write Authorization 생성 없음, Write Authorization 소비 없음, 스테이징된 핸들 소비 없음, 아티팩트 승격 또는 연결 없음, 증거 요약 갱신 없음, `project_state.state_version` 증가 없음입니다. `STATE_VERSION_CONFLICT`는 `close_task`의 사전 확인 거절에만 속합니다. `CloseBlocker.code`, `write_compatibility` 또는 `recovery` 행렬 행, 커밋된 닫기 차단 결과에 넣으면 안 됩니다.

사전 확인이 성공하면 `intent=check`는 읽기 전용 닫기 차단 사유 행렬 평가입니다. 응답에만 현재 complete 준비 상태 행렬, 닫기 차단 사유, 증거 요약, 아티팩트 참조, 다음 행동을 계산해 담을 수 있습니다. 요청에 `dry_run=true`가 함께 있어도 응답 분기는 `CloseTaskResult`로 유지되며 `base.dry_run=true`, `base.effect_kind=read_only`를 사용합니다. 두 경우 모두 상태 효과 없음: `close_state` 변경 없음, `task_events` 없음, 재실행 행 없음, 아티팩트 갱신 없음, 스테이징된 핸들 소비 없음, Write Authorization 생성 없음, Write Authorization 소비 없음, `project_state.state_version` 증가 없음입니다.

사전 확인이 성공하면 `intent=complete`는 결정적인 complete 닫기 차단 사유 행렬을 사용합니다. Core는 차단 사유를 둘 이상 모을 수 있지만, 응답 순서와 기본 닫기 차단 사유의 근거는 아래 행렬을 따릅니다.

| 순서 | 확인 | 실패했을 때의 차단 결과 |
|---:|---|---|
| 1 | 선택된 Task 생명주기 유효성 | 선택된 같은 프로젝트의 Task가 이미 종료되었거나 complete 전이에 들어갈 수 없으면 `task` 차단 사유를 반환합니다. Project/Task 식별 정보가 없거나, 읽을 수 없거나, 다른 프로젝트에 속하면 사전 확인 거절이며 닫기 차단 사유가 아닙니다. |
...
| 7 | baseline과 접점 역량 확인 | 로컬 접근 사전 확인이 성공한 뒤에도 baseline이나 확인된 로컬 접점 역량이 닫기 주장 또는 필요한 보장 표시를 정직하게 뒷받침할 수 없으면 `baseline` 또는 `surface_capability` 차단 사유를 반환합니다. 현재 로컬 접근 실패는 사전 확인 거절이며 닫기 차단 사유가 아닙니다. |
...
`idempotency_key`의 `request_hash` 충돌, 오래된 `expected_state_version`, 오래된 `WriteAuthorization.basis_state_version`은 complete 닫기 차단 사유 행렬 밖에 있습니다. 모두 사전 확인 거절이며 의미적 차단 사유가 아닙니다.
...
사전 확인이 성공한 뒤 `intent=complete`, `intent=cancel`, `intent=supersede`에 `dry_run=true`가 있으면 요청이 그 밖에는 유효하고 미리보기 가능할 때 `ToolDryRunResponse`를 반환합니다. 이 미리보기는 종료 또는 커밋된 닫기 차단 예상 효과, 예상 닫기 차단 사유, 다음 행동을 `DryRunSummary.would_blockers`의 `PlannedBlocker` 항목으로 보여 줄 수 있습니다. 하지만 실제 `CloseBlocker` 객체를 반환하지 않고, Task 생명주기, 닫기 필드, 차단 사유, 이벤트, 재실행 행, 아티팩트, 스테이징된 핸들 소비, Write Authorization 생성 또는 소비, `project_state.state_version`을 바꾸지 않습니다.

사전 확인이 성공한 `dry_run=false` 상태 변경 닫기 intent에서 Core는 종료 전이를 커밋하거나, 메서드 상태 효과 계약이 허용하는 커밋된 닫기 차단 결과를 반환합니다. `CloseTaskResult(close_state=blocked)`는 커밋된 닫기 차단 결과에만 쓰며, 이는 사전 확인이 성공한 뒤 발견한 의미적 차단 사유를 뜻합니다. 오래된 상태, 오래된 권한 근거, `idempotency_key`의 `request_hash` 충돌, 읽을 수 없는 Core 상태, 요청 식별 정보 실패, 닫기 차단 사유 행렬 평가 전 로컬 접근/역량 실패에 대한 응답이 아닙니다. 커밋된 닫기 차단 결과는 Task를 열린 상태로 둡니다.
