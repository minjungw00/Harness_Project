...
`allowed`는 `prepare_write` decision이지 durable lifecycle status가 아닙니다. `blocked`도 Write Authorization lifecycle status가 아닙니다. 막힌 쓰기, 민감 동작 승인이 필요한 쓰기, 사용자 판단이 필요한 쓰기, state conflict가 있는 쓰기는 consumable authorization row를 만들지 않습니다. Core는 이를 `prepare_write.decision`, blocker, validator finding, error로 표현합니다.
...
11. 모든 required check가 pass하면 `decision=allowed`를 반환하고, compatible active single-use Write Authorization record를 만들거나 반환하며, response/idempotency path로 decision을 기록합니다.
...
Out-of-scope change, missing Write Authorization, expired Write Authorization, stale Write Authorization, revoked Write Authorization, consumed Write Authorization, incompatible Write Authorization은 case에 따라 rejection, violation, recovery, stale/blocker state가 됩니다. 이런 Run은 relevant owner record를 통해 repair되기 전까지 affected scope의 evidence sufficiency, verification, QA, work acceptance, residual-risk acceptance, close readiness를 만족하지 않습니다.
...
| Product write에는 explicit scope와 compatible 쓰기 전 범위 확인이 필요합니다. | `prepare_write`는 missing scope를 block하고 `decision=allowed`일 때만 active single-use Write Authorization record를 만듭니다. `record_run`이 active row를 소비합니다. |
