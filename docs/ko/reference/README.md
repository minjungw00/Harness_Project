...
| Core 권한, 작업 생명주기, `ShapingReadiness` 의미, 사용자 소유 제품/기술/범위/민감 동작/최종/잔여 위험/취소 판단 경계, 최종 수락/잔여 위험 수락 대체 불가, 활성 gate 의미, `CompletionPolicy` 닫기 영향, `EvidenceSummary` 닫기 영향, 닫기 차단 사유 행렬 순서, `write_compatibility`와 `recovery`를 포함한 의미적 `CloseBlocker` 뜻, waiver, 잔여 위험 | [core-model.md](core-model.md) |
| 활성 공개 API 메서드별 동작, 메서드별 응답 공용체와 `MethodResult` 분기, 메서드 상태 효과, 확인된 로컬 접점 요청 조건, 요청 수준 `VerifiedSurfaceContext.access_class`, `harness.update_scope`, `harness.prepare_write`가 Write Authorization에 미치는 효과, `artifact_registration`으로 처리하는 `harness.stage_artifact`, `run_recording`으로 처리하는 `harness.record_run`, 스테이징된 아티팩트 승격, `artifact_read`로 처리하는 아티팩트 본문 읽기, `harness.close_task` 응답 분기와 닫기 차단 사유 행렬 계약 | [api/mvp-api.md](api/mvp-api.md) |
| 정확한 활성 메서드 이름 집합, `ToolEnvelope.expected_state_version`, `ToolResultBase`, `ToolRejectedResponse`, `ToolDryRunResponse`를 포함한 공통 응답 분기, 공통 `ToolError`와 `EventRef` 형태, `response_kind`, `effect_kind`, `LocalSurfaceRegistration`, `VerifiedSurfaceContext.access_class`, `run_recording`, `artifact_registration`, `artifact_read`를 포함한 활성 접근 분류 값, `StagedArtifactHandle`, `existing_artifact`를 포함한 `ArtifactInput`, `CompletionPolicy`, `EvidenceSummary`, `SensitiveActionScope`, 제품 파일 쓰기 범위인 `AuthorizedAttemptScope`, 닫기 차단 사유 스키마, `ShapingReadiness` 필드, 활성 enum/값 집합, 표시 라벨 경계, `GuaranteeDisplay.level` 값 | [api/schema-core.md](api/schema-core.md) |
| 공개 `ErrorCode` 계약, 오류 우선순위, 로컬 접점 오류, `STATE_VERSION_CONFLICT`, 사전 확인 거절 의미, 거절/차단/`dry_run` 오류 경계, 커밋된 차단 결과 경계, `close_task` 차단 사유의 공개 오류 매핑 | [api/errors.md](api/errors.md) |
| 저장소, DDL, 단일 공개 프로젝트 전체 상태 시계인 `project_state.state_version`, `surfaces`, `write_authorizations`, 스테이징된 아티팩트 저장소, 스테이징된 아티팩트 승격의 지속 효과, `existing_artifact` / `ArtifactRef` 영속 연결, 아티팩트 본문 읽기 가능성의 저장소 근거, 지속되는 증거 요약 행, 멱등성, 거절된 닫기 효과와 커밋된 차단 닫기 효과의 차이, 마이그레이션 | [storage.md](storage.md) |
