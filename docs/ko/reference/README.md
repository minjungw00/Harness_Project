...
이 색인은 하네스 참조 질문에서 다음 담당 문서를 고를 때 쓰는 읽기용 색인입니다. 정확한 machine-readable 담당 경로는 [`docs/doc-index.yaml`](../../doc-index.yaml)을 사용합니다. 이 파일이 `doc_id`, 대응 경로, 역할, 담당 범위, 의존 관계, 규범 수준, 독자 메타데이터를 담당합니다.
