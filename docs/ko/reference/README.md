...
이 색인은 하네스 참조 질문에서 다음 담당 문서를 고를 때 쓰는 읽기용 색인입니다. 기계가 읽는 정확한 담당 경로는 [`docs/doc-index.yaml`](../../doc-index.yaml)을 사용합니다. 이 파일이 `doc_id`, 유지 경로, 문서 종류, 집중 `canonical_for` 범위, 유지보수 `owner_area`, `created_on`, `last_updated_on`, `last_verified_on`, `applies_to`, 의존 관계, 규범 수준, 주요 독자, 독자 여정, 번역 정책 메타데이터를 담당합니다.
