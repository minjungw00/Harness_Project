...
`dry_run` 응답 분기 선택은 [API Schema Core](api/schema-core.md), [MVP API](api/mvp-api.md), [API Errors](api/errors.md)가 담당합니다. `ToolDryRunResponse`는 Core 커밋이나 스테이징 부작용을 만들 수 있는 선택 동작의 유효한 `dry_run` 미리보기 응답이지, 모든 `dry_run=true` 요청의 응답은 아닙니다. `harness.status dry_run=true`와 `harness.close_task intent=check dry_run=true` 같은 읽기 전용 선택 동작은 `base.dry_run=true`, `effect_kind=read_only`인 실제 메서드 결과, 즉 `StatusResult`나 `CloseTaskResult`를 반환합니다.
...
| Core 권한, 작업 생명주기, `ShapingReadiness` 의미, 사용자 소유 제품/기술/범위/민감 동작/최종/잔여 위험/취소 판단 경계, 최종 수락/잔여 위험 수락 대체 불가, 활성 gate 의미, `CompletionPolicy` 닫기 영향, `EvidenceSummary` 닫기 영향, `prepare_write` 판단 평가와 `close_task` close 가능성 평가의 분리, close 가능성 평가 순서, `write_compatibility`와 `recovery`를 포함한 `CloseReadinessBlocker` 닫기 차단 사유 의미, waiver, 잔여 위험 | [core-model.md](core-model.md) |
| 활성 공개 API 메서드별 동작, 메서드별 응답 공용체와 `MethodResult` 분기, 메서드 상태 효과, 확인된 로컬 접점 요청 조건, 요청 수준 `VerifiedSurfaceContext.access_class`, `harness.update_scope`, `harness.prepare_write`가 Write Authorization에 미치는 효과와 `PrepareWriteResult.write_decision_reasons`, `artifact_registration`으로 처리하는 `harness.stage_artifact`, `run_recording`으로 처리하는 `harness.record_run`, 스테이징된 아티팩트 승격, `artifact_read`로 처리하는 아티팩트 본문 읽기, `StatusResult.close_blockers`를 포함한 `harness.status` 응답 필드, `harness.close_task` 응답 분기와 `CloseTaskResult.blockers`, dry-run 응답 필드 | [api/mvp-api.md](api/mvp-api.md) |
| 정확한 활성 메서드 이름 집합, `ToolEnvelope.expected_state_version`, `ToolResultBase`, `ToolRejectedResponse`, `ToolDryRunResponse`를 포함한 공통 응답 분기, 공통 `ToolError`와 `EventRef` 형태, `ToolError`, `WriteDecisionReason`, `CloseReadinessBlocker`, `PlannedBlocker` 같은 차단 사유형 스키마 타입의 소유권과 데이터 구조/응답 효과 분리, `response_kind`, `effect_kind`, `LocalSurfaceRegistration`, `VerifiedSurfaceContext.access_class`, `run_recording`, `artifact_registration`, `artifact_read`를 포함한 활성 접근 분류 값, `StagedArtifactHandle`, `existing_artifact`를 포함한 `ArtifactInput`, `CompletionPolicy`, `EvidenceSummary`, `SensitiveActionScope`, 제품 파일 쓰기 범위인 `AuthorizedAttemptScope`, `ShapingReadiness` 필드, 활성 enum/값 집합, 표시 라벨 경계, `GuaranteeDisplay.level` 값 | [api/schema-core.md](api/schema-core.md) |
| 공개 `ErrorCode` 계약, 오류 우선순위, 로컬 접점 오류, `STATE_VERSION_CONFLICT`, 사전 확인 거절 의미, `ToolRejectedResponse.errors: ToolError[]`에 담는 커밋 전 실패 오류, 거절/차단/`dry_run` 오류 경계, `WriteDecisionReason`, `CloseReadinessBlocker`, `PlannedBlocker` 코드 제외 규칙, 커밋된 차단 결과 경계, `close_task` 차단 사유의 공개 오류 매핑 | [api/errors.md](api/errors.md) |
| 저장소, DDL, 단일 공개 프로젝트 전체 상태 시계인 `project_state.state_version`, `surfaces`, `write_authorizations`, 스테이징된 아티팩트 저장소, 스테이징된 아티팩트 승격의 지속 효과, `existing_artifact` / `ArtifactRef` 영속 연결, 아티팩트 본문 읽기 가능성의 저장소 근거, 지속되는 증거 요약 행, 멱등성, 읽기 전용 관찰, dry-run 미리보기, 커밋된 차단 결과 사이의 저장 효과 차이, close 가능성 평가에서 나온 닫기 차단 사유의 데이터 구조/지속 저장 분리, 마이그레이션 | [storage.md](storage.md) |
...
| 문서 작성 규칙, 담당 문서 경계 위생, active/later 점검, 한국어 품질, 의미 일치, 공개 오류 코드, `access_class`, 아티팩트 생명주기 일관성 문서 점검, 번역 규칙 | [작성 가이드](../maintain/authoring-guide.md), [번역 가이드](../maintain/translation-guide.md), [문서 점검](../maintain/checks.md) |
