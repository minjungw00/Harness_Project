...
Projection은 읽기용 파생 view입니다. Core 소유 상태와 아티팩트 참조에서 생성되며 current state, ref, freshness, proposed edit를 표시할 수 있지만 Core 소유 상태를 대체하지 않습니다. 사람이 projection을 편집해도 future Core/reconcile path가 state-changing action으로 받아들이기 전까지는 상태 변경이 아닙니다.
...
이 산출물은 template shape를 재사용할 수 있지만, template 종류가 구현 범위를 키우면 안 됩니다. Core state와 ref에서 파생된 하나의 간결한 status/next surface와 명확한 판단 요청 display만으로도 MVP 표시 경로를 충족할 수 있습니다.
