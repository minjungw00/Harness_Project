...
| Approval | `approvals`, Approval 형태 Decision Packet, 구현이 유지하는 경우 optional decision request routing/replay record, event; `approval_request_candidate`만 있는 경우는 제외 | `APR` projection과 Approval Card | `request_user_judgment(judgment_route=approve-sensitive-action)`이 pending Approval 기록을 만들고, `record_user_judgment`가 Approval judgment를 업데이트한 뒤 projector |
...
| `APR` | `request_user_judgment(judgment_route=approve-sensitive-action)`이 커밋된 Approval request를 create하거나, `record_user_judgment`를 통해 Approval judgment가 changed될 때 | Approval 형태 Decision Packet, 연결된 Approval 기록 status, scope, baseline, expiry, judgment note가 changed |
