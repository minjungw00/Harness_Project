...
| Autonomy Boundary | active `state.sqlite.change_units` Autonomy Boundary field와 관련 Decision Packet 해소/event | `TASK` Autonomy Boundary, Change Unit block, Journey Card autonomy line, standalone projection이 enabled일 때 optional related `DEC` | shaping update 또는 user Decision Packet 해소, then projector |
...
- Missing 또는 hash-mismatched artifact는 related evidence 또는 projection 최신성을 stale로 표시합니다.
