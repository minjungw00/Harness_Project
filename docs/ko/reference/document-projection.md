...
| First User-Value Slice projection | [Compact Status Card](templates/compact-status-card.md) | v0.2 projection shape는 하나의 compact card입니다. 무엇을 하는지, 범위/하지 않을 일, 대기 중인 사용자 판단, 근거/gap, close blocker, 보이는 잔여 위험, 다음 안전한 행동, source/freshness ref를 보여줍니다. 작업 수락과 잔여 위험 사실은 관련 있을 때 distinct하게 남지만 필수 projection kind를 늘리지는 않습니다. |
