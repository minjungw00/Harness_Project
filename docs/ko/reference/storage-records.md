...
| `state.sqlite` | `user_judgments` | 사용자 소유 판단 상태 | 근거 스냅샷, 근거 상태, 필요한 경우 민감 동작 승인 범위를 포함하는 대기, 해결됨, 오래됨, 대체됨, 레거시 미결속 사용자 소유 판단. |
...
저장된 근거가 없는 판단은 감사를 위해 `legacy_unbound`로 보존됩니다. 이 판단은 주소 지정 가능한 이력 판단 기록으로 남지만 현재 닫기, 쓰기, 민감 승인 요구사항을 만족할 수 없습니다.
...
| `user_judgments` | 판단 요청, 맥락, 선택지, 영향 참조, 아티팩트 참조, 근거 스냅샷, 민감 동작 승인 범위, 해결 데이터. |
