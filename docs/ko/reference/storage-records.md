...
- 이 문서는 기준 범위 원천 설계에서 저장소 기록의 영속 범주와 저장 배치 기대를 담당합니다.
...
- 기준 범위 영속 기록 범주와 테이블 수준 저장 역할.
- 저장소 설계를 위한 기록 열 의미.
- 저장소 소유 기록 값과 상태 필드.
- 저장소가 소유하는 JSON `TEXT` 배치와 검증 기대.
- 기록 수준의 기준 범위/지원 범위 밖 제외 경계.
...
- 런타임 홈과 제품 저장소 경계: [런타임 경계](runtime-boundaries.md)

## 저장소 기록과 API 스키마

저장소 기록과 API 스키마는 서로 다른 담당 문서 경계를 가집니다.

- API 스키마 파일은 요청/응답 데이터 형태, 공개 API 값, 공개 오류, 응답 분기를 정의합니다. [API 코어 스키마](api/schema-core.md), [API 상태 스키마](api/schema-state.md), [API 아티팩트 스키마](api/schema-artifacts.md), [API 판단 스키마](api/schema-judgment.md), [API 값 집합](api/schema-value-sets.md)을 봅니다.
- 저장소 기록은 영속 기록 범주, 파일/테이블 배치, 저장소가 소유하는 JSON `TEXT` 위치, 저장 관계, 검증 기대를 정의합니다.
- 같은 이름이 양쪽에 보여도 같은 권한이 아닙니다. `CloseReadinessBlocker`는 API 데이터 형태이고, `blockers`는 저장 행입니다. `ArtifactRef`는 API 스키마이고, `artifacts`와 `artifact_links`는 저장 기록입니다.
- 저장 배치는 렌더링된 템플릿 문구를 소유하지 않습니다. 사용자에게 보이는 상태 카드, 판단 요청, 실행/증거 요약, 닫기 결과, 에이전트 맥락 패킷은 [템플릿 본문](template-bodies.md)과 [상태 보기 권한 참조](projection-and-templates.md)가 담당합니다.
...
- 생성된 상태 보기 본문, 템플릿 표시 문구, 확장된 증거 패키지 본문, QA 기록, 수락 기록, 잔여 위험 기록, 닫기 기록은 이 저장소 기록 배치에 속하지 않습니다.
...
- 기준 범위는 지원되는 상태 변경 메서드 집합에 필요한 Core 영속 기록만 저장합니다.
...
기준 범위 Core 영속 기록은 다음뿐입니다.

- `registry.sqlite`의 런타임 홈 식별 정보.
- `registry.sqlite`의 최소 프로젝트 등록.
- `project.yaml`의 정적 프로젝트 설정.
- `project_state`.
- `surfaces`. 단, 지원되는 API 요청 래퍼, 기능 표시, 로컬 접근 상태에 필요한 등록된 로컬/참조 접점 사실로 제한합니다.
- `tasks`.
- `change_units`.
- `user_judgments`.
...
- `artifact_staging` 또는 동등한 저장소 소유 스테이징 기록.
- `artifacts/tmp/` 아래 안전한 임시 바이트 또는 알림.

담당 문서 링크:

- 스테이징 생명주기, 출처, 소비, 승격은 [아티팩트 저장소](storage-artifacts.md)가 담당합니다.

### 저장되지 않는 것

허용되지 않는 것:

- 그 밖의 영속 테이블 계열이나 임시 핸들 계열은 기준 범위가 아닙니다.
- 요구사항 구체화는 위에 나열한 기준 범위 기록 계열을 통해서만 저장되며 별도 요구사항 발견 기록을 만들지 않습니다.
- 증거는 기준 범위의 간결 증거 요약과 아티팩트 참조 경로를 통해서만 저장합니다.

### 상태 보기 비주장

허용되지 않는 것:

- 상태 보기는 기준 범위 영속 테이블 계열이 없습니다.
- `status-card`, `judgment-request`, `run-evidence-summary`, `close-result`, `agent-context-packet`은 저장된 상태나 저장소 변경 경로가 아닙니다.

허용되는 것:

- `status-card`, `judgment-request`, `run-evidence-summary`, `close-result`, `agent-context-packet`은 기준 범위 기록 위에서 읽는 시점에 만든 보기입니다.

### 구체화 저장

허용되는 것:

- 요구사항 구체화는 `tasks`, `change_units`, `user_judgments`, `evidence_summaries`, `blockers`를 통해 지속됩니다.
- 증거는 간결한 증거 요약, `Task` 또는 Change Unit의 `CompletionPolicy`, 필수 범위 항목, 아티팩트 참조를 통해 지속됩니다.

최소 기준 범위 구체화 정보도 기존 기록에 저장합니다. 여기에는 아래 항목이 포함됩니다.
...
- 빠졌거나 알 수 없는 항목은 `unknown`, 대기 중인 `user_judgments`, 증거 공백, `blockers`로 남깁니다.

허용되지 않는 것:

- 저장소는 요청을 준비된 것처럼 보이게 만들려고 별도 기준 범위 계획 테이블을 만들면 안 됩니다.

## 테이블 개요

아래 표는 기준 범위의 저장 기록 범주를 간결하게 보여 주고 범주별 세부 설명으로 연결합니다. 전체 DDL이 아니며 API 스키마나 렌더링된 템플릿 본문을 복사하지 않습니다. 세부 설명은 영속 기록이 저장하는 내용과 저장하지 않는 내용을 분리합니다.
...
목적:
- 아티팩트와 그것이 뒷받침하는 지원 대상 Core/API 기록 사이의 담당 관계를 저장합니다.
...
- 이 섹션은 SQLite 첫 스키마 설계를 위한 저장 계약입니다.
...
- 이 섹션은 전체 DDL이나 마이그레이션 파일이 아닙니다.
- 구현 방식 선택이 Core/API/저장소 담당 문서의 검증을 대체하지 않습니다.

필수 식별성과 고유 제약은 다음과 같습니다.

- 기준 범위 테이블은 불투명하고 안정적인 식별자를 기본 키 또는 동등한 고유 키로 사용합니다.
...
- 프로젝트 등록에는 고유한 `project_id`와 고유한 `project_home`이 필요합니다. 담당 문서가 다중 등록 동작을 정의하기 전까지 `repo_root` 하나에는 활성 등록 하나만 둡니다.
...
- `change_units`에는 고유한 `(task_id, change_unit_id)`가 필요하며, `Task`마다 `status=active`인 Change Unit은 최대 하나입니다.
...
- `tasks.active_change_unit_id`는 같은 `Task`의 `change_units` 행을 가리킵니다.
...
- `artifact_staging.consumed_by_run_id`와 `artifact_staging.promoted_artifact_id`는 값이 있으면 소비하는 `harness.record_run` 트랜잭션이 만든 같은 프로젝트 같은 `Task`의 `runs`와 `artifacts` 행을 가리킵니다.
- `artifact_links.artifact_id`는 `artifacts`를 가리키며, `artifact_links.task_id`는 아티팩트와 담당 관계가 속한 같은 `Task`를 가리킵니다.
- `supporting_run_ids_json`, `supporting_artifact_link_ids_json`, `gap_blocker_ids_json`, `related_refs_json`, `response_json` 같은 JSON 참조 배열은 단순 미검증 텍스트 참조가 될 수 없습니다. SQLite가 직접 외래 키로 표현할 수 없는 관계라도 커밋 전에 파싱하고 같은 프로젝트/`Task`와 담당 관계를 확인해야 합니다.

### 권한 행 삭제 비주장

규칙:

- 일반적인 기준 범위 Core 동작은 권한 행을 하드 삭제하지 않습니다.
...
- `Task`를 완료, 취소, 대체해도 아래 행이 연쇄 삭제되면 안 됩니다.
  - `tasks`.
  - `change_units`.
  - `user_judgments`.
  - `write_authorizations`.
...
- 이 예외가 허용되는 이유는 소비되지 않은 스테이징 행과 임시 바이트가 증거 권한이 아니기 때문입니다.
...
- 저장 값 집합은 기준 범위의 테이블 수준 영속 제약입니다.
...
저장 규칙:
- 기준 범위에는 등록된 활성 프로젝트만 있습니다.
- 등록 해제 동작은 지원 범위 밖 기능입니다.
...
저장 규칙:
- 영속되는 상위 수준 결과입니다.
- 실패한 실행 기록, 위반, 차단된 닫기, 증거 공백은 담당 기록에 남습니다.
...
기준 범위 JSON `TEXT` 열은 기준 범위 기록에 필요한 간결한 담당 형태 데이터로 제한합니다.

- `surfaces.capability_profile_json`.
- `success_criteria_json`, `acceptance_criteria_json`, `scope_boundary_json`, `non_goals_json`, `affected_areas_json`, `affected_path_candidates_json`, `constraints_json`, `autonomy_boundary_json`, `completion_policy_json` 같은 `Task`와 Change Unit 구체화 열.
...
- `Task`와 Change Unit 구체화 JSON은 간결한 요약과 제한된 목록만 저장합니다.

허용되지 않는 것:

- 아래 항목을 다른 이름으로 저장하면 안 됩니다.
  - 별도 요구사항 발견 기록이나 계획 기록.
  - 생성된 상태 보기 본문.
  - 확장된 증거 패키지 본문.
  - QA, 수락, 잔여 위험, 닫기 기록.
...
- 지원 범위 밖 기능 묶음의 저장소는 기준 범위 밖에 있습니다.

예외:

- 담당 문서가 범위, 대체 동작, 승격에 필요한 증명 경로 기대치와 함께 좁은 동작을 승격하면 그 범위에서만 달라질 수 있습니다.

허용되지 않는 것:

- 참조 스키마에 존재한다는 사실만으로 저장소가 지원되지 않습니다.

### 저장되지 않는 것

[범위 참조](scope.md)와 저장소 담당 문서가 지원 계약을 정의하지 않는 한, 기준 범위는 지원 범위 밖 기능 묶음, 생성된 운영 출력, 확장된 증거 패키지, 호스팅 서비스, 접점 간 조율, 장기 설계 지원 기록을 위한 저장소를 제외합니다.

### 비주장

허용되는 것:

- 상태, 닫기 준비 상태, 실행/증거 요약, 다음 행동, 읽기용 카드, `agent-context-packet`, 보장 표시는 기준 범위 영속 기록 위에 읽는 시점에 파생하는 보기입니다.
