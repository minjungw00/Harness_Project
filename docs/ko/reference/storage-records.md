...
| `registry.sqlite` | Agent Connection | MCP 호스트 연결 단위 | 지속되는 `connection_id`, 호스트 종류/범위, 서버 이름, 설정 대상, 모드, 활성 상태, 관리 fingerprint, 마지막 검증 상태, 메타데이터. |
...
- [에이전트 통합](agent-integration.md): Agent Connection, Connection Projects, 모드로 제한되는 MCP 도구 접근, User Channel 경계를 정의합니다.
