...
Volicord는 기준 범위 기록을 로컬 `Volicord Runtime Home` 하나와 등록된 프로젝트별 로컬 상태 데이터베이스 하나에 저장합니다. `volicord setup`은 선택된 Runtime Home과 설치 프로필을 마련합니다. 일반 사용자 흐름은 Runtime Home 경로를 다시 제공할 필요가 없습니다.
...
- `registry.sqlite`는 Runtime Home 식별 정보, 설치 프로필 기록, 프로젝트 등록 매핑, 프로젝트 alias, Agent Connection 기록, Connection Projects 멤버십, registry 메타데이터를 저장합니다. 설치 프로필에는 setup 시점의 `volicord` 명령, `volicord-mcp` 명령, bin 디렉터리, 기본 연결 모드, 메타데이터, 타임스탬프가 포함됩니다. 프로젝트 등록에는 내부 프로젝트 식별 정보, 표시 이름, CLI 선택 alias, Runtime Home 관계, 등록된 `repo_root`, `project_home`, 프로젝트 `state.sqlite` 경로, 상태, 메타데이터, 타임스탬프가 포함됩니다.
...
| `registry.sqlite` | 설치 프로필 | setup 시점 실행 파일 프로필 | `volicord setup`이 마련한 setup 시점의 `volicord` 명령, `volicord-mcp` 명령, bin 디렉터리, 기본 연결 모드, 메타데이터, 타임스탬프. |
| `registry.sqlite` | 프로젝트 등록과 alias | 프로젝트 매핑 | 내부 프로젝트 식별 정보, 표시 이름, CLI 선택 alias, Runtime Home 관계, 고유한 `repo_root`, 위치를 담당하는 `project_home`, 실행 시 `project_home/state.sqlite`와 일치해야 하는 저장된 `state_db_path`, 상태, 메타데이터, alias에서 내부 식별 정보로 가는 매핑. |
| `registry.sqlite` | Agent Connection | MCP 호스트 연결 단위 | 지속되는 내부 연결 식별 정보, 호스트 종류, 연결 의도, 호스트 범위, 선택적 내부 프로젝트 식별 정보, 내부 서버 이름, 설정 대상, 모드, 활성 상태, 관리 fingerprint, 검증 요약 상태, 검증 보고서 JSON, 사용자 동작 JSON, 메타데이터, 타임스탬프. |
| `registry.sqlite` | Connection Projects | 연결 프로젝트 허용 목록 | 내부 연결 식별 정보와 내부 프로젝트 식별 정보를 사용하는 Agent Connection과 등록된 프로젝트 사이의 명시적 다대다 멤버십. |
...
- 프로젝트 등록에는 고유한 내부 프로젝트 식별 정보, 고유한 프로젝트 alias, 고유한 저장소 루트, 고유한 프로젝트 홈, 고유한 상태 데이터베이스 경로가 필요합니다. `project_name`은 표시 이름이고 `project_alias`는 CLI 선택 보조 값입니다.
- Agent Connection 식별 정보는 내부 연결 식별 정보별로 고유합니다.
- Connection Projects 멤버십은 내부 연결 식별 정보와 내부 프로젝트 식별 정보의 조합별로 고유하며, 하나의 연결이 등록된 프로젝트를 주소 지정할 수 있게 하는 유일한 registry 멤버십입니다.
