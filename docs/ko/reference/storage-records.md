...
이 문서는 기준 범위의 영속 저장소 기록 계열, 배치, 관계 배치, 저장소 소유 값, 저장소 소유 JSON 배치를 담당합니다. 영속 기록은 Core가 나중에 `Volicord Runtime Home` 안에서 다시 읽을 수 있도록 커밋한 로컬 기록입니다.
...
Volicord는 기준 범위 기록을 로컬 `Volicord Runtime Home` 하나와 등록된 프로젝트별 로컬 상태 데이터베이스 하나에 저장합니다. `volicord setup`은 선택된 Runtime Home과 setup 프로필을 마련합니다. 일반 사용자 흐름은 Runtime Home 경로를 다시 제공할 필요가 없습니다.
...
- `registry.sqlite`는 Runtime Home 식별 정보, setup 프로필 기록, 프로젝트 등록 매핑, Agent Connection 기록, Connection Projects 멤버십, registry 메타데이터를 저장합니다. Setup 프로필에는 선택된 MCP 명령 위치와 선택적 실행 파일 링크 메타데이터가 포함됩니다. 프로젝트 등록에는 사용자 대상 프로젝트 이름, 등록된 `repo_root`, `project_home`, 프로젝트 `state.sqlite` 경로, 상태가 포함됩니다.
...
`Product Repository`는 `repo_root`로 등록되는 사용자 제품 파일 경계입니다. Volicord Runtime Home이 아니며, Core 권한 저장소가 아니고, 런타임 기록, 재실행 행, 판단, Write Check, Agent Connection registry 상태를 저장하는 위치도 아닙니다.
...
|---|---|---|---|
| `registry.sqlite` | Runtime Home 식별 정보 | 런타임 식별 | 저장된 `runtime_home_id` 하나, 스키마/저장 프로필, 로컬 registry 메타데이터. |
| `registry.sqlite` | Setup 프로필 | setup과 설치 프로필 | `volicord setup`이 마련한 선택 MCP 명령, 선택적 실행 파일 링크 메타데이터, 비권한 setup 메타데이터. |
| `registry.sqlite` | 프로젝트 등록 | 프로젝트 매핑 | 등록된 프로젝트 식별자와 사용자 대상 프로젝트 이름을 고유한 `repo_root`, 위치를 담당하는 `project_home`, 실행 시 `project_home/state.sqlite`와 일치해야 하는 저장된 `state_db_path`에 연결합니다. |
| `registry.sqlite` | Agent Connection | MCP 호스트 연결 단위 | 지속되는 내부 `connection_id`, 호스트 종류, 연결 의도, 내부 서버 이름, 설정 대상, 모드, 활성 상태, 관리 fingerprint, 마지막 검증 상태, 메타데이터. |
| `registry.sqlite` | Connection Projects | 연결 프로젝트 허용 목록 | Agent Connection과 등록된 프로젝트 사이의 명시적 다대다 멤버십. |
...
- 프로젝트 등록에는 고유한 프로젝트 식별자, 고유한 프로젝트 이름, 고유한 저장소 루트, 고유한 프로젝트 홈이 필요합니다.
- Agent Connection 식별 정보는 `connection_id`별로 고유합니다.
- Connection Projects 멤버십은 `(connection_id, project_id)`별로 고유하며, 하나의 연결이 등록된 프로젝트를 주소 지정할 수 있게 하는 유일한 registry 멤버십입니다.
...
이 보존 규칙은 `tasks`, `change_units`, `user_judgments`, `project_continuity_records`, `write_checks`, `runs`, `artifacts`, `artifact_links`, `evidence_summaries`, `evidence_observations`, `blockers`, `task_events`, `tool_invocations`에 적용됩니다. 아티팩트별 임시/영속 보존 규칙은 [아티팩트 저장소](storage-artifacts.md)가 담당합니다.
...
| Agent Connection `connection_intent` | `personal`, `shared`, `global`; generic MCP 설정 export가 사용하는 내부 상태에만 쓰는 `export` |
...
| 기록 계열 | JSON `TEXT` 범주 |
|---|---|
| Setup 프로필 | 호스트 신뢰 결정, 사용자 판단, 공개 API 스키마가 아닌 설치 프로필 메타데이터. |
| Agent Connection | 권한, 프로젝트 선택, 호스트 신뢰 증명, 외부 호스트 설정의 대체물로 쓰지 않는 메타데이터. |
