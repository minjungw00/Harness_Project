...
- Runtime Home과 제품 저장소 경계: [런타임 경계](runtime-boundaries.md)
...
- 기준 범위는 활성 상태 변경 메서드 집합에 필요한 Core 영속 기록만 저장합니다.
...
- 요구사항 구체화는 별도 커밋 Discovery Brief, Shared Design, Question Queue, Assumption Register, First Safe Change Unit Candidate 테이블이 아닙니다.
- 증거는 활성 간결 증거 요약과 아티팩트 참조 경로를 통해서만 저장합니다.

### 상태 보기 비주장

허용되지 않는 것:

- 상태 보기는 활성 영속 테이블 계열이 없습니다.
- `status-card`, `judgment-request`, `run-evidence-summary`, `close-result`, `agent-context-packet`은 저장된 상태나 저장소 변경 경로가 아닙니다.

허용되는 것:

- `status-card`, `judgment-request`, `run-evidence-summary`, `close-result`, `agent-context-packet`은 활성 기록 위에서 읽는 시점에 만든 보기입니다.
...
- 저장소는 요청을 준비된 것처럼 보이게 만들려고 별도 활성 계획 테이블을 만들면 안 됩니다.

## 테이블 개요

아래 표는 기준 범위의 활성 저장 기록 범주를 간결하게 보여 주고 범주별 세부 설명으로 연결합니다. 전체 DDL이 아니며 API 스키마나 렌더링된 템플릿 본문을 복사하지 않습니다. 세부 설명은 영속 기록이 저장하는 내용과 저장하지 않는 내용을 분리합니다.
...
- 담당 문서가 범위, 대체 동작, 승격에 필요한 증명 경로 기대치와 함께 좁은 동작을 승격하면 해당 범위에서만 달라질 수 있습니다.
...
- [런타임 경계](runtime-boundaries.md): Runtime Home과 제품 저장소 경계.
