...
- `registry.sqlite`는 Runtime Home 식별 정보, 설치 프로필 기록, 프로젝트 등록 매핑, 프로젝트 alias, Agent Connection 기록, Connection Projects 멤버십, guard 설치 기록, registry 메타데이터를 저장합니다. 설치 프로필에는 setup 시점의 `volicord` 명령, MCP 시작 명령, bin 디렉터리, 기본 연결 모드, 메타데이터, 타임스탬프가 포함됩니다. 프로젝트 등록에는 `project_internal_id`, 표시 이름, CLI 선택 alias, Runtime Home 관계, 등록된 `repo_root`, `project_home`, 프로젝트 `state.sqlite` 경로, 상태, 메타데이터, 타임스탬프가 포함됩니다.
...
| `registry.sqlite` | 설치 프로필 | setup 시점 실행 파일 프로필 | `volicord setup`이 마련한 setup 시점의 `volicord` 명령, MCP 시작 명령, bin 디렉터리, 기본 연결 모드, 메타데이터, 타임스탬프. |
...
- Guard 세션, guard 이벤트, prompt capture, unrecorded change는 프로젝트별 `state.sqlite` 하나에 속하며 그 기록을 관찰했거나 만든 Agent Connection을 이름 붙입니다.
...
- guard 설치, Agent Session, guard 이벤트, prompt capture, unrecorded change의 프로젝트 및 연결 범위
...
`guard_installations`는 Runtime Home, Agent Connection, 선택적 프로젝트 범위별 설정 health와 호스트 capability를 기록합니다. `agent_sessions`, `guard_events`, `prompt_captures`, `unrecorded_changes`는 프로젝트별 로컬 행이며 프로젝트 `state.sqlite` 데이터베이스 사이로 새면 안 됩니다. 미해결 `unrecorded_changes` 행은 관찰된 Product Repository 변경이 아직 담당자가 정의한 조정을 필요로 한다는 뜻입니다. 그 행을 해결하면 로컬 해결 사실을 기록하고 행은 보존됩니다.
...
| `unrecorded_changes` | 기록되지 않은 Product Repository 변경의 관찰 경로 배열, detection JSON, resolution JSON, 메타데이터. |
