...
| `state.sqlite` | `user_judgments` | 사용자 소유 판단 상태 | 근거 스냅샷, 근거 상태, 선택된 선택지, 해결 결과, 해결 행위자, 필요한 경우 민감 동작 승인 범위를 포함하는 대기, 해결됨, 오래됨, 대체됨, 만료됨, 레거시 미결속 사용자 소유 판단. |
...
기존 열린 Task는 종료 닫기 요약 JSON이나 레거시 요약 JSON을 현재 닫기 근거로 자동 변환하지 않습니다. 현재 닫기 근거가 없다는 사실은 빈 생성 근거가 아니라 현재 근거 필드의 부재로 표현합니다.

저장된 근거가 없거나 기계 판독 가능한 해결 결과가 없는 판단은 감사를 위해 보존됩니다. 이 판단은 주소 지정 가능한 이력 판단 기록으로 남지만 현재 닫기, 쓰기, 민감 승인, 취소, 최종 수락, 잔여 위험 수락 요구사항을 만족할 수 없습니다.

저장된 판단 권한에서 `user_judgments.status='resolved'`는 답변이 있다는 사실을 기록합니다. 사용자가 승인했다는 뜻이 아닙니다. 권한을 지니는 사용은 선택된 선택지와 저장된 `resolution_outcome`을 확인해야 하며, 결과 부재는 절대 수락이 아닙니다.
...
| `user_judgments` | 판단 요청, 맥락, 선택지, 영향 참조, 아티팩트 참조, 근거 스냅샷, 민감 동작 승인 범위, 선택된 선택지, 해결 결과, 해결 데이터. |
