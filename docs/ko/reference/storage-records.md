...
런타임 홈 식별은 파일시스템 경로에만 의존하면 안 됩니다. 복사되거나 이동된 런타임 홈은 같은 저장된 `runtime_home_id`를 가질 수 있습니다. 새 런타임 홈은 새 식별자를 가져야 합니다. 이 식별자는 의심스러운 복사본, 중복 등록, 경로 변경을 감지하는 데 도움이 되지만 보안 보장은 아닙니다.
