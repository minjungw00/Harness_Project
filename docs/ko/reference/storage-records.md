...
이 문서는 조건, 저장되는 것, 저장되지 않는 것, 예외, 비주장을 분리해 설명합니다. 저장 효과를 API 스키마 형태나 사용자 표시 문구와 혼동하지 마세요.
...
현재 MVP는 활성 상태 변경 메서드 집합에 필요한 Core 기록만 지속 저장합니다. 대상은 아래 메서드와 intent입니다.
...
그 밖의 지속 테이블 계열이나 임시 핸들 계열은 현재 MVP 범위가 아닙니다.
...
상태 보기는 현재 MVP에서 별도 테이블 계열로 지속 저장하지 않습니다. `status-card`, `judgment-request`, `run-evidence-summary`, `close-result`, `agent-context-packet`은 활성 기록 위에 읽는 시점에 만든 보기이며, 저장된 상태나 저장소 변경 경로가 아닙니다.

### 구체화 저장

최소 활성 구체화 정보도 기존 기록들에 저장합니다. 여기에는 아래 항목이 포함됩니다.
...
닫힌 현재 MVP 저장소 값 집합은 테이블 수준 지속 제약입니다.
