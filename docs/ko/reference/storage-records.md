...
- `registry.sqlite`는 런타임 홈 식별 정보와 프로젝트 등록 매핑을 저장합니다. 여기에는 등록된 `repo_root`, `project_home`, 프로젝트 `state.sqlite` 경로, 상태, 저장소 메타데이터가 포함됩니다.
...
저장된 판단 권한에서 `user_judgments.status='resolved'`는 답변이 있다는 사실을 기록합니다. 사용자가 승인했다는 뜻이 아닙니다. 권한을 지니는 사용은 선택된 선택지, 저장된 `resolution_machine_action`, 저장된 `resolution_outcome`, `resolved_by_actor_kind`, 확인된 해결 접점/인스턴스 출처를 확인해야 합니다. 결과나 필요한 출처의 부재는 절대 수락이 아닙니다.
