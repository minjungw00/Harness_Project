...
이 문서는 기준 범위의 영속 저장소 기록 계열, 배치, 저장소 소유 값을 담당합니다. 영속 기록은 Core가 나중에 `Volicord Runtime Home` 안에서 다시 읽을 수 있도록 커밋한 로컬 기록입니다.

영속 기록은 Volicord 기록에 대한 로컬 Core 저장소 권한입니다.
...
| `registry.sqlite` | Agent Integration Profile | 에이전트 호스트 통합 바인딩 | 지속되는 통합 식별자, 상호작용 역할, 묶인 접점 식별자, 활성 상태, 선택적 기본 프로젝트, 통합 메타데이터. |
| `registry.sqlite` | 통합 프로젝트 멤버십 | 통합 프로젝트 허용 목록 | Agent Integration Profile과 등록된 프로젝트 사이의 명시적 다대다 멤버십. |
| `registry.sqlite` | Host Installation | 호스트 설정 인벤토리 | 설정되었거나 내보낸 에이전트 호스트 항목의 호스트 종류, 호스트 범위, 서버 이름, 설정 대상, 관리 fingerprint, 마지막 검증 상태, 설치 메타데이터. |
...
| `state.sqlite` | `user_judgments` | 사용자 소유 판단 상태 | 필수 근거 스냅샷, 근거 상태, 선택된 선택지, 기계 동작, 해결 결과, 해결 판단 이유 메타데이터, 해결 행위자, 해결된 행의 확인된 행위자 출처, 필요한 경우 민감 동작 승인 범위를 포함하는 대기, 해결됨, 오래됨, 대체됨, 만료됨 사용자 소유 판단. |
...
| `state.sqlite` | `evidence_observations` | 증거 관찰 | 하나의 보고되었거나 관찰된 증거 주장에 대한 지속 출처 기록입니다. 출처 종류, 보장 수준, 관찰자 접점 메타데이터, 도구 메타데이터, 입력 참조, 출력 아티팩트 참조, 한계, 타임스탬프를 포함합니다. |
...
저장된 판단에는 `JudgmentBasis`가 필요합니다. 해결된 저장 판단에는 완전한 기계 판독 가능 해결, 구조화된 설명용 판단 이유 메타데이터, 행위자 출처, 확인된 해결 접점 출처가 필요합니다. 이 사실이 빠진 행은 감사 호환 권한 기록이 아니라 유효하지 않은 소유자 상태입니다.

저장된 판단 권한에서 `user_judgments.status='resolved'`는 답변이 있다는 사실을 기록합니다. 사용자가 승인했다는 뜻이 아닙니다. 현재 권한을 지니는 판단 사용에는 선택된 선택지, 저장된 `resolution_machine_action`, 저장된 `resolution_outcome`, 적용 가능한 행위자 출처, 적용 가능한 확인된 해결 접점 출처가 필요합니다. 판단 이유 메타데이터는 답변의 이유와 맥락을 보존하지만 그 자체가 권한, 증거, 수락, 닫기 준비 상태, 잔여 위험 수락은 아닙니다. 결과, 기계 동작, 적용 가능한 행위자 출처, 확인된 해결 접점 출처의 부재는 유효하지 않은 소유자 상태이며 절대 수락이 아닙니다.
...
| `evidence_observations.observed_by_actor_kind` | 값이 있으면 `agent`, `user` |
| `evidence_observations.observed_actor_role` | 값이 있으면 `agent`, `user_interaction` |
...
| `user_judgments` | 판단 요청, 맥락, 선택지, 영향 참조, 아티팩트 참조, 근거 스냅샷, 민감 동작 승인 범위, 선택된 선택지, 기계 동작, 해결 결과, 설명용 판단 이유 메타데이터, 행위자 출처, 해결 데이터. |
