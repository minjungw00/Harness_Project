...
| 어떤 메서드 분기가 효과를 만드는지 | [저장 효과](storage-effects.md), [MVP API 경로 문서](api/mvp-api.md), 메서드 담당 문서 |
...
- 차단 결과가 상태 효과를 가질 수 있는지는 메서드 담당 문서와 [커밋된 차단 결과의 저장 효과](storage-effects.md#커밋된-차단-결과의-저장-효과)가 정합니다.

예외:

- 차단 결과라고 해서 자동으로 `project_state.state_version`이 증가한다고 추정하지 않습니다.
...
담당 문서 링크: 메서드별 지속 효과는 [저장 효과](storage-effects.md)와 [MVP API 경로 문서](api/mvp-api.md)가 안내하는 메서드 담당 문서가 담당합니다.
...
`harness.record_run`에서는 같은 트랜잭션에 아래 효과도 함께 포함됩니다.
...
- 이벤트 추가.
- 재실행 행 삽입.
- 정확히 한 번의 `project_state.state_version` 증가.
...
재시도 규칙은 실패 종류에 따라 다릅니다. 요약 표는 세부 블록으로 연결합니다.
