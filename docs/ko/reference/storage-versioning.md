...
기준 registry 저장소는 Runtime Home 식별 정보, setup 프로필 기록, 프로젝트 등록, Agent Connection 기록, `connection_projects`를 포함합니다. 기준 project-state 저장소는 Core 상태 기록, replay 행, staged artifact, persistent artifact, evidence, user judgment, run, blocker, `write_checks`를 포함합니다.
