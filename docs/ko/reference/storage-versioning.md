...
- 잠금 정책.
- 지원되는 저장소 프로필/스키마 마이그레이션 의미와 기준 범위/지원 범위 밖 마이그레이션 경계.
...
현재 기준 저장소 프로필은 `baseline_sqlite_v2`입니다.

이 프로필에서는 아래와 같습니다.

- `registry.sqlite`는 스키마 버전 `1`에서 시작하며 레지스트리 `schema_migrations` 행 하나를 기록합니다. 그 행은 `database_kind='registry'`, `version=1`, `name='registry_initial_v1'`, `storage_profile='baseline_sqlite_v2'`입니다.
- 프로젝트 `state.sqlite`는 스키마 버전 `1`에서 시작하며 프로젝트 상태 `schema_migrations` 행 하나를 기록합니다. 그 행은 `database_kind='project_state'`, `version=1`, `name='project_state_initial_v1'`, `storage_profile='baseline_sqlite_v2'`입니다.
- 새 데이터베이스는 해당 데이터베이스의 전체 초기 DDL을 하나의 트랜잭션 마이그레이션으로 적용한 뒤, 같은 프로필의 메타데이터와 마이그레이션 원장 행을 기록해서 만듭니다.
- 이미 현재인 데이터베이스를 다시 열 때의 멱등성은 저장된 스키마 버전, 마이그레이션 버전, 마이그레이션 이름, 데이터베이스 종류, 저장소 프로필이 컴파일된 기준과 일치할 때만 성립합니다.
- 알 수 없는 더 높은 버전, 알 수 없는 마이그레이션 행, 빠진 마이그레이션 행, 부분적이거나 손상된 원장, 마이그레이션 이름 불일치, 저장소 프로필 불일치는 복구하거나 추론하거나 조용히 변환하지 않습니다. 시작과 검사는 기록 의미를 추측하지 말고 구조화된 저장소/런타임 사용 불가 진단으로 실패해야 합니다.
- 정확한 이전 저장소 프로필 `baseline_sqlite`는 지원되지 않습니다. Store는 그 프로필을 가진 Runtime Home을 변환, 삭제, 재작성, 마이그레이션하면 안 됩니다. 운영자는 새 프로필을 사용하기 전에 Runtime Home을 명시적으로 다시 초기화해야 합니다.
- 향후 프로필 또는 스키마 변경은 [범위 참조](scope.md), [저장소 기록](storage-records.md), [저장소 DDL](storage-ddl.md), 이 문서가 버전, 저장소 프로필, 검증, 보존, 복구, 재시도, 메타데이터 전진 동작을 정의할 때만 지원됩니다.
