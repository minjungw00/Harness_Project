...
- 오래된 상태 충돌은 아래 항목을 만들거나 바꾸면 안 됩니다.
  - `CloseReadinessBlocker`.
  - 현재 기록.
  - `task_event` 또는 `task_events` 추가.
  - 아티팩트.
  - 증거 요약.
  - 다른 지원 계약이 명시적으로 말하지 않는 한 `Write Authorization` 상태 변경, 생성 또는 소비.
  - `close_state` 변경.
  - 재실행 행.
  - `project_state.state_version` 증가.
...
일반적인 기준 범위 운영에서 `task_events`는 추가 전용입니다. 이벤트가 커밋된 뒤에는 Core가 그 행을 갱신하거나 삭제해 기록을 바꾸면 안 됩니다. 수정이나 복구는 영향받는 메서드 담당 문서와 저장소 담당 문서가 그 경로를 허용할 때만 새 이벤트와 현재 행 갱신으로 기록합니다.
