...
| `volicord.status` | 선택적 세션 watch 초기화를 포함하는 읽기형 응답 | [`volicord.status`](#volicordstatus) |
...
세션에 연결된 Agent Connection의 경우 `volicord.status`는 세션 watch 진단
...
- 위에서 허용한 세션 watch 진단 효과가 발생한 뒤에도, 계획된 해결이나 대기
