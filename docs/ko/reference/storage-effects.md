...
응답 계산을 위해 `harness.status`와 `harness.close_task intent=check`는 `CurrentCloseBasis`, 닫기 상태, 위험 수락 범위, 차단 사유, `CloseReadinessBlocker[]`, 증거 요약, 아티팩트 참조, 진단, 다음 행동을 계산할 수 있습니다.
...
- 선택된 선택지, `resolution_outcome`, 해결 행위자, 답변 본문, 근거 상태를 메서드 담당 문서가 허용한 대로 저장합니다.
...
`status='resolved'`는 답변이 기록되었다는 뜻이며 그 자체로 수락이 아닙니다. 저장된 `resolution_outcome`이 없는 기존 `resolved` 행은 이력 감사 기록으로 남으며 현재 권한 요구사항을 만족할 수 없습니다.
...
취소 효과에는 메서드가 담당하는 현재 `accepted` 취소 판단이 필요합니다. 취소 권한이 없거나 호환되지 않으면 담당 문서가 허용한 차단된 취소 효과를 만들 수 있지만, 수락이나 완료 전용 닫기 증거를 만들어 내면 안 됩니다.
