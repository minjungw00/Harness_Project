...
- 각 분기가 담당 record, `task_events`, 재실행 행, `project_state.state_version`, 스테이징된 아티팩트 핸들, 아티팩트 승격, `Write Authorization`을 바꿀 수 있는지 여부.
...
| 주제 | 담당 문서 |
|---|---|
| record 배치와 DDL | [저장소 기록](storage-records.md) |
...
이 표에서 담당 record는 각 메서드 담당 문서가 소유하는 현재 record를 뜻합니다. event record는 `task_events` 추가를 뜻합니다.

| 분기 | 담당 record 변경 | event record | `state_version` 증가 | 메모 |
|---|---|---|---|---|
...
| 스테이징 생성 (`StageArtifactResult`, `effect_kind=staging_created`) | Core 담당 record 없음 | 없음 | 없음 | 저장소 소유 임시 스테이징만 만들 수 있습니다. 지속 `ArtifactRef`는 만들지 않습니다. |
| 커밋된 차단 결과 (`MethodResult`) | 메서드 담당 문서가 허용한 record만 가능 | 허용된 경우만 가능 | 허용된 커밋인 경우만 가능 | 보고한 부족 권한 자체를 만들면 안 됩니다. |
| 성공 결과(상태 변경 커밋) | 메서드 담당 문서가 허용한 record 가능 | 허용된 경우 가능 | 커밋당 정확히 한 번 | 성공한 상태 변경만 `project_state.state_version`을 올립니다. |
...
`ToolDryRunResponse`는 `DryRunSummary.would_blockers: PlannedBlocker[]` 또는 계획된 효과를 포함할 수 있습니다. 이 값들은 커밋된 담당 record, 저장된 닫기 차단 사유, 지속 아티팩트, 재실행 저장을 뜻하지 않습니다.
...
|---|---|---|
| `harness.intake` | Task, 선택적 Change Unit, 구체화 record, 이벤트, 재실행 행, `project_state.state_version` 한 번 증가를 만들 수 있습니다. | 유효한 `dry_run=true`는 `ToolDryRunResponse`를 반환하며 Task, 참조, 이벤트, 재실행 행, `state_version` 증가를 만들지 않습니다. 거절은 효과가 없습니다. |
...
`project_state.state_version`은 성공한 상태 변경 커밋에서만 증가합니다.
...
- 응답에 `StagedArtifactHandle`이 있으므로 스테이징된 아티팩트 핸들이 소비되었다.
- `dry_run` 응답에 계획된 효과가 있으므로 담당 record가 바뀌었다.
...
`harness.status`의 `close_blockers: CloseReadinessBlocker[]`는 읽기 전용 관찰입니다. 이 결과는 `task_events`, 재실행 행, `tool_invocations.response_json`, `close_state`, `Write Authorization`, 스테이징된 아티팩트 핸들, 아티팩트, 증거 요약, `project_state.state_version`을 바꾸지 않습니다.
...
- [저장소 기록](storage-records.md): 저장 효과가 건드릴 수 있는 record.
- [아티팩트 저장소](storage-artifacts.md): 스테이징된 아티팩트 핸들과 아티팩트 생명주기 세부사항.
