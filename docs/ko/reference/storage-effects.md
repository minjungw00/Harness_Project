...
응답 계산을 위해 `harness.status`와 `harness.close_task intent=check`는 메서드 담당 문서가 그 상태 보기를 선택할 때 `CurrentCloseBasis`, 닫기 상태, 위험 수락 범위, 차단 사유, `CloseReadinessBlocker[]`, 증거 요약, 아티팩트 참조, 진단, 다음 행동을 계산할 수 있습니다.
...
| `harness.record_run` | 실행, 현재 닫기 근거, 증거 효과 기록 | [`harness.record_run`](#harnessrecord_run) |
...
- 그 `CurrentCloseBasis`에 저장되는 민감 동작 요구사항은 커밋된 실행 기록과 소비된 `Write Authorization`에서 Core가 파생하며, 동작, 정규화된 경로, 민감 범주, 기준선, Change Unit, 출처 실행 기록 참조, 출처 `Write Authorization` 참조를 닫기까지 보존합니다.
...
- 실행 기록, 현재 닫기 근거, 증거, 아티팩트, 승인, 재실행, 이벤트, 리비전 효과는 원자적으로 커밋됩니다.
...
- 선택된 선택지, `resolution_machine_action`, `resolution_outcome`, 파생된 해결 행위자 출처, 답변 본문, 근거 상태를 메서드 담당 문서가 허용한 대로 저장합니다.
...
`status='resolved'`는 답변이 기록되었다는 뜻이며 그 자체로 수락이 아닙니다. 현재 해결 행에는 완전한 근거, 선택된 동작, `resolution_outcome`, 해결 요청 본문, 해결 타임스탬프, 해결 접점 식별성, 검증 근거, 보증 수준, 필요한 행위자 출처가 있어야 합니다. 필요한 해결 권한 정보가 빠진 행은 읽을 수 있는 이력 감사 판단이 아니라 유효하지 않은 저장 상태입니다.
...
- 계산된 닫기 준비 상태를 반환합니다.
- `harness.status include.close=true`와 같은 닫기 준비 상태 계산을 사용합니다.
- 재실행 행을 만들지 않습니다.
- 이벤트를 추가하지 않습니다.
- 차단 사유 행을 만들지 않습니다.
...
취소 효과에는 `machine_action=accept`, `resolution_outcome=accepted`, 호환되는 근거, 확인된 `user_interaction` 행위자 출처를 가진 메서드 담당 현재 취소 판단이 필요합니다. 취소 권한이 없거나 호환되지 않으면 담당 문서가 허용한 차단된 취소 효과를 만들 수 있지만, 수락이나 완료 전용 닫기 증거를 만들어 내면 안 됩니다.
