...
응답 계산을 위해 `volicord.status`와 `volicord.close_task intent=check`는 메서드 담당 문서가 그 상태 보기를 선택할 때 `CurrentCloseBasis`, 닫기 상태, 위험 수락 범위, 차단 사유, `CloseReadinessBlocker[]`, 증거 요약, 아티팩트 참조, 진단, 다음 행동을 계산할 수 있습니다.
...
조건: `volicord.prepare_write` 또는 `volicord.close_task`의 커밋된 차단 결과는 관련 메서드 담당 문서가 차단 커밋을 허용할 때만 `MethodResult`입니다.
...
| `volicord.record_run` | 실행, 현재 닫기 근거, 증거 효과 기록 | [`volicord.record_run`](#volicordrecord_run) |
