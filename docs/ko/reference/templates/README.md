...
- 사용자 상태 카드, 에이전트 맥락 패킷, 렌더링된 template은 Core 상태에서 파생된 보기이지 Core 상태가 아닙니다.
- 렌더링된 template은 민감 동작 승인, 작업 수락, 잔여 위험 수용, 근거, 닫기 준비 상태, Write Authorization, close를 만들 수 없습니다.
...
| Audience | Shape | Rule |
|---|---|---|
| 사용자 상태 카드 | [Compact Status Card](compact-status-card.md) | MVP-1 사용자 작업 루프 projection입니다. 사용자를 위한 하나의 작은 현재 상태 카드입니다. |
| 에이전트 맥락 패킷/reference payload | Structured refs, blocker label, source clock, freshness, next-action hint | 다음 안전한 행동을 돕는 파생된 지원 payload입니다. 기본은 compact이며, 단계상 꼭 필요할 때만 full report body를 pull합니다. |
...
Card는 사용자가 읽기 쉬우면서도 에이전트가 부담 없이 다룰 만큼 작아야 합니다. Schema field, DDL, complete history, event log, full artifact, full reference doc, 전체 template, 관련 없는 template, future catalog, full Evidence Manifest, full report body를 쏟아내면 안 됩니다.

## Template-to-stage matrix

| Template | Audience | First active stage | Authority status | Notes |
|---|---|---|---|---|
| [Compact Status Card](compact-status-card.md) | 사용자용 간결한 카드; agent context packet source | MVP-1 사용자 작업 루프 projection; 내부 엔지니어링 점검 status rendering에서는 optional | 파생 표시 전용 | 유일한 MVP-1 사용자 작업 루프 projection shape입니다. 내부 엔지니어링 점검은 plain structured output만으로도 충분합니다. |
| [Decision Packet display](decision-packet.md) | Full-format 사용자 판단 presentation | 사용자 판단 flow가 active인 later/profile 또는 complex judgment display | `state.sqlite.user_judgments`에서 파생된 표시입니다. Standalone authority가 아닙니다. | 필요한 판단은 보통 compact status/next 또는 user-judgment resource를 통해 나타납니다. Standalone `DEC` Markdown은 later optional입니다. |
...
| [APR](approval.md) | Sensitive-action approval report | 보증 프로필 | Committed Approval과 관련 user judgment ref를 표시합니다. Approval을 부여하지 않습니다. | Committed Approval support/profile이 active인 뒤에만 사용합니다. Minimum MVP-1은 compact 민감 동작 승인 user judgment display를 사용합니다. |
| [Approval Card](approval-card.md) | Sensitive-action approval prompt/card | 보증 프로필 | Committed approval boundary를 표시합니다. Write를 허가하지 않습니다. | Full Approval Card는 MVP-1 사용자 작업 루프가 아닙니다. MVP-1 path는 compact 민감 동작 승인 판단 요청을 사용할 수 있습니다. |
...
여러 source가 관련될 때 compact authority display는 짧은 refs 줄을 우선 사용합니다. 예: `write=`, `judgment=`, `approval=`, `evidence=`, `eval=`, `manual_qa=`, `acceptance=`, `residual_risk=`, `artifacts=`, `redaction=`, `freshness=`. 이 label은 기존 ref, redaction state, projection freshness를 가리킬 뿐이며 schema field나 authority record가 아닙니다.

파생 표시 요약에는 `approval_covers`, `approval_does_not_cover`, `secret_exposure_boundary` 같은 Approval boundary line, close context, close blocker, waiver path, projection freshness, redaction availability, 에이전트 맥락 패킷 요약, Journey Card, Review Stages, judgment-context 관련 summary가 포함됩니다. 이 이름들은 새로운 기준 기록, schema field, DDL column, `ProjectionKind` value, gate, 권한을 만드는 입력이나 권한 경로가 아닙니다. Label 자체는 검증 입력으로 사용하면 안 됩니다. 검증기는 그 label이 요약하는 owner record, ref, gate, artifact, user judgment를 읽어야 합니다.
...
User judgment visibility는 standalone `DEC` Markdown projection에 의존하지 않습니다. Required surfaces는 active user judgment를 간결한 상태 카드(compact status card), status/next response, user-judgment resource, 또는 dedicated prompt를 통해 보여줄 수 있습니다. Later continuity profile이 active이면 `TASK`도 이를 보여줄 수 있습니다. Standalone `DEC`는 해당 projection이 켜져 있을 때만 쓰는 선택적 full-format 렌더링 보기입니다.

User judgment 표시는 judgment title, `judgment_type`, `presentation`, `display_label`, 왜 지금 필요한지, 사용자가 정확히 판단하는 것, 간결한 options 또는 상세 trade-offs, recommendation, uncertainty, deferral consequence, 해당하는 경우 residual risk 같은 기준 schema field와 읽기용 shape field를 포함할 수 있습니다. `presentation=short`는 기본 compact prompt이고, `presentation=full`은 복잡한 판단을 위한 선택적 Decision Packet-style display입니다. `display_label`은 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용으로 제한됩니다. `judgment_type` 예시는 `product_choice`, `technical_choice`, `sensitive_action_approval`, `work_acceptance`, `residual_risk_acceptance`입니다. Judgment가 여러 영역에 걸쳐 있으면 display label을 배타적으로 다루지 말고 부차적인 고려사항을 trade-offs, affected gates, risk, evidence, follow-up에 렌더링해야 합니다. `presentation` 또는 `display_label`에서 파생한 표시용 label은 독자를 돕지만 schema field, `ProjectionKind` value, gate, owner record, validator input, close aggregation rule, authority path, `judgment_type` replacement가 아닙니다.
