...
Decision Packet visibility는 standalone `DEC` Markdown projection에 의존하지 않습니다. Required surfaces는 active Decision Packet을 간결한 상태 카드(compact status card), status/next response, judgment-context resource, decision-packet resource, 또는 dedicated prompt를 통해 보여줄 수 있습니다. Later continuity profile이 active이면 `TASK`도 이를 보여줄 수 있습니다. Standalone `DEC`는 해당 projection이 켜져 있을 때만 쓰는 선택적 렌더링 보기입니다.
