...
Always-on Harness context envelope를 짧은 현재 상태 표시로 보여줄 때 Compact Status Card를 사용합니다. 여기에는 Task 상태, next safe action, active Change Unit, 대기 중인 user decision, write authority, guarantee level, gate, Manual QA, residual risk, projection freshness, latest refs가 포함됩니다.
...
````text
TASK-{id} {title}
표시 전용: 현재 상태를 보여주는 읽기용 view이며 기준 상태나 write authority가 아닙니다.
