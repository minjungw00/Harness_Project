...
Later Approval profile이 Approval request record를 commit한 뒤, 민감한 행동 요청과 결정을 읽기 쉽게 보여줘야 할 때 `APR`을 사용합니다. `APR`은 민감 동작 승인 범위를 보여줄 뿐이며 사용자 소유의 제품 판단이나 기술 구조 판단, correctness, 작업 수락, 잔여 위험 수용, QA 면제 판단, 검증 면제 판단, deployment, merge, Write Authorization을 결정하지 않습니다.
...
Boundary Summary는 Approval 범위, 연결된 Approval 기록, 관련 Decision Packet ref, 현재 쓰기 또는 닫기 context에서 파생한 표시 block입니다. Later Approval profile의 사용자 경계 reminder이며 독립된 권한 출처나 gate가 아닙니다.
