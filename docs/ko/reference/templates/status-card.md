...
- 활성 차단 사유와 차단된 이유
- Run 참조, `evidence_ref` 참조, ArtifactRefs, `redaction_state`, 증거 공백
- 닫기 차단 사유, 최종 수락 필요 여부/상태, 잔여 위험 표시, 필요한 경우 잔여 위험 수락 참조
...
증거: status={evidence_summary.status}; summary={known_evidence_summary|none}
