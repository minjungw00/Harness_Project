...
근거, 검증, 수동 QA, 최종 수락, 잔여 위험 표시, 잔여 위험 수락, blocker, 읽기용 보기 최신성을 하나의 "완료" 줄로 뭉개지 않습니다. 닫기가 막혔으면 primary blocker를 먼저 말하고, 다음 경로에 영향을 주는 secondary blocker도 보이게 둡니다. 읽기용 close view가 stale 또는 failed이면 이 템플릿의 prose에서 close하지 말고 current Core close result를 가져와야 합니다.
