...
Later Approval profile에서 민감 행동(sensitive-action) 요청 범위, 목적, 경계, 위험, 대안, expiry/use behavior, 추천안을 사용자에게 간결하게 보여줄 때 Approval Card를 사용합니다. 이 card는 민감 동작 승인을 묻는 표시일 뿐이며 사용자 소유의 제품 판단이나 기술 구조 판단, 정확성 검토(correctness review), 작업 수락, 잔여 위험 수용, QA 면제 판단, 검증 면제 판단, Write Authorization이 아닙니다.

경계: projection template일 뿐이며 runtime/server 구현이나 생성된 운영 산출물에 권한을 주지 않습니다. 공통 phase와 projection 규칙은 [템플릿 참조](README.md#사용-시점)를 따릅니다.

구현 계층: Agency assurance reports입니다. Committed sensitive-action approval support가 active profile에 있을 때만 사용합니다. Minimum MVP-1의 Approval 형태 사용자 판단 요청은 Decision Packet display로 충분하며 approval card가 필요하지 않습니다.
...
`{approval_covers}`, `{approval_does_not_cover}` 같은 coverage placeholder는 Approval 범위, 관련 Decision Packet ref, 해당 profile이 active일 때의 연결된 Approval 기록, 현재 쓰기 또는 닫기 context에서 파생한 표시 전용 요약입니다. Approval 경계만 보여주며, active owner path가 계속 기준 출처입니다.
...
이 template은 렌더링 결과인 카드 형태일 뿐 Approval 권한 자체가 아닙니다. Committed Approval record는 later-profile이며, minimum MVP-1의 Approval 형태 request는 Decision Packet prompt로 보여줄 수 있습니다.
