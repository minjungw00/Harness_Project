...
- QA waiver 또는 failure와 관련된 Residual Risk와 Decision Packet 또는 필요한 판단 경로 참조
...
> Projection 보기: `source_state_version`와 `updated_at` 기준으로 렌더링되며 Manual QA record와 `qa_gate`를 표시합니다. QA result, waiver, risk acceptance는 해당되는 경우 QA path, 필요한 판단 경로, residual-risk path를 통해 기록되어야 합니다.
