...
- QA waiver display: `qa_gate=waived`와 Manual QA record 또는 waiver reason, 필요한 경우 QA waiver Decision Packet
- 사람의 확인 요약:
- summary:
- 면제 사유:
...
Manual QA 표시는 passed Manual QA record, failed Manual QA record, pending required QA, QA waiver를 눈에 띄게 구분해야 합니다. `qa_gate=waived`는 필요한 경우 ref와 accepted risk/follow-up을 동반하는 waiver display입니다. Passed Manual QA result, final acceptance, detached verification이 아닙니다.
