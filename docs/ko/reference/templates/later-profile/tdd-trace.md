...
> 상태 보기(Projection): `source_state_version`와 `updated_at` 기준으로 렌더링되며 TDD 기록과 참조를 표시합니다. 계획 문구는 기록된 아티팩트 또는 결과 참조가 뒷받침하기 전까지 RED 근거가 아닙니다.
...
- 대상 / 계획은 근거 목록(Evidence Manifest) 뒷받침 범위로 계산됨: no
