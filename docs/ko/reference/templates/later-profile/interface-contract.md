...
이 템플릿은 렌더링 결과일 뿐 기준 상태가 아닙니다. 기준 인터페이스 참조는 `StateRecordRef.record_kind=interface_contract`를 사용합니다. `검토` 섹션은 인터페이스, 검증기, 판단 참조 위의 상태 보기 표시이며 민감 동작 승인(Approval), 증거, QA, 검증, 최종 수락, 잔여 위험 수락, 닫기, 쓰기 승인 기록(Write Authorization)이 아닙니다.
