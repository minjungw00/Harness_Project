...
- 관련 권한 맥락으로 표시될 때 쓰기 허가 기록(Write Authorization), 민감 동작 허가, Eval(분리 검증 결과), 수동 QA, 최종 수락 맥락, 잔여 위험 참조, ArtifactRef 참조, 가림 상태, 읽기용 보기 최신성(projection freshness)
...
판단 패킷(Decision Packet) 상태 보기는 권한 맥락 참조를 간결하고 명시적으로 유지해야 합니다. 이 템플릿에 쓰기 허가 기록(Write Authorization), 민감 동작 허가 참조, 근거 요약, 해당 프로필이 활성화된 경우의 근거 목록(Evidence Manifest), Eval(분리 검증 결과), 수동 QA, 최종 수락, 잔여 위험 표시, 잔여 위험 수락, 아티팩트, 가림 상태, 최신성 참조를 표시하더라도 문장이 그 기록의 권한이 되지는 않습니다.
