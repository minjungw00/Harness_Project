...
- 기술 판단(`judgment_type=technical_choice`): 세션 쿠키, bearer/JWT 토큰, OAuth/OIDC 제공자, social-login 제공자 통합 중에서 세션 모델을 고릅니다. 철회, CSRF/XSS 노출, 클라이언트 호환성, 구현 비용, identity-provider 경계, 마이그레이션 영향은 `선택지`와 `판단에 필요한 최소 맥락`에 둡니다.
- 기술 판단(`judgment_type=technical_choice`): 의존성 채택, schema/data-model 마이그레이션, 공개 API/interface 방향, 모듈 경계(module boundary) 변경, 개인정보/로그 정책(privacy/logging policy), QA 기대치, 검증 기대치, 면제, 범위/자율성(scope/autonomy) 확장, 나중 프로필이 활성화된 경우의 조정(reconcile) 선택을 다룹니다.
...
민감 동작 승인(Approval) 범위: 이름 붙은 설치 명령 또는 의존성 파일 업데이트, 이름 붙은 manifest/lockfile 경로, 현재 Task와 승인 유효 기간만 포함.
