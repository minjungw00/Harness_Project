...
이 템플릿은 렌더링 결과일 뿐 기준 상태가 아닙니다. 기준 모듈 참조는 `StateRecordRef.record_kind=module_map_item`을 사용합니다. 검토, 주의 지점, 스튜어드십 모음 문구는 owner 기록 위의 표시이며 민감 동작 승인(Approval), 증거, QA, 검증, 최종 수락, 잔여 위험 수락, 닫기, 쓰기 승인 기록(Write Authorization)을 만들지 않습니다.
