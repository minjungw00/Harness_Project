...
구현 계층: 향후/진단용 상태 보기(projection)입니다. 모듈 맵(Module Map) 출력은 나중 스튜어드십/참조 보기이며 첫 구현 조각이나 MVP-1 다섯 가지 보기 세트에 필요하지 않습니다.
...
- 관련 사용자 판단과 설계 참조
- `deep_module_interface` 또는 `codebase_stewardship` 관련 설계 품질 validator 결과
...
## 모듈
| 모듈 | 역할 | 공개 인터페이스(interface) | 내부 복잡도 | 의존성 | 테스트 경계 | 소유자 판단 | 주의 지점 |
|---|---|---|---|---|---|---|---|
| AuthService | 인증(auth)을 확인하고 세션(session)을 발급함 | `login`, `logout` | 자격 증명 검증(credential validation), 세션 발급(session issue) | UserRepo, SessionStore | 서비스 인터페이스 테스트(service interface tests) | human_reviewed | 세션 만료 drift(불일치) |
...
- 기준 owner: Module Map Item; 전용 아키텍처 주의 지점 참조는 나중 DDL batch가 정의한 경우에만 사용한다
- 얕은 모듈 성장:
- 의존성 방향 위험:
- 공개 인터페이스 drift(불일치):
<!-- HARNESS:END managed -->
...
## 사용자 메모와 제안
<!-- 사람이 편집 가능: 여기의 모듈(module) 제안은 reconcile/Core를 통해 accepted되기 전에는 기준 Module Map Item이 아닙니다. -->
-
````
