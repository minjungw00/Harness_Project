...
전체 보고서가 명시적으로 유용한 later/profile 단계에서, 진행 중인 작업을 이어서 파악할 수 있는 continuity 또는 reference projection이 필요할 때 `TASK`를 사용합니다. 이 template은 범위, 사용자 판단, 근거, 닫기 준비 상태, 작업의 현재 위치, 사용자 판단 맥락, 막힘 소유자, 자율성 경계(Autonomy Boundary), 쓰기 권한 요약, 구현 마이크로 계획, 검토 단계, Stewardship 영향, 다음 근거, 잔여 위험, 닫기 요약, 필요할 때의 kernel gate detail, 활성 Change Unit, 대기 중인 판단, 관련 보고서 ref, 읽기용 보기 최신성을 보여줄 수 있습니다.
...
- mode, lifecycle, 다음 행동(next action), 가장 먼저 해소할 막힘, 가장 작은 해소 방법, 보장 수준, 읽기용 보기 최신성(projection freshness)을 위한 현재 상태 표시 input
...
> 상태 보기(Projection): `source_state_version`와 `updated_at` 기준으로 렌더링된 보기입니다. 관리 섹션(Managed section)은 생성된 표시 영역이며, 그 안의 edit는 상태 변경이 아니라 drift 또는 reconcile candidate입니다.
...
- 사용자 판단:
- 민감 동작 승인 user judgment / Approval 참조:
...
## 대기 중인 사용자 판단
| 표시 라벨 | 질문 | `judgment_type` / refs | 상태 | 다음 행동 |
|---|---|---|---|---|
...
### Step Queue(단계 대기열)
| 단계 | 상태 정렬 | 범위 정렬 / 예상 path | Feedback Loop / TDD 상태 | 근거 목표 | 중단 조건 |
|---|---|---|---|---|---|
...
`TASK`의 authority claim은 source ref 또는 명시적 absence로 해소되어야 합니다. 쓰기 권한 claim은 쓰기 허가 기록(Write Authorization) ref를 가리킵니다. 민감 동작 허가는 minimum MVP-1에서는 `judgment_type=sensitive_action_approval`인 resolved `user_judgment`를 가리키고, later 민감 동작 승인(Approval) profile이 활성화된 경우에만 Approval ref를 가리킵니다. Minimum MVP-1 근거 표시는 있을 때 `evidence_ref`, Run refs, ArtifactRef 참조, 보이는 공백 요약을 가리킵니다. 활성 owner path가 full evidence sufficiency를 세울 수 없으면 full sufficiency를 주장하지 않아야 합니다. Full criteria-to-evidence sufficiency는 근거 목록(Evidence Manifest) profile이 활성화된 경우에만 근거 목록 refs를 가리킵니다. 분리 검증은 해당 profile이 활성화된 경우에만 Eval(분리 검증 결과) refs를 가리킵니다. 수동 QA는 해당 profile이 활성화된 경우에만 수동 QA records 또는 valid waiver refs를 가리킵니다. 작업 수락은 작업 수락 user judgment path를 가리킵니다. MVP-1에서 잔여 위험 표시는 blocker/user-judgment ref 또는 `ResidualRiskSummary.status=none`을 가리키고, rich Residual Risk ref는 해당 profile이 활성화된 경우에만 가리킵니다. MVP-1에서 잔여 위험 수용은 residual-risk acceptance user judgment와 관련 blocker/evidence ref를 가리키고, accepted Residual Risk ref는 해당 later profile이 활성화된 경우에만 가리킵니다. Ref가 없으면 completed authority가 아니라 missing support로 렌더링해야 합니다.
...
닫기 요약은 민감 동작 승인, 근거, 검증, 수동 QA, 작업 수락, 잔여 위험 표시, 잔여 위험 수용을 하나의 "완료" flag로 합치면 안 됩니다. 테스트가 통과했지만 민감 동작 승인, 수동 QA, 작업 수락, 잔여 위험 수용이 pending이면 close display는 정확히 그 범주를 blocker로 보여줘야 합니다.

직접 작업은 `DIRECT-RESULT`에서 가벼운 닫기 영향 요약을 보여주고, Journey Card close context는 간결한 status/resume 표시입니다. `TASK` 닫기 요약은 [projection/report 경계](../../projection-and-templates.md#projection-principles) 안의 이어가기 표시이며, close와 gate effect는 여전히 owner record에서 옵니다.
