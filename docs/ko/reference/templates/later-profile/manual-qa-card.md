...
구현 계층: 보증 프로필 보고서입니다. Manual QA profile이 명시적으로 활성화된 경우 사용하며 full Manual QA policy coverage는 이후 staged scope입니다.
...
브라우저 QA 캡처(Browser QA Capture): 승격되고 지원될 때 유용합니다. 작업 수락이 아니며, 독립적인 Eval(분리 검증 결과) 없이는 분리 검증이 아니고, required human inspection을 대체하지 않습니다.
