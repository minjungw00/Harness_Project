...
- 현재 수용 기준 스냅샷
- 쓰기 승인 기록(Write Authorization), 민감 동작 승인, 기준선, 보장 수준 참조
- 민감 동작 승인 상태
- 활성 사용자 판단 참조
- 가장 먼저 해소할 차단 사유, 추가 차단 사유, 가장 작은 해소 방법 표시 요약
- 차단 사유 소유자 표시 요약
- 증거 뒷받침 범위, 검증, 수동 QA 요약
- 잔여 위험 요약과 참조
- 최종 수락, 잔여 위험 수락, 닫기 이유 요약
- 최신 증거, Eval(분리 검증 결과), 수동 QA, 보고서 참조
...
````text
TASK-{id} {title}
표시 전용: 현재 위치를 보여주는 읽기용 보기이며 기준 상태나 쓰기 승인 기록이 아닙니다.
...
현재 막는 것:
- 가장 먼저 해소할 차단 사유: {primary_blocker_label|none}
...
이 카드 안이나 주변에 표시되는 `status`/`next` 추천은 읽기 전용 안내입니다. 사용자 판단 요청, 선택적 전체 형식 판단 패킷(Decision Packet) 표시, `prepare_write`, 증거 수집, 검증, QA, 조정(reconcile), 닫기 시도를 가리킬 수는 있지만, 상태를 변경하거나, 쓰기를 승인하거나, 관문을 충족하거나, 최종 수락을 기록하거나, 잔여 위험을 수락하거나, Task를 닫지 않습니다.
...
차단 사유(Blocker) 줄은 API와 상태 기록을 사용자에게 보이는 상태로 바꿔 보여줍니다. 가장 먼저 해소할 차단 사유는 다음 행동이 먼저 해소해야 하는 차단 사유여야 하며, 소유자 라벨은 다음 움직임이 사용자 소유인지, 에이전트가 해소 가능한지, 접점/시스템 소유인지 분명히 해야 합니다. 가장 먼저 해소할 차단 사유가 없으면 소유자는 `none`으로 렌더링하거나 생략할 수 있습니다. 추가 차단 사유는 후속 경로에 영향을 줄 때만 계속 보여줍니다. 원시 `ErrorCode` 값만으로 설명을 끝내면 안 됩니다.

최신 증거 또는 다음 증거에 `secret_omitted`나 `blocked` 아티팩트 참조가 포함되면 이 카드는 사용 가능성 영향만 표시해야 합니다. 생략된 값 또는 차단된 원본 페이로드(payload) 내용을 포함하면 안 됩니다.
