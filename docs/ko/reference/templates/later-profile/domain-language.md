...
- `domain_language` 관련 설계 품질 validator 결과
...
|---|---|---|---|---|---|---|
| Account | 로그인할 수 있는 사용자 identity | `src/auth/account.ts` | Profile | User, Session | TASK-0001 | active |
...
## 사용자 메모와 제안
<!-- 사람이 편집 가능: 여기의 용어 제안은 reconcile/Core를 통해 accepted되기 전에는 기준 도메인 용어(domain term)가 아닙니다. -->
-
````
...
용어 충돌이 제품 의미, 공개 동작, API/interface 이름, 문서 약속, 수용 기준, 모듈 책임(module responsibility)을 바꾸면 해당 판단은 기존 설계 품질 및 사용자 판단 경로로 라우팅합니다. 충돌을 여기에 렌더링하는 것만으로 `design_gate`, `decision_gate`, 닫기 영향이 해소되지는 않습니다.
