...
이 템플릿은 렌더링 결과일 뿐 기준 상태가 아닙니다. 기준 도메인 용어 참조는 `StateRecordRef.record_kind=domain_term`을 사용합니다. 대기 중인 용어 판단, 최신 검토 문구, 사람이 제안한 내용은 표시 또는 조정(reconcile) 입력입니다. 그 자체로 관문을 충족하거나, 쓰기를 승인하거나, 증거를 만들거나, 위험을 수락하거나, 작업을 닫지 않습니다.
