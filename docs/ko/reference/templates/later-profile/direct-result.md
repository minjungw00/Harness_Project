...
작은 직접 작업이 닫혔거나 `work`로 전환된 뒤 결과를 간결하고 부담 없이 보여줘야 할 때 `DIRECT-RESULT`를 사용합니다. 전체 Task 관문 보고서가 아니라 직접 결과처럼 읽혀야 합니다.
...
> 상태 보기(Projection): `source_state_version`와 `updated_at` 기준으로 렌더링되며 직접 실행(Run) 결과를 표시합니다. 이 문서를 편집해도 결과, 보장 수준, 전환, 닫기 상태는 바뀌지 않습니다.
