...
- Eval(분리 검증 결과) 기록
- 보장 영향과 검증 독립성 상태
- 분리 검증 후보(detached-candidate), 자체 확인(self-checked), 분리 검증됨(detached-verified), 수용된 위험으로 면제됨(waived-with-accepted-risk) 표시 문구
- 수동 QA와 작업 수락 영향
...
사용자 표시 검증 상태: {자체 확인|분리 검증 후보|분리 검증됨|수용된 위험으로 면제됨}
