...
- Eval(분리 검증 결과), 근거 목록(Evidence Manifest), 수동 QA, 작업 수락 맥락, 잔여 위험(Residual Risk), 검증 면제 user judgment, 아티팩트 참조, 가림 상태, 읽기용 보기 최신성(projection freshness)을 위한 간결한 참조
...
- QA waiver 표시: {qa_gate=waived with 수동 QA 또는 waiver refs|none}
