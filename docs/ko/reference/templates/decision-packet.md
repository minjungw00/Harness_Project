...
구현 계층: User judgment prompt shape입니다. 사용자 소유 판단이 대기 중일 때 Decision Packet display/card shape로 사용하며, MVP-1 사용자 작업 루프 projection도 standalone `DEC` `ProjectionKind`도 아닙니다. Standalone persisted `DEC` Markdown projection은 standalone Decision Packet projection 기능이 켜진 경우에만 사용하며, 필요한 prompt는 compact status card, status/next, Decision Packet resource를 통해 나타날 수 있습니다.
...
- Minimum MVP-1 민감 동작 permission을 위한 `approval_scope`와 Approval 형태 Decision Packet ref, later Approval profile이 active일 때만 Approval 기록
- 필요한 경우 관련 reconcile 기록
- 잔여 위험(residual risk) 참조
- Minimum MVP-1의 evidence summary, Run refs, ArtifactRefs, 보이는 근거 공백. Full Evidence Manifest profile이 active일 때만 Evidence Manifest refs
- 관련 authority context로 표시될 때 Write Authorization, 민감 동작 permission, Eval, 수동 QA, 작업 수락 context, 잔여 위험 ref, ArtifactRefs, redaction state, projection freshness
- 영향을 받는 범위 표시 input: product area, screen 또는 flow, module, interface, path, 수용 기준, gate, sensitive category
- 읽기용 보기 최신성(projection freshness) 입력

Approval 형태 표시 항목인 "이 Approval이 포괄하는 것", "이 Approval이 포괄하지 않는 것", "secret 노출 경계"는 `judgment_payload.approval_scope`, 관련 Decision Packet ref, later profile이 active일 때의 연결된 Approval 기록, 현재 쓰기 또는 닫기 context에서 파생한 표시 전용 요약입니다. 경계를 설명할 뿐이며 별도의 사용자 소유 판단을 확정하거나 Write Authorization을 만들거나 minimum MVP-1에서 committed Approval record를 암시하지 않습니다. Approval 형태 표시는 민감 동작 승인으로 label해야 하며 작업 수락처럼 보이면 안 됩니다.

해소된 Decision Packet은 `judgment_route=approve-sensitive-action`과 compatible `approval_scope`를 가질 때만 민감 동작 permission을 부여합니다. Minimum MVP-1에서는 그 resolved packet이 민감 동작 route의 기준입니다. Later Approval profile은 연결된 Approval 기록을 추가로 요구하거나 표시할 수 있습니다. 그 밖의 Decision Packet resolution은 사용자 소유 판단, waiver, 잔여 위험 수용, 작업 수락, reconcile choice를 확정할 수 있지만 민감 동작 permission을 부여하지 않습니다.
...
Decision Packet projection은 authority context ref를 간결하고 명시적으로 유지해야 합니다. 이 template에 Write Authorization, 민감 동작 permission ref, evidence summary, 해당 profile이 active일 때의 Evidence Manifest, Eval, 수동 QA, 작업 수락, 잔여 위험, artifact, redaction, freshness ref를 표시하더라도 packet prose가 그 기록의 authority가 되지는 않습니다.
