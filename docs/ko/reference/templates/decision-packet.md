...
구현 계층: User judgment prompt shape입니다. 사용자 소유 판단이 대기 중일 때 Decision Packet display/card shape로 사용하며, v0.2 First User-Value Slice projection도 standalone `DEC` `ProjectionKind`도 아닙니다. Standalone persisted `DEC` Markdown projection은 standalone Decision Packet projection 기능이 켜진 경우에만 사용하며, 필요한 prompt는 compact status card, status/next, Decision Packet resource를 통해 나타날 수 있습니다.
...
- Minimum v0.2 민감 동작 permission을 위한 `approval_scope`와 Approval 형태 Decision Packet ref, later Approval profile이 active일 때만 Approval 기록
- 필요한 경우 관련 reconcile 기록
- 잔여 위험(residual risk) 참조
- Minimum v0.2의 evidence summary, Run refs, ArtifactRefs, 보이는 근거 공백. Full Evidence Manifest profile이 active일 때만 Evidence Manifest refs
