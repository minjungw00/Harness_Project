...
- `StateSummary`, 차단 사유, 대기 중인 `UserJudgment` 항목, 증거 요약과 출처 상태, 닫기 준비 상태 관찰, 잔여 위험 범위, 프로젝트 연속성 요약, 보장 표시, 다음 안전한 행동 같은 표시 입력입니다.
...
- 카드가 `Write Check`을 만들거나, 증거를 기록하거나, 위험을 수락하거나, `Task`를 닫는다는 의미.
...
- 결정 하나만 묻는 집중 요청으로, 사용자의 답변을 증거, 수락, 잔여 위험 수락, `Write Check`과 분리해서 표시합니다.
- 사용자가 판단해야 하는 정확한 질문.
...
- 에이전트가 사용자 판단, `Write Check`, 아티팩트 규칙, 닫기 차단 사유를 우회할 수 있다는 의미.
