...
- 예약된 값, profile-gated 값, 이후 후보가 현재 범위에 미치는 경계
...
이 저장소는 사용자의 `Product Repository`도 아니고 `Harness Runtime Home`도 아닙니다. 이 문서들은 런타임 상태, 생성된 상태 보기, 아티팩트, 증거 기록, QA 기록, 수락 기록, 닫기 기록, 잔여 위험 기록, 실행 가능한 fixture, 적합성 결과, 구현 완료 동작을 만들지 않습니다.
...
현재 MVP의 보장 경계는 기본적으로 `cooperative`입니다. 현재 MVP는 `isolated` 보장 의미를 제공하지 않습니다. 예약된 값이나 profile-gated 값인 보장 라벨은 현재 MVP 범위를 확장하지 않습니다.
