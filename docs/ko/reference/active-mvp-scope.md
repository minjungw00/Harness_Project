...
| 닫기 시도 | `harness.close_task`는 증거, 최종 수락, 잔여 위험, 대체 불가 경계를 지키면서 닫기 준비 상태를 확인하고 지원되는 닫기 결과를 시도할 수 있습니다. | [MVP API](api/mvp-api.md), [Core Model](core-model.md), [오류](api/errors.md) |

읽는 시점의 상태나 파생 표시는 상태 및 닫기 준비 상태 확인의 일부일 때만 현재 범위에 들어옵니다. 지속 저장되는 상태 보기 작업, 생성된 상태 보기 파일, 관리되는 상태 보기 복구는 현재 범위가 아닙니다.

## 현재 MVP에서 제외되는 것

현재 MVP는 OS 권한 제어 시스템이 아닙니다. 샌드박스가 아닙니다. 변조 방지 저장소가 아닙니다. 전체 보안 격리 계층이 아닙니다. 이 저장소가 문서 전용 상태인 동안에는 런타임 구현도 아닙니다. 이후 후보는 현재 요구사항을 만들지 않습니다.
...
명령, 의존성 변경, 호스트, 네트워크 접근, 비밀값 핸들, 배포, 파괴적 동작, 시스템 접근을 승인해도 현재 MVP에서 하네스가 그 동작을 관찰하거나 차단할 수 있다는 뜻은 아닙니다.
...
현재 MVP의 보장 경계는 기본적으로 협력형입니다. 하네스는 권한 경계, 호환성 확인, 증거 요약, 사용자 소유 판단, 닫기 준비 상태 발견 사항을 기록하고 표시할 수 있지만, 그 표시가 OS 강제력, 임의 도구 격리, 변조 방지 저장소, 넓은 보안 격리를 뜻하지는 않습니다.

탐지형 표현은 대상 관찰 범위에 필요한 활성 역량 확인이 통과한 뒤에만 사용할 수 있습니다. 예방형이나 격리형 표현은 별도로 문서화되고 증명된 메커니즘이 있어야 현재 범위가 될 수 있습니다.

`harness.prepare_write`와 `Write Authorization`은 협력형 제품 파일 쓰기 호환성 메커니즘입니다. OS 권한을 부여하지 않고, 임의 도구를 샌드박스 처리하지 않으며, 모든 도구가 실행 전에 막혔음을 증명하지 않고, 로컬 파일을 변조 방지 상태로 만들지 않습니다.
...
| Core 권한, Task 상태, 사용자 소유 판단 경계 | [Core Model](core-model.md) |
...
| 공개 오류와 닫기 준비 상태 차단 사유 동작 | [오류](api/errors.md) |
