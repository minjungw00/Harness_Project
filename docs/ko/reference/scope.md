...
- 해당 기능은 에이전트, 구현자, 유지보수자가 의존할 수 있는 기준 계약에 속합니다.
...
- 경로 문서, 예시, 스키마 메모, 값 집합이 지원 범위 밖 기능을 언급한다고 해서 그 기능이 활성화되는 것은 아닙니다.
...
| 닫기 시도 | `harness.close_task`는 증거, 최종 수락, 잔여 위험, 대체 불가 경계를 유지하면서 닫기 준비 상태를 평가하고 지원되는 닫기 결과를 시도할 수 있습니다. | [Task 닫기 메서드](api/method-close-task.md), [Core 모델](core-model.md), [오류](api/errors.md) |
...
아래 기능 묶음은 이 범위 참조와 영향받는 담당 문서가 승격하기 전까지 지원되는 기준 범위 밖에 있습니다.

포함되지 않는 것:

- 접점 자체 아티팩트 캡처
- 지속 저장되는 상태 보기 작업, 상태 보기 조정, 생성된 상태 보기 파일, 관리되는 상태 보기 복구
- 전체 `Evidence Manifest` 생성
- 수동 QA 작업 흐름과 검증 게이트 작업 흐름
- 명령, 네트워크, 비밀값 접근 관찰
- 명령, 네트워크, 비밀값의 도구 실행 전 차단
- 예방형 격리, 샌드박싱, 또는 `isolated` 보장 의미
- 호스팅 대시보드
- 커넥터 마켓플레이스
- 내보내기 또는 전달 패키지
- 실행 가능한 픽스처 실행기
...
- 민감 동작 승인은 관련 담당 문서가 그 기능을 지원된다고 정의하지 않는 한 관찰, 차단, 격리, QA, 검증 동작을 만들지 않습니다.

담당 문서 링크:

- 보안 비주장, 보장 수준, 관찰 경계: [보안](security.md).
- 값 이름과 예약된 값: [API 값 집합](api/schema-value-sets.md).
- 적합성 절차와 점검: [적합성](conformance.md).
...
- 예약된 값은 이름만으로 동작을 활성화하거나 기본값을 만들거나 닫기 준비 상태를 충족하거나 보장을 만들지 않습니다.
...
- 이 문서와 의미 담당 문서가 모두 해당 동작을 정의하지 않는 한 기본값, 필수값, 지원됨, 강제됨, 수락됨, 검증됨, 닫기 준비 상태, 탐지형, 예방형, 격리됨처럼 설명하지 않습니다.

담당 문서 링크:

- 정확한 값 이름과 값 집합 위치: [API 값 집합](api/schema-value-sets.md).
- `isolated`를 포함한 보장 의미와 비주장: [보안](security.md).
...
해당 기능의 현재 담당 문서가 없다면 지원 기능으로 설명하기 전에 담당 문서를 만들거나 지정해야 합니다. 자리표시자를 현재 담당 문서처럼 독자에게 안내하지 않습니다.

의미하지 않는 것:

- 제외된 기능, 예약된 값, 프로필 조건부 기능이 예시, 경로 문구, 스키마 메모, 값 집합, 이 참조 문서에 언급된다고 해서 승격되는 것은 아닙니다.
...
- 기준 범위는 `isolated` 보장 의미를 제공하지 않습니다.
...
세부 계약 질문에는 아래 담당 문서를 사용합니다. 이 표는 책임 경로를 안내하며, 해당 계약을 중복 정의하지 않습니다.
