...
이 참조 문서는 Volicord의 지원되는 기준 범위 경계를 담당합니다. 제품 정체성은 [시작하기 개요](../getting-started/overview.md)가 담당합니다. 이 문서는 기준 범위 안에 있는 기능 묶음, 기준 범위 밖에 남는 기능, 예약된 값과 프로필 조건부 값이 범위에 미치는 영향을 정의합니다.
...
- 기준 범위 구현 읽기 경로. [구현 가이드](../development/change-guide.md)를 봅니다.
- API 메서드 동작
- 스키마 필드
- 저장소 기록이나 저장 효과
...
어떤 기능이 지원되는 기준 범위에 속하는지 판단할 때는 이 문서를 기준으로 삼습니다. 경로 문서, 개발 문서, README, 참조 문서는 상세 범위 목록을 반복하지 말고 이 문서로 연결합니다.
...
| 쓰기 준비 권한 부여 | `volicord.prepare_write`는 호환되는 제품 파일 쓰기 시도 하나를 위해 담당 범위의 1회용 `Write Authorization`을 만들 수 있습니다. | [쓰기 준비 메서드](api/method-prepare-write.md), [저장 효과](storage-effects.md), [보안](security.md) |
| 로컬 접점 등록 | 등록된 로컬 접점은 범위 확인에 쓸 선택된 접점과 지원 역량을 식별할 수 있습니다. | [에이전트 통합](agent-integration.md), [접점별 사용 레시피](../guides/surface-recipes.md), [보안](security.md) |
...
| 닫기 시도 | `volicord.close_task`는 증거, 최종 수락, 잔여 위험, 대체 불가 경계를 유지하면서 닫기 준비 상태를 평가하고 지원되는 닫기 결과를 시도할 수 있습니다. | [Task 닫기 메서드](api/method-close-task.md), [Core 모델](core-model.md), [API 차단 사유 처리 경로](api/blocker-routing.md) |
...
Volicord의 지원되는 기준 범위는 Volicord를 아래 제품 역할로 만들지 않습니다.

- 코딩 에이전트 또는 코드 생성 에이전트
- 전체 워크플로 엔진 또는 워크플로 오케스트레이터
- 테스트 러너 또는 독립적인 제품 정확성 검증기
- 기술 부채 분석기 또는 상환 시스템
- 보안 샌드박스, OS 수준 격리 계층, 전역 파일시스템 모니터, 외부 파일 변경 방지 시스템
- 전체 에이전트 하네스, 호스팅 에이전트 플랫폼, 대시보드, 보고서 생성기

Volicord는 AI 지원 작업 주변의 담당 문서가 정의한 상태를 기록하거나, 경로를 잡거나, 요약하거나, 표시할 수 있습니다. 이런 지원 동작이 Volicord를 제외된 제품 역할로 승격하지 않습니다.
...
- 제외된 기능은 이 범위 참조와 영향받는 기존 담당 문서가 지원된다고 명시적으로 정의할 때만 지원 동작이 됩니다.
- 그 전에는 지원 동작이 아닙니다.
...
- `volicord.prepare_write`와 `Write Authorization`은 제품 파일 쓰기 호환성 메커니즘이며 격리나 샌드박싱 보장이 아닙니다.
