...
이 참조 문서는 독립적인 기준 범위 관문, 지원되는 설계 품질 차단 사유 범주, 지원되는 검증기 계열, 품질 면제 경로, 심각도 기반 차단 정책, 증거 권한, QA 결과, 수락 판단, 잔여 위험 판단, 닫기 권한, 운영 보고서, 적합성 목록, SQLite DDL, 지속 테이블, 상태 보기 권한, 렌더링된 보고서, 템플릿 본문을 정의하지 않습니다.
...
| 경계 | 규칙 |
|---|---|
| 독립적인 닫기 권한 | 설계 품질 발견 사항은 자동으로 닫기 차단 사유, 차단 사유 범주, 범위 재정의, 증거 규칙, 보장이 되지 않습니다. |
