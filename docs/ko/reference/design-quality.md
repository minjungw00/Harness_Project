...
이 문서는 독립적인 기준 범위 관문, 설계 품질 전용 지원 닫기 차단 범주, 지원 검증기 계열, 품질 면제 경로, 심각도 기반 차단 정책, 증거 권한, QA 결과, 수락 판단, 잔여 위험 판단, 닫기 권한을 정의하지 않습니다.
...
- 기준 범위 설계 품질 심각도 경계. 심각도 형태의 문구는 영향받는 담당 문서가 별도 행동을 요구하지 않는 한 조언성 우선순위입니다.
...
- MCP 요청/응답 스키마, `ValidatorResult`, `UserJudgment`, `AcceptedRiskInput`, 기준 범위/지원 범위 밖 스키마 값. [API 메서드](api/methods.md), 메서드 담당 문서, [API 코어 스키마](api/schema-core.md), [API 판단 스키마](api/schema-judgment.md)를 봅니다.
- 공개 오류 의미. [API 오류 코드](api/error-codes.md), [API 오류 처리 경로](api/error-routing.md), [API 오류 우선순위](api/error-precedence.md), [API 오류 세부사항](api/error-details.md)을 봅니다.
...
기준 범위에서 설계 품질은 판단 라우팅을 하고 증거와 범위를 참조하는 좁은 층입니다. 품질 관련 우려를 이해할 수 있게 만들고, 그 우려를 영향받는 담당 문서로 보냅니다.
...
| 영향받는 담당 문서 없음 | [영향받는 담당 문서 없음](#design-quality-no-applicable-owner-path) 참조 |
...
라우팅:
- 영향받는 담당 문서가 별도 행동을 요구하지 않는 한 심각도 형태의 문구는 조언성 우선순위로 다룹니다.
...
다른 영향받는 담당 문서가 좁은 일부를 명시적으로 요구하지 않는 한, 아래 항목은 기준 범위 차단 사유가 아닙니다.
...
설계 품질 관찰 사항은 집중 담당 문서가 그 효과를 정의할 때만 기준 범위 상태에 영향을 줍니다. 관찰 사항은 자신이 의존하는 경로를 이름 붙여야 합니다.
...
설계 품질 관찰 사항은 집중 담당 문서가 닫기 관련 차단 사유나 판단을 정의할 때만 닫기를 차단합니다.
...
- 관찰 사항이 닫기 차단 집합 안의 기존에 지원되는 `CloseReadinessBlocker.category`, 지원되는 `judgment_kind`, 지원되는 API 오류, 또는 다른 영향받는 담당 문서를 이름 붙입니다.
...
라우팅:
- 영향받는 담당 문서가 그 좁은 행동을 필요로 할 때만 조언성 다음 행동, 증거 요청, 집중된 사용자 판단, 잔여 위험 표시를 사용합니다.
...
기준 범위에는 별도의 품질 면제 경로가 없습니다. 어떤 요구사항을 유예하거나, 위험으로 수락하거나, 사용자 판단으로 해결할 수 있는지는 영향받는 담당 문서가 정합니다. 그 문서의 정확한 `judgment_kind`, 차단 사유 범주, 증거 동작을 사용해야 합니다.
...
조건:
- 닫기 근거가 보이고 영향받는 담당 문서가 사용자의 결과 판단을 요청합니다.
...
닫기 영향:
- 판단을 요청한 담당 문서를 통해서만 닫기에 영향을 줄 수 있습니다.

허용되지 않는 것:
- 포괄 승인, 별도 품질 면제, 지원되지 않는 판단 범주로 취급하지 않습니다.

넓은 승인, "좋아 보인다" 같은 말, 일반적인 진행 승인은 영향받는 담당 문서가 그 특정 판단을 물은 경우가 아니라면 위 판단으로 취급하면 안 됩니다.
...
- 지속 `ArtifactRef` 값, 실행 기록 참조, 명령/확인 요약, 출처 참조
...
지원 범위 밖 품질 자료를 기준 범위 요구사항, 차단 사유, 면제 규칙, 증거 기대치, 검증기 매핑, 적합성 시나리오 요구사항, 운영 보고, 구현 작업처럼 제시하면 안 됩니다.
