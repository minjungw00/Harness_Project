...
기준 범위에서 설계 품질은 판단 라우팅을 하고 증거와 범위를 참조하는 좁은 층입니다. 품질 관련 우려를 읽을 수 있게 만들고, 그 우려를 영향받는 담당 문서로 보냅니다.
...
기준 범위 밖 품질 정책 자료는 다른 영향받는 담당 문서가 좁은 행동을 명시적으로 요구하지 않는 한 조언으로만 쓰일 수 있습니다.
...
기준 범위에는 별도의 품질 면제 경로가 없습니다. 어떤 영향받는 담당 문서가 요구사항을 유예하거나, 위험으로 수락하거나, 사용자 판단으로 해결하도록 허용한다면 그 문서의 정확한 `judgment_kind`, 차단 사유 범주, 증거 동작을 사용합니다.
...
- 넓은 승인, "좋아 보인다" 같은 말, 일반적인 진행 승인은 영향받는 담당 문서가 그 특정 판단을 물은 경우가 아니라면 위 판단 중 하나가 아닙니다.
...
[API 값 집합](api/schema-value-sets.md)이 공개한 지원 값 밖의 검증기 ID는 [범위 참조](scope.md)와 영향받는 담당 문서가 좁은 지원 계약을 정의하지 않는 한 기준 범위 효과가 없습니다.
