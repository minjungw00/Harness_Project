...
활성 담당 경로가 지원하면 Product Repository에는 생성된 읽기용 출력이 있을 수 있습니다. Projection, 템플릿, 상태 카드, 작은 증거 요약, 닫기 준비 상태 보기, 관리형 Markdown 블록 같은 출력입니다. 이 파일은 사람과 에이전트가 작업을 읽도록 돕습니다. Core가 소유한 상태가 아니라 파생 표시입니다. 사람이 편집할 수 있는 제안 영역은 Core 상태 변경 조치가 수락하기 전까지 입력일 뿐입니다.
...
Projection, 템플릿, 상태 카드, 생성된 Markdown, 읽기 전용 상태 리소스는 파생 표시입니다. Core가 소유한 상태 기록과 등록된 아티팩트 참조에서 렌더링됩니다. 최신성, 실패, 차단 사유, 다음 행동 정보를 담을 수 있지만, 그 정보도 담당 기록을 보여주는 표시일 뿐 두 번째 권한 근거가 아닙니다.
...
관리되는 생성 영역은 계속 파생 표시입니다. 사람이 편집할 수 있는 영역은 제안 입력입니다. 제안은 Core 소유 경로가 상태 변경 조치로 수락한 뒤에만 하네스 상태에 영향을 줍니다.
...
아티팩트 등록은 담당 문서가 문서화한 활성 `stage_artifact` 스테이징 핸들이나 호환되는 기존 `ArtifactRef`만 사용합니다. 원시 로컬 경로, 복사된 파일, `captured_artifact` 핸들, 원시 캡처 어댑터 출력, 접점 자체 캡처 주장은 현재 MVP의 등록 권한이 아닙니다.
