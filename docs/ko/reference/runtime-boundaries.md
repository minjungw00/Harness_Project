...
| `Agent Connection` | `connection_id`로 식별되는 로컬 MCP 호스트 연결 단위입니다. `connection.mode`는 `read_only` 또는 `workflow`입니다. | OS 샌드박스, 파일시스템 ACL, 네트워크 정책, 비밀값 격리 장치, 사용자 판단 경로로 보면 안 됩니다. |
| `Connection Projects` | Agent Connection이 다룰 수 있는 `project_id` 값의 명시적 허용 목록입니다. | 기본적으로 등록된 모든 프로젝트를 포함하거나 Product Repository 권한을 증명한다고 보면 안 됩니다. |
...
Agent Connection은 `volicord-mcp`를 위한 로컬 MCP 호스트 연결 단위입니다. 연결은 `connection_id`로 식별되고, `connection.mode=read_only` 또는 `connection.mode=workflow`를 가지며, Connection Projects 허용 목록에 명시된 `project_id` 값만 다룰 수 있습니다.
...
추론하면 안 되는 것:
- 복사된 `connection_id`가 권한, 사용자 신원, OS 권한, 호스트 신뢰, 역량을 증명한다는 주장.
