...
`harness.stage_artifact`는 로컬 아티팩트 유틸리티이지 Core 변경 권한이 아닙니다. 임시 `StagedArtifactHandle`을 만들 수 있지만, 호환되는 Core/API 경로가 이를 소비하기 전까지 기준 증거를 만들거나, gate를 만족하거나, 지속 `ArtifactRef`를 등록하거나, 닫기 준비 상태에 영향을 주지 않습니다. 호출자가 제공한 안전한 바이트 또는 안전한 알림을 스테이징할 뿐이며, 임의 로컬 파일이 안전하거나 허가되었거나 하네스가 관찰했다는 증명이 아닙니다.
...
아티팩트 읽기도 같은 권한 경계를 사용합니다. 원시 아티팩트 저장소 경로는 로컬 호출자에게 기본으로 허용되지 않습니다. 아티팩트 메타데이터나 바이트를 읽으려면 등록된 `ArtifactRef`, 호환되는 project/task 맥락, 일치하는 담당 관계, [Storage](storage.md)와 [API Schema Core](api/schema-core.md#artifactref)가 담당하는 가림/가용성 확인이 필요합니다. 아티팩트 본문 읽기는 추가로 `access_class=artifact_read`에 대한 서버 확인 로컬 접점 맥락을 요구합니다. 복사된 `surface_id`, 생성된 Markdown 주장, Product Repository 파일은 아티팩트 바이트 접근을 승인하지 못합니다.

아티팩트 등록은 활성 `harness.stage_artifact` 유틸리티가 만든 담당 문서의 `StagedArtifactHandle`이나 호환되는 기존 `ArtifactRef`만 사용합니다. 새 아티팩트 바이트는 입력 스테이징을 통해서만 현재 MVP에 들어오고, 기존 바이트는 `existing_artifact` 참조를 통해서만 재사용합니다. 원시 로컬 경로, 원시 로그, 임의 로컬 경로 문자열, 복사된 파일, `captured_artifact` 핸들, 원시 캡처 어댑터 출력, 접점 자체 캡처 주장, 로컬 파일이 안전하다는 주장은 현재 MVP의 등록 권한이 아닙니다.
