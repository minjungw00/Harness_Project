...
`volicord setup`은 로컬 setup 프로필을 마련합니다. Setup은 Runtime Home 경로나
MCP 명령 위치를 직접 선택하는 유일한 기준 명령입니다.
...
| `--link-bin PATH` | `volicord` 실행 파일을 위한 사용자 선택 명령 링크를 설치하거나 갱신합니다. 명령은 대상 경로를 보고하고 안전하지 않은 교체를 거절합니다. |
| `--mcp-command PATH` | 관리 호스트 설정과 generic export가 `volicord-mcp`를 시작할 때 사용할 명령을 저장합니다. 생략하면 실행 중인 `volicord` 실행 파일 옆의 `volicord-mcp`, 그다음 `PATH`의 명령을 찾습니다. |
| `--json` | 기계 판독 출력을 선택합니다. |

Setup 효과:

- Runtime Home registry를 만들거나 검증합니다.
- Runtime Home 식별 정보와 setup 프로필 메타데이터를 기록합니다.
- 이후 `connect`, `doctor`, export 흐름이 사용할 MCP 명령 위치를 기록합니다.
- `--link-bin`으로 지정한 명령 링크를 갱신할 수 있습니다.
...
registry, 저장소 프로필 가용성, MCP 명령 준비 상태, 명령 링크가 있을 때의 준비
상태, 작업 디렉터리가 있을 때의 현재 저장소 감지를 확인합니다. 프로젝트를 만들거나,
호스트 설정을 설치하거나, 연결 모드를 바꾸거나, 사용자 판단에 답하지 않습니다.
