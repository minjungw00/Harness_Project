...
| `--link-bin PATH` | 필요하면 디렉터리를 만들고 쓰기 가능 여부를 확인한 뒤, 가능할 때 그곳에 `volicord`와 `volicord-mcp` 둘 다에 대한 명령 링크를 만들거나 갱신합니다. 명령은 각 대상 경로를 보고하고 안전하지 않은 교체를 거절하며, 이 옵션 자체가 셸 시작 파일이나 부모 셸 `PATH`를 편집하지는 않습니다. |
...
- 대화형 텍스트 모드에서는 안전한 명령 가용성 선택지를 물어볼 수 있습니다.
  ...
  `HOME` 아래의 없는 관례적 사용자 명령 디렉터리를 setup이 안전하게 만들 수 있고
  생성 뒤 쓰기 가능성을 확인할 수 있을 때 그곳에 링크 생성, 승인된 셸 시작
  `PATH` 블록 쓰기, 셸 명령 출력, 링크 건너뛰기입니다.
...
`volicord-mcp`를 시작할 수 있도록 저장된 `connection_internal_id`에서 파생된
