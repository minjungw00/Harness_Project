...
setup의 최상위 상태는 안내형 첫 실행 설정 경험에 이름 붙은 사용자 동작이 아직
필요한지를 답합니다. Runtime Home과 설치 프로필을 저장한 뒤에도 선택된 명령을 이후
셸이나 에이전트 호스트가 `PATH`로 찾을 준비가 되어 있지 않으면 setup은
...
표시합니다. guard 설치 기록이 있으면 doctor는 guard 파일 설치, 호스트 reload 필요,
hook 관찰, active/stale/broken guard 상태도 진단으로 보고할 수 있습니다. 이 guard
진단은 로컬 setup 및 관찰 점검이며 OS 강제, sandboxing, 쓰기 방지, 제품 정확성,
닫기 준비 상태의 증명이 아닙니다. 프로젝트를 만들거나, 호스트 설정을 설치하거나,
연결 모드를 바꾸거나, 사용자 판단에 답하지 않습니다.
...
`volicord init --host codex --repo PATH`와
`volicord init --host claude-code --repo PATH`는 chat-first 사용을 위한 기본 첫 실행
저장소 설정 및 호스트 연결 명령입니다. 이 명령은 shared 프로젝트 범위 호스트
레이아웃을 사용하므로 생성된 호스트 MCP 설정은 `PATH`를 통해 `volicord mcp --stdio`를
시작하고 개인 Runtime Home 경로를 포함하지 않습니다.
...
- `guarded`가 기본값입니다. MCP 설정, 관리되는 `AGENTS.md` 안내 블록,
  `.volicord/policy.json` guard 명령 policy, 그리고 호스트가 지원하는 프로젝트 로컬
  rule 관례가 있을 때 호스트 rule 파일을 씁니다.
- `managed`는 `guarded`와 같은 설정 표면을 사용하며, 이를 구분하는 호스트나 미래
  통합을 위해 managed guard 모드를 기록합니다.

`guarded`와 `managed`에서 init은 생성된 guard hook을 로드하려면 호스트 restart
또는 reload가 아직 필요할 때 `reload_required`를 기록하고, 파일은 설치되었지만
일치하는 guard hook이 아직 관찰되지 않았을 때 `configured`를 기록합니다. 파일을
썼다는 이유만으로 guard 설치를 `active`로 표시하지 않습니다.
...
- `.claude/rules/volicord.md` 같은 지원 호스트 rule 파일을 씁니다.
...
|---|---|---|---|
| `volicord init` | 필요하면 Runtime Home과 설치 프로필을 초기화하고, 선택된 저장소 프로젝트를 등록하거나 재사용하며, shared 프로젝트 범위 Agent Connection을 만들거나 갱신하고, Connection Projects 멤버십을 보장하며, guard 설치 상태를 기록합니다. | `codex` 또는 `claude-code`를 위한 관리 프로젝트 로컬 MCP 설정, `AGENTS.md` 안내, `.volicord/policy.json`, 지원 호스트 rule 파일을 설치하거나 갱신합니다. | 관찰 가능한 곳에서 호스트 설정, MCP 시작, 초기화, `tools/list` 점검을 실행한 뒤 필요한 host reload, restart, trust, approval 동작을 보고합니다. |
...
합니다. 연결 상태 및 검증 출력은 guard 파일 설치, 호스트 reload 필요, hook 관찰,
prompt-capture 가용성, 알 수 있을 때의 최근 guard event, guard 상태를 별도 진단으로
유지해야 합니다. 파일이 설치되었거나 설정되었다는 사실을 활성 관찰된 guard hook으로
보고하면 안 됩니다.
...
호스트 종류, guard 모드, policy hash와 일치하는 유효한 event를 받으면 Volicord는
관찰 메타데이터를 기록하고 그 guard 설치를 `active`로 승격할 수 있습니다.
프로젝트, 연결, 호스트 종류, guard 모드, policy hash 데이터가 맞지 않으면 설치를
활성화하지 않습니다. `active`는 Volicord가 일치하는 hook event를 관찰했다는
뜻입니다. OS 수준 집행, sandboxing, 쓰기 방지를 주장하지 않습니다.
...
setup과 doctor JSON은 진단 소비자가 첫 실행 설정 준비 상태와 설치 프로필 상태를
구분할 수 있도록 `status_meaning`을 포함해야 합니다. doctor JSON은 최상위 상태가
