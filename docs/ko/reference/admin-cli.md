...
- 관리 명령은 공개 Volicord API 메서드가 아니며 공개 메서드 목록에 추가되면
  안 됩니다.
- 텍스트 모드 사용자 흐름은 내부 프로젝트 식별 정보, Agent Connection 식별 정보,
...
등록은 내부 프로젝트 식별 정보, 사용자 대상 프로젝트 이름, Runtime Home 아래의
...
이름을 바꿉니다. 내부 프로젝트 식별 정보, 저장소 루트, 프로젝트 홈, Core 상태는
바꾸지 않습니다.
...
`volicord-mcp`를 시작할 수 있도록 내부 연결 식별 정보, 서버 이름, 명령 인자가 들어갈 수
...
식별자나 선택지 식별자는 참조와 JSON 세부사항이며 일반 입력으로 필요하지
않습니다.
