...
명령 경로를 저장합니다. Setup은 Runtime Home 경로나 MCP 시작 명령 위치를 직접
선택하는 유일한 기준 명령입니다. `volicord`를 `PATH`에서 사용할 수 있게 돕지만 부모
...
- 이후 `connect`, `doctor`, export, MCP 시작 흐름이 사용할 `volicord` 명령 위치와 MCP
  시작 명령을 기록합니다.
