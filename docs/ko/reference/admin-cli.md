...
- Codex 설치는 선택한 호스트 범위가 선택된 Runtime Home 경로 저장을 허용할 때 선택적 `env.VOLICORD_HOME`과 함께, `command`, `args = ["--integration", "<integration_id>"]`를 가진 `[mcp_servers.<server_name>]`와 동등한 MCP 서버 테이블을 씁니다.
- Claude Code 설치는 선택한 호스트 범위가 선택된 Runtime Home 경로 저장을 허용할 때 선택적 `env.VOLICORD_HOME`과 함께, `command`, `args`를 가진 `mcpServers.<server_name>` MCP 서버 항목을 씁니다.
- Generic export는 같은 command, args, environment 값을 호스트 중립 JSON 객체로 출력합니다.
- Generic export가 명시적 `--export-path` 대신 내보내기 디렉터리에 쓸 때 생성 파일 이름은 `volicord-<integration>.mcp.json`입니다.
...
- 새 기준 호스트 설정은 `VOLICORD_PROJECT_ID`, `VOLICORD_SURFACE_ID`, `VOLICORD_SURFACE_INSTANCE_ID`를 요구하면 안 됩니다.
...
| 상태 | 의미 |
|---|---|
| `complete` | 오래 유지되는 통합 상태가 있고, 관리되는 호스트 설정이 존재하며 예상 관리 지문과 일치하고, 호스트별 로드 가능성 게이트가 충족되고, 필요한 신뢰나 승인 동작이 남아 있지 않고, 통합 사전 점검이 성공하고, MCP 초기화가 성공하고, `tools/list`가 필요한 도구를 노출합니다. |
...
성공한 `volicord-mcp --check --integration <integration_id>`만으로는 전체 호스트 통합을 `complete`로 설명하면 안 됩니다. 이는 MCP 프로세스의 시작 검증일 뿐입니다.
...
- `volicord-mcp --check`는 통과했지만 MCP 초기화나 도구 탐색이 성공하지 않았다면 결과는 `complete`가 될 수 없습니다.
- Volicord가 직접 시작한 MCP handshake는 Codex 또는 Claude Code가 서버를 로드, 신뢰, 승인, 노출했다는 사실을 증명하지 않습니다.

## 통합 프로젝트 멤버십 명령

`volicord agent project add`는 기존 통합에 허용 프로젝트 하나를 추가하거나 복원합니다.
...
`volicord agent project remove`는 기존 통합에서 허용 프로젝트 하나를 제거합니다.
...
`volicord agent project default set`은 기존 통합의 기본 프로젝트를 설정합니다.
...
`volicord agent project default clear`는 기존 통합의 기본 프로젝트를 지웁니다.
...
`volicord agent status`는 호스트 담당 문서가 가벼운 상태 점검을 정의하지 않는 한 호스트를 시작하지 않고 레지스트리와 호스트 인벤토리 상태를 보고합니다.
...
`volicord agent verify`는 하나의 통합 또는 하나의 설치에 대한 검증 상태를 갱신합니다.

선택 규칙:

- `volicord agent verify --installation-id <id>`는 정확히 그 Host Installation 하나를 검증하며, 그 설치가 다른 통합에 속하면 실패합니다.
...
- `volicord-mcp --check --integration <integration_id>`가 성공합니다.
- MCP 초기화가 성공합니다.
- `tools/list`가 공개 Volicord 도구 아홉 개와 `volicord.list_projects`를 노출합니다.
...
`volicord agent uninstall`은 선택된 관리 Host Installation 하나 이상을 제거합니다. 성공한 제거는 소유권과 안전 점검이 허용할 때 일치하는 관리 호스트 설정을 제거하고, 해당하는 각 Host Installation 인벤토리 기록을 제거하며, 그 Agent Integration Profile에 남은 Host Installation이 없으면 Agent Integration Profile을 비활성화할 수 있습니다.
...
- Codex: `AGENTS.md`의 Volicord 관리 블록.
- Claude Code: `.claude/rules/volicord.md`에 있는 Volicord 관리 Markdown 규칙 파일.

규칙:

- `volicord agent guidance apply`는 `--integration-id`, `--project-id`, `--host`, 그리고 비대화식 실행에서 `--allow-repository-write`를 요구합니다.
- 명령은 정확한 파일 경로와 관리 내용을 미리 보여 줘야 합니다.
- 비관리 충돌을 감지해야 하며, `--replace-managed`는 일치하는 이전 관리 내용에만 적용되어야 합니다.
- 관리 지침에는 Volicord 관리와 통합을 식별하는 소유 마커가 들어가야 합니다.
- 관리 저장소 지침 지문은 형식 식별자 `volicord-repository-guidance-v1`을 사용합니다.
- Codex 관리 지침 블록은 정확한 마커 `<!-- BEGIN VOLICORD MANAGED GUIDANCE v1 -->`와 `<!-- END VOLICORD MANAGED GUIDANCE v1 -->`를 사용합니다.
- `volicord agent guidance status`는 관리 마커 상태를 읽고 지침이 없음, 있음, 변경됨, 충돌 중 어느 상태인지 보고합니다.
- `volicord agent guidance remove`는 일치하는 관리 내용만 제거하며 비대화식 실행에서는 `--remove-managed`를 요구합니다.
- 지침은 Volicord MCP 서버 지침과 저장소 지침이 도구 선택을 도울 수 있지만 모델 동작을 보장할 수 없다고 밝혀야 합니다.
...
- `--remove-managed`는 Volicord 관리 내용의 안전한 제거에만 적용됩니다.
...
`volicord surface register`는 등록된 프로젝트에 로컬 접점 인스턴스 하나를 기록합니다.
...
- 코딩 에이전트 MCP 통합에는 `volicord agent install`을 선호합니다. 이 명령은 통합 프로필, 프로젝트 멤버십, 호스트 설치, 프로젝트별 접점 바인딩을 함께 만들거나 검증합니다.
- 낮은 수준의 `volicord surface register --kind mcp`는 명시적 관리 복구나 사용자 지정 자동화를 위해 계속 사용할 수 있습니다.
...
`volicord surface list --project-id ID`는 선택된 Runtime Home에서 한 프로젝트의 등록된 접점을 나열합니다.
...
관리 CLI는 로컬 리소스를 초기화하고 등록할 수 있습니다. 그 자체로 공개 Volicord API 메서드를 만들지 않으며 Core 권한, `Write Authorization`, 증거 충분성, 닫기 준비 상태, 사용자 소유 판단, 수락, 잔여 위험 수락, 아티팩트 권한, 보안 보장을 만들지 않습니다.
