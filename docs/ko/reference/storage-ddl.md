...
`registry.sqlite`는 Runtime Home 식별 정보, 프로젝트 등록, Agent Connection 기록, Connection Projects 멤버십, 호스트 설정 인벤토리를 저장합니다. 프로젝트별 Core 상태는 저장하지 않습니다.
...
- `agent_connections`는 로컬 MCP 호스트 연결 단위 하나, 그 모드, 활성 상태, 호스트 설정 인벤토리, 마지막 검증 상태를 저장합니다.
...
- `agent_connections.host_kind`와 `agent_connections.host_scope`는 [관리 CLI](admin-cli.md)가 정의하는 지원 호스트/범위 행렬로 제한됩니다.
