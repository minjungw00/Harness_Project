...
수용하거나 close하기 전에는 결과가 scope와 맞는지, evidence가 acceptance criteria를 뒷받침하는지, verification이 통과했거나 recorded risk와 함께 명시적으로 waived되었는지, Manual QA가 통과/완료되었거나 validly waived되었는지, close-relevant residual risk가 표시되었거나 agent가 `ResidualRiskSummary.status=none`을 report했는지, final acceptance가 approval과 별도인지 확인한다.
