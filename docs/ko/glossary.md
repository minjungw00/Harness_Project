...
Record-level Manual QA result입니다. performer, profile, result, artifacts, findings, applicable한 경우 waiver reason, next action을 포함합니다. Result value set은 [QA Gate](03-kernel-spec.md#qa-gate)와 [`harness.record_manual_qa`](05-mcp-api-and-schemas.md#harnessrecord_manual_qa)가 담당합니다. Pending required QA는 `qa_gate=pending`으로 표현하며 Manual QA record result가 아닙니다.
...
Projection job과 template kind를 나타내는 API enum입니다. Tier, values, extension rules는 [Shared Schemas](05-mcp-api-and-schemas.md#shared-schemas)가 담당합니다. 어떤 ProjectionKind도 projection을 canonical state로 만들지 않습니다.

### Projection Freshness

Projection과 source records, managed hash, artifact refs, projection job state 사이의 관계입니다. Value set은 [MCP API와 스키마](05-mcp-api-and-schemas.md)와 [Document Projection](07-document-projection.md)이 담당합니다.
