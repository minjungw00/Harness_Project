...
Record-level Manual QA result입니다. performer, profile, result, artifacts, findings, applicable한 경우 waiver reason, next action을 포함합니다. `qa_gate`에 feed되지만 그 자체가 canonical gate는 아닙니다.
