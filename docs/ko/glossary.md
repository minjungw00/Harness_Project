...
Required user acceptance를 위한 kernel gate입니다. Value set과 compatibility meaning은 [Acceptance Gate](03-kernel-spec.md#acceptance-gate)가 담당합니다. Acceptance는 QA나 verification을 대신할 수 없습니다.
...
Ignored paths를 제외한 뒤 sorted NFC-normalized relative POSIX paths, file bytes, size, executable bit, symlink target handling을 사용해 계산하는 baseline file snapshot의 deterministic hash입니다. 세부 규칙은 [Storage와 DDL](reference/storage-and-ddl.md)이 정의합니다.
...
Bilingual parity, links, owner boundaries, stable catalogs, glossary terms, source-of-truth phrasing, TODO usage, non-owner duplicate contracts의 drift를 detect하는 read-only documentation maintenance check profile입니다. Rule bodies는 Authoring Guide가 담당하고, operator reporting과 entrypoint expectation은 Operations And Conformance가 담당합니다. Core fixture conformance, runtime validator, projection refresh, QA, acceptance, evidence, residual-risk acceptance, canonical state transition이 아닙니다. `harness conformance run` 또는 다른 operator command로 expose되면 명시적으로 선택하는 separate docs-only profile이며, 관례적 이름은 `docs-maintenance`입니다. Console 또는 ephemeral result는 runtime fixture pass/fail, implementation readiness, close readiness, Task state에 영향을 주지 않습니다.
...
Required evidence coverage를 위한 kernel gate입니다. Value set과 close meaning은 [Evidence Gate](03-kernel-spec.md#evidence-gate)가 담당합니다.
...
Record-level Manual QA result입니다. performer, profile, result, artifact, finding, applicable한 경우 waiver reason, next action을 포함합니다. Result value set은 [QA Gate](reference/kernel.md#qa-gate)와 [`harness.record_manual_qa`](reference/mcp-api-and-schemas.md#harnessrecord_manual_qa)가 담당합니다. Pending required QA는 `qa_gate=pending`으로 표현하며 Manual QA record result가 아닙니다.
...
Projection job과 template kind를 나타내는 API enum입니다. Tier, values, extension rules는 [Shared schemas](reference/mcp-api-and-schemas.md#shared-schemas)가 담당합니다. 어떤 ProjectionKind도 projection을 canonical state로 만들지 않습니다.
