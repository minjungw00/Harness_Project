...
아래 항목은 승격 전 후보 목록입니다. 현재 MVP 요구사항, 활성 API/schema 계약, fixture 본문, template 본문, 런타임 동작, 구현 작업, 생성 산출물, 수락 증거가 아닙니다. 상세 본문 없음.
...
|---|---|---|---|
| Later candidate index | 활성 문서 색인일 뿐입니다. | 세부 계약을 되살리려면 향후 owner가 좁은 후보를 먼저 승격해야 합니다. | 없음 |
...
| later 후보 | 현재 상태 | 승격 조건 | 현재 MVP 영향 |
|---|---|---|---|
| Owner decision | 후보는 승격 전까지 보관 상태입니다. | 이름 있는 향후 owner, 좁은 범위, 대체 동작, 명시적인 비보장이 필요합니다. | 없음 |
...
| Active-scope inflation check | Later 자료는 기본적으로 첫 내부 smoke 목표와 현재 MVP 밖에 남습니다. | 승격 후보가 earlier active scope에 unsupported requirement를 더하지 않는다는 확인이 필요합니다. | 없음 |
...
| later 후보 | 현재 상태 | 승격 조건 | 현재 MVP 영향 |
|---|---|---|---|
| assurance hardening | Later 후보입니다. 상세 본문 없음. | Exact gate, fallback behavior, proof expectation을 갖춘 owner-scoped assurance behavior가 필요합니다. | 없음 |
...
| 전체 설계 품질 later 정책 후보 | 이름만 남깁니다: full `shared_design` policy, `domain_language`, `vertical_slice`, `feedback_loop`, `tdd_trace`, `deep_module_interface`, `codebase_stewardship`, detailed `manual_qa`, `two_stage_review_display`, detached-verification policy, steward policy. | 향후 owner가 좁은 family 하나를 exact scope, fallback behavior, validator boundary, 면제와 증거 규칙, proof expectation과 함께 승격해야 합니다. | 없음 |
...
| later 후보 | 현재 상태 | 승격 조건 | 현재 MVP 영향 |
|---|---|---|---|
| operations hardening | Later 후보입니다. 상세 본문 없음. 활성 Reference owner는 없습니다. | Owner가 범위를 정한 운영 동작이 필요합니다. 정확한 운영자 명령, 진단, 대체 동작, 증명 기대치를 함께 정의해야 합니다. | 현재 MVP 영향 없음 |
...
| Later next-action values | `launch_verify`, `record_eval`, `record_manual_qa`, `reconcile` 후보입니다. | 대응하는 API 또는 owner activation이 필요합니다. | 없음 |
...
| Later ref and artifact values | Bundle, manifest, QA capture, export component, design, Eval, Manual QA, TDD, projection 관련 ref 후보입니다. | ArtifactRef, StateRecordRef, Storage, owner activation이 필요합니다. | 없음 |
...
| later 후보 | 현재 상태 | 승격 조건 | 현재 MVP 영향 |
|---|---|---|---|
| Dashboard, hosted workflows, artifact dashboard, richer cards, richer visualizations | Later 후보입니다. | Read-only derived display rule이 필요합니다. Authority, readiness, QA, verification, acceptance 효과는 없어야 합니다. | 없음 |
| Browser capture automation | Later 후보입니다. | Capture boundary, redaction/PII handling, artifact retention, fallback behavior, QA/acceptance non-substitution이 필요합니다. | 없음 |
| Cross-surface verification | Later 후보입니다. | Core-owned return record, Eval independence semantics, unsupported-surface fallback이 필요합니다. | 없음 |
| Broader connectors, connector marketplace, hosted UI, hosted/remote runtime | Later 후보입니다. | Connector/API/security owner와 local-authority boundary proof가 필요합니다. | 없음 |
| Native hooks, preventive guard expansion, advanced sidecar watcher | Later 후보입니다. | `preventive`, 격리, 임의 도구 제어 주장을 하기 전에 covered mechanism proof가 필요합니다. | 없음 |
| Context Index, local derived metrics, long-term metrics | Later 후보입니다. | Read-only retrieval/diagnostic owner가 필요하며 authority나 close effect가 없어야 합니다. | 없음 |
| Team workflows, permissions, shared capability sets, orchestration, parallel lanes | Later 후보입니다. | Scope, authority, permission, user-owned judgment owner가 필요합니다. | 없음 |
| Advanced exports, release/deployment/canary/rollback/merge/production-monitoring automation | Later 후보입니다. | 별도 owner scope가 필요합니다. 명시적으로 승격하기 전까지 deploy와 production authority는 외부에 남습니다. | 없음 |
| Advanced validators and language or interface checks | Later 후보입니다. | Exact policy, validator ID, severity, waiver, fixture behavior가 필요합니다. | 없음 |
...
| Full later schema definitions | Active documentation에서 폐기했습니다. 상세 본문 없음. | 승격 뒤 promoted owner schema로만 다시 도입합니다. | 없음 |
| Full later template bodies | Active documentation에서 폐기했습니다. 상세 본문 없음. | Template owner가 후보를 켜고 source record를 정의할 때만 다시 도입합니다. | 없음 |
| Full YAML fixtures and scenario scripts | Active documentation에서 폐기했습니다. 상세 본문 없음. | 승격 뒤 conformance owner 아래 exact-shape fixture 자료로만 다시 도입합니다. | 없음 |
