...
| 계약 위치 | 색인 경계만 있음 | 정확한 API, schema, storage, Projection, template, fixture, 운영자 명령 계약은 알맞은 active 담당 문서에 둡니다. | 승격 전까지 없음 |
...
| 후보 | 상태 | 승격 조건 | 현재 MVP 영향 |
|---|---|---|---|
| assurance hardening | later 후보 | 담당 문서가 gate, 대체 동작, 증명 기대치를 정해야 합니다. | 승격 전까지 없음 |
| Evidence Manifest | later 후보 | 증거 담당 문서가 artifact ref, 가림, 닫기 영향, 증명 기대치를 정해야 합니다. | 승격 전까지 없음 |
| Manual QA | later 후보 | Manual QA 담당 문서가 waiver 경로, artifact ref, finding, QA gate 영향을 정해야 합니다. | 승격 전까지 없음 |
...
| 향후 로컬 운영자 명령 묶음: `harness connect`, `harness serve mcp`, `harness doctor`, `harness projection refresh`, `harness reconcile`, `harness recover`, `harness export`, `harness artifacts check`, `harness conformance run` | 명령 이름만 있음 | 운영 담당 문서가 정확한 구문, 보안 태세, API/storage 영향, 보고, 대체 동작, 증명 기대치를 정해야 합니다. | 승격 전까지 없음 |
| Export | later 후보 | Export 담당 문서가 storage/artifact 처리, 가림, 생략, 무결성, 유출 방지 증명을 정해야 합니다. | 승격 전까지 없음 |
| Release Handoff | later 후보 | Handoff 담당 문서가 배포, 병합, 롤백, production 권한을 별도 승격 전까지 외부에 남겨야 합니다. | 승격 전까지 없음 |
| Recovery and reconcile | later 후보 | Operations, Storage, Projection, Reconcile, Security 담당 문서 규칙이 필요합니다. | 승격 전까지 없음 |
| Operator readiness and `doctor` surfaces | later 후보 | 운영 담당 문서가 진단, capability 확인, 보안 태세, 지원되지 않는 접점의 대체 동작을 정해야 합니다. | 승격 전까지 없음 |
...
| 후보 | 상태 | 승격 조건 | 현재 MVP 영향 |
|---|---|---|---|
| Decision Packet full-format presentation (`DEC`), `APR`, Approval Card, `RUN-SUMMARY`, `EVIDENCE-MANIFEST`, `EVAL`, Verification Result Card, `MANUAL-QA`, Manual QA Card, `TASK`, `DIRECT-RESULT`, `JOURNEY-CARD`, `DESIGN`, `DOMAIN-LANGUAGE`, `MODULE-MAP`, `INTERFACE-CONTRACT`, `TDD-TRACE`, `EXPORT` | template 이름만 있음 | Template 담당 문서 지정, 원천 기록, 대체 동작, 대체 불가 규칙, 최신성 동작, 증명 기대치가 필요합니다. | 승격 전까지 없음 |
...
| Broader connectors, connector marketplace, hosted UI, hosted/remote runtime | later 후보 | Connector/API/security 담당 문서와 로컬 권위 경계 증명이 필요합니다. | 승격 전까지 없음 |
