...
이 문서는 이후 후보와 승격 경계를 다루는 단일 활성 색인입니다.
...
| 설계 정책 관문과 닫기 category 이름: `design_gate`, `design_policy` | 이름만 있음 | Core/API/설계 품질 담당 문서가 승격 전에 정확한 필드, 범주 값, 대체 동작, 닫기 대체 불가 규칙, 증명 경로 기대치를 정해야 합니다. | 승격 전까지 없음 |
...
| 명령, 네트워크 효과, 비밀값 접근에 대한 역량 조건부 `prepare_write` / `record_run` 관찰 | 이후 후보 | API, Core, Security, Agent Integration, Conformance 담당 문서가 정확한 요청 필드, 호환성 확인, validator 동작, 공개 오류, 저장소 영향, 증명 기대치를 정해야 합니다. | 승격 전까지 없음 |
