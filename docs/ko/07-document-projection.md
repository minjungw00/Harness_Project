...
| Residual Risk | `state.sqlite.residual_risks`, accepted-risk event/ref, related Decision Packet, evidence/QA/eval ref, artifact ref | `TASK` Residual Risk, standalone projection이 enabled일 때 optional `DEC` accepted-risk context, Journey Card residual-risk line | decision, evidence, QA, Eval, reconcile 또는 close flow에서 Core transition, then projector |
...
| Manual QA | record가 있을 때 `manual_qa_records` plus artifact refs; pending 또는 satisfied QA에는 `qa_gate`가 canonical gate | `MANUAL-QA` projection과 QA card | `record_manual_qa`, then projector |
...
Source: record가 있을 때 `manual_qa_records` plus artifact refs, 그리고 aggregate requirement state에는 `qa_gate`. User-facing card는 `Manual QA: pending/passed/failed/waived`라고 말할 수 있지만, `pending`은 `qa_gate`에서 온 requirement/gate display이지 `manual_qa_record.result=pending`이 아니다.
