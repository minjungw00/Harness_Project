...
Reference copy: 재설계된 reference version은 [문서 Projection 참조](reference/document-projection.md)에 있습니다. 분리된 template body는 [Template 참조](reference/templates/README.md)에 있습니다. 이 legacy chapter는 최종 정리 전까지 마이그레이션 참고 자료로 남습니다.

## 문서 역할

이 legacy chapter는 reference 이동 전 projection rule을 마이그레이션 참고 자료로 보존한다. 사람이 읽는 Markdown projection rule의 active owner는 이제 [문서 Projection 참조](reference/document-projection.md)이며, 이 reference가 projection principle, document authority boundary, managed block rule, human-editable rule, artifact reference rendering, template tier, required MVP template summary, optional design-quality 및 appendix variant template summary, projection freshness rule을 정의한다.

Canonical kernel state, MCP request/response schema, SQLite DDL, design-quality policy requirement, active full template text는 정의하지 않는다. Projection template body는 [Template 참조](reference/templates/README.md)에 있다.

Legacy consolidated [Appendix A](appendix/A-template-library.md)는 최종 정리 전까지 마이그레이션 참고 자료로만 남는다.

## Projection Principles

1. Projection은 source-of-truth가 아닙니다.
2. 기준 operational state는 `state.sqlite` current record와 `state.sqlite.task_events`다.
3. Raw evidence는 artifact store에서 canonical하다.
4. Markdown report는 state record와 artifact reference에서 렌더링된다.
...
9. 수용된 human edits만 reconcile 또는 Core state-changing action을 통해 상태가 된다.
...
Optional Release Handoff report는 external VCS, review, deployment, monitoring system을 위한 readiness를 summarize하기 위해 `EXPORT` projection/report shape를 재사용할 수 있다. 이는 derived report로 남는다. Deployment authority, merge authority, production monitoring, final acceptance, residual-risk acceptance, Task close는 report 밖에 남는다.
...
- State record ref는 record identity와 optional projection path를 사용한다. `record_kind=projection`에서 identity는 `projection_jobs.projection_job_id`이고 path는 locator일 뿐이다. Raw artifact ref로 render하지 않는다.
- `artifact_links.record_kind`는 existing same-Task state owner 또는 projection ref로 resolve되어야 한다. MVP artifact links는 Task-scoped이다. `record_kind=projection`은 같은 `task_id`를 가진 completed `projection_jobs` row로 resolve되며, link의 `record_id`는 `projection_jobs.projection_job_id`이고 path display는 `StateRecordRef.projection_path` 또는 `projection_jobs.output_path`를 사용한다. Project-level owner rows와 project-level projection jobs는 future extension이 project-scoped artifact linking을 추가하지 않는 한 state 또는 projection job freshness/output metadata를 사용한다. `EXPORT`는 `ProjectionKind`일 뿐이다. Export snapshot과 component는 owner record 또는 `record_kind=projection`에 link되는 artifact로 남으며 `record_kind=export`에 link하지 않는다.
...
| Tier | Templates | Rule |
|---|---|---|
| MVP-required | `TASK`, `APR`, `RUN-SUMMARY`, `EVIDENCE-MANIFEST`, `EVAL`, `DIRECT-RESULT` | MVP projector는 이를 렌더링해야 한다. |
| MVP-optional | `MANUAL-QA`, `TDD-TRACE`, `DOMAIN-LANGUAGE`, `MODULE-MAP`, `INTERFACE-CONTRACT` | Policy가 적용되거나, 관련 기록이 있거나, user/operator가 projection을 enable할 때 렌더링한다. |
| Extension / appendix | `DEC`, `DESIGN`, `EXPORT`, `JOURNEY-CARD` | 대응하는 extension 또는 appendix projection이 enabled일 때만 렌더링한다. Full text는 이제 [Template 참조](reference/templates/README.md)에 있다. |
...
Main doc은 각 template의 purpose와 기준 기록만 정의한다. 모든 active projection template body는 [Template 참조](reference/templates/README.md)에 있다.

[Appendix A](appendix/A-template-library.md)는 최종 정리 전까지 마이그레이션 참고 자료로만 남는다.

Persisted `JOURNEY-CARD` Markdown은 선택 사항이다. `harness.status`, `harness.next`, significant resume flow의 current-position Journey Card output은 agency conformance에 필요하다.
...
Standalone `DEC` Markdown은 standalone Decision Packet projection feature가 enabled인 경우가 아니면 선택 사항이다.
...
Boundary: evidence가 필요한 곳에서 close 판단은 report 문장만이 아니라 기준 `evidence_gate`에 의존한다.
...
목적: module, responsibility, public interface, dependency, internal complexity, 테스트 경계, owner decision, watchpoint의 readable projection이다.
...
User-facing card는 `Manual QA: pending/passed/failed/waived`라고 말할 수 있지만, `pending`은 `qa_gate`에서 온 requirement/gate display이지 `manual_qa_record.result=pending`이 아니다.

## Appendix Variant Summaries

### EXPORT

Purpose: 리뷰, 보관, 마이그레이션, release handoff use를 위한 선택적 export/report projection이다. Release Handoff profile은 external system을 위한 close readiness, evidence refs, verification refs, Manual QA refs, residual-risk refs, changed files, projection freshness, redaction notes, suggested PR/deploy checklist item을 요약한다.

Sources: current Task state와 gate, active Change Unit, Journey Spine ref, Decision Packet, Residual Risk, latest Evidence Manifest, latest Eval, latest Manual QA record, changed-file coverage, projection job과 freshness, artifact ref, export manifest metadata, redaction/omission note.

Boundary: `EXPORT` report는 derived projection이자 artifact bundle index이다. Task close, deploy, merge, approve, residual risk acceptance, result acceptance, check waiver, assurance upgrade를 하지 않으며, 사용자의 VCS, review process, deployment system, production monitoring을 대체하지 않는다.
