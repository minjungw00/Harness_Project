...
- `artifact_links.record_kind`는 existing state owner 또는 projection ref로 resolve되어야 한다. `record_kind=projection`은 completed `projection_jobs` row로 resolve되며, link의 `record_id`는 `projection_jobs.projection_job_id`이고 path display는 `StateRecordRef.projection_path` 또는 `projection_jobs.output_path`를 사용한다. `EXPORT`는 `ProjectionKind`일 뿐이다. Export snapshot과 component는 owner record 또는 `record_kind=projection`에 link되는 artifact로 남으며 `record_kind=export`에 link하지 않는다.
