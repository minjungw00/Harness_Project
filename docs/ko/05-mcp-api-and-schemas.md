...
`tdd_trace_required`가 적용되면 `prepare_write`는 actual RED evidence와 valid TDD waiver가 없는 non-test implementation write에 design-policy blocker를 report할 수 있습니다. Intended operation이 failing RED check를 만드는 test-path write라면 scope, baseline, approval, Autonomy Boundary, other required checks가 통과할 때 계속 proceed할 수 있습니다. RED target 또는 plan은 그 test-path write를 support할 수 있지만, non-test implementation write를 위한 RED-evidence precondition이나 Evidence Manifest coverage를 satisfy하면 안 됩니다. Blocker는 validator results, blocked reasons, 필요한 경우 secondary errors/details로 표현하고 primary `ToolError.code`는 API precedence table에 따라 선택합니다.
