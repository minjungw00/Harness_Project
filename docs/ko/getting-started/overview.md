...
| Agent Connection | 로컬 MCP 호스트 connection 단위. 하나의 호스트 설정 대상, `connection_id`, 모드, 명시적으로 연결된 Project를 묶습니다. | [Agent Connection Reference](../reference/agent-integration.md) |
...
| 첫 setup | [설치](installation.md) -> [Quickstart](quickstart.md) |
| 에이전트 호스트 운영자 | [Quickstart](quickstart.md) -> [에이전트 호스트 Setup](../guides/agent-host-setup.md) -> [에이전트 호스트 문제 해결](../guides/agent-host-troubleshooting.md) |
| 여러 저장소 운영자 | [여러 저장소 Agent Setup](../guides/multi-repository-agent-setup.md) |
| 에이전트 작성자 | [에이전트 가이드](../guides/agent-workflow.md) -> [Agent Connection Reference](../reference/agent-integration.md) |
