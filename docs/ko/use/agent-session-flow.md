...
| 사용자 판단 | 계속하기 전에 사용자가 판단해야 하는 것. 각 대기 항목을 유형별로 나눕니다. 민감 동작 permission은 그 경로가 pending일 때만 여기에 포함합니다. | 판단 요청, Decision Packet, minimum v0.2 민감 동작 permission을 위한 Approval 형태 Decision Packet, later Approval profile이 active일 때만 Approval ref, 작업 수락 Decision Packet, 잔여 위험. |
...
|---|---|---|
| `ask_user` | Minimum v0.2 | 사용자 소유 답변이 필요합니다. 간결한 질문, 추천, 영향, 참조를 보여줍니다. |
| `prepare_write` | Minimum v0.2 | 정확히 의도한 쓰기에 대한 쓰기 권한을 확인합니다. |
| `implement` | Minimum v0.2 | 범위 안 구현 경로를 계속합니다. 제품 파일 쓰기에는 현재 호환되는 쓰기 허가 기록만 사용합니다. |
...
| `APPROVAL_REQUIRED`, `APPROVAL_DENIED`, 또는 `APPROVAL_EXPIRED` | 민감 동작 permission이 필요하거나 사용할 수 없습니다. | Minimum v0.2에서는 Approval 형태 Decision Packet을 요청하거나 해소합니다. Later Approval profile이 active일 때만 Approval을 갱신하거나 복구한 뒤 쓰기 확인을 다시 시도합니다. |
...
| 민감 동작 permission / Approval | 이 민감한 단계를 진행해도 되는가? | 기록된 범위와 만료 조건 안에서 이름 붙인 민감 동작을 허용합니다. Minimum v0.2는 Approval 형태 Decision Packet으로 표현할 수 있고, later Approval profile은 Approval record를 사용할 수 있습니다. | 사용자 소유 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, 범위/자율성 판단, 면제 판단, 작업 수락, 잔여 위험 수용, 정확성(correctness), 쓰기 허가 기록을 대신하지 않습니다. |
...
- 근거 공백 또는 뒷받침 자료: active evidence owner path를 갱신합니다. Minimum v0.2에서는 evidence summary, Run refs, ArtifactRefs, 보이는 gap을 cite하고, full evidence profile이 active일 때만 Evidence Manifest 범위를 갱신합니다.
