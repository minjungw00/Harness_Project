...
- 판단: 진행을 막는 사용자 소유 질문, 결정 패킷, sensitive-action Approval
- 근거: supporting refs, missing support, stale support, 이미 실행한 check
- 닫기 준비 상태: 다음 결정이나 close에 영향을 주는 verification, 수동 QA, 잔여 위험, acceptance, close-blocker 상태
- 다음 안전한 행동
...
Status, next, result, acceptance, close display의 모든 authority claim은 source ref 또는 명시적 absence로 추적 가능해야 합니다. "Write allowed"에는 쓰기 허가 기록 ref를, sensitive-action permission에는 Approval ref를, evidence sufficient에는 Evidence Manifest ref를, 분리 검증에는 Eval ref를, 수동 QA에는 수동 QA record 또는 valid waiver ref를, 작업 수락에는 Acceptance 결정 패킷 ref를, 잔여 위험 표시에는 잔여 위험 refs 또는 `ResidualRiskSummary.status=none`을, log/diff/screenshot/trace/bundle에는 아티팩트 참조를 사용합니다. Ref가 없으면 아직 뒷받침되지 않은 claim이라고 말합니다.
...
- 사용자 소유: 제품 방향, 중요한 기술 방향, sensitive-action Approval, 수동 QA 판단 또는 waiver, 잔여 위험 수용, 작업 수락처럼 사용자가 결정해야 하는 일.
...
요청 정리는 사용자가 하네스 용어로 말하지 않아도 평범한 요청을 실제로 진행 가능한 작업 모양으로 바꾸는 단계입니다. 예를 들어 사용자가 "이메일 로그인을 추가하고 reset은 범위 밖으로 둬"라고 말하면, 에이전트가 mode, 범위, 가능한 판단, 근거 필요성, 쓰기 확인, 닫기 준비 상태 처리로 번역해야 합니다.
...
세션 시작에서 보는 것과 같은 작업 모양 신호를 살핍니다. 제품 파일 쓰기, 범위가 흐트러질 위험, 모호한 요구사항, 여러 파일이나 구조 변경, 민감하거나 정책·품질 판단이 필요한 영역, 사용자가 판단해야 하는 결정, 근거, 검증, 수동 QA, 작업 수락, 잔여 위험이 필요한 작업이 여기에 해당합니다. 이런 신호가 있으면 평범한 요청을 예상 모드, 범위, 범위 밖, 다음 안전한 행동으로 바꿔 제안합니다.
...
구체화 결과(Discovery results)는 Shared Design, 결정 패킷 candidate, Change Unit shaping으로 라우팅합니다. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate를 scope authority, sensitive-action Approval, 작업 수락, 잔여 위험을 받아들이는 판단, evidence, 닫기 준비 상태, 쓰기 허가 기록으로 취급하지 않습니다.
...
| Approval | 이 민감한 단계를 진행해도 되는가? | 기록된 scope와 expiry 안에서 이름 붙인 sensitive action을 허용합니다. | 사용자 소유 판단, 정확성(correctness), 잔여 위험 수용, 쓰기 허가 기록을 대신하지 않습니다. |
...
| Acceptance | 작업 수락이 required일 때 결과를 받아들일 수 있는가? | 닫기에 영향을 주는 잔여 위험이 보였거나 없다고 확인된 뒤 사용자의 결과 판단을 기록합니다. | Evidence, verification, 수동 QA, Approval, 쓰기 허가 기록, 잔여 위험을 받아들이는 판단을 대체하지 않습니다. |
...
프롬프트나 상태에서 "approved" 또는 "승인"이라는 말을 쓸 때는 실제 권한이나 기록되는 판단을 이름 붙입니다. Sensitive-action Approval, 범위 확인(scope confirmation), 결정 패킷 resolution, 잔여 위험 수용, 작업 수락(Acceptance), 쓰기 허가 기록 status를 구분해서 말하고, "승인"을 모든 허가와 판단을 뭉뚱그리는 포괄 라벨로 쓰면 안 됩니다.
...
"approved", "승인", "go ahead", "진행해" 같은 말은 underlying choice가 제품 장단점, architecture 방향, QA waiver, verification gap에서 생기는 risk, 작업 수락, 잔여 위험을 받아들이는 판단이라면 충분하지 않습니다. Prompt는 decision route, 사용자가 결정하는 것, 결정하지 않는 것, evidence 또는 risk refs, 에이전트가 사용자 없이 결정해도 되는 일, close나 write 영향을 이름 붙여야 합니다.
...
Scope expansion, Approval 없는 새 sensitive action, Autonomy Boundary breach, 잔여 위험을 받아들이는 판단, 작업 수락, QA 또는 verification waiver, public API 또는 module contract 변경, public meaning에 영향을 주는 domain-language change, release/support promise, 사용자나 다른 시스템이 의존할 수 있는 다른 public commitment 전에는 멈추고 가장 작은 unblocker를 보여줘야 합니다.
...
`work` Task의 닫기 요약은 닫을 수 있는 근거를 보여줘야 합니다. 적용된 변경 범위, 근거 범위, 검증, 수동 QA, 잔여 위험, 작업 수락, close reason을 해당되는 만큼 표시합니다. Gate가 waived, `not_required`, failed, pending, blocked 중 하나라면 일반적인 성공 문장에 묻지 말고 그대로 말해야 합니다.

작업 모양에 맞는 닫기 표시를 사용합니다. `DIRECT-RESULT`는 direct 작업의 간결한 결과 표시이고, `TASK` Close Summary는 진행 중이거나 최근 닫힌 `work` Task의 이어가기 표시이며, Journey Card close context는 compact status/resume 표시입니다. 이 표시들은 state, gate, 작업 수락, QA, verification, 잔여 위험 수용, close, write authority를 만들지 않습니다.
...
일반적인 "다 괜찮나요?" 같은 prompt로 작업 수락을 요청하지 않습니다. 작업 수락 prompt는 close basis, 즉 evidence, verification, 수동 QA, 잔여 위험 표시 또는 `none`, acceptance가 대체하지 않는 것, 요청하는 close가 normal close인지 잔여 위험을 받아들이고 닫는 흐름인지를 보여줘야 합니다.
...
```text
direct로 완료했습니다.
범위: 설정 label 하나였고 계정 동작은 범위 밖으로 유지했습니다. 쓰기 허가 기록 WA-031이 consumed됐고 `work` 전환은 없습니다.
판단: 사용자 소유 판단은 필요하지 않았습니다.
근거: `src/settings/Profile.tsx`를 변경했습니다. 확인: copy test RUN-031과 diff ART-DIFF-031. Evidence Manifest EM-031이 claim을 뒷받침합니다.
닫기 준비 상태: close-relevant blocker는 남아 있지 않습니다. 잔여 위험은 이번 close에는 없음(`ResidualRiskSummary.status=none`).
```
