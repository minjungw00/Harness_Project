...
- Close Readiness: verification, 수동 QA, acceptance, 잔여 위험 처리, close 전에 무엇이 남았는가?
...
| Close | Close-readiness summary, blockers, evidence/verification/QA/acceptance status, 잔여 위험 요약 또는 accepted refs, projection freshness. | Blocker 뒤에 있는 exact close, acceptance, 잔여 위험, QA, verification, artifact detail. |
...
- Close Readiness: 다음 결정이나 close에 영향을 주는 verification, 수동 QA, 잔여 위험, acceptance, close-blocker 상태
- 다음 안전한 행동
...
읽기용 투영 문서(projection), `source_state_version`, 읽기용 상태가 stale이거나 unknown이면 그 사실을 말하고, 거기에 의존하기 전에 refresh 또는 reconcile합니다. 기준 상태를 직접 읽을 수 있으면 그 상태에서 계속할 수 있지만, 읽기용 투영 문서는 권한의 출처가 아니라고 알려야 합니다.
...
| `request_acceptance` | Evidence, verification, QA, 잔여 위험 표시를 보여준 뒤 final acceptance를 요청합니다. |
...
Status, next, result, acceptance, close display의 모든 authority claim은 source ref 또는 명시적 absence로 추적 가능해야 합니다. "Write allowed"에는 쓰기 허가 기록 ref를, sensitive-action permission에는 Approval ref를, evidence sufficient에는 Evidence Manifest ref를, 분리 검증에는 Eval ref를, 수동 QA에는 수동 QA record 또는 valid waiver ref를, final acceptance에는 Acceptance 결정 패킷 ref를, 잔여 위험 표시에는 잔여 위험 refs 또는 `ResidualRiskSummary.status=none`을, log/diff/screenshot/trace/bundle에는 아티팩트 참조를 사용합니다. Ref가 없으면 아직 뒷받침되지 않은 claim이라고 말합니다.
...
Discovery 결과는 Shared Design, 결정 패킷 candidate, Change Unit shaping으로 라우팅합니다. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate를 scope authority, sensitive-action Approval, final acceptance, 잔여 위험을 받아들이는 판단, evidence, close readiness, 쓰기 허가 기록으로 취급하지 않습니다.
...
각 사용자 소유 질문은 사용자가 정확히 결정하는 것을 이름 붙이고, 가능한 선택지, 에이전트의 추천안, 불확실성, 의미 있을 때 영향을 받는 gate 또는 수용 기준, 사용 가능하거나 관련 있을 때 source ref, evidence ref, risk ref, design ref, 결정을 미뤄도 계속할 수 있는 일 또는 결정 전에는 진행하면 안 되는 이유를 함께 보여줘야 합니다. 에이전트가 둔 가정은 사용자에게 속한 제품, 기술, security, QA, 운영, scope, approval, acceptance, 잔여 위험을 받아들이는 판단과 따로 기록합니다.
...
| High-risk Work | Auth, security, privacy, secrets, infra 또는 비슷하게 민감한 work. | 계속 `work`에 두고 approval, 결정 패킷, evidence, verification, QA, 잔여 위험을 owner path로 라우팅합니다. |
...
Tiny direct에서는 사용자에게 보이는 예산을 더 줄일 수 있습니다. Trivial scope, changed path 또는 file 변경 없음 결과, self-check 정도면 됩니다. 이 작은 표시가 권한 우회는 아닙니다. Tiny direct도 active scope, 제품 파일 쓰기에 적용되는 compatible `prepare_write`, 사용자 소유 판단, sensitive-action Approval, security와 privacy boundary, 잔여 위험 표시, close rule을 지켜야 합니다.
...
| 결정 패킷 | 어떤 사용자 소유 판단을 기록하는가? | 이름 붙은 제품 판단, 중요한 기술 판단, waiver, acceptance, 잔여 위험, reconcile 선택을 resolved, deferred, rejected, blocked 상태로 기록합니다. | Approval record에 연결된 Approval 형태 packet이 아닌 한 sensitive-action Approval을 부여하지 않습니다. |
| Acceptance | 최종 수락이 required일 때 결과를 받아들일 수 있는가? | 닫기에 영향을 주는 잔여 위험이 보였거나 없다고 확인된 뒤 사용자의 최종 결과 판단을 기록합니다. | Evidence, verification, 수동 QA, Approval, 쓰기 허가 기록, 잔여 위험을 받아들이는 판단을 대체하지 않습니다. |
| 잔여 위험을 받아들이는 판단 | 이번 close에서 알려진 잔여 위험을 수락할 수 있는가? | 보이는 닫기 관련 위험의 수용을 기록하며 다른 gate가 허용할 때 잔여 위험을 받아들이고 닫는 흐름을 뒷받침합니다. | 분리 검증, correctness proof, QA pass를 만들지 않고, close를 위험 없는 일반 close로 만들지 않습니다. |
...
프롬프트나 상태에서 "approved" 또는 "승인"이라는 말을 쓸 때는 실제 권한이나 기록되는 판단을 이름 붙입니다. Sensitive-action Approval, 범위 확인(scope confirmation), 결정 패킷 resolution, 잔여 위험을 받아들이는 판단, 최종 수락(final acceptance), 쓰기 허가 기록 status를 구분해서 말하고, "승인"을 모든 허가와 판단을 뭉뚱그리는 포괄 라벨로 쓰면 안 됩니다.
...
"approved", "승인", "go ahead", "진행해" 같은 말은 underlying choice가 제품 장단점, architecture 방향, QA waiver, verification gap에서 생기는 risk, final acceptance, 잔여 위험을 받아들이는 판단이라면 충분하지 않습니다. Prompt는 decision route, 사용자가 결정하는 것, 결정하지 않는 것, evidence 또는 risk refs, 에이전트가 사용자 없이 결정해도 되는 일, close나 write 영향을 이름 붙여야 합니다.
...
- 잔여 위험: 닫기 전 잔여 위험 수락 판단은 남은 한계, 이미 있는 근거, 그래도 닫을 수 있다고 볼 수 있는 이유, 잔여 위험 후속 작업을 보여줍니다. 잔여 위험을 받아들이고 닫는 흐름은 detached-verified close가 아닙니다.
...
Role Lens와 status/next recommendation은 기존 Core/MCP path가 실제 동작을 기록하기 전까지 guidance입니다. 결정 패킷 후보, evidence gap, Eval 필요, 수동 QA 필요, 잔여 위험 후보, Approval 필요, Change Unit update recommendation, close blocker를 찾을 수는 있지만, 그 자체로 state를 mutate하거나 해당 route를 충족하지 않습니다.
...
Review Stages는 관리되는 표시/절차일 뿐입니다. 기준 기록이 아니며, 새 `ProjectionKind` value, Approval, evidence, verification, QA, acceptance, 잔여 위험을 받아들이는 판단, close, 쓰기 허가 기록도 아닙니다. 같은 세션에서 하는 검토(review)는 조건을 충족하는 independent Eval 또는 verification record가 없는 한 자체 확인(self-check) 또는 stewardship signal입니다. Finding이 영향받는 write나 Task 닫기에 영향을 주려면 기존 경로로 연결해야 합니다.
...
- 알려진 남은 불확실성 또는 생략된 check: acceptance 또는 잔여 위험을 받아들이고 닫는 흐름 전에 잔여 위험 후보 또는 ref를 보여줍니다.
...
잔여 위험은 알려진 한계, 불확실성, 확인하지 못한 조건, 장단점입니다. 잔여 위험을 수락하고 닫거나 최종 수락을 하기 전에는 반드시 보여야 합니다. 잔여 위험을 받아들이는 판단은 보장 수준을 높이지 않고, verification이나 QA를 대체하지 않습니다.

잔여 위험 표시는 `status=none`과 `not_visible`을 구분해야 합니다. `status=none`은 현재 Task와 requested action에 대해 Core가 알고 있는 닫기 관련 잔여 위험이 없다는 뜻입니다. `not_visible`은 알려진 닫기 관련 위험이 있지만 acceptance 또는 close에 필요한 맥락으로 아직 보이지 않았다는 뜻이므로, 다음 행동은 그 위험과 refs를 보여주는 것입니다. `not_visible`을 "위험 없음"으로 요약하면 안 됩니다.

최종 수락은 Task 경로가 요구할 때 사용자가 결과를 받아들이는 판단입니다. sensitive-action Approval, 검증, QA, 잔여 위험을 받아들이는 판단, 정확성 증명과 다릅니다.

Verification waiver와 QA waiver는 assurance를 높이지 않습니다. Verification waiver는 분리 검증을 충족하지 않은 상태로 둡니다. close가 otherwise 허용되더라도, 면제된 verification gap에 대한 잔여 위험을 받아들이는 판단 경로로만 닫을 수 있습니다. 이를 verified close로 요약하면 안 됩니다. QA waiver는 이름 붙인 QA requirement만 닫고 evidence, verification, acceptance, 잔여 위험 처리는 각각의 경로에 그대로 남깁니다. Waiver prompt와 summary는 이름 붙은 requirement, 수락된 잔여 위험, 잔여 위험 owner refs, 필요할 때 잔여 위험 후속 작업, 영향받는 owner path 또는 close impact를 보여줘야 합니다. 정확한 waiver metadata와 gate effect는 [설계 품질 정책](../reference/design-quality-policies.md#waiver-규칙)과 [커널 참조](../reference/kernel.md#waiver-semantics)가 담당합니다.
...
- 잔여 위험을 받아들이고 닫는 흐름: 남은 한계, 이미 있는 근거, 빠졌거나 면제된 verification 또는 QA, 수락된 잔여 위험, 잔여 위험 후속 작업을 보여줍니다. 결과를 detached verified처럼 표시하면 안 됩니다.
...
닫기 표시는 같은 네 가지 그룹을 먼저 사용해야 합니다. Scope는 changed scope, Judgment는 수락 또는 잔여 위험 판단, Evidence는 support refs, Close Readiness는 verification, 수동 QA, 잔여 위험, close blockers, close reason을 보여줍니다. Raw gate name은 blocker를 설명할 때 뒤에 붙일 수 있지만, 정확한 gate 값과 close 의미는 kernel이 담당합니다.
...
일반적인 "다 괜찮나요?" 같은 prompt로 최종 수락을 요청하지 않습니다. 최종 수락 prompt는 close basis, 즉 evidence, verification, 수동 QA, 잔여 위험 표시 또는 `none`, acceptance가 대체하지 않는 것, 요청하는 close가 normal close인지 잔여 위험을 받아들이고 닫는 흐름인지를 보여줘야 합니다.
...
Judgment: mobile Safari 잔여 위험은 DEC-022에서 받아들였고 최종 수락은 DEC-023에 기록되었습니다.
Evidence: Evidence Manifest EM-009가 RUN-018과 ART-TEST-018을 근거로 AC-01과 AC-02를 뒷받침합니다.
Close Readiness: 검증은 RUN-018에 기록된 자체 확인(self-check)이며, 이 경로에서는 detached Eval이 필요하지 않았습니다. 수동 QA는 MQA-006에서 최종 문구와 레이아웃에 대해 통과했습니다. 잔여 위험 RISK-004에는 후속 TASK-144가 있습니다. close reason: 잔여 위험을 받아들이고 완료.
```
