...
매 턴마다 계속 보여주는 맥락은 간결한 Harness envelope여야 합니다. 여기에는 활성 Task id와 모드, 다음 안전한 행동, 활성 Change Unit 요약, 막고 있는 결정, 쓰기 권한 상태, 보장 수준(guarantee level), 게이트 요약, projection 최신성이 들어갑니다. 근거(Evidence), Run, Eval, Manual QA, artifact, log, screenshot, diff, large trace는 기본적으로 ref와 짧은 결과만 보여주고, 다음 행동이 내용을 실제로 살펴봐야 할 때만 가져옵니다.
...
- 간결한 게이트, Manual QA, 남은 위험, projection 최신성 상태
