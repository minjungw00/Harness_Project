...
- 닫기 준비 상태: 다음 결정이나 close에 영향을 주는 검증, 수동 QA, 잔여 위험, 작업 수락, close-blocker 상태
- 다음 안전한 행동
...
중요한 작업을 이어가기 전에는 하네스 상태를 읽고 현재 위치를 보여줍니다. 이어가기는 오래된 chat, stale status text, remembered prior recommendation이 아니라 current Core state와 owner records를 기준으로 해야 합니다. 오래된 chat memory는 살펴볼 ref를 찾는 데 도움을 줄 수 있지만 write를 허가하거나, Task를 close하거나, 작업 수락을 기록하거나, check를 면제하거나, 잔여 위험을 받아들이거나, current state를 대체할 수 없습니다.
...
`harness.status`, `harness.next`, compact status card, recommendation line은 read-only display입니다. 결정 패킷, `prepare_write`, 근거 수집, 검증, 수동 QA, reconcile, close attempt를 추천할 수는 있지만, 추천 자체가 state를 mutate하거나, write를 허가하거나, gate를 충족하거나, 작업 수락을 기록하거나, 잔여 위험을 받아들이거나, Task를 close하지 않습니다.
...
| `launch_verify` | Current evidence refs에서 independent 검증 path를 시작하거나 준비합니다. |
...
Status, next, result, 작업 수락, close display의 모든 authority claim은 source ref 또는 명시적 absence로 추적 가능해야 합니다. "Write allowed"에는 쓰기 허가 기록 ref를, 민감 동작 permission에는 Approval ref를, 근거 sufficient에는 Evidence Manifest ref를, 분리 검증에는 Eval ref를, 수동 QA에는 수동 QA record 또는 valid waiver ref를, 작업 수락에는 Acceptance 결정 패킷 ref를, 잔여 위험 표시에는 잔여 위험 refs 또는 `ResidualRiskSummary.status=none`을, 잔여 위험 수용에는 accepted Residual Risk refs를, log/diff/screenshot/trace/bundle에는 아티팩트 참조를 사용합니다. Ref가 없으면 아직 뒷받침되지 않은 claim이라고 말합니다.
...
요구사항 구체화 밖에서는 다음 안전한 행동을 바꾸는 질문만 합니다. 구체화 중에는 목표, 사용자 가치, 범위, 비목표, 수용 기준, 제품/UX 동작, 기술 아키텍처, security/privacy 자세, QA 또는 검증 기대 수준, 첫 구현 후보, 작업 분할, 사용자 소유 판단, 숨은 가정을 구체화하는 targeted question을 묻습니다. 긴 질문지를 한꺼번에 던지지 말고 decision area별로 묶어 묻고, 불확실성을 명시합니다. Useful-but-not-blocking 질문은 사용자를 계속 방해하지 말고 따로 남겨 둡니다. 긴 양식보다 추천이 딸린 가장 막힘이 큰 decision area 하나가 낫습니다.
...
| 추적되는 작업 | `work` | Feature work, UX workflow, auth-facing behavior, schema, public API/interface, 구조 변경, 위험한 수정, multi-file/multi-step delivery, 의미 있는 근거와 독립 검증이 필요한 work. | 계속 추적되는 작업으로 두고, auth, security, privacy, secrets, infrastructure 또는 비슷한 민감 영역이 나타나면 approval, 결정 패킷, 근거, 검증, 수동 QA, 잔여 위험을 owner path로 라우팅합니다. |
...
Sufficiency는 양이 아니라 coverage로 표시합니다. 중요한 질문은 어떤 수용 기준, completion condition, close-relevant claim에 current supporting refs가 있는지입니다. 긴 artifact list는 missing criterion을 supported로 만들지 않으며, chat text나 Markdown report prose만으로 근거가 sufficient하다고 증명하면 안 됩니다.
...
| 수동 QA가 통과했다 | 사용자의 작업 수락이 기록됐거나 남은 위험을 수용함. |
...
일반적인 "다 괜찮나요?" 같은 prompt로 작업 수락을 요청하지 않습니다. 작업 수락 prompt는 close basis, 즉 evidence, 검증, 수동 QA, 잔여 위험 표시 또는 `none`, 작업 수락이 대체하지 않는 것, 요청하는 close가 normal close인지 잔여 위험을 받아들이고 닫는 흐름인지를 보여줘야 합니다.

닫기가 막혔으면 정확한 이유와 가장 작은 해소 방법을 말합니다.

둘 이상의 범주가 close를 막으면 primary blocker를 먼저 말하되 나머지도 보이게 둡니다. 근거, 검증, 수동 QA, 작업 수락, 잔여 위험 표시, 잔여 위험 수용, projection freshness를 하나의 "not done" 또는 "done" 줄로 압축하면 안 됩니다.
