...
- Close Readiness: verification, Manual QA, acceptance, residual-risk handling, close 전에 무엇이 남았는가?
...
- Judgment: 진행을 막는 사용자 소유 질문, Decision Packet, sensitive-action Approval
- Evidence: supporting refs, missing support, stale support, 이미 실행한 check
- Close Readiness: 다음 결정이나 close에 영향을 주는 verification, Manual QA, residual-risk, acceptance, close-blocker 상태
- 다음 안전한 행동
...
중요한 작업을 이어가기 전에는 Harness 상태를 읽고 현재 위치를 보여줍니다. 이어가기는 오래된 chat, stale status text, remembered prior recommendation이 아니라 current Core state와 owner records를 기준으로 해야 합니다. 오래된 chat memory는 살펴볼 ref를 찾는 데 도움을 줄 수 있지만 write를 허가하거나, Task를 close하거나, 결과를 수락하거나, check를 면제하거나, 잔여 위험을 수락하거나, current state를 대체할 수 없습니다.
...
|---|---|---|
| Scope | 무엇이 바뀔 수 있는지, 무엇이 범위 밖인지, 의도한 write가 맞는지. | Task, Change Unit, Autonomy Boundary, Write Authorization. |
| Judgment | 계속하기 전에 사용자가 결정하거나 승인해야 하는 것. | Decision Packet, Approval, Acceptance Decision Packet, Residual Risk. |
| Evidence | 주장을 무엇이 뒷받침하는지, 무엇이 빠졌는지, 뒷받침이 오래됐는지. | Evidence Manifest, Run, artifact refs, Eval input refs. |
| Close Readiness | close를 시도하거나 수락하기 전에 남은 것. | Eval, Manual QA, Acceptance, Residual Risk, close blockers. |
...
`harness.status`, `harness.next`, compact status card, recommendation line은 read-only display입니다. Decision Packet, `prepare_write`, evidence collection, verification, QA, reconcile, close attempt를 추천할 수는 있지만, 추천 자체가 state를 mutate하거나, write를 허가하거나, gate를 충족하거나, 결과를 수락하거나, 잔여 위험을 수락하거나, Task를 close하지 않습니다.
...
| `implement` | 범위 안 구현 경로를 계속합니다. 제품 파일 쓰기에는 current compatible Write Authorization만 사용합니다. |
| `launch_verify` | Current evidence refs에서 independent verification path를 시작하거나 준비합니다. |
| `record_eval` | Evaluator 결과를 기록합니다. Eval이 조건을 충족하기 전에는 detached verification을 주장하지 않습니다. |
| `record_manual_qa` | Human QA outcome 또는 valid waiver를 기록합니다. Browser artifact만으로 Manual QA처럼 다루지 않습니다. |
...
| `idle` | 이 focus에 필요한 즉시 Harness action이 없습니다. |

정확한 enum과 API 계약은 [`harness.next`](../reference/mcp-api-and-schemas.md#harnessnext)가 담당합니다. 이 표는 표시 안내일 뿐 새 route나 gate가 아닙니다.

Status, next, result, acceptance, close display의 모든 authority claim은 source ref 또는 명시적 absence로 추적 가능해야 합니다. "Write allowed"에는 Write Authorization ref를, sensitive-action permission에는 Approval ref를, evidence sufficient에는 Evidence Manifest ref를, detached verification에는 Eval ref를, Manual QA에는 Manual QA record 또는 valid waiver ref를, final acceptance에는 Acceptance Decision Packet ref를, residual-risk visibility에는 Residual Risk refs 또는 `ResidualRiskSummary.status=none`을, log/diff/screenshot/trace/bundle에는 artifact refs를 사용합니다. Ref가 없으면 아직 뒷받침되지 않은 claim이라고 말합니다.
...
- 사용자 소유: 제품 방향, 중요한 기술 방향, sensitive-action Approval, Manual QA 판단 또는 waiver, 잔여 위험 수락 판단, 최종 수락처럼 사용자가 결정해야 하는 일.
...
| `MCP_UNAVAILABLE` 또는 `CAPABILITY_INSUFFICIENT`(`details.mcp_unavailable_kind=surface_mcp_unavailable`) 또는 진단상 `SURFACE_MCP_UNAVAILABLE` | 이 접점은 필요한 Harness 도구를 사용할 수 없습니다. | 접점을 복구하거나 사용할 수 있는 접점으로 전환합니다. 해당 covered operation에 대해 profile이 pre-tool blocking을 입증한 경우가 아니면 제품 파일 쓰기는 지시로 보류합니다. |
...
Discovery 결과는 Shared Design, Decision Packet candidate, Change Unit shaping으로 라우팅합니다. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate를 scope authority, sensitive-action Approval, final acceptance, residual-risk acceptance, evidence, close readiness, Write Authorization으로 취급하지 않습니다.
...
한 번에 하나의 막힘 질문을 묻는다는 말이 구체화도 한 번이면 끝난다는 뜻은 아닙니다. 요청이 넓거나 설계 판단이 크면 목표, 범위, 비목표, 수용 기준, 영향받는 제품 영역, 사용자 화면이나 흐름, 모듈, interface, 민감 카테고리(sensitive categories), 사용자가 소유하는 제품 또는 중요한 기술 장단점 판단, 검증 또는 Manual QA 기대 수준, 알려진 제품·구현·검증·QA·후속 위험이 첫 번째 안전한 Change Unit을 제안할 수 있을 만큼 잡힐 때까지 짧은 확인을 여러 차례 이어갈 수 있습니다. Discovery는 필요한 경우 여러 targeted question을 물을 수 있지만, 안전한 첫 Change Unit을 제안할 수 있으면 멈춥니다.

질문하기 전에 각 열린 질문을 분류합니다. Blocking 질문은 다음 안전한 행동 전에 사용자 판단이 필요한 질문입니다. Useful-but-not-blocking 질문은 Discovery Brief, Assumption Register, follow-up work, 이후 Decision Packet candidate에 남겨둘 수 있습니다. Codebase-answerable 질문은 사용자에게 묻지 말고 current repo, docs, Harness state, source refs를 살펴 답해야 합니다.
...
| Approval | 이 민감한 단계를 진행해도 되는가? | 기록된 scope와 expiry 안에서 이름 붙인 sensitive action을 허용합니다. | 사용자 소유 판단, 정확성(correctness), 잔여 위험 수락 판단, Write Authorization을 대신하지 않습니다. |
...
| Residual-risk acceptance | 이번 close에서 알려진 잔여 위험을 수락할 수 있는가? | 보이는 close-relevant risk의 수용을 기록하며 다른 gate가 허용할 때 residual-risk accepted close를 뒷받침합니다. | Detached verification, correctness proof, QA pass를 만들지 않고, close를 위험 없는 일반 close로 만들지 않습니다. |
...
프롬프트나 상태에서 "approved" 또는 "승인"이라는 말을 쓸 때는 실제 권한이나 기록되는 판단을 이름 붙입니다. Sensitive-action Approval, 범위 확인(scope confirmation), Decision Packet resolution, 잔여 위험 수락 판단(residual-risk acceptance), 최종 수락(final acceptance), Write Authorization status를 구분해서 말하고, "승인"을 모든 허가와 판단을 뭉뚱그리는 포괄 라벨로 쓰면 안 됩니다.

예시:

- Dependency install sensitive-action Approval: install을 실행하거나 dependency 파일을 갱신해도 된다는 Approval은 그 dependency가 올바른 architecture 선택이라는 결정이 아닙니다. 그 선택이 호환성, rollback, 비용, 유지보수에 영향을 주면 Decision Packet을 사용합니다.
...
사용자가 소유하는 제품, 기술, security/privacy, QA/acceptance, residual-risk, scope/autonomy 판단이 진행을 막고 있으면 Decision Packet을 보여주거나 요청합니다. 이를 넓은 승인 질문이나 막연한 "계속할까요?"로 바꾸면 안 됩니다.
...
판단 유형: QA / acceptance
이번 close에서 남은 mobile Safari wrapping risk를 받아들이겠다고 기록할까요, 아니면 Manual QA를 실행할 때까지 close를 막아 둘까요? 추천은 release timing 때문에 waiver가 필요한 경우가 아니라면 막아 두는 것입니다. 영향받는 그룹: Close Readiness; owner path/gate ref: Manual QA / qa_gate; 영향받는 기준: AC-03 onboarding copy layout.
```
...
- Product / UX: 로그인 실패 문구는 일반적인 문구, 더 구체적인 문구, hybrid 문구를 비교합니다. 계정 열거(account enumeration) 위험, 명확성, 복구 도움 정도, 지원 부담, 제품 톤을 기준으로 추천합니다. 결정을 미루면 validation wiring은 계속할 수 있지만 release-ready copy와 Manual QA는 열어 둬야 합니다.
- QA / acceptance: 제품 감각과 Manual QA 필요성은 사람이 시각적으로 확인해야 하는 완성도 높은 상호작용과 test 및 browser smoke로 확인 가능한 더 보수적인 동작을 비교합니다. 감각상의 장단점, QA 비용, 사용자 영향, Manual QA를 미뤘을 때 계속할 수 있는 일 또는 결정 전에는 진행하면 안 되는 이유를 설명합니다.
...
- QA / acceptance: QA 또는 verification waiver는 해당 Task에서 요구하는 기존 기록 경로를 사용하고 owner refs를 cite합니다. QA waiver 효과는 Manual QA / QA policy path가 담당하며, product/user risk 또는 policy-required judgment가 있으면 QA waiver Decision Packet을 사용합니다. Verification waiver 효과는 kernel verification-waiver path가 담당하며, 사용자 소유 판단이 필요하면 관련 Decision Packet을 사용합니다. 생략하는 확인이나 대상, 수락된 잔여 위험, 잔여 위험 후속 작업, 관련 refs, 닫기 영향을 이름 붙입니다. 예를 들어 copy-only 변경에서 mobile Safari Manual QA를 면제한다면 viewport wrapping에 대한 잔여 위험을 수락하고 release 전 browser pass를 후속 작업으로 남깁니다.
...
- 알려진 남은 불확실성 또는 생략된 check: acceptance 또는 residual-risk accepted close 전에 Residual Risk 후보 또는 ref를 보여줍니다.
...
외부 영향(side effect)이 있을 때는 실행 전의 주장과 실행 뒤의 기록을 분리합니다. 실행 전에는 의도한 영향, 민감 범주(sensitive category), Approval 또는 Decision Packet 필요 여부, 보장 수준을 말합니다. 실행 뒤에는 실제로 일어난 일, 기록된 Run/artifact/evidence ref, redaction/omission/block/stale/violation 여부를 말합니다. 보장 수준은 표시와 risk context입니다. Approval을 부여하거나, 결과를 검증하거나, QA를 기록하거나, 잔여 위험을 수락하거나, 결과를 수락하거나, Task를 닫지 않습니다. 정확한 보장 수준 의미는 [런타임 아키텍처 참조](../reference/runtime-architecture.md#보장-수준)가 담당합니다.
...
남은 위험은 알려진 한계, 불확실성, 확인하지 못한 조건, 장단점입니다. 잔여 위험을 수락하고 닫거나 최종 수락을 하기 전에는 반드시 보여야 합니다. Residual-risk acceptance는 보장 수준을 높이지 않고, verification이나 QA를 대체하지 않습니다.
...
닫기 표시는 같은 네 가지 그룹을 먼저 사용해야 합니다. Scope는 changed scope, Judgment는 수락 또는 residual-risk 판단, Evidence는 support refs, Close Readiness는 verification, Manual QA, residual risk, close blockers, close reason을 보여줍니다. Raw gate name은 blocker를 설명할 때 뒤에 붙일 수 있지만, 정확한 gate 값과 close 의미는 kernel이 담당합니다.

성공적으로 닫기 전에 보여주거나 확인할 것:

- 범위와 결과가 맞는지
- 근거가 충분한지 또는 근거 요구가 없는지
- stale evaluator bundle 또는 baseline drift가 있으면 이를 포함한 검증 상태, 또는 필요한 경우 관련 Decision Packet과 수락된 Residual Risk refs가 있는 kernel-owned verification-waiver path
- Manual QA 상태 또는 타당한 면제
- 닫기에 영향을 주는 남은 위험 표시 또는 알려진 닫기 관련 남은 위험(close-relevant residual risk)이 없다는 보고
- 필요한 경우 최종 수락 기록

일반적인 "다 괜찮나요?" 같은 prompt로 최종 수락을 요청하지 않습니다. 최종 수락 prompt는 close basis, 즉 evidence, verification, Manual QA, residual-risk visibility 또는 `none`, acceptance가 대체하지 않는 것, 요청하는 close가 normal close인지 residual-risk accepted close인지를 보여줘야 합니다.
...
```text
닫기가 막혀 있습니다.
Judgment: 온보딩 문구에 대한 사용자 소유 Manual QA 판단이 아직 필요합니다.
Evidence: AC-02 근거가 아직 없으며, Manual QA 뒤에 에이전트가 수집할 수 있습니다.
Close Readiness: Manual QA 판단이 기록되기 전에는 close를 진행할 수 없습니다.
...
```text
direct로 완료했습니다.
Scope: 설정 label 하나였고 계정 동작은 범위 밖으로 유지했습니다. Write Authorization WA-031이 consumed됐고 `work` 전환은 없습니다.
...
Close Readiness: close-relevant blocker는 남아 있지 않습니다. 남은 위험은 이번 close에는 없음(`ResidualRiskSummary.status=none`).
```
...
Close Readiness: 검증은 RUN-018에 기록된 자체 확인(self-check)이며, 이 경로에서는 detached Eval이 필요하지 않았습니다. Manual QA는 MQA-006에서 최종 문구와 레이아웃에 대해 통과했습니다. Residual Risk RISK-004에는 후속 TASK-144가 있습니다. close reason: completed with accepted residual risk.
```
...
```text
이 접점(surface)에서 Harness/Core에 닿을 수 없어 제품 파일 쓰기를 보류합니다. 상태 변경, gate 갱신, Approval, 결과 수락, 잔여 위험 수락 판단, 닫기가 처리됐다고 주장하지 않겠습니다. 가장 작은 해소 방법은 Core를 다시 연결하거나 가능한 접점(surface)에서 계속하는 것입니다.
```
