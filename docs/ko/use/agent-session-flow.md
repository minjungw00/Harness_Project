...
이 문서는 Use 문서입니다. 문서 세트가 구현 계획에 사용할 수 있다고 승인되기 전에는 runtime/server 구현, 생성된 운영 파일, 실행 가능한 fixture 파일, runtime data를 만들라는 뜻이 아닙니다. 첫 제품 MVP 목표는 v0.1 Kernel MVP이며, Kernel Smoke는 이를 좁게 실행하는 conformance profile입니다. v0.2부터 v0.4까지는 Agency-Hardened MVP reference conformance target으로 가는 staged pack이고, v1+ Expansion은 owner 문서가 승격하고 증명하기 전까지 roadmap 범위에 남습니다.
...
- Scope: 무엇이 바뀔 수 있고, 무엇이 범위 밖인가?
- Judgment: 사용자가 지금 무엇을 결정해야 하는가?
- Evidence: 이미 무엇을 확인했고, 어떤 ref가 그 근거인가?
- Close Readiness: verification, 수동 QA, acceptance, residual-risk handling, close 전에 무엇이 남았는가?
...
| Verification | Acceptance criteria, changed files, evidence refs, 아티팩트 참조, relevant decisions, residual-risk summary, 수동 QA requirement, independence/freshness notes. | Full evaluator bundle material, source files, logs, Eval/수동 QA contract detail, verification guidance. |
...
- Scope: 현재 또는 제안 범위, 범위 밖, 다음 행동에 영향을 주는 active Change Unit 또는 write-authority boundary
- Judgment: 진행을 막는 사용자 소유 질문, 결정 패킷, sensitive-action Approval
- Evidence: supporting refs, missing support, stale support, 이미 실행한 check
- Close Readiness: 다음 결정이나 close에 영향을 주는 verification, 수동 QA, residual-risk, acceptance, close-blocker 상태
- 다음 안전한 행동
- 가장 먼저 해소할 막힘, 다음 움직임의 소유자, 가장 작은 해소 방법
- 후속 경로에 계속 영향을 줄 때만 추가 막힘
- 쓰기가 가능하거나 가까울 때 쓰기 권한 상태
- 보장 수준(guarantee level)과 접점이 실제로 막을 수 있는 것 또는 감지만 할 수 있는 것. 이는 sensitive-action Approval, verification, acceptance, gate가 아니라 표시와 risk context입니다.
...
중요한 작업을 이어가기 전에는 하네스 상태를 읽고 현재 위치를 보여줍니다. 이어가기는 오래된 chat, stale status text, remembered prior recommendation이 아니라 current Core state와 owner records를 기준으로 해야 합니다. 오래된 chat memory는 살펴볼 ref를 찾는 데 도움을 줄 수 있지만 write를 허가하거나, Task를 close하거나, 결과를 수락하거나, check를 면제하거나, 잔여 위험을 수락하거나, current state를 대체할 수 없습니다.
...
| Judgment | 계속하기 전에 사용자가 결정하거나 승인해야 하는 것. | 결정 패킷, Approval, Acceptance 결정 패킷, 잔여 위험. |
| Evidence | 주장을 무엇이 뒷받침하는지, 무엇이 빠졌는지, 뒷받침이 오래됐는지. | Evidence Manifest, Run, 아티팩트 참조, Eval input refs. |
| Close Readiness | close를 시도하거나 수락하기 전에 남은 것. | Eval, 수동 QA, Acceptance, 잔여 위험, close blockers. |
...
`harness.status`, `harness.next`, compact status card, recommendation line은 read-only display입니다. 결정 패킷, `prepare_write`, evidence collection, verification, QA, reconcile, close attempt를 추천할 수는 있지만, 추천 자체가 state를 mutate하거나, write를 허가하거나, gate를 충족하거나, 결과를 수락하거나, 잔여 위험을 수락하거나, Task를 close하지 않습니다.
...
| `request_acceptance` | Evidence, verification, QA, residual-risk visibility를 보여준 뒤 final acceptance를 요청합니다. |
...
Status, next, result, acceptance, close display의 모든 authority claim은 source ref 또는 명시적 absence로 추적 가능해야 합니다. "Write allowed"에는 쓰기 허가 기록 ref를, sensitive-action permission에는 Approval ref를, evidence sufficient에는 Evidence Manifest ref를, 분리 검증에는 Eval ref를, 수동 QA에는 수동 QA record 또는 valid waiver ref를, final acceptance에는 Acceptance 결정 패킷 ref를, residual-risk visibility에는 잔여 위험 refs 또는 `ResidualRiskSummary.status=none`을, log/diff/screenshot/trace/bundle에는 아티팩트 참조를 사용합니다. Ref가 없으면 아직 뒷받침되지 않은 claim이라고 말합니다.
...
- 사용자 소유: 제품 방향, 중요한 기술 방향, sensitive-action Approval, 수동 QA 판단 또는 waiver, 잔여 위험 수락 판단, 최종 수락처럼 사용자가 결정해야 하는 일.
...
요청 정리는 사용자가 하네스 용어로 말하지 않아도 평범한 요청을 실제로 진행 가능한 작업 모양으로 바꾸는 단계입니다.

Discovery는 구현 계획과 쓰기 권한 전에 요구사항을 구체화하는 조건부 요청 정리 자세입니다. 요청이 모호하거나, feature 성격이 있거나, auth/security에 민감하거나, UX/copy/workflow 판단이 크거나, public interface 또는 module boundary에 닿거나, policy에 영향을 줄 수 있거나, `work`가 될 가능성이 높아서 구체화가 필요할 때 사용합니다. 명확한 direct work에 의식처럼 추가하지 않습니다. 이것은 approval, sensitive-action Approval, 쓰기 허가 기록, evidence, verification, QA, acceptance, residual-risk acceptance, close, scope authority, 새 authority path가 아닙니다.

세션 시작에서 보는 것과 같은 작업 모양 신호를 살핍니다. 제품 파일 쓰기, 범위가 흐트러질 위험, 모호한 요구사항, 여러 파일이나 구조 변경, 민감하거나 정책·품질 판단이 필요한 영역, 사용자가 판단해야 하는 결정, 근거, 검증, 수동 QA, 수락, 잔여 위험이 필요한 작업이 여기에 해당합니다. 이런 신호가 있으면 평범한 요청을 예상 모드, 범위, 범위 밖, 다음 안전한 행동으로 바꿔 제안합니다.

요청 정리 경로는 다음과 같습니다.

```text
요청 -> 작업 모양 분류 -> 구체화가 필요하면 Discovery 사용 -> Discovery Brief 작성 -> 사용자 소유 결정 라우팅 -> First Safe Change Unit Candidate 제안 -> 제품 쓰기가 의도되면 prepare_write 경로
```

Discovery 출력은 owner 참조가 이미 underlying fact를 기록하는 경우가 아니라면 support 또는 projection 개념으로 다루고, 기존 owner path로 흘려보냅니다.

- Discovery Brief: 목표, 사용자 가치, 비목표, 수용 기준, 열린 가정, 제품/기술/security/QA/운영/scope 판단의 분리, 첫 번째 안전한 Change Unit 후보를 담은 compact summary.
...
- First Safe Change Unit Candidate: 해소되지 않은 사용자 소유 판단을 숨기지 않고 제안할 수 있는 가장 작은 구현 slice.

Discovery 결과는 Shared Design, 결정 패킷 candidate, Change Unit shaping으로 라우팅합니다. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate를 scope authority, sensitive-action Approval, final acceptance, residual-risk acceptance, evidence, close readiness, 쓰기 허가 기록으로 취급하지 않습니다.

Discovery 밖에서는 다음 안전한 행동을 바꾸는 질문만 합니다. Discovery 중에는 첫 번째 안전한 Change Unit을 분명하게 만들거나, 사용자 소유 판단을 드러내거나, 숨은 가정을 막는 targeted question만 묻습니다. Useful-but-not-blocking 질문은 사용자를 계속 방해하지 말고 따로 남겨 둡니다. 긴 양식보다 추천이 딸린 막힘 질문 하나가 낫습니다.
...
각 사용자 소유 질문은 사용자가 정확히 결정하는 것을 이름 붙이고, 가능한 선택지, 에이전트의 추천안, 불확실성, 의미 있을 때 영향을 받는 gate 또는 수용 기준, 사용 가능하거나 관련 있을 때 source ref, evidence ref, risk ref, design ref, 결정을 미뤄도 계속할 수 있는 일 또는 결정 전에는 진행하면 안 되는 이유를 함께 보여줘야 합니다. 에이전트가 둔 가정은 사용자에게 속한 제품, 기술, security, QA, 운영, scope, approval, acceptance, residual-risk acceptance와 따로 기록합니다.
...
`work`는 기능 추가, 구조 변경, 위험한 수정, 여러 파일 변경, 요구가 불명확한 일, 의미 있는 근거와 분리 검증이 필요한 일에 씁니다.
...
| High-risk Work | Auth, security, privacy, secrets, infra 또는 비슷하게 민감한 work. | 계속 `work`에 두고 approval, 결정 패킷, evidence, verification, QA, residual risk를 owner path로 라우팅합니다. |
...
대상이 더 이상 분명하지 않거나, 변경 경로가 활성 Change Unit을 넘거나, 여러 제품 영역에 영향을 주거나, public API 또는 module contract를 바꿀 수 있거나, 민감하거나 위험한 동작이 나타나거나, 수동 QA 또는 분리 검증이 중요해지거나, 사용자 소유의 제품 판단 또는 중요한 기술 판단이 필요하면 같은 Task를 `work`로 전환합니다.
...
First Safe Change Unit Candidate를 제안할 만큼 충분히 안다는 것은 위 항목들을 해소되지 않은 사용자 판단을 숨기지 않고 말할 수 있다는 뜻입니다. 아직 그러지 못하면 Discovery를 이어가며 다음 막힘 질문을 하거나, useful-but-not-blocking 질문은 남겨 두거나, codebase-answerable 질문은 current source에서 답하거나, 해소되지 않은 영역을 피하는 더 작은 Change Unit candidate를 제안합니다.
...
| Approval | 이 민감한 단계를 진행해도 되는가? | 기록된 scope와 expiry 안에서 이름 붙인 sensitive action을 허용합니다. | 사용자 소유 판단, 정확성(correctness), 잔여 위험 수락 판단, 쓰기 허가 기록을 대신하지 않습니다. |
| 결정 패킷 | 어떤 사용자 소유 판단을 기록하는가? | 이름 붙은 제품 판단, 중요한 기술 판단, waiver, acceptance, residual-risk, reconcile 선택을 resolved, deferred, rejected, blocked 상태로 기록합니다. | Approval record에 연결된 Approval 형태 packet이 아닌 한 sensitive-action Approval을 부여하지 않습니다. |
| Acceptance | 최종 수락이 required일 때 결과를 받아들일 수 있는가? | 닫기에 영향을 주는 잔여 위험이 보였거나 없다고 확인된 뒤 사용자의 최종 결과 판단을 기록합니다. | Evidence, verification, 수동 QA, Approval, 쓰기 허가 기록, residual-risk acceptance를 대체하지 않습니다. |
| Residual-risk acceptance | 이번 close에서 알려진 잔여 위험을 수락할 수 있는가? | 보이는 close-relevant risk의 수용을 기록하며 다른 gate가 허용할 때 residual-risk accepted close를 뒷받침합니다. | 분리 검증, correctness proof, QA pass를 만들지 않고, close를 위험 없는 일반 close로 만들지 않습니다. |
...
프롬프트나 상태에서 "approved" 또는 "승인"이라는 말을 쓸 때는 실제 권한이나 기록되는 판단을 이름 붙입니다. Sensitive-action Approval, 범위 확인(scope confirmation), 결정 패킷 resolution, 잔여 위험 수락 판단(residual-risk acceptance), 최종 수락(final acceptance), 쓰기 허가 기록 status를 구분해서 말하고, "승인"을 모든 허가와 판단을 뭉뚱그리는 포괄 라벨로 쓰면 안 됩니다.

예시:

- Dependency install sensitive-action Approval: install을 실행하거나 dependency 파일을 갱신해도 된다는 Approval은 그 dependency가 올바른 architecture 선택이라는 결정이 아닙니다. 그 선택이 호환성, rollback, 비용, 유지보수에 영향을 주면 결정 패킷을 사용합니다.
- Secret access sensitive-action Approval: 요청된 scope 안에서 secret을 읽거나 사용해도 된다는 Approval은 secret 값을 artifacts, projections, exports, logs, screenshots, summaries에 노출해도 된다는 뜻이 아닙니다.
- Auth/system change sensitive-action Approval: auth 파일, permission, system configuration을 만져도 된다는 Approval은 local session cookie, bearer token/JWT, OAuth/OIDC sign-in, social-login provider integration 같은 identity-provider 또는 session/storage model을 선택하는 결정이 아니며, role model, lockout behavior, user notice도 결정하지 않습니다.
...
- 최종 수락(Final acceptance): 결과 수락은 추가 쓰기를 허가하거나, 새 민감 동작에 Approval을 부여하거나, 빠진 근거, QA, verification, 쓰기 허가 기록을 나중에 충족시켜 주지 않습니다.

Shared Design은 Discovery에서 나온 goal, user value, scope, non-goals, assumptions, domain/module/interface impact, 분리된 사용자 소유 판단, 첫 Change Unit shape에 대한 shared understanding을 기록할 때 사용합니다. Shared Design을 sensitive-action Approval, final acceptance, residual-risk acceptance, evidence, close readiness, 쓰기 허가 기록으로 보여주면 안 됩니다. Shared Design에서 사용자가 소유하는 public API/interface choice, domain-language conflict, module boundary move, architecture direction, security/privacy trade-off, QA/verification waiver, scope expansion, known-risk waiver가 드러나면 그 선택은 결정 패킷으로 라우팅합니다.

Autonomy Boundary 안에서는 에이전트가 기존 helper를 재사용할지, private function을 어떻게 나눌지, focused tests를 어디에 둘지, 합의된 결과에 맞는 보수적인 내부 접근을 고를지 같은 일상적인 구현 세부사항을 판단할 수 있습니다. Public API 또는 module contract 변경, security 또는 privacy trade-off, UX 또는 제품 trade-off, dependency나 migration 같은 중요한 기술 방향 선택, scope expansion, 잔여 위험 수락 판단 전에는 사용자 판단을 위해 멈춰야 합니다.

## 사용자 판단으로 막힐 때

사용자가 소유하는 제품, 기술, security/privacy, QA/acceptance, residual-risk, scope/autonomy 판단이 진행을 막고 있으면 결정 패킷을 보여주거나 요청합니다. 이를 넓은 승인 질문이나 막연한 "계속할까요?"로 바꾸면 안 됩니다.

"approved", "승인", "go ahead", "진행해" 같은 말은 underlying choice가 제품 장단점, architecture 방향, QA waiver, verification gap에서 생기는 risk, final acceptance, residual-risk acceptance라면 충분하지 않습니다. Prompt는 decision route, 사용자가 결정하는 것, 결정하지 않는 것, evidence 또는 risk refs, 에이전트가 사용자 없이 결정해도 되는 일, close나 write 영향을 이름 붙여야 합니다.
...
판단 유형: QA / acceptance
이번 close에서 남은 mobile Safari wrapping risk를 받아들이겠다고 기록할까요, 아니면 수동 QA를 실행할 때까지 close를 막아 둘까요? 추천은 release timing 때문에 waiver가 필요한 경우가 아니라면 막아 두는 것입니다. 영향받는 그룹: Close Readiness; owner path/gate ref: 수동 QA / qa_gate; 영향받는 기준: AC-03 onboarding copy layout.
```
...
- QA / acceptance: QA 또는 verification waiver는 해당 Task에서 요구하는 기존 기록 경로를 사용하고 owner refs를 cite합니다. QA waiver 효과는 수동 QA / QA policy path가 담당하며, product/user risk 또는 policy-required judgment가 있으면 QA waiver 결정 패킷을 사용합니다. Verification waiver 효과는 kernel verification-waiver path가 담당하며, 사용자 소유 판단이 필요하면 관련 결정 패킷을 사용합니다. 생략하는 확인이나 대상, 수락된 잔여 위험, 잔여 위험 후속 작업, 관련 refs, 닫기 영향을 이름 붙입니다. 예를 들어 copy-only 변경에서 mobile Safari 수동 QA를 면제한다면 viewport wrapping에 대한 잔여 위험을 수락하고 release 전 browser pass를 후속 작업으로 남깁니다.
- Residual risk: 닫기 전 잔여 위험 수락 판단은 남은 한계, 이미 있는 근거, 그래도 닫을 수 있다고 볼 수 있는 이유, 잔여 위험 후속 작업을 보여줍니다. Residual-risk accepted close는 detached-verified close가 아닙니다.

가능하면 한 번에 하나의 막힘 질문만 묻습니다.

## 검토 관점과 표시

사용자가 product, engineering, design, security, QA, release-handoff 관점으로 봐 달라고 하면 `product-review`, `eng-review`, `design-review`, `security-review`, `qa-review`, `release-handoff`를 Role Lens 또는 권장 playbook 표시로 다룹니다. 라벨은 검토 관점을 고를 뿐이며 새 mode, Approval, 쓰기 허가 기록, gate, close path가 아닙니다. 정확한 Role Lens 경계는 [Agent Integration](../reference/agent-integration.md#role-lens-동작)이 담당합니다.

Role Lens와 status/next recommendation은 기존 Core/MCP path가 실제 동작을 기록하기 전까지 guidance입니다. 결정 패킷 후보, evidence gap, Eval 필요, 수동 QA 필요, residual-risk 후보, Approval 필요, Change Unit update recommendation, close blocker를 찾을 수는 있지만, 그 자체로 state를 mutate하거나 해당 route를 충족하지 않습니다.
...
Review Stages는 관리되는 표시/절차일 뿐입니다. 기준 기록이 아니며, 새 `ProjectionKind` value, Approval, evidence, verification, QA, acceptance, residual-risk acceptance, close, 쓰기 허가 기록도 아닙니다. 같은 세션에서 하는 검토(review)는 조건을 충족하는 independent Eval 또는 verification record가 없는 한 자체 확인(self-check) 또는 stewardship signal입니다. Finding이 영향받는 write나 Task 닫기에 영향을 주려면 기존 경로로 연결해야 합니다.
...
- 사용자 소유 제품, 기술, waiver, acceptance, risk 선택: 결정 패킷 후보 또는 기존 결정 패킷 ref를 보여줍니다.
- Scope, completion, autonomy mismatch: Change Unit update, 더 작은 Change Unit, follow-up Change Unit을 추천합니다.
- Stewardship 또는 design-quality issue: 영향이 실리는 기존 design, decision, QA, evidence, residual-risk, close-blocker, Change Unit recommendation 경로를 표시합니다.
- 알려진 남은 불확실성 또는 생략된 check: acceptance 또는 residual-risk accepted close 전에 잔여 위험 후보 또는 ref를 보여줍니다.
...
사용자가 자리를 비운 동안 계속하라고 했더라도, 그것은 이미 기록된 latitude를 쓰라는 뜻이지 새 권한을 만든다는 뜻이 아닙니다. 에이전트는 active Change Unit, active Autonomy Boundary, granted sensitive-action Approvals, 각 제품 파일 쓰기에 맞는 `prepare_write` / 쓰기 허가 기록 안에서만 계속할 수 있습니다.
...
관찰된 변경 경로가 사용된 쓰기 허가 기록 또는 active Change Unit 밖이면, 승인된 작업처럼 요약하지 않습니다. 불일치를 보여주고 추가 제품 파일 쓰기를 멈춘 뒤 복구로 연결합니다. 추가 변경을 되돌리거나 분리할지, 범위 결정을 요청할지, 더 넓은 변경이 의도된 것이라면 `work`로 전환할지 선택해야 합니다.
...
| Waived with accepted residual risk | Verification 또는 다른 close-relevant check가 waived되었고 보이는 remaining residual risk가 residual-risk accepted close를 위해 accepted되었을 때. |

수동 QA는 사람이 봐야 하는 품질을 확인했는지에 답합니다. 흔한 대상은 UI/UX, workflow, copy, accessibility 해석, product taste, visual output입니다. 수동 QA 결과가 실제로 기록되었거나 타당하게 면제된 것이 아니라면 browser smoke, screenshot capture, Browser QA Capture artifact, verifier note를 수동 QA처럼 보여주면 안 됩니다. Browser QA Capture는 owner 문서가 명시적으로 승격하고 증명하기 전까지 v1+ Expansion 후보입니다. 사용할 수 있더라도 그 artifact는 supporting ref일 뿐이며, 별도의 Eval 경로가 independence를 충족하지 않는 한 최종 수락이나 분리 검증이 아닙니다. Browser capture가 해당 접점에서 지원되지 않으면 사람이 작성한 수동 QA notes와 수동 제공 artifacts를 사용합니다.

잔여 위험은 알려진 한계, 불확실성, 확인하지 못한 조건, 장단점입니다. 잔여 위험을 수락하고 닫거나 최종 수락을 하기 전에는 반드시 보여야 합니다. Residual-risk acceptance는 보장 수준을 높이지 않고, verification이나 QA를 대체하지 않습니다.

Residual-risk display는 `status=none`과 `not_visible`을 구분해야 합니다. `status=none`은 현재 Task와 requested action에 대해 Core가 알고 있는 close-relevant residual risk가 없다는 뜻입니다. `not_visible`은 알려진 close-relevant risk가 있지만 acceptance 또는 close에 필요한 맥락으로 아직 보이지 않았다는 뜻이므로, 다음 행동은 그 risk와 refs를 보여주는 것입니다. `not_visible`을 "위험 없음"으로 요약하면 안 됩니다.

최종 수락은 Task 경로가 요구할 때 사용자가 결과를 받아들이는 판단입니다. sensitive-action Approval, 검증, QA, residual-risk acceptance, 정확성 증명과 다릅니다.

Verification waiver와 QA waiver는 assurance를 높이지 않습니다. Verification waiver는 분리 검증을 충족하지 않은 상태로 둡니다. close가 otherwise 허용되더라도, 면제된 verification gap에 대한 residual-risk acceptance 경로로만 닫을 수 있습니다. 이를 verified close로 요약하면 안 됩니다. QA waiver는 이름 붙인 QA requirement만 닫고 evidence, verification, acceptance, residual-risk 처리는 각각의 경로에 그대로 남깁니다. Waiver prompt와 summary는 이름 붙은 requirement, 수락된 잔여 위험, residual-risk owner refs, 필요할 때 잔여 위험 후속 작업, 영향받는 owner path 또는 close impact를 보여줘야 합니다. 정확한 waiver metadata와 gate effect는 [설계 품질 정책](../reference/design-quality-policies.md#waiver-규칙)과 [커널 참조](../reference/kernel.md#waiver-semantics)가 담당합니다.

닫기 적용 예시:

- Direct 작업: 변경 파일, 근거 refs, 자체 확인(self-check), `work`로 전환됐는지 여부를 보여줍니다. 조건을 충족하는 Eval 없이 detached verified라고 부르면 안 됩니다.
- UI/UX, workflow, copy, accessibility, product-taste, visual-output 작업: 테스트, browser smoke, Browser QA artifacts, 수동 QA, 수락을 각 줄로 분리합니다. 수동 QA를 면제한다면 생략한 대상, 수락된 잔여 위험, 잔여 위험 후속 작업을 보여줍니다.
...
- Residual-risk accepted close: 남은 한계, 이미 있는 근거, 빠졌거나 면제된 verification 또는 QA, 수락된 잔여 위험, 잔여 위험 후속 작업을 보여줍니다. 결과를 detached verified처럼 표시하면 안 됩니다.
...
`work` Task의 닫기 요약은 닫을 수 있는 근거를 보여줘야 합니다. 적용된 변경 범위, 근거 범위, 검증, 수동 QA, 잔여 위험, 수락, close reason을 해당되는 만큼 표시합니다. Gate가 waived, `not_required`, failed, pending, blocked 중 하나라면 일반적인 성공 문장에 묻지 말고 그대로 말해야 합니다.

작업 모양에 맞는 닫기 표시를 사용합니다. `DIRECT-RESULT`는 direct 작업의 간결한 결과 표시이고, `TASK` Close Summary는 진행 중이거나 최근 닫힌 `work` Task의 이어가기 표시이며, Journey Card close context는 compact status/resume 표시입니다. 이 표시들은 state, gate, 수락, QA, verification, 잔여 위험 수락 판단, close, write authority를 만들지 않습니다.

닫기 표시는 같은 네 가지 그룹을 먼저 사용해야 합니다. Scope는 changed scope, Judgment는 수락 또는 residual-risk 판단, Evidence는 support refs, Close Readiness는 verification, 수동 QA, residual risk, close blockers, close reason을 보여줍니다. Raw gate name은 blocker를 설명할 때 뒤에 붙일 수 있지만, 정확한 gate 값과 close 의미는 kernel이 담당합니다.

성공적으로 닫기 전에 보여주거나 확인할 것:

- 범위와 결과가 맞는지
- 근거가 충분한지 또는 근거 요구가 없는지
- stale evaluator bundle 또는 baseline drift가 있으면 이를 포함한 검증 상태, 또는 필요한 경우 관련 결정 패킷과 수락된 잔여 위험 refs가 있는 kernel-owned verification-waiver path
- 수동 QA 상태 또는 타당한 면제
- 닫기에 영향을 주는 잔여 위험 표시 또는 알려진 닫기 관련 잔여 위험(close-relevant residual risk)이 없다는 보고
- 필요한 경우 최종 수락 기록

일반적인 "다 괜찮나요?" 같은 prompt로 최종 수락을 요청하지 않습니다. 최종 수락 prompt는 close basis, 즉 evidence, verification, 수동 QA, residual-risk visibility 또는 `none`, acceptance가 대체하지 않는 것, 요청하는 close가 normal close인지 residual-risk accepted close인지를 보여줘야 합니다.
...
```text
하네스 상태로 가볍게 추적할 작은 작업으로 보입니다. 요청은 문구 하나만 바꾸는 일이라 direct로 보입니다. 범위는 설정 화면 문구 하나입니다. 범위 밖은 계정 동작과 API 변경입니다. 다음 안전한 행동은 해당 파일의 쓰기 권한을 확인한 뒤 수정하고 자체 확인하는 것입니다.
```
...
```text
계속하려면 sensitive-action Approval이 필요합니다.
```

좋은 sensitive-action Approval 요약:

```text
Sensitive-action Approval이 부여된 것: CU-03 안에서 zod를 install하고 package lock을 갱신하는 일. 아직 결정되지 않은 것: validation을 public API의 일부로 만들지 여부. 다음 안전한 행동: 정확한 dependency 파일 쓰기에 대해 `prepare_write`를 다시 시도합니다.
```
...
```text
닫기가 막혀 있습니다.
Judgment: 온보딩 문구에 대한 사용자 소유 수동 QA 판단이 아직 필요합니다.
Evidence: AC-02 근거가 아직 없으며, 수동 QA 뒤에 에이전트가 수집할 수 있습니다.
Close Readiness: 수동 QA 판단이 기록되기 전에는 close를 진행할 수 없습니다.
...
Evidence: `src/settings/Profile.tsx`를 변경했습니다. 확인: copy test RUN-031과 diff ART-DIFF-031. Evidence Manifest EM-031이 claim을 뒷받침합니다.
...
Judgment: mobile Safari residual risk는 DEC-022에서 받아들였고 최종 수락은 DEC-023에 기록되었습니다.
Evidence: Evidence Manifest EM-009가 RUN-018과 ART-TEST-018을 근거로 AC-01과 AC-02를 뒷받침합니다.
Close Readiness: 검증은 RUN-018에 기록된 자체 확인(self-check)이며, 이 경로에서는 detached Eval이 필요하지 않았습니다. 수동 QA는 MQA-006에서 최종 문구와 레이아웃에 대해 통과했습니다. 잔여 위험 RISK-004에는 후속 TASK-144가 있습니다. close reason: completed with accepted residual risk.
```
...
```text
이 접점(surface)에서 하네스/Core에 닿을 수 없어 제품 파일 쓰기를 보류합니다. 상태 변경, gate 갱신, Approval, 결과 수락, 잔여 위험 수락 판단, 닫기가 처리됐다고 주장하지 않겠습니다. 가장 작은 해소 방법은 Core를 다시 연결하거나 가능한 접점(surface)에서 계속하는 것입니다.
```
