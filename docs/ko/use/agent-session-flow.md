...
에이전트가 평소 말로 들어온 사용자 요청을 하네스 절차로 바꿉니다. 사용자가 Discovery, Change Unit, Decision Packet, Write Authorization, Evidence Manifest, 읽기용 요약(Projection), Autonomy Boundary, `task_events` 같은 용어를 말해야 작업이 진행되는 구조로 만들면 안 됩니다. 에이전트나 런타임 동작을 정확히 설명해야 할 때는 내부 용어를 쓰되, 사용자에게 보이는 상태에서는 작업, 범위, 판단 또는 결정할 것, 근거, 확인 또는 검증, 마무리 또는 닫기의 쉬운 설명 뒤에 붙입니다.
...
턴 맥락(Turn context)은 작고 최신이어야 하며, 단계에 맞는 내용만 담아야 합니다. 항상 주입되는 운영 맥락은 한 화면 안팎으로 제한합니다. 현재 Task 요약, 작업 모양, 범위와 하지 않을 일, 대기 중인 사용자 판단, 활성 막힘, 다음 안전한 행동, 근거 공백, 닫기 막힘, 잔여 위험 요약, 보장 수준, 출처 참조와 최신성만 포함합니다. 항목은 `none`이나 `unknown`일 수 있지만, 토큰을 아낀다는 이유로 닫기에 영향을 주는 막힘, 판단, 근거 공백, 잔여 위험을 숨기면 안 됩니다.

전체 reference docs, 전체 schema, 전체 Storage DDL, 전체 historical event log, 읽기용 요약 전체 본문, full artifact contents, raw log/screenshot/diff/trace, 관련 없는 template, future catalog 자료, 오래된 작업 이력, 관련 없는 Roadmap 자료를 항상 주입하면 안 됩니다. 그런 자료는 정확한 다음 행동에 필요할 때만 pull-on-demand로 가져옵니다.
...
에이전트 메모리, 대화 이력, 검색된 맥락, 색인된 맥락, 읽기용 요약(Projection)은 읽기 전용으로 남습니다. 무엇을 살펴볼지 제안할 수는 있지만 쓰기를 허가하거나, `gate`를 충족하거나, 근거를 만들거나, 검증을 수행하거나, 위험을 받아들이거나, Task를 닫거나, 다른 권한 주장을 만들 수 없습니다. 상태가 중요하면 행동하기 전에 현재 Core 상태 또는 상태에서 만든 간결한 맥락을 가져옵니다. 토큰을 아낀다는 이유로 사용자 소유 판단, 막힘, 범위 제한, 안전 경계, 근거 공백, 닫기 막힘, 닫기 관련 잔여 위험을 숨기면 안 됩니다. 사용자 판단 요청은 판단 내용, 선택지, 결과 영향, 불확실성, 에이전트가 사용자 대신 판단하지 않는 것을 포함해야 합니다.
...
| 근거 | 주장을 무엇이 뒷받침하는지, 무엇이 빠졌는지, 뒷받침이 오래됐는지. | evidence summary refs, Run refs, ArtifactRefs; full evidence profile이 active일 때만 Evidence Manifest. |
...
"approved", "승인", "go ahead", "proceed", "looks good", "좋아", "진행해" 같은 말은 실제 선택이 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용, 면제 판단, 단순 계속 진행 중 무엇인지 모호하면 충분하지 않습니다. 이 표현은 민감 동작 승인, 작업 수락, 잔여 위험 수용으로 자동 해석되면 안 됩니다. 질문은 판단 유형, 사용자가 판단하는 것, 판단하지 않는 것, 관련 범위와 참조, 에이전트가 사용자 없이 판단해도 되는 일, 닫기나 쓰기 영향을 이름 붙여야 합니다.
...
사용자가 보는 판단 요청에는 다음이 있어야 합니다.

- 판단 제목
- 판단 유형: 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용
- 왜 지금 이 결정이 필요한지
- 사용자가 정확히 판단하는 것
- 선택지 또는 선택된 결과
- 필요한 경우 장단점, 추천안, 불확실성, 미룰 때의 결과
- 해당하는 경우 잔여 위험
- 영향을 받는 `gate`와 수용 기준
- 사용 가능하거나 관련 있을 때 출처/근거/위험/설계 참조
- 에이전트가 사용자 없이 판단해도 되는 일
- 이 답변이 확정하지 않는 것
- 해당하는 경우 후속 작업
...
- 근거 공백 또는 뒷받침 자료: active evidence owner path를 갱신합니다. Minimum MVP-1에서는 evidence summary, Run refs, ArtifactRefs, 보이는 gap을 cite하고, full evidence profile이 active일 때만 Evidence Manifest 범위를 갱신합니다.
