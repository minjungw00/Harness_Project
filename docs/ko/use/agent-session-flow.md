...
- 로그인 실패 문구: 보안을 우선한 짧은 문구, 복구를 쉽게 설명하는 평문 문구, 필드 수준 안내를 더 구체적으로 주는 방식을 비교합니다. 계정 열거(account enumeration) 위험, 명확성, 지원 부담, 제품 톤을 기준으로 추천합니다. 결정을 미루면 validation wiring은 계속할 수 있지만 release-ready copy와 Manual QA는 열어 둬야 합니다.
