...
- 닫기 준비 상태: 검증, 수동 QA, 작업 수락, 잔여 위험 처리, close 전에 무엇이 남았는가?
...
| Discovery | 요구사항 구체화 요약, decision area별 blocking question, 확인 가능한 사실, assumption, 사용자 소유 판단 후보, QA/verification 기대 수준, current source refs, 첫 구현 후보 또는 작업 분할 제안. | 에이전트가 확인할 수 있는 사실과 사용자 결정이 필요한 항목을 나누고 안전한 다음 작업을 좁히는 데 필요한 repo docs, module/interface/domain refs, older PRD/design, decision guidance. |
...
| Close | Close-readiness summary, blockers, evidence/verification/QA/작업 수락 status, 잔여 위험 요약 또는 accepted refs, projection freshness. | Blocker 뒤에 있는 exact close, 작업 수락, 잔여 위험, QA, verification, artifact detail. |
...
- 닫기 준비 상태: 다음 결정이나 close에 영향을 주는 verification, 수동 QA, 잔여 위험, 작업 수락, close-blocker 상태
- 다음 안전한 행동
- 가장 먼저 해소할 막힘, 다음 움직임의 소유자, 가장 작은 해소 방법
- 후속 경로에 계속 영향을 줄 때만 추가 막힘
- 쓰기가 가능하거나 가까울 때 쓰기 권한 상태
- 보장 수준(guarantee level)과 접점이 실제로 막을 수 있는 것 또는 감지만 할 수 있는 것. 이는 민감 동작 승인, verification, 작업 수락, gate가 아니라 표시와 risk context입니다.
...
중요한 작업을 이어가기 전에는 하네스 상태를 읽고 현재 위치를 보여줍니다. 이어가기는 오래된 chat, stale status text, remembered prior recommendation이 아니라 current Core state와 owner records를 기준으로 해야 합니다. 오래된 chat memory는 살펴볼 ref를 찾는 데 도움을 줄 수 있지만 write를 허가하거나, Task를 close하거나, 결과를 수락하거나, check를 면제하거나, 잔여 위험을 받아들이거나, current state를 대체할 수 없습니다.
...
| Judgment | 계속하기 전에 사용자가 결정해야 하는 것. 민감 동작 permission은 그 경로가 pending일 때만 여기에 포함합니다. | 결정 패킷, Approval, Acceptance 결정 패킷, 잔여 위험. |
| Evidence | 주장을 무엇이 뒷받침하는지, 무엇이 빠졌는지, 뒷받침이 오래됐는지. | Evidence Manifest, Run, 아티팩트 참조, Eval input refs. |
| 닫기 준비 상태 | close를 시도하거나 수락하기 전에 남은 것. | Eval, 수동 QA, Acceptance, 잔여 위험, close blockers. |

이 그룹들은 gate alias가 아니며 정확한 enum 값을 정의하지 않습니다. 정확한 gate name이 유용하면 쉬운 그룹 요약 뒤에 보여주고 owner record를 link 또는 cite합니다.
...
`harness.status`, `harness.next`, compact status card, recommendation line은 read-only display입니다. 결정 패킷, `prepare_write`, evidence collection, verification, QA, reconcile, close attempt를 추천할 수는 있지만, 추천 자체가 state를 mutate하거나, write를 허가하거나, gate를 충족하거나, 결과를 수락하거나, 잔여 위험을 받아들이거나, Task를 close하지 않습니다.
...
| `prepare_write` | 정확히 의도한 쓰기에 대한 write authority를 확인합니다. |
| `implement` | 범위 안 구현 경로를 계속합니다. 제품 파일 쓰기에는 current compatible 쓰기 허가 기록만 사용합니다. |
| `launch_verify` | Current evidence refs에서 independent verification path를 시작하거나 준비합니다. |
| `record_eval` | Evaluator 결과를 기록합니다. Eval이 조건을 충족하기 전에는 분리 검증을 주장하지 않습니다. |
| `record_manual_qa` | Human QA outcome 또는 valid waiver를 기록합니다. Browser artifact만으로 수동 QA처럼 다루지 않습니다. |
| `request_acceptance` | Evidence, verification, QA, 잔여 위험 표시를 보여준 뒤 작업 수락을 요청합니다. |
| `close_task` | Close path로 close를 시도하고 blocker를 보여줄 준비를 합니다. |
...
정확한 enum과 API 계약은 [`harness.next`](../reference/mcp-api-and-schemas.md#harnessnext)가 담당합니다. 이 표는 표시 안내일 뿐 새 route나 gate가 아닙니다.

Status, next, result, 작업 수락, close display의 모든 authority claim은 source ref 또는 명시적 absence로 추적 가능해야 합니다. "Write allowed"에는 쓰기 허가 기록 ref를, 민감 동작 permission에는 Approval ref를, evidence sufficient에는 Evidence Manifest ref를, 분리 검증에는 Eval ref를, 수동 QA에는 수동 QA record 또는 valid waiver ref를, 작업 수락에는 Acceptance 결정 패킷 ref를, 잔여 위험 표시에는 잔여 위험 refs 또는 `ResidualRiskSummary.status=none`을, 잔여 위험 수용에는 accepted Residual Risk refs를, log/diff/screenshot/trace/bundle에는 아티팩트 참조를 사용합니다. Ref가 없으면 아직 뒷받침되지 않은 claim이라고 말합니다.
...
요구사항 구체화(Discovery)는 구현 계획과 쓰기 권한 전에 에이전트가 하는 조건부 요구사항 정리 행동입니다. 주로 사용자가 외워야 하는 명령어가 아닙니다. 사용자는 "구현 전에 계획을 먼저 구체화해줘" 또는 "코드를 바꾸기 전에 필요한 걸 물어봐"처럼 평범하게 말해도 됩니다. 요청이 모호하거나, feature 성격이 있거나, auth/security에 민감하거나, UX/copy/workflow 판단이 크거나, public interface 또는 module boundary에 닿거나, policy에 영향을 줄 수 있거나, 추적되는 작업이 될 가능성이 높아서 구체화가 필요할 때 사용합니다. 명확한 작은 변경에 의식처럼 추가하지 않습니다. 이것은 일반 승인, 민감 동작 승인, 쓰기 허가 기록, evidence, verification, QA, 작업 수락, 잔여 위험 수용, close, scope authority, 새 authority path가 아닙니다.
...
```text
요청 -> 작업 모양 분류 -> 필요하면 요구사항 구체화 -> Discovery Brief 또는 동등한 support 작성 -> 사용자 소유 결정 라우팅 -> 안전한 다음 작업, 첫 구현 후보, 또는 작업 분할 제안 -> 제품 쓰기가 의도되면 prepare_write 경로
```

구체화 출력(Discovery outputs)은 owner 참조가 이미 underlying fact를 기록하는 경우가 아니라면 support 또는 projection 개념으로 다루고, 기존 owner path로 흘려보냅니다.

- Discovery Brief: 목표, 사용자 가치, 범위, 비목표, 수용 기준, repo/docs/Harness state에서 에이전트가 확인할 수 있는 사실, 사용자만 결정할 수 있는 판단, 제품/UX 판단 후보, 기술 아키텍처 판단 후보, security/privacy 판단 후보, QA와 verification 기대 수준, 열린 가정, 첫 구현 후보 또는 작업 분할 제안을 담은 compact summary.
...
구체화 결과(Discovery results)는 Shared Design, 결정 패킷 candidate, Change Unit shaping으로 라우팅합니다. Discovery Brief, Question Queue, Assumption Register, First Safe Change Unit Candidate를 scope authority, 민감 동작 승인, 작업 수락, 잔여 위험 수용, evidence, 닫기 준비 상태, 쓰기 허가 기록으로 취급하지 않습니다.

요구사항 구체화 밖에서는 다음 안전한 행동을 바꾸는 질문만 합니다. 구체화 중에는 목표, 사용자 가치, 범위, 비목표, 수용 기준, 제품/UX 동작, 기술 아키텍처, security/privacy 자세, QA 또는 verification 기대 수준, 첫 구현 후보, 작업 분할, 사용자 소유 판단, 숨은 가정을 구체화하는 targeted question을 묻습니다. 긴 질문지를 한꺼번에 던지지 말고 decision area별로 묶어 묻고, 불확실성을 명시합니다. Useful-but-not-blocking 질문은 사용자를 계속 방해하지 말고 따로 남겨 둡니다. 긴 양식보다 추천이 딸린 가장 막힘이 큰 decision area 하나가 낫습니다.
...
한 번에 하나의 막힘 질문을 묻는다는 말이 구체화도 한 번이면 끝난다는 뜻은 아닙니다. 요청이 넓거나 설계 판단이 크면 목표, 사용자 가치, 범위, 비목표, 수용 기준, 영향받는 제품 영역, 사용자 화면이나 흐름, 모듈, interface, 민감 카테고리(sensitive categories), 사용자가 소유하는 제품 또는 중요한 기술 장단점 판단, security/privacy 선택, 검증 또는 수동 QA 기대 수준, 알려진 제품·구현·검증·QA·후속 위험이 안전한 다음 작업을 제안할 수 있을 만큼 잡힐 때까지 짧은 확인을 여러 차례 이어갈 수 있습니다. 구체화 과정에서는 필요한 경우 여러 targeted question을 물을 수 있습니다. 에이전트가 직접 확인할 수 있는 것과 사용자가 결정해야 하는 것을 나누고, 안전한 다음 작업, 더 작은 범위, 또는 작업 분할을 제안할 만큼 충분한 scoped information이 생기면 잠시 멈출 수 있습니다.
...
각 사용자 소유 질문은 사용자가 정확히 결정하는 것을 이름 붙이고, 가능한 선택지, 에이전트의 추천안, 불확실성, 의미 있을 때 영향을 받는 gate 또는 수용 기준, 사용 가능하거나 관련 있을 때 source ref, evidence ref, risk ref, design ref, 결정을 미뤄도 계속할 수 있는 일 또는 결정 전에는 진행하면 안 되는 이유를 함께 보여줘야 합니다. 에이전트가 둔 가정은 사용자에게 속한 제품, 기술, security, QA, 운영, scope, 민감 동작 승인, 작업 수락, 잔여 위험 수용과 따로 기록합니다.
...
| 추적되는 작업 | `work` | Feature work, UX workflow, auth-facing behavior, schema, public API/interface, 구조 변경, 위험한 수정, multi-file/multi-step delivery, 의미 있는 근거와 독립 검증이 필요한 work. | 계속 추적되는 작업으로 두고, auth, security, privacy, secrets, infrastructure 또는 비슷한 민감 영역이 나타나면 approval, 결정 패킷, evidence, verification, QA, 잔여 위험을 owner path로 라우팅합니다. |
...
| Acceptance / 작업 수락 | 작업 수락이 required일 때 결과를 받아들일 수 있는가? | 닫기에 영향을 주는 잔여 위험이 보였거나 없다고 확인된 뒤 사용자의 최종 결과 판단을 기록합니다. | Evidence, verification, 수동 QA, 민감 동작 승인, 쓰기 허가 기록, 면제 판단, 잔여 위험 수용을 대체하지 않습니다. |
...
프롬프트나 상태에서 "approved" 또는 "승인"이라는 말을 쓸 때는 실제 권한이나 기록되는 판단을 이름 붙입니다. 민감 동작 승인, 범위 확인(scope confirmation), 결정 패킷 resolution, scoped 면제 판단, 잔여 위험 수용, 작업 수락(Acceptance), 쓰기 허가 기록 status를 구분해서 말하고, "승인"을 모든 허가와 판단을 뭉뚱그리는 포괄 라벨로 쓰면 안 됩니다.
...
- Secret access 민감 동작 승인: 요청된 scope 안에서 secret을 읽거나 사용해도 된다는 Approval은 secret 값을 artifacts, projections, exports, logs, screenshots, summaries에 노출해도 된다는 뜻이 아닙니다.
...
- 작업 수락(Acceptance): 작업 수락은 추가 쓰기를 허가하거나, 새 민감 동작에 Approval을 부여하거나, 알려진 잔여 위험을 수용하거나, 빠진 근거, QA, verification, 면제 판단, 쓰기 허가 기록을 나중에 충족시켜 주지 않습니다.

Shared Design은 요구사항 구체화(Discovery)에서 나온 goal, user value, scope, non-goals, assumptions, domain/module/interface impact, 분리된 사용자 소유 판단, QA/verification 기대 수준, 안전한 다음 작업 모양에 대한 shared understanding을 기록할 때 사용합니다. Shared Design을 민감 동작 승인, 작업 수락, 잔여 위험 수용, 면제 판단, evidence, 닫기 준비 상태, 쓰기 허가 기록으로 보여주면 안 됩니다. Shared Design에서 사용자가 소유하는 public API/interface choice, domain-language conflict, module boundary move, architecture direction, security/privacy trade-off, QA/verification 면제 판단, scope expansion, known-risk 수용이 드러나면 그 선택은 결정 패킷으로 라우팅합니다.
...
사용자가 소유하는 제품, 기술, security/privacy, QA/acceptance, 잔여 위험, scope/autonomy 판단이 진행을 막고 있으면 결정 패킷을 보여주거나 요청합니다. 이를 넓은 승인 질문이나 막연한 "계속할까요?"로 바꾸면 안 됩니다.

"approved", "승인", "go ahead", "proceed", "looks good", "좋아", "진행해" 같은 말은 underlying choice가 제품 장단점, architecture 방향, QA 면제 판단, verification gap에서 생기는 risk, 작업 수락, 잔여 위험 수용이라면 충분하지 않습니다. Prompt는 decision route, 사용자가 결정하는 것, 결정하지 않는 것, evidence 또는 risk refs, 에이전트가 사용자 없이 결정해도 되는 일, close나 write 영향을 이름 붙여야 합니다.
...
- QA / acceptance(`qa_acceptance`): QA 또는 verification 면제 판단은 해당 Task에서 요구하는 기존 기록 경로를 사용하고 owner refs를 cite합니다. QA 면제 판단 효과는 수동 QA / QA policy path가 담당하며, product/user risk 또는 policy-required judgment가 있으면 QA 면제 판단 결정 패킷을 사용합니다. 검증 면제 판단 효과는 kernel verification-waiver path가 담당하며, 사용자 소유 판단이 필요하면 관련 결정 패킷을 사용합니다. 생략하는 확인이나 대상, 사용자가 수용한 잔여 위험, 잔여 위험 후속 작업, 관련 refs, 닫기 영향을 이름 붙입니다. 예를 들어 copy-only 변경에서 mobile Safari 수동 QA를 면제하려면 viewport wrapping 잔여 위험을 수용할지 사용자에게 묻고, release 전 browser pass를 후속 작업으로 남깁니다.
...
Role Lens와 status/next recommendation은 기존 Core/MCP path가 실제 동작을 기록하기 전까지 guidance입니다. 결정 패킷 후보, evidence gap, Eval 필요, 수동 QA 필요, 잔여 위험 후보, 민감 동작 승인 필요, Change Unit update recommendation, close blocker를 찾을 수는 있지만, 그 자체로 state를 mutate하거나 해당 route를 충족하지 않습니다.
...
Review Stages는 관리되는 표시/절차일 뿐입니다. 기준 기록이 아니며, 새 `ProjectionKind` value, 민감 동작 승인, evidence, verification, QA, 작업 수락, 잔여 위험 수용, close, 쓰기 허가 기록도 아닙니다. 같은 세션에서 하는 검토(review)는 조건을 충족하는 independent Eval 또는 verification record가 없는 한 자체 확인(self-check) 또는 stewardship signal입니다. Finding이 영향받는 write나 Task 닫기에 영향을 주려면 기존 경로로 연결해야 합니다.
...
- Stewardship 또는 design-quality issue: 영향이 실리는 기존 design, decision, QA, evidence, 잔여 위험, close-blocker, Change Unit recommendation 경로를 표시합니다.
- 알려진 남은 불확실성 또는 생략된 check: 작업 수락 또는 잔여 위험을 받아들이고 닫는 흐름 전에 잔여 위험 후보 또는 ref를 보여줍니다.
- QA 또는 verification outcome: 수동 QA 또는 Eval record와 gate effect를 가리킵니다.
...
Scope expansion, 민감 동작 승인 없는 새 sensitive action, Autonomy Boundary breach, 잔여 위험 수용, 작업 수락, QA 또는 verification 면제 판단, public API 또는 module contract 변경, public meaning에 영향을 주는 domain-language change, release/support promise, 사용자나 다른 시스템이 의존할 수 있는 다른 public commitment 전에는 멈추고 가장 작은 unblocker를 보여줘야 합니다.
...
근거 표시는 먼저 참조를 보여주는 방식(refs-first)으로 합니다. Evidence, Run, Eval, 수동 QA, artifact, log, screenshot, diff, trace ref와 짧은 결과를 보여주고, 사용자나 evaluator가 다음 행동을 결정하기 위해 내용을 살펴봐야 할 때만 excerpt를 본문에 넣습니다.

Task shape에 따라 "충분함"의 모습은 달라집니다. 읽기/조언은 recorded evidence가 요청된 경우에만 보통 source refs 또는 review bundle을 cite합니다. Tiny docs-only 작은 변경은 changed path, 한 줄 patch summary 또는 diff ref, 의미가 바뀌지 않았다는 self-check로 뒷받침될 수 있습니다. Evidence Manifest coverage, 아티팩트 참조, link/render proof, 또는 tiny result note를 넘는 다른 evidence가 필요하면 scope에 따라 일반 작은 변경 또는 추적되는 작업으로 상향합니다. 작은 docs-only 변경은 changed path, diff 또는 patch summary, self-check로 뒷받침될 수 있습니다. 작은 code 변경은 focused check 또는 automated check가 적용되지 않는다는 recorded reason을 더합니다. 추적되는 feature work는 각 criterion을 Run과 아티팩트 참조에 map합니다. UI/UX, workflow, copy, accessibility, product-taste, visual-output work는 visual 또는 Browser QA artifact evidence와 수동 QA judgment를 분리합니다. Sensitive work는 Approval, redaction, omission refs를 visible하게 유지하지만 Approval을 correctness로 취급하지 않습니다. Verification-required work에는 reviewed evidence를 이름 붙이는 Eval이 필요합니다.
...
잔여 위험은 알려진 한계, 불확실성, 확인하지 못한 조건, 장단점입니다. 잔여 위험을 받아들이고 닫거나 작업 수락을 하기 전에는 반드시 보여야 합니다. 잔여 위험 수용은 보장 수준을 높이지 않고, verification이나 QA를 대체하지 않습니다.
...
닫기 표시는 같은 네 가지 그룹을 먼저 사용해야 합니다. 범위는 changed scope, 판단은 작업 수락 또는 잔여 위험 수용, 근거는 support refs, 닫기 준비 상태는 verification, 수동 QA, 잔여 위험, close blockers, close reason을 보여줍니다. Raw gate name은 blocker를 설명할 때 뒤에 붙일 수 있지만, 정확한 gate 값과 close 의미는 kernel이 담당합니다.

성공적으로 닫기 전에 보여주거나 확인할 것:

- 범위와 결과가 맞는지
- 근거가 충분한지 또는 근거 요구가 없는지
- stale evaluator bundle 또는 baseline drift가 있으면 이를 포함한 검증 상태, 또는 필요한 경우 관련 결정 패킷과 수용된 잔여 위험 refs가 있는 kernel-owned verification-waiver path
- 수동 QA 상태 또는 타당한 면제
- 닫기에 영향을 주는 잔여 위험 표시 또는 알려진 닫기 관련 잔여 위험이 없다는 보고
- 필요한 경우 작업 수락 기록

일반적인 "다 괜찮나요?" 같은 prompt로 작업 수락을 요청하지 않습니다. 작업 수락 prompt는 close basis, 즉 evidence, verification, 수동 QA, 잔여 위험 표시 또는 `none`, 작업 수락이 대체하지 않는 것, 요청하는 close가 normal close인지 잔여 위험을 받아들이고 닫는 흐름인지를 보여줘야 합니다.
