...
이 문서는 하네스에 연결된 세션에서 에이전트가 사용자에게 어떻게 보여야 하는지 설명합니다. 무엇을 보여주고, 언제 묻고, 언제 계속하고, 언제 멈춰야 하는지를 다룹니다.
...
상태 메모: 이 문서는 향후 하네스에 연결된 작업에서 의도한 에이전트 동작을 설명합니다. 이 저장소는 현재 문서 전용이며 하네스 서버/런타임 구현이나 런타임 기록을 담고 있지 않습니다.
...
사용자의 다음 판단에 영향을 주는 상태, 막힘, 대기 중인 사용자 판단, 다음 행동만 보여줍니다.

에이전트가 평소 말로 들어온 사용자 요청을 하네스 절차로 바꿉니다. 사용자가 Discovery, Change Unit, Decision Packet, Write Authorization, Evidence Manifest, Autonomy Boundary, `task_events`, 읽기용 요약 같은 내부 표시 용어를 말해야 작업이 진행되는 구조로 만들면 안 됩니다. 에이전트나 런타임 동작을 정확히 설명해야 할 때는 내부 용어를 쓰되, 사용자에게 보이는 상태에서는 작업, 범위, 판단 또는 결정할 것, 근거, 확인 또는 검증, 마무리 또는 닫기의 쉬운 설명 뒤에 붙입니다.
...
```mermaid
flowchart LR
  Request["사용자 요청"] --> Check["저장소, 문서, 현재 하네스 상태 확인"]
  Check --> Shape["작업 모양 분류"]
  Shape --> Missing["빠진 정보 식별"]
  Missing --> Split["막히는 질문과 참고 질문 분리"]
  Split --> Judgment{"사용자 소유 판단 필요?"}
  Judgment -->|예| Options["선택지, 결과, 불확실성, 추천 표시"]
  Judgment -->|아니오| Next["다음 안전한 행동 제안"]
  Options --> Next
...
- 사용자 판단: 사용자가 지금 무엇을 판단해야 하며, 어떤 판단 유형이 대기 중인가?
- 근거: 이미 무엇을 확인했고, 어떤 참조가 그 근거인가?
- 닫기 준비 상태: 민감 동작 permission, 검증, 수동 QA, 작업 수락, 잔여 위험 표시, 잔여 위험 수용, 닫기 전에 무엇이 남았는가?

관문 상태는 범위, 사용자 판단, 근거, 닫기 준비 상태라는 네 가지 사용자에게 보이는 표시 그룹으로 보여줍니다. 쉬운 개념을 먼저 설명하고, 정확한 내부 용어나 참조는 경계, 막힘, 출처 참조, 런타임 규칙을 분명히 할 때만 덧붙입니다. 사용자 판단은 하나의 넓은 판단 묶음이 아닙니다. 각 항목을 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용 중 무엇인지 표시해야 합니다. 이들은 표시 그룹일 뿐입니다. `Kernel gate`를 대체하거나, 스키마 필드를 추가하거나, 재계산 규칙을 바꾸거나, 쓰기를 허가하거나, `gate`를 충족하거나, 잔여 위험을 받아들이거나, Task를 닫지 않습니다. 정확한 `gate` 값, 재계산 동작, 닫기 의미는 [Core Model 참조](../reference/core-model.md#gates)와 [`close_task`](../reference/core-model.md#close_task)가 담당합니다.

세 접점을 분리합니다.

- 사용자 상태 카드: 사용자가 읽는 짧은 현재 상태 보기입니다. 현재 위치, 막힘, 다음 안전한 행동, 참조, 최신성을 보여줄 수 있지만 상태 자체는 아닙니다.
- 에이전트 맥락 패킷: 다음 안전한 행동에 필요한 최소 현재 상태, 참조, 소유자 섹션 포인터입니다. 행동을 돕는 작은 맥락이지 전체 보고서나 완전한 기억이 아닙니다.
- Core 상태: 로컬 권한 기록이며 유일한 운영 기준입니다. 범위, gate, 사용자 판단, 근거 참조, 쓰기 확인, 작업 수락, 잔여 위험 수용, 닫기는 Core 상태가 기준입니다.

Minimum MVP-1은 상태 카드, 에이전트 맥락 패킷, 판단 요청, 실행/근거 요약, 닫기 결과라는 다섯 가지 작은 보기만 렌더링합니다. 이들은 소유자 기록 위의 보기일 뿐 권한 기록이 아닙니다. 상세 assurance, diagnostic, report, export, continuity 템플릿은 later-profile 자료입니다.

에이전트 맥락 패킷은 작고 최신이어야 하며, 단계에 맞는 내용만 담아야 합니다. 항상 주입되는 운영 맥락은 한 화면 안팎으로 제한합니다. 현재 Task 요약, 작업 모양, 범위와 하지 않을 일, 대기 중인 사용자 판단, 활성 막힘, 다음 안전한 행동, 근거 공백, 닫기 막힘, 잔여 위험 요약, 보장 수준, 출처 참조와 최신성만 포함합니다. 항목은 `none`이나 `unknown`일 수 있지만, 토큰을 아낀다는 이유로 닫기에 영향을 주는 막힘, 판단, 근거 공백, 잔여 위험을 숨기면 안 됩니다.

전체 reference docs, 전체 schema, 전체 Storage DDL, complete history, 전체 historical event log, 읽기용 요약 전체 본문, full artifact contents, raw log/screenshot/diff/trace, 전체 template, 관련 없는 template, future catalog 자료, 오래된 작업 이력, 관련 없는 Roadmap 자료를 항상 주입하면 안 됩니다. 그런 자료는 정확한 다음 행동에 필요할 때만 pull-on-demand로 가져옵니다.
...
|---|---|---|---|
| 세션 시작 | 현재 상태 또는 간결한 요약, 예상 작업 모양, 알려진 범위/하지 않을 일, 활성 막힘, 대기 중인 사용자 판단, 다음 안전한 행동, 근거 공백, 닫기 막힘, 잔여 위험 요약, 보장 수준, 출처/최신성 참조. | [세션 시작](#세션-시작), [이어가기](#이어가기), 현재 `harness.status` / `status.next_actions`, 상태 카드나 요약이 오래됐거나 다음 행동에 쓰일 때만 읽기용 보기 최신성 규칙. 별도 `harness.next` output은 later/compatibility profile이 포함할 때만 나타납니다. | 전체 작업 이력, 전체 참조 문서, 전체 schema, 오래된 상태 카드나 보고서, 관련 없는 template, 관련 없는 Roadmap, future catalog. |
...
| 쓰기 준비 | 활성 범위 또는 Change Unit, Autonomy Boundary, 의도한 경로/도구/명령 요약, 민감 동작 permission 상태, later Approval profile이 active일 때만 Approval 상태, 활성 사용자 판단, 쓰기 전 범위 확인 요약 / WriteAuthoritySummary, 기준선/최신성. | [제품 파일 쓰기](#제품-파일-쓰기), [커널: prepare_write](../reference/core-model.md#prepare_write), intended write에 필요한 [`harness.prepare_write`](../reference/api/mvp-api.md#harnessprepare_write). | 전체 Kernel/reference 문서, 관련 없는 schema, historical event log, 큰 diff/log, 전체 Storage DDL, future catalog. |
| 실행 / Run 기록 | 실행 요약, 변경 경로 요약, 소비된 내부 Write Authorization record 또는 no-write basis, 아티팩트 참조, 가림/무결성 메모, 즉시 필요한 다음 행동. MVP-1 사용자에게 렌더링할 때는 실행/근거 요약 보기가 됩니다. | [근거와 확인](#근거와-확인), [커널: record_run](../reference/core-model.md#record_run), [`harness.record_run`](../reference/api/mvp-api.md#harnessrecord_run), 표시나 복구에 필요할 때만 artifact-ref 표시 규칙. | 전체 log, raw diff, 화면 캡처, trace, bundle, artifact inventory, 전체 보고서 본문, 전체 Template 세트, future catalog. |
| 근거 검토 | 알려진 근거 요약, 있을 때 `evidence_ref`, Run refs, ArtifactRefs, 보이는 근거 공백, 오래됐거나 부족한 뒷받침, 영향을 받는 수용 기준 또는 주장, 가림/무결성 메모, 다음 근거 행동. MVP-1에서는 이때도 실행/근거 요약 보기를 사용합니다. Full Evidence Manifest profile이 active일 때만 Evidence Manifest 참조를 포함합니다. | [근거와 확인](#근거와-확인), [`harness.record_run`](../reference/api/mvp-api.md#harnessrecord_run), artifact-ref 표시 규칙, 해당 profile이 active일 때만 [커널: Evidence Manifest](../reference/core-model.md#evidence-manifest). | 전체 evidence body, 전체 log, raw diff, 화면 캡처, trace, bundle, artifact inventory, 전체 보고서 본문, 전체 Template 세트, future catalog. |
| 닫기 준비 상태 | 닫기 준비 요약, 막힘, 민감 동작 permission 상태, 근거/검증/수동 QA/작업 수락 상태, 잔여 위험 표시 또는 수용된 참조, 상태 카드/읽기용 보기 최신성, 가장 작은 해소 방법. MVP-1 사용자에게 렌더링할 때는 닫기 결과 보기가 됩니다. | [닫기](#닫기), [검증, 수동 QA, 잔여 위험, 작업 수락](#검증-수동-qa-잔여-위험-작업-수락), [커널: close_task](../reference/core-model.md#close_task), [`harness.close_task`](../reference/api/mvp-api.md#harnessclose_task). | 일반적인 all-done rollup, 전체 report 본문, 전체 historical log, 관련 없는 template, 전체 Conformance 카탈로그, 읽기용 요약 전체 본문, future catalog. |
| 사용자 판단 요청 | 사용자 소유 판단, 간결한 선택지 또는 선택된 결과, 결과 영향, 불확실성, 에이전트가 사용자 대신 판단하지 않는 것. MVP-1에서는 판단 요청 보기입니다. 그 판단을 식별하는 데 필요한 최소 범위와 참조만 보여줍니다. 복잡한 later-profile 판단이 필요할 때만 full-format Decision Packet presentation을 사용합니다. | [사용자 소유 판단으로 막힐 때](#사용자-소유-판단으로-막힐-때), [User Judgment](../reference/core-model.md#user-judgment), 정확한 field가 필요할 때만 특정 MCP 메서드. | 포괄적 approval 표현, 관련 없는 판단, 전체 근거 본문, 전체 log, 전체 schema reference, 전체 Template 세트, future catalog. |
...
에이전트 메모리, 대화 이력, 검색된 맥락, 색인된 맥락, 상태 카드, 요약, 렌더링된 template, 읽기용 요약은 읽기 전용으로 남습니다. 무엇을 살펴볼지 제안할 수는 있지만 쓰기를 허가하거나, `gate`를 충족하거나, 근거를 만들거나, 민감 동작 승인을 만들거나, 검증을 수행하거나, 작업을 수락하거나, 잔여 위험을 받아들이거나, 닫기 준비 상태를 만들거나, Task를 닫거나, 다른 권한 주장을 만들 수 없습니다. 상태가 중요하면 행동하기 전에 현재 Core 상태 또는 상태에서 만든 에이전트 맥락 패킷을 가져옵니다. 오래된 상태 카드나 읽기용 요약은 권한이 아닙니다. 토큰을 아낀다는 이유로 사용자 소유 판단, 막힘, 범위 제한, 안전 경계, 근거 공백, 닫기 막힘, 닫기 관련 잔여 위험을 숨기면 안 됩니다. 사용자 판단 요청은 판단 내용, 선택지, 결과 영향, 불확실성, 에이전트가 사용자 대신 판단하지 않는 것만 포함하고, 영향을 받는 범위를 식별하는 최소 참조만 덧붙입니다.
...
일상적인 말로 들어온 요청도 범위, 사용자 판단, 근거, 닫기 준비 상태가 계속 보여야 하는 모양이면 추적합니다.
...
- 사용자가 소유하는 제품/UX 판단 또는 비용·호환성·보안·유지보수·마이그레이션·인터페이스·의존성·위험 영향이 큰 기술 판단
- 근거, 검증, 수동 QA, 작업 수락, 잔여 위험이 필요한 작업

작은 변경은 가볍게 유지합니다. 질문에 답하거나, 코드를 살펴보거나, 결과를 설명하거나, 이미 좁은 모양이 분명한 작고 위험이 낮은 변경을 처리하는 데 불필요한 절차를 덧붙이지 않습니다. 오탈자, 문서 한 문장, 분명한 이름 변경은 그 안에 사용자 소유 판단, 민감 범주, 보안 경계, 작은 변경 경로/자체 확인 메모를 넘는 근거 필요가 숨어 있지 않을 때 내부적으로는 `direct` 아래의 tiny 하위 프로필을 사용할 수 있습니다. 사용자에게 보이는 표시는 내부 프로필이 아니라 쉬운 범위, 결과, 확인을 먼저 말해야 하며, 내부 라벨은 경계를 설명할 때만 붙입니다.
...
상태 카드, `source_state_version`, 읽기용 상태가 stale이거나 unknown이면 그 사실을 말하고, 거기에 의존하기 전에 새로 고치거나 조정(reconcile)합니다. 기준 상태를 직접 읽을 수 있으면 그 상태에서 계속할 수 있지만, 읽기용 보기는 운영 권한의 출처가 아니라고 알려야 합니다.

표시 문제는 구분해서 말합니다. 오래된 읽기용 보기는 상태 카드나 보고서가 뒤처졌을 수 있으므로 신뢰할 수 있는 맥락으로 쓰기 전에 새로 고치거나 조정해야 한다는 뜻입니다. 오래된 상태, baseline, 근거는 실제 입력이 이동했거나 부족해져 쓰기나 닫기를 막을 수 있다는 뜻입니다. MCP에 닿지 못하는 상태(MCP unavailable)는 에이전트가 필요한 하네스/Core 기능에 닿지 못한다는 뜻입니다. 그 기능이 다시 사용 가능해지기 전에는 기준 상태 변경, 민감 동작 승인, 작업 수락, 잔여 위험 수용, gate 갱신, 읽기용 보기 복구, 닫기가 처리됐다고 주장하면 안 됩니다.
...
| 마무리 또는 닫기 | 닫기를 시도하거나 작업 수락을 요청하기 전에 남은 것. | 민감 동작 승인 user judgment 또는 active일 때 Approval, 작업 수락 judgment, Residual Risk, 닫기 막힘. |
...
`harness.status`, `status.next_actions`, later/compatibility `harness.next`, 상태 카드, 에이전트 맥락 패킷, 판단 요청, 실행/근거 요약, 닫기 결과, 추천 줄은 읽기 전용 표시입니다. 판단 요청, full-format Decision Packet view, `prepare_write`, 근거 수집, 작업 수락 요청, 검증, 수동 QA, 조정(`reconcile`), 닫기 시도를 추천할 수는 있지만, 추천 자체가 상태를 변경하거나, 쓰기를 허가하거나, `gate`를 충족하거나, 작업 수락을 기록하거나, 잔여 위험을 받아들이거나, Task를 닫지 않습니다.
...
| `request_acceptance` | 작업 수락이 required일 때 Minimum MVP-1 | 알려진 근거, active-profile 검증/QA 상태, 잔여 위험 표시를 보여준 뒤 작업 수락을 요청합니다. |
| `close_task` | Minimum MVP-1 close path | 닫기 경로로 닫기를 시도하고 blocker를 보여줄 준비를 합니다. |
...
- 사용자 소유: 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용처럼 사용자가 판단해야 하는 일.
- 에이전트가 해소 가능: 상태 새로 읽기 또는 조정(`reconcile`), `prepare_write` 재시도, 빠진 근거 수집, 범위 안 확인 실행, 아티팩트 복구나 교체, 사용자 소유 판단을 바꾸지 않는 Change Unit 축소.
...
| `PROJECTION_STALE` | 읽기용 상태 요약이 오래됐습니다. | 그 요약에 의존하기 전에 읽기용 보기를 새로 고치거나 조정합니다. |
...
```text
사용자 요청 -> 저장소, 문서, 현재 하네스 상태에서 확인 가능한 사실 확인 -> 작업 모양 분류 -> 빠진 정보 식별 -> 막히는 질문과 참고 질문 분리 -> 사용자 소유 판단 식별 -> 사용자 판단이 필요하면 선택지, 결과, 불확실성, 추천 제시 -> 다음 안전한 행동 제안 -> 제품 쓰기가 의도되면 prepare_write 경로
```

요구사항 구체화 출력은 Discovery에서 나온 보조 정보를 포함해, 소유자 참조가 이미 바탕 사실을 기록하는 경우가 아니라면 보조 정보 또는 읽기용 요약 개념으로 다루고, 기존 소유자 경로로 보냅니다.

- 요구사항 요약(`Discovery Brief` 내부 라벨): 목표, 사용자 가치, 범위, 비목표, 수용 기준, 저장소/문서/테스트/현재 하네스 상태/수락된 결정/현재 작업 아티팩트에서 이미 확인한 확인 가능한 사실, 빠진 정보, 사용자만 할 수 있는 판단, 제품/UX 판단 후보, 기술 판단 후보, 민감 동작 승인 필요, QA와 검증 기대 수준, 열린 가정, 남은 불확실성, 안전한 다음 작업 후보 또는 작업 분할 제안을 담은 간결한 요약.
- Question Queue: 열린 항목을 확인 가능한 사실, 막히는 질문, 참고 질문으로 나눈 순서 있는 목록.
...
요구사항 구체화 밖에서는 다음 안전한 행동을 바꾸는 질문만 합니다. 구체화 중에는 목표, 사용자 가치, 범위, 비목표, 수용 기준, 제품/UX 판단, 기술 판단, 민감 동작 승인 필요, QA 또는 검증 기대 수준, 안전한 다음 작업 후보, 작업 분할, 사용자 소유 판단, 숨은 가정을 구체화하는 목적이 분명한 질문을 묻습니다. 긴 질문지를 한꺼번에 던지지 말고 판단 영역별로 묶어 묻고, 불확실성을 명시합니다. 참고 질문은 사용자를 계속 방해하지 말고 따로 남겨 둡니다. 긴 양식보다 추천이 딸린 가장 큰 막히는 질문 하나가 낫습니다.

질문하기 전에는 에이전트가 안전하게 직접 확인할 수 있는 확인 가능한 사실을 사용 가능한 최신 저장소, 코드베이스, 기존 문서, 테스트, 현재 하네스 상태, 수락된 결정, 현재 작업 아티팩트에서 먼저 찾아봅니다. 이미 보이는 파일 경로, 기존 동작, 용어, 제약, 수락된 선택, 테스트 기대, 아티팩트 사실을 사용자에게 다시 설명해 달라고 요구하지 않습니다. 소스가 없거나 오래됐으면 그것을 현재 사실의 근거로 삼지 말고, 불확실성으로 표시합니다.

한 번에 하나의 막히는 질문을 묻는다는 말이 요구사항 구체화도 한 번이면 끝난다는 뜻은 아닙니다. 요청이 넓거나 설계 판단이 크면 목표, 사용자 가치, 범위, 비목표, 수용 기준, 영향받는 제품 영역, 사용자 화면이나 흐름, 모듈, 인터페이스, 민감 카테고리(sensitive categories), 사용자가 소유하는 제품/UX 판단 또는 중요한 기술 판단, 검증 또는 수동 QA 기대 수준, 알려진 제품·구현·검증·QA·후속 위험이 안전한 다음 작업을 제안할 수 있을 만큼 잡힐 때까지 짧은 확인을 여러 차례 이어갈 수 있습니다. 구체화 과정에서는 먼저 확인 가능한 사실을 살피고, 다음 막히는 질문을 묻고, 참고 질문을 남겨 두며, 다음 안전한 행동으로 이동합니다. 긴 양식으로 한꺼번에 묻지 않습니다.
...
- 확인 가능한 사실은 저장소, 기존 문서, 테스트, 현재 하네스 상태, 수락된 결정, 현재 작업 아티팩트에서 확인한 뒤에만 사용자에게 묻습니다.
- 사용자만 할 수 있는 판단과 확인 가능한 사실이 분리되어 있습니다.
- 막히는 질문과 참고 질문이 분리되어 있습니다.
- 다음 안전한 행동이 읽기/조언, 작은 직접 변경, 추적되는 작업 중 무엇인지 분류되어 있습니다.
- 남은 불확실성이 숨지 않고 보입니다.

이 조건을 충족할 수 없으면 에이전트는 사용 가능한 출처를 더 확인하거나, 다음으로 가장 작은 막히는 질문을 묻거나, 참고 질문은 따로 두거나, 해결되지 않은 판단을 피하는 더 작은 안전한 조각을 제안해야 합니다.

질문하기 전에 각 열린 항목을 분류합니다. 확인 가능한 사실은 현재 저장소, 문서, 테스트, 하네스 상태, 수락된 결정, 현재 작업 아티팩트, 출처 참조를 살펴 답해야 하며 사용자에게 묻지 않습니다. 막히는 질문은 다음 안전한 행동 전에 사용자 답변이 필요한 질문입니다. 참고 질문은 요구사항 요약, Assumption Register, follow-up work, 이후 판단 요청 후보에 남겨둘 수 있습니다.
...
에이전트: 아키텍처를 추천하기 전에 현재 사용자/세션 모델, 인증 경로, 테스트, 의존성 상태, 보안 메모를 살피겠습니다. 사용자 소유 판단 후보는 인증 방식, 세션 유지 시간, 계정 존재 여부 노출 자세, ID 제공자 의존성, 검증 기대 수준, 로그인 흐름 수동 QA입니다. 다음 안전한 행동은 읽기 전용 조사와 범위가 잡힌 아키텍처 제안이며, 구현은 아닙니다.
```

고급/내부 사용자 판단 요청 예시:

```text
판단 유형: 제품/UX 판단 (`product_choice`)
판단 대상: 로그인 실패 동작.
...
| 추적되는 작업 | `work` | 기능 작업, UX 흐름, 인증에 닿는 동작, 스키마, 공개 API/인터페이스, 구조 변경, 위험한 수정, 여러 파일이나 여러 단계의 전달, 또는 active profile에 따라 의미 있는 근거, QA, 검증, 작업 수락, 위험 처리가 필요할 수 있는 작업. | 계속 추적되는 작업으로 두고, 인증, 보안, 개인정보, 비밀값, 인프라 또는 비슷한 민감 영역이 나타나면 민감 동작 permission, 판단 요청, 근거, 검증, 수동 QA, 잔여 위험을 소유자 경로로 라우팅합니다. |

정확한 mode/profile 계약은 [Core Model 참조](../reference/core-model.md#작업-모드)가 담당합니다. 이 쉬운 작업 모양은 표시 지침이며 schema 값을 추가하거나 권한 규칙을 바꾸지 않습니다.
...
대상이 더 이상 분명하지 않거나, 범위가 불분명하거나, 변경 경로가 활성 Change Unit을 넘거나, 여러 파일·제품 영역·subsystem에 영향을 주거나, public API 또는 모듈 계약을 바꿀 수 있거나, 제품/UX 판단이 필요하거나, 중요한 기술 판단이 필요하거나, 보안/개인정보 영향이 있거나, 민감한 행동이 나타나거나, QA/검증 요구가 커지거나, 근거가 부족하거나, 잔여 위험이 작지 않거나, multi-step delivery가 필요하면 같은 Task를 추적되는 작업으로 전환합니다.
...
- 비밀값 접근 민감 동작 승인: 요청된 범위 안에서 비밀값을 읽거나 사용해도 된다는 Approval은 비밀값을 아티팩트, 상태 카드, 보고서, 내보내기, 로그, 스크린샷, 요약에 노출해도 된다는 뜻이 아닙니다.
...
<a id="사용자-소유-판단으로-막힐-때"></a>
<a id="사용자-소유-결정으로-막힐-때"></a>

## 사용자 소유 판단으로 막힐 때

사용자가 소유하는 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용이 진행을 막고 있으면 판단 요청을 보여줍니다. Full-format Decision Packet presentation은 복잡하거나 later-profile 판단에만 사용합니다. 일반 사용자에게 보이는 질문은 판단, 선택지, 결과, 다음 행동에서 시작해야 합니다. 이름 붙은 민감 동작이 막고 있으면 민감 동작 승인을 사용합니다. 이를 막연한 동의 질문이나 막연한 "계속할까요?"로 바꾸면 안 됩니다.

"approved", "승인", "go ahead", "proceed", "looks good", "좋아", "진행해" 같은 말은 실제 선택이 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용, 면제 판단, 단순 계속 진행 중 무엇인지 모호하면 충분하지 않습니다. 이 표현은 민감 동작 승인, 작업 수락, 잔여 위험 수용으로 자동 해석되면 안 됩니다. 활성 질문이나 주변 상태는 판단 유형을 모호하지 않게 해야 하지만, 질문 자체는 간결해야 합니다.

토큰을 아끼더라도 사용자가 판단하는 데 필요한 맥락을 빼면 안 됩니다. 간결한 판단 요청도 decision, options 또는 selected outcome, consequence, uncertainty, agent가 사용자 대신 판단하지 않는 것을 보여줘야 합니다.

기본 사용자 판단 요청에는 다음만 넣습니다.

- 사용자가 판단할 결정 또는 정확한 선택
- 선택지 또는 선택된 결과
- 결과 영향
- 불확실성
- 에이전트가 사용자 대신 판단하지 않는 것

활성 질문을 구분해야 할 때만 짧은 제목, 판단 유형, 출처 참조 하나를 붙입니다. 장단점, 추천, 미룰 때의 영향, 영향을 받는 `gate`나 수용 기준, 근거/위험/설계 참조, 후속 작업은 active profile과 복잡도가 필요로 할 때 pull/drill-down 또는 full-format presentation에 둡니다.
...
`request_user_decision`, `judgment_domain`, `decision_kind`, `decision_profile`은 오래된 요청 형태를 위한 compatibility alias입니다. 현재 field든 alias든 사용자가 독립된 축을 이해해야 하는 것처럼 보여주면 안 됩니다. 영향을 받는 `gate`나 막힌 행동은 별도 field와 owner record가 담당합니다. 정확한 public field는 [`harness.request_user_judgment`](../reference/api/mvp-api.md#harnessrequest_user_judgment)이 소유하고, 기준 권한은 [User Judgment](../reference/core-model.md#user-judgment)와 [Decision Gate](../reference/core-model.md#decision-gate)가 소유합니다. 사용자에게는 쉬운 말로 판단을 보여주고, 필요한 참조는 자세히 볼 수 있게 둡니다.
...
판단 요청: 설정 버튼 문구
판단 유형: 제품/UX 판단 (`product_choice`)
판단 내용: 이 설정 문구 변경에는 "Save"와 "Update" 중 어떤 버튼 문구를 쓸까요?
선택지: "Save" 또는 "Update".
...
판단 요청: 로그인 실패 피드백 패턴
판단 유형: 제품/UX 판단 (`product_choice`)
판단 내용: 이번 범위에는 어떤 로그인 실패 경험을 쓸까요?
선택지: 인라인 메시지, 토스트, 모달.
...
```text
판단 요청: 개인정보(PII) 로그 정책
판단 유형: 기술 판단 (`technical_choice`)
판단 내용: 로그인 진단 로그에 어떤 사용자 식별자를 남길 수 있을까요?
선택지: 개인정보를 남기지 않음, 가림 처리/토큰화된 id, 제한된 email/domain field.
추천: 지원 흐름이 더 좁은 visible field를 요구하지 않는 한 가림 처리/토큰화된 id를 씁니다.
결과 영향: 이 선택은 logging policy 준수를 증명하는 데 필요한 근거를 정합니다.
불확실성: support workflow에 repo만으로 확인할 수 없는 visible field가 필요할 수 있습니다.
에이전트가 사용자 대신 판단하지 않는 것: legal/compliance acceptance, data-retention policy, external log access를 위한 민감 동작 승인, 잔여 위험 수용.
```
...
판단 유형: 기술 판단 (`technical_choice`)
판단 내용: 이번 닫기에서 모바일 Safari 수동 QA 요구를 면제할까요, 아니면 수동 QA를 실행할 때까지 닫기를 막아 둘까요?
선택지: 해당 수동 QA check 면제, 닫기 막힘 유지, 닫기 미루기.
...
- 제품/UX 판단(`product_choice`): 로그인 실패 UX는 인라인 메시지, 토스트, 모달을 비교하고, 흐름, 접근성, 방해 정도, 문구 위험을 기준으로 하나를 추천합니다. 판단을 미루면 백엔드 인증 작업은 계속할 수 있지만 최종 로그인 실패 경험이 완료됐다고 말하면 안 됩니다.
- 제품/UX 판단(`product_choice`): 로그인 실패 문구는 일반적인 문구, 더 구체적인 문구, 혼합 문구를 비교합니다. 계정 존재 여부 노출(account enumeration) 위험, 명확성, 복구 도움 정도, 지원 부담, 제품 톤을 기준으로 추천합니다. 판단을 미루면 유효성 검사 연결은 계속할 수 있지만 출시 가능한 문구와 수동 QA는 열어 둬야 합니다.
- 제품/UX 판단(`product_choice`): 제품 감각과 수동 QA 필요성은 사람이 시각적으로 확인해야 하는 완성도 높은 상호작용과 테스트 및 브라우저 간단 확인으로 확인 가능한 더 보수적인 동작을 비교합니다. 감각상의 장단점, QA 비용, 사용자 영향, 수동 QA를 미뤘을 때 계속할 수 있는 일 또는 결정 전에는 진행하면 안 되는 이유를 설명합니다.
- 기술 판단(`technical_choice`): 인증 방식은 세션 쿠키, Bearer/JWT 토큰, OAuth/OIDC, 소셜 로그인 제공자 연동을 비교할 수 있습니다. 다만 OAuth/OIDC도 결과적으로 로컬 세션이나 토큰 전략을 만들 수 있으므로, 둘 다 중요하면 ID 제공자 선택과 세션/저장 모델을 분리해야 합니다. 폐기 가능성, CSRF/XSS 노출, 클라이언트 호환성, 운영 복잡도, 이전 비용을 설명합니다. 판단을 미루면 세션 모델에 약속을 만들지 않는 범위에서만 폼 골격을 계속할 수 있습니다.
- 기술 판단(`technical_choice`): 의존성 선택은 설치 또는 의존성 파일 갱신을 허용하는 민감 동작 승인과, 그 의존성을 채택하는 아키텍처 결정을 분리합니다. 의존성을 추가할지, 기존 유틸리티를 쓸지, 기능을 미룰지 비교하고 호환성, 되돌리기, 비용, 유지보수 영향을 설명합니다.
- 기술 판단(`technical_choice`): 도메인 언어 충돌은 현재 제품 용어를 유지할지, 좁은 코드 별칭을 추가할지, 새 용어로 마이그레이션할지 비교합니다. 제품 의미, 공개 문서, API/인터페이스 이름, 호출자 기대, 모듈 책임, 마이그레이션 비용, 결정을 미뤘을 때 계속할 수 있는 일을 설명합니다.
- 기술 판단(`technical_choice`): 스키마/데이터 모델 마이그레이션은 추가형 마이그레이션, 호환성 완충 계층, 호환성을 깨는 정리를 비교합니다. 마이그레이션 근거, 데이터 보강 위험, 되돌리기 경로, 테스트 경계, 유지보수 비용을 설명합니다.
- 기술 판단(`technical_choice`): 공개 API/인터페이스 또는 모듈 경계는 현재 인터페이스를 유지할지, 좁은 확장을 추가할지, 책임을 모듈 경계 너머로 옮길지 비교합니다. 호출자 영향, 호환성 또는 호환성 깨짐 위험, 경계 테스트, 문서화된 약속, 이전 경로, 향후 변경 비용을 설명합니다.
- 기술 판단(`technical_choice`): 범위 또는 Autonomy Boundary 확장은 현재 작은 범위를 유지할지, 요청된 대상을 추가할지, 후속 Change Unit으로 분리할지 비교합니다. 영향을 받는 경로, 사용자에게 보이는 동작, 계속 범위 밖에 남는 것, 쓰기 영향, 에이전트가 혼자 판단해도 되는 일을 설명합니다.
- 민감 동작 승인(`sensitive_action_approval`): 비밀값 접근, 권한 변경, 데이터 내보내기, 의존성 설치 같은 민감 동작은 범위가 정해진 승인 경계일 뿐입니다. 역할, 필드, 가림 처리, 감사 기록, 보관 기간, 되돌리기, 사용자 고지에는 별도의 제품 또는 기술 판단이 여전히 필요할 수 있습니다.
- 기술 판단(`technical_choice`): 개인정보(PII) 로그 정책은 개인정보를 로그에 남기지 않는 방식, 가림 처리(redacted) 또는 토큰화된 식별자, 제한된 진단 필드를 비교해야 합니다. 개인정보 노출, 디버깅 가치, 보관 기간, 가림 처리, 감사 기록, 정책 준수를 증명할 근거를 설명합니다.
- 기술 판단(`technical_choice`): QA 또는 검증 면제 판단은 해당 Task에서 요구하는 기존 기록 경로를 사용하고 소유자 참조를 인용합니다. QA 면제 판단 효과는 수동 QA / QA 정책 경로가 담당합니다. 검증 면제 판단 효과는 커널의 검증 면제 경로가 담당합니다. 생략하는 확인이나 대상, 별도로 수용된 잔여 위험, 잔여 위험 후속 작업, 관련 참조, 닫기 영향을 이름 붙입니다. 면제 판단과 잔여 위험 수용이 모두 필요하면 별도 판단 줄 또는 별도 판단 요청으로 렌더링합니다. 예를 들어 문구만 바꾸는 변경에서 mobile Safari 수동 QA를 면제하려면 viewport wrapping 잔여 위험을 별도로 수용할지 사용자에게 묻고, 릴리스 전 브라우저 확인을 후속 작업으로 남깁니다.
...
가능하면 한 번에 하나의 막히는 질문만 묻습니다. 하지만 이것이 질문 하나로 구체화를 끝내야 한다는 뜻은 아닙니다. 요구사항 구체화는 확인 가능한 사실을 먼저 살피고, 다음 막히는 질문을 묻고, 참고 질문을 남겨 두며, 다음 안전한 행동을 추천하는 짧은 여러 차례의 흐름일 수 있습니다. 긴 양식으로 변하면 안 됩니다.
...
Role Lens와 status/next 추천은 기존 Core/MCP 경로가 실제 동작을 기록하기 전까지 안내일 뿐입니다. 판단 요청 후보, optional full-format Decision Packet 후보, 근거 공백, Eval 필요, 수동 QA 필요, 잔여 위험 후보, 민감 동작 permission 필요, later Approval 필요, Change Unit 업데이트 추천, 닫기 막힘을 찾을 수는 있지만, 그 자체로 상태를 변경하거나 해당 경로를 충족하지 않습니다.
...
- 근거 공백 또는 뒷받침 자료: active evidence owner path를 갱신합니다. Minimum MVP-1에서는 `evidence_ref` ref, Run refs, ArtifactRefs, 보이는 gap을 cite하고, full evidence profile이 active일 때만 Evidence Manifest 범위를 갱신합니다.
- 사용자 소유 제품, 기술, 면제 판단, 작업 수락, 잔여 위험 선택: 판단 요청 후보 또는 기존 user judgment 참조를 보여줍니다.
...
Feedback Loop는 선택된 루프와 루프 발견사항을 위한 기준 근거 기록 경로입니다. 정확한 routing 경계는 [설계 품질 정책](../reference/design-quality-policies.md#finding-라우팅)과 [Core Model 참조](../reference/core-model.md#finding-라우팅)가 담당합니다. 이 Use 문서는 에이전트가 사용자에게 어떻게 보여줘야 하는지만 설명합니다.
...
외부 영향(side effect)이 있을 때는 실행 전의 주장과 실행 뒤의 기록을 분리합니다. 실행 전에는 의도한 영향, 민감 범주(sensitive category), Approval 또는 판단 요청 필요 여부, 보장 수준을 말합니다. 실행 뒤에는 실제로 일어난 일, 기록된 Run/아티팩트/근거 참조, 가림/생략/차단/오래됨/위반 여부를 말합니다. 보장 수준은 표시와 위험 맥락입니다. Approval을 부여하거나, 결과를 검증하거나, QA를 기록하거나, 잔여 위험을 받아들이거나, 작업 수락을 하거나, Task를 닫지 않습니다. 정확한 보장 수준 의미는 [보안 참조](../reference/security.md#정직한-guarantee-display)가 담당합니다.
...
작업 모양에 따라 "충분함"의 모습은 달라집니다. 읽기/조언은 기록된 근거가 요청된 경우에만 보통 출처 참조 또는 검토 자료 묶음을 인용합니다. 아주 작은 문서 변경은 변경 경로, 변경 차이 또는 패치 요약, 의미가 바뀌지 않았다는 자체 확인으로 뒷받침될 수 있습니다. Full Evidence Manifest 범위, 아티팩트 참조, 링크/렌더링 증명, 또는 작은 결과 메모를 넘는 다른 근거가 필요하면 범위에 따라 일반 작은 변경 또는 추적되는 작업으로 상향하며, Evidence Manifest는 해당 profile이 active일 때만 사용합니다. 작은 코드 변경은 집중 확인 또는 자동 확인이 적용되지 않는다는 기록된 이유를 더합니다. 추적되는 기능 작업은 각 기준을 Run refs, ArtifactRefs, evidence summary에 연결합니다. Full Evidence Manifest criteria mapping은 later/profile owner path입니다. UI/UX, 작업 흐름, 문구, 접근성, 제품 감각, 시각 출력 작업은 시각 근거 또는 브라우저 QA 아티팩트와 수동 QA 판단을 분리합니다. 민감한 작업은 민감 동작 승인 user judgment 또는 later Approval refs, 가림 처리, 생략 참조를 보이게 유지하지만 민감 동작 permission을 정확성 판단으로 취급하지 않습니다. 검증이 필요한 작업에는 detached verification profile이 active일 때 검토한 근거를 이름 붙이는 Eval이 필요합니다.

근거가 오래되면 이유를 쉬운 말로 말하고 가장 작은 복구 방법을 이름 붙입니다. 흔한 원인은 기준선 불일치, 뒷받침하던 Run 또는 Eval 이후 변경 파일이 달라짐, 민감 동작 permission / Approval 상태 변화 또는 만료, 빠졌거나 무결성에 실패한 아티팩트, 관련 Shared Design, 도메인 용어, 모듈 지도, 인터페이스 계약 변경입니다.
...
| 작업 수락 | 그런 판단이 요구되는 경우 사용자가 결과를 받아들이는가. |
...
| 표시 이름 | 사용할 때 |
|---|---|
| 자체 확인 | 구현 경로가 자기 결과를 확인했을 때. |
| 분리 검증 후보 | 새 세션, 새 작업 트리, 샌드박스, 수동 평가 묶음, 또는 조건을 충족하는 하위 에이전트 경로가 독립적일 수 있지만 아직 분리 보증을 만들지 않았을 때. |
| 분리 검증됨 | Eval이 유효한 독립성, same-session self-review 문제 없음, stale baseline 또는 bundle input 없음으로 pass했을 때. |
| 사용자가 잔여 위험을 수용한 면제 | Verification 또는 다른 닫기 관련 확인이 waived되었고 사용자가 보이는 잔여 위험을 수용한 뒤 닫기로 선택했을 때. |

수동 QA는 사람이 봐야 하는 품질을 확인했는지에 답합니다. 흔한 대상은 UI/UX, 작업 흐름, 문구, 접근성 해석, 제품 감각, 시각 출력입니다. 수동 QA 결과가 실제로 기록되었거나 타당하게 면제된 것이 아니라면 브라우저 간단 확인, 스크린샷 캡처, 브라우저 QA 캡처 아티팩트, 검증자 메모를 수동 QA처럼 보여주면 안 됩니다. Browser QA Capture는 owner 문서가 명시적으로 승격하고 증명하기 전까지 로드맵 Expansion 후보입니다. 사용할 수 있더라도 그 아티팩트는 뒷받침하는 참조일 뿐이며, 별도의 Eval 경로가 독립성을 충족하지 않는 한 작업 수락이나 분리 검증이 아닙니다. 브라우저 캡처가 해당 접점에서 지원되지 않으면 사람이 작성한 수동 QA 메모와 수동 제공 아티팩트를 사용합니다.
...
MVP-1 닫기 표시는 닫기 결과를 사용합니다. 작은 변경에는 짧게, 추적되는 작업에는 조금 더 자세히 쓸 수 있지만 닫기 준비 상태, 작업 수락, 잔여 위험, 막힘은 보여야 합니다. 상세 `DIRECT-RESULT`, `TASK` Close Summary, Journey Card close context는 later-profile 또는 compatibility 표시입니다. 이 표시들은 state, gate, 작업 수락, QA, 검증, 잔여 위험 수용, 닫기, 쓰기 전 범위 확인 기록을 만들지 않습니다.
...
- 해당될 때 민감 동작 permission 상태
- evaluator bundle이 오래됐거나 기준선 불일치가 있으면 이를 포함한 검증 상태, 또는 필요한 경우 관련 user judgment와 수용된 잔여 위험 참조가 있는 커널이 소유한 검증 면제 경로
- 수동 QA 상태 또는 타당한 면제
...
닫기가 막혔으면 정확한 이유와 가장 작은 해소 방법을 말합니다. 사용자가 이해할 수 있는 닫기 막힘 표시는 어떤 판단이 남았는지, 어떤 근거가 빠졌거나 오래됐는지, 현재 적용된 프로필에서 검증이나 수동 QA가 필요한지, 작업 수락이 필요한지, 어떤 잔여 위험이 보이거나 아직 충분히 보이지 않는지, 가장 작은 다음 해소 행동이 무엇인지를 보여줘야 합니다.
...
```text
사용자가 소유하는 제품 판단 때문에 막혔습니다. AC-02의 빈 상태 동작을 선택해야 합니다. 옵션은 바로 설정을 유도하거나 데이터가 생길 때까지 조용히 두는 것입니다. 추천은 설정 유도이며 불확실성은 낮습니다. 판단을 미루면 데이터 로딩 리팩터링은 끝낼 수 있지만 빈 상태 UX가 완료됐다고 말할 수는 없습니다.
```
...
```text
민감 동작 승인이 부여된 것: CU-03 안에서 zod를 설치하고 package lock을 갱신하는 일. 아직 판단되지 않은 것: 검증을 공개 API의 일부로 만들지 여부. 다음 안전한 행동: 정확한 의존성 파일 쓰기에 대해 `prepare_write`를 다시 시도합니다.
```

나쁜 민감 동작 승인 요약:

```text
민감 동작 승인이 있으니 auth 설계를 끝내고, 구현하고, 배포하고, Task를 닫겠습니다.
```
...
```text
닫기가 막혀 있습니다.
사용자 판단: 온보딩 문구에 대한 수동 QA 판단이 아직 필요합니다.
근거: AC-02 근거가 아직 없으며, 수동 QA 뒤에 에이전트가 수집할 수 있습니다.
닫기 준비 상태: 수동 QA 판단이 기록되기 전에는 close를 진행할 수 없습니다. 이 프로필에서 검증은 현재 주요 막힘이 아니고, 작업 수락은 아직 요청하지 않았으며, 잔여 위험 수용은 기록되지 않았습니다.
...
사용자 판단: 표시된 mobile Safari 위험에 대한 잔여 위험 수용은 DEC-022에 기록됐고, 작업 수락은 별도로 DEC-023에 기록되었습니다.
근거: evidence summary EVS-009가 RUN-018과 ART-TEST-018을 근거로 AC-01과 AC-02를 뒷받침합니다. Full evidence profile이 active이면 Evidence Manifest EM-009가 criteria-to-evidence map을 담습니다.
