...
다음 안전한 행동: 현재 인증 흐름을 살핀 뒤, 가장 작은 안전한 구현 조각과 그 전에 필요한 사용자 판단을 가져오겠습니다.
```
...
```text
설정 화면 버튼 문구를 "저장"에서 "프로필 업데이트"로 바꿔줘.
```
...
대상 component와 직접 관련된 copy test만 수정하겠습니다.
```
...
```text
완료했습니다.
변경: 설정 버튼 문구를 "프로필 업데이트"로 변경.
확인: 직접 관련된 copy test와 diff 확인.
더 큰 제품 판단으로 번지지 않았고, 이번 작은 변경에 대해 알려진 닫기 관련 잔여 위험은 없습니다.
```

사용자에게 보이는 흐름이 가볍다는 뜻이 하네스를 내부적으로 건너뛴다는 뜻은 아닙니다. 제품 파일이 바뀔 수 있으면 에이전트는 여전히 범위를 좁게 유지하고, 의도한 쓰기가 그 범위에 맞는지 확인하고, 무엇이 바뀌었는지 기록하고, 일이 원래 요청을 넘어 커지면 알려야 합니다.

작은 일은 더 이상 작지 않을 때 커져야 합니다. 예를 들어 버튼 문구가 결제, 온보딩, 프로필 흐름에 공유되어 있거나, 문구가 계정 동작을 바꾸거나, localization 전략이 바뀌거나, 단순 copy check를 넘는 시각적 QA가 필요해지면 더 넓은 작업으로 다뤄야 합니다.
...
| 하네스 라벨 | 쉬운 뜻 |
|---|---|
| Discovery | 구현 계획 전에 요청을 구체화하는 단계입니다. |
