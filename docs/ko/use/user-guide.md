...
하네스가 연결된 상태에서 AI와 함께 하는 작업을 시작하거나, 이어가거나, 막힘을 풀거나, 닫으려 할 때 읽습니다. 제품 파일이 바뀔 수 있거나, 범위가 흔들릴 수 있거나, 사람의 판단이 필요하거나, 완료 주장에 근거가 필요하거나, 민감한 행동이 관련될 때 특히 유용합니다.
...
에이전트가 사용자의 요청을 필요한 하네스 절차로 바꿔야 합니다. 사용자가 내부 기록을 직접 조작하거나 Discovery, Change Unit, Decision Packet, Write Authorization, Evidence Manifest, Projection, Autonomy Boundary, `task_events` 같은 라벨을 알 필요는 없습니다. 먼저 쉬운 말로 설명하고, 정확한 하네스 라벨은 실제로 멈춘 이유, 경계, source ref, 닫기 조건을 설명해야 할 때만 덧붙이면 됩니다.

작은 `direct` 작업의 절차 예산은 의도적으로 작습니다. 간단한 범위, 제품 파일이 바뀔 수 있을 때의 최소 작업 경계, 정확한 쓰기 전 확인, 무엇이 바뀌었는지, 무엇을 확인했는지, `work`로 전환됐는지, 어떤 잔여 위험이 있는지, 닫기를 막는 결정이 필요한지를 담은 짧은 결과면 충분합니다. 내부 라벨로는 Change Unit과 Write Authorization이 나올 수 있지만, 사용자가 거기서 시작할 필요는 없습니다. `direct`는 사용자에게 보이는 단계가 적다는 뜻이지, 범위나 쓰기 확인을 건너뛴다는 뜻이 아닙니다.
...
|---|---|---|
| 범위 | 무엇이 바뀔 수 있는가? | 합의한 작업 영역, 범위 밖 항목, 다음 행동이 맞는지, 필요할 때 Change Unit이나 Autonomy Boundary 같은 내부 참조. |
| 판단 | 사용자가 무엇을 결정해야 하는가? | Product/UX 선택, 중요한 기술 선택, 민감 동작 승인, QA 또는 waiver 선택, 작업 수락, 위험, 범위 판단. 내부 기록은 Decision Packet일 수 있습니다. |
| 근거 | 완료 주장을 무엇이 뒷받침하는가? | 변경 경로, test, log, screenshot, 아티팩트 참조, 자체 확인, 빠진 근거, 오래된 근거, 필요할 때 Evidence Manifest 참조. |
| 닫기 준비 상태 | 닫기를 아직 막는 것은 무엇인가? | 검증, 수동 QA, 작업 수락, 잔여 위험 표시 또는 수용, close blocker, close reason. |
...
- 범위: 무엇이 범위 안이고, 무엇이 범위 밖인가?
- 판단: 지금 필요한 사용자 소유 결정이 있는가?
- 근거: 이미 있는 뒷받침이나 확인은 무엇이고, 아직 부족한 것은 무엇인가?
- 닫기 준비 상태: 검증, 수동 QA, 작업 수락, 잔여 위험 처리, close를 아직 막는 것은 무엇인가?
...
구현 계획 전에 더 적극적인 구체화를 원하면 먼저 요구를 정리해 달라고 할 수 있습니다.

```text
먼저 요구를 구체화해줘. 첫 번째 안전한 범위가 분명해질 때까지 필요한 질문을 해줘. 제품 결정과 기술 결정을 분리해줘. 선택지, 추천안, 불확실성을 보여줘. 코드베이스가 답할 수 없는 것만 나에게 물어봐.
```

Reference 문서는 이 구체화 자세를 Discovery라고 부릅니다. 좋은 구체화는 목표, 사용자 가치, 비목표, 완료 기준, 가정, 제품 결정, 기술 결정, 보안 선택, QA 또는 검증 기대 수준, 운영 고려사항, 범위 경계를 분리합니다. 필요한 경우 여러 질문을 물을 수 있지만, 첫 번째 안전한 범위를 제안할 만큼 분명해지면 멈춰야 합니다. 내부적으로 그 출력은 Shared Design, 결정 패킷 candidate, Change Unit shaping에 입력될 수 있지만 approval, sensitive-action Approval, 쓰기 허가 기록, evidence, verification, QA, acceptance, 잔여 위험을 받아들이는 판단, close, scope authority, 새 authority path가 아닙니다.
...
- 범위와 범위 밖
- 다음 안전한 행동
- 네 가지 표시 그룹: 범위, 판단, 근거, 닫기 준비 상태
- 무엇이 막혀 있는지, 무엇을 확인했는지, 무엇이 남았는지, 사용자가 무엇을 결정하는지
- 필요한 결정이나 막힘, 그리고 다음 움직임의 소유자
- 가장 작은 해소 방법
- 쓰기 권한, 근거, 검증, 수동 QA, 잔여 위험, 작업 수락 상태가 의미 있을 때의 요약
...
범위: 로그인 폼, 로그인 API 호출, 세션 저장; 범위 밖: 비밀번호 재설정, 계정 생성; 쓰기 확인은 아직 요청하지 않음
판단: 로그인 실패 메시지는 사용자 선택이 필요함; decision ref DEC-014; 가장 작은 해소 방법은 선택지 하나 고르기
근거: current Core state v42를 읽음; 아직 구현 근거 없음
닫기 준비 상태: 이후 흐름에는 최종 문구/레이아웃 수동 QA, 잔여 위험 표시, 작업 수락, close 판단이 남아 있음
능력/상태: 협력형(cooperative) 접점; source_state_version v42에서 온 읽기용 상태 current
```

먼저 다음 안전한 행동과 가장 작은 해소 방법을 봅니다. 막힘은 다음 움직임을 누가 소유하는지 말해야 합니다. 제품 판단, 중요한 기술 판단, Approval, QA, 잔여 위험을 받아들이는 판단, 작업 수락처럼 사용자가 판단해야 하면 사용자 소유 막힘입니다. 상태 refresh, 근거 수집, check 재실행, `prepare_write` 재시도, 범위 축소처럼 에이전트가 사용자 판단을 바꾸지 않고 처리할 수 있으면 에이전트가 해소 가능한 막힘입니다.
...
에이전트는 Approval, 결정 패킷 결과, 쓰기 허가 기록, 근거, 검증, 수동 QA, 작업 수락, 잔여 위험을 서로 분리해서 보여줘야 합니다. 하나를 다른 것의 대체물처럼 쓰면 안 됩니다.
...
가볍게 "go ahead" 또는 "진행해"라고 답하는 말은 에이전트가 사용자가 정확히 무엇을 결정하는지 이미 이름 붙였을 때만 사용할 수 있습니다. 제품 장단점, architecture 선택, QA 또는 verification waiver, 작업 수락, 잔여 위험을 받아들이는 판단에는 선택지, 결과, 관련 ref, 에이전트가 여전히 사용자 없이 결정해도 되는 일, 기록되는 경로가 보이지 않는 한 충분하지 않습니다.
...
대신 사용자나 다른 코드가 기대하는 약속을 바꾸는 선택에서는 멈춰야 합니다. Public API나 module contract, 보안 또는 개인정보 장단점, UX나 제품 동작, 중요한 의존성 또는 migration 방향, 범위 확장, 알려진 잔여 위험을 받아들이는 일이 여기에 해당합니다.

간단히 나누면 범위는 어떤 작업 표면이 바뀔 수 있는지 말하고, 에이전트의 자율성은 그 표면 안에서 에이전트가 어떤 판단을 혼자 해도 되는지 말합니다. Reference 문서는 이를 Change Unit scope와 Autonomy Boundary라고 부릅니다. 둘 다 그 자체로 쓰기 권한은 아닙니다.

하네스는 이 구분을 설명할 때 여러 관련 내부 라벨을 쓸 수 있습니다.
...
| Approval | 민감한 단계에 대한 허가입니다. 작업 수락, 정확성(correctness), 사용자 소유 판단이 아닙니다. |
| 결정 패킷 | 사용자 소유의 제품 판단, 중요한 기술 판단, waiver, 작업 수락, 잔여 위험, reconcile 판단을 기록하는 경로입니다. Approval 형태이며 Approval에 연결된 경우가 아니면 sensitive-action permission이 아닙니다. |
| Acceptance | 작업 경로가 요구할 때 결과를 받아들일 수 있다는 최종 판단입니다. 근거, QA, verification, Approval, 잔여 위험 수용을 대체하지 않습니다. |
| 잔여 위험 수용 | 알려진 잔여 위험을 이번 close에서 받아들일 수 있다는 판단입니다. 위험 없는 일반 close가 아니며 assurance를 높이지 않습니다. |
...
에이전트가 승인을 요청한다면 무엇을 허용하거나 기록하려는지 분명히 이름 붙여야 합니다. 사용자는 민감한 동작을 허용하는 것일 수도 있고, 범위를 확인하는 것일 수도 있으며, 결정 패킷을 해결하거나, 잔여 위험을 받아들이거나, 작업 수락을 하거나, 쓰기 허가 기록 상태를 확인하는 것일 수도 있습니다. "승인"은 모든 판단을 뭉뚱그리는 포괄 라벨이나 포괄 위임으로 쓰면 안 됩니다.
...
- 수동 QA가 필요한지, waiver를 받아들일 수 있는지, 결과를 받아들일 수 있는지 같은 QA / 작업 수락 판단
...
- 결정 제목
- 표시용 판단 유형: 제품 / UX, 기술 아키텍처, 보안 / 개인정보, QA / 작업 수락, 잔여 위험, 범위 / 자율성
- 왜 지금 필요한지
- 사용자가 정확히 결정하는 것
- 선택지
- 장단점
...
- QA / 작업 수락: 더 완성도 높은 상호작용은 레이아웃, 접근성 해석, 사용감을 위해 수동 QA가 필요할 수 있고, 더 보수적인 동작은 검증하기 쉽습니다. 결정 패킷은 장단점과 QA를 미뤄도 계속할 수 있는 일, 또는 결정 전에는 진행하면 안 되는 이유를 보여줘야 합니다.
...
- QA / 작업 수락: QA waiver prompt는 생략하는 check나 surface, 사용자가 받아들일 잔여 위험, 잔여 위험 후속 작업, 관련 refs, close가 잔여 위험을 받아들이고 닫는 흐름이 되는지까지 이름 붙여야 합니다. "진행해"만으로는 충분하지 않습니다.
- 잔여 위험: 잔여 위험을 받아들이고 닫는 흐름은 남은 한계, 이미 있는 evidence, 빠졌거나 면제된 QA/verification, 수용된 잔여 위험, 잔여 위험 후속 작업을 보여줘야 합니다. Detached verified close가 아닙니다.
- QA / 작업 수락과 잔여 위험: 작업 수락은 required일 때 결과를 받아들인다는 뜻이고, 잔여 위험을 받아들이는 판단은 이름 붙은 remaining risk를 close에 대해 받아들인다는 뜻입니다. 에이전트는 evidence, verification, QA, 잔여 위험 표시를 보여준 뒤 이 둘을 따로 요청해야 합니다.
...
닫기 준비 상태는 "이 작업을 끝내거나 닫기 전에 아직 무엇을 확인하거나 처리해야 하는가?"에 답합니다.

이 그룹은 검증, 수동 QA, 작업 수락, 잔여 위험 표시 또는 수용, close blocker처럼 닫기에 직접 닿는 항목을 함께 보여줍니다. 새 gate가 아니라 사용자에게 보이는 요약 층입니다. 정확한 kernel gate 상태가 중요하면 에이전트는 chat에서 값을 새로 정의하지 말고 관련 source ref와 [커널 참조](../reference/kernel.md#close_task)를 가리켜야 합니다.

유용한 닫기 준비 상태 한 줄은 결과가 막혔는지, 작업 수락 요청 준비가 되었는지, close 시도 준비가 되었는지, 특정 확인이나 판단을 기다리는지 말합니다. 가장 작은 해소 방법을 이름 붙이고, 잔여 위험을 받아들이고 닫는 흐름을 일반 self-checked 또는 detached-verified close와 눈에 보이게 구분해야 합니다.

### 닫기 준비 상태 막힘 matrix

이 matrix는 사용자에게 보이는 blocker summary를 빠르게 훑기 위한 보조 도구입니다. 눈여겨볼 점은 각 항목이 기존 owner record와 gate를 가리킨다는 것입니다. 닫기 준비 상태 자체는 새 gate, schema field, authority path가 아닙니다.

```mermaid
flowchart TB
  subgraph Matrix["닫기 준비 상태 막힘 matrix"]
    direction LR
    State["상태와 범위<br/>active Run clear<br/>필요하면 Change Unit 호환<br/>쓰기 범위 통과"]
    Judgment["판단<br/>결정 패킷 해소 또는 호환 가능한 보류<br/>필요한 민감 동작 승인<br/>필요한 작업 수락 처리"]
    Checks["근거와 확인<br/>필요한 근거 충분<br/>검증 통과, 불필요,<br/>또는 잔여 위험 수용과 함께 면제<br/>필요한 수동 QA 통과 또는 면제"]
    Risk["위험과 결과<br/>잔여 위험 visible 또는 none<br/>risk-close이면 accepted<br/>close reason stays honest"]
  end
...
| 먼저 요구를 구체화해줘. 첫 번째 안전한 범위가 분명해질 때까지 필요한 질문을 해줘. 제품 결정과 기술 결정을 분리해줘. 선택지, 추천안, 불확실성을 보여줘. 코드베이스가 답할 수 없는 것만 나에게 물어봐. | Shared Design, 결정 패킷 candidate, Change Unit shaping으로 이어지는 Discovery. |
...
| 작업 수락 전에 잔여 위험을 보여줘. | 잔여 위험과 닫기에 영향을 주는 위험 상태. |
| 작업 수락이 필요하면 닫기 전에 먼저 요청해. | 닫기 전 작업 수락. |
| 여기서는 별도 작업 수락이 필요 없으니, 관련 막힘이 해소되면 닫아. | 이 작업 경로에서는 작업 수락이 필요 없음. |
...
flowchart LR
  ...
  Classify --> Discovery["필요하면 요구 구체화"]
  Classify --> Scope["범위"]
  Discovery --> Brief["구체화 요약"]
  Brief --> Decisions["필요하면 사용자 판단 ref"]
  Brief --> Scope["첫 번째 안전한 범위"]
  Decisions --> Scope
  Scope --> Work["허용된 작업"]
  Work --> Evidence["근거: supporting refs"]
  Evidence --> Readiness["닫기 준비 상태: 검증 / QA / 위험 / 작업 수락"]
...
3. 요청이 모호하거나, feature 성격이 있거나, auth/security에 민감하거나, UX/workflow 판단이 크거나, public-interface에 닿거나, `work`가 될 가능성이 높고 구체화가 필요하면 계획 전에 요구를 구체화합니다. 내부적으로 하네스는 이를 Discovery라고 부르고 Discovery Brief를 만들 수 있습니다.
4. 사용자가 소유하는 제품, 기술, security, QA, 운영, 범위 판단은 판단 그룹 아래에서 기존 결정 경로로 연결합니다. 내부 기록은 결정 패킷일 수 있습니다.
5. 에이전트가 첫 번째 안전한 범위를 제안한 뒤, 제품 파일을 쓸 수 있는 경우 활성 작업 경계를 확인합니다. 내부 기록은 Change Unit일 수 있습니다.
6. 제품 파일을 쓰기 전에 정확한 쓰기가 허용되는지 확인합니다. 내부 결과는 쓰기 허가 기록일 수 있습니다.
7. 변경이나 조언 뒤에는 필요한 결과와 근거를 기록합니다.
8. 필요하면 닫기 준비 상태에서 검증, 수동 QA, 잔여 위험, 작업 수락, close blocker를 처리합니다.
...
```text
tiny direct로 완료했습니다.
범위: `docs/help.md`의 typo 하나를 고쳤고 의미나 계약 변경은 없습니다.
근거: changed path와 self-check, escalation 없음.
```
...
근거: `src/settings/Profile.tsx`를 변경했고 RUN-031과 diff ART-DIFF-031로 확인했습니다. Evidence Manifest EM-031이 이 claim을 뒷받침합니다.
...
판단: 잔여 위험은 DEC-022에서 받아들였고 작업 수락은 DEC-023에 기록됐습니다.
근거: Evidence Manifest EM-009가 RUN-018과 ART-TEST-018을 근거로 AC-01과 AC-02를 뒷받침합니다.
닫기 준비 상태: 검증은 RUN-018에 기록된 자체 확인(self-check)이며, 이 경로에서는 detached Eval이 필요하지 않았습니다. 수동 QA는 MQA-006에서 통과했습니다. 잔여 위험 RISK-004는 mobile Safari 미확인 동작을 다루며 후속 TASK-144가 있습니다. Close reason: 잔여 위험을 받아들이고 완료.
```
...
MCP에 닿지 못하는 상태(MCP unavailable)도 별개입니다. 에이전트가 필요한 하네스/Core 기능에 닿지 못한다면 그 사실을 바로 말해야 하며, 연결이나 기능이 복구되기 전에는 기준 상태 변경, Approval, 작업 수락, 잔여 위험을 받아들이는 판단, gate 갱신, projection 복구, 닫기가 처리됐다고 주장하면 안 됩니다.
...
- MCP에 닿지 못하는 경우: 필요한 하네스/Core capability를 사용할 수 있거나 가능한 접점(surface)으로 옮길 때까지 제품 파일 쓰기와 gate 갱신을 보류하고, Approval, 작업 수락, 잔여 위험을 받아들이는 판단, 닫기가 처리됐다고 주장하지 않습니다.
...
```text
막힘:
- 판단(사용자 소유): 빈 상태 동작이 아직 정해지지 않았습니다.
- 근거: 동작을 고른 뒤 AC-02 근거는 에이전트가 수집할 수 있습니다.
- 닫기 준비 상태: 바뀐 온보딩 문구는 닫기 전 사용자 소유 수동 QA가 필요합니다.
...
| 작업 수락 | Task가 요구할 때 사용자가 결과를 받아들인다는 판단을 기록합니다. | 정확성 증명, QA, 검증, sensitive-action Approval. |
...
| Approval | 이름 붙은 민감한 행동을 진행해도 된다는 허가입니다. | 작업 수락, 정확성, 근거, 검증, QA, 잔여 위험 수용. |

Sensitive-action Approval은 작업 수락이 아닙니다. 테스트를 통과했다고 수동 QA가 끝난 것도 아닙니다. 같은 세션의 자체 검토는 유용한 자체 확인(self-check)일 수 있지만 분리 검증은 아닙니다. 결과를 수락해도 그 자체가 정확성을 증명하지 않습니다. 잔여 위험을 받아들이는 것도 증명이 아니라, 알려진 불확실성이 이 Task에서 보였고 이 Task 안에서 받아들여졌다는 뜻입니다. 작업 수락이 필요한 경우에는 닫기에 영향을 주는 잔여 위험이 표시되었거나 알려진 close-relevant risk가 없다고 보고된 뒤에 따로 요청되어야 합니다.

수동 QA 판단은 Browser QA artifact와 분리됩니다. Screenshot, browser smoke, console log, network trace, accessibility snapshot, workflow recording은 기존 artifact 경로로 등록되면 근거를 뒷받침할 수 있지만, 그 자체가 수동 QA, 작업 수락, 분리 검증이 되지는 않습니다. Browser QA Capture는 owner 문서가 명시적으로 승격하고 증명하기 전까지 v1+ Expansion 후보입니다. 어떤 접점이 browser capture를 지원하지 않으면 사람이 작성한 수동 QA notes와 수동 제공 artifacts를 fallback으로 사용합니다.
...
- 작업 수락(Acceptance)은 해당 Task 경로가 요구할 때 결과를 수락한다는 뜻입니다. 추가 edit를 위한 쓰기 허가 기록이 아닙니다.

에이전트가 QA 또는 검증 면제(verification waiver)를 요청한다면 어떤 기존 기록 경로를 사용할지 이름 붙이고 owner refs를 연결해야 합니다. QA waiver 효과는 수동 QA / QA policy path가 담당하며, product/user risk 또는 policy-required judgment가 있으면 QA waiver 결정 패킷을 참조해야 합니다. Verification waiver 효과는 kernel verification-waiver path가 담당하며, 사용자 소유 판단이 필요하면 관련 결정 패킷과 수용된 잔여 위험 refs를 참조해야 합니다. 그 판단 요청은 무엇을 확인하지 않는지, 사용자가 어떤 잔여 위험을 받아들이는지, 어떤 잔여 위험 후속 작업이 남는지, 어떤 refs가 관련되는지, 닫기에 어떤 영향을 주는지 말해야 합니다. 닫기에 영향을 주는 waiver나 잔여 위험을 받아들이는 판단이 있다면 가벼운 채팅 문장만으로 waiver가 기록된 것처럼 다루면 안 됩니다. 잔여 위험을 안고 닫자고 요청한다면 먼저 남은 한계를 보여준 뒤, 그 잔여 위험을 이 Task에서 받아들일지 물어야 합니다. Verification waiver는 잔여 위험을 받아들이는 close로만 닫을 수 있으며 detached verified로 제시하면 안 됩니다. 정확한 gate effect는 [커널 참조](../reference/kernel.md#waiver-semantics)와 [설계 품질 정책](../reference/design-quality-policies.md#waiver-규칙)이 담당합니다.
...
- 잔여 위험을 받아들이고 닫는 흐름: 에이전트는 존재하는 근거, 빠졌거나 면제된 verification 또는 QA, 남은 한계, 수용된 잔여 위험, 잔여 위험 후속 작업을 보여줘야 합니다. 수용된 잔여 위험으로 닫는 것은 detached verified로 닫는 것과 다릅니다.

## 닫기 체크리스트

닫기 전에 에이전트는 다음을 쉬운 말로 분명히 해야 합니다.

`work` Task의 닫기 요약은 적용된 변경 범위, 근거, 검증, 수동 QA, 잔여 위험, 작업 수락, close reason을 해당되는 만큼 보여줘야 합니다. 어떤 확인이 면제되었거나, 아직 대기 중이거나, 필요하지 않았다면 그대로 말해야 합니다. 닫기에 영향을 주는 잔여 위험을 일반적인 "완료" 문장 속에 묻으면 안 됩니다.
...
- 작업 수락이 필요한 경우 sensitive-action Approval이 아니라 별도 수락으로 요청되었습니다.

닫을 때 쓸 수 있는 말:

```text
닫기 체크리스트를 보여줘.
표시된 잔여 위험을 받아들일게. 잔여 위험을 받아들이고 닫아.
수락해. 이 작업 닫아.
