...
평소처럼 요청하면 됩니다. 원하는 일, 이미 아는 경계, 어느 정도 조심해서 진행하길 원하는지를 말하세요. 구현 전에 계획을 구체화해 달라고 하거나, 제품 판단이나 중요한 기술 구조 판단으로 커지지 않는 한 작게 유지해 달라고 하거나, 제품 판단과 기술 구조 판단을 분리해 달라고 하거나, 부족한 근거와 닫기 막힘을 보여 달라고 요청할 수 있습니다.
...
| 추적되는 작업 | 범위가 불분명하거나, 여러 부분이 있거나, 제품 또는 기술 판단, 보안/개인정보 영향, 의미 있는 근거, QA, 검증, 작업 수락, 닫기 관련 위험이 있을 때. | 범위, 사용자 결정, 근거, 닫기 준비 상태, 다음 안전한 행동, 가장 작은 막힘. |
...
사용자 소유 결정이 필요하면 에이전트는 넓은 "승인할까요?" 대신 결정 유형을 이름 붙여야 합니다. 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, 범위/자율성 판단, 민감 동작 승인, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용은 서로 다른 결정입니다. 작은 결정은 짧고 명확한 질문으로 물을 수 있고, 복잡하거나 위험한 결정은 더 자세한 trade-off와 근거를 포함해야 합니다.
...
좋은 답변은 질문하기 전에 기존 auth, session, user model, test, 문서를 살피고, 기술 구조 선택과 security/privacy 선택을 분리하며, 검증과 수동 QA 기대 수준을 이름 붙이고, 안전한 조사 조각 또는 작업 분할을 제안해야 합니다. 첫 번째 그럴듯한 구현 경로가 보였다는 이유만으로, 수용 기준, 중요한 판단 후보, 남은 불확실성이 아직 흐린데 구체화를 끝내면 안 됩니다.
...
작은 일은 더 이상 작지 않을 때 추적되는 작업으로 커져야 합니다. 범위가 불분명하거나, 여러 파일·제품 영역·subsystem이 관련되거나, 제품/UX 판단이 필요하거나, 중요한 기술 구조 판단이 필요하거나, public interface/API에 영향이 있거나, 보안/개인정보에 영향이 있거나, 민감한 행동이 필요하거나, QA 또는 검증 요구가 커지거나, 근거가 부족하거나, 잔여 위험이 작지 않거나, multi-step delivery가 필요하면 상향합니다.
...
사용자 소유 결정이 필요할 가능성이 큰 것:
- 제품 / UX: credential flow, 로그인 실패 동작, 로그인 문구, 복구 안내.
- 기술 구조 판단: session model, token/cookie 전략, password storage 또는 identity-provider 경로, migration 영향, dependency 선택.
...
아직 닫을 수 없는 이유: 범위, 사용자 결정, 근거, QA 기대 수준, 잔여 위험이 정리되지 않았습니다.
```
...
| 사용자 결정 | 사용자가 무엇을 결정해야 하는가? | 대기 중인 항목을 각각 나눠 보여주고 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, 범위/자율성 판단, 민감 동작 승인, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용 중 무엇인지 label을 붙입니다. |
...
이 네 가지는 읽기용 요약이지 권한 문서가 아닙니다. 에이전트가 뒤에 있는 ref를 보여줄 수는 있지만, 상태 변경, 근거 기록, 작업 수락, 잔여 위험 수용, 닫기는 여전히 Harness/Core 기록과 action에서만 옵니다. 사용자 결정 그룹은 하나의 승인 checklist처럼 보이면 안 됩니다. 민감 동작 승인은 이름 붙은 민감한 단계에 대한 허가이고, 작업 수락은 결과에 대한 판단이며, 잔여 위험 수용은 이름 붙은 남은 위험을 받아들인다는 기록입니다.

사용자 결정 그룹에 정식 결정 패킷(Decision Packet)이 필요하면 하네스는 결정 경로, 결정 profile, 사용자에게 보이는 판단 영역을 함께 기록합니다. 결정 경로는 어떤 lifecycle 또는 gate path를 쓰는지 말합니다. Profile은 prompt가 간단한 판단 기록인지, 더 자세한 trade-off, approval, waiver, 작업 수락, 잔여 위험 수용, reconcile 기록인지 말합니다. 판단 영역은 결정을 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, QA/작업 수락, 잔여 위험, 범위/자율성 판단, 복합 같은 label로 묶어 설명합니다. 표시에서는 그래도 민감 동작 승인, 작업 수락, QA 면제 판단, 검증 면제 판단, 잔여 위험 수용처럼 구체적인 결정 유형을 이름 붙여야 합니다. Profile은 prompt detail을 정하고, 판단 영역은 결정을 설명하며, 영향을 받는 gate나 막힌 행동은 별도로 기록되어 결정의 영향을 정합니다.

```mermaid
flowchart LR
  Need["사용자 결정"] --> Kind["decision_kind"]
  Need --> Domain["judgment_domain"]
  Need --> Profile["decision_profile"]
  Kind --> Prompt["profile에 맞는 prompt"]
  Domain --> Prompt
  Profile --> Prompt
...
사용자 결정: 로그인 실패 UX에 대한 제품/UX 판단이 필요합니다.
...
여러 결정이 대기 중이면 에이전트는 나눠서 보여줘야 합니다.

```text
사용자 결정:
- 제품/UX 판단: 로그인 실패 feedback pattern 선택.
- 민감 동작 승인: auth helper package 설치를 허용할지 거부할지.
- 잔여 위험 수용: 이름 붙은 mobile Safari wrapping 위험을 받아들일지 거부할지.
```
...
사용자 결정: 변경 범위를 넓힐지에 대한 제품/UX 판단이 필요합니다.
...
에이전트가 해결할 수 있는 일을 사용자에게 떠넘기면 안 됩니다. 코드 조사, 상태 갱신, 테스트 재실행, 빠진 근거 수집, 사용자 결정을 바꾸지 않는 범위 축소는 에이전트가 다음에 무엇을 할지 말하고 진행해야 합니다.
...
"go ahead", "proceed", "looks good", "좋아", "진행해" 같은 가벼운 답은 에이전트가 사용자가 정확히 무엇을 결정하는지 이미 이름 붙였을 때만 충분합니다. 제품 장단점, 아키텍처 선택, QA나 검증 면제 판단, 작업 수락, 잔여 위험 수용에는 선택지, 결과, 관련 참조, 그 결정 밖에 남는 것이 보여야 합니다. 그 표현이 여러 대기 중인 결정에 적용될 수 있으면 에이전트는 어떤 결정을 뜻하는지 다시 물어야 합니다.
...
| Decision Packet | 사용자 소유의 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, 범위/자율성 판단, 면제 판단, 작업 수락, 잔여 위험 수용, reconcile 결정을 기록하는 경로입니다. 작은 unblocker에는 간단할 수 있고, 복잡하거나 위험한 선택에는 상세할 수 있습니다. |
| Decision profile | Decision Packet의 prompt 깊이입니다. 간단한 판단 기록, 상세 trade-off, 민감 동작 승인, waiver, 작업 수락, 잔여 위험 수용, reconcile처럼 보일 수 있습니다. |
| 판단 영역 | Decision Packet의 사용자에게 보이는 판단 영역입니다. 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, QA/작업 수락, 잔여 위험, 범위/자율성 판단, 복합 같은 label로 보일 수 있습니다. |
| Approval | 이름 붙은 민감한 행동에 대한 민감 동작 승인입니다. 막연한 동의나 작업 수락이 아닙니다. |
| Write Authorization | 의도한 제품 파일 쓰기가 현재 작업, 범위, 사용자 결정, 허가와 맞는지 한 번 확인한 기록입니다. |
| Evidence Manifest | 완료 주장을 뒷받침하는 근거를 연결한 기록입니다. |
| 읽기용 요약(Projection) | 소유자 기록과 아티팩트 참조에서 파생된 읽기용 요약입니다. 초기에는 full Markdown report가 아니라 compact status text나 card일 수 있습니다. 방향을 잡는 데 유용하지만 그 자체가 권한은 아닙니다. |
