...
"go ahead", "proceed", "looks good", "좋아", "진행해" 같은 가벼운 답은 에이전트가 사용자가 정확히 무엇을 결정하는지 이미 이름 붙였을 때만 충분합니다. 제품 장단점, 아키텍처 선택, QA나 verification 면제 판단, 작업 수락, 잔여 위험 수용에는 선택지, 결과, 관련 참조, 그 판단 밖에 남는 것이 보여야 합니다. 그 표현이 여러 대기 중인 판단에 적용될 수 있으면 에이전트는 어떤 결정을 뜻하는지 다시 물어야 합니다.
