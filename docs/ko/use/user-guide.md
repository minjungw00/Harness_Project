...
사용자 판단이 필요할 때는 상태만으로 충분하지 않습니다. 에이전트는 선택지, 추천안, 불확실성, 영향을 받는 gate나 수용 기준, 결정을 미뤘을 때 계속할 수 있는 일, 관련 source, 근거, 설계 기록 ref를 담은 간결한 판단 요청을 함께 보여줘야 합니다.
...
사용자가 소유하는 제품 판단이나 중요한 기술 판단이 진행을 막고 있으면 에이전트는 Decision Packet을 보여줘야 합니다. 옵션, 장단점, 추천, 불확실성, 미룰 경우의 영향이 있어야 합니다. 또한 영향을 받는 gate나 수용 기준, source refs, evidence refs, 에이전트가 사용자 없이 결정해도 되는 일을 이름 붙여야 합니다. 이를 막연한 "전부 승인할까요?"로 뭉개면 안 됩니다.

좋은 Decision Packet은 허가서가 아니라 판단을 도와주는 자료처럼 느껴져야 합니다. 실제로 골라야 할 선택을 이름 붙이고, 현실적인 경로를 비교하고, 하나를 추천하고, 결정을 미루면 무엇을 안전하게 계속할 수 있는지 또는 결정 전에는 왜 아무것도 계속하면 안 되는지 말해야 합니다. 정확한 public field는 [`harness.request_user_decision`](../reference/mcp-api-and-schemas.md#harnessrequest_user_decision)이 소유하고, 기준 authority는 [Decision Packet](../reference/kernel.md#decision-packet)과 [Decision Gate](../reference/kernel.md#decision-gate)가 소유합니다.
...
- 기술 선택: 의존성 추가에는 sensitive-action Approval과 Decision Packet이 둘 다 관련될 수 있습니다. Install 또는 dependency 파일 edit에 Approval을 부여하는 것은 그 dependency를 architecture 방향으로 선택하는 것과 다릅니다.
...
- 범위/autonomy: copy fix가 account behavior로 넓어지거나 private helper 변경이 public module boundary로 넓어지면 새 surface, 계속 범위 밖에 남는 것, 더 작은 Change Unit으로 계속할 수 있는지 여부를 이름 붙이는 결정이 필요합니다.
- 보안 민감 작업: secret 접근, 권한 변경, 데이터 export에 대한 sensitive-action Approval은 그 민감한 단계를 진행해도 되는지만 답합니다. 어떤 데이터를 export할지, 누가 export할 수 있는지, 무엇을 redaction할지, artifact에서 무엇을 생략할지, 어떤 감사 기록이 필요한지는 따로 결정해야 합니다.
...
- Auth 또는 system-file access에 sensitive-action Approval을 부여하는 것은 session auth, JWT, social login, role design, lockout behavior, user notice를 선택하는 것이 아닙니다.
