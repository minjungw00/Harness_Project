...
Authority claim에는 ref가 붙어야 합니다. "Write allowed"는 쓰기 허가 기록 ref를, "evidence sufficient"는 Evidence Manifest ref를, "detached verified"는 Eval ref를, "수동 QA 통과"는 수동 QA record를, "accepted"는 Acceptance 결정 패킷을, "residual risk handled"는 잔여 위험 refs 또는 명시적인 `ResidualRiskSummary.status=none`을 가리켜야 합니다.

잔여 위험 wording도 정확해야 합니다. `status=none`은 이 requested action에 대해 알려진 close-relevant residual risk가 없다는 뜻입니다. `not_visible`은 알려진 close-relevant risk가 있지만 acceptance 또는 close에 충분한 맥락으로 아직 보이지 않았다는 뜻입니다. `not_visible`은 "위험 없음"이 아니라 먼저 보여줘야 할 대상으로 다룹니다.
...
| Residual-risk acceptance | 알려진 잔여 위험을 이번 close에서 받아들일 수 있다는 판단입니다. 위험 없는 일반 close가 아니며 assurance를 높이지 않습니다. |
...
사용자가 소유하는 제품, 기술, security/privacy, QA/acceptance, residual-risk, scope/autonomy 판단이 진행을 막고 있으면 에이전트는 결정 패킷을 보여줘야 합니다. 이를 막연한 "전부 승인할까요?"로 뭉개면 안 됩니다.
...
- Residual risk: 잔여 위험을 받아들이고 닫는 흐름(residual-risk accepted close)은 남은 한계, 이미 있는 evidence, 빠졌거나 면제된 QA/verification, 수락된 잔여 위험, 잔여 위험 후속 작업을 보여줘야 합니다. Detached verified close가 아닙니다.
...
유용한 Close Readiness 한 줄은 result가 막혔는지, acceptance 요청 준비가 되었는지, close 시도 준비가 되었는지, 특정 check나 judgment를 기다리는지 말합니다. 가장 작은 해소 방법을 이름 붙이고, 잔여 위험을 받아들이고 닫는 흐름(residual-risk accepted close)을 일반 self-checked 또는 detached-verified close와 눈에 보이게 구분해야 합니다.
...
Close Readiness: 검증은 RUN-018에 기록된 자체 확인(self-check)이며, 이 경로에서는 detached Eval이 필요하지 않았습니다. 수동 QA는 MQA-006에서 통과했습니다. 잔여 위험 RISK-004는 mobile Safari 미확인 동작을 다루며 후속 TASK-144가 있습니다. Close reason: completed with accepted residual risk.
```
...
- 알려진 닫기에 영향을 주는 잔여 위험이 표시되었거나, 에이전트가 알려진 닫기 관련 잔여 위험(close-relevant residual risk)이 없다고 보고했습니다.
...
"수락해. 이 작업 닫아."는 residual-risk accepted close를 요청하지 않을 때의 일반 close 문구입니다. 알려진 잔여 위험(Known residual risk)이 close 근거에 포함되어 있다면 residual-risk accepted wording을 사용해야 하며, close reason은 verified 또는 self-checked close와 계속 구분되어 보여야 합니다.
