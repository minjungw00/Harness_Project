...
사용자 소유 결정이 필요하면 에이전트는 넓은 "승인할까요?" 대신 결정 유형을 이름 붙여야 합니다. 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, 범위/자율성 판단, 민감 동작 승인, QA 면제 판단, 검증 면제 판단, 작업 수락, 잔여 위험 수용은 서로 다른 결정입니다.
...
사용자 결정 그룹에 정식 결정 패킷(Decision Packet)이 필요하면 하네스는 결정 경로와 사용자에게 보이는 판단 영역을 함께 기록합니다. 결정 경로는 어떤 lifecycle 또는 gate path를 쓰는지 말합니다. 판단 영역은 결정을 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, QA/작업 수락, 잔여 위험, 범위/자율성 판단, 복합 같은 label로 묶어 설명합니다. 표시에서는 그래도 민감 동작 승인, 작업 수락, QA 면제 판단, 검증 면제 판단, 잔여 위험 수용처럼 구체적인 결정 유형을 이름 붙여야 합니다. 영향을 받는 gate나 막힌 행동은 별도로 기록되며, 그 별도 기록이 결정의 영향을 정합니다.
...
| Decision Packet | 사용자 소유의 제품/UX 판단, 기술 구조 판단, 보안/개인정보 판단, 범위/자율성 판단, 면제 판단, 작업 수락, 잔여 위험 수용, reconcile 결정을 기록하는 경로입니다. |
