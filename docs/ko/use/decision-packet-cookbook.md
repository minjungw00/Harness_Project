...
아래 예시는 전체 schema가 아니라 prompt 예시입니다. 예시는 schema-owned `judgment_domain`을 사용자에게 보이는 판단 영역으로 보여줍니다. `decision_kind`는 lifecycle과 gate 의미를 제어하고, `judgment_domain`은 결정을 사용자에게 어떻게 설명하고 묶어 보여줄지 제어합니다.
...
## UX 결정: inline layer vs toast vs modal
...
Options:
- Form field 근처 inline layer.
- Failed submit 뒤 toast.
- Flow를 끊는 modal.
Recommendation: inline layer를 선택합니다.
...
- 필요한 경우 별도의 local session 또는 token strategy를 두는 OAuth/OIDC identity provider.
- Provider별 account linking과 support 영향을 포함하는 social-login provider integration.
Recommendation: first-party web app이라면 지금 third-party identity provider sign-in, social-login conversion, non-browser client가 필요하지 않은 한 server-side session cookie를 선택합니다.
