...
독자별 읽기 경로 전체를 에이전트에 항상 주입하면 안 됩니다. 연결된 에이전트에 항상 주입되는 맥락은 한 화면 이하로 유지합니다. 현재 Task 요약, 작업 모양, 범위/하지 않을 일, 대기 중인 사용자 판단, 활성 blocker, 다음 안전한 행동, 근거 gap, close blocker, 잔여 위험 요약, guarantee level, source refs/freshness만 기본으로 둡니다.
