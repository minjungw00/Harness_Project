...
현재 문서 세트는 재설계 이후 검토 상태입니다. 이 저장소는 여전히 문서 전용이며, 문서 승인 이후 미래의 하네스 서버 소스 저장소가 되는 것을 목표로 하고, runtime/server 구현은 아직 시작하지 않았습니다. 완전히 수락되었거나, 구현 완료되었거나, 서버 코딩이 승인된 상태가 아닙니다.
...
문서 승인과 구현 계획 상태는 [구현 개요](build/implementation-overview.md#문서-승인-상태)에서 확인합니다. 현재 리비전은 재설계 이후 검토 중인 문서 수락 후보이지 구현 시작 승인이 아닙니다.
...
전체 재설계 범위, 보존 원칙, 문서군별 역할, 유지보수자 검토 점검 목록은 [문서 작성 가이드](maintain/authoring-guide.md#현재-재설계-범위)가 담당합니다. 검토 tracker는 현재 문서에서 확인된 drift, 확인 대상 후보, 회귀 방지 점검, 기준 상태 점검을 구분합니다.
...
4. [서버 코딩 전 필요한 구현 결정](build/mvp-plan.md#서버-코딩-전-필요한-구현-결정): 현재 검토에서 드러나는 주요 결정을 확인합니다. 이 기준에서는 의도적으로 기록된 주요 결정이 없습니다.

이 인계는 문서가 유지보수자 수락 검토 대상 후보로 올라와 있다는 뜻입니다. 문서가 이미 수락되었다거나 server/runtime 구현이 시작되었다는 뜻이 아닙니다.
