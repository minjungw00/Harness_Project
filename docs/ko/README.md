...
| 문서군 | 역할 |
|---|---|
| Learn | 하네스가 왜 필요한지와 엄격한 계약 전에 필요한 개념. |
| Use | 하네스 기준으로 작업할 때 사용자와 에이전트가 상호작용하는 법. |
| Build | 향후 구현 순서, 단계 경계, 유지보수자 인계. |
| Reference | 정확한 owner 계약: schema, DDL, gate, 상태 전이, projection 규칙, 보안 의미, conformance 의미, template, 용어. |
| Maintain | 문서 규칙, 재설계 범위, 의미 일치 기대, drift 처리. |

## 학습 문서

정확한 계약에 들어가기 전에 전체 그림을 잡을 때 사용합니다.

| 문서 | 고유 역할 |
|---|---|
| [개요](learn/overview.md) | 가장 먼저 읽는 문서입니다. 제품 명제, 세 공간, 하네스가 기록하는 것, 하네스가 아닌 것을 설명합니다. |
| [목적과 원칙](learn/purpose-and-principles.md) | 가치, 비목표, 실패 모델, MVP 경계입니다. 문구나 범위가 제품 명제와 맞는지 확인할 때 사용합니다. |
| [핵심 개념](learn/concepts.md) | 평소 말에서 구현 용어로 넘어가는 어휘 다리입니다. 또 하나의 개요나 튜토리얼이 아닙니다. |
| [15분 만에 보는 하네스](learn/harness-in-15-minutes.md) | 흔히 만나는 하네스 순간을 짧게 보여 주는 시나리오 모음입니다. |
| [하나의 작업으로 보는 하네스](learn/harness-in-one-task.md) | 작은 변경 하나와 추적되는 작업 하나를 따라가는 더 긴 튜토리얼입니다. |
...
- [결정 패킷 예시 모음](use/decision-packet-cookbook.md)은 고급 사용 예시이자 Reference 인접 결정 예시입니다.
...
| Trust boundary, asset, threat category, control, guarantee-level wording | [보안 위협 모델 참조](reference/security-threat-model.md) |
...
| 구현 결정 상태 | 서버/런타임 구현 결정은 코드 작성용으로 공식 수락되지 않았습니다. 현재 기준에서 결정 기록에는 확인된 항목이 없지만, 검토 중 열린 설계 쟁점이 발견될 수 있습니다. |
...
이 인계는 문서가 maintainer 수락 검토 대상 후보로 올라와 있고 남은 결정과 drift 상태가 한곳에 모였다는 뜻입니다. 서버/런타임 구현 결정은 코드 작성용으로 공식 수락되지 않았으며, 열린 설계 쟁점이 아직 있을 수 있습니다. 문서가 이미 수락되었거나, 구현 준비가 끝났거나, 서버/런타임 구현이 시작되었다는 뜻이 아닙니다.
