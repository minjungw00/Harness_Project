...
문서 검토 상태, 구현 계획 준비 상태, 런타임 구현 상태는 [구현 개요](build/implementation-overview.md#문서-수락-상태)에서 확인합니다. 현재 리비전은 재설계 이후 검토 중인 문서 수락 후보이지 구현 시작 허가가 아닙니다.
...
2. [문서 수락 상태](build/implementation-overview.md#문서-수락-상태): 세 상태 모델과 유지보수자가 첫 런타임 배치 계획을 수락했는지 확인합니다.
...
구현 방향을 파악하고 계획을 리뷰하기 위한 경로입니다. 먼저 [문서 수락 상태](build/implementation-overview.md#문서-수락-상태)를 확인합니다. 유지보수자가 그곳에서 구현 계획을 명시적으로 수락하기 전까지 Build 문서는 계획 지침이며 하네스 서버/런타임 구현을 허가하지 않습니다.
