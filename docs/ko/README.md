...
3. 향후 하네스 서버 구현을 검토할 때만 [MVP 계획](build/mvp-plan.md)를 봅니다.
...
권장 경로는 [MVP 계획](build/mvp-plan.md)을 먼저 읽고, 정확한 계약 담당 문서가 필요할 때 [Reference 색인](reference/README.md)을 사용하는 것입니다.
...
현재 인계 상태, 문서 수락 경계, 서버 코딩 전 결정은 [MVP 계획](build/mvp-plan.md)이 담당합니다.
