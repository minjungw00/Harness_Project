...
| 관리 CLI 바이너리 테스트 | `harness-cli` 패키지의 `binary_admin` 대상인 [`crates/harness-cli/tests/binary_admin.rs`](../../../crates/harness-cli/tests/binary_admin.rs). | `harness` 바이너리, Runtime Home 설정 명령, `harness agent` install/status/verify/uninstall/guidance 동작, 로컬 MCP 호환 설정, dry-run, 호스트 설정 쓰기, 저장소 guidance 안전성, 사전 점검 실패 처리, 명령줄 오류 경로. | 공개 API 메서드 동작. |
...
| 관리 에이전트 설정 동작 | `binary_admin`과 `agent_command.rs`, 호스트 어댑터, 저장소 guidance, 호환 설정의 CLI 모듈 테스트. | 부트스트랩, 검사, registry, 마이그레이션 동작이 바뀌면 Store 테스트를 추가합니다. |
