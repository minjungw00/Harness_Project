...
| 관리 CLI 바이너리 테스트 | `volicord-cli` 패키지의 `binary_admin` 대상인 [`crates/volicord-cli/tests/binary_admin.rs`](../../../crates/volicord-cli/tests/binary_admin.rs). | `volicord` 바이너리, Runtime Home 설정 명령, `volicord agent` connect/status/verify/project membership/uninstall 동작, 지원되지 않는 설정 명령 거부, zero-write dry-run, 호스트 상태 검증, 연결 프로젝트 멤버십 생명주기, 보상과 잔류 효과 보고, 호스트 설정 쓰기, 저장소 지침 안전성, 사전 점검 실패 처리, 명령줄 오류 경로. | 공개 API 메서드 동작. |
