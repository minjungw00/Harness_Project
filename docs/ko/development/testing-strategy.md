...
사용합니다. 변경 작업 흐름은 [구현 가이드](change-guide.md)를 사용합니다. 문서 명령
예시 검증, 용어 역할 검증, 한영 링크 일치, 검증 보고 경계는
[검증](../maintain/validation.md) 정책을 사용합니다.
...
| 관리 CLI 바이너리 테스트 | `volicord-cli` 패키지의 `binary_admin` 대상인 [`crates/volicord-cli/tests/binary_admin.rs`](../../../crates/volicord-cli/tests/binary_admin.rs). | `volicord` 바이너리, Runtime Home setup 명령, 프로젝트 감지, `volicord connect`, `volicord connections`, `volicord connection status/verify/mode/remove`, `volicord export mcp-config`, `volicord user ...`, zero-write dry-run, 호스트 상태 검증, 연결 프로젝트 멤버십 생명주기, 잔류 효과 보고, 호스트 설정 쓰기, 사전 점검 실패 처리, 명령줄 오류 경로. | 공개 API 메서드 동작. |
...
| MCP 어댑터 시작, 도구 스키마, `tools/call`, stdio 전송 | `crates/volicord-mcp/src/lib.rs` 테스트와 `mcp_transport`. | MCP를 통과한 Core/Store 계층 간 동작은 `tests/integration/mcp_connection.rs`를 추가합니다. |
