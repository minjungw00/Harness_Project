...
- 정책과 권한 부여에 민감한 경로인 `volicord.prepare_write`
...
sequenceDiagram
  ...
  MCP->>MCP: prepare_integration_arguments가 프로젝트를 선택하고 envelope 채움
...
- `prepare_typed_request<T>`는 호출자가 제출한 권한 필드가 아니라 어댑터가
  통제하는 도구/메서드 매핑에서 메서드 수준 접근 등급을 파생한 뒤 준비된
  인자를 `decode_params<T>`로 디코딩합니다.
- `prepare_integration_arguments`는 요청 래퍼를 `McpIntegrationContext`와
  비교하고, 호출에 허용되는 프로젝트 하나를 선택하고, 타입 지정 디코딩 전에
  신뢰된 요청 래퍼 필드를 채웁니다.
- 프로젝트 선택은 선택된 프로젝트, 묶인 접점, 접점 인스턴스, 요청 접근 등급,
  어댑터 바인딩 근거를 담은 `McpDerivedInvocationContext`를 만듭니다.
- `McpDerivedInvocationContext::core_invocation`은 Core `InvocationContext`를
  만듭니다.

시작과 세션 검증도 `harness-mcp`에 있으며, 특히
`McpIntegrationStartupInspection::resolve`가 핵심입니다. 이 시작 경로는
Runtime Home, Agent Integration Profile 상태, 접점과 접점 인스턴스 바인딩,
역할, 멤버십, 메타데이터, 로컬 레지스트리 JSON 검사를 위해 Store를 직접
읽습니다. 모든 호출에 쓸 프로젝트 하나를 시작 시점에 선택하지 않으며, 공개
메서드 동작을 구현하는 다른 경로도 아닙니다. 공개 메서드 실행은
`harness-core`를 통과합니다.
...
4. `prepare_typed_request`는 status 접근 등급을 파생하고,
   `McpIntegrationContext`에서 허용된 프로젝트를 선택하고, 신뢰된 요청 래퍼
...
## `volicord.prepare_write`: 정책과 권한 부여 경로
