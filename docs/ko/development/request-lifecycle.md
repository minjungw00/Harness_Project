...
- 읽기 전용 경로인 `harness.status`
- 커밋된 상태 변이 경로인 `harness.intake`
- 정책과 권한 부여에 민감한 경로인 `harness.prepare_write`
...
sequenceDiagram
  ...
  MCP->>MCP: McpAdapter::call_tool이 타입 지정 요청 디코딩
  MCP->>MCP: typed_invocation이 InvocationContext 파생
  ...
  MCP-->>Host: Harness JSON을 담은 tools/call content text
```

공유 어댑터 경로는
[`crates/harness-mcp/src/lib.rs`](../../../crates/harness-mcp/src/lib.rs)에
있습니다.
...
- `McpAdapter::call_tool`은 도구 이름을 match하고, `decode_params<T>`를
  호출한 뒤 해당 `CoreService` 메서드로 디스패치합니다.
- `typed_invocation`은 `HasEnvelope`와 `MethodAccessClass`를 함께 사용해
  호출자가 제출한 권한 필드가 아니라 타입 지정 요청에서 메서드 수준 접근
  등급을 파생합니다.
- `McpAdapter::derive_invocation_context`는 요청 래퍼를
  `McpSessionContext`와 비교하고 `McpDerivedInvocationContext`를 만듭니다.
- `McpDerivedInvocationContext::core_invocation`은 Core `InvocationContext`를
  만듭니다.

시작과 세션 검증도 `harness-mcp`에 있으며, 특히
`McpStartupInspection::resolve`가 핵심입니다. 이 시작 경로는 Runtime Home,
프로젝트, 접점, 접점 인스턴스, 역할, 역량 프로필, 메타데이터, 로컬 접근
검사를 위해 Store를 직접 읽습니다. 하지만 공개 메서드 동작을 구현하는
다른 경로가 아니며, 공개 메서드 실행은 `harness-core`를 통과합니다.

공유 Core 경로는 주로
[`crates/harness-core/src/pipeline.rs`](../../../crates/harness-core/src/pipeline.rs)와
[`crates/harness-core/src/methods/mod.rs`](../../../crates/harness-core/src/methods/mod.rs)에
있습니다.

- 메서드 파일은 `prepare_or_response`를 호출하고, 이 helper는
  `CoreService::prepare_request`로 위임합니다.
- `MethodPolicy`는 필요한 `AccessClass`, `TaskRequirement`,
...
- `CoreService::prepare_request`는 요청 래퍼를 검증하고, 어댑터 바인딩
  ...
  `project_state`를 읽고, `VerifiedSurfaceContext`를 파생하고, 재실행 사전
...
Store 커밋 경로는
[`crates/harness-store/src/core_pipeline.rs`](../../../crates/harness-store/src/core_pipeline.rs)에
있습니다.
...
들어 `harness.prepare_write`는 커밋 전 거부되어 효과가 없을 수 있고,
dry-run 미리보기로 효과가 없을 수 있고, `Write Authorization`을 만들지
않는 non-allow 결정 이벤트를 커밋할 수 있으며, 허용 결정에서는
`Write Authorization`을 삽입할 수 있습니다.

## `harness.status`: 읽기 전용 경로
...
1. [`crates/harness-types/src/methods.rs`](../../../crates/harness-types/src/methods.rs)는
   `StatusRequest`, `StatusInclude`, `StatusResult`, 그리고
   `AccessClass::ReadStatus`를 반환하는 `MethodAccessClass` 구현을 정의합니다.
2. [`crates/harness-mcp/src/lib.rs`](../../../crates/harness-mcp/src/lib.rs)는
   `McpAdapter::call_tool`에서 `"harness.status"`를 라우팅하고,
   `StatusRequest`를 디코딩하고, `InvocationContext`를 파생하고,
   `CoreService::status`를 호출합니다.
3. [`crates/harness-core/src/methods/status.rs`](../../../crates/harness-core/src/methods/status.rs)는
   `CoreService::status`, `status_task`, `status_result_fields`를 구현합니다.
4. [`crates/harness-core/src/pipeline.rs`](../../../crates/harness-core/src/pipeline.rs)는
   공통 사전 점검과 `OwnerPipelineBranch::ReadOnly` 응답 경로를 실행합니다.
5. [`crates/harness-store/src/core_pipeline.rs`](../../../crates/harness-store/src/core_pipeline.rs)는
   `project_state`, Task 읽기, Change Unit 읽기, 쓰기 권한 읽기, 증거 읽기,
   닫기 준비 상태 입력 읽기 같은 `CoreProjectStore` 읽기를 제공합니다.

생명주기:

1. MCP 호스트가 `name="harness.status"`로 `tools/call`을 보냅니다.
2. `call_tool_result`가 도구 이름과 인자를 추출합니다.
3. `McpAdapter::call_tool`이 인자를 `StatusRequest`로 디코딩합니다.
4. `typed_invocation`은 `StatusRequest::requested_access_class`와 요청 래퍼를
   사용해 고정 `McpSessionContext`에서 `InvocationContext`를 파생합니다.
...
1. [`crates/harness-types/src/methods.rs`](../../../crates/harness-types/src/methods.rs)는
   ...
   `AccessClass::CoreMutation`을 반환하는 `MethodAccessClass` 구현을 정의합니다.
2. [`crates/harness-mcp/src/lib.rs`](../../../crates/harness-mcp/src/lib.rs)는
   `McpAdapter::call_tool`에서 `"harness.intake"`를 라우팅하고,
   `IntakeRequest`를 디코딩하고, `InvocationContext`를 파생하고,
   `CoreService::intake`를 호출합니다.
3. [`crates/harness-core/src/methods/intake.rs`](../../../crates/harness-core/src/methods/intake.rs)는
   `CoreService::intake`와 `plan_intake`를 구현합니다.
4. [`crates/harness-core/src/methods/mod.rs`](../../../crates/harness-core/src/methods/mod.rs)는
   `mutation_method_policy`, `prepare_or_response`, 공통 메서드 계획 도우미,
   응답 도우미를 제공합니다.
5. [`crates/harness-core/src/pipeline.rs`](../../../crates/harness-core/src/pipeline.rs)는
   `OwnerPipelineBranch::DryRunPreview` 또는
   `OwnerPipelineBranch::CommitMutation`을 실행합니다.
6. [`crates/harness-store/src/core_pipeline.rs`](../../../crates/harness-store/src/core_pipeline.rs)는
   `CoreStorageMutation` 값을 적용하고 이벤트와 재실행 행을 커밋합니다.

생명주기:

1. MCP 호스트가 `name="harness.intake"`로 `tools/call`을 보냅니다.
...
## `harness.prepare_write`: 정책과 권한 부여 경로
...
1. [`crates/harness-types/src/methods.rs`](../../../crates/harness-types/src/methods.rs)는
   ...
   `AccessClass::WriteAuthorization`을 반환하는 `MethodAccessClass` 구현을
   정의합니다.
2. [`crates/harness-mcp/src/lib.rs`](../../../crates/harness-mcp/src/lib.rs)는
   `McpAdapter::call_tool`에서 `"harness.prepare_write"`를 라우팅하고,
   `PrepareWriteRequest`를 디코딩하고, `InvocationContext`를 파생하고,
   `CoreService::prepare_write`를 호출합니다.
3. [`crates/harness-core/src/methods/prepare_write.rs`](../../../crates/harness-core/src/methods/prepare_write.rs)는
   `CoreService::prepare_write`, `prepare_write_policy`,
   `plan_prepare_write`를 구현합니다.
4. [`crates/harness-core/src/policy/write_authorization.rs`](../../../crates/harness-core/src/policy/write_authorization.rs)는
   `prepare_write_decision`, `prepare_write_dry_run_summary`,
   `surface_supports_prepare_write`, `write_authorization_expires_at`,
   `write_authorization_is_expired`, `write_decision_reason`을 제공합니다.
5. [`crates/harness-core/src/policy/path.rs`](../../../crates/harness-core/src/policy/path.rs)는
   `Product Repository` 경로 정규화 도우미를 제공합니다.
6. [`crates/harness-core/src/policy/judgment_relevance.rs`](../../../crates/harness-core/src/policy/judgment_relevance.rs)는
   계획기가 사용하는 판단 관련성 점검을 제공합니다.
7. [`crates/harness-store/src/core_pipeline.rs`](../../../crates/harness-store/src/core_pipeline.rs)는
   커밋된 allowed 분기가 권한 부여를 만들 때
   `CoreStorageMutation::InsertWriteAuthorization`을 적용합니다.

생명주기:

1. MCP 호스트가 `name="harness.prepare_write"`로 `tools/call`을 보냅니다.
...
6. `plan_prepare_write`는 `intended_operation`, `sensitive_categories`,
   ...
   사용자 소유 판단, 민감 동작 승인, 검증된 접근 등급, 접점 역량을 비교합니다.
...
- 사전 점검 또는 초기 검증 거부는 Core 커밋이 없고 `Write Authorization`을
  만들지 않습니다.
- Dry-run은 `ToolDryRunResponse`를 반환하고, Core 커밋이 없으며, durable
  `Write Authorization` ID를 할당하지 않습니다.
- 커밋된 non-allow 결정은 감사/결과 이벤트를 커밋하지만 소비 가능한
  `Write Authorization`을 만들지 않습니다.
- 커밋된 allowed 결정은 이벤트와
  `CoreStorageMutation::InsertWriteAuthorization`을 커밋합니다.
- Idempotent replay는 다른 권한 부여를 만들지 않고 재실행 처리에서 저장된
  원래 응답을 반환합니다.
...
- [`crates/harness-core/src/methods/tests.rs`](../../../crates/harness-core/src/methods/tests.rs)의
  `prepare_write_blocked_path_creates_no_authorization`
- [`crates/harness-core/src/methods/tests.rs`](../../../crates/harness-core/src/methods/tests.rs)의
  `prepare_write_dry_run_has_no_authorization_effect`
- [`crates/harness-core/src/methods/tests.rs`](../../../crates/harness-core/src/methods/tests.rs)의
  `prepare_write_unregistered_grant_fails_before_method_decision`
- [`tests/integration/mcp_surface.rs`](../../../tests/integration/mcp_surface.rs)의
...
- 메서드 동작과 결정 분기:
  [쓰기 준비 메서드 담당 문서](../reference/api/method-prepare-write.md)
- `Write Authorization`, 쓰기 승인, 민감 동작 승인, 최종 수락, 잔여 위험
