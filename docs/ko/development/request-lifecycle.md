...
그림 역할: 런타임 순서도입니다. 소스 코드 학습자의 "공개 MCP `tools/call`이 Core가
Volicord 응답을 반환하기 전 어떤 호출 순서를 따르는가?"라는 질문에 답합니다. 순서도
화살표는 공유 경로의 대표 구현 순서와 반환 흐름을 보여 주며, 온보딩 단계, 정확한
공개 메서드 계약, 저장 효과 정의가 아닙니다. 구현 정확성은 아래에서 설명하는
`volicord-mcp`, `volicord-core`, 메서드 모듈, `volicord-store` 코드 영역이 담당하고,
제품 동작의 정확성은 연결된 참조 담당 문서에 남습니다.
