...
| MCP 어댑터 동작 | `crates/volicord-mcp/src/lib.rs`, `crates/volicord-mcp/src/main.rs` | [MCP 전송](../reference/mcp-transport.md), 검증된 연결 맥락은 [Agent Connection](../reference/agent-connection.md), 공개 도구 집합은 [API 메서드](../reference/api/methods.md) | `crates/volicord-mcp/src/lib.rs` 테스트, `binary_transport`, `tests/integration/mcp_connection.rs` | [요청 생명주기](request-lifecycle.md), [아키텍처 결정](decisions/README.md), [테스트 전략](testing-strategy.md) |
| 관리 에이전트 설정 동작 | `crates/volicord-cli/src/agent_command.rs`, `host_integration/`, `registration.rs` | [관리 CLI](../reference/admin-cli.md), 인접 관심사는 [Agent Connection](../reference/agent-connection.md), [런타임 경계](../reference/runtime-boundaries.md), [MCP 전송](../reference/mcp-transport.md) | `binary_admin`, 부트스트랩/검사/registry/마이그레이션 동작은 Store 설정 테스트 | [구현 아키텍처](architecture.md), [Runtime Home과 Product Repository 분리](decisions/runtime-home-and-product-repository.md) |
