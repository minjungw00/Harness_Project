...
| 관리 설정 동작 | `crates/harness-cli/src/local_mcp_command.rs`, `setup.rs`, `wizard.rs`, `host_config.rs`, `registration.rs` | [관리 CLI](../reference/admin-cli.md), 인접 관심사는 [런타임 경계](../reference/runtime-boundaries.md)와 [MCP 전송](../reference/mcp-transport.md) | `binary_admin`, 부트스트랩/검사/마이그레이션 동작은 Store 설정 테스트 | [구현 아키텍처](architecture.md), [Runtime Home과 Product Repository 분리](decisions/runtime-home-and-product-repository.md) |
