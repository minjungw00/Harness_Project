...
Volicord는 Codex, Claude Code 같은 호스트와 직접 코딩 에이전트 통합을
제공하면서도 등록된 Product Repository가 둘 이상인 사용 방식을 지원해야
합니다. 통합에 묶인 MCP 시작 모델은 서버 프로세스를 Product Repository 하나가
아니라 Agent Integration Profile 하나에 묶습니다. 이 형태는 사용자 범위 호스트
설정, 명시적 다중 프로젝트 허용 목록, 호스트별 신뢰와 승인 흐름을 지원합니다.

MCP 클라이언트는 roots나 시작 디렉터리 맥락을 제공할 수 있습니다. 하지만
그 값은 호스트 힌트일 뿐이며 Volicord 권한이 아니므로 그 자체로 프로젝트를
선택할 수 없습니다.

## 결정

Volicord는 Agent Integration Profile을 코딩 에이전트 통합 하나의 지속되는
...
- 레지스트리는 통합 식별 정보, 묶인 코딩 에이전트 접점 식별 정보, 명시적
  ...
  인벤토리를 저장합니다.
- `volicord-mcp`는 시작 때 통합을 검증하고, 프로필에서 묶인 접점 맥락을
  파생하며, 공개 Volicord 도구와 `volicord.list_projects` 도우미를 노출하고,
  모호한 프로젝트 선택을 거절합니다.
...
- 호스트 setup 상태는 설정은 되었지만 호스트 동작을 기다리는 상태와 완전한
  검증 완료를 구분할 수 있습니다.
- 생성되는 호스트 설정은 `volicord-mcp --integration <integration_id>`를
  사용하며 프로젝트, 접점, 접점 인스턴스 환경 변수를 요구하지 않습니다.

## 비목표

- 이 결정은 공개 Volicord API 메서드를 추가하지 않습니다.
- CLI 명령을 공개 API 메서드로 만들지 않습니다.
- MCP roots, 현재 작업 디렉터리, 호스트 라벨을 Volicord 권한으로 만들지
  않습니다.
...
- [`crates/volicord-mcp`](../../../../crates/volicord-mcp): 통합에 묶인 시작,
  MCP 초기화, 도구 발견, 프로젝트 선택, Core 호출 전 어댑터 검증.
- [`crates/volicord-cli`](../../../../crates/volicord-cli): 관리 install/status/
  verify/uninstall 흐름과 저장소 안내 관리.
- [`crates/volicord-store`](../../../../crates/volicord-store): 레지스트리
...
상태, 저장소 쓰기 승인, 관리 marker 교체, 지원되지 않는 시작 형식 거부를
다뤄야 합니다.
