...
이 체크아웃은 Volicord 소스 저장소입니다. Volicord를 위한 유지되는 서버 구현 집합이며, 구현 크레이트, `volicord` 관리 CLI, `volicord-mcp` 로컬 MCP 어댑터, 테스트, 문서, 검증 도구, 저장소 설정을 담습니다. Volicord 설치는 배포된 실행 파일과 필요한 런타임 리소스의 부분집합이므로, 이 소스 지도는 설치 매니페스트처럼 읽으면 안 됩니다.
