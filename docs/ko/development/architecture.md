...
| `crates/volicord-cli/tests/binary_admin.rs` | `volicord` 바이너리로 관리 초기화, 등록, `volicord agent` connect/status/verify/project membership/uninstall 동작, dry-run 동작, 호스트 통합 사전 점검 처리, 호스트 설정 쓰기, 저장소 쓰기 게이트, 명령줄 오류 경로를 실행합니다. |
