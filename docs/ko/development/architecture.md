...
| `crates/volicord-cli` | `volicord-cli` | 라이브러리와 `volicord` 바이너리 | Runtime Home 설정, 프로젝트 등록, User Channel 명령, Agent Connection 설정, 호스트 어댑터, 저장소 지침을 위한 로컬 관리 CLI. |
...
| `crates/volicord-cli/tests/binary_admin.rs` | `volicord` 바이너리로 관리 초기화, 등록, `volicord agent` connect/status/verify/project membership/uninstall 동작, dry-run 동작, 호스트 통합 사전 점검 처리, 호스트 설정 쓰기, 저장소 지침 안전성, 명령줄 오류 경로를 실행합니다. |
