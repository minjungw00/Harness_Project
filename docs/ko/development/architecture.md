...
| `crates/volicord-cli` | `volicord-cli` | 라이브러리와 `volicord` 바이너리 | Runtime Home 설정, 프로젝트 등록, User Channel 명령, Agent Connection 설치, 호스트 어댑터, 저장소 지침을 위한 로컬 관리 CLI. |
...
| `tests/integration` | `volicord-integration-tests` | `mcp_surface` 테스트 대상 | MCP, Core, Store, Agent Connection 바인딩, 작업 범주를 가로지르는 검증. |
...
| `tests/integration/mcp_surface.rs` | MCP 연결 바인딩, 도구 스키마, 공개 메서드 노출, 메서드별 `operation_category` 파생, Core/MCP 일치, 세션 거부 사례, 재실행 맥락 바인딩, 계층 간 저장 효과를 검증합니다. |
