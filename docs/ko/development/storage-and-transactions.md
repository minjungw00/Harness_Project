...
`Harness Runtime Home`은 하네스 소유 기록과 아티팩트 데이터를 위한 로컬
런타임 데이터 위치입니다. `Product Repository`는 사용자의 제품 파일 작업
공간입니다. 구현은 이 위치를 분리합니다.

- Runtime Home 경로 처리는
  [`crates/harness-store/src/runtime_home.rs`](../../../crates/harness-store/src/runtime_home.rs)에
  있습니다.
- registry와 프로젝트 부트스트랩은
  [`crates/harness-store/src/bootstrap.rs`](../../../crates/harness-store/src/bootstrap.rs)에
  있습니다.
- SQLite 열기, 검증, 트랜잭션 도우미는
  [`crates/harness-store/src/sqlite.rs`](../../../crates/harness-store/src/sqlite.rs)에
  있습니다.
- 기준 마이그레이션 적용은
  [`crates/harness-store/src/migrations.rs`](../../../crates/harness-store/src/migrations.rs)에
  있습니다.
...
- 아티팩트 스테이징과 영구 아티팩트 본문 검증은
  [`crates/harness-store/src/artifacts.rs`](../../../crates/harness-store/src/artifacts.rs)에
  있습니다.
...
1. `harness-cli`는
   [`crates/harness-cli/src/agent_command.rs`](../../../crates/harness-cli/src/agent_command.rs)와
   [`crates/harness-cli/src/registration.rs`](../../../crates/harness-cli/src/registration.rs)의
...
1. [`crates/harness-core/src/pipeline.rs`](../../../crates/harness-core/src/pipeline.rs)의
   공유 Core 사전 점검이 요청 래퍼, 어댑터 바인딩, 커밋 효과 요청 래퍼
   요구사항, 요청 해시, 프로젝트 상태, 검증된 접점 맥락, 재실행 가능성,
   Task 요구사항, 최신성, 접근 등급을 검증합니다.
2. [`crates/harness-core/src/methods/`](../../../crates/harness-core/src/methods/)의
...
[`crates/harness-types/src/canonical.rs`](../../../crates/harness-types/src/canonical.rs)의
...
- 직접 Product Repository 파일 쓰기는 공개 Harness API 경로 밖에 있습니다.
