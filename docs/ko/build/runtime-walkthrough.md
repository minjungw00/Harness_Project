...
Evidence는 completion claim 또는 acceptance criteria를 supporting owner records와 등록된 아티팩트 참조에 매핑합니다. Raw artifact는 durable evidence bytes를 보관하고, artifact record와 아티팩트 참조는 identity, integrity, redaction, retention, owner relation을 담습니다.
...
Projector는 state record, event, 아티팩트 참조에서 readable Markdown과 card를 렌더링합니다. 읽기용 투영 문서의 최신성(projection freshness)은 readable view가 current인지 판단하는 데 도움을 주지만, Markdown이 state 또는 evidence authority가 되지는 않습니다.
