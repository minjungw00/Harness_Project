...
- sensitive category를 위한 sensitive-action Approval
...
- sensitive category를 위한 sensitive-action Approval 요청과 결정 흐름
- 최소 changed-path, scope, sensitive-action Approval, baseline, decision, autonomy, 능력 확인
...
이 단계에서는 원천 기록이 존재할 때 API-owned MVP-required `ProjectionKind` value를 위한 MVP-required pre-verification renderer를 만듭니다. `ProjectionKind` value와 API-owned MVP tiering은 [MCP API와 스키마](../reference/mcp-api-and-schemas.md#shared-schemas)가 담당합니다. [문서 Projection 참조](../reference/document-projection.md#template-tiers)는 projection authority boundary, source-record rule, freshness rule, template tier presentation을 담당하고, [Template 참조](../reference/templates/README.md)는 rendered template body와 display card를 담당합니다. Eval renderer는 MVP-required로 남지만 Eval 원천 기록이 생기는 MVP-4에서 실행 가능한 렌더링 경로가 완료됩니다.
...
| MVP-2 | active Change Unit 하나, `prepare_write` allow/block, durable Write Authorization 생성, artifact registration basics가 필요합니다. | Hardened conformance에 필요한 더 넓은 sensitive-action Approval, baseline, autonomy, sensitive-category, drift handling을 추가합니다. |
...
- Sensitive change는 sensitive-action Approval을 요구한다.
...
- Approval drift는 sensitive-action Approval을 차단하거나 만료시킬 수 있다.
...
| MVP-2 | `prepare_write`가 missing scope, out-of-scope path, sensitive-action Approval 필요성, stale baseline, unresolved decision, compatible Write Authorization 생성을 설명할 수 있습니다. |
