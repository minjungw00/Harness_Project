...
| v0.3 | Agency Pack | User judgment, Manual QA, 분리 검증(detached verification), 잔여 위험, stewardship, TDD, feedback policy에 대한 Agency-Hardened MVP behavior를 다룹니다. |
...
v0.1은 분리 검증(detached verification) 독립성 전체, Manual QA policy matrix, 잔여 위험을 받아들인 close semantics, stewardship validators, TDD trace, feedback loop policy, release handoff, full export/recover behavior, large fixture suite를 증명하면 안 됩니다. 이들은 later pack 작업입니다.
...
- acceptance와 close 전 잔여 위험 표시
- 잔여 위험을 받아들인 close full semantics
- stewardship validators와 codebase stewardship coverage
- policy가 요구하는 TDD trace behavior
- policy가 요구하는 feedback loop policy
- distinct Approval, Manual QA, verification-waiver, acceptance, 잔여 위험을 받아들이는 판단
...
- state snapshot, report projection snapshot, 아티팩트 참조, redaction status, omitted-secret notes, retained/expired/unavailable artifact status에 대한 export behavior
...
- Sensitive-action Approval이 Decision Packet, Write Authorization, Manual QA, verification, acceptance, 잔여 위험을 받아들이는 판단을 대체하지 않는다.
...
- 닫기 관련 잔여 위험이 successful acceptance 또는 close 전에 보인다.
...
- Agency conformance가 Journey visibility, user judgment, Autonomy Boundary respect, distinct user judgments, 잔여 위험 표시를 증명한다.
...
- Export는 state snapshot, report projection snapshot, 아티팩트 참조, redaction status, omitted-secret notes, retained/expired/unavailable artifact status를 포함한다.
...
| v0.3 Agency Pack | Decision quality, Approval separation, Manual QA, 분리 검증, 잔여 위험, stewardship, TDD, feedback, acceptance, close behavior가 work를 진행하거나 닫을 수 있는지 설명합니다. |
