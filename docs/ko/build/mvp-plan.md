...
첫 조각은 의도적으로 좁게 유지합니다. 로컬 프로젝트 하나, Task 하나, 기본 범위 하나, 쓰기 권한 경로 하나, 기록된 Run 하나, 근거 링크 하나, 구조화된 막힘/상태 응답 하나를 증명합니다. 이것은 MVP가 아닙니다. 일반적인 작업을 범위, 판단, 근거, 닫기 준비 상태, 잔여 위험 언어로 바꾸고 민감 동작 승인(Approval), 작업 수락, 잔여 위험 수용을 혼동하지 않게 만드는 단계가 MVP입니다.
...
|---|---|---|---|
| v0.1 | 코어 권한 조각(Core Authority Slice) | 로컬 프로젝트 하나, Task 하나, 기본 범위 하나, 쓰기 권한 경로 하나, 기록된 Run 하나, 근거 링크 하나, 구조화된 막힘/상태 응답 하나로 구성된 첫 실행 가능한 내부 kernel loop. | 사용자 대상 MVP 가치, full intake/discovery, full Decision Packet 품질, 잔여 위험 semantics, 수동 QA, 분리 검증, 읽기용 요약 완전성, operations readiness. |
...
- Core state, events, artifacts, projection/freshness facts, errors를 통해 judgment, QA, verification, 잔여 위험, 작업 수락의 분리를 증명하는 agency-hardened conformance fixtures
...
- connect, doctor, serve MCP, 읽기용 요약 refresh, reconcile, recover, export, artifacts check, conformance run에 대한 operator smoke
...
### 보증과 스튜어드십 팩(v0.3 Assurance & Stewardship Pack) 종료 점검 목록
...
- Agency conformance가 Journey visibility, user judgment, Autonomy Boundary respect, distinct user judgments, 잔여 위험 처리를 증명한다.
