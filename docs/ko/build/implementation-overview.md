...
- 읽기용 요약(Projection) 범위는 단계화되어 있습니다. 내부 엔지니어링 점검은 owner path가 이미 만든 경우에만 freshness/read fact를 노출할 수 있고, MVP-1은 Core에서 파생한 간결한 상태 카드(compact status card)와 최소 사용자 읽기용 출력을 제공하며, detailed report/template은 승격되기 전까지 later-profile scope입니다.
...
- 이전에 open이었던 simplified judgment naming, `request_user_judgment`, `harness.status.next_actions` vs `harness.next`, MVP-1 storage minimum, local access error taxonomy, compact status card scope, small direct change evidence requirement, 작업 수락/잔여 위험 minimal records는 이 문서 기준에서 해소되었지만 아직 코드 작성용으로 수락된 것은 아닙니다.
...
이후 프로필은 원천 기록이 존재하거나 변경되고 owner profile이 승격할 때 optional, future, diagnostic `ProjectionKind` value를 켤 수 있습니다. `ProjectionKind` value와 API 소유 지원 계층은 [MCP API와 스키마](../reference/mcp-api-and-schemas.md#shared-schemas)가 담당합니다.
