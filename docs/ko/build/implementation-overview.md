...
- evidence relation 또는 minimal Evidence Manifest 갱신 하나
- 유일한 완료 판단 지점으로서의 `close_task` 차단 조건 동작

사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)에서는 같은 API surface를 넓혀 ordinary request가 scope, user-owned judgments, evidence expectations, close readiness, final acceptance, residual-risk display로 정리되게 합니다.
...
Projection output은 그것이 의존하는 Core 원천 기록에서 파생합니다. 예를 들어 Task, gate, Run, artifact, evidence, Eval, QA, 그 밖의 owner record가 존재한 뒤 그 기록에서 나와야 합니다. 코어 권한 조각(v0.1 Core Authority Slice)은 owner path가 이미 projection freshness를 만든다면 이를 보고할 수 있지만, projection rendering이 첫 증명은 아닙니다. 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)는 사용자가 scope, judgment, evidence, close readiness, acceptance, residual risk를 이해할 만큼의 readable projection 또는 card를 제공해야 합니다. Projection template은 권한을 만들거나, 근거를 충족하거나, state를 대체하거나, state model을 정하거나, 첫 증명이 될 수 없습니다.

이후 단계는 원천 기록이 존재하거나 변경될 때 Reference-required `ProjectionKind` value를 지원해야 합니다. `ProjectionKind` value와 API-owned tiering은 [MCP API와 스키마](../reference/mcp-api-and-schemas.md#shared-schemas)가 담당합니다. [문서 Projection 참조](../reference/document-projection.md#template-tiers)는 projection authority boundary, source-record rule, freshness rule, template tier presentation을 담당하고, [Template 참조](../reference/templates/README.md)는 rendered template body와 display card를 담당합니다.
...
|---|---|
| Dashboard, hosted workflow UI, rich UI | v0.1부터 v0.4까지 authority, evidence, close readiness, final acceptance, residual-risk acceptance가 아닙니다. |
...
| Browser QA Capture | v1+ 후보입니다. Required automation, Manual QA replacement, acceptance replacement가 아닙니다. |
| Cross-Surface Verification | v1+ automation 후보입니다. Detached verification은 이 automation 없이 local path에서 먼저 증명할 수 있습니다. |
...
- minimal evidence state가 support, partial support, insufficiency를 기록함
...
첫 제품 MVP 목표는 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)입니다. 코어 권한 조각(v0.1 Core Authority Slice) 뒤에 도달하는 목표이지 첫 실행 batch를 키워서 만들지 않습니다. 이 단계는 하네스가 scope, user-owned judgment, evidence, close readiness, acceptance, residual risk를 로컬 권한 기록에 보존한다는 점을 사용자에게 보여 줍니다.

다음을 보여야 합니다.

- ordinary user language가 Harness vocabulary를 요구하지 않고 tracked work를 시작하거나 resume할 수 있음
- work가 scope, non-goals, acceptance criteria, evidence expectations, close readiness, judgment boundaries로 정리됨
...
- required evidence 또는 user judgment가 없으면 close가 block됨
- close-relevant risk가 있으면 successful acceptance 또는 close 전에 residual risk가 보임
- final user acceptance가 sensitive-action Approval과 residual-risk acceptance와 구분됨
