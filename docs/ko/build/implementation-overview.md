...
이 문서는 문서 재설계 / 검토와 maintainer용 문서 수락 후보 리뷰를 위한 구현 계획 문서입니다. 이 저장소는 현재 문서 전용이며, 문서 승인을 받으면 하네스 서버 소스 저장소가 되는 것을 목표로 합니다. 아직 이곳에는 하네스 서버/런타임 구현, executable fixture file, runnable Harness Server conformance test가 없습니다. 이 revision은 maintainer review를 위한 문서 수락 후보이지 구현 시작 승인이 아닙니다. 첫 실행 목표는 코어 권한 조각(v0.1 Core Authority Slice)이며, 커널 스모크(Kernel Smoke)는 이 조각을 위한 좁은 conformance 작성 프로파일입니다. 모듈을 가진 로컬 프로세스 하나로 가장 작은 권한 루프를 증명하는 경로입니다. 첫 제품 MVP 목표는 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)입니다. v0.3과 v0.4는 assurance, stewardship, operations, handoff 동작을 단단하게 만듭니다. v1+ Expansion은 owner 문서가 승격하고 증명하기 전까지 roadmap 범위에 둡니다.
...
- maintainer 인계가 문서를 첫 runtime batch 계획에 사용할 수 있다고 명시적으로 승인한 뒤 첫 구현 형태를 계획할 때.
...
이 Build 경로의 모든 구현 동사는 maintainer 인계가 문서를 첫 runtime batch 계획에 사용할 수 있다고 명시적으로 승인한 뒤의 future runtime-batch planning을 설명합니다. [문서 승인 상태](#문서-승인-상태)가 첫 runtime batch 계획을 승인하지 않는 동안에는 이 문서를 scope와 인계 준비 상태를 리뷰하는 용도로만 사용합니다.
...
이 항목은 maintainer가 직접 갱신하는 문서 인계 표시입니다. Reference 계약, conformance 결과, 생성된 운영 record, runtime 구현 승인으로 쓰지 않습니다. 아래 checkpoint에서 acceptance를 자동 추론하지 않습니다. maintainer가 이 표를 명시적으로 바꿔야 합니다.

현재 revision status: maintainer review를 위한 문서 수락 후보입니다. Maintainer가 명시적으로 바꾸기 전까지 문서 승인은 여전히 아니오입니다. 이 상태 표시는 runtime/server 구현, runtime conformance, 구현 준비 상태가 아닙니다.

| 질문 | 현재 상태 |
|---|---|
| 문서 재설계 / 검토가 현재 저장소 단계인가? | 예. 이 revision은 문서 수락 후보이며 maintainer 수락은 아직 남아 있습니다. 문서 편집은 계속될 수 있지만, runtime/server 구현은 시작하지 않았습니다. |
...
| 서버 코딩 전 필요한 구현 결정이 남아 있는가? | 아니오. 이 인계에서 의도적으로 남긴 구현 시작 전 결정은 없습니다. Maintainer 검토에서 새 결정이 발견되면 [MVP 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)에 소유자, 영향, 필요한 결정을 기록합니다. [문서 작성 가이드](../maintain/authoring-guide.md#알려진-재설계-쟁점-트래커)의 검토 risk는 문서 검토 항목이지 runtime conformance, 구현 준비 상태, 서버/런타임 구현 시작 승인이 아닙니다. |

Build 독자는 이 표를 진입 기준으로 보아야 합니다. maintainer 인계가 두 번째 행을 예로 바꾸기 전까지 코어 권한 조각(v0.1 Core Authority Slice)도 이 저장소에서는 계획 전용이며 하네스 서버/런타임 구현을 시작하면 안 됩니다.

## 문서 수락 후보 요약

이 섹션은 문서 세트의 maintainer용 짧은 수락 후보 요약입니다. 현재 하네스가 무엇인지, 무엇이 정리되었는지, 무엇이 열려 있는지, 이 저장소에서 하네스 서버 구현을 시작하기 전에 무엇이 참이어야 하는지 보여 줍니다. 이것은 문서 수락 후보 요약일 뿐이며 runtime state, Acceptance record, generated projection, conformance result, server code를 만들지 않습니다.

현재 단계와 향후 저장소 역할:

- 이 저장소는 문서 재설계 / 검토 단계이며 문서 수락 후보입니다.
...
- Harness는 사용자 소유 판단을 보존합니다. 제품 결정, 중요한 기술 결정, QA 기대치, 작업 수락, waiver, 잔여 위험 수용은 소유자 계약이 달리 정하지 않는 한 사용자 판단으로 남습니다.
...
- 보증과 스튜어드십 팩(v0.3 Assurance & Stewardship Pack)은 Decision Packet, Approval separation, detached verification, 수동 QA, residual-risk accepted close, stewardship, TDD, feedback-loop policy, context hygiene를 단단하게 만듭니다.
...
- v1+ Expansion은 향후 소유자 결정이 exact contract, fixture, fallback behavior, projection-as-canonical dependency 없음으로 승격하기 전까지 roadmap 범위입니다.
...
- Projection과 대화는 읽기/대화 접점이며 운영 기준이 아닙니다.
- Projection scope는 단계화되어 있습니다. v0.1은 freshness/read fact를 노출할 수 있고, v0.2는 MVP 이해에 필요한 사용자 읽기용 출력을 제공하며, detailed report/template은 승격되기 전까지 later-profile scope입니다.
...
- Agent context는 제한됩니다. 항상 주입되는 맥락은 짧고 최신이어야 하며, 상세 contract는 필요할 때 owner 문서나 retrieval path에서 가져옵니다.
- Conformance fixture 문서는 단계화된 향후 검증 계획입니다. 현재 executable fixture file이나 runnable conformance test가 있다는 뜻이 아닙니다.

남은 질문:

- 현재 기록된 구현 시작 전 결정은 없습니다. 이 문서는 maintainer review를 위한 문서 수락 후보로 준비되어 있지만, [문서 승인 상태](#문서-승인-상태)를 maintainer가 명시적으로 바꾸기 전까지 수락된 것이 아닙니다.
- Maintainer 검토에서 서버 코딩 전 필요한 구현 결정이 발견되면 [MVP 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)에 기록합니다. 큰 결정을 흩어진 TODO로 남기지 않습니다.

## 하네스 서버 구현 준비 조건

이 checkpoint는 maintainer가 문서 승인 상태를 문서 유지보수에서 첫 runtime batch 계획으로 바꾸기 전에 무엇이 참이어야 하는지 판단할 때 사용합니다. 이것은 계획 인계일 뿐입니다. 그 자체로 runtime/server 구현을 승인하지 않으며, 정확한 schema, DDL, fixture 의미, runtime contract를 정의하지 않습니다.
...
- Projection scope가 단계화되어 있고 권한이 없다. 읽기용 Projection과 card는 Core record와 artifact ref에서 파생되며, 권한을 만들거나 첫 증명이 되지 않는다.
...
- Agent context 전략이 정의되어 있다. 항상 주입되는 맥락은 한 화면 안팎, current-state 기반, profile-scoped로 유지하고, 전체 reference docs, schema, old log, projection body는 알맞은 owner/retrieval path로만 가져온다.
...
- v1+ Expansion 기능은 [로드맵 승격 규칙](../roadmap.md#승격-규칙)에 따라 owner 문서가 승격하기 전까지 코어 권한 조각(v0.1 Core Authority Slice), 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP), v0.3과 v0.4 단계, 강화된 로컬 기준 목표(hardened local reference target) 밖에 남아 있다.
...
Core는 운영 상태의 기준 기록을 변경하는 유일한 경로입니다. [런타임 아키텍처](../reference/runtime-architecture.md#state-transaction-flow)가 담당하는 transaction order를 구현합니다. 순서는 envelope와 state-version validation, lock 획득, 현재 상태 읽기, validators, record update, `task_events` append, projection job enqueue, commit입니다. Build 계층에서 요약하면 Core는 다음을 해야 합니다.
...
State store는 권한 루프의 기준 운영 상태를 보관합니다. 여기에는 project와 Task state, scope, write authority, Run, evidence, judgment record, projection/reconcile tracking, `task_events`가 포함됩니다.
...
MCP server에 닿을 수 없으면 해당 call path에서 기준이 되는 Core response가 없습니다. 첫 구현은 이를 MCP unavailable로 보고하고, reference surface의 실제 guarantee level에 따라 write-capable work를 보류하며, cached projection, generated file, chat text에서 상태를 만들어 내지 않아야 합니다.

코어 권한 조각(v0.1 Core Authority Slice)에서는 다음을 우선합니다.

- 현재 Core 상태를 읽는 status/next
- 첫 Task와 scope를 만들거나 seed하는 owner-valid path 하나
- write-authority path: `prepare_write`, compatible Write Authorization 하나, `record_run`
- artifact/evidence owner path 하나
- missing scope, evidence, authorization, seeded 필요한 판단에 대한 close/status blocker 동작

사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)에서는 같은 API surface를 넓혀 ordinary request가 scope, user-owned judgments, evidence expectations, close readiness, 작업 수락, 잔여 위험 표시로 정리되게 합니다.

Public request와 response 규칙은 [MCP API와 스키마](../reference/mcp-api-and-schemas.md)가 담당합니다.

State conflict와 idempotency replay 동작도 그 public tool 계약의 일부입니다. Build code는 [Idempotency](../reference/mcp-api-and-schemas.md#idempotency)와 [State Conflict 동작](../reference/mcp-api-and-schemas.md#state-conflict-동작) owner section을 사용하고, 지속 저장 세부 내용은 [Storage와 DDL](../reference/storage-and-ddl.md)에 맡깁니다.
...
Projection output은 그것이 의존하는 Core 원천 기록에서 파생합니다. 예를 들어 Task, gate, Run, artifact, 근거, Eval, QA, 그 밖의 owner record가 존재한 뒤 그 기록에서 나와야 합니다. 코어 권한 조각(v0.1 Core Authority Slice)은 owner path가 이미 읽기용 요약 최신성을 만든다면 이를 보고할 수 있지만, projection rendering이 첫 증명은 아닙니다. 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)는 현재 작업 상태, 판단 요청, 근거 요약, 닫기 준비 상태, 작업 수락, 잔여 위험을 사용자가 이해할 만큼의 readable summary 또는 card를 제공해야 합니다. Projection template은 권한을 만들거나, 근거를 충족하거나, 상태를 대체하거나, 상태 모델을 정하거나, 첫 증명이 될 수 없습니다.
...
[문서 Projection 참조](../reference/document-projection.md#템플릿-구현-계층)는 Projection 권한 경계, source-record rule, freshness rule, 템플릿 구현 계층을 담당하고, [Template 참조](../reference/templates/README.md)는 rendered template body와 display card를 담당합니다.
...
- project connect/register와 MCP boundary 제공
- doctor/readiness fact 보고
- projection refresh와 generated-file 또는 managed-block drift reconcile
- 문서화된 owner path를 통한 interrupted 또는 stale operational state recovery
- omitted secret을 유출하지 않는 state/report/artifact snapshot export
- artifact integrity check
- 향후 conformance fixture가 구체화된 뒤 그 fixture run
...
첫 구현 계획은 좁게 유지합니다. 아래 항목은 owner 문서가 승격하기 전까지 선행 조건으로 만들지 않습니다.
...
| Broad connector ecosystem 또는 marketplace | Owner 문서가 승격하기 전까지 reference surface 하나를 넘어 staged delivery 범위를 넓히지 않습니다. |
...
유용한 향후 capability라도 owner 문서가 capability profile, redaction/secret/PII policy, 필요한 경우 retention 또는 test-environment rule, fixture coverage, fallback 동작, projection-as-canonical 의존성 없음을 정의하기 전까지는 읽기 전용 표시, metadata, 기존 owner path를 위한 artifact 후보, fixture candidate로만 나타날 수 있습니다. 코어 권한 조각(v0.1 Core Authority Slice)을 실행하거나, 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)를 완료하거나, staged-delivery close readiness를 주장하기 위한 전제 조건이 되어서는 안 됩니다.

## 첫 증명

첫 실행 목표는 코어 권한 조각(v0.1 Core Authority Slice)입니다. 하네스가 하나의 권한 결정을 만들고 적용할 수 있음을 보여 주는 가장 작은 실행 가능한 경로입니다. 커널 스모크(Kernel Smoke)는 이 목표의 좁은 conformance authoring profile입니다.
...
- artifact 또는 evidence ref 하나를 등록하고 Run 또는 evidence relation에 연결할 수 있음
- minimal 근거 상태가 support, partial support, insufficiency를 기록함
- status와 next 읽기가 mutation을 만들지 않음
...
첫 제품 MVP 목표는 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)입니다. 코어 권한 조각(v0.1 Core Authority Slice) 뒤에 도달하는 목표이지 첫 실행 batch를 키워서 만들지 않습니다. 이 단계는 하네스가 scope, user-owned judgment, 근거, close readiness, 작업 수락, 잔여 위험을 로컬 권한 기록에 보존한다는 점을 사용자에게 보여 줍니다.
...
- product/UX judgment와 material technical architecture judgment를 분리해 제시할 수 있음
...
- close-relevant risk가 있으면 작업 수락 또는 close 전에 잔여 위험이 보임
- 사용자의 작업 수락이 sensitive-action Approval과 잔여 위험 수용과 구분됨
- 사용자에게 보이는 Projection 또는 card가 Core records에서 파생되며, template polish가 기준 권한이 되지 않아도 충분함
