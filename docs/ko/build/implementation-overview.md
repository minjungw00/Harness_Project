...
|---|---|---|
| Kernel Smoke | 하나의 로컬 Task가 Core 권한 루프를 통과할 수 있음을 증명합니다. 여기에는 write authorization point로서의 `prepare_write`, single-use Write Authorization, observed changes와 artifact가 있는 `record_run` consumption, artifact로 뒷받침되는 근거, status, 최소 projection 최신성, structured close blocker가 포함됩니다. | Status가 active Task, gate, Change Unit, 근거, blocker, projection 최신성을 보여 줍니다. 범위 밖 작업은 차단되고, compatible scoped work는 권한을 받아 한 번만 사용되며, 근거 또는 필요한 decision이 없으면 `close_task`가 structured blocker와 함께 거절합니다. |
...
Harness MVP는 AI 지원 제품 작업을 위한 로컬 권한 커널입니다. 첫 구현 대상은 Kernel Smoke입니다. 초기 구현 가정은 명확한 내부 모듈을 가진 하나의 로컬 시스템이며, 분산 플랫폼으로 시작하지 않습니다.
