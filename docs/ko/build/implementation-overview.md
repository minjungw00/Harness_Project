...
구현 계획이 full user-facing system, 보증 프로필이나 운영 프로필의 동작 전체, 읽기용 요약 템플릿 다듬기, dashboard 또는 hosted workflow UI, Context Index, connector marketplace, hook expansion, metrics, parallel orchestration, broad automation lane에서 시작한다면 첫 구현 smoke slice보다 큰 곳에서 시작하는 것입니다.
...
- Conformance fixture 문서는 단계화된 향후 검증 계획입니다. 현재 실행 가능한 fixture file이나 실행 가능한 conformance test가 있다는 뜻이 아닙니다.
...
- 커널 스모크(Kernel Smoke) fixture queue가 내부 엔지니어링 점검 conformance 작성 순서이자 향후 적합성 검증 계획으로 확인되어 있다. 정확한 fixture format, assertion, catalog semantics는 [Conformance Fixtures 참조](../reference/conformance-fixtures.md#kernel-smoke-authoring-queue)에 둡니다. 이 checkpoint는 fixture file이나 실행 가능한 conformance test가 이미 존재한다는 뜻이 아닙니다.
...
첫 향후 구현 목표는 내부 엔지니어링 점검입니다. 구현이 시작된 뒤 하네스가 하나의 권한 결정을 만들고 적용할 수 있음을 보여 주는 가장 작은 구현 경로입니다. 커널 스모크(Kernel Smoke)는 이 목표의 smoke check를 위한 좁은 향후 작성 label이지 전체 conformance suite가 아닙니다.
...
첫 사용자 가치 목표는 MVP-1 사용자 작업 루프입니다. 내부 엔지니어링 점검 뒤에 도달하는 목표이지 첫 구현 batch를 키워서 만들지 않습니다. 이 단계는 사용자가 평범한 말로 작업을 시작하면 하네스가 작업 범위, 대기 중인 판단 요청, 근거 요약, 닫기 막힘, 다음 안전한 행동, 작업 수락과 잔여 위험 수용 사이의 최소 분리를 로컬 기준으로 보존한다는 점을 보여 줍니다. 근거와 간결한 상태 출력은 지원 수단이지 전체 제품 시스템 자체가 아닙니다.
