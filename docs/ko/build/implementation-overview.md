...
로컬 커널은 조율과 권한의 기록이지 제품 저장소, 소스 관리, 테스트, 코드 리뷰, 대화, 사용자 소유 제품 판단과 기술 구조 판단을 대체하지 않습니다. 첫 경로는 상태/막힘 출력이 최소 권한 상태와 무엇이 빠졌는지를 설명할 수 있게 계획하되, ordinary-language intake, close blocker summaries, work acceptance display, residual-risk visibility, compact user-facing explanation은 v0.2와 이후 단계에 둡니다.
...
이 저장소 단계에서는 서버/런타임 구현 결정을 코드 작성용으로 공식 수락하지 않았습니다. [단계별 전달 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)의 open decision ledger에는 영향을 받는 stage를 구현하기 전에 수락하거나 명시적으로 미뤄야 하는 pre-coding decision이 기록되어 있습니다.
...
- v0.2 First User-Value Slice는 좁은 범위의 ordinary user value를 증명합니다. Start/resume, work-shape classification, scope/non-goals/success criteria, user-owned judgment routing, evidence summary, close blocker summary, work acceptance display, residual-risk visibility가 여기에 속합니다.
...
- 현재 open item에는 simplified judgment model and naming, `request_user_decision` vs `request_user_judgment`, `harness.next` vs `status.next_actions`, v0.2 storage minimum, local access error taxonomy, compact status card scope, small direct change evidence requirement, acceptance/residual-risk minimal records가 포함됩니다.
...
| v0.2 First User-Value Slice | 평범한 사용자 작업이 scope, non-goals, success criteria, user-owned judgment, evidence summary, close blocker summary, work-acceptance display, residual-risk visibility로 정리됨을 증명합니다. | 사용자는 work shape classification, product/UX judgment와 architecture judgment의 분리, 사용자에게 묻기 전 codebase-answerable fact 확인, small direct change와 tracked work의 다른 procedural budget, ambiguous feature request의 clarification 보류, required evidence 또는 필요한 user-owned judgment 누락 시 close blocker, residual risk display, ambiguous consent가 judgment resolution으로 취급되지 않는 점, MCP/Core unavailable 상태가 authority를 만들어내지 않는 점, Core-derived compact status card, active profile/user request/task type/risk profile이 요구할 때만 필요한 detached verification, required verification을 일부러 건너뛸 때만 필요한 waiver, Approval과 residual-risk acceptance에서 분리된 work acceptance를 볼 수 있습니다. |
...
v0.2 First User-Value Slice에서는 같은 API surface를 필요한 만큼만 넓힙니다. Ordinary request가 tracked work를 시작하거나 이어 가고, work shape를 분류하며, scope/non-goals/success criteria를 요약하고, minimal user judgment를 요청/기록하며, status/next output, evidence summary, close blocker summary, residual-risk visibility, sensitive approval / work acceptance / risk acceptance 분리 표시를 제공하는 정도입니다.
...
| v0.2 First User-Value Slice | 현재 작업, 사용자 판단, 근거 상태, close blocker, 작업 수락 필요 여부/상태, sensitive approval display, 잔여 위험 표시를 위한 user-facing status/next diagnostic. |
...
첫 사용자 가치 목표는 v0.2 First User-Value Slice입니다. v0.1 Core Authority Smoke 뒤에 도달하는 목표이지 첫 실행 batch를 키워서 만들지 않습니다. 이 단계는 평범한 요청이 작은 범위의 하네스 작업으로 보이게 하고, 하네스가 scope, non-goals, success criteria, user-owned judgment, evidence summary, close blocker summary, work acceptance display, residual-risk visibility를 로컬 권한 기록에 보존한다는 점을 사용자에게 보여 줍니다. 근거와 compact status output은 지원 수단이지 full product system 자체가 아닙니다.
...
- compact status card가 Core records에서 파생되며, template polish가 기준 권한이 되지 않아도 충분함
