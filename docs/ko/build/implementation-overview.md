...
Harness MVP는 AI 지원 제품 작업을 위한 로컬 권한 커널입니다. 작업 흐름 주변에 durable local state, artifact ref, 읽기용 projection을 유지하되, 제품 이력, 실행 가능한 확인, 리뷰, 사용자 판단은 기존 엔지니어링 절차에 남겨 둡니다. 첫 구현 대상은 Kernel Smoke입니다. 초기 구현 가정은 명확한 내부 모듈을 가진 하나의 로컬 시스템이며, 분산 플랫폼으로 시작하지 않습니다.
