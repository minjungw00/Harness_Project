...
| 층 | 활성 구현 범위 | 이 층 밖 |
|---|---|---|
| 첫 실행 가능한 권한 루프 | 내부 엔지니어링 점검입니다. Project state, reference surface registration 하나, 활성 task/scope boundary, 담당 문서가 정의한 쓰기 전 확인, compatible run/evidence path 하나, 좁은 status/닫기 차단 사유 확인을 포함하는 로컬 Core 권한 루프 하나입니다. | Ordinary natural-language intake 품질, 저장된 사용자 판단 흐름, full close semantics, full projection renderer, 상세 report, operations, conformance runner, broad connector APIs, hosted connector registry, cross-surface orchestration, later-profile storage. |
| 첫 사용자 작업 루프 | MVP-1 사용자 작업 루프입니다. 평소 말 intake, 작업 모양/범위 요약, 집중된 사용자 판단, Core-owned evidence summary, close result/blocker, 다음 안전한 행동, guarantee display, 최종 수락과 잔여 위험 표시, 명시적 잔여 위험 수락이 있는 accepted-risk close, 작은 파생 출력을 더합니다. | Full Evidence Manifest, detached Eval, full Manual QA matrix, Assurance hardening, operations/export/recover, dashboard, hosted UI, broad connector, hosted connector registry, cross-surface orchestration, automation, detailed report. |
...
Build는 단계 경계와 읽기 경로를 담당합니다. 정확한 public method 계약과 활성 MVP-1 method set은 [MVP API](../reference/api/mvp-api.md), shared shape는 [API Schema Core](../reference/api/schema-core.md), storage contract는 [Storage](../reference/storage.md), 작은 view/template 계약은 [Projection과 Template 참조](../reference/projection-and-templates.md)와 [Template 참조](../reference/templates/README.md)가 담당합니다. [MVP-1 사용자 작업 루프](mvp-user-work-loop.md#핵심-생각)는 stage goal을 요약하고 이 담당 문서들로 연결합니다.

Persisted Journey Card, full Evidence Manifest, Eval report, Manual QA report, TDD Trace, Module Map, Interface Contract, Export report 같은 상세 report는 owner가 좁은 non-required display로 표시하거나 stage impact와 함께 승격하기 전까지 후속/profile 범위에 남습니다.
...
| 설계 품질 차단 경계 | 활성 MVP design-quality blocker는 담당 문서가 정의한 작은 차단 집합으로 제한됩니다. 더 넓은 policy catalog는 기본적으로 Routed candidate 또는 Advisory/later입니다. | [설계 품질 정책](../reference/design-quality-policies.md#활성-mvp-차단-집합). |
...
| 향후 conformance 경계 | Fixture 문서는 향후 계획으로 남습니다. 활성 예시는 future state-assertion target이며, 실행 가능한 runner나 pass/fail result가 문서만으로 생긴 것처럼 쓰지 않습니다. | [Conformance Fixtures 참조](../reference/conformance-fixtures.md), [향후 Fixtures](../later/future-fixtures.md). |
...
| API | Minimal status/blocker read, owner-valid setup path, 쓰기 전 확인, run/evidence 기록, artifact/evidence ref path 하나, 좁은 close-blocker check. Natural-language intake는 필요하지 않습니다. | Status/next actions, intake, user judgment, write check, run/evidence, close를 위한 owner-defined MVP-1 public surface를 더합니다. | [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md). |
...
| 운영/conformance | 향후 state-assertion smoke 작성 계획일 뿐입니다. | Runtime fixture가 실제로 만들어지기 전까지 behavior example로 다룹니다. Rendered Markdown/prose는 conformance proof가 아닙니다. | [운영과 Conformance 참조](../reference/operations-and-conformance.md), [Conformance Fixtures 참조](../reference/conformance-fixtures.md). |
