...
| 첫 사용자 작업 루프 | MVP-1 사용자 작업 루프입니다. 평소 말 intake, `harness.status.next_actions`가 있는 status, 집중된 사용자 판단 요청/기록, Core-owned evidence summary, close result/blocker, 최종 수락과 잔여 위험 표시, 명시적 잔여 위험 수락이 있는 accepted-risk close, compact Core-derived view를 더합니다. | Full Evidence Manifest, detached Eval, full Manual QA matrix, Assurance hardening, operations/export/recover, dashboard, hosted UI, broad connector, automation, detailed report. |
