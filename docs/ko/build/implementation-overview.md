...
- 사용자-facing category, internal route, display depth, 작은 Decision Packet weight와 관련해 추적하던 판단 모델 drift는 이 문서 기준에서 해소되었습니다. Review에서 남은 owner-contract decision이 드러나면 흩어진 TODO가 아니라 MVP 계획 decision log로 routing합니다.
...
| v0.2 User-Facing Harness MVP | 현재 작업, 사용자 판단, 근거 상태, close blocker, 작업 수락 필요 여부/상태, 잔여 위험 표시를 위한 user-facing status/next diagnostic. |
