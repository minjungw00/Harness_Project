...
Learn 경로에서 하네스의 기본 개념을 먼저 이해해 두는 것이 좋습니다. 정확한 동작은 이 문서 끝에 연결된 Reference 문서들을 봅니다. 로드맵 후보와 승격 기준은 [로드맵](../roadmap.md)을 봅니다.
...
내부 엔지니어링 점검을 먼저 만듭니다. 가장 작은 로컬 Core 권한 경로를 증명하며, 커널 스모크(Kernel Smoke)는 좁은 향후 smoke-check 작성 라벨입니다. 이것은 내부 smoke 단계이지 제품 MVP가 아닙니다. 그다음 MVP-1 사용자 작업 루프를 만들어 평범한 말로 작업 시작/이어가기, 작업 형태 분류, 범위/하지 않을 일/성공 기준, 최소 판단 요청, 근거 요약, 닫기 막힘, 다음 안전한 행동, 민감 동작 승인 / 작업 수락 / 잔여 위험 수용 분리 표시를 사용자가 볼 수 있게 합니다. 근거와 간결한 상태 출력은 이 경험을 지원하지만 전체 보증, QA, Eval, 보고, 운영, 대시보드 시스템이 아닙니다. 보증 프로필과 운영 프로필이 그 경로를 단단하게 만듭니다.
...
로컬 커널은 조율과 권한의 기록이지 제품 저장소, 소스 관리, 테스트, 코드 리뷰, 대화, 사용자 소유 제품/UX 판단과 기술 판단을 대체하지 않습니다. 첫 경로는 상태/막힘 출력이 최소 권한 상태와 무엇이 빠졌는지를 설명할 수 있게 계획하되, 평범한 말의 작업 접수, 닫기 막힘 요약, 작업 수락 표시, 잔여 위험 표시, 사용자용 간결한 설명은 MVP-1와 이후 단계에 둡니다.

첫 권한 루프는 좁게 유지합니다. `prepare_write`는 제품 파일 쓰기에 대한 유일한 권한 판단 지점이고, 반환된 쓰기 허가 기록은 지속적이며 한 번만 쓸 수 있으며, `record_run`은 관찰된 변경과 artifact/evidence ref 하나를 기록하면서 하나의 호환되는 direct Run 또는 implementation Run에 대해 이를 소비합니다. 내부 엔지니어링 점검은 blocker를 위해 status나 좁은 close-task smoke를 사용할 수 있지만, 작업 수락이나 잔여 위험 close semantics를 증명하지 않습니다. 정확한 상태 로직은 [커널 참조](../reference/kernel.md#prepare_write)에, public request/response detail은 [MVP API](../reference/api/mvp-api.md)에 둡니다.

기준 상태, local project registration 하나, active Task 하나, Reference 계약상 필요한 경우에만 Change Unit 소유자 형태로 표현되는 scoped boundary 하나, Write Authorization 경로 하나, 기록된 Run 하나, artifact/evidence link 하나, Core tool 동작, 그리고 그 경로를 실행해 볼 최소 MCP reachability에서 시작합니다. 초기 구현 가정은 분산 platform이 아니라 모듈을 가진 로컬 프로세스 하나입니다. Natural-language intake, full Discovery, full-format user judgment presentation, full Evidence Manifest behavior, Eval, Manual QA, Acceptance, residual-risk acceptance, full close semantics, detached verification, projection rendering, conformance runner, dashboard 또는 hosted workflow UI, index, 넓은 connector ecosystem 또는 marketplace, team workflow, 접점별 connector automation, hook expansion, Browser QA automation, derived metrics, parallel orchestration, operations/export/recover, 넓은 operator entrypoint, broad automation은 이후 단계이거나 그 권한 루프가 존재한 뒤 그것을 읽거나 감싸는 권한 없는 요소로 다룹니다.

구현 계획이 full user-facing system, 보증 프로필이나 운영 프로필의 동작 전체, 읽기용 요약 템플릿 다듬기, dashboard 또는 hosted workflow UI, Context Index, connector marketplace, hook expansion, metrics, parallel orchestration, broad automation lane에서 시작한다면 첫 runnable smoke slice보다 큰 곳에서 시작하는 것입니다.
...
이 저장소 단계에서는 서버/런타임 구현 결정을 코드 작성용으로 공식 수락하지 않았습니다. [단계별 전달 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)의 결정 기록에는 문서 기준에서 해소된 MVP-1 계약 결정과, 영향을 받는 stage를 코딩하기 전에 아직 수락하거나 명시적으로 미뤄야 하는 구현 준비 항목이 함께 정리되어 있습니다.
...
| 단계 경계 결정 | capability가 내부 엔지니어링 점검, MVP-1 사용자 작업 루프, 보증 프로필, 운영 프로필, 로드맵 중 어디에 속하는지에 대한 선택입니다. | 구현 개요, 단계별 전달 계획, 담당 문서, 필요한 경우 로드맵 승격 항목. | 경계가 수락되기 전까지 영향을 받는 단계 구현을 막습니다. 명시적으로 기록되어 있으면 문서 검토에는 막힘이 아닐 수 있습니다. |
...
| 향후 로드맵 항목 | 승격되기 전까지 내부 엔지니어링 점검부터 운영 프로필 밖에 있는 유용한 capability입니다. | [로드맵](../roadmap.md)과 승격 뒤 담당 문서. | 담당자가 단계 목표로 승격하지 않는 한 문서 검토, 내부 엔지니어링 점검, MVP-1을 막지 않습니다. |
...
| 서버 코딩 전 결정 기록 | 예전 ledger의 MVP-1 scope 결정은 단계별 전달 계획에서 문서 기준으로 해소되었습니다. 하지만 아직 server/runtime implementation decision은 코드 작성용으로 수락되지 않았습니다. | 영향을 받는 API, DDL, state transition, local access, status card, evidence, acceptance, risk behavior를 코딩하기 전에 [단계별 전달 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)의 문서 기준 결정을 maintainer가 수락하고, 아직 열린 구현 준비 항목을 해결하거나 명시적으로 미룹니다. |

Build 독자는 이 표를 진입 기준으로 보아야 합니다. 유지보수자 인계가 구현 계획 준비 상태를 명시적으로 수락하기 전까지 내부 엔지니어링 점검도 이 저장소에서는 계획 전용이며 하네스 서버/런타임 구현을 시작하면 안 됩니다.
...
- Harness는 사용자 소유 판단을 보존합니다. 제품/UX 판단, 아키텍처와 보안/개인정보 고려사항을 포함한 기술 판단, QA 기대치, 작업 수락, waiver, 잔여 위험 수용은 소유자 계약이 달리 정하지 않는 한 사용자 판단으로 남습니다.
...
- 내부 엔지니어링 점검은 가장 작은 로컬 Core 권한 루프를 증명합니다. Kernel Smoke는 이 단계의 좁은 향후 smoke-check 작성 라벨입니다.
- MVP-1 사용자 작업 루프는 좁은 범위의 사용자 가치를 증명합니다. 평범한 말로 작업 시작/이어가기, 범위/하지 않을 일/성공 기준, 판단 요청/기록, OS-level 차단이 아닌 Core를 통한 협력형 사전 쓰기 범위 확인, 실행/근거 참조 기록, 간결한 상태와 다음 안전한 행동, 닫기 막힘 표시, 작업 수락과 잔여 위험 수용의 최소 분리가 여기에 속합니다.
- 보증 프로필은 verification, 수동 QA, 잔여 위험 수용 close, 작업 수락 분리, stewardship, user judgment 품질, Approval 분리, TDD, feedback-loop policy, context hygiene를 단단하게 만듭니다.
- 운영 프로필은 doctor/readiness, recover/export, artifact integrity, release handoff, 더 넓은 fixture coverage, later-boundary check를 단단하게 만듭니다.
- 로드맵은 향후 소유자 결정이 exact contract, fixture, fallback behavior, 읽기용 요약을 기준으로 삼는 의존성 없음으로 승격하기 전까지 로드맵 범위입니다.
...
- 판단 모델은 Approval, User Judgment, 작업 수락, 잔여 위험 수용, QA/검증 waiver judgment, Write Authorization을 분리합니다.
- 읽기용 요약(Projection)과 대화는 읽기/대화 접점이며 운영 기준이 아닙니다.
- 읽기용 요약(Projection) 범위는 단계화되어 있습니다. 내부 엔지니어링 점검은 owner path가 이미 만든 경우에만 freshness/read fact를 노출할 수 있고, MVP-1은 Core에서 파생한 상태 카드, 에이전트 맥락 패킷, 판단 요청, 실행/근거 요약, 닫기 결과라는 다섯 가지 작은 보기만 제공하며, detailed report/template은 승격되기 전까지 later-profile scope입니다.
...
- 서버/런타임 구현 결정: 예전 MVP-1 scope ledger 항목은 이 문서 기준에서 해소되었고, 이 문서 기준 결정도 코딩 전 maintainer acceptance가 필요합니다. Maintainer review나 구현 준비 검토에서 추가 설계 쟁점이 발견될 수 있습니다.

서버 코딩 전 결정 기록 상태:

- 문서 기준에서 해소된 server-coding decision-ledger 항목은 [단계별 전달 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)에 기록되어 있습니다.
- 이전에 open이었던 simplified judgment naming, `request_user_judgment`, MVP-1 next-action output을 `harness.status.next_actions`에 두고 별도 `harness.next`를 later/compatibility material로 남기는 결정, MVP-1 storage minimum, local access error taxonomy, MVP-1 작은 보기 범위, small direct change evidence requirement, 작업 수락/잔여 위험 minimal records는 이 문서 기준에서 해소되었지만 아직 코드 작성용으로 수락된 것은 아닙니다.
- Server/runtime implementation decision은 아직 코드 작성용으로 공식 수락되지 않았습니다. 해당 behavior의 server code나 DDL을 바꾸기 전에 maintainer가 관련 owner 문서를 수락하거나 stage impact와 함께 behavior를 명시적으로 미뤄야 합니다.
...
- 판단 모델 drift는 이 문서 기준에서 해소되었습니다. Canonical docs는 `user_judgment`, `harness.request_user_judgment`, `judgment_type`, `presentation`, `display_label`을 사용하고, Decision Packet은 optional full-format/legacy presentation으로 다룹니다. Review에서 남은 owner-contract decision이 드러나면 흩어진 TODO가 아니라 단계별 전달 계획 decision log로 routing합니다.
...
- 확인된 schema/design decision, stage boundary decision, 기타 server-coding decision은 server code나 DDL 변경 전에 단계별 전달 계획에 기록합니다. 문서 기준에서 해소된 결정은 코드 작성용으로 수락되어야 하며, 아직 열린 구현 결정은 stage impact와 함께 해결하거나 명시적으로 미룹니다.
...
첫 구현 계획은 MVP-1 사용자 작업 루프, 보증 프로필, 운영 프로필, roadmap automation이 아니라 내부 엔지니어링 점검 계획부터 시작한다는 뜻입니다. 편집 정리는 필요하지만 그것만으로 충분하지 않습니다. 스키마/설계 결정과 단계 경계 결정은 담당 문서에서 정리되거나, 서버 코딩 전에 단계별 전달 계획에 단계 영향과 함께 기록되어야 합니다. 아래 조건이 모두 참일 때만 첫 구현 계획을 시작할 수 있습니다.

- 내부 엔지니어링 점검 API subset이 accepted 상태다.
- 내부 엔지니어링 점검 DDL이 accepted 상태다.
...
- MVP-1 promotion criteria가 accepted 상태다.
...
- 판단 모델이 Kernel, MCP/API schema, storage, template, fixture, Learn/Use 설명, glossary term과 schema-aligned 상태다. `judgment_type`은 internal judgment type을 담당하고, `presentation`은 compact/full display shape를 담당하며, `display_label`은 제품/UX 판단, 기술 판단, 민감 동작 승인, 작업 수락, 잔여 위험 수용으로 제한된다. Affected gate와 blocked action은 별도의 owner field에 남는다.
- Judgment prompt는 판단의 크기에 맞다. 작은 명시적 unblocker는 `presentation=short`를 사용하고, 복잡하거나 high-risk이거나 close-affecting이거나 민감 동작 승인, waiver, 작업 수락, 잔여 위험 수용, reconcile prompt는 `presentation=full` 또는 optional full-format Decision Packet presentation을 사용할 수 있다. 모든 작은 판단을 무겁게 만들지 않는다.
- Approval, 작업 수락, 잔여 위험 수용이 예시, template, API/schema 문구, close behavior, user-facing routing에서 분리되어 있다.
- 단계가 일관적이다. 내부 엔지니어링 점검은 제품 MVP가 아니고, MVP-1 사용자 작업 루프는 첫 좁은 사용자 가치 조각이지 전체 보증/Eval/QA/보고 시스템이 아니며, 보증 프로필은 verification, QA, residual risk, work acceptance, stewardship를 단단하게 만들고, 운영 프로필은 운영 인계 capability를 추가하며, 로드맵은 승격 전까지 향후 범위다.
...
- Storage와 API는 단계화되어 있다. Reference schema와 DDL은 future-profile field나 table을 정의할 수 있지만, implementation은 active method, record, profile을 따른다. Future-profile 존재 자체가 내부 엔지니어링 점검 범위를 넓히지는 않는다.
...
- Conformance fixture plan이 단계화되고 향후 검증 계획으로 유지된다. Kernel Smoke는 작은 smoke check를 위한 좁은 내부 엔지니어링 점검 작성 label일 뿐이고, 이후 suite profile은 MVP-1 사용자 작업 루프, 보증 프로필, 운영 프로필, 승격된 로드맵 item에 맞으며, fixture file, 향후 fixture catalog, full 내부 엔지니어링 점검 conformance suite, runnable conformance test가 이미 존재한다고 암시하지 않는다.
...
- Link, TODO, terminology, 영어/한국어 의미 일치가 정리되어 있다. Active docs에 흩어진 unresolved major-decision TODO가 없고, 서버 코딩 전 필요한 구현 결정은 [단계별 전달 계획](mvp-plan.md#서버-코딩-전-필요한-구현-결정)에 분류되어 있으며, 문서 기준에서 해소된 결정과 아직 열려 있는 구현 결정이 분리되어 있다.
...
- 내부 엔지니어링 점검의 local-only MCP 노출 baseline이 수락되어 있다. Remote, shared, tunneled, non-loopback 노출은 담당 문서가 connector profile을 승격하고 증명하기 전까지 내부 엔지니어링 점검 baseline 밖입니다. [런타임 아키텍처](../reference/runtime-architecture.md#로컬-접근-기대사항), [보안 위협 모델 참조](../reference/security-threat-model.md#mcp-local-access와-caller-boundary), [API Schema Core](../reference/api/schema-core.md#mcp-boundary-and-caller-trust)를 봅니다.
...
- Core-only mutation model이 수락되어 있다. 기준 운영 상태를 변경하는 것은 Core뿐이며, resource, projection, report, diagnostic, MCP caller, operator entrypoint는 Core의 상태 변경 경로에 들어가지 않는 한 read-only 또는 derived로 남습니다. [Core process model](../reference/runtime-architecture.md#core-process-model), [State transaction flow](../reference/runtime-architecture.md#state-transaction-flow), API [Idempotency](../reference/api/errors.md#idempotency)와 [State conflict behavior](../reference/api/errors.md#state-conflict-behavior)를 봅니다.
- 커널 스모크(Kernel Smoke) fixture queue가 내부 엔지니어링 점검 conformance 작성 순서이자 향후 적합성 검증 계획으로 확인되어 있다. 정확한 fixture format, assertion, catalog semantics는 [Conformance Fixtures 참조](../reference/conformance-fixtures.md#kernel-smoke-authoring-queue)에 둡니다. 이 checkpoint는 fixture file이나 runnable conformance test가 이미 존재한다는 뜻이 아닙니다.
- 첫 실행 가능한 조각은 로컬, 단일 프로젝트, minimal authority loop 범위를 유지한다. 계획 점검 목록은 [첫 실행 가능한 조각](first-runnable-slice.md)을 사용합니다.
- 로드맵 기능은 [로드맵 단계 승격 조건](../roadmap.md#단계-승격-조건)에 따라 담당 문서가 승격하기 전까지 내부 엔지니어링 점검, MVP-1 사용자 작업 루프, 보증 프로필, 운영 프로필 밖에 남아 있다.

이 인계는 roadmap 항목, dashboard 또는 hosted workflow UI, Browser QA Capture automation, Context Index, broad connector ecosystem 또는 marketplace, team workflow, remote MCP exposure, preventive guard expansion, Local Derived Metrics 또는 long-term metrics, parallel orchestration을 내부 엔지니어링 점검, MVP-1 사용자 작업 루프, 보증 프로필, 운영 프로필로 승격하지 않습니다. 정확한 계약은 Reference 문서에 두고, 이 섹션은 짧은 readiness checkpoint로만 사용합니다.

## 증명 경계

| 경계 | 증명하는 것 | 사용자 또는 운영자가 관찰할 수 있는 것 |
|---|---|---|
| 내부 엔지니어링 점검 | 하나의 로컬 Task가 첫 Core 권한 루프를 통과할 수 있음을 증명합니다. 여기에는 local project registration, active Task, Reference 계약상 필요한 경우에만 Change Unit 소유자 형태로 표현되는 scoped boundary 하나, `prepare_write`, single-use 쓰기 허가 기록, `record_run`, artifact/evidence ref 하나, 구조화된 상태/막힘 응답이 포함됩니다. | 상태/막힘 출력이 현재 Task, scope, 쓰기 권한, artifact/evidence support, blocker를 보여 줍니다. `prepare_write`가 범위 밖 쓰기 권한을 거절하고, 호환되는 scoped work는 권한을 받아 한 번만 사용되며, scope, write authority, 또는 artifact/evidence support가 없으면 status 또는 좁은 close-task smoke가 구조화된 막힘과 함께 거절합니다. |
| MVP-1 사용자 작업 루프 | 평범한 사용자 작업이 범위, 하지 않을 일, 성공 기준, 사용자 판단, 근거 요약, 닫기 막힘 요약, 다음 안전한 행동, 작업 수락 표시, 잔여 위험 표시로 정리됨을 증명합니다. | 사용자는 작업 형태 분류, 제품/UX 판단과 아키텍처 판단의 분리, 사용자에게 묻기 전 확인 가능한 사실 확인, 작은 직접 변경과 추적되는 작업의 다른 절차 예산, Core를 통한 협력형 사전 쓰기 범위 확인, 실행/근거 참조 기록, 모호한 기능 요청의 요구사항 구체화 보류, 필요한 근거 또는 사용자 판단 누락 시 닫기 막힘, 잔여 위험 표시, 모호한 동의가 판단 해소로 취급되지 않는 점, MCP/Core unavailable 상태가 authority를 만들어내지 않는 점, Core에서 파생한 다섯 가지 작은 MVP 보기, active profile/user request/task type/risk profile이 요구할 때만 필요한 detached verification, required verification을 일부러 건너뛸 때만 필요한 waiver, Approval과 잔여 위험 수용에서 분리된 작업 수락을 볼 수 있습니다. |
| 보증 프로필 | MVP-1 user-value path가 verification, 수동 QA, 잔여 위험 수용 close, 작업 수락 분리, stewardship, profile별 user judgment 품질, Approval separation, TDD, feedback-loop policy, context hygiene를 정직한 경계 안에서 처리함을 증명합니다. | Fixture가 같은 Core record와 error를 통해 work가 진행, 검증, 수동 QA 요구, 작업 수락, 잔여 위험 수용, close될 수 있는지 보여 줍니다. |
| 운영 프로필 | Operator readiness, recover/export, artifact integrity, release handoff, broader fixture suite coverage, later-boundary checks가 [강화된 로컬 기준 목표](../reference/glossary.md#강화된-로컬-기준-목표)를 완성합니다. | Operator 진입점이 두 번째 authority model을 만들지 않고 같은 Core state 위에서 diagnose, recover, export, artifact check, conformance run, release handoff 준비를 수행합니다. |
| Roadmap 경계: 로드맵 | 로컬 kernel과 agency 증명이 안정된 뒤에만 later surface 또는 automation을 검토할 수 있음을 분리합니다. | 선택 capability는 담당자가 [로드맵 단계 승격 조건](../roadmap.md#단계-승격-조건)에 따라 exact contract와 fixture로 승격하기 전까지 read-only, display-only, metadata-only, 또는 artifact 후보 제공 전용으로 남습니다. |

## 무엇을 만드는가

유지보수자 인계가 첫 런타임 배치를 위한 구현 계획 준비 상태를 명시적으로 수락한 뒤, 하네스 구현은 이 저장소에서 내부 엔지니어링 점검로 시작합니다. 이것은 로컬 작업 장부와 판단 라우터를 위한 내부 kernel입니다. MVP-1 사용자 작업 루프가 그 장부를 사용자 가치로 보여 주는 첫 단계입니다. 내부 엔지니어링 점검은 authority loop를 확인하는 데 필요한 지속 로컬 상태, 쓰기 허가 기록, Run 기록, artifact/evidence ref, 구조화된 상태/막힘 출력만 유지합니다. MVP-1은 평범한 말로 작업 시작/이어가기, 범위/하지 않을 일/성공 기준, 최소 판단 요청/기록, 협력형 사전 쓰기 범위 확인, 실행/근거 참조 기록, 간결한 상태와 다음 안전한 행동, 닫기 막힘 표시, 작업 수락과 잔여 위험 수용의 최소 분리를 더합니다. 전체 작업 여정을 다루는 projection과 다듬어진 report는 later derived output으로 남깁니다. 제품 이력, 실행 가능한 확인, 리뷰, 사용자 판단은 기존 엔지니어링 절차에 남겨 둡니다. 사용자 판단권을 보존하는 로컬 권한 커널 원칙은 구현의 중심에 남습니다. Core가 기준 로컬 상태를 소유하고, 사용자 소유 판단은 사용자에게 남습니다. 초기 구현 가정은 명확한 내부 모듈을 가진 하나의 로컬 시스템이며, 분산 플랫폼으로 시작하지 않습니다.
...
내부 엔지니어링 점검은 모듈을 가진 단일 프로세스로 충분합니다. Core, projection, validation, 운영자 도구를 별도 서비스로 나눌 필요는 없습니다.
...
State store는 권한 루프의 기준 운영 상태를 보관합니다. 내부 엔지니어링 점검에서는 project와 Task state, scoped boundary, write authority, Run 하나, artifact/evidence ref 하나, 상태/막힘 출력에 필요한 최소 owner record를 뜻합니다. Judgment record, projection/reconcile tracking, full Evidence Manifest behavior, Eval, 수동 QA, 더 넓은 event history는 이후 단계 또는 owner-profile 범위입니다.
...
Artifact store는 느슨한 파일 덤프가 아닙니다. 하네스 상태를 뒷받침하는 artifact는 artifact owner path로 등록하고, 이를 사용하는 Task 또는 owner record와 연결해야 합니다. Exact artifact ref, integrity field, redaction state, retention rule은 [API Schema Core](../reference/api/schema-core.md#artifactref)와 [Storage와 DDL](../reference/storage-and-ddl.md#artifact-directory-layout)이 담당합니다.
...
MVP-1 사용자 작업 루프에서는 같은 API surface를 필요한 만큼만 넓힙니다. 평범한 요청이 추적되는 작업을 시작하거나 이어 가고, 작업 형태를 분류하며, 범위/하지 않을 일/성공 기준을 요약하고, 최소 판단 요청/기록을 만들며, Core를 통한 협력형 사전 쓰기 범위 확인, 실행/근거 참조 기록, 상태/다음 행동 출력, 근거 요약, 닫기 막힘 요약, 잔여 위험 표시, 민감 동작 승인 / 작업 수락 / 잔여 위험 수용 분리 표시를 제공하는 정도입니다.

Active MVP-1 public request와 response 규칙은 [MVP API](../reference/api/mvp-api.md)가 담당합니다. Shared envelope, ref, resource, staged value set은 [API Schema Core](../reference/api/schema-core.md)가 담당합니다. Later/profile-gated method는 [API Schema Later](../reference/api/schema-later.md)가 담당합니다.

State conflict와 idempotency replay 동작도 그 public tool 계약의 일부입니다. Build code는 [API Errors: Idempotency](../reference/api/errors.md#idempotency)와 [State conflict behavior](../reference/api/errors.md#state-conflict-behavior)를 사용하고, 지속 저장 세부 내용은 [Storage와 DDL](../reference/storage-and-ddl.md)에 맡깁니다.
...
읽기용 요약 출력은 그것이 의존하는 Core 원천 기록에서 파생합니다. 예를 들어 Task, gate, Run, artifact, 근거, Eval, QA, 그 밖의 owner record가 존재한 뒤 그 기록에서 나와야 합니다. 내부 엔지니어링 점검은 full projection renderer나 여러 projection kind를 요구하지 않습니다. 최소 상태/막힘 출력이면 충분합니다. Owner path가 이미 freshness/read fact를 만든 경우에만 이를 보고할 수 있지만, 읽기용 요약 렌더링이 첫 증명은 아닙니다. MVP-1 사용자 작업 루프는 현재 작업 상태, 다음 안전한 행동, 판단 요청/기록, 근거 요약, 닫기 막힘 요약, 작업 수락 표시, 잔여 위험 표시를 사용자가 이해할 만큼의 읽기 쉬운 요약 또는 card로 제공해야 합니다. 이 산출물은 사용자 경험을 지원할 뿐이며, MVP-1을 projection, evidence, reporting, Eval, QA, operations, dashboard pack으로 만들지 않습니다. 읽기용 요약 템플릿은 권한을 만들거나, 근거를 충족하거나, 상태를 대체하거나, 상태 모델을 정하거나, 첫 증명이 될 수 없습니다.

이후 프로필은 원천 기록이 존재하거나 변경되고 owner profile이 승격할 때 optional, future, diagnostic `ProjectionKind` value를 켤 수 있습니다. `ProjectionKind` value와 API 소유 지원 계층은 [API Schema Core](../reference/api/schema-core.md#shared-schemas)가 담당합니다.
...
Operator 진입점은 Core 동작 위에 놓이는 경로이지 두 번째 상태 모델이 아닙니다. 넓은 내부 엔지니어링 점검 요구사항이 아닙니다. 관련 stage 또는 owner profile이 범위에 넣을 때만 command-independent 기능으로 계획합니다.

| Stage | 운영자 capability boundary |
|---|---|
| 내부 엔지니어링 점검 | 최소 connect/register, 기본 상태 또는 진단 읽기, 첫 조각이 그 boundary를 요구할 때만 local MCP/API exposure. |
| MVP-1 사용자 작업 루프 | 현재 작업, 사용자 판단, 근거 상태, 닫기 막힘, 다음 안전한 행동, 작업 수락 필요 여부/상태, 민감 동작 승인 display, 잔여 위험 표시를 위한 user-facing status/next diagnostic. |
| 보증 프로필 | Verification, Manual QA, residual risk, 작업 수락, stewardship, context hygiene를 위한 assurance-profile diagnostic과 owner-path support. |
| 운영 프로필 | Full local operations입니다. Doctor/readiness, 읽기용 요약 refresh, reconcile, recover, export, artifact integrity, 담당 문서가 정의한 release handoff, suite가 materialized된 뒤 conformance run을 포함합니다. |
| 로드맵 | Remote/shared operations, dashboard, broad connector automation, team workflow, orchestration, higher automation은 승격 뒤에만 포함합니다. |
...
|---|---|
| Dashboard, hosted workflow UI, rich UI | 내부 엔지니어링 점검부터 운영 프로필까지 authority, 근거, close readiness, 작업 수락, 잔여 위험 수용이 아닙니다. |
| Broad connector ecosystem 또는 marketplace | 담당 문서가 승격하기 전까지 첫 local authority path를 넘어 staged delivery 범위를 넓히지 않습니다. |
| Context Index | 읽기 전용 로드맵 후보입니다. Authority 또는 read/write prerequisite가 아닙니다. |
| Browser QA Capture | 로드맵 후보입니다. Required automation, 수동 QA 대체물, 작업 수락 대체물이 아닙니다. |
| Cross-Surface Verification | 로드맵 automation 후보입니다. 분리 검증은 이 automation 없이 local path에서 먼저 증명할 수 있습니다. |
...
내부 엔지니어링 점검은 협력형(cooperative) 또는 탐지형(detective) guard/freeze 상태를 표시할 수 있고, existing scope, 이미 존재하는 경우의 Autonomy Boundary, `prepare_write` 동작을 통해 작업을 보류하거나 범위를 좁힐 수 있습니다. 접점 label만으로 저장된 guarantee level이 올라가지는 않습니다.

유용한 향후 capability라도 담당 문서가 capability profile, redaction/secret/PII policy, 필요한 경우 retention 또는 test-environment rule, fixture coverage, fallback 동작, 읽기용 요약을 기준으로 삼는 의존성 없음을 정의하기 전까지는 읽기 전용 표시, metadata, 기존 owner path를 위한 artifact 후보, fixture candidate로만 나타날 수 있습니다. 내부 엔지니어링 점검을 실행하거나, MVP-1 사용자 작업 루프를 완료하거나, staged-delivery close readiness를 주장하기 위한 전제 조건이 되어서는 안 됩니다.

## 첫 증명

첫 실행 목표는 내부 엔지니어링 점검입니다. 하네스가 하나의 권한 결정을 만들고 적용할 수 있음을 보여 주는 가장 작은 실행 가능한 경로입니다. 커널 스모크(Kernel Smoke)는 이 목표의 smoke check를 위한 좁은 향후 작성 label이지 전체 conformance suite가 아닙니다.

내부 엔지니어링 점검은 내부 authority loop를 증명하는 단계입니다. Product MVP, template 완성도, broad automation을 증명하는 단계가 아닙니다.
...
내부 엔지니어링 점검은 MVP-1 사용자 작업 루프가 아닙니다. 쓰기 권한 경로가 살아 있음을 증명하는 단계입니다. 문서 수준 수락 점검은 [첫 실행 가능한 조각](first-runnable-slice.md#문서-수준-수락-점검)을 사용하고, 정확한 fixture 의미는 [Conformance Fixtures 참조](../reference/conformance-fixtures.md#conformance-fixture-format)를 사용합니다.

## 첫 사용자 가치 증명

첫 사용자 가치 목표는 MVP-1 사용자 작업 루프입니다. 내부 엔지니어링 점검 뒤에 도달하는 목표이지 첫 실행 batch를 키워서 만들지 않습니다. 이 단계는 사용자가 평범한 말로 작업을 시작하면 하네스가 작업 범위, 대기 중인 판단 요청, 근거 요약, 닫기 막힘, 다음 안전한 행동, 작업 수락과 잔여 위험 수용 사이의 최소 분리를 로컬 기준으로 보존한다는 점을 보여 줍니다. 근거와 간결한 상태 출력은 지원 수단이지 전체 제품 시스템 자체가 아닙니다.

다음을 보여야 합니다.

- 평범한 사용자 언어가 Harness vocabulary를 요구하지 않고 추적되는 작업을 시작하거나 이어 갈 수 있음
- 작업 형태가 분류되고 작은 직접 변경과 추적되는 작업이 구분됨
- 작업이 범위, 하지 않을 일, 성공 기준, 근거 기대 수준, 닫기 준비 상태, 판단 경계로 정리됨
- 코드베이스나 상태에서 답할 수 있는 사실은 사용자에게 반복 질문하기 전에 확인됨
- 구체화 질문은 blocking question과 useful-but-not-blocking question을 분리하고 긴 questionnaire가 되지 않음
- 제품/UX 판단과 기술 판단을 분리해 제시할 수 있음
- 최소 판단 요청과 기록이 full-format judgment presentation machinery 없이 존재함
- 작은 직접 변경과 추적되는 작업이 authority를 우회하지 않고 서로 다른 절차 예산을 사용함
- 사전 쓰기 범위 확인은 `prepare_write`를 통한 협력형 Core 동작이며 OS-level 차단이나 권한 격리가 아님
- Run과 근거 참조가 `record_run`, registered artifact, 또는 최소 근거 요약 path로 기록됨
- ambiguous feature request는 premature implementation이 아니라 clarification으로 들어감
- 필수 근거 또는 user judgment가 없으면 close가 block됨
...
- MCP/Core를 사용할 수 없다는 상태가 authority를 만들어내지 않음
- 사용자의 작업 수락이 sensitive-action Approval과 잔여 위험 수용과 구분됨
- 잔여 위험 수용을 지원하는 경우, 이것이 작업 수락과 뚜렷하게 구분되어 보임
- 다섯 가지 작은 MVP 보기가 Core records에서 파생되며, template polish가 기준 권한이 되지 않아도 충분함
...
MVP-1은 대시보드, 호스팅 UI, 모든 판단 요청에 대한 full-format Decision Packet 출력, OS-level sandbox 또는 권한 격리, 상세 Evidence Manifest, 전체 report/export, runtime conformance suite, 운영/복구/인계 pack을 포함하지 않습니다.

## 강화된 로컬 기준 증명

[강화된 로컬 기준 목표](../reference/glossary.md#강화된-로컬-기준-목표)(hardened local reference target)는 MVP-1 사용자 작업 루프 이후 보증 프로필과 운영 프로필을 통해 도달하는 향후 reference 목표이지 첫 구현 batch가 아닙니다. 별도 stage, fixture profile, alternate implementation path도 아닙니다. Agent가 정직한 경계 안에서 행동하기 위해 필요한 나머지 conformance를 추가합니다.

- User judgment 품질과 사용자 판단 라우팅
- 민감 동작 승인, User Judgment, 쓰기 허가 기록의 분리
...
- broad automation을 로드맵에 두는 later 경계 확인
- 보증 프로필, 운영 프로필, 또는 승격된 로드맵 후보 fixture를 통한 fixture coverage
...
2. [단계별 전달 계획](mvp-plan.md): 내부 엔지니어링 점검부터 운영 프로필까지의 단계별 전달, 단계 경계, 서버 코딩 전 결정 기록을 확인합니다.
3. [첫 실행 가능한 조각](first-runnable-slice.md): 내부 엔지니어링 점검 구현 순서를 확인합니다.
4. [Runtime Walkthrough](runtime-walkthrough.md): request-to-close runtime path를 확인합니다.

로드맵 후보와 승격 규칙은 [로드맵](../roadmap.md)을 사용합니다.
...
- [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md), [API Schema Later](../reference/api/schema-later.md): public resource, tool, schema, error, 아티팩트 참조, idempotency, state conflict behavior, later/profile-gated API material.
