...
| [내부 엔지니어링 점검](engineering-checkpoint.md) | 첫 내부 authority-loop smoke. 제품 MVP도 아니고 사용자 가치 검증도 아닙니다. |
...
2. 내부 엔지니어링 점검을 구현합니다. 범위는 로컬 Core authority loop 하나입니다.
...
- 제품 명제: 하네스는 범위, 사용자 소유 판단, 근거, 검증 기대, QA 기대, 작업 수락, 잔여 위험, 닫기 준비 상태를 위한 로컬 authority record입니다.
...
- 내부 엔지니어링 점검은 가장 작은 로컬 Core authority loop를 증명합니다. 내부 구현 신뢰를 위한 점검이지 제품 MVP가 아닙니다.
...
| MVP-1 범위 | MVP-1은 사용자에게 보이는 범위, 판단, 근거, 닫기 막힘, 다음 행동, 작업 수락, 잔여 위험 표시를 포함하고 이후 프로필은 제외합니다. | [MVP-1 사용자 작업 루프](mvp-user-work-loop.md#mvp-1에-포함되는-것). |
| API 담당 문서 합의 | Active MVP-1 API, shared schema, resource, error, idempotency, state conflict behavior가 수락되어 있습니다. | [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md). |
| Storage 담당 문서 합의 | Minimal storage profile, runtime home layout, lock, artifact, migration, later-profile storage boundary가 수락되어 있습니다. | [Storage](../reference/storage.md). |
| Core 담당 문서 합의 | Task, scope, user judgment, `prepare_write`, Write Authorization, `record_run`, blocker, close semantics가 active stage 기준으로 수락되어 있습니다. | [Core Model 참조](../reference/core-model.md). |
| 보안 자세 | MVP-1 guarantee wording이 cooperative plus limited detective에 머물고 OS sandboxing, arbitrary-tool isolation, tamper-proof storage, default pre-tool blocking, permission isolation을 주장하지 않습니다. | [보안 참조](../reference/security.md). |
| Compact output 경계 | 내부 엔지니어링 점검은 status/blocker output을 사용하고, MVP-1은 projection을 authority로 만들지 않는 compact Core-derived view를 사용합니다. | [MVP-1 사용자 작업 루프](mvp-user-work-loop.md#mvp-1에-포함되는-것), [Projection과 Template 참조](../reference/projection-and-templates.md). |
...
Readiness가 수락된 뒤에는 넓은 사용자 경험보다 먼저 Core가 소유한 가장 작은 로컬 authority path를 구현합니다.
...
| Core | Authority loop 하나를 통해 canonical state를 변경합니다. | 두 번째 authority model 없이 user-facing work-loop state와 close/status path를 더합니다. | [Core Model 참조](../reference/core-model.md). |
| API | Minimal status/blocker read, setup path, `prepare_write`, `record_run`, artifact/evidence ref path. | Intake, status/next actions, user judgment, run/evidence, close를 위한 MVP-1 public tool/resource를 더합니다. | [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md). |
| Storage | Authority loop에 필요한 owner-approved minimal persistence만 사용합니다. | MVP-1 user judgment, evidence summary, blocker, compact output에 필요한 storage만 더합니다. | [Storage](../reference/storage.md). |
...
| Projection/view | Status/blocker output만 필요하며 full renderer는 필요하지 않습니다. | 다섯 compact Core-derived view로 user loop를 만족할 수 있습니다. | [Projection과 Template 참조](../reference/projection-and-templates.md), [Template 참조](../reference/templates/README.md). |
| 운영/conformance | 향후 smoke 작성 계획일 뿐입니다. | Runtime fixture가 materialize되기 전까지 behavior example로 다룹니다. | [운영과 Conformance 참조](../reference/operations-and-conformance.md), [Conformance Fixtures 참조](../reference/conformance-fixtures.md). |
...
- 로드맵 후보: dashboard, hosted workflow UI, Context Index, broad connector marketplace, Browser QA Capture automation, Cross-Surface Verification automation, preventive guard expansion, native hook expansion, Advanced Sidecar Watcher, Local Derived Metrics, team workflow, permission, orchestration, deployment, canary, rollback, production monitoring.

이후 capability는 담당 문서가 exact contract, fallback behavior, fixture/conformance expectation, 필요한 redaction/secret handling, projection-as-canonical이 아님을 정의한 뒤에만 authority loop를 읽거나 표시하거나 감쌀 수 있습니다.
...
2. [내부 엔지니어링 점검](engineering-checkpoint.md): 첫 내부 authority-loop smoke.
