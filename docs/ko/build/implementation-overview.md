...
로컬 커널은 조율과 권한의 기록이지 제품 저장소, 소스 관리, 테스트, 코드 리뷰, 대화, 사용자 소유 제품 판단과 기술 구조 판단을 대체하지 않습니다. 첫 경로는 상태/막힘 출력이 최소 권한 상태와 무엇이 빠졌는지를 설명할 수 있게 계획하되, ordinary-language intake, 닫기 막힘 요약, 작업 수락 display, residual-risk visibility, 사용자용 간결한 설명은 MVP-1와 이후 단계에 둡니다.
...
- 보증 프로필은 verification, 수동 QA, 잔여 위험 수용 close, 작업 수락 분리, stewardship, Decision Packet, Approval 분리, TDD, feedback-loop policy, context hygiene를 단단하게 만듭니다.
...
- 현재 open item에는 simplified judgment model and naming, `request_user_decision` vs `request_user_judgment`, `harness.next` vs `status.next_actions`, MVP-1 storage minimum, local access error taxonomy, compact status card scope, small direct change evidence requirement, 작업 수락/잔여 위험 minimal records가 포함됩니다.
...
| 보증 프로필 | MVP-1 user-value path가 verification, 수동 QA, 잔여 위험 수용 close, 작업 수락 분리, stewardship, profile별 Decision Packet 품질, Approval separation, TDD, feedback-loop policy, context hygiene를 정직한 경계 안에서 처리함을 증명합니다. | Fixture가 같은 Core record와 error를 통해 work가 진행, 검증, 수동 QA 요구, 작업 수락, 잔여 위험 수용, close될 수 있는지 보여 줍니다. |
