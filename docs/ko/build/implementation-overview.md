...
| 서버 코딩 전 결정 기록 | 예전 ledger의 MVP-1 scope 결정은 MVP-1 사용자 작업 루프 계획에서 문서 기준으로 해소되었습니다. 하지만 아직 server/runtime implementation decision은 코드 작성용으로 수락되지 않았습니다. | 영향을 받는 API, DDL, state transition, local access, status card, evidence, acceptance, risk behavior를 코딩하기 전에 [MVP-1 사용자 작업 루프 계획: 서버 코딩 전 필요한 구현 결정](mvp-user-work-loop.md#서버-코딩-전-필요한-구현-결정)의 문서 기준 결정을 maintainer가 수락하고, 아직 열린 구현 준비 항목을 해결하거나 명시적으로 미룹니다. |
...
- 문서 기준에서 해소된 server-coding decision-ledger 항목은 [MVP-1 사용자 작업 루프 계획: 서버 코딩 전 필요한 구현 결정](mvp-user-work-loop.md#서버-코딩-전-필요한-구현-결정)에 기록되어 있습니다.
...
첫 구현 계획은 MVP-1 사용자 작업 루프, 보증 프로필, 운영 프로필, roadmap automation이 아니라 내부 엔지니어링 점검 계획부터 시작한다는 뜻입니다. 편집 정리는 필요하지만 그것만으로 충분하지 않습니다. 스키마/설계 결정과 단계 경계 결정은 담당 문서에서 정리되거나, 서버 코딩 전에 MVP-1 사용자 작업 루프 계획에 단계 영향과 함께 기록되어야 합니다. 아래 조건이 모두 참일 때만 첫 구현 계획을 시작할 수 있습니다.
...
- 내부 엔지니어링 점검은 로컬, 단일 프로젝트, minimal authority loop 범위를 유지한다. 계획 점검 목록은 [내부 엔지니어링 점검](engineering-checkpoint.md)을 사용합니다.
...
Build 계층에서 이를 새로 설계하지 않습니다. Storage의 세부 내용은 [Storage](../reference/storage.md)이 담당합니다.
...
Artifact store는 느슨한 파일 덤프가 아닙니다. 하네스 상태를 뒷받침하는 artifact는 artifact owner path로 등록하고, 이를 사용하는 Task 또는 owner record와 연결해야 합니다. Exact artifact ref, integrity field, redaction state, retention rule은 [API Schema Core](../reference/api/schema-core.md#artifactref)와 [Storage](../reference/storage.md#artifact-directory-layout)이 담당합니다.
...
State conflict와 idempotency replay 동작도 그 public tool 계약의 일부입니다. Build code는 [API Errors: Idempotency](../reference/api/errors.md#idempotency)와 [State conflict behavior](../reference/api/errors.md#state-conflict-behavior)를 사용하고, 지속 저장 세부 내용은 [Storage](../reference/storage.md)에 맡깁니다.
...
[읽기용 요약(Projection) 참조](../reference/projection-and-templates.md#템플릿-구현-계층)는 Projection 권한 경계, source-record rule, freshness rule, 템플릿 구현 계층을 담당하고, [Template 참조](../reference/templates/README.md)는 rendered template body와 display card를 담당합니다.
...
내부 엔지니어링 점검은 MVP-1 사용자 작업 루프가 아닙니다. 쓰기 권한 경로가 살아 있음을 증명하는 단계입니다. 문서 수준 수락 점검은 [내부 엔지니어링 점검](engineering-checkpoint.md#문서-수준-수락-점검)을 사용하고, 정확한 fixture 의미는 [Conformance Fixtures 참조](../reference/conformance-fixtures.md#conformance-fixture-format)를 사용합니다.
...
2. [MVP-1 사용자 작업 루프 계획](mvp-user-work-loop.md): 내부 엔지니어링 점검부터 운영 프로필까지의 단계별 전달, 단계 경계, 서버 코딩 전 결정 기록을 확인합니다.
3. [내부 엔지니어링 점검](engineering-checkpoint.md): 내부 엔지니어링 점검 구현 순서를 확인합니다.
...
- [Core Model 참조](../reference/core-model.md): entity, gate, state logic, `prepare_write`, `close_task`.
...
- [Storage](../reference/storage.md): runtime layout, staged DDL, migration, lock, artifact와 later-profile baseline, projection job, validator-run candidate를 다룹니다.
...
- [향후 Fixtures](../later/future-fixtures.md): 그 자체로 early-stage requirement가 아닌 detailed later scenario candidate.
