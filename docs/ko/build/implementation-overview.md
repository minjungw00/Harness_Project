...
| v0.3 Agency Pack | 로컬 reference MVP가 user judgment, sensitive-action Approval separation, 분리 검증, 수동 QA, residual-risk visibility와 accepted-close semantics, stewardship, TDD, feedback-loop policy를 정직한 경계 안에서 처리함을 증명합니다. | Fixture가 같은 Core record와 error를 통해 work가 진행, verify, QA 요구, accept, close될 수 있는지 보여 줍니다. |
