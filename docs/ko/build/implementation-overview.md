...
내부 엔지니어링 점검을 먼저 만듭니다. 가장 작은 로컬 Core 권한 경로를 증명하며, 커널 스모크(Kernel Smoke)는 좁은 future smoke-check 작성 label입니다. 이것은 내부 smoke 단계이지 제품 MVP가 아닙니다. 그다음 MVP-1 사용자 작업 루프를 만들어 ordinary-language start/resume, work-shape classification, scope/non-goals/success criteria, minimal user judgment, evidence summary, 닫기 막힘, residual-risk visibility, 민감 동작 승인 / 작업 수락 / 잔여 위험 수용 분리 표시를 사용자가 볼 수 있게 합니다. 근거와 간결한 상태 출력은 이 경험을 지원하지만 full assurance, QA, Eval, reporting, operations, dashboard 시스템이 아닙니다. 보증 프로필과 운영 프로필이 그 경로를 단단하게 만듭니다.
...
로컬 커널은 조율과 권한의 기록이지 제품 저장소, 소스 관리, 테스트, 코드 리뷰, 대화, 사용자 소유 제품/UX 판단과 기술 판단을 대체하지 않습니다. 첫 경로는 상태/막힘 출력이 최소 권한 상태와 무엇이 빠졌는지를 설명할 수 있게 계획하되, ordinary-language intake, 닫기 막힘 요약, 작업 수락 display, residual-risk visibility, 사용자용 간결한 설명은 MVP-1와 이후 단계에 둡니다.
...
- 내부 엔지니어링 점검은 가장 작은 로컬 Core 권한 루프를 증명합니다. Kernel Smoke는 이 단계의 좁은 future smoke-check 작성 label입니다.
- MVP-1 사용자 작업 루프는 좁은 범위의 ordinary user value를 증명합니다. Start/resume, work-shape classification, scope/non-goals/success criteria, user-owned judgment routing, evidence summary, 닫기 막힘 요약, 작업 수락 display, residual-risk visibility가 여기에 속합니다.
...
- 읽기용 요약(Projection) 범위는 단계화되어 있습니다. 내부 엔지니어링 점검은 owner path가 이미 만든 경우에만 freshness/read fact를 노출할 수 있고, MVP-1은 Core-derived 간결한 상태 카드(compact status card)와 최소 사용자 읽기용 출력을 제공하며, detailed report/template은 승격되기 전까지 later-profile scope입니다.
...
- 서버/런타임 구현 결정: open decision-ledger 항목은 남아 있고, 문서에서 해소된 항목도 코딩 전 maintainer acceptance가 필요합니다. Maintainer review나 구현 준비 검토에서 추가 설계 쟁점이 발견될 수 있습니다.

서버 코딩 전 결정 기록 상태:

- Open server-coding decision-ledger 항목은 [단계별 전달 계획: 서버 코딩 전 필요한 구현 결정](mvp-plan.md#서버-코딩-전-필요한-구현-결정)에 기록되어 있습니다.
- 현재 open item에는 `harness.next` vs `status.next_actions`, MVP-1 storage minimum, local access error taxonomy, compact status card scope, small direct change evidence requirement, 작업 수락/잔여 위험 minimal records가 포함됩니다. Simplified judgment model과 `request_user_judgment` naming은 이 문서 기준에서 해소되었지만 아직 코드 작성용으로 수락된 것은 아닙니다.
- Server/runtime implementation decision은 아직 코드 작성용으로 공식 수락되지 않았습니다. 해당 behavior의 server code나 DDL을 바꾸기 전에 영향을 받는 ledger item을 해결, 수락, 또는 명시적으로 미룹니다.
...
- 단계가 일관적이다. 내부 엔지니어링 점검은 제품 MVP가 아니고, MVP-1 사용자 작업 루프는 첫 narrow user-value slice이지 full assurance/Eval/QA/reporting system이 아니며, 보증 프로필은 verification, QA, residual risk, work acceptance, stewardship를 단단하게 만들고, 운영 프로필은 operational handoff capability를 추가하며, 로드맵은 승격 전까지 향후 범위다.
...
MVP-1 사용자 작업 루프에서는 같은 API surface를 필요한 만큼만 넓힙니다. Ordinary request가 tracked work를 시작하거나 이어 가고, work shape를 분류하며, scope/non-goals/success criteria를 요약하고, minimal user judgment를 요청/기록하며, status/next output, evidence summary, 닫기 막힘 요약, residual-risk visibility, 민감 동작 승인 / 작업 수락 / 잔여 위험 수용 분리 표시를 제공하는 정도입니다.
...
| MVP-1 사용자 작업 루프 | 현재 작업, 사용자 판단, 근거 상태, 닫기 막힘, 작업 수락 필요 여부/상태, 민감 동작 승인 display, 잔여 위험 표시를 위한 user-facing status/next diagnostic. |
...
첫 사용자 가치 목표는 MVP-1 사용자 작업 루프입니다. 내부 엔지니어링 점검 뒤에 도달하는 목표이지 첫 실행 batch를 키워서 만들지 않습니다. 이 단계는 평범한 요청이 작은 범위의 하네스 작업으로 보이게 하고, 하네스가 scope, non-goals, success criteria, user-owned judgment, evidence summary, 닫기 막힘 요약, 작업 수락 display, residual-risk visibility를 로컬 권한 기록에 보존한다는 점을 사용자에게 보여 줍니다. 근거와 간결한 상태 출력은 지원 수단이지 full product system 자체가 아닙니다.
...
- minimal user judgment request와 record가 full-format judgment presentation machinery 없이 존재함
