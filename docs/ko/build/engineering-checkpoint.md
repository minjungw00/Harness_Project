...
5. 담당 문서가 정의한 `harness.prepare_write` path가 compatible, blocked, dry-run, replay behavior를 보여 준다. Build가 request/response detail을 다시 정의하지 않는다.
...
8. Artifact/evidence ref는 owner path를 통해 integrity, redaction, owner relation, safe omission/block metadata를 다룬다.
9. Evidence summary가 registered ref에서 최소 coverage와 gap을 보여줄 수 있다.
...
| 6. Artifact/evidence ref | Durable artifact 또는 evidence ref 하나를 owner path로 등록합니다. | Run 또는 minimal owner relation이 담당 문서가 정의한 artifact/evidence contract에 따라 그 ref를 cite할 수 있습니다. | [API Schema Core](../reference/api/schema-core.md#artifactref), [Storage](../reference/storage.md). |
...
- Active path에서 support가 필요한 경우 scope, idempotency/replay, Write Authorization, observed scope mismatch, raw secret artifact input, missing artifact/evidence support에 대해 담당 문서가 정의한 blocker와 error case를 다룬다.
- Status text, generated prose, projection-like output은 모두 Core record에서 파생된 read로 취급하며 fixture proof로 보지 않는다.
- Future smoke check는 [Conformance Fixtures 참조](../reference/conformance-fixtures.md)가 담당하는 structured draft shape을 사용합니다. Build가 fixture schema를 새로 정의하지 않습니다.
...
Build 문서는 fixture 파일, runner code, generated conformance artifact, pass/fail result를 만들지 않습니다. Runtime conformance는 구현과 fixture materialization 이후에만 시작됩니다.
