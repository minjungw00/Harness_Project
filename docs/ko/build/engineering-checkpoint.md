...
- Active path에서 support가 필요한 경우 missing scope, out-of-scope intended work, product-write Run의 missing Write Authorization, consumed Write Authorization 재사용, missing artifact/evidence support에 대해 구조화된 차단 사유나 non-`allowed` response를 반환한다.
