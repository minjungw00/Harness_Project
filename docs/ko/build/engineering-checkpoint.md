...
- 제안된 첫 조각이 제품 MVP나 MVP-1 사용자 작업 루프로 커지지 않을 만큼 작은지 검토할 때.

## 읽기 전에

[구현 개요](implementation-overview.md)를 먼저 읽고 [문서 수락 상태](implementation-overview.md#문서-수락-상태)를 확인합니다. 그 인계 표가 Build 진입 기준입니다. 유지보수자가 첫 런타임 배치를 위한 구현 계획 준비 상태를 수락하기 전까지 이 조각은 계획 전용입니다. Storage의 세부 내용은 [Storage](../reference/storage.md)를 봅니다. 이 조각 이후의 단계별 전달은 [MVP-1 사용자 작업 루프 계획](mvp-user-work-loop.md)을 사용합니다. 로드맵 후보는 [로드맵](../roadmap.md)을 봅니다.
...
첫 조각은 Harness 상태가 로컬에 있으며, 지속적이고, 기준이 된다는 점을 보여야 합니다. 하지만 사용자에게 보이는 제품 경험 전체를 증명하려고 하면 안 됩니다. `prepare_write`는 제품 파일 쓰기 허가 판단 지점으로, Write Authorization은 지속적이며 한 번만 쓸 수 있는 기록으로, `record_run`은 호환되는 Run 하나가 권한을 소비하는 곳으로, status/blocker 출력은 missing scope, missing write authority, 또는 missing artifact/evidence support를 구조화된 막힘으로 보고하는 곳으로 유지합니다. Owner path가 이미 그 방식을 가장 단순한 blocker 응답으로 만든 경우 `close_task` smoke를 사용할 수 있지만, 내부 엔지니어링 점검은 작업 수락이나 잔여 위험 close semantics를 증명하지 않습니다.

정확한 계약은 [Core Model 참조](../reference/core-model.md#prepare_write), [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md)를 사용합니다.

API staging에서는 API [Stage Profile Manifest](../reference/api/schema-core.md#stage-profile-manifest)에서 시작하고 내부 엔지니어링 점검 surface만 사용합니다. 범위는 minimal `harness.status` status/blocker read, `harness.prepare_write`, `harness.record_run`, owner-valid Task/scope setup path 하나, optional narrow `harness.close_task` blocker smoke입니다. 별도 `harness.next` method는 later/compatibility material이며 first-slice exit criterion이 아닙니다. Later-profile field는 해당 profile이 active일 때 exact하게 유지되지만 first-slice exit criteria는 아닙니다.

## 목표

내부 엔지니어링 점검을 계획합니다. 하나의 Task에 대해 로컬 권한을 증명하는 가장 작은 Harness 경로입니다.
...
이 문서는 특정 command에 묶이지 않는 구현 안내서입니다. CLI 문법이 아니라 기능과 관찰 가능한 동작을 설명합니다. 여기에 전체 DDL을 포함하거나 반복하지 않습니다. Storage의 세부 내용은 [Storage](../reference/storage.md)이 담당합니다.

Storage planning에서는 내부 엔지니어링 점검에 [MVP-1 minimal storage schema](../reference/storage.md#mvp-1-minimal-storage-schema)의 최소 authority subset만 사용합니다. Full Approval record, Evidence Manifest, Manual QA, Eval, projection job, reconcile item, validator run, Journey record, diagnostic 같은 later storage profile은 first-slice requirement가 아닙니다.
...
향후 내부 엔지니어링 점검 implementation이 생기면, 구현자는 임시 제품 저장소에 대해 로컬 Harness 프로세스를 실행하고 다음 흐름을 관찰할 수 있어야 합니다.
...
이 흐름을 통과하면 내부 엔지니어링 점검이 동작한다는 뜻입니다. 사용자가 Harness 가치를 경험했다는 뜻은 아닙니다. MVP-1 사용자 작업 루프는 평범한 요청이 추적되는 작업으로 시작/재개되고, 하네스가 범위, 하지 않을 일, 성공 기준, 대기 중인 판단 요청, 근거 요약, 닫기 막힘, 다음 안전한 행동, 작업 수락과 잔여 위험 수용 사이의 최소 분리를 보존할 때 시작됩니다.

## 문서 수준 수락 점검

Executable fixture가 생기기 전에는 이 점검으로 계획된 내부 엔지니어링 점검을 리뷰하고, [커널 스모크(Kernel Smoke) Authoring Queue](../reference/conformance-fixtures.md#kernel-smoke-authoring-queue)에 매핑할 때 다시 사용합니다. 이는 planning check이며 fixture body field, schema 추가, DDL, runtime authorization이 아닙니다.

제안된 내부 엔지니어링 점검은 다음을 만족할 때 적절합니다.
...
- Projection-like output은 내부 엔지니어링 점검에서 status/blocker 출력으로 취급한다. Full projection renderer, 여러 projection kind, detailed template은 필요하지 않다.
...
- 제외된 capability는 내부 엔지니어링 점검의 failed requirement가 아니라 아직 첫 조각이 증명하지 않은 capability로 이름 붙인다.
...
Owner contract: runtime home layout과 minimal storage schema는 [Storage](../reference/storage.md#mvp-1-minimal-storage-schema)이 담당합니다. Local space와 guarantee-level placement는 [런타임 아키텍처 참조](../reference/runtime-architecture.md)가 담당하고, guarantee-level 의미는 [보안 참조](../reference/security.md#정직한-guarantee-display)가 담당하며, connector reporting은 [Agent 통합 참조](../reference/agent-integration.md)가 담당합니다.
...
- Natural-language intake quality, work-shape classification, small direct work와 tracked work의 구분은 MVP-1 사용자 작업 루프에 둔다.
...
Owner contract: Task lifecycle과 state conflict behavior는 [Core Model 참조](../reference/core-model.md#task), [Lifecycle and transitions](../reference/core-model.md#lifecycle-and-transitions), [API Errors](../reference/api/errors.md#state-conflict-behavior)가 담당합니다.
...
- Full Discovery와 사용자에게 보이는 work-shape routing은 MVP-1 사용자 작업 루프에 둔다.
...
Owner contract: Change Unit과 Autonomy Boundary semantics는 [Core Model 참조](../reference/core-model.md#change-unit)와 [Autonomy Boundary](../reference/core-model.md#autonomy-boundary)가 담당합니다.
...
Owner contract: write-gate semantics는 [Core Model 참조: prepare_write](../reference/core-model.md#prepare_write)가 담당합니다. Public request/response shape와 error precedence는 [`harness.prepare_write`](../reference/api/mvp-api.md#harnessprepare_write)와 [Primary Error Code Precedence](../reference/api/errors.md#primary-error-code-precedence)가 담당합니다.
...
Owner contract: Run semantics는 [Core Model 참조: record_run](../reference/core-model.md#record_run)이 담당합니다. Public schema는 [`harness.record_run`](../reference/api/mvp-api.md#harnessrecord_run)이 담당합니다. Transaction order는 [State transaction flow](../reference/runtime-architecture.md#state-transaction-flow)가 담당합니다.

### 6. Artifact Or Evidence Link

Owner path를 통해 지속적으로 보관할 evidence file 하나 또는 동등한 evidence ref 하나를 등록합니다. 내부 엔지니어링 점검은 reference와 owner link만 필요하며, full Evidence Manifest model이나 rendered `EVIDENCE-MANIFEST` output은 필요하지 않습니다.
...
Owner contract: artifact ref는 [ArtifactRef](../reference/api/schema-core.md#artifactref)가 담당합니다. Storage layout과 registration detail은 [Artifact directory layout](../reference/storage.md#artifact-directory-layout) 및 [Artifact Registration Contract](../reference/storage.md#artifact-registration-contract)가 담당합니다.
...
Owner contract: status와 `status.next_actions` schema는 [`harness.status`](../reference/api/mvp-api.md#harnessstatus)가 담당합니다. 별도 [`harness.next`](../reference/api/schema-later.md#harnessnext)는 later/compatibility material입니다. Close behavior는 [Core Model 참조: close_task](../reference/core-model.md#close_task)가 담당합니다.

## 이것이 증명하는 것

내부 엔지니어링 점검은 다음을 증명합니다.
...
이 조각은 아래 항목을 아직 증명하지 않습니다. 이들은 stage boundary이지 failed 내부 엔지니어링 점검 requirement가 아닙니다.

| 이후 단계 | 내부 엔지니어링 점검이 아직 증명하지 않는 것 |
|---|---|
| MVP-1 사용자 작업 루프 | 평범한 말로 작업 시작/이어가기, 작업 형태 분류, 자연어 접수 품질, 범위/하지 않을 일/성공 기준 요약, 최소 판단 요청/기록, 제품/UX 판단과 아키텍처 판단의 구분 표시, 협력형 사전 쓰기 범위 확인, small direct와 tracked work의 서로 다른 절차 예산, 실행/근거 참조 기록, 근거 요약, 닫기 막힘 요약, 다음 안전한 행동, 잔여 위험 표시, 작업 수락 display, 민감 동작 승인 display, 잔여 위험 수용 display, Core에서 파생한 간결한 상태 카드 충분성. |
...
- [Core Model 참조](../reference/core-model.md): Task, Change Unit, User Judgment, gate, `prepare_write`, Write Authorization, `record_run` semantics, `close_task`.
...
- [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md): public resource, tool envelope, request/response schema, error taxonomy, 아티팩트 참조, `ProjectionKind`.
- [Storage](../reference/storage.md): runtime layout, staged schema profile, migration, lock, artifact와 later-profile baseline, projection job, validator-run candidate를 다룹니다.
...
- [향후 Fixtures](../later/future-fixtures.md): 내부 엔지니어링 점검 requirement가 아닌 detailed later scenario candidate.
