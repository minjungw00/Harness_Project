...
이 문서는 첫 내부 하네스 서버 구현 조각인 내부 엔지니어링 점검을 계획할 때 사용합니다. 범위는 로컬 Core authority loop의 smoke입니다. 제품 MVP도 아니고 MVP-1 사용자 작업 루프도 아니며, 오늘 런타임이 존재한다는 증거도 아닙니다.
...
- 가장 작은 향후 runnable slice가 필요할 때.
- 첫 batch가 사용자 가치 MVP 범위로 커지지 않았는지 확인할 때.
- API, DDL, fixture 정의를 복사하지 않고 checkpoint 담당 문서만 찾고 싶을 때.

## 핵심 생각

내부 엔지니어링 점검은 하네스가 Core를 통해 로컬 authority record 하나를 유지할 수 있음을 증명합니다.

1. Local project 하나를 안다.
2. Active Task 하나가 있다.
3. 의도한 write를 위한 scope boundary 하나가 있다.
4. `prepare_write`가 맞지 않는 작업은 거절하고 맞는 작업은 허용한다.
5. Durable single-use Write Authorization 하나가 만들어진다.
6. `record_run`이 compatible Run 하나를 기록하고 authorization을 한 번 소비한다.
7. Artifact/evidence ref 하나가 owner path로 등록되고 연결된다.
8. Status/blocker output이 Core 상태를 변경하지 않고 읽는다.

여기까지입니다. 이 checkpoint는 사용자-facing value를 더하기 전에 authority loop가 살아 있는지 확인하기 위해 존재합니다.
...
- detached verification, Eval, Manual QA, work acceptance, residual-risk acceptance, full close semantics
...
제안된 첫 slice가 통과하려면 이런 capability가 필요하다면 더 이상 내부 엔지니어링 점검이 아닙니다.
...
| 3. Basic scope 하나 | Intended product write 하나를 제한할 수 있는 가장 작은 scope boundary를 붙입니다. | Compatible scope가 없으면 product write가 write authority를 받을 수 없습니다. | [Core Model 참조](../reference/core-model.md). |
| 4. `prepare_write` allow/block | Intended write를 owner pre-write scope check로 보냅니다. | Missing scope나 out-of-scope work는 block되고, compatible work는 Write Authorization ref를 돌려줍니다. | [Core Model 참조](../reference/core-model.md#prepare_write), [`harness.prepare_write`](../reference/api/mvp-api.md#harnessprepare_write), [API Errors](../reference/api/errors.md). |
...
- Local, single-project, one Task authority loop에 집중한다.
- [문서 수락 상태](implementation-overview.md#문서-수락-상태)가 구현 계획 준비 상태를 수락하기 전까지 계획 전용으로 남는다.
- `prepare_write`, Write Authorization, `record_run`, artifact/evidence ref, structured status/blocker output을 지나는 scoped write path 하나를 증명한다.
...
- Status text, generated prose, projection-like output은 모두 Core record downstream read로 취급한다.
...
- Product-write authority 전에 scope가 필요하다.
- Write Authorization은 durable하고 single-use다.
- `record_run`은 write authority를 소비하고 observed work를 기록한다.
...
MVP-1 사용자 작업 루프는 이 checkpoint 이후에 시작합니다. 거기에는 ordinary-language start/resume, work-shape classification, scope/non-goals/success criteria, minimal user judgment, evidence summary, close blocker summary, next safe action, residual-risk visibility, Sensitive action approval/work acceptance/risk acceptance의 분리 표시가 추가됩니다.
