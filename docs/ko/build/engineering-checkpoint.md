...
- Active path에서 support가 필요한 경우 missing scope, out-of-scope intended work, product-write Run의 missing Write Authorization, consumed Write Authorization 재사용, missing artifact/evidence support에 대해 구조화된 거절이나 차단 사유를 반환한다.
...
- 하네스와 호환되는 제품 쓰기 권한 경로를 만들기 전에 scope가 필요하다.
