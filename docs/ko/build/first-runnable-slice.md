...
[구현 개요](implementation-overview.md)를 먼저 읽고 [문서 수락 상태](implementation-overview.md#문서-승인-상태)를 확인합니다. 그 인계 표가 Build 진입 기준입니다. 유지보수자가 첫 런타임 배치를 위한 구현 계획 준비 상태를 수락하기 전까지 이 조각은 계획 전용입니다. Storage와 DDL의 세부 내용은 [Storage와 DDL](../reference/storage-and-ddl.md)을 봅니다. 이 조각 이후의 단계별 전달은 [MVP 계획](mvp-plan.md)을 사용합니다. v1+ Expansion 후보는 [로드맵](../roadmap.md)을 봅니다.
...
- [문서 수락 상태](implementation-overview.md#문서-승인-상태)가 첫 런타임 배치에 대한 구현 계획 준비 상태를 명시적으로 수락하기 전까지 계획 전용으로 남는다.
