...
- missing authority를 차단할 만큼 sensitive-action Approval과 Decision Packet 요구사항을 확인한다.
- write 전에 적용되는 design-policy precondition을 확인한다.
- 접점 능력을 정직하게 확인하고 cooperative 또는 detective limit을 표시한다.
- scope, state version, sensitive-action Approval, decision, baseline, 능력이 맞지 않으면 차단 조건을 반환한다.
...
- gate, evidence, Decision Packet, sensitive-action Approval state, residual-risk visibility, QA, verification, acceptance를 최소 수준에서 점검할 만큼의 `close_task` state logic을 계획한다.
