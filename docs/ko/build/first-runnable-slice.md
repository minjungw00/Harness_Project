...
Task 하나가 가장 작은 Core authority record를 통과할 수 있음을 증명합니다. 그 기록은 project registration, Reference contract상 필요한 경우 Change Unit owner shape로 표현되는 기본 scope 하나, write authorization decision 하나, authorized Run 하나, evidence link 하나, structured blocker/status response 하나로 구성됩니다.
...
- selected claim을 support하거나 fail할 수 있는 minimal 근거 상태 하나
...
8. Status와 next read가 상태를 변경하지 않고 current Task, scope, write authority, 근거 상태, blocker를 보여 준다.
...
- Active Task, 기본 scope 하나, `prepare_write` allow/block, durable single-use Write Authorization, `record_run` consumption, artifact/evidence link, read-only status/next, structured blocker/status response로 이루어진 scoped write path 하나만 증명한다.
- Missing scope, out-of-scope intended path, product-write Run의 missing Write Authorization, consumed Write Authorization 재사용, required 근거 누락, required seeded user judgment 누락처럼 authority가 부족한 경우를 block하거나 refuse한다.
- Status read, generated prose, projection output이 있다면 모두 Core record의 downstream으로 유지한다. 이들은 write를 authorize하거나, 근거를 충족하거나, work를 close하거나, 읽히는 것만으로 state를 repair하지 않는다.
...
- 나중에 Task-scoped action이 사용할 local project 하나와 reference surface 하나를 찾을 수 있게 만든다.
- Runtime home, registry, project state, artifact store, 정적 프로젝트 설정은 storage owner path에 둔다.
- Unregistered, registered-idle, active-work state를 구분하는 read-only status를 제공한다.
...
- Status와 이후 Core action이 Task를 참조할 수 있을 만큼 current state와 event history를 유지한다.
...
- 첫 selected claim에 필요한 evidence expectation만 유지한다.
...
- Compatible scoped write 하나만 허용하거나 owner-shaped blocker를 반환한다.
...
- Compatible Write Authorization을 한 번만 consume한다.
...
- Required 근거가 없으면 close/status output이 block된다.
...
- Current Task, scope, write-authority summary, evidence status, blockers, next safe action을 canonical record에서 반환한다.
- Fixture가 prose matching 없이 비교할 수 있을 만큼 blocker identity를 structured하게 유지한다.
...
|---|---|
| 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP) | Natural-language intake quality, Discovery, product/UX versus architecture judgment presentation, small-change versus tracked-work budgets, 잔여 위험 표시, 작업 수락의 분리, user-facing projection/card sufficiency. |
