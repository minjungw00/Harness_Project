...
이 문서는 구현 계획 문서입니다. 문서 세트가 구현 계획에 사용할 수 있다고 승인되기 전에는 runtime/server 구현, 생성된 운영 파일, 실행 가능한 fixture 파일, 런타임 데이터를 만들라는 뜻이 아닙니다. 첫 실행 목표는 코어 권한 조각(v0.1 Core Authority Slice)이며, 커널 스모크(Kernel Smoke)는 이 조각을 위한 좁은 conformance 작성 프로파일입니다. 이것은 내부 구현 단계이지 사용자 대상 MVP가 아닙니다. 첫 제품 MVP 목표는 사용자 대상 하네스 MVP(v0.2 User-Facing Harness MVP)입니다.
...
[구현 개요](implementation-overview.md)를 먼저 읽고 [문서 승인 상태](implementation-overview.md#문서-승인-상태)를 확인합니다. 그 인계 표가 Build 진입 기준입니다. Maintainer가 첫 runtime batch 계획을 승인하기 전까지 이 조각은 계획 전용입니다. Storage와 DDL의 세부 내용은 [Storage와 DDL](../reference/storage-and-ddl.md)을 봅니다. 이 조각 이후의 단계별 전달은 [MVP 계획](mvp-plan.md)을 사용합니다. v1+ Expansion 후보는 [로드맵](../roadmap.md)을 봅니다.

## 핵심 생각

Task 하나가 가장 작은 Core 권한 기록을 통과할 수 있음을 증명합니다. 그 기록은 프로젝트 등록, Reference 계약상 필요한 경우 Change Unit 소유자 형태로 표현되는 기본 scope 하나, 쓰기 허가 판단 하나, 허가된 Run 하나, 근거 링크 하나, 구조화된 막힘/상태 응답 하나로 구성됩니다.

첫 조각은 하네스 상태가 로컬에 있으며, 지속적이고, 기준이 된다는 점을 보여야 합니다. 하지만 사용자에게 보이는 제품 경험 전체를 증명하려고 하면 안 됩니다. `prepare_write`는 제품 파일 쓰기 허가 판단 지점으로, Write Authorization은 지속적이며 한 번만 쓸 수 있는 기록으로, `record_run`은 호환되는 Run 하나가 권한을 소비하는 곳으로, `close_task` 또는 상태 응답은 누락된 근거나 필요한 판단을 구조화된 막힘으로 보고하는 곳으로 유지합니다.

정확한 계약은 [커널 참조](../reference/kernel.md#prepare_write)와 [MCP API와 스키마](../reference/mcp-api-and-schemas.md#public-tools)를 사용합니다.

## 목표

코어 권한 조각(v0.1 Core Authority Slice)을 계획합니다. 하나의 Task에 대해 로컬 권한을 증명하는 가장 작은 Harness 경로입니다.
...
- 선택한 주장을 뒷받침하거나 실패로 판정할 수 있는 최소 근거 상태 하나
- `status`/`next` 읽기 전용 응답 하나
- scope, evidence, 또는 필요한 seeded user judgment가 없을 때 구조화된 막힘/상태 응답 하나
...
8. Status와 next 읽기가 상태를 변경하지 않고 현재 Task, scope, 쓰기 권한, 근거 상태, blocker를 보여 준다.
9. 필요한 근거, scope, 또는 필요한 seeded user judgment가 없으면 close 또는 상태 출력이 구조화된 막힘을 반환한다.

이 흐름을 통과하면 코어 권한 조각(v0.1 Core Authority Slice)이 동작한다는 뜻입니다. 사용자가 Harness MVP를 경험했다는 뜻은 아닙니다. 사용자 대상 MVP는 평범한 요청이 scope, judgment, 근거, close-readiness, 작업 수락, 잔여 위험 언어로 정리될 때 시작됩니다.
...
- 로컬, 단일 프로젝트, 단일 reference surface 범위를 유지하고 Task 하나의 권한 루프에 집중한다.
- [문서 승인 상태](implementation-overview.md#문서-승인-상태)가 첫 runtime batch 계획을 명시적으로 허용하기 전까지 계획 전용으로 남는다.
- Active Task, 기본 scope 하나, `prepare_write` allow/block, 지속적이며 한 번만 쓸 수 있는 Write Authorization, `record_run` 소비, artifact/evidence link, `status`/`next` 읽기 전용 응답, 구조화된 막힘/상태 응답으로 이루어진 scoped write path 하나만 증명한다.
- Missing scope, out-of-scope intended path, product-write Run의 missing Write Authorization, consumed Write Authorization 재사용, required 근거 누락, required seeded user judgment 누락처럼 권한이 부족한 경우를 block하거나 refuse한다.
- Status read, generated prose, projection output이 있다면 모두 Core record에서 파생된 것으로 유지한다. 이들은 write를 허가하거나, 근거를 충족하거나, work를 close하거나, 읽히는 것만으로 상태를 복구하지 않는다.
...
아래 Build 순서는 문서 승인 이후를 위한 계획 순서입니다. Heading은 future runtime batch를 실행하기 쉽도록 구현 동사를 사용하지만, 이 문서는 문서 승인 전에 runtime/server 구현, 생성된 운영 파일, 실행 가능한 fixture, 런타임 데이터를 만들라는 뜻이 아닙니다.
...
- 나중에 Task-scoped action이 사용할 로컬 project 하나와 reference surface 하나를 찾을 수 있게 만든다.
...
Owner contract: runtime home layout과 DDL은 [Storage와 DDL](../reference/storage-and-ddl.md)이 담당합니다. Local space와 surface guarantee는 [런타임 아키텍처 참조](../reference/runtime-architecture.md)와 [Agent 통합 참조](../reference/agent-integration.md)가 담당합니다.
...
- Status와 이후 Core action이 Task를 참조할 수 있을 만큼 현재 상태와 event history를 유지한다.
...
- 첫 selected claim에 필요한 근거 기대 수준만 유지한다.
...
Owner path를 통해 지속적으로 보관할 evidence file 하나 또는 동등한 evidence ref 하나를 등록합니다.
...
- Selected claim 하나를 Run과 artifact/evidence ref에 매핑한다.
- Supported, partial, insufficient evidence의 구분은 owner gate semantics를 통해 유지한다.
- Chat text, status prose, projection prose를 근거로 취급하지 않는다.

완료 기준:

- Completed Run이 supported 또는 partial 근거 상태를 만들 수 있다.
- 필요한 근거가 없으면 close/status 출력이 block된다.

Owner contract: evidence record와 close-support category는 [Evidence Manifest](../reference/kernel.md#evidence-manifest), [Evidence Gate](../reference/kernel.md#evidence-gate), [`harness.close_task`](../reference/mcp-api-and-schemas.md#harnessclose_task)가 담당합니다.

### 8. Status, Next, And Structured Blockers

현재 작업 상태와 안전한 다음 행동을 변경 없이 제공하고, 첫 조각이 close 또는 proceed할 수 없을 때 구조화된 막힘을 반환합니다.

계획 초점:

- 현재 Task, scope, 쓰기 권한 요약, 근거 상태, blockers, 안전한 다음 행동을 canonical record에서 반환한다.
- Fixture가 prose matching 없이 비교할 수 있을 만큼 blocker identity를 구조화된 형태로 유지한다.
- Read 동작에서 event를 추가하거나, projection을 대기열에 넣거나, artifact를 만들거나, gate를 충족하거나, write를 authorize하거나, Task를 닫지 않는다.

완료 기준:

- 다른 action이 상태를 바꾸지 않는 한 반복 status/next 읽기가 같은 state version을 반환한다.
- 구조화된 막힘을 prose matching 없이 fixture가 비교할 수 있다.
...
- Status와 next는 읽기 전용이다.
- 구조화된 막힘이 missing scope, evidence, authorization, seeded required user judgment를 보고할 수 있다.
