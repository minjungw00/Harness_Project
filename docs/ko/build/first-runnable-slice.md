...
- Task가 생성되거나 변경될 때 task_events에 추가한다.
...
- 현재 기록 갱신과 같은 transaction에서 task_events에 추가한다.
...
- Unresolved blocking Decision Packet이 있는 Task는 successful close 상태가 될 수 없다.
...
- no-active-task 상태 조회가 idle state를 반환하고 event를 추가하지 않음
...
- projection failure가 committed state를 롤백하지 않음
