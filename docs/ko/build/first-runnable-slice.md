...
이 문서는 Build 개요를 구현자가 가장 먼저 계획해야 하는 작은 실행 가능한 제품 MVP로 바꿔 줍니다.
...
더 넓은 pack을 만들기 전에, Task 하나가 Core state, `task_events`, artifact 경로 위에서 범위가 정해진 쓰기 권한, Run 기록, artifact로 뒷받침되는 근거, basic Decision Packet blocker, status/next, 최소 projection 최신성 또는 enqueue, 닫기 차단 조건을 통과하는지 증명합니다.

루프는 의도적으로 작게 유지합니다. `prepare_write`가 제품 파일 쓰기 권한을 판단하고, 반환된 Write Authorization은 durable하고 single-use이며, `record_run`은 하나의 compatible direct Run 또는 implementation Run에 대해 이를 consume하면서 observed changes와 artifact를 기록하고, `close_task`는 structured blocker와 함께 완료 여부를 판단합니다. 정확한 계약은 [커널 참조](../reference/kernel.md#prepare_write)와 [MCP API와 스키마](../reference/mcp-api-and-schemas.md#public-tools)를 사용합니다.

## 목표

v0.1 Kernel MVP를 계획합니다. 하나의 로컬 Task에 대해 Harness가 권한을 행사할 수 있음을 증명하는 가장 작은 경로입니다. 이 조각은 프로젝트 하나, direct/work/advisor mode basics를 가진 Task 하나, active Change Unit 하나, basic Decision Packet lifecycle 하나, 허용된 `prepare_write` decision 하나, single-use Write Authorization 하나, 이를 consume하는 compatible recorded Run 하나, 등록된 artifact 하나, 최소 Evidence Manifest 하나, status/next read, minimal `TASK` projection 또는 projection enqueue 하나, structured close blocker 하나를 만들어야 합니다.
...
2. Task가 현재 상태, 초기 gate, direct/work/advisor mode basics를 가진다.
3. Change Unit이 의도한 제품 파일 쓰기의 범위를 정한다.
4. Basic Decision Packet은 request, record될 수 있고, status/next에 표시되며, unresolved이면 blocker로 쓰인다.
5. 범위 밖의 쓰기는 차단된다.
6. 범위 안의 쓰기는 `prepare_write`에서 durable single-use Write Authorization을 받는다.
7. Work task에 대한 direct Run 또는 implementation Run이 쓰기를 기록하고 그 Write Authorization을 한 번 사용한 것으로 기록한다.
8. diff나 log artifact가 등록되고 Run에 연결된다.
9. 최소 Evidence Manifest가 Run과 artifact를 참조한다.
10. Status와 next read는 상태를 변경하지 않고 현재 Task, gate, 쓰기 권한, 근거 상태, Decision Packet refs, 차단 조건, projection 최신성을 보여 준다.
11. `TASK` projection이 최신이거나 렌더링을 위해 durable queue에 들어간다.
12. 근거 또는 결정 요구사항이 아직 충족되지 않았으면 `close_task`가 structured blocker와 함께 차단된다.

이 흐름을 통과하면 v0.1 Kernel MVP가 동작한다는 뜻입니다. Agency-Hardened MVP가 완료되었다는 뜻도 아니고, later automation을 MVP로 끌어온다는 뜻도 아닙니다.

관찰 결과는 단순해도 됩니다. 사용자 또는 운영자는 현재 Task, mode basics, 쓰기가 차단되거나 허용된 이유, 어떤 Decision Packet이 unresolved 또는 resolved인지, 어떤 Write Authorization이 사용되었는지, 어떤 artifact가 Run을 뒷받침하는지, Evidence Manifest가 충분한지, `TASK` projection이 최신이거나 대기열에 있는지, 다음 safe action이 무엇인지, close가 왜 아직 막히는지를 볼 수 있어야 합니다.

## 문서 수준 승인 점검

Executable fixture가 생기기 전에는 이 점검으로 계획된 v0.1 Kernel MVP 조각을 리뷰하고, 이후 [Kernel Smoke Authoring Queue](../reference/conformance-fixtures.md#kernel-smoke-authoring-queue)에 매핑할 때 다시 사용합니다. 이는 planning check이며 fixture body field, schema 추가, DDL, runtime authorization이 아닙니다.
...
- Active Task, direct/work/advisor mode basics, active Change Unit, basic Decision Packet lifecycle, `prepare_write` allow/block, durable single-use Write Authorization, `record_run` consumption, artifact registration, minimal Evidence Manifest, status/next, minimal `TASK` projection freshness 또는 enqueue, structured close blocker로 이루어진 scoped write path 하나만 증명한다.
...
- Strict fixture body shape, assertion mode, primary error, 아티팩트 참조, projection assertion, seed validation은 여기서 복사하지 않고 [Conformance Fixtures 참조](../reference/conformance-fixtures.md#conformance-fixture-format)에 연결한다.
- 제외된 capability는 v0.1 Kernel MVP의 failed requirement가 아니라 아직 v0.1이 증명하지 않은 capability로 이름 붙인다.
...
- direct, work, advisor mode basics에 대해 gate를 보수적으로 초기화한다.
...
- Advisor mode는 read-only로 남을 수 있고, direct/work mode는 compatible Change Unit이 있을 때 scoped authority loop에 참여할 수 있다.
...
User-owned 또는 material technical judgment를 표현하고 선택한 loop를 block할 수 있음을 증명하는 데 필요한 최소 Decision Packet 동작만 추가합니다.

점검 목록:

- Canonical Decision Packet record를 만들거나 seed하거나, 이를 만들고 resolve하는 public request/record route를 실행한다. Decision Packet candidate는 그 route에 입력될 수 있지만, candidate만으로 `decision_gate`를 충족하거나 blocker를 제거하거나 runtime fixture를 통과할 수 없다.
- Owner contract에 따라 decision이 unresolved, resolved, deferred, blocking인지 기록한다.
- Status와 next-action read를 통해 active Decision Packet ref를 표시한다.
- Required decision이 missing 또는 unresolved이면 `prepare_write` 또는 `close_task`가 block되게 한다.
- Full Decision Packet quality, option completeness, 잔여 위험 영향, judgment separation은 later Agency Pack에 둔다.

완료 기준:

- Status와 next가 state를 변경하지 않고 active required decision을 보여 줄 수 있다.
- Required unresolved Decision Packet이 write authority 또는 close를 block할 수 있다.
- Compatible recorded decision은 그 특정 decision blocker만 제거하며, 그 자체로 write authority를 부여하지 않는다.
...
- Decision Packet 요구사항을 확인하고, existing owner rule 또는 seeded owner record가 sensitive-action Approval이 missing 또는 incompatible이라고 말하는 경우 선택된 경로를 block 또는 hold한다. 이때 full Approval lifecycle을 증명하지 않는다.
- write 전에 적용되는 design-policy precondition을 확인한다.
- 접점 능력을 정직하게 확인하고 cooperative 또는 detective limit을 표시한다.
- scope, state version, decision, baseline, capability, 또는 applicable seeded/owner-defined sensitive-action Approval requirement가 맞지 않으면 차단 조건을 반환한다.
...
- 그 ref 없이는 direct Run이나 implementation Run으로 제품 쓰기를 기록할 수 없다.
...
- 제품 파일 쓰기를 기록하는 direct Run 또는 implementation Run에는 compatible, unexpired, unconsumed Write Authorization을 요구한다.
- successful commit에서 Write Authorization을 정확히 한 번 consumed로 표시한다.
- actor, 접점, kind, intended operation, observed changes, command results, 아티팩트 참조, summary, Run 상태를 기록한다.
...
- approved staged file 또는 기존 committed 아티팩트 참조를 받는다.
...
record와 아티팩트 참조에서 첫 근거 summary를 만듭니다.

점검 목록:

- 하나 이상의 completion condition 또는 acceptance criterion을 Run ref와 아티팩트 참조에 매핑한다.
...
현재 작업 상태와 다음 safe action을 변경 없이 제공합니다.

점검 목록:

- 프로젝트, active Task, 현재 gate, active Change Unit, 쓰기 권한 요약, active Decision Packet refs, 근거 상태, 닫기 차단 조건, projection 최신성, 다음 safe action을 읽는다.
...
- 반복 next read는 authority를 만들거나 blocker를 충족하지 않는다.
...
- gate, evidence, Decision Packet, applicable seeded/owner-defined sensitive-action Approval blocker를 최소 수준에서 점검할 만큼의 `close_task` state logic을 계획한다.
- 잔여 위험을 받아들인 close, Manual QA, 분리 검증(detached verification), acceptance, full Approval lifecycle 또는 drift handling은 later pack에 둔다.
...
- Direct, work, advisor mode basics가 full policy coverage 주장 없이 표현된다.
- 제품 파일 쓰기에는 scoped Change Unit이 필요하다.
- Basic Decision Packet lifecycle이 required decision을 표시하고 block할 수 있다.
...
- v0.3 Agency Pack: 전체 Decision Packet 품질, 전체 Approval lifecycle과 Approval drift handling, 분리 검증(detached verification) 독립성, Manual QA policy matrix, 잔여 위험 표시와 accepted-close semantics, feedback-loop policy, TDD trace, codebase stewardship, stewardship validators, context-hygiene coverage.
...
- direct/work/advisor mode basics가 mode label만으로 write authority를 만들지 않으면서 표시됨
- active Change Unit이 intended path 하나의 범위를 정함
- basic Decision Packet request, record, status/next visibility, unresolved-decision blocker behavior가 동작함
...
- status와 next read가 상태 변경 없이 gate, evidence, 쓰기 권한, Decision Packet refs, next safe action, projection 최신성을 표시함
...
Fixture shape와 비교 규칙은 [Conformance Fixtures 참조](../reference/conformance-fixtures.md#conformance-fixture-format)를 따릅니다. Suite stage, authoring order, docs-maintenance result를 표현하기 위해 fixture body에 field를 추가하지 않습니다.
