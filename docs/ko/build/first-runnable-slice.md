...
이 문서는 Build 개요를 구현자가 가장 먼저 계획해야 하는 내부 엔지니어링 점검으로 바꿉니다.
...
[구현 개요](implementation-overview.md)를 먼저 읽고 [문서 수락 상태](implementation-overview.md#문서-수락-상태)를 확인합니다. 그 인계 표가 Build 진입 기준입니다. 유지보수자가 첫 런타임 배치를 위한 구현 계획 준비 상태를 수락하기 전까지 이 조각은 계획 전용입니다. Storage와 DDL의 세부 내용은 [Storage와 DDL](../reference/storage-and-ddl.md)을 봅니다. 이 조각 이후의 단계별 전달은 [단계별 전달 계획](mvp-plan.md)을 사용합니다. 로드맵 후보는 [로드맵](../roadmap.md)을 봅니다.
...
정확한 계약은 [커널 참조](../reference/kernel.md#prepare_write), [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md)를 사용합니다.
...
Storage planning에서는 내부 엔지니어링 점검에 [MVP-1 minimal storage schema](../reference/storage-and-ddl.md#mvp-1-minimal-storage-schema)의 최소 authority subset만 사용합니다. Full Approval record, Evidence Manifest, Manual QA, Eval, projection job, reconcile item, validator run, Journey record, diagnostic 같은 later storage profile은 first-slice requirement가 아닙니다.
...
Owner contract: runtime home layout과 minimal storage schema는 [Storage와 DDL](../reference/storage-and-ddl.md#mvp-1-minimal-storage-schema)이 담당합니다. Local space와 guarantee-level placement는 [런타임 아키텍처 참조](../reference/runtime-architecture.md)가 담당하고, guarantee-level 의미는 [보안 위협 모델 참조](../reference/security-threat-model.md#정직한-guarantee-display)가 담당하며, connector reporting은 [Agent 통합 참조](../reference/agent-integration.md)가 담당합니다.
...
Owner contract: Task lifecycle과 state conflict behavior는 [커널 참조](../reference/kernel.md#task), [Lifecycle and transitions](../reference/kernel.md#lifecycle-and-transitions), [API Errors](../reference/api/errors.md#state-conflict-behavior)가 담당합니다.
...
Owner contract: write-gate semantics는 [커널 참조: prepare_write](../reference/kernel.md#prepare_write)가 담당합니다. Public request/response shape와 error precedence는 [`harness.prepare_write`](../reference/api/mvp-api.md#harnessprepare_write)와 [Primary Error Code Precedence](../reference/api/errors.md#primary-error-code-precedence)가 담당합니다.
...
Owner contract: Run semantics는 [커널 참조: record_run](../reference/kernel.md#record_run)이 담당합니다. Public schema는 [`harness.record_run`](../reference/api/mvp-api.md#harnessrecord_run)이 담당합니다. Transaction order는 [State transaction flow](../reference/runtime-architecture.md#state-transaction-flow)가 담당합니다.
...
Owner contract: artifact ref는 [ArtifactRef](../reference/api/schema-core.md#artifactref)가 담당합니다. Storage layout과 registration detail은 [Artifact directory layout](../reference/storage-and-ddl.md#artifact-directory-layout) 및 [Artifact Registration Contract](../reference/storage-and-ddl.md#artifact-registration-contract)가 담당합니다.
...
Owner contract: status와 `status.next_actions` schema는 [`harness.status`](../reference/api/mvp-api.md#harnessstatus)가 담당합니다. 별도 [`harness.next`](../reference/api/schema-later.md#harnessnext)는 later/compatibility material입니다. Close behavior는 [커널 참조: close_task](../reference/kernel.md#close_task)가 담당합니다.
...
- [향후 Fixture Catalog](../reference/future-fixture-catalog.md): 내부 엔지니어링 점검 requirement가 아닌 detailed later scenario candidate.
