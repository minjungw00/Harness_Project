...
더 넓은 MVP를 만들기 전에, Task 하나가 범위가 정해진 쓰기 권한, Run 기록, artifact로 뒷받침되는 근거, 상태, projection 최신성, 닫기 차단 조건을 통과하는지 증명합니다.
...
10. `TASK` projection이 최신이거나 render를 위해 durable queue에 들어간다.
...
- 같은 project identity에 대해 registration이 idempotent하게 동작하게 한다.

완료 기준:

- Core가 이후 모든 Task-scoped action에 대해 현재 프로젝트를 resolve할 수 있다.
...
- mode, lifecycle phase, result, close reason, assurance level, state version, current summary, gate state를 저장한다.
- 선택된 mode에 대해 gate를 보수적으로 초기화한다.
- Task가 생성되거나 변경될 때 task_events를 append한다.
...
- Change Unit을 active Task에 attach하고 active 쓰기 범위로 만든다.
...
- active Task와 active Change Unit을 resolve한다.
...
- missing authority를 차단할 만큼 approval과 Decision Packet requirement를 확인한다.
- 접점 능력을 정직하게 확인하고 cooperative 또는 detective limit을 표시한다.
- scope, state version, approval, decision, baseline, 능력이 incompatible하면 차단 조건을 반환한다.
...
- authorization 밖의 observed change를 감지하고 violation, 차단 조건, stale 근거, 또는 Decision Packet 경로로 연결한다.
- 현재 기록 갱신과 같은 transaction에서 task_events를 append한다.
...
- 하나 이상의 completion condition 또는 acceptance criterion을 Run ref와 artifact ref에 mapping한다.
...
- Read 동작에서 event를 append하거나, projection을 enqueue하거나, artifact를 만들거나, gate를 satisfy하거나, 쓰기를 허가하거나, Task를 close하지 않는다.
...
- Task state가 바뀌면 `TASK` projection job을 enqueue하거나, commit 뒤 최소 managed `TASK` projection을 render한다.
- source state version과 projection 최신성을 추적한다.
- Projection render failure를 Core 상태 failure가 아니라 projection failure로 취급한다.
- Markdown projection은 source of truth가 아니라 derived view라는 rule을 유지한다.
...
- gate, evidence, Decision Packet, approval state, residual-risk visibility, QA, verification, acceptance를 최소 수준에서 inspect할 만큼의 `close_task` state logic을 계획한다.
...
- Required evidence가 없는 Task는 successful close될 수 없다.
- Unresolved blocking Decision Packet이 있는 Task는 successful close될 수 없다.
- Close result가 rendered report가 아니라 기준 기록에 근거한다.
...
- State store와 task_events를 사용할 수 있다.
...
- 상태 조회는 read-only다.
- Projection은 derived이며 failure-isolated하다.
- Required evidence 또는 decision이 없으면 close가 차단될 수 있다.
...
Core 동작을 drive하고 state, events, artifacts, projections, errors를 검증하는 fixture를 작성합니다. Rendered prose matching만으로 success를 assert하지 않습니다.

첫 조각의 최소 fixture:

- no-active-task 상태 조회가 idle state를 반환하고 event를 append하지 않음
...
- Task 변경이 `TASK` projection을 enqueue하거나 render함
- projection failure가 committed state를 rollback하지 않음
- `close_task`가 evidence-insufficient close를 차단함
- `close_task`가 unresolved decision close를 차단함

Fixture shape와 비교 규칙은 [운영과 Conformance](../11-operations-and-conformance.md#conformance-fixture-format)를 따릅니다.
