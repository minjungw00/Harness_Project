...
- Close/status blocker에 필요한 수준에서 supported, partial, insufficient 근거를 구분한다.
...
- minimal 근거 상태가 supported, partial, insufficient support를 보고함
- status와 next read가 상태 변경 없이 current Task, scope, write authority, evidence, blockers, next safe action을 표시함
- close/status output이 structured blocker와 함께 missing 근거를 차단함
