...
Active schema, API 문서, DDL, Build scope 표현, Later 문서, later 후보, 예시를 봅니다.

Active block에 active 자료만 있고 later 후보가 승격 전까지 Later 색인이나 승격한 담당 문서에 남으면 통과입니다. Later enum 값, method, table, command, template, assurance behavior, operations behavior, fixture family가 active requirement처럼 보이면 실패입니다.
...
유지보수 문서와 주변 경로에서 과거 재작성 리뷰, 해결된 issue 기록, 오래된 수락 기록, 오래된 전달 label 설명, 오래된 별칭 이력, later 후보 지역화 점검 기록, 과거 번역 문제 기록, 임시 migration plan을 찾습니다.
