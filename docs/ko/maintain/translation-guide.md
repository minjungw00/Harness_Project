...
| User Decisions display group | 사용자 결정 | 대기 중인 결정 항목을 나열하는 사용자 표시 그룹에만 씁니다. Schema field, gate, route, authority path로 다루지 않습니다. |
| decision type | 결정 유형 | 구체적인 대기 결정 항목을 label할 때 씁니다. |
| Product/UX judgment | 제품/UX 판단 | 제품 동작, 문구, flow, UX 선택처럼 사용자가 소유하는 결정을 가리킵니다. |
| technical architecture judgment | 기술 구조 판단 | Architecture, dependency, migration, interface, 중요한 기술 방향처럼 사용자가 소유하는 결정을 가리킵니다. |
| security/privacy judgment | 보안/개인정보 판단 | Security, privacy, retention, redaction, audit, exposure 선택처럼 사용자가 소유하는 결정을 가리킵니다. |
| scope/autonomy judgment | 범위/자율성 판단 | Scope expansion, work split, Autonomy Boundary 선택처럼 사용자가 소유하는 결정을 가리킵니다. |
| QA waiver | QA 면제 판단 | Required QA path를 면제할지에 대한 사용자 결정을 가리킵니다. Generic `승인`으로 옮기지 않습니다. |
| verification waiver | 검증 면제 판단 | Required verification path를 면제할지에 대한 사용자 결정을 가리킵니다. Generic `승인`으로 옮기지 않습니다. |
| `judgment_domain` | 판단 영역 | Schema/API/reference 문맥에서는 field name을 그대로 둡니다. 사용자 표시에서는 `제품/UX 판단`, `보안/개인정보 판단`, `QA/작업 수락`, `잔여 위험`, `복합`처럼 어떤 판단 영역인지 설명합니다. |
| `decision_kind` | 결정 경로 | Field name과 enum value는 그대로 둡니다. 사용자 표시에서는 그 결정이 어떤 결정 경로나 lifecycle path를 쓰는지 설명합니다. |
| Decision Packet | 결정 패킷 | Record ID, API/schema 이름, heading anchor 등 literal context에서는 `Decision Packet`을 유지할 수 있습니다. 사용자용 prose에서는 exact label 전에 `사용자 결정 요청/기록`처럼 풀어 쓸 수 있습니다. |
