...
사용자용 한국어에서는 자연스러운 공개 개념을 먼저 둡니다. `작업`, `범위` 또는 `작업 조각`, `판단` 또는 `결정할 것`, `증거`, `확인` 또는 `검증`, `마무리` 또는 `닫기`입니다. `요구사항 구체화`, `쓰기 전 범위 확인`, `판단 요청`, `판단 요약`, `증거 목록`, `상태 보기`, `요약`, `상태 카드`, `수동 QA`, `최종 수락`, `잔여 위험` 또는 `남은 위험`, `닫기 가능 여부`, `닫기 준비 상태`, `닫기 차단 사유`, `다음 안전한 행동` 같은 구체적인 표현은 이 개념들을 설명할 때 쓸 수 있습니다. `Discovery`, `Change Unit`, `Decision Packet`, `Write Authorization`, `Evidence Manifest`, `Projection`, `Residual Risk`, `Manual QA`, `detached verification`, `Acceptance` 같은 라벨은 독자 친화 표현과 하네스 라벨이 모두 필요할 때만 괄호에 둡니다. 참고 문서의 한국어에서는 정확한 스키마 식별자, enum 값, field 이름, API 용어, 안정적인 제품 라벨을 정밀도가 필요할 때 그대로 둘 수 있습니다.

## 사용자용 어휘 규칙

한국어 사용자용 문서는 자연스러운 공개 개념인 `작업`, `범위` 또는 `작업 조각`, `판단` 또는 `결정할 것`, `증거`, `확인` 또는 `검증`, `마무리` 또는 `닫기`를 우선합니다. `요구사항 구체화`, `범위 밖`, `쓰기 전 범위 확인`, `판단 요청`, `판단 요약`, `증거 목록`, `상태 보기`, `요약`, `상태 카드`, `수동 QA`, `최종 수락`, `잔여 위험`, `남은 위험`, `닫기 가능 여부`, `닫기 준비 상태`, `닫기 차단 사유`, `다음 안전한 행동`은 그 개념을 설명할 때 유용하지만, 새 사용자가 외워야 하는 더 큰 개념 모델이 되면 안 됩니다. 안정적인 English identifier는 주로 참조 문서, schema/API 문맥, 정확한 record name, code-like string, anchor, 내부 구현 용어를 의도적으로 설명하는 표에서 보존합니다.

사용자용 문서에서 내부 구현 용어가 필요하면 한국어 개념을 먼저 설명하고, 실제 경계, 차단 사유, 출처 참조, 참조 링크를 분명히 하는 경우에만 정확한 English label을 괄호로 덧붙입니다. 한국어 문장이 영어 명사 여러 개에 조사만 붙인 형태가 되지 않게 합니다. 사용자 예시는 record label이나 procedure name이 아니라 평소 사용자 말로 시작합니다.
...
- 사용자에게 보이는 목록이나 카드에서는 `현재 증거 참조`, `빠진 증거`, `오래된 증거`, `이미 실행한 확인`처럼 자연스러운 한국어 묶음을 씁니다.
...
- 자세한 증거 목록을 사용자에게 보여줄 때는 `증거 목록`을 씁니다. `Evidence Manifest`는 record, template, API shape, owner reference를 이름 붙일 때만 소개합니다.
...
지역화된 표시 라벨은 화면에 렌더링되는 문구이지 schema identifier가 아닙니다. `제품 판단`, `기술 판단`, `범위 판단` 같은 한국어 label은 렌더링된 화면, template text, 예시, 한국어 prose에 나타날 수 있습니다. 하지만 `product_decision`, `technical_decision`, `scope_decision`, `judgment_kind`, `UserJudgment` 같은 canonical identifier를 대신하면 안 됩니다. `display_label`이 나오면 field name은 정확히 보존하고, 그 값은 stable identifier와 locale에서 파생한 localized response-only 또는 compatibility display text로 다룹니다. Localized label을 active schema enum block, storage value set, API method definition, owner contract name에 넣지 않습니다.
...
| Sensitive action approval | 민감 동작 승인 | 이름 붙은 민감 동작에 대한 범위 있는 승인을 가리킵니다. 제품 판단, 기술 판단, 범위 판단, 최종 수락, 잔여 위험 수락과 구분합니다. |
| QA waiver | QA 면제 판단 | 정책이 허용할 때 수동 QA를 건너뛰는 판단입니다. QA 증거나 QA 통과가 아닙니다. |
...
| `display_label` | 표시 라벨 | Active canonical schema/storage field가 아닙니다. Compatibility 또는 response-only output에서 보이면 identifier는 정확히 보존하고, 값은 `judgment_kind`와 locale에서 파생한 localized display text로 다룹니다. `제품 판단`, `기술 판단`, `범위 판단` 같은 한국어 라벨은 렌더링 문자열이지 schema value가 아닙니다. |
| `judgment_category`, `judgment_route`, `display_depth` | legacy 판단 field | 오래된 Decision Packet draft의 legacy 또는 implementation routing term입니다. 오래된 schema/API/reference compatibility 문맥에서만 정확히 보존합니다. 새 예시는 `judgment_kind`, `presentation`, locale에서 파생한 표시 라벨을 우선합니다. |
...
| Write Authorization | 쓰기 전 범위 확인 / 쓰기 승인 기록 | 사용자용 prose에서는 `쓰기 전 범위 확인`을 우선합니다. `prepare_write`가 남기는 내부 협력형 하네스 record나 result를 이름 붙일 때만 `쓰기 승인 기록(Write Authorization)`을 씁니다. API/tool 이름과 exact field는 유지합니다. OS 권한, sandboxing, 변조 방지 enforcement, 사전 차단, 권한 격리를 뜻하지 않는다고 설명합니다. |
| evidence | 증거 | 주장을 뒷받침하는 자료를 사용자용 prose에서 설명할 때 씁니다. Record나 API를 이름 붙일 때는 `Evidence`, `Evidence Manifest`, schema field를 정확히 유지합니다. |
| Evidence Manifest | 증거 목록 | 사용자용 prose에서는 자세한 증거 목록을 가리킬 때 씁니다. Record/template/schema/API 문맥에서는 `Evidence Manifest`를 유지합니다. |
...
| Approval | 민감 동작 승인 | Canonical Approval 개념의 사용자용 표현입니다. Permission을 설명하는 prose에서도 되도록 `승인`을 쓰고, `허가`를 두 번째 기준 용어로 만들지 않습니다. 일반적인 `승인`은 최종 수락, 제품 판단, QA 면제 판단, 잔여 위험 수락, 쓰기 승인 기록을 뜻하면 안 됩니다. Reference/schema 문맥에서는 `Approval`을 유지합니다. |
...
| close / Close | 마무리 / 닫기 | 작업을 정직하게 끝낼 수 있는지를 가리키는 쉬운 개념입니다. 사용자 요청에서는 `마무리`가 자연스러울 수 있고, 하네스 close 상태나 닫기 차단 사유를 맞출 때는 `닫기`가 유용합니다. `close_task` 같은 정확한 식별자는 유지합니다. |
...
| close blocker | 닫기 차단 사유 | 닫기가 진행될 수 없는 구체적인 이유를 가리킵니다. API/reference 문맥에서는 `close blocker`나 정확한 schema name을 유지할 수 있습니다. |
| next safe action | 다음 안전한 행동 | 해결되지 않은 판단, 범위, 증거, QA, 검증, 최종 수락, 위험을 숨기지 않고 진행할 수 있는 다음 행동을 말합니다. |
| blocker | 차단 사유 | 사용자용 prose에서는 진행이나 닫기를 막는 것을 `차단 사유`로 쓸 수 있습니다. API/reference 문맥에서는 `blocker`를 유지하거나, 독자에게 도움이 될 때 `차단 조건(blocker)`으로 설명합니다. `blockers`, `CloseBlockerCategory` 같은 field name, template key, enum-like value, schema name은 번역하지 않습니다. |
| ArtifactRef | `ArtifactRef` / 아티팩트 참조 | Schema name은 정확히 유지합니다. Prose에서는 `아티팩트 참조`를 씁니다. Evidence 문맥에서는 `증거 아티팩트 참조`도 가능합니다. |
...
| evidence | 제품 용어로 필요할 때만 `evidence`를 유지합니다. 일반 문장에서는 `증거`를 씁니다. |
| exact contract mismatch | Enum mismatch, API field mismatch, lifecycle status mismatch, table/column mismatch, owner document mismatch, later/profile material을 active material처럼 번역하거나 제시한 경우는 이중 언어 리뷰 `FAIL`입니다. 문체 차이가 아닙니다. |
| evidence manifest / detailed evidence list | 사용자용 prose에서는 `증거 목록`을 씁니다. 내부 record, template, schema/API context, owner reference를 말할 때만 `Evidence Manifest`를 씁니다. |
...
| approval / Approval | 사용자용 prose에서는 민감 동작 승인을 `민감 동작 승인`으로 씁니다. 기준 하네스 status, `gate`, record, schema, exact reference term을 이름 붙일 때는 `Approval`을 유지합니다. 일반적인 `승인`이 최종 수락, 제품 판단, QA 면제 판단, 잔여 위험 수락, 쓰기 승인 기록을 뜻하게 하면 안 됩니다. |
| write authority | 사용자용 prose에서는 `쓰기 전 범위 확인`을 우선합니다. `prepare_write`가 남기는 하네스 기록을 말할 때만 `쓰기 승인 기록(Write Authorization)`을 씁니다. OS-level permission, sandboxing, tamper-proof enforcement를 암시하지 않습니다. |
...
| gate | 사용자용 흐름에서는 `관문`, `확인`, `닫기 확인`, `차단 사유`처럼 문맥에 맞는 말을 우선합니다. 참조 문서에서 kernel field나 strict contract를 가리킬 때는 `gate`를 유지할 수 있습니다. |
...
| unblocker | 사용자에게는 `차단 사유를 푸는 최소 조치` 또는 `해소 방법`으로 씁니다. Schema/API field나 exact label이 아니면 `unblocker`를 그대로 두지 않습니다. |
...
대소문자 규칙: `Approval`은 민감한 행동에 대한 기준 하네스 permission 개념입니다. 소문자 `approval`은 `approval_gate`, `decision_kind=approval`, `approval_scope`, `approval-shaped`, approval drift처럼 stable identifier, enum value, schema name, 의도적으로 고정된 표현, quoted legacy/user wording에서만 유지할 수 있습니다.
...
[ ] Enum value, API field, lifecycle status, table/column name, owner doc, active-vs-later boundary에 `FAIL` mismatch가 없는가?
[ ] `제품 판단`, `기술 판단`, `범위 판단` 같은 한국어 label을 schema identifier나 enum value가 아니라 localized display text로 다루었는가?
