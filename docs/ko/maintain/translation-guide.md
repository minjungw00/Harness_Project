...
Exact stage label과 component label도 그대로 유지합니다. 일반 설명에서는 한국어 기준 용어를 우선하지만, `v0.1 Kernel MVP`, `Kernel Smoke`, `v0.2 Evidence & Projection Pack`, `v0.3 Agency Pack`, `v0.4 Operations Pack`, `Agency-Hardened MVP`, `Agency-Hardened MVP reference conformance target`, `v1+ Expansion`, `Product Repository`, `Harness Server / Installation`, `Harness Runtime Home`, component label로 쓰인 `Harness Runtime Home` path는 부분 번역하지 않습니다. 필요한 경우 `v0.1 Kernel MVP, 즉 첫 커널 MVP`처럼 exact label 뒤에 한국어 설명을 붙일 수 있지만 label 자체는 유지합니다.

Glossary의 official term heading은 lookup anchor입니다. Heading에는 정확한 Harness term을 유지하고, 한국어 기준 표현은 정의나 alias line에 둡니다.
