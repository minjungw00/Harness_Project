...
사용자용 한국어에서는 여섯 가지 자연스러운 공개 개념을 먼저 둡니다. `작업`, `범위`, `판단` 또는 `결정할 것`, `근거`, `확인` 또는 `검증`, `마무리` 또는 `닫기`입니다. `범위 밖`, `쓰기 전 범위 확인`, `판단 요청`, `수동 QA`, `작업 수락`, `잔여 위험`, `닫기 막힘`, `다음 안전한 행동` 같은 구체적인 표현은 이 여섯 개념을 설명할 때 쓸 수 있습니다. `Change Unit`, `Decision Packet`, `Write Authorization`, `Evidence Manifest`, `Residual Risk`, `Manual QA`, `detached verification`, `Acceptance` 같은 라벨은 독자 친화 표현과 하네스 라벨이 모두 필요할 때만 괄호에 둡니다. 참고 문서의 한국어에서는 정확한 스키마 식별자, enum 값, field 이름, API 용어를 정밀도가 필요할 때 그대로 둘 수 있습니다.

## 사용자용 어휘 규칙

한국어 사용자용 문서는 여섯 가지 자연스러운 공개 개념인 `작업`, `범위`, `판단` 또는 `결정할 것`, `근거`, `확인` 또는 `검증`, `마무리` 또는 `닫기`를 우선합니다. `범위 밖`, `판단 요청`, `수동 QA`, `작업 수락`, `잔여 위험`, `닫기 막힘`, `닫기 준비 상태`, `다음 안전한 행동`은 그 개념을 설명할 때 유용하지만, 새 사용자가 외워야 하는 더 큰 개념 모델이 되면 안 됩니다. 안정적인 English identifier는 주로 참조 문서, schema/API 문맥, 정확한 record name, code-like string, anchor, 내부 구현 용어를 의도적으로 설명하는 표에서 보존합니다.
...
| Technical judgment | 기술 판단 | 아키텍처, 의존성, 마이그레이션, 인터페이스, 보안/개인정보, 범위/자율성, QA/검증 기대치, waiver, 중요한 기술 방향처럼 사용자가 소유하는 결정을 가리킵니다. |
| Sensitive action approval | 민감 동작 승인 | 이름 붙은 민감 동작에 대한 범위 있는 permission을 가리킵니다. 제품/UX 판단, 기술 판단, 작업 수락, 잔여 위험 수용과 구분합니다. |
| Work acceptance | 작업 수락 | 작업 경로가 요구할 때 사용자가 결과를 받아들이는 판단입니다. |
| Residual risk acceptance | 잔여 위험 수용 | 이름 붙은 보이는 잔여 위험을 명시적으로 받아들이는 판단입니다. |
...
| `judgment_type` | 판단 유형 | Canonical compact type field입니다. Schema/API/reference 문맥에서는 field name과 enum-like value를 그대로 둡니다. |
...
| `display_label` | 표시 라벨 | User-facing label field입니다. 허용 라벨은 `제품/UX 판단`, `기술 판단`, `민감 동작 승인`, `작업 수락`, `잔여 위험 수용`입니다. |
| `judgment_category`, `judgment_route`, `display_depth` | legacy 판단 field | 오래된 Decision Packet draft의 legacy 또는 implementation routing term입니다. 오래된 schema/API/reference compatibility 문맥에서만 정확히 보존합니다. 새 예시는 `judgment_type`, `presentation`, `display_label`을 우선합니다. |
...
| Decision Packet | Decision Packet / full-format 판단 요청 | Optional full-format presentation, legacy ref, template file, anchor, migration note를 이름 붙일 때 유지합니다. 사용자용 prose에서는 `판단 요청`을 먼저 쓰거나 도움이 되지 않으면 라벨을 생략합니다. |
| pre-write scope check | 쓰기 전 범위 확인 | 제품 파일을 쓰기 전에 하는 user-facing 확인을 가리키는 기본 표현입니다. `Write Authorization` 같은 내부 라벨보다 먼저 씁니다. |
| Write Authorization | 쓰기 허가 기록 | `prepare_write`가 남기는 내부 하네스 record나 result를 이름 붙일 때만 사용합니다. API/tool 이름과 exact field는 유지합니다. OS 권한, sandboxing, 변조 방지 enforcement, 사전 차단, 권한 격리를 뜻하지 않는다고 설명합니다. |
...
| work acceptance / Acceptance | 작업 수락 | 작업 경로가 요구할 때 사용자가 결과를 받아들이는 판단을 가리킵니다. 민감 동작 승인에는 쓰지 않습니다. 원문이나 예전 영어 표현이 `final acceptance`라도 영어 기준 용어는 `work acceptance`, 한국어 기준 용어는 `작업 수락`입니다. 최종성은 새 기준어를 만들지 말고 문장 안에서 풀어 설명합니다. 예: `작업 수락은 완료 결과를 사용자가 받아들이는 최종 판단입니다.` |
...
| acceptance / final acceptance | 사용자가 결과를 받아들인다는 판단이면 영어는 `work acceptance`, 한국어는 `작업 수락`을 우선합니다. 원문 영어가 finality를 강조하면 별도 기준어로 바꾸기보다 주변 문장에서 그 의미를 풀어 씁니다. |
