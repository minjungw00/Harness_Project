...
정확한 식별자는 영어와 한국어에서 그대로 보존합니다. 코드, 스키마, 경로, 검색 기준으로 쓰이는 값은 산문에 나올 때 backtick으로 감쌉니다.
...
- 필요한 곳에서는 정확한 식별자를 backtick으로 보존합니다.
...
링크 텍스트는 보이는 제목의 의미와 맞아야 합니다. 보이는 제목이 `## 닫기 준비 상태`라면 링크 텍스트도 "닫기 준비 상태"처럼 씁니다. "close readiness"나 "close 가능성"처럼 쓰지 않습니다.
...
아래 표현은 코드 블록 안에 있거나 이 가이드에서 나쁜 예로 인용하는 경우가 아니라면 한국어 산문에서 쓰지 않습니다.
...
- [ ] 대응 파일의 독자 목적, 경로 역할, 담당 문서 경로, active/later 경계가 맞습니다.
- [ ] 식별자, API 메서드, 파일 경로, enum 값, 스키마 이름, 테이블 이름, validator ID, 오류 코드, 앵커, 코드 리터럴이 보존되었습니다.
- [ ] 산문에서 명확성이나 검색성이 필요한 정확한 식별자는 backtick으로 감쌌습니다.
...
- [ ] 이 가이드에서 예시로 인용한 경우를 제외하고 금지할 혼합어 패턴을 제거했습니다.
