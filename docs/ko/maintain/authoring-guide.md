...
- [ ] Schema/API/storage 계약 표현을 고쳤다면 active schema block에 active enum value만 있는지 확인합니다. Later/profile 값은 `later/index.md#later-schema-candidates`, Later 문서, 또는 명확히 표시된 later/profile owner section에 남깁니다.
...
- [ ] Active API 문서가 active API schema owner가 정의한 schema type만 참조하는지 확인합니다. Later/profile type은 `later/index.md#later-schema-candidates`로 명확히 보냅니다.
...
| Projection | [Projection과 Template 참조](../reference/projection-and-templates.md) | Derived view rule, output tier, managed block, human-editable section, artifact-ref rendering, projection freshness/failure behavior, 활성 현재 MVP rendered template body. |
...
| Conformance | [적합성 참조](../reference/conformance.md)와 [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families) | 적합성 참조는 현재 상태, 적합성의 의미, 아직 없는 것, future fixture 형태, 주장 권한, 대표 예시, catalog-only future 경계, metrics 경계를 담당합니다. Later 후보 색인은 active MVP path 밖의 이름만 남긴 future fixture family 후보를 담당합니다. |
...
| Agent Integration | [Agent 통합 참조](../reference/agent-integration.md) | Connector capability profile, generated manifest, context push/pull profile, fallback semantics, Role Lens, reference-surface behavior, connector conformance overview, compact surface recipe. |
...
| Runtime Boundaries | [런타임 경계 참조](../reference/runtime-boundaries.md) | Product Repository / Harness Server / Runtime Home 분리, Core-only canonical mutation authority, projection/status card의 파생 표시 경계, artifact storage boundary, recovery boundary, 현재 non-isolation claim. |
| Design Quality | [설계 품질](../reference/design-quality.md) | 활성 현재 MVP 설계 품질 역할, 발견 사항 심각도, 닫기 blocker 조건, 면제 경계, 증거 기대치, validator ID 경계, later 정책 후보 경계. |
...
| API/schema collisions | [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md), [API Schema Later](../later/index.md#later-schema-candidates) | Use/Agent guide, Build stage, Operations/Conformance summary, Agent Integration summary, README route | Method name, request/response field, shared ref, envelope, error, `ValidatorResult`, `ArtifactRef`, later-profile branch가 API owner와 다르거나 owning tool/profile이 active가 되기 전에 활성 요구사항처럼 보입니다. |
| Active schema와 later/profile enum 누수 | [API Schema Core](../reference/api/schema-core.md), [API Schema Later](../later/index.md#later-schema-candidates), [MVP API](../reference/api/mvp-api.md), active Build stage owner | Build summary, Use example, Reference introduction, Maintain check | Active schema block이 later/profile enum value, compatibility-only value, locale label, Roadmap value를 담습니다. 또는 주변 prose만으로 active schema 안의 value가 실제로는 active가 아니라고 설명하려 합니다. |
| Undefined active API schema type | [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Schema Later](../later/index.md#later-schema-candidates), [API Errors](../reference/api/errors.md) | Build 문서, Agent Integration, Operations summary, conformance example | Active API method나 response가 active schema owner가 정의하지 않았거나 later/profile owner로 명확히 보내지 않은 schema type, shared ref, error shape, `CloseTaskResponse`, `ArtifactRef`, `ValidatorResult`, envelope member를 이름 붙입니다. |
| Storage/DDL collisions | [Storage](../reference/storage.md) | Build implementation sequencing, Runtime Boundaries, Operations diagnostic, Conformance fixture note, README route | Table name, DDL profile, JSON `TEXT`, enum hardening, migration/lock/artifact/projection-job storage rule, persisted status value가 Storage와 다르거나 Storage 밖에 중복됩니다. |
| Core transition collisions | [Core Model 참조](../reference/core-model.md). Authority placement는 [런타임 경계 참조](../reference/runtime-boundaries.md) | Start/Use task explanation, Build stage summary, API/storage/projection reference, template summary | Task/run/gate 상태 전이, `prepare_write`, Write Authorization, `record_run`, waiver, close, non-substitution rule, Core-only mutation authority가 Core owner와 충돌하거나 표시/보고만으로 동작하는 것처럼 쓰입니다. |
| Cross-owner field coverage gap | [Core Model 참조](../reference/core-model.md), [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [Storage](../reference/storage.md) | Build stage summary, Agent Integration summary, Operations summary, conformance example, README route | Write Authorization 같은 공유 개념의 field, status/value set, row boundary, response semantics, storage implication이 owner마다 다릅니다. 또는 summary가 차이를 schema/design decision으로 라우팅하지 않고 field를 더하거나 뺍니다. |
| Active MVP vs later/profile boundary drift | [MVP 계획](../build/mvp-plan.md)과 Later profile 문서 | Reference introduction, Use example, README route, Roadmap entry, stage owner 밖의 Build summary | Engineering Checkpoint, Kernel Smoke, MVP-1, later-profile, Roadmap material이 stage/profile owner가 scope, fallback behavior, proof expectation과 함께 승격하지 않았는데 문구만으로 승격되거나 필수처럼 보입니다. |
...
| Evidence sufficiency and close-readiness ambiguity | [Core Model 참조](../reference/core-model.md), [MVP API](../reference/api/mvp-api.md), close에 영향을 주는 design-quality finding은 [설계 품질](../reference/design-quality.md#when-a-finding-blocks-close) | User Guide, One Task, 사용자 소유 판단 예시, Build close criteria, Projection/template summary, README route | 증거, 검증, 수동 QA, final acceptance, 잔여 위험 수락, waiver, close readiness가 서로를 대신합니다. 또는 필요한 증거, 판단, QA, 검증, final acceptance, 위험 차단 사유가 남았는데 close가 가능하다고 설명합니다. |
...
Review 중 발견한 큰 구현 결정은 [MVP 계획: 서버 코딩 전 결정](../build/mvp-plan.md#서버-코딩-전-결정)에 둡니다. Active docs 곳곳에 큰 결정을 `TODO_DECISION`으로 흩어 놓지 않습니다. 짧은 maintainer handoff status는 [MVP 계획: 저장소 상태](../build/mvp-plan.md#저장소-상태)가 담당합니다.
...
| Conformance fixture 문서가 지금 runnable suite가 있는 것처럼 보입니다. | Fixture 문서는 future-oriented와 staged 상태를 유지합니다. MVP behavior example을 runnable fixture file이나 current pass/fail criteria라고 부르지 않습니다. | Implementation-readiness criterion |
...
|---|---|---|
| Active schema block에 later/profile value가 들어갑니다. | Active schema block은 owning tool, record, profile에 active인 value만 나열합니다. Later/profile enum value는 `later/index.md#later-schema-candidates`, Later 문서, 명확히 표시된 later/profile owner section에 둡니다. | Schema/design decision |
...
| Active schema/later-profile separation | Active schema block에는 active enum value만 둡니다. Later/profile value는 `later/index.md#later-schema-candidates`, Later 문서, 명확히 표시된 later/profile owner section에 남깁니다. |
...
### Projection 템플릿 본문 섹션

활성 현재 MVP 템플릿 본문은 별도 템플릿 디렉터리가 아니라 [Projection과 Template 참조](../reference/projection-and-templates.md)에 있습니다.

각 활성 템플릿 본문은 아래 부분을 포함합니다.

- `사용 시점`
- `기준 기록`
- `렌더링 섹션`
- `템플릿`
- 권한, compactness, 최신성, 표시 라벨 경계를 지키는 데 필요한 메모

템플릿 본문 section은 non-authority boundary를 분명히 보여줘야 합니다. 템플릿은 rendered display일 뿐입니다. Canonical state, gate authority, 민감 동작 승인, 최종 수락, 잔여 위험 수락, evidence, schema, DDL, runtime behavior가 아닙니다.
...
| 경계 | 정확한 문구의 owner |
|---|---|
| Context Index와 retrieved/indexed context | Future feature boundary는 [로드맵: 후보 항목 목록](../later/index.md#roadmap-candidates), connector context handling은 [Agent Integration: Context Push/Pull Principles](../reference/agent-integration.md#context-pushpull-principles) |
| Local Derived Metrics | [로드맵: 후보 항목 목록](../later/index.md#roadmap-candidates) |
| Role Lens | [Agent Integration: Role Lens 동작](../reference/agent-integration.md#role-lens-동작) |
| Review Stages | [설계 품질: later 정책 후보 경계](../reference/design-quality.md#later-policy-catalog-boundary); 이름만 남긴 later family는 [Later 후보 색인](../later/index.md#assurance-candidates) |
| Release Handoff와 export | [Operations And Conformance: Release Handoff Export Profile](../reference/operations-and-conformance.md#release-handoff-export-profile); later rendered shape은 승격 전까지 [Later template 후보](../later/index.md#later-template-candidates)에 후보로만 남습니다. |
...
| Projection과 report surface | [Projection And Templates Reference](../reference/projection-and-templates.md) |
...
| Strategic thesis, failure model, MVP boundary, principle groups | Reader explanation은 `start.md`; exact contract impact는 `reference/design-quality.md`와 `reference/core-model.md` |
| Core entity, lifecycle, gate, state transition, close semantics, `prepare_write`, `close_task` | `reference/core-model.md` |
| Runtime boundary spaces, Product Repository / Harness Server / Runtime Home 분리, Core-only mutation authority, projection/status card 파생 표시 경계, artifact storage boundary, recovery boundary, 현재 non-isolation claim | `reference/runtime-boundaries.md` |
...
| MCP resource/tool, request/response schema, error taxonomy, validator result schema, artifact ref shape | `reference/api/mvp-api.md`, `reference/api/schema-core.md`, `reference/api/errors.md`, `later/index.md#later-schema-candidates` |
...
| MVP implementation order, smoke target, planning exit criteria | `build/mvp-plan.md` |
...
| 모든 활성 현재 MVP projection template body와 display card shape | `reference/projection-and-templates.md` |
| Active design-quality role, 발견 사항 심각도, 닫기 blocker 조건, 면제 경계, 증거 기대치, validator ID 경계, later 정책 후보 경계 | `reference/design-quality.md` |
| User-facing conversation, status reading, user judgment, close checklist | `use/user-guide.md` |
| Practical user-owned judgment example과 user-facing judgment request pattern | Example은 `use/judgment-examples.md`; exact user judgment behavior는 `reference/core-model.md`와 `reference/api/mvp-api.md` |
| User/agent session procedure | `use/agent-guide.md` |
| Agent surface capability profile, common connector contract, fallback semantics, Role Lens, connector conformance overview, compact surface recipe, generic capability profile example | `reference/agent-integration.md` |
...
| 현재 적합성 상태, fixture 형태, 주장 권한, 대표 예시, catalog-only future 경계, metrics 경계 | `reference/conformance.md` |
| Active MVP path 밖의 이름만 남긴 future fixture family 후보 | `later/index.md#future-fixture-families` |
| Official term definition과 capitalization | `reference/glossary.md` |
| Roadmap candidate | `later/index.md#roadmap-candidates` |
