...
- 하네스는 prompt 묶음이 아닙니다. Scope, 사용자 소유 판단, 근거, 검증, QA 기대치, 최종 수락, 잔여 위험 상태, 닫기 준비 상태를 다루는 로컬 기준 기록입니다.
...
이 지도는 사전 구현 검토 기준에서 자주 흔들리는 문서 정비 대상을 담당 문서군으로 보내기 위한 것입니다. 이후 편집에서 요약이나 링크를 고치기 전에 owner를 먼저 확인하게 합니다. 문서 수락, 구현 준비 증명, 서버/런타임 구현 허가, 하네스 런타임 객체 생성을 하지 않습니다.

| 정비 축 | 기준 owner 문서군 | 요약하거나 link할 수 있는 non-owner 문서 | Docs-maintenance `FAIL` 증상 |
|---|---|---|---|
| Owner contract collisions | [Reference 계약 owner 지도](#reference-계약-owner-지도)의 strict owner. 용어 정의는 [용어집 참조](../reference/glossary.md) | Learn, Use, Build, Maintain, README, Reference 색인 route | 두 active 문서가 같은 strict contract를 다르게 정의합니다. 또는 non-owner가 owner로 연결하지 않고 full schema, DDL, transition table, gate matrix, enum table, validator table, template body, projection table, glossary definition을 담습니다. |
...
| User output vs agent context mixing | [Agent 통합 참조](../reference/agent-integration.md), [Surface Cookbook](../reference/surface-cookbook.md), [Projection과 Template 참조](../reference/projection-and-templates.md) | User Guide, Agent Guide, template, README route, Learn example, Operations summary | 사용자에게 보이는 summary, status card, projection, retrieved context, chat memory가 operational authority, Write Authorization, gate 충족, 사용자 판단, 근거, QA, 최종 수락, 잔여 위험 수락, close를 만족하는 것으로 쓰입니다. |
...
| Judgment terminology가 legacy alias를 current axis처럼 되살립니다. | Active owner docs는 `UserJudgment` / `user_judgment`, `harness.request_user_judgment`, `judgment_kind`, `presentation`, `display_label`을 우선합니다. Legacy alias는 compatibility-only로 남깁니다. | Schema/design decision |
...
| Sensitive-action Approval, final acceptance, residual-risk acceptance가 섞입니다. | 이름 붙은 민감 동작 permission, 최종 결과 수락, 보이는 남은 위험 수락을 예시와 routing에서 분리합니다. | Schema/design decision |
...
| Shared reader mental model과 first-reader authority overview | `learn/overview.md` |
| First-reader practical tour와 single work-loop story | `learn/one-task.md` |
