...
파일 경로, `doc_id` 값, API 메서드 이름, 스키마 이름, 필드 이름, enum 값, 오류 코드, 테이블 이름, validator ID, 코드 형태 문자열은 양쪽 언어에서 정확히 보존합니다.
...
| Later 후보 API와 스키마 자료 | [Later 후보 색인](../later/index.md#later-schema-candidates) |
...
| 에이전트 연결 동작, capability profile, 맥락 접점, 대체 의미 | [Agent 통합 참조](../reference/agent-integration.md) |
...
Active 문서는 later 후보, 진단, 운영, 내보내기, 풍부한 템플릿, 향후 적합성 실행기 자료를 활성 요구사항처럼 만들면 안 됩니다.

값, 메서드, 테이블, fixture 계열, 명령, 템플릿, 보안 보장은 담당 문서가 범위, 대체 동작, 증명 기대치와 함께 승격할 때만 active입니다. 참조 문서에 존재한다는 사실만으로 active 전달 범위가 커지지 않습니다.

profile-gated 값을 기본 active MVP 값처럼 나열하지 않습니다. 값이 특정 profile, 기능, 연결 모드, 향후 설정에서만 가능하다면 기본 active 값 목록에서 빼거나 담당 문서에서 해당 프로필 조건을 명확히 표시합니다.

later 후보를 active MVP 요구사항, 필수 기본값, 필수 점검, 필수 템플릿, 서버 의무처럼 설명하지 않습니다. 예시에 later 후보가 필요하면 later 자료라고 표시하고 규범 세부사항은 담당 문서나 Later 색인으로 보냅니다.

Later 자료는 `docs/*/later/*`, 명확히 표시된 later 후보 섹션, 또는 승격한 담당 문서에 둡니다. Active 스키마나 DDL 블록에 비활성 값이 들어 있으면 주변 문장으로 설명하지 말고 담당 문서 경계를 고칩니다.
...
편집 전에는 예전 경로, 예전 제목, 예전 앵커, 예전 제목 문자열, README 경로, 담당 문서 링크, 대응 언어 링크를 검색합니다. 편집 뒤에는 다시 검색합니다. Active 문서는 삭제된 파일, 오래된 앵커, 비활성 경로, 이전 안내, 예전 구조를 가리키면 안 됩니다.

삭제된 경로, 오래된 경로 묶음, 예전 구조 이름을 active 경로처럼 쓰지 않습니다. 현재 compact active 경로로 바꾸거나 오래된 경로 문구를 삭제합니다.

2차 요약보다 담당 문서 링크를 선호합니다. 삭제한 유지보수 페이지의 보관 사본을 만들지 않습니다.
...
| `move` | 내용이 다른 담당 문서나 문서군에 속할 때. | 의미를 그 담당 문서로 옮기거나 링크하고, 예전 사본은 제거합니다. |
...
- [ ] 정확한 식별자, 파일 경로, 스키마/API 이름, enum 값, 오류 코드, 테이블 이름, validator ID가 보존되었습니다.
...
- [ ] 링크, 앵커, README 경로, 대응 언어 링크가 해소됩니다.
- [ ] 삭제된 경로와 예전 구조 이름을 active 경로처럼 쓰지 않았습니다.
