...
이 섹션은 알려진 fixture 계약 일관성 영역에 대한 최종 문서 유지보수 기록입니다. 소스 수준 점검만 기록합니다. 실행 가능한 conformance fixture가 존재한다거나, conformance runner가 통과했다거나, generated conformance artifact가 존재한다거나, implementation readiness가 수락되었다고 말하지 않습니다. Readiness, handoff, documentation acceptance, implementation-planning readiness, coding acceptance는 Build owner 문서에서 maintainer가 계속 결정합니다.

| 점검한 fixture 관련 영역 | 최종 일관성 기록 |
|---|---|
| Public API/schema 기준값 | 확인했습니다. Active fixture draft는 `conformance-fixtures.md`의 owner guidance와 public owner인 `mvp-api.md`, `schema-core.md`, `core-model.md`, `storage.md`, `errors.md`를 통해 값을 맞춥니다. 알려진 active draft 영역에 fixture-only dialect는 기록되어 있지 않습니다. |
...
| Judgment display label | 확인했습니다. Judgment fixture는 `display_label`이나 localized label을 canonical state, storage identity, validator key, gate key, blocker key, compatibility input, close aggregation key로 취급하지 않습니다. |
| Structured fixture body validity | 알려진 정리 범위를 소스 수준에서 확인했습니다. Active `MVP-ACTIVE-display-label-not-canonical` draft는 backtick으로 시작하는 `display_label` forbidden-side-effect assertion을 따옴표로 감싸므로 YAML-like structured body에 fixture-only parser rule이 필요하지 않습니다. 이는 runner 실행도 아니고 readiness/status 변경도 아닙니다. |
