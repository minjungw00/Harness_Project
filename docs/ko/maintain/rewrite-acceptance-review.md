...
| 한국어 later-profile template 표현 표본 확인 | `docs/ko/reference/templates/later-profile/README.md`, `decision-packet.md`, `task.md`, `direct-result.md`, `run-summary.md`, `eval.md`를 표본으로 확인했습니다. 대부분의 렌더링 섹션 제목과 사용자 표시 라벨은 자연스러운 한국어이며, template ID, schema/API name, field name, ref 같은 exact identifier는 보존합니다. 다만 `Rendered example: ...`, `Display label`, `User Judgment`, `Close Summary`처럼 template example 안의 일부 사용자 표시 안내 라벨이나 표 라벨은 아직 영어식으로 남아 있습니다. 이는 `documentation drift` 성격의 한국어 표현 후속 정리입니다. Runtime conformance나 server-coding status가 아닙니다. |
| Core Model 판단 route와 display-depth 의미 | `docs/en/reference/core-model.md`와 `docs/ko/reference/core-model.md`를 수동으로 확인했습니다. Route 경계는 의미가 맞습니다. Route verb는 내부 owner-path metadata이고, broad approval은 사용자 표시 모델에 없습니다. Display depth는 presentation metadata이며, 사용자는 같은 다섯 가지 display type을 봅니다. Canonical-schema bullet은 같은 `user_judgment`, request/record action, `judgment_type`, `presentation`, `display_label`, compatibility/legacy 용어를 이름 붙입니다. 예전 Core Model canonical-schema wording mismatch는 이 대상 spot check 기준으로 해소되었습니다. |
...
- `AGENTS.md`와 `docs` 아래 Markdown에 대한 local relative link/anchor checker를 실행했습니다. 명시적 `<a id="...">` / `<a name="...">` anchor도 포함했습니다. 130개 Markdown 파일을 확인했고 unresolved relative link나 anchor는 보고되지 않았습니다.
- `docs/en`과 `docs/ko`의 active file-map을 확인했습니다. 양쪽 모두 Markdown 파일 64개였고 active file-map 차이는 보고되지 않았습니다.
- 같은 130개 Markdown 파일에 대해 fenced code block balance check를 실행했습니다. 닫히지 않은 fenced code block은 보고되지 않았습니다.
...
- 한국어 later-profile template 표현 표본 확인을 실행했습니다. 렌더링 heading/prose는 대부분 한국어로 정리되어 있지만, 위에 적은 일부 영어식 안내 라벨은 follow-up으로 남습니다.
- Core Model 영어/한국어 judgment schema wording 표본 확인을 실행했습니다. 현재 wording은 `user_judgment`, `harness.request_user_judgment`, `harness.record_user_judgment`, `judgment_type`, `presentation`, `display_label`, compatibility/legacy terms에서 의미가 맞습니다.
...
- 대상 표본 확인을 넘는 전체 한국어 later-profile template localization audit.
...
- 한국어 later-profile template example에는 일부 영어식 user-visible cue label과 table label에 대한 minor localization polish가 남아 있습니다. 이는 `documentation drift` 성격의 한국어 표현 후속 정리입니다. Runtime 또는 server-coding blocker가 아닙니다.
