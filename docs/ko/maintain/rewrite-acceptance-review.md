...
재설계된 문서는 별도의 구현 계획 준비 결정을 위해 maintainer에게 인계할 만큼 일관적입니다. 조건은 maintainer가 [MVP 계획: 문서 수락 상태](../build/mvp-plan.md#문서-수락-상태)를 의도적으로 갱신하고, [하네스 서버 구현 준비 조건](../build/mvp-plan.md#하네스-서버-구현-준비-조건)을 수락하거나 재분류하며, [MVP-1 사용자 작업 루프: 서버 코딩 전 필요한 구현 결정](../build/mvp-plan.md#서버-코딩-전-필요한-구현-결정)의 중앙 결정 기록을 수락하거나 단계 영향과 함께 미루는 것입니다.
...
- [내부 엔지니어링 점검](../build/mvp-plan.md#first-internal-smoke-target)
- [MVP-1 사용자 작업 루프](../build/mvp-plan.md#user-work-loop)
...
재설계는 broad workflow, dashboard, reporting, hosted-agent, evaluation-harness, generic MCP-wrapper prose를 제품 중심으로 두지 않습니다. 이런 아이디어는 active MVP framing에서 제거되었거나, non-goal 표현으로 줄었거나, [로드맵](../later/index.md#roadmap-candidates)과 later-profile 문서로 이동했습니다.
...
| Kernel Smoke | Engineering Checkpoint 아래의 좁은 future smoke-check authoring label입니다. Stage가 아니고 현재 runnable fixture set도 아닙니다. |
...
열린 구현 결정은 [MVP-1 사용자 작업 루프: 아직 열려 있는 구현 결정](../build/mvp-plan.md#아직-열려-있는-구현-결정)에 중앙화되어 있습니다.
...
| 영역 | 현재 리뷰 결과 |
|---|---|
| Later-profile template collapse | 상세 later-profile template body는 active documentation에서 폐기하고 [Later 후보 색인: Later template 후보](../later/index.md#later-template-candidates)에 요약했습니다. Full-format Decision Packet presentation은 later 후보이며 canonical MVP-1 judgment path가 아닙니다. |
| 한국어 later-template localization boundary | 한국어 상세 later-template body는 active documentation에서 폐기하고 [Later 후보 색인: Later template 후보](../later/index.md#later-template-candidates)에 요약했습니다. 나중에 template 후보가 승격되면 exact identifier를 보존하면서 자연스러운 한국어 prose를 사용해야 합니다. |
...
| 구현 준비 wording | `docs/en/build/mvp-plan.md`, `docs/ko/build/mvp-plan.md`, 그 문서들이 연결하는 MVP-1 decision log, Maintain guidance, 이 리뷰를 확인했습니다. 문서는 documentation redesign review, 대기 중인 documentation acceptance, 아직 수락되지 않은 implementation-planning readiness, 아직 수락되지 않은 server-coding decision, 시작하지 않은 runtime implementation, future runtime conformance를 구분합니다. |
| Future fixture catalog scope pressure | 상세 future fixture catalog prose는 active documentation에서 폐기하고 [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)에 요약했습니다. 남은 index row는 승격 전 후보 목록일 뿐이며 fixture body, runnable suite, active API/DDL, current conformance, implementation task가 아닙니다. |
...
| 정리 영역 | 최종 일관성 기록 |
|---|---|
| Undefined active schema type reference | 확인한 API/schema owner 경로에서 해소되었습니다. Active reference는 owner 없이 떠 있는 active type이 아니라 `mvp-api.md`, `schema-core.md`, `later/index.md#later-schema-candidates`, `errors.md`로 해소되어야 합니다. |
| Active/later schema separation | 알려진 정리 범위에서는 해소되었습니다. Active schema block은 active value만 두고, later/profile material은 `later/index.md#later-schema-candidates`, Later 문서, 또는 명확히 표시된 later/profile owner section으로 보냅니다. |
...
| 대표 conformance example | [적합성 참조](../reference/conformance.md)에 compact non-executable behavior example과 future fixture 형태 guidance가 있습니다. 이 예시는 fixture file, full YAML body, current pass/fail criterion, generated artifact, runtime conformance result가 아닙니다. |
...
## 현재 conformance 정리 메모

이 섹션은 현재 접은 conformance 구조를 기록합니다. Implementation-readiness approval, final documentation acceptance, runtime conformance, conformance runner pass, server-coding acceptance가 아닙니다. Readiness, handoff, documentation acceptance, implementation-planning readiness, coding acceptance는 Build owner 문서에서 maintainer가 결정합니다.

| Conformance 영역 | 현재 정리 기록 |
|---|---|
| Owner path | 활성 conformance owner는 영어와 한국어의 [적합성 참조](../reference/conformance.md)입니다. |
| Example 경계 | Active example은 compact representative behavior example일 뿐입니다. Full YAML body, fixture file, runner-loadable snippet, runnable suite, current pass/fail criterion, generated artifact, runtime conformance result가 아닙니다. |
| Assertion 경계 | Example은 response fact, Core state, storage row, 승격된 stable event, artifact ref, blocker, error, guarantee fact, forbidden side effect에 대한 structured assertion category를 가리킵니다. Prose-only matching은 여전히 충분하지 않습니다. |
| Future family 경계 | [Later 후보 색인: Future fixture families](../later/index.md#future-fixture-families)는 이름만 나열합니다. Scenario script, fixture body, active API payload example, suite requirement, implementation task를 담지 않습니다. |
| 한국어 prose | 한국어 conformance prose는 exact identifier를 보존하면서 `적합성`, `fixture 형태`, `주장 권한`, `대표 예시`, `실행 가능한 suite 아님`, `future fixture family` 같은 자연스러운 한국어 기술 표현을 사용해야 합니다. |
...
- Future fixture catalog scope-pressure spot check를 실행했습니다. 영어와 한국어 Later 후보 색인의 future fixture family row와 대상 검색 결과를 확인했고, catalog를 MVP-1 요구사항, runnable fixture set, active API/DDL, current conformance result, implementation checklist로 바꾸는 wording은 찾지 못했습니다.
...
- 모든 Start와 Use 문장의 full user-language audit.
...
- 모든 Start/Use 문장의 full manual user-language audit는 수행하지 않았습니다.
...
재설계된 문서는 maintainer implementation-planning review에 조건부 준비 상태입니다. 아직 accepted implementation-ready material이 아니며 server coding을 승인하지 않습니다. Maintainer는 다음 explicit readiness decision을 위해 이 리뷰를 [MVP 계획](../build/mvp-plan.md), [MVP-1 사용자 작업 루프](../build/mvp-plan.md#user-work-loop), [문서 점검표](documentation-checks.md)와 함께 사용해야 합니다.
