...
| 활성 structured conformance fixture 초안이 owner contract와 일치합니다. | `manual`, exact identifier와 structured body 후보 검색은 `scriptable`; 향후 실행은 `future-runtime-only`. | [Conformance Fixtures 참조](../reference/conformance-fixtures.md), [MVP API](../reference/api/mvp-api.md), [API Schema Core](../reference/api/schema-core.md), [API Errors](../reference/api/errors.md), [Core Model 참조](../reference/core-model.md), [Storage](../reference/storage.md). | Active draft의 request payload는 public API schema와 맞고, active value는 `schema-core.md`에서 오며, fixture shorthand와 later/profile-only value가 없습니다. 예상 Core/storage/blocker/error assertion은 owner와 맞아야 하고, assertion은 rendered prose가 아니라 structured outcome을 대상으로 합니다. | Draft가 작거나 작성자 note가 있어도 모든 value가 active owner로 해소되고 note가 fixture dialect를 만들지 않습니다. | Draft가 fixture-only payload branch, active body 안의 shorthand, active schema에 없는 enum, state로 쓰는 `display_label`, later/profile-only value, owner와 충돌하는 expected rows/state/blockers/errors를 사용합니다. |
...
- 점검 목록:
  ...
  - [ ] Fixture는 Core state, storage row, 승격된 stable event, artifact ref, blocker field, error code, forbidden side effect를 assert합니다. Rendered Markdown, template prose, status prose, generated summary를 conformance truth로 assert하지 않습니다.
