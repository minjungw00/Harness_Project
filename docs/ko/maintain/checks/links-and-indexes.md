...
- API 오류 링크는 [API 오류](../../reference/api/errors.md)를 문서 묶음 색인으로만 사용합니다. 공개 코드 의미, 우선순위, 응답 분기 경로, 닫기 준비 상태 blocker 처리 경로와 매핑, 기계 판독용 세부사항은 각각의 집중 API 담당 문서로 보냅니다.
...
- API 오류 코드 의미, 오류 우선순위, API 응답 분기 경로, 닫기 준비 상태 blocker 처리 경로, `ToolError.details` 대상이 서로 분리되어 있는지 확인합니다.

실패 조건:
- 용어, 용어집, 메타데이터, 경로 대상이 집중 담당 문서가 있는데도 넓은 색인을 가리킵니다.
- API 오류 문서 묶음 색인을 공개 코드 의미, 우선순위, 응답 분기 경로, 닫기 준비 상태 blocker 처리 경로, 기계 판독용 세부사항의 담당 문서처럼 씁니다.
