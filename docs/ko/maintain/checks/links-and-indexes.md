...
- `related_references`와 용어집의 `Related references`는 주변 맥락만 담는지 확인합니다. 이를 다른 `Primary owner`처럼 쓰면 안 됩니다.
...
- 용어집 요약 표는 한 `Primary owner`를 가리키고, 자세한 카드나 용어 지도는 다른 담당 문서를 가리킵니다.
...
- `related_references`나 `Related references` 항목을 두 번째 `Primary owner`처럼 취급합니다.
...
- 담당 대상이 바뀌면 같은 문서 작업 묶음에서 용어집 요약 표, 자세한 용어 카드, 용어 지도, `doc-index.yaml` 메타데이터를 함께 맞춥니다.
