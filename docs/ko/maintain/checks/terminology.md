...
실패 조건:
- 적용되는 담당 경로, 지원되는 계약, 지원되는 API 메서드, 유지되는 참조 문서를 "active owner path", "active contract", "active API method", "active reference document"처럼 부릅니다.
...
점검:
- 유지되는 참조 문서에서 더 이상 쓰지 않거나, 삭제되었거나, 지원되지 않는 개념 이름이 부정 예시로만 남아 있는지 검색합니다.
...
실패 조건:
- 참조 문서가 제거되었거나 지원되지 않는 개념 이름을 "지원하지 않는다"라고 말하기 위해서만 보존합니다.
