...
이 문서는 post-MVP automation 후보와 capability expansion을 모아 둡니다. 독자가 나중에 다룰 수 있는 일을 볼 수 있게 하되, 그것을 첫 구현 작업으로 오해하지 않게 하는 것이 목적입니다.

이 문서는 다음을 확인할 때 사용합니다.

- 어떤 아이디어가 MVP implementation contract 밖에 있는지
- 어떤 future capability가 promotion 전에 policy, fixture, fallback decision을 필요로 하는지
- 어떤 roadmap item이 owner가 명시적으로 scope를 부여하기 전까지 non-authoritative로 남아야 하는지

## 이 문서는 MVP 범위가 아닙니다

이 문서는 MVP implementation contract의 일부가 아닙니다.

Kernel invariant, public MCP schema, MVP implementation requirement, MVP에 required한 conformance는 이 문서가 소유하지 않습니다. MVP는 local kernel을 입증합니다. 즉 state, gate, artifact, verification, projection, reconcile, one reference surface가 안정적으로 동작하는지를 먼저 증명합니다. 아래 항목들은 이 기본 요소가 안정된 뒤에 이어갈 수 있는 follow-on입니다.
...
Kernel Smoke와 Agency-Hardened MVP는 모두 MVP delivery stage이지 roadmap scope가 아닙니다. 이 roadmap은 MVP owner docs가 요구하는 kernel authority, Decision Packet, residual-risk visibility, detached verification, Manual QA, recover/export, fixture-conformance behavior를 흡수하면 안 됩니다.

## 승격 규칙

Roadmap candidate는 future owner decision이 다음을 부여한 뒤에만 v1 또는 later scoped work가 될 수 있습니다.
...
MVP는 dashboard가 display할 record, projection, conformance fixture를 먼저 stabilize해야 하므로 이 항목은 later입니다. Future first version은 `state.sqlite`, artifact ref, projection job status 위의 read-only view여야 합니다.
...
Context Index는 future decision이 owner, freshness and staleness rule, privacy/redaction behavior, connector capability expectation, fixture coverage, retrieved context와 canonical state를 구분하는 display rule을 assign할 때만 v1 work가 되어야 합니다.

### Local Derived Metrics

Local Derived Metrics는 `state.sqlite.task_events`, run, validator result, projection job, reconcile item에서 diagnostic rate, count, duration, guard-trigger summary를 derive할 수 있습니다.

Metric은 authority가 아니라 derived value이므로 later입니다. User가 process bottleneck, reporting gap, 반복되는 operational pattern을 찾는 데 도움을 줄 수 있지만 diagnostic-only입니다. Metric readout은 state mutate, gate satisfaction, write authorization, approval grant, evidence creation, projection enqueue 또는 refresh, projection freshness change, close readiness 또는 implementation readiness 변경, verification perform 또는 record, QA recording, QA 또는 verification waiver, residual risk acceptance, result acceptance, assurance upgrade, Task close를 하면 안 됩니다.
...
이 metric들은 future decision이 owner, fixture coverage, retention behavior, 필요한 경우 privacy/redaction behavior, user-facing interpretation rule을 assign할 때만 v1 work가 되어야 합니다. 그 경우에도 metric value는 derived로 남으며 state change는 여전히 normal Core owner path를 거쳐야 합니다.
