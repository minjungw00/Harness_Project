...
| `CONN-guard-display-matches-capability` | `status` or `prepare_write` | Guard display는 connected profile의 actual `guarantee_level`과 limitation notes를 report합니다. Cooperative guard는 prevention을 claim하지 않습니다. Detective guard에는 changed-path/log/artifact validation assertion이 필요합니다. Proven pre-tool blocking path가 없는 한 preventive guard는 MVP requirement가 아닙니다. |
