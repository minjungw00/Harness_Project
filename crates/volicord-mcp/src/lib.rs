...
//! invocation facts from adapter context, and hands execution to `volicord-core`.
...
mod tests {
    ...
    use serde_json::json;
    use volicord_core::{AdapterSessionBinding, CoreBoundary, CoreService, InvocationContext};
    use volicord_store::{
        agent_integrations::{
...
"#
    ...
    fn adapter_and_direct_core_status_have_equivalent_response_meaning(
        ...
        request.envelope.task_id = Some(volicord_types::TaskId::new(&task_id)).into();
        let direct = harness
            .core()
    ...
    fn update_scope_request(
        ...
        UpdateScopeRequest {
            ...
            task_id: volicord_types::TaskId::new(task_id),
    ...
    fn envelope(
        ...
        ToolEnvelope {
            ...
            task_id: task_id.map(volicord_types::TaskId::new).into(),
            actor_kind: ActorKind::Agent,
            surface_id: SurfaceId::new(SURFACE_ID),
            request_id: volicord_types::RequestId::new(request_id),
            idempotency_key: idempotency_key
                .map(volicord_types::IdempotencyKey::new)
                .into(),
