...
런타임 구성 요소, 호스트 통합 기록, 지원되는 Agent Connection, 문서 경로를 포함합니다.
...
| Agent Connection | `connection_id`로 선택되는 지속되는 Volicord 통합 기록입니다. | [에이전트 통합](docs/ko/reference/agent-integration.md) |
...
| 옵션 | 범주 | 이 예시에서의 의미 |
|---|---|---|
| `--host` | 필수 | 호스트 통합을 식별합니다. 값은 `codex`, `claude-code`, `generic`입니다. |
...
프로젝트를 기본값으로 사용하고, CLI는 `connection_id`에서
`volicord-conn-codex-team` 같은 안정적인 호스트 MCP 서버 이름을 파생합니다.
...
검증이 `status: complete`를 보고하고 선택된 managed host configuration state이
...
|---|---|---|
| `volicord agent status` | 레지스트리 상태, 허용된 프로젝트, managed host configuration state 인벤토리, 마지막 검증 상태, 지침 상태입니다. | 호스트가 MCP 서버를 로드했거나 노출했음을 증명하지 않습니다. |
