...
런타임 구성 요소, 호스트 통합 기록, 지원되는 접점, 문서 경로를 포함합니다.
...
| `int-codex-team` | `--integration-id` | 예시 Codex `integration_id`입니다. |
| `int-claude-acme` | `--integration-id` | 예시 Claude Code `integration_id`입니다. |
| `int-generic-acme` | `--integration-id` | 예시 generic export `integration_id`입니다. |
...
통합에 묶인 `volicord-mcp --integration <integration_id>` 프로세스 사용법을
보여야 합니다.
...
이 빠른 참조는 이 README의 `volicord agent install` 예시에 나타나는 옵션만
다룹니다. 전체 옵션 목록, 생략 규칙, 예외는 `volicord agent install --help`와
...
| `--integration-id` | 재현성을 위해 고정한 선택 사항 | 이후 관리 명령과 생성되는 호스트 구성이 같은 ID를 쓰도록 예측 가능한 통합 식별자를 고정합니다. 생략하면 CLI가 안정적인 값을 파생합니다. |
...
선택 사항인 `--integration-id`, `--runtime-home`, 명시적 `--mcp-command`,
