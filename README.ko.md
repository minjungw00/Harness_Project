...
사용자와 에이전트가 작업의 중요한 부분, 즉 범위, 사용자 소유 판단, 증거,
검증 기준, 최종 수락, 잔여 위험 수락, 닫기 준비 상태를 계속 볼 수 있게
돕습니다.

이 README는 현재 저장소를 처음 쓰는 독자를 위한 경로입니다. 하네스가
무엇인지, 로컬 실행 파일과 호스트 설정이 어떻게 맞물리는지, 실행 파일을
빌드하고 확인하는 방법, 지원되는 호스트 경로를 고르는 방법, 설정 뒤 첫
하네스 지원 상호작용을 시도하는 방법을 설명합니다.
...
- [개요](#overview)
- [하네스가 필요한 이유](#why-harness-exists)
- [구체적인 시나리오](#scenario)
...
하네스는 AI 에이전트가 변경을 살펴보고, 계획하고, 작성하고, 확인하고,
요약하는 로컬 제품 작업을 위한 도구입니다. 하네스의 역할은 모든 에이전트
행동을 자동화하는 것이 아닙니다. 작업이 진행되는 동안 권한의 근거를 보이는
상태로 유지하는 것입니다.

실제로 하네스는 에이전트가 다음과 같은 질문을 계속 묻고 답하게 돕습니다.
...
하네스 자체는 로컬 기준 기록이 아닙니다. Core가 하네스 상태를 위한 로컬 기준
기록입니다. 하네스는 그 기록을 둘러싼 더 넓은 제품이자 시스템이며, 로컬
...
하네스는 흐려지면 안 되는 부분만 늦추기 위해 존재합니다. 에이전트는 여전히
...
하네스 지원 작업 흐름은 다음 사실들을 서로 구분해 보이게 유지해야 합니다.
...
`harness-mcp`는 호스트가 시작하는 로컬 stdio 자식 프로세스입니다. TCP, HTTP,
소켓 또는 다른 네트워크 리스너가 아닙니다. `harness-mcp` 프로세스 하나는
`--integration <integration_id>`로 하나의 Agent Integration Profile에 묶입니다.
프로젝트 선택은 공개 하네스 도구 호출마다 이루어집니다.

`harness`는 관리 CLI입니다. 설정 상태를 만들고, 호스트 구성을 설치하거나
내보내고, 상태를 살펴보고, 검증을 새로 고치는 데 사용합니다. 장기 실행
서버가 아니며 공개 하네스 API 메서드 표면도 아닙니다.
...
|---|---|---|
| 하네스 | AI 지원 제품 작업을 위한 로컬 작업 권한 제품이자 시스템입니다. | [시작하기 개요](docs/ko/getting-started/overview.md) |
| Core | 하네스 상태를 위한 로컬 기준 기록입니다. | [Core 모델](docs/ko/reference/core-model.md) |
| `Harness Server` | 이 저장소가 유지하는 서버 구현 집합입니다. | [런타임 경계](docs/ko/reference/runtime-boundaries.md) |
| `harness` | `harness-cli` 패키지의 관리 CLI 실행 파일입니다. | [관리 CLI](docs/ko/reference/admin-cli.md) |
| `harness-mcp` | AI 호스트가 시작하는 로컬 MCP stdio 실행 파일입니다. | [MCP 전송](docs/ko/reference/mcp-transport.md) |
| `Harness Runtime Home` | 하네스 기록과 운영 데이터를 위한 로컬 런타임 저장소입니다. | [런타임 경계](docs/ko/reference/runtime-boundaries.md) |
| `Product Repository` | 사용자의 프로젝트 작업 공간과 제품 파일 경계입니다. | [런타임 경계](docs/ko/reference/runtime-boundaries.md) |
| 에이전트 호스트 | `harness-mcp`를 시작할 수 있는 Codex, Claude Code, 또는 사용자 관리 MCP 호스트입니다. | [에이전트 통합](docs/ko/reference/agent-integration.md) |
| Agent Integration Profile | `integration_id`로 선택되는 지속되는 하네스 통합 기록입니다. | [에이전트 통합](docs/ko/reference/agent-integration.md) |
| Host Installation | 호스트 설정과 마지막 검증 상태를 위한 하네스 관리 인벤토리입니다. | [에이전트 통합](docs/ko/reference/agent-integration.md) |
...
- Cargo Rust 워크스페이스
- `harness-cli`의 `harness` 관리 실행 파일
- `harness-mcp`의 로컬 stdio `harness-mcp` 실행 파일
...
| 영역 | 현재 기준 |
|---|---|
| 실행 파일 원천 | 이 소스 체크아웃에서 빌드하거나, `harness`와 `harness-mcp`가 모두 들어 있는 이미 사용 가능한 `Harness Server` 설치 디렉터리를 사용합니다. |
| 직접 호스트 설정 | Codex와 Claude Code에는 지원되는 직접 `harness agent install` 경로가 있습니다. |
| 일반 MCP 호스트 | Generic export는 사용자 관리 호스트를 위한 구성을 렌더링합니다. 하네스는 그 호스트에 직접 설치하거나 호스트가 그것을 로드했음을 증명하지 않습니다. |
| MCP 전송 | 기준 프로세스는 로컬 stdio입니다. 호스트가 `harness-mcp`를 자식 프로세스로 시작합니다. |
...
남습니다. 하네스가 외부 호스트가 내보낸 구성을 로드했는지 관찰할 수 없기
때문입니다.
...
| 실행 파일 배치 | 선택한 디렉터리 하나에 실행 가능한 `harness`와 `harness-mcp`가 모두 들어 있어야 합니다. |
| Runtime Home | 선택한 사용자와 이후 호스트 프로세스가 읽고 쓸 수 있는 로컬 `Harness Runtime Home`을 선택합니다. |
...
| 예시 값 | 의미 |
|---|---|
| `HARNESS_BIN="/absolute/path/to/selected/bin"` | 두 실행 파일이 모두 들어 있는 디렉터리 하나를 가리키는 셸 편의 변수입니다. |
| `"$HARNESS_BIN/harness"` | 관리 CLI 호출입니다. |
| `"$HARNESS_BIN/harness-mcp"` | 사용자/로컬 범위 호스트 구성과 generic export에 쓰는 절대 `harness-mcp` 경로입니다. |
| `/Users/alex/.harness` | 예시 `Harness Runtime Home`입니다. |
...
`HARNESS_BIN`은 이 예시에서만 쓰는 셸 변수입니다. 하네스는 이것을 구성으로
읽지 않습니다. 새 셸마다 다시 설정하거나 절대 경로를 직접 사용하세요.

`HARNESS_HOME`은 다릅니다. 관리 명령과, 기본 home에서 파생되는 Runtime Home이
의도한 값이 아닐 때 이후 `harness-mcp` 프로세스 시작에 쓰는 실제 Runtime Home
선택 입력입니다.

`Harness Runtime Home`과 `Product Repository`는 파일시스템 경로를 실제 위치로
...
`Harness Server` 설치 디렉터리가 이미 있을 때는 경로 B를 사용합니다.
...
예시 경로를 `harness`와 `harness-mcp`가 모두 들어 있는 절대 디렉터리로
바꿉니다.

### 선택한 디렉터리 확인

`HARNESS_BIN`이 설정된 같은 셸에서 실행합니다.
...
버전 명령은 `harness <version>`과 `harness-mcp <version>`을 출력해야 합니다.
도움말 명령은 `harness agent` 명령군과 통합에 묶인 `harness-mcp
...
| Generic `export` 범위 | 다른 MCP 호스트를 사용하고 그 구성을 직접 관리할 때. | 하네스는 구성을 렌더링할 수 있습니다. 외부 호스트가 그것을 로드했는지는 증명할 수 없습니다. |
...
개인 Codex 구성 하나가 Codex 프로젝트들에서 같은 하네스 통합을 로드해야 할
때 이 경로를 사용합니다.

실행하기 전에:

- `HARNESS_BIN`은 확인된 실행 파일 디렉터리를 가리킵니다.
- Codex는 `CODEX_HOME` 또는 `HOME`을 통해 사용자 `config.toml`을 읽을 수
  있습니다.
- 호환성 확인을 위해 `codex` 실행 파일을 관리 명령 `PATH`에서 사용할 수
  있습니다.
- `/Users/alex/.harness`와 `/work/acme-api`는 분리된 경로입니다.
...
`--server-name`이 생략되었으므로 CLI는 `integration_id`에서
`harness-int-codex-team` 같은 안정적인 호스트 MCP 서버 이름을 파생하고 결과에
보고합니다.
...
설정은 `/Users/alex/.harness` 아래의 Runtime Home 기록과 Codex 사용자 MCP
...
- `HARNESS_BIN`은 확인된 실행 파일 디렉터리를 가리킵니다.
- `/Users/alex/.harness`와 `/work/acme-api`는 분리된 경로입니다.
- 이후 Claude Code 프로세스가 사용하는 `PATH`에서 `harness-mcp`를 사용할 수
  있습니다.
- Claude Code가 기본적으로 `/Users/alex/.harness`를 쓰지 않는다면, 이후
  Claude Code 실행 환경은 `HARNESS_HOME=/Users/alex/.harness`를 제공해야
  합니다.
...
생성된 `.mcp.json` 항목은 의도적으로 이식 가능한 명령 `harness-mcp`를 저장하고
개인 `HARNESS_HOME`을 넣지 않습니다. 위의 인라인 `HARNESS_HOME`과 `PATH` 값은
관리 명령에만 적용됩니다. 나중에 Claude Code가 서버를 시작할 때에는 Claude
Code 자체 환경이 `harness-mcp`를 찾을 수 있어야 하고, 기본값이 다르다면 의도한
...
Generic export는 하네스가 직접 설치하지 않는 호스트에만 사용합니다. 이 경로는
...
그 구성을 외부 호스트 자체의 지침에 따라 적용합니다. 하네스는 그것을 직접
...
|---|---|---|
| `harness agent status` | 레지스트리 상태, 허용된 프로젝트, Host Installation 인벤토리, 마지막 검증 상태, 지침 상태입니다. | 호스트가 MCP 서버를 로드했거나 노출했음을 증명하지 않습니다. |
| `harness agent verify` | 선택된 Host Installation에 대한 관리 검증입니다. 관찰 가능한 경우 시작 점검과 호스트별 gate를 포함합니다. | 호스트 소유 trust 또는 승인 결정을 사용자 대신 하지 않습니다. |
| `harness-mcp --check --integration <integration_id>` | 로컬 `harness-mcp` 프로세스와 선택된 통합의 시작 검증입니다. | 완전한 호스트 통합이 아니며 Codex, Claude Code, generic 호스트가 그것을 로드했음을 증명하지 않습니다. |
...
성공한 MCP 시작은 호스트가 하네스를 로드했거나 계속 일관되게 사용할 것임을
증명하지 않습니다. 도구 발견도 AI 모델이 모든 요청에 대해 하네스를 선택한다는
보장이 아닙니다.
...
|---|---|---|---|
| `Harness Server` 소스 또는 설치 | `Harness Server` 유지보수자 또는 설치자 | 소스 체크아웃, 설치된 실행 파일, 빌드 출력, 문서, 테스트, 필수 실행 파일 리소스입니다. | 소스 빌드는 Cargo 출력을 `target/` 아래에 씁니다. |
| `Harness Runtime Home` | 로컬 하네스 운영자 | 하네스 레지스트리, 통합 상태, 프로젝트 상태, 런타임 기록, 저장소 담당 문서가 정의한 런타임 데이터입니다. | 에이전트 설정은 그곳에 하네스 기록을 만들거나 재사용합니다. |
...
| Codex 또는 Claude Code 구성 | 호스트 운영자 | `harness-mcp --integration <integration_id>`를 시작하는 호스트 소유 설정입니다. | 직접 설정은 선택한 호스트와 범위가 요구하는 위치에 관리되는 호스트 구성을 쓸 수 있습니다. |
| Generic export 대상 | 사용자 관리 호스트 운영자 | 다른 호스트를 위한 내보낸 MCP 구성입니다. | `/tmp/harness-mcp-export` 같은 사용자가 선택한 내보내기 파일 또는 디렉터리입니다. |

하네스 런타임 데이터베이스, 런타임 기록, 생성 기록, 로그, Projection, QA 결과,
...
| 도움말 또는 버전 명령이 실패합니다. | 실제로 실행 가능한 `harness`와 `harness-mcp`가 들어 있는 디렉터리를 선택합니다. | [에이전트 호스트 문제 해결](docs/ko/guides/agent-host-troubleshooting.md) |
...
| 결과가 `action_required`입니다. | 이름 붙은 호스트 소유 trust, approval, reload, restart, OAuth, 또는 실행 파일 가용성 동작을 완료한 뒤 `harness agent verify`를 실행합니다. | [에이전트 호스트 문제 해결](docs/ko/guides/agent-host-troubleshooting.md) |
...
| 프로젝트 범위 호스트가 `harness-mcp`를 찾지 못합니다. | 프로젝트 파일은 이식 가능한 형태로 유지하고 이후 호스트 프로세스의 `PATH`를 고칩니다. | [에이전트 호스트 문제 해결](docs/ko/guides/agent-host-troubleshooting.md) |
| Generic export가 계속 `action_required`로 남습니다. | 내보낸 구성을 외부 호스트에 직접 적용합니다. 하네스는 그 호스트의 로드 상태를 관찰할 수 없습니다. | [에이전트 호스트 설정](docs/ko/guides/agent-host-setup.md) |
