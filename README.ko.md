...
Volicord는 사용자와 에이전트가 함께 작업할 때 로컬 권한 제어 평면 역할을
합니다. 범위, 사용자 소유 판단, 증거, 검증 기준, 최종 수락, 잔여 위험 수락,
닫기 준비 상태처럼 작업의 중요한 부분을 계속 볼 수 있게 합니다.
...
Volicord는 AI 지원 제품 작업을 위한 로컬 권한 제어 평면입니다. 코딩 에이전트,
워크플로 엔진, 테스트 러너가 아닙니다. 사용자, 호스트, 에이전트가 작업을
진행하는 동안 권한의 근거를 보이는 상태로 유지합니다.
...
런타임 구성 요소, Agent Connection 기록, managed host configuration state, 문서
경로를 포함합니다.
...
Volicord 지원 상호작용은 다음 사실들을 서로 구분해 보이게 유지해야 합니다.
...
현재 로컬 설정은 세 가지 흐름과 두 파일시스템 경계를 분리해서 유지합니다.

```text
MCP 런타임 흐름
에이전트 호스트
  Codex, Claude Code, 또는 사용자 관리 MCP 호스트
        |
        | 설정된 Agent Connection에 대한 stdio 자식 프로세스를 시작
        v
volicord-mcp --connection <connection_id>
        |
        | Core를 통해 공개 Volicord 도구 호출 처리
        v
Volicord Runtime Home                    Product Repository
  /Users/alex/.volicord                    /work/acme-api
  (Volicord 런타임 상태)                  (제품 파일, 별도 경로)

관리 CLI 흐름
운영자 터미널 -> volicord -> Runtime Home 레지스트리와 호스트 설정

User Channel 권한 흐름
사용자 터미널 -> volicord user -> User Channel -> Core/Runtime Home 기록
```

`volicord-mcp`는 호스트가 시작하는 로컬 stdio 자식 프로세스입니다. TCP, HTTP,
소켓 또는 다른 네트워크 리스너가 아닙니다. `volicord-mcp` 프로세스 하나는
`--connection <connection_id>`로 하나의 Agent Connection에 묶입니다.
프로젝트 선택은 공개 Volicord 도구 호출마다 이루어집니다.

`volicord`는 관리 CLI입니다. 설정 상태를 만들고, 호스트 구성을 설치하거나
내보내고, 상태를 살펴보고, 검증을 새로 고치는 데 사용합니다. 장기 실행
서버가 아니며 공개 Volicord API 메서드 집합도 아닙니다. `volicord user` 하위
명령은 권한을 지니는 사용자 판단을 위한 User Channel 경로입니다.

`Product Repository`는 `Volicord Runtime Home` 옆에 표시되어 있습니다. 별도의
제품 파일 경계이며 Runtime Home 안의 하위 디렉터리나 런타임 상태가 아니기
때문입니다.
...
|---|---|---|
| Volicord | AI 지원 제품 작업을 위한 로컬 작업 권한 제품이자 시스템이며 권한 제어 평면입니다. | [시작하기 개요](docs/ko/getting-started/overview.md) |
...
| 직접 호스트 설정 | Codex와 Claude Code에는 지원되는 직접 `volicord agent connect` 경로가 있습니다. |
...
| `"$VOLICORD_BIN/volicord-mcp"` | 예시가 `--mcp-command`를 제공할 때 쓰는 확인된 절대 실행 파일 경로입니다. |
| `VOLICORD_HOME=/Users/alex/.volicord` | `project` 범위 관리 명령이 기본값이 아닌 Runtime Home을 사용해야 할 때 쓰는 환경 변수 할당입니다. CLI 옵션이 아닙니다. |
| `PATH="$VOLICORD_BIN:$PATH"` | `project` 범위 관리 명령이 선택한 실행 파일을 찾게 해 주는 환경 변수 할당입니다. CLI 옵션이 아닙니다. |
| `/Users/alex/.volicord` | 예시 `Volicord Runtime Home` 경로입니다. |
| `/work/acme-api` | 예시 `Product Repository` 경로입니다. |

### 아래에서 쓰는 CLI 인자 값

| 예시 값 | 사용하는 옵션 | 의미 |
|---|---|---|
| `acme-api` | `--project-id` | 운영자가 선택하거나 재사용하는 안정적인 논리 프로젝트 식별자입니다. 저장소 디렉터리 이름과 같을 필요는 없습니다. |
| `/work/acme-api` | `--repo-root` | 선택된 프로젝트의 `Product Repository` 경로입니다. |
| `conn-codex-team` | `--connection-id` | 예시 Codex `connection_id`입니다. |
| `conn-claude-acme` | `--connection-id` | 예시 Claude Code `connection_id`입니다. |
| `conn-generic-acme` | `--connection-id` | 예시 generic export `connection_id`입니다. |
| `/Users/alex/.volicord` | `--runtime-home` | 사용자 범위와 export 예시에서 직접 제공하는 Runtime Home 경로입니다. |
| `"$VOLICORD_BIN/volicord-mcp"` | `--mcp-command` | 사용자 범위와 export 예시에서 제공하는 확인된 절대 `volicord-mcp` 경로입니다. |
| `/tmp/volicord-mcp-export` | `--export-dir` | 생성되는 generic export 파일을 둘 디렉터리입니다. |
...
`volicord agent --help`는 `agent` 명령군을 나열해야 합니다. `volicord agent
install --help`는 설치별 인자와 필수성을 설명해야 합니다. `volicord-mcp --help`는
connection에 묶인 `volicord-mcp --connection <connection_id>` 프로세스 사용법을
보여야 합니다.

두 실행 파일이 같은 선택 디렉터리에서 실행된 뒤에만 계속합니다. 이것은 선택한
실행 파일을 호스트 설정에 사용할 수 있음을 확인합니다. Runtime Home을 만들거나,
...
이 빠른 참조는 이 README의 `volicord agent connect` 예시에 나타나는 옵션만
다룹니다. 전체 옵션 목록, 생략 규칙, 예외는 `volicord agent connect --help`와
[관리 CLI](docs/ko/reference/admin-cli.md#volicord-agent-install)를 사용합니다.

| 옵션 | 범주 | 이 예시에서의 의미 |
|---|---|---|
| `--host` | 필수 | 호스트 어댑터를 식별합니다. 값은 `codex`, `claude-code`, `generic`입니다. |
| `--scope` | 필수 | 대상 설정 범위 또는 export 모드를 식별합니다. 값은 `user`, `project`, `export`입니다. |
| `--project-id` | 이 새 프로젝트 예시에서는 필수 | 운영자가 선택하거나 재사용하는 안정적인 논리 프로젝트 식별자를 제공합니다. 이 예시들은 `/work/acme-api`를 프로젝트 등록으로 도입하므로 `acme-api`를 전달합니다. 이 ID는 디렉터리 이름과 같을 필요가 없습니다. |
| `--repo-root` | 이 새 프로젝트 예시에서는 필수 | 선택된 프로젝트의 `Product Repository` 경로를 제공합니다. `Volicord Runtime Home`과 구분해서 둡니다. |
| `--connection-id` | 재현성을 위해 고정한 선택 사항 | 이후 관리 명령과 생성되는 호스트 구성이 같은 ID를 쓰도록 예측 가능한 connection 식별자를 고정합니다. 생략하면 CLI가 안정적인 값을 파생합니다. |
| `--runtime-home` | 재현성을 위해 고정한 선택 사항 | 관리 명령이 사용할 `Volicord Runtime Home`을 선택하고, 호스트 범위가 허용하는 경우 생성되는 호스트 환경에도 반영합니다. 예시들은 일반 Runtime Home 해석에 기대지 않아야 할 때 `/Users/alex/.volicord`를 고정합니다. |
| `--mcp-command` | 재현성을 위해 고정한 선택 사항 | `user`와 `export` 예시에서는 확인된 절대 `volicord-mcp` 실행 파일을 고정합니다. `project` 예시는 생략 시 이식 가능한 `volicord-mcp` 명령을 사용하므로 이 옵션을 생략합니다. |
| `--dry-run` | 선택적 미리보기 제어 | 실제 쓰기를 수행하지 않고 설치 계획을 미리 봅니다. 필수 설치 입력이 아니며 `--allow-repository-write`를 요구하지 않습니다. |
| `--output` | 선택적 출력 형식 | 출력 형식을 선택합니다. 기본값은 text이고, dry-run 예시는 미리보기를 검사하기 쉽도록 JSON을 요청합니다. |
| `--allow-repository-write` | 조건부 필수 | `/work/acme-api/.mcp.json`에 대한 실제 `project` 범위 쓰기를 명시적으로 권한 부여합니다. 대응 dry-run에는 필요하지 않습니다. |
| `--export-dir` | 재현성을 위해 고정한 선택 사항 | `--export-path`를 제공하지 않을 때 generic으로 내보낸 MCP 구성을 둘 디렉터리를 선택합니다. 예시는 대상 위치로 `/tmp/volicord-mcp-export`를 고정합니다. |

<a id="codex"></a>
## Codex 사용자 범위 설정

개인 Codex 구성 하나가 Codex 프로젝트들에서 같은 Volicord Agent Connection을 로드해야 할
때 이 경로를 사용합니다.
...
`--server-name`이 생략되었으므로 CLI는 기본 호스트 MCP 서버 이름 `volicord`를
사용합니다. 새 Agent Connection은 선택한 프로젝트 `acme-api`를 명시적으로
연결합니다.
...
항목을 쓸 수 있습니다. 이 사용자 범위 설정은 `/work/acme-api`에는 쓰지 않습니다.
...
- 아래의 선택적 dry-run은 저장소 쓰기 권한 부여 없이 계획된 쓰기를 미리
  봅니다. 적용 명령은 실제 쓰기이므로 명시적인 저장소 쓰기 권한 부여를
  포함합니다.
...
검증이 `verification_status: complete`를 보고하면 이 경로는 완료된 것입니다.
...
모든 설치에는 `--host`와 `--scope`가 필요합니다. 이 Generic export 예시는
`/work/acme-api`를 새 프로젝트 등록으로도 도입하므로, 표시된 명령에는
`--project-id acme-api`와 `--repo-root /work/acme-api`도 필요합니다. Export 대상과
실행 파일 경로 선택은 별개의 선택입니다. `--export-dir`는 렌더링된 구성을 쓸
위치를 선택하고, `--mcp-command`는 그 export에 들어갈 명령 경로를 선택합니다.
선택 사항인 `--connection-id`, `--runtime-home`, 명시적 `--mcp-command`,
`--export-dir`는 `connection_id`, Runtime Home 환경, 명령 경로, 대상 위치를 재현
가능하게 하려고 유지합니다. 전체 생략 규칙은
[관리 CLI](docs/ko/reference/admin-cli.md#volicord-agent-install)에 남아 있습니다.
...
|---|---|---|
| `volicord agent status` | Agent Connection 레지스트리 상태, 연결 프로젝트, managed host configuration state 인벤토리, 마지막 검증 상태입니다. | 호스트가 MCP 서버를 로드했거나 노출했음을 증명하지 않습니다. |
| `volicord agent verify` | 선택된 managed host configuration state에 대한 관리 검증입니다. 관찰 가능한 경우 시작 점검과 호스트별 gate를 포함합니다. | 호스트 소유 trust 또는 승인 결정을 사용자 대신 하지 않습니다. |
| `volicord-mcp --check --connection <connection_id>` | 로컬 `volicord-mcp` 프로세스와 선택된 Agent Connection의 시작 검증입니다. | 완전한 호스트 통합이 아니며 Codex, Claude Code, generic 호스트가 그것을 로드했음을 증명하지 않습니다. |
...
Agent Connection에 둘 이상의 프로젝트가 연결되어 있고 에이전트가 어느 프로젝트를
사용할지 확신하지 못한다면, 폴더 이름, 호스트 라벨, 채팅 기억으로 추측하지 말고
연결 프로젝트를 나열한 뒤 명시적 프로젝트 선택으로 다시 시도해야 합니다.
...
| `Volicord Runtime Home` | 로컬 Volicord 운영자 | Volicord 레지스트리 상태, Agent Connection 상태, 프로젝트 상태, 런타임 기록, 저장소 담당 문서가 정의한 런타임 데이터입니다. | 에이전트 설정은 그곳에 Volicord 기록을 만들거나 재사용합니다. |
| `Product Repository` | 제품 프로젝트 소유자 | 제품 파일과 명시적으로 선택된 프로젝트 범위 호스트 구성 파일입니다. | `.codex/config.toml` 또는 `.mcp.json` 같은 명시적으로 선택되고 권한이 부여된 호스트 구성 파일만 씁니다. |
| Codex 또는 Claude Code 구성 | 호스트 운영자 | `volicord-mcp --connection <connection_id>`를 시작하는 호스트 소유 설정입니다. | 직접 설정은 선택한 호스트와 범위가 요구하는 위치에 관리되는 호스트 구성을 쓸 수 있습니다. |
